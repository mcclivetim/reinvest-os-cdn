...
`;var I="ws-crm-claim-release-pill",tt=o=>{let e=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},It=({contactId:o,mode:e})=>{let t=document.createElement("button");t.id=I,t.type="button",t.dataset.contactId=o,t.dataset.mode=e;let r=e==="claim"?"emerald":"amber",n=e==="claim"?l.emerald:l.amber;return t.style.cssText=Z(r),t.innerHTML=`
  ...
  `,t.appendChild(r),document.body.appendChild(t);let n=()=>t.remove(),s=()=>{n(),e()};t.addEventListener("click",a=>{a.target===t&&s()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",s),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),o()});let i=a=>{a.key==="Escape"&&(document.removeEventListener("keydown",i),s())};document.addEventListener("keydown",i)},at=o=>{let e=document.createElement("div");e.id=R,e.dataset.contactId=o,e.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
...
/* KILL TRANSITIONS on dropdown options. Naive UI applies a
   transition on background-color (~0.3s) that fades through white
   between states. Result: moving the pointer from row A to row B
   makes row A flash white briefly while it transitions out of its
   hover state \u2014 Tim's actual diagnosis after a few rounds of looking
   in the wrong places.
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var Ct=()=>{p.info(`Loaded v${N} (built ${$})`),lt(),_t(),X(),J(o=>{p.debug("Page handler firing for:",o),St(),et(),st()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ct):Ct();})();
