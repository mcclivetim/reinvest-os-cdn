...
`,I=`
  ...
  flex-shrink: 0;
`;var D="ws-crm-claim-release-pill",ie=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Re=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=D,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?c.emerald:c.amber;return e.style.cssText=ae(r),e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,We.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
    ...
    `,s.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=c.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{h.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let i=e.firstElementChild,n=e.children[1],l=e.children[2];i&&(i.style.background=Z[a.color]),n&&(n.textContent=a.label),r.style.display="none";let p=S()?.locationId,m=Ue();if(!p){h.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=c.amber);return}if(a.color==="green"&&!m){h.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=c.amber);return}let d=l?.textContent??"\u25BE",u=async v=>{l&&(l.textContent="\u2026"),e.style.opacity="0.75";try{await $e({contactId:o,locationId:p,status:a.label,userId:m,deadReason:v}),l&&(l.textContent="\u2713"),setTimeout(()=>{l&&(l.textContent=d),e.style.opacity="1"},900)}catch(_){h.warn("Call Status webhook failed:",_),l&&(l.textContent="\u2717"),n&&(n.style.color=c.amber),e.style.opacity="1",setTimeout(()=>{l&&(l.textContent=d),n&&(n.style.color=c.bone)},1800)}};if(a.value==="dead-deal"){let v=n?.textContent??"";Je(_=>{u(_)},()=>{n&&(n.textContent=v)});return}if(a.value==="appointment-booked"){let v=n?.textContent??"";try{(await qe({contactId:o,tenantId:p})).ok?await u():Ze(()=>{u()},()=>{n&&(n.textContent=v)})}catch(_){h.warn("Appointment sanity check failed; firing anyway:",_),await u()}return}await u()}),r.appendChild(s)}),e.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{t.contains(a.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},Xe=o=>{document.getElementById(de)?.remove();let t=document.createElement("div");t.id=de,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
