...
`,A=`
  ...
  flex-shrink: 0;
`;var lr=/^[A-Za-z0-9]{15,30}$/,Q=o=>typeof o=="string"&&lr.test(o),dr=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},cr=()=>{let o=window.WS_CRM_USER_ID;return Q(o)?o:null},br=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(Q(t))return t;return null},hr=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=dr(e);if(!t)return null;let r=t.uid;if(Q(r))return r;let n=t.claims;if(n&&Q(n.user_id))return n.user_id}catch{}return null},pr=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(Q(n))return n}catch{}}}catch{}return null},I=()=>cr()||br()||hr()||pr()||null;var Wt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",mr=`${Wt}/active-follow-up-get`,ur=`${Wt}/active-follow-up-set`,V="ws-crm-active-follow-up-button",at={amber:{label:"Add to Active Follow Up",bg:i.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:i.amber,clickable:!0,dotColor:i.amber},blue:{label:"In Active Follow Up",bg:i.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:i.blue,clickable:!1,dotColor:i.blue},gray:{label:"Not in Follow Up",bg:i.slate,border:i.steel,color:i.coolGray,clickable:!1,dotColor:i.coolGray},loading:{label:"Active Follow Up\u2026",bg:i.slate,border:i.steel,color:i.ash,clickable:!1,dotColor:i.ash},error:{label:"Follow Up status unavailable",bg:i.slate,border:i.steel,color:i.ash,clickable:!1,dotColor:i.crimson}},_r=()=>{let o=S();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},vr=async o=>{let e=await fetch(mr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},yr=async(o,e)=>{let t=await fetch(ur,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},W=(o,e)=>{let t=at[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},zt=o=>{let e=document.createElement("button");return e.type="button",e.id=V,e.dataset.contactId=o,e.style.cssText=`
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 6px 12px;
  ...
  `,W(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=_r();if(!(!r||r.contactId!==o)){W(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await yr(r,I()),a=n&&n.state&&n.state in at?n.state:"blue";W(e,a),b.info(`Active Follow Up set for ${o} \u2192 ${a}`)}catch(n){b.error("Active Follow Up Set webhook failed",n),W(e,"error"),setTimeout(()=>void Te(e,r),4e3)}}}),e},Te=async(o,e)=>{W(o,"loading");try{let t=await vr(e),r=t&&t.state&&t.state in at?t.state:"gray",n=document.getElementById(V);if(!n||n.dataset.contactId!==e.contactId)return;W(n,r)}catch(t){b.warn("Active Follow Up Get webhook failed",t),W(o,"error")}},gr=[".central-panel","[class*='central-panel']"],nt="ws-crm-contact-actions-bar",fr=async()=>{for(let o of gr)try{let e=await C(o,{timeoutMs:3e3});if(e)return e}catch{}try{return(await C("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},wr=async(o,e)=>{let t=e.querySelector(`#${nt}`);if(!t)try{t=await C(`#${nt}`,{timeoutMs:3e3,root:e})}catch{t=null}if(t){let a=e.querySelector("#ws-crm-afu-wrap");return a&&a.remove(),t.appendChild(o),{container:t,placement:"actions-bar"}}let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
      ...
      flex-shrink: 0;
    `;let a=e.querySelector(`#${nt}`);a&&a.parentElement===e?e.insertBefore(n,a.nextSibling):e.insertBefore(n,e.firstChild)}return n.appendChild(o),{container:n,placement:"fallback"}},xr=()=>{let o=document.querySelector("[data-testid='CENTRALPANEL_NAME']");if(!o)return null;let e=o;for(let t=0;t<8&&!(!e||!e.parentElement);t++){let r=e.parentElement,n=(r.className||"").toString();if(n.includes("justify-between")&&n.includes("border-b")){let a=r.children[1];return a instanceof HTMLElement?a:r}e=r}return null},kr=(o,e)=>{o.style.marginRight="4px",e.insertBefore(o,e.firstChild)},Er=async o=>{if(o==="url"){let t=await fr();return t?{kind:"contact-detail",target:t}:null}let e=xr();return e?{kind:"conversations",target:e}:null},qt=async(o,e)=>{if(e.kind==="contact-detail"){let{placement:t}=await wr(o,e.target);return t}return kr(o,e.target),"conversations-header"},Tr=async()=>{let o=S();if(!o||!o.locationId){document.getElementById(V)?.remove(),document.getElementById("ws-crm-afu-wrap")?.remove();return}let e={contactId:o.contactId,tenantId:o.locationId},t=document.getElementById(V);if(t&&t.dataset.contactId===e.contactId){Te(t,e);return}t&&t.remove();let r=await Er(o.source);if(!r){b.warn(`Active Follow Up: no mount target on ${o.source}-source page`);return}let n=S();if(!n||n.contactId!==e.contactId)return;let a=zt(e.contactId),s=await qt(a,r);b.debug(`Active Follow Up mounted (${r.kind}/${s}) for ${e.contactId}`),Vt();let l=document.getElementById(V)??a;Te(l,e);let d=new MutationObserver(()=>{let c=document.getElementById(V);if(!(S()?.contactId===e.contactId)){d.disconnect();return}if(!c){let m=zt(e.contactId);qt(m,r).then(()=>{Vt(),Te(m,e)}),b.debug(`Active Follow Up re-mounted after wipe for ${e.contactId}`)}});d.observe(r.target,{childList:!0})},ee=null,jt=()=>{let o=S()?.contactId??null;if(ee&&ee.contactId===o)return;let e=Tr().finally(()=>{ee?.promise===e&&(ee=null)});ee={contactId:o,promise:e}},Vt=()=>{let o=document.querySelectorAll(`#${V}`);if(!(o.length<=1)){b.debug(`Active Follow Up: found ${o.length} duplicate buttons, keeping first, removing ${o.length-1}`);for(let e=1;e<o.length;e++)o[e].remove()}};var G="ws-crm-claim-release-pill",Sr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-get",Cr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-set",Ir=async o=>{let e=await fetch(Sr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,contact_id:o.contactId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Get returned ${e.status} ${e.statusText}`);return await e.json()},Ar=async o=>{let e=await fetch(Cr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,contact_id:o.contactId,triggered_by_user_id:o.userId,action:o.action}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Set returned ${e.status} ${e.statusText}`);return await e.json()},Xt=o=>{let e=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},X=(o,e)=>{let t=e==="claim"?"emerald":"amber",r=e==="claim"?i.emerald:i.amber;o.dataset.mode=e,o.style.cssText=Ut(t),o.title=e==="claim"?"Take ownership of this lead":"Return this lead to the pool",o.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{let t=Le();!t||!t.locationId||t.oppId!==o||an({oppId:t.oppId,locationId:t.locationId,onSuccess:()=>{let r=e.querySelector(".ws-mt-om-label");r&&(r.textContent="Offer Recorded"),e.style.background=i.emeraldGlow,e.style.color=i.emeraldBright,e.style.borderColor=i.emeraldBorder,setTimeout(()=>{r&&r.textContent==="Offer Recorded"&&(r.textContent="Mark Offer Made",e.style.cssText=A)},4e3)}})}),e},ln=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],dn=async()=>{for(let o of ln)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},Ae=!1,bt=async()=>{if(!Ae){Ae=!0;try{await cn()}finally{Ae=!1}}},cn=async()=>{let o=Le();if(!o||!o.locationId){document.getElementById(D)?.remove(),document.getElementById(`${D}-wrap`)?.remove(),ne();return}let e=document.getElementById(D);if(e&&e.dataset.oppId===o.oppId){let l=L();l!==null&&l!=="Marketing Tracker"&&(e.remove(),document.getElementById(`${D}-wrap`)?.remove(),ne());return}e&&(e.remove(),document.getElementById(`${D}-wrap`)?.remove());let t=await dn();if(!t){b.warn("MT Offer Made: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await U();if(r!=="Marketing Tracker"){b.debug(`[mt-offer] gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=Le();if(!n||n.oppId!==o.oppId)return;let a=sn(o.oppId),s=document.createElement("span");s.id=`${D}-wrap`,s.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),s.appendChild(a),t.appendChild(s),b.debug(`Mounted MT Offer Made button on opp ${o.oppId}`),bn(o.oppId)},re=null,ct=null,ne=()=>{re?.disconnect(),re=null,ct=null},bn=o=>{if(re&&ct===o)return;ne();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(ct=o,re=new MutationObserver(()=>{let t=Le();if(!t||t.oppId!==o){ne();return}let r=L();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(D)?.remove(),document.getElementById(`${D}-wrap`)?.remove(),ne();return}document.getElementById(D)||bt()}),re.observe(e,{childList:!0,subtree:!0}))},lt=null,dt,hn=()=>{lt||(lt=new MutationObserver(()=>{dt===void 0&&(dt=window.setTimeout(()=>{dt=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(D);o&&!e&&!Ae&&bt()},300))}),lt.observe(document.body,{childList:!0,subtree:!0}))},po=()=>{hn(),bt()};var pn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/mt/not-interested",O="ws-crm-not-interested-button",mn=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,un=/\/v2\/location\/([A-Za-z0-9]+)/,Me=()=>{let o=window.location.pathname,e=o.match(mn);if(!e)return null;let t=o.match(un);return{oppId:e[1],locationId:t?t[1]:null}},_n=async o=>{let e=await fetch(pn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,mt_opp_id:o.oppId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Not Interested webhook returned ${e.status} ${e.statusText}`);return await e.json()},vn=o=>{let e=document.createElement("button");return e.id=O,e.type="button",e.dataset.oppId=o,e.style.cssText=A,e.title="Close this Marketing Tracker opp as Not Interested for THIS property only",e.innerHTML=`
    <span style="
      display: inline-block;
      width: 8px;
      height: 8px;
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let t=Me();if(!t||!t.locationId||t.oppId!==o)return;let r=I(),n=e.querySelector(".ws-ni-label");e.disabled=!0,e.style.opacity="0.7",n&&(n.textContent="Closing\u2026");try{let a=await _n({oppId:t.oppId,locationId:t.locationId,userId:r});if(a?.ok)n&&(n.textContent="Marked Not Interested"),e.style.background=i.crimsonGlow,e.style.color=i.crimson,b.info(`Not Interested set for opp ${o} \u2192 ${a.new_stage}`);else throw new Error("Webhook returned ok=false")}catch(a){b.error("Not Interested webhook failed",a),n&&(n.textContent="Try again"),e.disabled=!1,e.style.opacity="1",setTimeout(()=>{n&&n.textContent==="Try again"&&(n.textContent="Mark Not Interested")},3e3)}}),e},yn=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],gn=async()=>{for(let o of yn)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},Fe=!1,ut=async()=>{if(!Fe){Fe=!0;try{await fn()}finally{Fe=!1}}},fn=async()=>{let o=Me();if(!o||!o.locationId){document.getElementById(O)?.remove(),document.getElementById(`${O}-wrap`)?.remove(),se();return}let e=document.getElementById(O);if(e&&e.dataset.oppId===o.oppId){let l=L();l!==null&&l!=="Marketing Tracker"&&(e.remove(),document.getElementById(`${O}-wrap`)?.remove(),se());return}e&&(e.remove(),document.getElementById(`${O}-wrap`)?.remove());let t=await gn();if(!t){b.warn("Not Interested: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await U();if(r!=="Marketing Tracker"){b.debug(`[not-interested] gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=Me();if(!n||n.oppId!==o.oppId)return;let a=vn(o.oppId),s=document.createElement("span");s.id=`${O}-wrap`,s.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),s.appendChild(a),t.appendChild(s),b.debug(`Mounted Not Interested button on opp ${o.oppId}`),wn(o.oppId)},ae=null,mt=null,se=()=>{ae?.disconnect(),ae=null,mt=null},wn=o=>{if(ae&&mt===o)return;se();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(mt=o,ae=new MutationObserver(()=>{let t=Me();if(!t||t.oppId!==o){se();return}let r=L();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(O)?.remove(),document.getElementById(`${O}-wrap`)?.remove(),se();return}document.getElementById(O)||ut()}),ae.observe(e,{childList:!0,subtree:!0}))},ht=null,pt,xn=()=>{ht||(ht=new MutationObserver(()=>{pt===void 0&&(pt=window.setTimeout(()=>{pt=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(O);o&&!e&&!Fe&&ut()},300))}),ht.observe(document.body,{childList:!0,subtree:!0}))},mo=()=>{xn(),ut()};var kn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/property/listing-price-changed",P="ws-crm-reduce-price-button",uo="ws-crm-reduce-price-modal",En=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Tn=/\/v2\/location\/([A-Za-z0-9]+)/,Pe=()=>{let o=window.location.pathname,e=o.match(En);if(!e)return null;let t=o.match(Tn);return{oppId:e[1],locationId:t?t[1]:null}},Sn=async o=>{let e=await fetch(kn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,dispo_opp_id:o.oppId,new_price:o.newPrice,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Reduce Price webhook returned ${e.status} ${e.statusText}`);let t=await e.text();try{let r=JSON.parse(t);if(r?.ok===!1&&r?.error)throw new Error(r.error);return r}catch(r){if(r instanceof Error&&/returned/.test(r.message))throw r;return{}}},De=()=>{document.getElementById(uo)?.remove()},Cn=o=>{De();let e=document.createElement("div");e.id=uo,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-1px)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",()=>{let r=Pe();!r||!r.locationId||r.oppId!==o||Cn({oppId:r.oppId,locationId:r.locationId,onSuccess:()=>{let n=t.querySelector(".ws-rp-label"),a=t.querySelector(":scope > span:first-child");n&&(n.textContent="Price Reduced \u2014 Buyers Alerted"),t.style.background=i.emeraldGlow,t.style.color=i.emeraldBright,t.style.borderColor=i.emeraldBorder,a&&(a.style.background=i.emerald,a.style.boxShadow=`0 0 6px ${i.emerald}`),setTimeout(()=>{n&&n.textContent==="Price Reduced \u2014 Buyers Alerted"&&(n.textContent="Reduce Price and Alert Buyers",t.style.cssText=A,a&&(a.style.background=i.amber,a.style.boxShadow=`0 0 6px ${i.amber}`))},4e3)}})}),t},An=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],Ln=async()=>{for(let o of An)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},Oe=!1,gt=async()=>{if(!Oe){Oe=!0;try{await Fn()}finally{Oe=!1}}},Fn=async()=>{let o=Pe();if(!o||!o.locationId){document.getElementById(P)?.remove(),document.getElementById(`${P}-wrap`)?.remove(),le();return}let e=document.getElementById(P);if(e&&e.dataset.oppId===o.oppId){let l=L();l!==null&&l!=="Disposition"&&(e.remove(),document.getElementById(`${P}-wrap`)?.remove(),le());return}e&&(e.remove(),document.getElementById(`${P}-wrap`)?.remove());let t=await Ln();if(!t){b.warn("Reduce Price: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await U();if(r!=="Disposition"){b.debug(`[reduce-price] gated off \u2014 pipeline="${r}" (expected Disposition)`);return}let n=Pe();if(!n||n.oppId!==o.oppId)return;let a=In(o.oppId,o.locationId),s=document.createElement("span");s.id=`${P}-wrap`,s.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),s.appendChild(a),t.appendChild(s),b.debug(`Mounted Reduce Price button on opp ${o.oppId}`),Mn(o.oppId)},ie=null,yt=null,le=()=>{ie?.disconnect(),ie=null,yt=null},Mn=o=>{if(ie&&yt===o)return;le();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(yt=o,ie=new MutationObserver(()=>{let t=Pe();if(!t||t.oppId!==o){le();return}let r=L();if(r!==null&&r!=="Disposition"){document.getElementById(P)?.remove(),document.getElementById(`${P}-wrap`)?.remove(),le();return}document.getElementById(P)||gt()}),ie.observe(e,{childList:!0,subtree:!0}))},_t=null,vt,Dn=()=>{_t||(_t=new MutationObserver(()=>{vt===void 0&&(vt=window.setTimeout(()=>{vt=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(P);o&&!e&&!Oe&&gt()},300))}),_t.observe(document.body,{childList:!0,subtree:!0}))},_o=()=>{Dn(),gt()};var go="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/tc/send-to-title-co",z="ws-crm-send-to-closing-office-button",j="ws-crm-send-to-title-co-wrap",Be="ws-crm-stc-dropdown",On=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Pn=/\/v2\/location\/([A-Za-z0-9]+)/,$e=()=>{let o=window.location.pathname,e=o.match(On);if(!e)return null;let t=o.match(Pn);return{oppId:e[1],locationId:t?t[1]:null}},Hn=5*60*1e3,fo=o=>`ws-crm-stc-cache:${o}`,Nn=o=>{try{let e=window.localStorage.getItem(fo(o));if(!e)return null;let t=JSON.parse(e);return!t.ts||Date.now()-t.ts>Hn||!Array.isArray(t.checks)||t.checks.length===0?null:t}catch{return null}},Bn=(o,e)=>{try{window.localStorage.setItem(fo(o),JSON.stringify(e))}catch{}},$n=async(o,e)=>{try{let t=await fetch(go,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:o,triggered_by_user_id:I()??null,dry_run:!0}),credentials:"omit"});return t.ok?await t.json():(b.warn(`[stc] dry_run returned HTTP ${t.status}`),null)}catch(t){return b.warn("[stc] dry_run fetch failed:",t),null}},Rn=async(o,e,t)=>{let r=await fetch(go,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:o,triggered_by_user_id:I()??null,confirm_resend:t}),credentials:"omit"});if(!r.ok&&r.status!==200)throw new Error(`HTTP ${r.status}`);return await r.json()},Gn=o=>{let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(!e)return!1;let t=e.querySelectorAll("a, button, li, [role='tab'], [role='menuitem'], .crm-opportunities-modal-content [class*='tab']");for(let n of t)if((n.textContent?.trim()??"")===o)return n.click(),!0;let r=e.querySelectorAll("*");for(let n of r){if(n.children.length>1)continue;if((n.textContent?.trim()??"")===o)return(n.closest("a, button, li, [role='tab']")||n.parentElement||n).click(),!0}return b.warn(`[stc] folder tab not found for "${o}"`),!1},vo=o=>{let e=document.getElementById(o);if(!e){b.warn(`[stc] field ${o} not found in DOM \u2014 can't scroll to it`);return}e.scrollIntoView({behavior:"smooth",block:"center"});let t=e.style.boxShadow,r=e.style.transition;e.style.transition="box-shadow 0.3s ease, background-color 0.3s ease",e.style.boxShadow=`0 0 0 3px ${i.emerald}, 0 0 18px ${i.emerald}`,window.setTimeout(()=>{e.style.boxShadow=t,window.setTimeout(()=>{e.style.transition=r},400)},2800)},Un=(o,e)=>{if(e&&Gn(e)){window.setTimeout(()=>vo(o),300);return}vo(o)},H=()=>{document.getElementById(Be)?.remove()},He=(o,e,t)=>{H();let r=document.createElement("div");r.id=Be;let n=t.getBoundingClientRect();r.style.cssText=`
    position: fixed;
    top: ${n.bottom+6}px;
  ...
  `,B(t,n);let a=new Set,s=async()=>{if(n.status==="sending")return;let y=await $n(o,e);if(!y){n={status:"error",message:"Check failed \u2014 click to retry"},B(t,n);return}if(y.rate_limited){v=Date.now()+p,n={status:"error",checks:n.checks,message:"Rate-limited by GHL \u2014 will retry in 5m"},B(t,n),b.warn(`[stc] dry_run rate-limited \u2014 cooling down until ${new Date(v).toLocaleTimeString()}`);return}if(!Array.isArray(y.checks)||y.checks.length===0){n={status:"error",message:y.error||"Check failed \u2014 click to retry"},B(t,n),b.warn("[stc] dry_run returned no checks:",y);return}let _=y.pipeline_name;if(_&&_!=="Transaction Coordination"){b.debug(`[stc] dry_run reports pipeline="${_}" (not TC) \u2014 retracting button + caching`),Re.add(o),document.getElementById(z)?.remove(),document.getElementById(j)?.remove(),H(),ce();return}let f=y.checks;if(n.checks&&n.checks.length===y.checks.length){let T=new Set;f=y.checks.map((k,M)=>n.checks[M]?.filled&&!k.filled?a.has(k.label)?k:(T.add(k.label),{...k,filled:!0}):k),a=T}else a=new Set;n={status:f.every(T=>T.filled)?"ready":"incomplete",checks:f,previouslySentAt:y.previously_sent_at??null},Bn(o,{checks:f,previously_sent_at:y.previously_sent_at??null,pipeline_name:_,ts:Date.now()}),B(t,n),document.getElementById(Be)&&He(f,y.previously_sent_at,t)},l=async()=>{let y=!!n.previouslySentAt;n={status:"sending",checks:n.checks,previouslySentAt:n.previouslySentAt},B(t,n),H();try{let _=await Rn(o,e,y);if(_.needs_confirm){n={status:"ready",checks:_.checks,previouslySentAt:_.previously_sent_at},B(t,n),_.checks&&He(_.checks,_.previously_sent_at,t);return}if(_.ok){n={status:"sent",checks:_.checks,previouslySentAt:new Date().toISOString()},B(t,n),window.setTimeout(()=>{g()},3500);return}n={status:"error",checks:_.checks,message:_.error||"Validation failed"},B(t,n),_.checks&&He(_.checks,_.previously_sent_at,t)}catch(_){b.error("[stc] send failed:",_),n={status:"error",message:"Network error \u2014 click to retry"},B(t,n)}};t.addEventListener("click",()=>{switch(n.status){case"ready":l();break;case"incomplete":{document.getElementById(Be)?H():n.checks&&He(n.checks,n.previouslySentAt,t);break}case"error":case"sent":g();break}});let d,c=!1,h=Date.now(),m=1e4,p=5*60*1e3,v=0,g=async()=>{if(!c){if(Date.now()<v){b.debug(`[stc] dry_run suppressed \u2014 rate-limit cooldown until ${new Date(v).toLocaleTimeString()}`);return}c=!0,h=Date.now();try{await s()}finally{c=!1}}};g();let x=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(x){let y=new MutationObserver(()=>{if(d!==void 0||c)return;let f=Date.now()-h,E=Math.max(1500,m-f);d=window.setTimeout(()=>{d=void 0,n.status!=="sending"&&g()},E)});y.observe(x,{childList:!0,subtree:!0,characterData:!0}),t.dataset.observerActive="1";let _=window.setInterval(()=>{document.body.contains(t)||(y.disconnect(),d!==void 0&&window.clearTimeout(d),window.clearInterval(_))},5e3)}return t},qn=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],Vn=async()=>{for(let o of qn)try{let e=await C(o,{timeoutMs:1500});if(e)return b.debug(`[send-to-title-co] mount target: ${o}`),e}catch{}return b.warn(`[send-to-title-co] no mount target found. URL: ${window.location.pathname}`),null},Re=new Set,yo=null,Wn=o=>{o&&o!==yo&&(Re.clear(),yo=o)},Ne=!1,kt=async()=>{if(Ne){b.debug("[stc] mount:00-LOCKED ensureMounted already in-flight \u2014 skip");return}Ne=!0;try{await jn()}catch(o){b.error("[stc] ensureMountedImpl threw:",o)}finally{Ne=!1}},jn=async()=>{b.debug("[stc] mount:01 ensureMountedImpl entered");let o=$e();if(!o||!o.locationId){b.debug(`[stc] mount:02-BAIL no ctx or no locationId. ctx=${JSON.stringify(o)}`),document.getElementById(z)?.remove(),document.getElementById(j)?.remove(),H();return}if(b.debug(`[stc] mount:02 ctx ok \u2014 oppId=${o.oppId} locId=${o.locationId}`),Wn(o.oppId),Re.has(o.oppId)){b.debug(`[stc] mount:02-BAIL opp ${o.oppId} previously confirmed non-TC this session`),document.getElementById(z)?.remove(),document.getElementById(j)?.remove(),H();return}let e=document.getElementById(z);if(e&&e.dataset.oppId===o.oppId){b.debug("[stc] mount:03-SKIP existing button for same opp");let l=L();l!==null&&l!=="Transaction Coordination"&&(e.remove(),document.getElementById(j)?.remove(),H(),ce());return}e&&(b.debug("[stc] mount:03 removing stale existing button"),e.remove(),document.getElementById(j)?.remove(),H()),b.debug("[stc] mount:04 calling findMountTarget");let t=await Vn();if(!t){b.debug("[stc] mount:04-BAIL findMountTarget returned null");return}b.debug(`[stc] mount:04 target found tag=${t.tagName} class="${t.className}"`);let r=L();if(r!==null&&r!=="Transaction Coordination"){b.debug(`[stc] mount:05-GATED DOM pipeline="${r}" \u2014 skipping mount`);return}let n=$e();if(!n||n.oppId!==o.oppId||!n.locationId){b.debug(`[stc] mount:06-BAIL ctx changed during awaits. was=${o.oppId} now=${n?.oppId}`);return}b.debug("[stc] mount:06 building button (pipeline gate will run via dry_run)");let a=zn(o.oppId,n.locationId),s=document.createElement("span");s.id=j,s.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),s.appendChild(a),t.appendChild(s),b.debug(`[stc] mount:07-OK Mounted on opp ${o.oppId}, inDoc=${document.body.contains(a)}`),Kn(o.oppId)},de=null,xt=null,ce=()=>{de?.disconnect(),de=null,xt=null},Kn=o=>{if(de&&xt===o)return;ce();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(xt=o,de=new MutationObserver(()=>{let t=$e();if(!t||t.oppId!==o){ce();return}let r=L();if(r!==null&&r!=="Transaction Coordination"){document.getElementById(z)?.remove(),document.getElementById(j)?.remove(),H(),ce();return}document.getElementById(z)||kt()}),de.observe(e,{childList:!0,subtree:!0}))},ft=null,wt,Yn=()=>{ft||(ft=new MutationObserver(()=>{wt===void 0&&(wt=window.setTimeout(()=>{wt=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(z);if(o&&!e&&!Ne){let t=$e();if(t&&Re.has(t.oppId))return;kt()}},300))}),ft.observe(document.body,{childList:!0,subtree:!0}))},wo=()=>{Yn(),kt()};var Xn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/search",Jn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/opp/set-field",Zn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/opp/get-field",Eo="ldtyB2UXKi9vOp8nLUH7",Qn="WVD2gb9swLsgcdBEtDVq",N="ws-crm-closing-agent-picker-button",To="ws-crm-closing-agent-picker-modal",ea=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,ta=/\/v2\/location\/([A-Za-z0-9]+)/,pe=()=>{let o=window.location.pathname,e=o.match(ea);if(!e)return null;let t=o.match(ta);return{oppId:e[1],locationId:t?t[1]:null}},Ft=async o=>{let e=await fetch(Xn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,query:o.query??"",contact_id:o.contactId??null,contact_type:o.contactType??null,limit:o.limit??20}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},xo=async o=>{let e=await fetch(Jn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,opp_id:o.oppId,field_id:o.fieldId,value:o.value}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},oa=async(o,e)=>{try{let t=await fetch(Zn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,opp_id:o,field_id:Eo}),credentials:"omit"});if(!t.ok)return null;let r=await t.json(),n=typeof r.value=="string"&&r.value.trim()?r.value.trim():null;if(!n||!/^[A-Za-z0-9]{16,}$/.test(n))return null;let a=await Ft({tenantId:e,contactId:n,limit:1});if(!a.ok||!a.results.length)return null;let s=a.results[0];return{id:s.id,name:s.name,companyName:s.companyName??""}}catch(t){return b.warn("[closing-agent] fetchCurrent failed:",t),null}},K=null,ra=60*60*1e3,Mt=o=>`ws-crm-to-cache:${o}`,So=o=>{try{let e=window.localStorage.getItem(Mt(o));if(!e)return null;let t=JSON.parse(e);return!t.ts||Date.now()-t.ts>ra||typeof t.contactId!="string"||!t.contactId||typeof t.contactName!="string"||!t.contactName?null:t}catch{return null}},Co=(o,e,t)=>{try{window.localStorage.setItem(Mt(o),JSON.stringify({contactId:e,contactName:t,ts:Date.now()}))}catch{}},na=o=>{try{window.localStorage.removeItem(Mt(o))}catch{}},Ct="ws-crm-closing-agent-field-link",aa=()=>{let o=document.querySelector(".crm-opportunities-modal");o&&(o.querySelectorAll(`.${Ct}`).forEach(e=>e.remove()),o.querySelectorAll("input[data-ws-closing-agent-decorated]").forEach(e=>{e.style.removeProperty("color"),e.style.removeProperty("caret-color"),e.style.removeProperty("visibility"),delete e.dataset.wsClosingAgentDecorated}))},Et=!1,Dt=async o=>{if(Et)return;let e=Io();if(!e)return;let t=(e.value||"").trim();if(!t){b.info("[closing-agent] tryDecorate: input has empty value");return}if(!/^[A-Za-z0-9]{16,}$/.test(t)){b.info(`[closing-agent] tryDecorate: value '${t}' doesn't match contact-ID pattern (expecting 16+ alphanumeric)`);return}if(e.dataset.wsClosingAgentDecorated!==t){Et=!0;try{b.info(`[closing-agent] tryDecorate: looking up ${t}`);let r=await Ft({tenantId:o,contactId:t,limit:1});if(!r.ok||!r.results.length){b.warn(`[closing-agent] tryDecorate: contact ${t} not found`);return}let n=r.results[0];b.info(`[closing-agent] tryDecorate: applying ${n.name} (${n.id})`),It(n.id,n.name,o);let a=document.getElementById(N);if(a){q(a,{status:"linked",contactName:n.name});let s=pe();s&&Co(s.oppId,n.id,n.name)}}catch(r){b.warn("[closing-agent] tryDecorate lookup failed:",r)}finally{Et=!1}}},sa=/^TC\s*[-‐–—]\s*Closing\s+Agent$/i,Io=()=>{let e=document.querySelectorAll("*"),t=[],r=[],n=[];for(let d of e){let c=(d.textContent||"").trim();sa.test(c)?t.push(d):c.length<80&&/Title\s+Officer/i.test(c)&&r.push(c);for(let h of["aria-label","placeholder","data-label","title","name","data-test"]){let m=d.getAttribute(h);m&&/Title\s+Officer/i.test(m)&&n.push({tag:d.tagName.toLowerCase(),attr:h,value:m})}}if(b.info(`[closing-agent] findInput: ${t.length} regex-matched, ${r.length} near-text matches, ${n.length} attribute matches`),r.length&&b.warn("[closing-agent] near-text samples:",[...new Set(r)].slice(0,8)),n.length&&b.warn("[closing-agent] attribute matches:",n.slice(0,8)),t.length===0)return null;t.sort((d,c)=>d.querySelectorAll("*").length-c.querySelectorAll("*").length);let s=t[0].parentElement,l=0;for(;s&&l<8;){let d=s.querySelector("input");if(d)return b.info(`[closing-agent] findInput: located input ${l} levels up from label`),d;s=s.parentElement,l+=1}return b.warn("[closing-agent] findInput: label found but no <input> within 8 ancestors"),null},It=(o,e,t)=>{let r=Io();if(!r)return;let n=r.dataset.wsClosingAgentDecorated;if(n!==o){n&&r.parentElement?.querySelectorAll(`.${Ct}`).forEach(s=>s.remove());{let a=r.parentElement;if(!a)return;r.dataset.wsClosingAgentDecorated=o,r.style.visibility="hidden",window.getComputedStyle(a).position==="static"&&(a.style.position="relative");let l=document.createElement("a");l.className=Ct,l.href=`/v2/location/${t}/contacts/detail/${o}`,l.textContent=e,l.style.cssText=`
      position: absolute;
      inset: 0;
      display: flex;
      align-items: center;
      ...
      border-radius: ${w.sm};
      z-index: 2;
    `,l.addEventListener("mouseenter",()=>{l.style.textDecoration="underline"}),l.addEventListener("mouseleave",()=>{l.style.textDecoration="none"}),l.addEventListener("click",d=>{d.preventDefault(),d.stopPropagation(),window.location.href=l.href}),a.appendChild(l)}}},Ge=()=>{document.getElementById(To)?.remove()},ia=o=>{Ge();let e=document.createElement("div");e.id=To,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `;let r={status:"loading"};q(t,r);let n=So(o);return n&&(r={status:"linked",contactName:n.contactName},q(t,r),K={oppId:o,contactId:n.contactId,contactName:n.contactName,tenantId:e}),(async()=>{let s=await oa(o,e);s?(r={status:"linked",contactName:s.name},q(t,r),K={oppId:o,contactId:s.id,contactName:s.name,tenantId:e},It(s.id,s.name,e)):n||(r={status:"empty"},q(t,r),K?.oppId===o&&(K=null),aa(),na(o))})(),t.addEventListener("mouseenter",()=>{r.status!=="loading"&&r.status!=="saving"&&(t.style.transform="translateY(-1px)")}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",()=>{r.status==="loading"||r.status==="saving"||ia({oppId:o,locationId:e,onPick:async s=>{r={status:"saving"},q(t,r);try{let l=[xo({tenantId:e,oppId:o,fieldId:Eo,value:s.id})];s.companyName&&s.companyName.trim()&&l.push(xo({tenantId:e,oppId:o,fieldId:Qn,value:s.companyName.trim()}));let c=(await Promise.all(l))[0];if(!c.ok)throw new Error(c.error??"Save failed");r={status:"linked",contactName:s.name},q(t,r),K={oppId:o,contactId:s.id,contactName:s.name,tenantId:e},Co(o,s.id,s.name),It(s.id,s.name,e),b.info(`Closing Agent linked: ${s.name} (${s.id})${s.companyName?` \u2014 Company: ${s.companyName}`:""}`)}catch(l){b.error("Closing Agent link failed",l),r={status:"empty"},q(t,r)}}})}),t},da=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading"],ca=async()=>{for(let o of da)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},be=!1,qe=async()=>{if(!be){be=!0;try{await ba()}finally{be=!1}}},ba=async()=>{let o=pe();if(!o||!o.locationId){document.getElementById(N)?.remove(),document.getElementById(`${N}-wrap`)?.remove(),ze();return}let e=document.getElementById(N);if(e&&e.dataset.oppId===o.oppId){let d=L();d!==null&&d!=="Transaction Coordination"&&(e.remove(),document.getElementById(`${N}-wrap`)?.remove(),ze());return}e&&(e.remove(),document.getElementById(`${N}-wrap`)?.remove());let t=L();if(t!==null&&t!=="Transaction Coordination")return;let r=await ca();if(!r||!(So(o.oppId)!==null||K!==null&&K.oppId===o.oppId)&&await U()!=="Transaction Coordination")return;let a=pe();if(!a||a.oppId!==o.oppId||!a.locationId)return;let s=la(o.oppId,a.locationId),l=document.createElement("span");l.id=`${N}-wrap`,l.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,r.classList.contains("description")&&(r.style.display="flex",r.style.alignItems="center",r.style.gap="16px",r.style.flexWrap="wrap"),l.appendChild(s),r.appendChild(l),b.debug(`Mounted Closing Agent picker on opp ${o.oppId}`),ha(o.oppId),Dt(a.locationId)},he=null,At=null,Lt=null,ze=()=>{he?.disconnect(),he=null,At=null,Lt=null},ha=o=>{let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(he&&At===o&&Lt===e||(ze(),At=o,Lt=e,he=new MutationObserver(()=>{let t=pe();if(!t||t.oppId!==o||!t.locationId){ze();return}document.getElementById(N)||qe(),Dt(t.locationId)}),he.observe(e,{childList:!0,subtree:!0})))},Tt=null,St,pa=()=>{Tt||(Tt=new MutationObserver(()=>{St===void 0&&(St=window.setTimeout(()=>{St=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(N);o&&!e&&!be&&qe()},300))}),Tt.observe(document.body,{childList:!0,subtree:!0}))},ko,ma=()=>{ko===void 0&&(ko=window.setInterval(()=>{let o=pe();if(!o||!o.locationId||!!!document.querySelector(".crm-opportunities-modal"))return;!!!document.getElementById(N)&&!be&&(b.info("[closing-agent] safety-net: button missing on opp page \u2014 remounting"),qe()),Dt(o.locationId)},2e3))},Ao=()=>{b.info("[closing-agent] mountClosingAgentPickerButton invoked"),pa(),ma(),qe()};var ua="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contract/stage",Ot="ws-crm-prepare-contract-button",Lo="ws-crm-prepare-contract-menu",Fo="ws-crm-prepare-contract-modal",ue="https://services.leadconnectorhq.com",_a=()=>{try{let o=localStorage.getItem("refreshedToken");return o?o.startsWith('"')?o.slice(1,-1):o:null}catch{return null}},_e=()=>{let o=_a();if(!o)throw new Error("GHL session token not found in localStorage. Are you logged in?");return{Authorization:`Bearer ${o}`,Version:"2021-07-28",Channel:"APP",Source:"WEB_USER"}},Oo=()=>{let o=S();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},va=o=>{let e=(o||"").trim();return/^\[?\s*PA\s*\]?\s*[-\s]/i.test(e)?"PA":/^\[?\s*AA\s*\]?\s*[-\s]/i.test(e)?"AA":"passthrough"},ya=async o=>{let e=`${ue}/proposals/templates/bulk?type.in%5B%5D=proposal&type.in%5B%5D=estimate&locationId.eq=${encodeURIComponent(o)}&skip=0&limit=100`,t=await fetch(e,{method:"GET",headers:_e()});if(!t.ok)throw new Error(`Templates fetch returned ${t.status} ${t.statusText}`);let r=await t.json();return(r.templates||r.items||[]).map(a=>({id:(a._id??a.id)||"",name:a.name||"(unnamed template)",type:va(a.name||"")})).filter(a=>!!a.id)},ga=async(o,e)=>{let t=await fetch(`${ue}/opportunities/pipelines?locationId=${encodeURIComponent(o)}`,{method:"GET",headers:_e()});if(!t.ok)throw new Error(`Pipelines fetch ${t.status}`);let n=((await t.json()).pipelines||[]).find(d=>/marketing\s*tracker/i.test(d.name));if(!n)throw new Error("No Marketing Tracker pipeline found on tenant");let a=await fetch(`${ue}/opportunities/search?location_id=${encodeURIComponent(o)}&pipeline_id=${encodeURIComponent(n.id)}&contact_id=${encodeURIComponent(e)}&status=open&limit=20`,{method:"GET",headers:_e()});if(!a.ok)throw new Error(`MT opp search ${a.status}`);return((await a.json()).opportunities||[]).map(d=>({id:d.id,name:d.name||"(unnamed opp)",propertyAddress:d.name||"",updatedAt:d.updatedAt||null})).sort((d,c)=>{let h=d.updatedAt?Date.parse(d.updatedAt):0;return(c.updatedAt?Date.parse(c.updatedAt):0)-h})},fa=async o=>{let e=await fetch(ua,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,template_id:o.templateId,template_type:o.templateType,opp_id:o.oppId,closing_date:o.closingDate,buyer_earnest_due_date:o.buyerEarnestDueDate,lender_source:o.lenderSource,triggered_by_user_id:o.userId}),credentials:"omit"});if(!e.ok)throw new Error(`Contract.Stage HTTP ${e.status}`);return await e.json()},wa=async(o,e)=>{let t=await fetch(`${ue}/proposals/templates/to-proposal/${o}/${e}?locale=en-US`,{method:"POST",headers:_e()});if(!t.ok)throw new Error(`to-proposal HTTP ${t.status} ${t.statusText}`);let r=await t.json(),n=r._id||r.id||r.proposal?._id||r.proposal?.id;if(!n)throw new Error("to-proposal returned no proposal id");return n},xa=async(o,e)=>{let t=await fetch(`${ue}/proposals/document/${o}`,{method:"PUT",headers:{..._e(),"Content-Type":"application/json"},body:JSON.stringify({contactId:e})});if(!t.ok)throw new Error(`assign-contact HTTP ${t.status} ${t.statusText}`)},ka=(o,e)=>{let t=`/v2/location/${o}/payments/proposals-estimates/edit/${e}`;window.location.href=t},Mo="ws-crm-prepare-contract-toast",me=(o,e="info",t=4e3)=>{document.getElementById(Mo)?.remove();let r=document.createElement("div");r.id=Mo;let n=e==="error"?i.crimson:i.emerald;r.style.cssText=`
    position: fixed;
    bottom: 24px;
    right: 24px;
    z-index: 999999;
  ...
  `,r.textContent=o,document.body.appendChild(r),window.setTimeout(()=>r.remove(),t)},Ea=({tenantId:o,contactId:e,template:t,mtOpps:r,userId:n})=>{document.getElementById(Fo)?.remove();let a=document.createElement("div");a.id=Fo,a.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `;let l=y=>y.toISOString().slice(0,10),d=new Date,c=l(new Date(d.getTime()+45*24*60*60*1e3)),h=l(new Date(d.getTime()+7*24*60*60*1e3)),m=(()=>{if(r.length===0)return`
        ...
        <input type="hidden" id="ws-pc-opp-id" value="${Do(r[0].id)}" />
      `;let y=r.map(_=>`<option value="${Do(_.id)}">${Z(_.propertyAddress||_.name)}</option>`).join("");return`
      <label style="
        display: block;
        font-family: ${u.mono};
        font-size: 10px;
  ...
  `,t.innerHTML=`
    ...
    ">Loading templates\u2026</div>`,o.appendChild(t);let r=a=>{!t.contains(a.target)&&a.target!==o&&(t.remove(),document.removeEventListener("click",r,!0))};window.setTimeout(()=>document.addEventListener("click",r,!0),0);let n;try{n=await ya(e.tenantId)}catch(a){t.innerHTML=`
  ...
  `,o.addEventListener("mouseenter",()=>{o.style.borderColor=i.emerald}),o.addEventListener("mouseleave",()=>{o.style.borderColor=i.steel}),o.addEventListener("click",e=>{e.stopPropagation(),Ta(o)}),o},Ca="ws-crm-contact-actions-bar",Ia=async()=>{let o=Oo();if(!o){document.getElementById(Ot)?.remove();return}let e=null;try{e=await C(`#${Ca}`,{timeoutMs:3e3})}catch{b.debug("[prepare-contract] actions bar not found within timeout");return}if(!e)return;let t=document.getElementById(Ot);if(t){if(t.dataset.contactId===o.contactId)return;t.remove()}let r=Sa();r.dataset.contactId=o.contactId,e.appendChild(r),b.debug(`[prepare-contract] mounted for contact ${o.contactId}`)},Ho=()=>{Ia()};function Z(o){return String(o).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Do(o){return Z(o)}var Xe="reos-theme-stylesheet",er="reos-theme-fonts",Aa=()=>{let o=document.getElementById(er);o&&o.remove()},La=`
/* ============================================
