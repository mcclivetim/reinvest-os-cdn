...
/* v0.12.17: ID-level specificity bump so Vue scoped CSS
   (.hr-input__input[data-v-XXX]) can't paint a gray-200 hairline.
   Tim flagged the Property Address / City / County / State / Zip
   row of inputs on the contact details panel \u2014 visible 1px gray-200
   border instead of steel. Vue scoped rule is (0,0,2,0); base
   rule above is (0,0,1,0) so the scoped variant won despite !important
   (specificity beats !important when both are !important). Repeat
   the rule with :not(#__reos_never_id) ID-trick for (0,1,1,2) which
   beats any Vue scoped variant. Also pin the full border shorthand
   so border-width + border-style + border-color can't fragment. */
...
/* Inner <input class="hr-input__input-el"> rides on top of the
   wrapper; kill its own border so we don't get a double-stripe. */
html body input.hr-input__input-el:not(#__reos_never_id),
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};console.log("[ws-crm-customizer] bundle-loaded");var tt=()=>{h.info(`Loaded v${W} (built ${j})`);try{se()}catch(o){h.warn("mountVersionBadge failed:",o)}Me(),Ze(),ye(),ue(o=>{h.debug("Page handler firing for:",o),se(),Ce(),De(),ke(),Pe()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",tt):tt();})();
