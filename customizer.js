/* Wholesaling CRM Customizer v0.10.22 — built 2026-05-21T19:28:31.041Z */
"use strict";var WholesalingCRMCustomizer=(()=>{var M="[wholesaling-crm-customizer]",Ie="__WS_CRM_DEBUG",De=()=>!!window[Ie],h={info(...r){console.log(M,...r)},warn(...r){console.warn(M,...r)},error(...r){console.error(M,...r)},debug(...r){De()&&console.log(M,"[debug]",...r)}};var ee=()=>{let r=window.location.pathname;return r.includes("/contacts/detail/")?"contact-detail":r.includes("/contacts/smart-list")||r.includes("/contacts/")?"contact-list":r.includes("/opportunities/detail/")?"opportunity-detail":r.includes("/opportunities/")?"opportunity-list":r.includes("/calendars/")?"calendar":r.includes("/dashboard")?"dashboard":r.includes("/conversations")?"conversations":r.includes("/settings")?"settings":"other"},R="",A="other",Y=new Set,P=()=>{if(window.location.pathname!==R){R=window.location.pathname,A=ee(),h.debug("Page changed:",A,R);for(let r of Y)try{r(A)}catch(t){h.error("Subscriber threw:",t)}}},te=r=>(Y.add(r),setTimeout(()=>r(A),0),()=>{Y.delete(r)}),oe=()=>{A=ee(),R=window.location.pathname,h.debug("Router init, current page:",A);let r=history.pushState.bind(history),t=history.replaceState.bind(history);history.pushState=function(e,o,a){r(e,o,a),P()},history.replaceState=function(e,o,a){t(e,o,a),P()},window.addEventListener("popstate",P),setInterval(P,1e3)};var O="0.10.22",B="2026-05-21T19:28:31.042Z";var He=/\/contacts\/detail\/([A-Za-z0-9]+)/,Ne=/\/v2\/location\/([A-Za-z0-9]+)/,T=()=>{let r=window.location.pathname,t=r.match(He);if(!t)return null;let e=r.match(Ne);return{contactId:t[1],locationId:e?e[1]:null}};var F=(r,t={})=>{let{timeoutMs:e=8e3,pollMs:o=100,root:a=document}=t;return new Promise((s,i)=>{let n=a.querySelector(r);if(n){s(n);return}let l,b,p=()=>{m&&m.disconnect(),l!==void 0&&window.clearTimeout(l),b!==void 0&&window.clearInterval(b)},m=new MutationObserver(()=>{let d=a.querySelector(r);d&&(p(),s(d))});m.observe(document.body,{childList:!0,subtree:!0}),b=window.setInterval(()=>{let d=a.querySelector(r);d&&(p(),s(d))},o),l=window.setTimeout(()=>{p(),h.warn(`waitForElement timed out for selector: ${r}`),i(new Error(`Selector not found within ${e}ms: ${r}`))},e)})};var c={obsidian:"#0A0D12",graphite:"#12161D",slate:"#1A1F28",steel:"#252C36",bone:"#EDEEF0",ash:"#9098A3",coolGray:"#5A6470",emerald:"#0FB57E",emeraldBright:"#14C98B",blue:"#4B8BF5",amber:"#E8A33C",crimson:"#D43F4A",emeraldGlow:"rgba(15, 181, 126, 0.12)",emeraldBorder:"rgba(15, 181, 126, 0.3)",blueGlow:"rgba(75, 139, 245, 0.12)",amberGlow:"rgba(232, 163, 60, 0.12)",crimsonGlow:"rgba(212, 63, 74, 0.12)"},k={sm:"4px",md:"6px",lg:"10px",pill:"999px"},f={sans:"'Geist', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",mono:"'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace"},X={purple:"#8B5CF6",green:c.emerald,orange:c.amber,red:c.crimson},re=r=>{let t=r==="emerald"?c.emeraldGlow:r==="amber"?c.amberGlow:c.slate,e=r==="emerald"?c.emeraldBorder:r==="amber"?"rgba(232, 163, 60, 0.3)":c.steel,o=r==="emerald"?c.emerald:r==="amber"?c.amber:c.bone;return`
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: 4px 10px;
...
`,L=`
  ...
  flex-shrink: 0;
`;var I="ws-crm-claim-release-pill",ae=r=>{let t=Array.from(r.children).find(o=>{if(!(o instanceof HTMLElement))return!1;let a=o.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Me=({contactId:r,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=r,e.dataset.mode=t;let o=t==="claim"?"emerald":"amber",a=t==="claim"?c.emerald:c.amber;return e.style.cssText=re(o),e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,e.title=t==="claim"?"Take ownership of this lead":"Return this lead to the pool",e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{h.info(`Claim/Release pill clicked for contact ${r}: ${t}`);let s=e.lastElementChild,i=s?.textContent??"";s&&(s.textContent="Working\u2026"),e.style.opacity="0.7",e.disabled=!0,setTimeout(()=>{s&&(s.textContent=i),e.style.opacity="1",e.disabled=!1},700)}),e},Pe=async(r,t=1e4)=>{let e=Date.now();for(;Date.now()-e<t;){let o=ae(r);if(o)return o;await new Promise(a=>setTimeout(a,150))}return null},ne=(r,t)=>{let e=Me({contactId:t,mode:"claim"}),o=document.createElement("span");o.id=`${I}-wrap`,o.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,o.appendChild(e),r.appendChild(o)},Re=async()=>{let r=T();if(!r){document.getElementById(I)?.remove();return}let t=document.getElementById(I);if(!(t&&t.dataset.contactId===r.contactId)){t&&t.remove();try{let e=await F("#record-details-lhs",{timeoutMs:1e4}),o=await Pe(e),a=T();if(!a||a.contactId!==r.contactId)return;if(!o){h.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}ne(o,r.contactId),h.debug(`Mounted Claim/Release pill next to name for ${r.contactId}`);let s=null;s=new MutationObserver(async()=>{if(!(T()?.contactId===r.contactId)){s?.disconnect();return}if(document.getElementById(I))return;let n=ae(e);n&&(ne(n,r.contactId),h.debug("Re-mounted Claim/Release pill after React wipe"))}),s.observe(e,{childList:!0,subtree:!0})}catch(e){h.warn("Could not mount Claim/Release pill:",e)}}},se=()=>{Re()};var Oe="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set";var Be="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Ge=()=>{let r=window.WS_CRM_USER_ID;if(r&&/^[A-Za-z0-9]{15,30}$/.test(r))return r;let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let o of e)if(typeof o=="string"&&/^[A-Za-z0-9]{15,30}$/.test(o))return o;try{let o=localStorage.getItem("user");if(o){let a=JSON.parse(o),s=a.id??a._id;if(s&&/^[A-Za-z0-9]{15,30}$/.test(s))return s}for(let a of["tokenUser","currentUser","ghl_user"]){let s=localStorage.getItem(a);if(s)try{let i=JSON.parse(s),n=i.id??i._id;if(n&&/^[A-Za-z0-9]{15,30}$/.test(n))return n}catch{}}}catch{}return null},Ve=async({contactId:r,locationId:t,status:e,userId:o,deadReason:a})=>{let s={call_status:e,triggered_by_user_id:o??""};a&&(s.dead_reason=a);let n=await fetch(Oe,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:r,location:{id:t},customData:s}),credentials:"omit"});if(!n.ok)throw new Error(`Webhook returned ${n.status} ${n.statusText}`)},Ue=async({contactId:r,tenantId:t})=>{let e=await fetch(Be,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t,contact_id:r}),credentials:"omit"});if(!e.ok)throw new Error(`Sanity check returned ${e.status} ${e.statusText}`);return await e.json()};var G="ws-crm-contact-actions-bar",$e="ws-crm-call-status-dropdown",ie="ws-crm-offer-made-modal",ze=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],qe=[".central-panel","[class*='central-panel']"],We=async()=>{for(let r of qe)try{let t=await F(r,{timeoutMs:3e3});if(t)return t}catch{}try{return(await F("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},je=r=>{let t=document.createElement("div");t.id=$e,t.style.cssText="position: relative; flex-shrink: 0;";let e=document.createElement("button");e.type="button",e.style.cssText=L,e.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `;let o=document.createElement("div");return o.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,ze.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
    ...
    `,s.innerHTML=`
      <span style="
        ...
        height: 8px;
        border-radius: 50%;
        background: ${X[a.color]};
        flex-shrink: 0;
      "></span>
      <span>${a.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=c.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{h.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${r}`);let i=e.firstElementChild,n=e.children[1],l=e.children[2];i&&(i.style.background=X[a.color]),n&&(n.textContent=a.label),o.style.display="none";let p=T()?.locationId,m=Ge();if(!p){h.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=c.amber);return}if(a.color==="green"&&!m){h.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=c.amber);return}let d=l?.textContent??"\u25BE",w=async u=>{l&&(l.textContent="\u2026"),e.style.opacity="0.75";try{await Ve({contactId:r,locationId:p,status:a.label,userId:m,deadReason:u}),l&&(l.textContent="\u2713"),setTimeout(()=>{l&&(l.textContent=d),e.style.opacity="1"},900)}catch(v){h.warn("Call Status webhook failed:",v),l&&(l.textContent="\u2717"),n&&(n.style.color=c.amber),e.style.opacity="1",setTimeout(()=>{l&&(l.textContent=d),n&&(n.style.color=c.bone)},1800)}};if(a.value==="dead-deal"){let u=n?.textContent??"";Ye(v=>{w(v)},()=>{n&&(n.textContent=u)});return}if(a.value==="appointment-booked"){let u=n?.textContent??"";try{(await Ue({contactId:r,tenantId:p})).ok?await w():Xe(()=>{w()},()=>{n&&(n.textContent=u)})}catch(v){h.warn("Appointment sanity check failed; firing anyway:",v),await w()}return}await w()}),o.appendChild(s)}),e.addEventListener("click",a=>{a.stopPropagation(),o.style.display=o.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{t.contains(a.target)||(o.style.display="none")}),t.appendChild(e),t.appendChild(o),t},Ke=r=>{document.getElementById(ie)?.remove();let t=document.createElement("div");t.id=ie,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `;let o=new Date,a=new Date(o.getTime()+5*24*60*60*1e3).toISOString().slice(0,10);e.innerHTML=`
    <div style="
      font-family: ${f.mono};
      font-size: 10px;
  ...
  `,t.appendChild(e),document.body.appendChild(t);let s=()=>t.remove();t.addEventListener("click",i=>{i.target===t&&s()}),e.querySelector("#ws-offer-cancel")?.addEventListener("click",s),e.querySelector("#ws-offer-submit")?.addEventListener("click",()=>{let i=(e.querySelector("#ws-offer-amount")?.value??"").trim(),n=e.querySelector("#ws-offer-expires")?.value??"";if(!i||Number(i)<=0){h.warn("Offer Made submit blocked \u2014 invalid amount");return}h.info(`Offer Made recorded for contact ${r}: $${i}, expires ${n}`),s()}),setTimeout(()=>{e.querySelector("#ws-offer-amount")?.focus()},0)},le="ws-crm-dead-deal-modal",Ye=(r,t)=>{document.getElementById(le)?.remove();let e=document.createElement("div");e.id=le,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(o),document.body.appendChild(e);let a=()=>e.remove(),s=()=>{a(),t()};e.addEventListener("click",n=>{n.target===e&&s()}),o.querySelector("#ws-dead-cancel")?.addEventListener("click",s);let i=()=>{let n=o.querySelector("#ws-dead-reason"),l=o.querySelector("#ws-dead-error"),b=(n?.value??"").trim();if(!b){l&&(l.textContent="Please enter a reason."),n?.focus();return}if(b.length<4){l&&(l.textContent="Please enter at least a few words."),n?.focus();return}a(),r(b)};o.querySelector("#ws-dead-submit")?.addEventListener("click",i),o.querySelector("#ws-dead-reason")?.addEventListener("keydown",n=>{n.key==="Enter"&&(n.metaKey||n.ctrlKey)&&(n.preventDefault(),i()),n.key==="Escape"&&s()}),setTimeout(()=>{o.querySelector("#ws-dead-reason")?.focus()},0)},de="ws-crm-appointment-sanity-modal",Xe=(r,t)=>{document.getElementById(de)?.remove();let e=document.createElement("div");e.id=de,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(o),document.body.appendChild(e);let a=()=>e.remove(),s=()=>{a(),t()};e.addEventListener("click",n=>{n.target===e&&s()}),o.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",s),o.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{a(),r()});let i=n=>{n.key==="Escape"&&(document.removeEventListener("keydown",i),s())};document.addEventListener("keydown",i)},ce=r=>{let t=document.createElement("div");t.id=G,t.dataset.contactId=r,t.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    ...
    flex-shrink: 0;
  `,t.appendChild(je(r));let e=document.createElement("button");return e.type="button",e.style.cssText=C,e.textContent="Offer Made",e.addEventListener("click",()=>Ke(r)),t.appendChild(e),t},Je=async()=>{let r=T();if(!r){document.getElementById(G)?.remove();return}let t=document.getElementById(G);if(t&&t.dataset.contactId===r.contactId)return;t&&t.remove();let e=await We();if(!e){h.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let o=T();if(!o||o.contactId!==r.contactId)return;let a=ce(r.contactId);a.dataset.mountTarget=e.className||"central-panel",e.insertBefore(a,e.firstChild),h.debug(`Mounted contact actions bar for ${r.contactId}`);let s=null;s=new MutationObserver(()=>{let i=document.getElementById(G);if(!(T()?.contactId===r.contactId)){s?.disconnect();return}if(!i){let l=ce(r.contactId);l.dataset.mountTarget=a.dataset.mountTarget??"",e.insertBefore(l,e.firstChild),h.debug(`Re-mounted actions bar after React wipe for ${r.contactId}`)}}),s.observe(e,{childList:!0})},be=()=>{Je()};var V="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",Ze=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),D="data-ws-crm-favicon",J=r=>r.hasAttribute(D),he=r=>{if(r.tagName!=="LINK")return!1;let t=(r.getAttribute("rel")??"").toLowerCase();return Ze.has(t)},U=()=>{document.head.querySelectorAll("link[rel]").forEach(o=>{he(o)&&!J(o)&&o.remove()});let t=document.head.querySelector(`link[${D}="1"]`);t?t.href!==V&&(t.href=V):(t=document.createElement("link"),t.setAttribute(D,"1"),t.rel="icon",t.type="image/png",t.href=V,document.head.appendChild(t),h.debug("Favicon installed"));let e=document.head.querySelector(`link[${D}="apple"]`);e||(e=document.createElement("link"),e.setAttribute(D,"apple"),e.rel="apple-touch-icon",e.href=V,document.head.appendChild(e))},pe=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)U();else{let o=window.setInterval(()=>{document.head&&(window.clearInterval(o),U())},50)}new MutationObserver(o=>{let a=!1;for(let s of o)s.addedNodes.forEach(i=>{i instanceof HTMLLinkElement&&he(i)&&!J(i)&&(a=!0)}),s.removedNodes.forEach(i=>{i instanceof HTMLLinkElement&&J(i)&&(a=!0)});a&&U()}).observe(document.head,{childList:!0,subtree:!1});let t=0,e=window.setInterval(()=>{t+=1,U(),t>=5&&window.clearInterval(e)},1e3)};var j="reos-theme-stylesheet",Se="reos-theme-fonts",Qe=()=>{let r=document.getElementById(Se);r&&r.remove()},et=`
/* ============================================
...
/* 2-pre. v0.10.22: FIRST-PAINT GRAPHITE GUARD.
   The killWhiteInHighriseApp() JS coercer eliminates white bg
   inside Highrise app pages, but only runs after the script
   loads. There's a 50-200ms window where GHL paints white
   before the JS kicks in \u2014 Tim flagged the resulting flash.
   This CSS rule paints the OUTER Highrise app wrappers graphite
   on first paint, so even before JS runs, the visible bg is
   already dark. Uses :not(#__reos_never_id) chains to bump
   specificity above any Vue scoped CSS \u2014 and the universal
   descendant selector `*` makes EVERY child graphite by default.
   Later rules (section 19+ for input slate, section 7 for
   accent buttons) override where intentional. */
html body .platform-ui__highrise:not(#__reos_never_id),
html body .hr-wrapper-container:not(#__reos_never_id),
html body #inventory:not(#__reos_never_id),
html body #collections:not(#__reos_never_id),
html body #products:not(#__reos_never_id),
html body #reviews:not(#__reos_never_id),
html body .platform-ui__highrise:not(#__reos_never_id) > *:not(svg):not(path):not(input):not(textarea):not(select),
html body .hr-wrapper-container:not(#__reos_never_id) > *:not(svg):not(path):not(input):not(textarea):not(select) {
  background-color: var(--reos-graphite) !important;
  color: var(--reos-bone) !important;
}
/* Deeper descendants \u2014 paint .bg-white and .hr-data-table-wrapper
   family graphite with extra-aggressive specificity (multiple
   :not chains bump A+B beyond any Vue scoped CSS could reach). */
html body .platform-ui__highrise .bg-white:not(#__reos_never_id):not(#__reos_a):not(#__reos_b),
html body .platform-ui__highrise .hr-data-table-wrapper:not(#__reos_never_id):not(#__reos_a):not(#__reos_b),
html body .platform-ui__highrise .hr-data-table-wrapper-header:not(#__reos_never_id):not(#__reos_a):not(#__reos_b),
html body .platform-ui__highrise .hr-data-table-wrapper-footer:not(#__reos_never_id):not(#__reos_a):not(#__reos_b),
html body .platform-ui__highrise .hr-data-table:not(#__reos_never_id):not(#__reos_a):not(#__reos_b),
html body .platform-ui__highrise .hr-data-table__body:not(#__reos_never_id):not(#__reos_a):not(#__reos_b),
html body .platform-ui__highrise .hr-table-freezer-wrapper:not(#__reos_never_id):not(#__reos_a):not(#__reos_b),
html body .platform-ui__highrise .hr-table-header-container:not(#__reos_never_id):not(#__reos_a):not(#__reos_b),
html body .platform-ui__highrise [class*="hr-header-lite"]:not(#__reos_never_id):not(#__reos_a):not(#__reos_b) {
  background-color: var(--reos-graphite) !important;
  color: var(--reos-bone) !important;
}
...
`,tt=[{selector:".hr-input__input-el, .hr-input__textarea-el, [class*='hr-input'] input, [class*='hr-input'] textarea",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".hr-data-table-wrapper, .hr-data-table-wrapper-responsive, .hr-data-table-wrapper-header, .hr-data-table-wrapper-footer, .hr-data-table, .hr-data-table__body, .hr-table-freezer-wrapper, [class*='hr-data-table-wrapper'], [class*='hr-table-freezer']",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".mb-2\\.5.bg-white, div.bg-white, .bg-white.px-3, .mx-auto > .bg-white",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-data-table__header, .hr-data-table__cell-header, thead.hr-data-table__header",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:".hr-wrapper-container, .platform-ui__highrise, .full-screen.platform-ui__highrise, [class*='platform-ui__highrise'], .hl_topbar-tabs, .hl_wrapper",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-collapse-item__header, .hr-collapse-item__header-main, .hr-collapse-item__title",styles:{color:"#EDEEF0","background-color":"#12161D"}},{selector:".hr-collapse-item__content, .hr-collapse-item__content-inner, .hr-collapse-item__content-wrap",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-tabs, .hr-tabs-nav, .hr-tabs-nav-scroll-content, .hr-tabs-content, .hr-tabs-pane, .hr-tabs-rail, .hr-tabs-tab-wrapper",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-tabs-capsule",styles:{"background-color":"transparent",background:"transparent",border:"none"}},{selector:".hr-avatar, .hr-avatar__text, .hr-avatar__text p, .hr-avatar p, .hr-avatar span, .hr-avatar div, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']), [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']) span, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']) p, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']) div",styles:{color:"#0A0D12","-webkit-text-fill-color":"#0A0D12","font-weight":"600"}},{selector:".hr-button--tertiary",styles:{"background-color":"#1A1F28",color:"#0FB57E"}},{selector:".hr-text:not([role='button']):not(.cursor-pointer)",styles:{color:"#EDEEF0"}},{selector:"[class*='field-value'], [class*='field-display'], [class*='empty-value'], [class*='field-empty'], [class*='value-display'], [class*='display-value']",styles:{color:"#9098A3","-webkit-text-fill-color":"#9098A3"}},{selector:".hr-select__value, .hr-select__display, .hr-select-trigger, .hr-select-trigger__value, .hr-base-select__value, .hr-base-select__display, [class*='select__value'], [class*='select__display'], [class*='select-trigger']",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".hr-select, .hr-select__input, .hr-select__inner, .hr-select-trigger, .hr-base-select, .hr-base-select__inner, .el-select, .el-select__wrapper, .el-input__wrapper, .el-input__inner, [role='combobox'], [aria-haspopup='listbox']",styles:{"background-color":"#1A1F28","border-color":"#252C36"}},{selector:"input[type='date'], input[type='datetime-local'], input[type='time'], input[type='month'], input[type='week'], .hr-date-picker, .hr-date-input, .hr-datepicker, .hr-time-picker, [class*='date-picker'], [class*='datepicker'], [class*='date-input'], [class*='time-picker'], [class*='time-input']",styles:{"background-color":"#1A1F28","border-color":"#252C36",color:"#EDEEF0"}},{selector:"[class*='F7F9FD'], [class*='F7F9FB'], [class*='F9FAFB'], [class*='eff4ff'], [class*='EFF4FF'], [class*='ECEEF2'], [class*='eceef2']",styles:{"background-color":"#12161D"}},{selector:"[style*='background-color: white'], [style*='background-color:white'], [style*='background-color: #fff'], [style*='background-color:#fff'], [style*='background-color: #FFF'], [style*='background-color:#FFF'], [style*='background-color: #ffffff'], [style*='background-color:#ffffff'], [style*='background-color: #FFFFFF'], [style*='background-color:#FFFFFF'], [style*='background-color: rgb(255, 255, 255)'], [style*='background-color:rgb(255, 255, 255)'], [style*='background-color: rgb(255,255,255)'], [style*='background-color:rgb(255,255,255)']",styles:{"background-color":"#12161D"}},{selector:".chart-container.hl-card, [class*='chart-container'][class*='hl-card'], .dashboard-widget-type-custom .hl-card, .dashboard-widget-module-custom .hl-card",styles:{"background-color":"#12161D"}},{selector:".bg-gray-50, .bg-white, .bg-gray-100",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".n-base-select-menu, .n-popselect-menu, .n-popover, .n-dropdown, .n-dropdown-menu, .hr-base-select-menu, .hr-popselect-menu, .hr-popover, .hr-dropdown, .hr-dropdown-menu",styles:{"background-color":"#12161D",color:"#EDEEF0","border-color":"#252C36","--n-option-color":"transparent","--n-option-color-hover":"#1A1F28","--n-option-color-active":"rgba(15, 181, 126, 0.12)","--n-option-color-active-hover":"rgba(15, 181, 126, 0.12)","--n-option-text-color":"#EDEEF0","--n-option-text-color-hover":"#EDEEF0","--n-option-text-color-active":"#0FB57E","--n-option-text-color-disabled":"#9098A3","--n-color":"transparent","--n-color-hover":"#1A1F28","--n-color-active":"rgba(15, 181, 126, 0.12)","--n-text-color":"#EDEEF0","--n-text-color-hover":"#EDEEF0","--n-text-color-active":"#0FB57E","--n-text-color-disabled":"#9098A3"}},{selector:".n-dropdown-option--disabled, .n-dropdown-option--disabled .n-dropdown-option-body, .n-dropdown-option--disabled .n-dropdown-option-body__label, .n-dropdown-option--disabled .n-dropdown-option-body__prefix, .n-dropdown-option--disabled .n-dropdown-option-body__suffix, .n-dropdown-option[aria-disabled='true'], .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__label, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__prefix, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__suffix, .n-base-select-option--disabled, .n-base-select-option--disabled *, .n-base-select-option[aria-disabled='true'], .n-base-select-option[aria-disabled='true'] *, .hr-dropdown-option--disabled, .hr-dropdown-option--disabled *, .hr-dropdown-option[aria-disabled='true'], .hr-dropdown-option[aria-disabled='true'] *, .hr-base-select-option--disabled, .hr-base-select-option--disabled *, .hr-base-select-option[aria-disabled='true'], .hr-base-select-option[aria-disabled='true'] *",styles:{"background-color":"transparent",color:"#9098A3","-webkit-text-fill-color":"#9098A3"}},{selector:".hl-card-header [role='heading'], .hl-card-header p[role='heading'], [class*='hl-card-header'] [role='heading']",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".n-button:not(.n-button--primary-type):not(.n-button--error-type):not(.n-button--warning-type):not(.n-button--success-type):not(.n-button--info-type)",styles:{"background-color":"#1A1F28"}}],ot=r=>{if(!r||r[0]!=="#")return r;let t=r.slice(1);if(t.length===3&&(t=t[0]+t[0]+t[1]+t[1]+t[2]+t[2]),t.length!==6&&t.length!==8)return r;let e=parseInt(t.slice(0,2),16),o=parseInt(t.slice(2,4),16),a=parseInt(t.slice(4,6),16);if(isNaN(e)||isNaN(o)||isNaN(a))return r;if(t.length===8){let s=parseInt(t.slice(6,8),16)/255;return`rgba(${e}, ${o}, ${a}, ${s})`}return`rgb(${e}, ${o}, ${a})`},rt=new Set(["color","background-color","background","border-color","border-top-color","border-right-color","border-bottom-color","border-left-color","fill","stroke","-webkit-text-fill-color","caret-color","outline-color"]),nt=["#inventory","#collections","#products","#reviews","[class*='platform-ui__highrise']",".hr-wrapper-container"],H=()=>{document.querySelectorAll(nt.join(", ")).forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']"))return;let e=t.querySelectorAll("*");[t,...Array.from(e)].forEach(o=>{if(o.id?.startsWith("ws-crm-")||o.tagName==="INPUT"||o.tagName==="TEXTAREA"||o.tagName==="SELECT"||o.tagName==="SCRIPT"||o.tagName==="STYLE"||o.tagName==="SVG"||o.tagName==="PATH")return;let s=window.getComputedStyle(o).backgroundColor;if(!s||s==="rgba(0, 0, 0, 0)"||s==="transparent")return;let i=s.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!i)return;let n=+i[1],l=+i[2],b=+i[3];n<230||l<230||b<230||(o.style.setProperty("background-color","#12161D","important"),o.style.setProperty("color","#EDEEF0","important"))})})},N=()=>{for(let r of tt){let t;try{t=document.querySelectorAll(r.selector)}catch{continue}t.forEach(e=>{for(let[o,a]of Object.entries(r.styles)){let s=e.style.getPropertyPriority(o),i=e.style.getPropertyValue(o),n=rt.has(o)?ot(a):a;s==="important"&&i===n||e.style.setProperty(o,a,"important")}})}},Q="#1A1F28",at=26,st=31,it=40,me=new Set,_e=new Set,$=()=>{document.body.querySelectorAll("*").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']")||t.children.length>0||(t.textContent||"").trim()!=="--")return;let o=t.getBoundingClientRect();if(o.width===0||o.height===0||o.width>800)return;t.style.setProperty("display","none","important");let a=t.className?.toString()||"<no class>";!_e.has(a)&&a!=="<no class>"&&(_e.add(a),h.debug(`Hid empty "--" leaf: "${a}"`))})},lt=["M3 6","M5 6","M2 6","M6 6","M9 3h6","M16 6v","M19 6v","M5 6V","M16.5 4","48.816","2.991","14.74 9","9.26 9","2.244","M19 4h-","M19 7l-"],dt=["trash","Trash","TRASH","delete","Delete","DELETE"],ct=()=>{document.querySelectorAll("button.hr-button, button.n-button, button.icon-only, .hr-button.icon-only, .n-button.icon-only").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']")||t.getAttribute("data-reos-trash")==="true")return;let e=t.querySelector("svg");if(!e)return;let o=!1,a=(e.getAttribute("class")||"")+" "+(e.getAttribute("data-icon")||"");if(dt.some(s=>a.includes(s))&&(o=!0),o||e.querySelectorAll("path").forEach(i=>{if(o)return;let n=i.getAttribute("d")||"";lt.some(l=>n.includes(l))&&(o=!0)}),!o){let s=(t.id||"").toLowerCase(),i=(t.getAttribute("aria-label")||"").toLowerCase(),n=(t.getAttribute("title")||"").toLowerCase();(s.includes("delete")||s.includes("remove")||s.includes("trash")||i.includes("delete")||i.includes("remove")||i.includes("trash")||n.includes("delete")||n.includes("remove")||n.includes("trash"))&&(o=!0)}o&&t.setAttribute("data-reos-trash","true")})},bt=new Set(["Delete Reminder","Delete","Remove","Trash","Edit","Copy","Duplicate","Add another reminder","Add","Close","Dismiss"]),ht=()=>{document.querySelectorAll(".hr-tooltip, .hr-popover-shared, [role='tooltip']").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']"))return;let e=(t.textContent||"").trim();e.length===0||e.length>40||bt.has(e)&&t.style.display!=="none"&&(t.style.setProperty("display","none","important"),t.style.setProperty("visibility","hidden","important"),t.style.setProperty("opacity","0","important"),t.style.setProperty("pointer-events","none","important"))})},ve=new Set,pt=()=>{document.body.querySelectorAll("*").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']"))return;let e=(t.className?.toString()||"").toLowerCase();if(e.includes("ai-button")||e.includes("ask-ai")||e.includes("bg-clip-text")||e.includes("text-transparent")||t.closest("[class*='ai-button'], [class*='ask-ai'], [class*='bg-clip-text'], [class*='text-transparent']"))return;let o=t.tagName.toLowerCase();if(o==="svg"||o==="path"||o==="circle"||o==="rect"||o==="polygon"||o==="polyline"||o==="line"||o==="g")return;let a=!1;for(let m of Array.from(t.childNodes))if(m.nodeType===Node.TEXT_NODE&&(m.textContent||"").trim()){a=!0;break}if(!a)return;let s=Array.from(t.children);if(s.length>0&&s.every(m=>{let d=m.tagName.toLowerCase();return d==="svg"||d==="img"})&&!a)return;let l=window.getComputedStyle(t).color;if(!(l==="rgb(255, 255, 255)"||l==="rgba(255, 255, 255, 1)"||l==="#ffffff"||l==="white"))return;t.style.setProperty("color","#EDEEF0","important"),t.style.setProperty("-webkit-text-fill-color","#EDEEF0","important");let p=t.className?.toString()||"<no class>";!ve.has(p)&&p!=="<no class>"&&(ve.add(p),h.debug(`Remapped pure-white text -> bone: "${p}"`))})},ue=new Set,mt=()=>{document.querySelectorAll(".hl_header, header.hl_header, [class*='topbar'], [class*='top-bar'], header.app-header").forEach(t=>{t.querySelectorAll("button, a, [role='button'], [class*='rounded-full'], [class*='rounded-md'], [class*='rounded-xl'], [class*='pill'], span[class*='rounded'], div[class*='rounded']").forEach(o=>{if(o.closest("[id^='ws-crm-']"))return;let a=(o.className?.toString()||"").toLowerCase();if(a.includes("avatar"))return;let s=window.getComputedStyle(o),i=s.backgroundColor,n=s.backgroundImage||"",l=!1;if(n!=="none"&&n.includes("gradient"))l=!0;else if(i&&i!=="rgba(0, 0, 0, 0)"&&i!=="transparent"){let y=i.match(/rgba?\(\d+,\s*\d+,\s*\d+,\s*([\d.]+)\)/);if(y&&parseFloat(y[1])<.6)return;let g=i.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(g){let E=parseInt(g[1],10),S=parseInt(g[2],10),K=parseInt(g[3],10);(E>=60||S>=60||K>=60)&&(E>230&&S>230&&K>230||(E>=80||S>=80||K>=80)&&(l=!0))}}if(!l)return;let p=(o.textContent||"").trim(),m=a,d=Array.from(o.querySelectorAll("*")).map(y=>(y.className?.toString()||"").toLowerCase()).join(" "),w=p.toLowerCase()==="ask ai"||m.includes("ai-button")||m.includes("ask-ai")||m.includes("askai"),u=m.includes("bg-clip-text")||m.includes("text-transparent")||d.includes("bg-clip-text")||d.includes("text-transparent"),v=Array.from(o.querySelectorAll("*")).some(y=>{let g=window.getComputedStyle(y),E=g.getPropertyValue("-webkit-background-clip")||"",S=g.getPropertyValue("background-clip")||"";return E.includes("text")||S.includes("text")});if(w||u||v){let y=["color","-webkit-text-fill-color","fill","stroke","background","background-color","background-image","background-clip","-webkit-background-clip","text-shadow"],g=E=>{y.forEach(S=>E.style.removeProperty(S))};g(o),o.querySelectorAll("*").forEach(g);return}let _="#12161D";o.style.setProperty("color",_,"important"),o.querySelectorAll("*").forEach(y=>{let g=y.tagName.toLowerCase();g==="svg"||g==="path"||g==="circle"||g==="rect"||g==="polygon"||g==="polyline"||g==="line"||g==="g"?(y.style.setProperty("fill",_,"important"),y.style.setProperty("stroke",_,"important"),y.style.setProperty("color",_,"important")):(y.style.setProperty("color",_,"important"),y.style.setProperty("-webkit-text-fill-color",_,"important"))});let x=o.className?.toString()||"<no class>";!ue.has(x)&&x!=="<no class>"&&(ue.add(x),h.debug(`Darkened SVG icons on colored button: "${x}"`))})})},ye=new Set,_t=()=>{document.querySelectorAll("[class*='avatar']").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;let e=t.getBoundingClientRect();if(e.width===0||e.height===0||e.width<20||e.width>64||e.height<20||e.height>64)return;let o=e.top+e.height/2;(t.parentElement||t).querySelectorAll("svg, img").forEach(n=>{let l=n;if(l.closest("[id^='ws-crm-']"))return;let b=n.getBoundingClientRect();if(b.width===0||b.width>28||b.height>28)return;let p=b.left<e.right&&b.right>e.left,m=b.top<e.bottom&&b.bottom>e.top;if(!(p&&m)||b.top+b.height/2<o)return;let w=b.width*b.height/(e.width*e.height);if(w>.55)return;let u=l,v=l.parentElement,_=0;for(;v&&v!==t&&_<3;){let y=v.getBoundingClientRect();if(y.width>0&&y.width<=30&&y.height<=30)u=v;else break;v=v.parentElement,_++}u.style.setProperty("transform","translateY(11px)","important");let x=(u.className?.toString()||"")+" "+u.tagName;ye.has(x)||(ye.add(x),h.debug(`Dropped activity badge 11px (${b.width.toFixed(0)}x${b.height.toFixed(0)} icon, ratio ${w.toFixed(2)}): "${x}"`))})})},vt="brightness(0) saturate(100%) invert(54%) sepia(78%) saturate(442%) hue-rotate(122deg) brightness(96%) contrast(89%)",ut=()=>{document.querySelectorAll("aside.default-bg-color a, [class*='sidebar'] a").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;let e=(t.className?.toString()||"").toLowerCase();e.includes("active")||e.includes("router-link-active")?(t.style.setProperty("color","#0FB57E","important"),t.style.setProperty("-webkit-text-fill-color","#0FB57E","important"),t.style.setProperty("opacity","1","important"),t.querySelectorAll("span, p, div").forEach(a=>{a.style.setProperty("color","#0FB57E","important"),a.style.setProperty("-webkit-text-fill-color","#0FB57E","important")}),t.querySelectorAll("img").forEach(a=>{a.style.setProperty("filter",vt,"important")}),t.querySelectorAll("svg").forEach(a=>{a.style.setProperty("color","#0FB57E","important"),a.style.setProperty("stroke","#0FB57E","important")})):((t.style.color==="rgb(18, 22, 29)"||t.style.color==="#12161D"||t.style.color==="#0FB57E"||t.style.color==="rgb(15, 181, 126)")&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color")),t.querySelectorAll("span, p, div").forEach(a=>{let s=a.style.color;(s==="rgb(18, 22, 29)"||s==="#12161D"||s==="#0FB57E"||s==="rgb(15, 181, 126)")&&(a.style.removeProperty("color"),a.style.removeProperty("-webkit-text-fill-color"))}),t.querySelectorAll("img").forEach(a=>{a.style.filter&&a.style.filter.includes("hue-rotate(122")&&a.style.removeProperty("filter")}))})},ge=new Set,yt=()=>{document.querySelectorAll(".n-dialog, [class*='n-dialog'], [class*='dialog'], [role='dialog']").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;t.querySelectorAll("div, span, i").forEach(o=>{if(o.closest("[id^='ws-crm-']"))return;let a=o.getBoundingClientRect();if(a.width===0||a.height===0||a.width>56||a.height>56||!o.querySelector("svg"))return;let i=window.getComputedStyle(o).backgroundColor.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!i)return;let n=parseInt(i[1],10),l=parseInt(i[2],10),b=parseInt(i[3],10);if(!(n>200&&l>200&&b>200))return;o.style.setProperty("background-color","transparent","important"),o.style.setProperty("background","transparent","important"),o.style.setProperty("box-shadow","none","important"),o.style.setProperty("border","none","important");let p=o.className?.toString()||"<no class>";!ge.has(p)&&p!=="<no class>"&&(ge.add(p),h.debug(`Stripped dialog icon circle (${a.width.toFixed(0)}x${a.height.toFixed(0)}, rgb(${n},${l},${b})): "${p}"`))})})},fe=new Set,gt=()=>{document.querySelectorAll("[class*='ai-button'], [class*='ask-ai'], [class*='askai']").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;let e=t.parentElement,o=0;for(;e&&o<3&&!e.closest("[id^='ws-crm-']");){let a=(e.className?.toString()||"").toLowerCase();if(!(a.includes("ai-button")||a.includes("ask-ai")||a.includes("askai"))){let i=e.getBoundingClientRect();if(i.width>0&&i.height>0&&i.width<=400&&i.height<=80){e.style.setProperty("background-color","#12161D","important"),e.style.setProperty("background-image","none","important");let n=e.className?.toString()||"<no class>";!fe.has(n)&&n!=="<no class>"&&(fe.add(n),h.debug(`Painted Ask AI wrapper graphite: "${n}"`))}}e=e.parentElement,o++}})},we=new Set,ft=()=>{let r=Array.from(document.querySelectorAll("#contact-conversation-panel, [class*='conversation-panel'], [class*='message-list'], [class*='activity-log'], .hl_header, header.hl_header, [class*='topbar'], [class*='top-bar'], header.app-header, .modal, .modal-content, [class*='modal'], [class*='dialog'], [role='dialog'], [class*='popover'], [class*='popup'], [class*='dropdown-menu'], [class*='select-menu'], [class*='popselect'], [class*='base-select'], .n-base-select-menu, .n-popselect, .n-popover")),t=document.querySelectorAll("div, section, aside"),e=[];t.forEach(a=>{if(a.id?.startsWith("ws-crm-")||a.closest("[id^='ws-crm-']"))return;let s=window.getComputedStyle(a);if(s.position!=="fixed"&&s.position!=="absolute")return;let i=parseInt(s.zIndex,10);if(!isFinite(i)||i<50)return;let n=a.getBoundingClientRect();n.width<240||n.height<120||n.width>1500||n.height>900||e.push(a)}),[...r,...e].forEach(a=>{a.querySelectorAll("*").forEach(i=>{if(i.id?.startsWith("ws-crm-")||i.closest("[id^='ws-crm-']"))return;let n=(i.className?.toString()||"").toLowerCase();if(n.includes("avatar")||n.includes("notification-dot")||n.includes("indicator"))return;if(n.includes("sidebar-option-button")||i.closest(".sidebar-option-button")||i.closest("nav[class*='w-13']")){let _=i.getAttribute("style")||"";(_.includes("background-color")||_.includes("color"))&&(i.style.removeProperty("background-color"),i.style.removeProperty("color"));return}if(n.includes("tabulator")||n.includes("tabulator-page")||i.closest("[class*='tabulator']")){(i.getAttribute("style")||"").includes("background-color")&&i.style.removeProperty("background-color");return}if(n.includes("hr-card")||i.closest(".hr-card")||i.id&&i.id.startsWith("opportunity-")){let _=i.getAttribute("style")||"";(_.includes("background-color")||_.includes("color"))&&(i.style.removeProperty("background-color"),i.style.removeProperty("color"));return}if(n.includes("ai-button")||n.includes("ask-ai")||n.includes("bg-clip-text")||n.includes("text-transparent")||i.closest("[class*='ai-button'], [class*='ask-ai'], [class*='bg-clip-text'], [class*='text-transparent']"))return;let l=window.getComputedStyle(i).backgroundColor;if(!l||l==="rgba(0, 0, 0, 0)"||l==="transparent")return;let b=l.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!b)return;let p=parseInt(b[1],10),m=parseInt(b[2],10),d=parseInt(b[3],10);if(!(p>200&&m>200&&d>200))return;let u=i.getBoundingClientRect();if(u.width<24||u.height<16||u.width>1400)return;i.style.setProperty("background-color","#12161D","important"),i.style.setProperty("color","#EDEEF0","important");let v=i.className?.toString()||"<no class>";!we.has(v)&&v!=="<no class>"&&(we.add(v),h.debug(`Coerced light bg (${u.width.toFixed(0)}x${u.height.toFixed(0)}, rgb(${p},${m},${d})): "${v}"`))})})},xe=new Set,wt=()=>{let r="input, select, textarea, [class*='hr-input'], [class*='hr-select'], [class*='hr-base-select'], [class*='hr-date-picker'], [class*='hr-datepicker']";document.body.querySelectorAll("*").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.children.length>0)return;let o=(e.textContent||"").trim();if(o.length===0||o.length>80||o==="--"||e.closest(r)||e.closest("[class*='avatar'], [class*='hr-avatar']")||e.closest("[class*='collapse-item__header'], [class*='hr-collapse-item__header'], [class*='collapse-header'], [class*='folder-header']")||e.closest("[class*='hr-tabs-tab'], [class*='tab-label'], [class*='tabs-nav'], [role='tab']"))return;if(e.matches("[role='heading']")||e.closest("[role='heading']")||e.matches("h1, h2, h3")||e.closest(".hl-card-header, [class*='hl-card-header'], [class*='card-header']")){e.matches("h1, h2, h3")&&(e.getAttribute("style")||"").includes("color")&&(e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color"));return}if(e.closest(".hr-alert, .hr-alert-body, [class*='hr-alert']")){let _=e.getAttribute("style")||"";(_.includes("color")||_.includes("background"))&&(e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color"),e.style.removeProperty("background-color"));return}if(e.closest("#SettingTexasSmsBlock")||e.matches("p.ui-text-sm-medium")&&e.parentElement?.classList?.contains("flex")&&e.parentElement?.classList?.contains("items-center")&&e.parentElement?.children?.length===1){(e.getAttribute("style")||"").includes("color")&&(e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color"));return}if(e.closest(".hr-button, .n-button")||e.matches(".hr-button__content, .n-button__content")||e.closest(".hr-button__content, .n-button__content")){(e.getAttribute("style")||"").includes("color")&&(e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color"));return}if(e.closest(".bg-primary-50, [class*='bg-primary-50']")){(e.getAttribute("style")||"").includes("color")&&(e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color"));return}let a=e,s=null;for(let _=0;_<4&&a;_++){let x=a.nextElementSibling;if(x&&(x.matches(r)||x.querySelector(r)!==null)){s=x;break}a=a.parentElement}if(!s)return;let i=e.getBoundingClientRect();if(i.width===0||i.height===0||i.width>400||i.height>60)return;e.style.setProperty("color","#9098A3","important"),e.style.setProperty("-webkit-text-fill-color","#9098A3","important");let n=s,l=n.getBoundingClientRect(),b=n.className?.toString()||"",p=b.includes("schedule-ui-form-weekly-schedule-checkbox")||n.closest(".schedule-ui-form-weekly-schedule-checkbox")!==null,m=b.includes("hl-form-item")||b.includes("n-form-item--top-labelled")||n.closest(".hl-form-item")!==null,d=".hr-input, .hr-input-number, .hr-base-select, .hr-select, .hr-date-picker, .hr-datepicker",w=(b.includes("hr-form-item-blank")||b.includes("hr-form-item")||n.matches(d))&&(n.matches(d)||n.querySelector(d)!==null);w&&(n.getAttribute("style")||"").includes("background-color")&&(n.style.removeProperty("background-color"),n.style.removeProperty("border-color"),n.style.removeProperty("border-style"),n.style.removeProperty("border-width"),n.style.removeProperty("border-radius"));let u=b.includes("hr-radio")||n.closest(".hr-radio, [class*='hr-radio']")!==null;u&&(n.getAttribute("style")||"").includes("background-color")&&(n.style.removeProperty("background-color"),n.style.removeProperty("border-color"),n.style.removeProperty("border-style"),n.style.removeProperty("border-width"),n.style.removeProperty("border-radius")),!p&&!m&&!w&&!u&&l.width>0&&l.width<=800&&l.height>0&&l.height<=100&&(n.style.setProperty("background-color",Q,"important"),n.style.setProperty("border-color","#252C36","important"),n.style.setProperty("border-style","solid","important"),n.style.setProperty("border-width","1px","important"),n.style.setProperty("border-radius","4px","important"));let v=e.className?.toString()||"<no class>";!xe.has(v)&&v!=="<no class>"&&(xe.add(v),h.debug(`Colored field label "${o.slice(0,30)}": "${v}"`))})},ke=new Set,xt=()=>{document.querySelectorAll("[class*='avatar']").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;let e=(t.className?.toString()||"").toLowerCase();if(e.includes("dropdown")||e.includes("option-avatar"))return;let o=t.getBoundingClientRect();if(o.width===0||o.height===0||o.height>80||t.querySelector("svg, img")!==null)return;let s=(t.textContent||"").trim();if(s.length===0||s.length>4)return;let i=Math.min(o.width,o.height),n=Math.max(16,Math.min(24,Math.round(i*.6)));t.style.setProperty("font-size",`${n}px`,"important"),t.style.setProperty("line-height","1","important"),t.style.setProperty("display","flex","important"),t.style.setProperty("align-items","center","important"),t.style.setProperty("justify-content","center","important"),t.style.setProperty("text-align","center","important"),t.querySelectorAll("*").forEach(b=>{b.tagName==="svg"||b.tagName==="SVG"||b.tagName!=="IMG"&&(b.style.setProperty("font-size",`${n}px`,"important"),b.style.setProperty("line-height","1","important"),b.style.setProperty("text-align","center","important"))});let l=t.className?.toString()||"<no class>";!ke.has(l)&&l!=="<no class>"&&(ke.add(l),h.debug(`Resized avatar text to ${n}px (circle ${i.toFixed(0)}px): "${l}"`))})},Ee=new Set,kt=()=>{document.querySelectorAll("input:not([type='checkbox']):not([type='radio']):not([type='button']):not([type='submit']):not([type='reset']):not([type='hidden']):not([type='file']), textarea, select").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;let e=t.getBoundingClientRect();if(e.width===0||e.height===0)return;t.style.setProperty("background-color",Q,"important"),t.style.setProperty("border-color","#252C36","important"),t.style.setProperty("color","#EDEEF0","important"),t.style.setProperty("-webkit-text-fill-color","#EDEEF0","important");let o=t.className?.toString()||"<no class>";!Ee.has(o)&&o!=="<no class>"&&(Ee.add(o),h.debug(`Forced input slate bg: "${o}"`))})},z=()=>{document.body.querySelectorAll("*").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']")||t.closest("[class*='collapse-item__header'], [class*='hr-collapse-item__header'], [class*='collapse-header'], [class*='folder-header']")||t.id==="chat-filter"||t.id==="phone-calls"||t.id==="archive-conversation"||t.id==="star-toggle"||t.id==="read-toggle"||t.id==="delete-conversation")return;if(t.closest(".hr-tooltip, .hr-popover-shared, .hr-popover, [class*='hr-tooltip'], [class*='hr-popover']")){let d=t.getAttribute("style")||"";(d.includes("background-color")||d.includes("color"))&&(t.style.removeProperty("background-color"),t.style.removeProperty("color"));return}let e=t.className?.toString()||"";if(e.includes("bg-inherit")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(t.closest(".pipeline-ribbon")&&e.split(" ").includes("count")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(e.split(" ").includes("tabulator-page")){let d=t.getAttribute("style")||"";(d.includes("background-color")||d.includes("color"))&&(t.style.removeProperty("background-color"),t.style.removeProperty("color"));return}if(["tabulator-cell","tabulator-row","tabulator-col","tabulator-header","tabulator-tableholder","tabulator-table"].some(d=>e.includes(d))){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(e.includes("schedule-ui-form-weekly-schedule-checkbox")){let d=t.getAttribute("style")||"";(d.includes("background-color")||d.includes("border-color")||d.includes("border-style")||d.includes("border-width"))&&(t.style.removeProperty("background-color"),t.style.removeProperty("border-color"),t.style.removeProperty("border-style"),t.style.removeProperty("border-width"));return}if(e.includes("n-button")&&e.includes("icon-only")&&!e.includes("quaternary")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(e.includes("hr-button--tertiary-type")||e.includes("n-button--tertiary-type")){let d=t.getAttribute("style")||"";(d.includes("background-color")||d.includes("color"))&&(t.style.removeProperty("background-color"),t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"),t.style.removeProperty("border-color"),t.style.removeProperty("border-style"),t.style.removeProperty("border-width"),t.style.removeProperty("border-radius"));return}if(e.includes("hr-data-table-td")||e.includes("hr-data-table-tr")||e.includes("hr-data-table__body-cell")||e.includes("hr-data-table__cell")||e.includes("hr-data-table__header-cell")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(e.includes("n-data-table-td--fixed-right")||e.includes("n-data-table-td--last-col")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(t.classList.contains("hr-tabs")||e.includes("hr-tabs-tab-wrapper")||e.includes("hr-tabs-nav")||e.includes("hr-tabs-nav-scroll-content")||e.includes("hr-tabs-content")||e.includes("hr-tabs-pane")||e.includes("hr-tabs-rail")||e.includes("hr-tabs-wrapper")||e.includes("hr-tabs-pane-wrapper")||e.includes("hr-tab-pane")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(e.includes("hr-date-panel")||e.includes("n-date-panel")||t.closest(".hr-date-panel, .n-date-panel")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(e.includes("hr-tag")&&e.includes("ui-default")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if((e.includes("bg-purple-50")||e.includes("bg-purple-100"))&&(e.includes("rounded-lg")||e.includes("rounded-md")||e.includes("rounded-full"))){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(e.includes("n-alert")||t.closest(".n-alert")||e.includes("hl-warning")||e.includes("hl-error")||e.includes("hl-info")||e.includes("hl-success")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(e.includes("hr-card")||t.closest(".hr-card")||t.id&&t.id.startsWith("opportunity-")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(e.includes("bg-gray-50")||e.includes("bg-gray-100")||e.includes("bg-gray-200")||e.includes("bg-white")||e.includes("bg-blue-50")||e.includes("bg-blue-100")||e.includes("bg-sky-50")||e.includes("bg-indigo-50")||e.includes("bg-slate-50")||e.includes("bg-neutral-50")||e.includes("bg-zinc-50")||e.includes("bg-stone-50")||e.includes("F7F9FD")||e.includes("F9FAFB")||e.includes("ECEEF2")||e.includes("eceef2")||e.includes("EFF4FF")||e.includes("eff4ff")){(t.getAttribute("style")||"").includes("background-color")&&t.style.removeProperty("background-color");return}if(e.includes("hr-collapse-item")||e.includes("hr-collapse hr-accordion")||e.includes("hr-accordion"))return;if(e.includes("sidebar-option-button")||t.closest(".sidebar-option-button")||t.closest("nav[class*='w-13']")){let d=t.getAttribute("style")||"";(d.includes("background-color")||d.includes("color"))&&(t.style.removeProperty("background-color"),t.style.removeProperty("color"));return}if(t.closest(".inbox-panel, .conversation-list-container, [class*='conversation-list-container']")){let d=t.getAttribute("style")||"";(d.includes("background-color")||d.includes("color"))&&(t.style.removeProperty("background-color"),t.style.removeProperty("color"));return}let a=window.getComputedStyle(t).backgroundColor;if(!a||a==="rgba(0, 0, 0, 0)"||a==="transparent")return;let s=a.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!s)return;let i=parseInt(s[1],10),n=parseInt(s[2],10),l=parseInt(s[3],10);if(!(i<at&&n<st&&l<it))return;let p=t.getBoundingClientRect();if(p.width===0||p.height===0||p.width>800||p.height>200)return;t.style.setProperty("background-color",Q,"important");let m=t.className?.toString()||"<no class>";!me.has(m)&&m!=="<no class>"&&(me.add(m),h.debug(`Coerced dark bg to slate (${p.width.toFixed(0)}x${p.height.toFixed(0)}, rgb(${i},${n},${l})): "${m}"`))})},q=()=>{let r="#4B8BF5",t=["#155EEF","#155eef","#004EEB","#004eeb","#175CD3","#175cd3"],e="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";document.querySelectorAll("img[src^='data:image/svg+xml;base64,']:not([data-reos-recolored]), .toolbar_button img[src^='data:image/svg+xml;base64,']:not([data-reos-recolored-v2]), #editor-toolbar img[src^='data:image/svg+xml;base64,']:not([data-reos-recolored-v2])").forEach(s=>{s.setAttribute("data-reos-recolored","1"),s.setAttribute("data-reos-recolored-v2","1");let n=(s.getAttribute("src")||"").match(/^data:image\/svg\+xml;base64,(.+)$/);if(!n)return;let l;try{l=atob(n[1])}catch{return}let b=l,p=!1;if(t.forEach(d=>{b.includes(d)&&(b=b.split(d).join(r),p=!0)}),s.closest(".toolbar_button, .editor-toolbar, [class*='toolbar']")!==null&&["#000000","#000","#040E13","#040e13","#111827","#111","#101828","#101"].forEach(w=>{b.includes(w)&&(b=b.split(w).join("#EDEEF0"),p=!0)}),!!p)try{let d=btoa(b);s.setAttribute("src",`data:image/svg+xml;base64,${d}`)}catch{}}),document.querySelectorAll(".hl-checkbox-group-item img:not([data-reos-masked]):not([data-reos-recolored]), .hl-checkbox-group-item-icon img:not([data-reos-masked]):not([data-reos-recolored]), .hl-checkbox-group-item-container img:not([data-reos-masked]):not([data-reos-recolored])").forEach(s=>{let i=s.getAttribute("src")||"";if(!i||i.startsWith("data:")||!/\.svg(?:[?#]|$)/i.test(i))return;s.setAttribute("data-reos-masked","1");let n=`url("${i}")`;s.style.setProperty("-webkit-mask-image",n,"important"),s.style.setProperty("-webkit-mask-size","contain","important"),s.style.setProperty("-webkit-mask-repeat","no-repeat","important"),s.style.setProperty("-webkit-mask-position","center","important"),s.style.setProperty("mask-image",n,"important"),s.style.setProperty("mask-size","contain","important"),s.style.setProperty("mask-repeat","no-repeat","important"),s.style.setProperty("mask-position","center","important"),s.style.setProperty("background-color","var(--reos-blue)","important"),s.setAttribute("src",e)})},W=()=>{document.querySelectorAll("[class*='n-date-panel']").forEach(t=>{[...t.classList].forEach(e=>{if(e.startsWith("n-date-panel")){let o="hr-date-panel"+e.slice(12);t.classList.contains(o)||t.classList.add(o)}})})},Te=()=>{let r,t=()=>{r===void 0&&(r=window.setTimeout(()=>{r=void 0,W(),N(),kt(),z(),$(),wt(),ft(),gt(),yt(),ut(),mt(),xt(),_t(),pt(),q(),ht(),ct(),H()},100))};W(),N(),z(),$(),q(),H(),window.setTimeout(()=>{W(),N(),z(),$(),q(),H()},500),window.setTimeout(()=>{W(),N(),z(),$(),q(),H()},1500),typeof requestAnimationFrame=="function"&&requestAnimationFrame(()=>{H(),N()}),new MutationObserver(()=>{t()}).observe(document.body,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["class","style"]})},Z,Et=()=>{let r=()=>{let e=document.getElementById(j);!e||!document.head||document.head.lastElementChild===e||document.head.appendChild(e)};new MutationObserver(e=>{let o=!1;for(let a of e){for(let s of Array.from(a.addedNodes)){if(!(s instanceof Element))continue;let i=s.tagName.toLowerCase();if(i==="style"||i==="link"){if(s.id===j||s.id===Se)continue;o=!0;break}}if(o)break}o&&Z===void 0&&(Z=window.setTimeout(()=>{Z=void 0,r()},150))}).observe(document.head,{childList:!0}),r()},Ae=()=>{if(Qe(),!document.getElementById(j)){let r=document.createElement("style");r.id=j,r.textContent=et,document.head.appendChild(r),h.info("REInvest OS theme stylesheet mounted")}window.__reosForceInstalled||(window.__reosForceInstalled=!0,document.body?Te():window.addEventListener("DOMContentLoaded",Te)),window.__reosCascadeInstalled||(window.__reosCascadeInstalled=!0,Et())};var Fe="ws-crm-customizer-version-badge",Ce=()=>{if(document.getElementById(Fe))return;let r=document.createElement("div");r.id=Fe,r.style.cssText=`
    position: fixed;
    bottom: 12px;
    right: 12px;
    z-index: 999999;
  ...
  `,r.textContent=`Customizer v${O}`,r.title=`Built ${B}
Click to dismiss for this session`,r.addEventListener("mouseenter",()=>{r.style.transform="scale(1.05)"}),r.addEventListener("mouseleave",()=>{r.style.transform="scale(1)"}),r.addEventListener("click",()=>{r.style.opacity="0",setTimeout(()=>r.remove(),200)}),document.body.appendChild(r)};var Le=()=>{h.info(`Loaded v${O} (built ${B})`),pe(),Ae(),oe(),te(r=>{h.debug("Page handler firing for:",r),Ce(),se(),be()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Le):Le();})();
