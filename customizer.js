...
`;var Gr=/^[A-Za-z0-9]{15,30}$/,ke=t=>typeof t=="string"&&Gr.test(t),zr=t=>{try{let e=t.split(".");if(e.length!==3)return null;let o=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=o+"=".repeat((4-o.length%4)%4);return JSON.parse(atob(r))}catch{return null}},Ur=()=>{let t=window.WS_CRM_USER_ID;return ke(t)?t:null},qr=()=>{let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let o of e)if(ke(o))return o;return null},Vr=()=>{try{let t=localStorage.getItem("refreshedToken");if(!t)return null;let e=t.startsWith('"')?t.slice(1,-1):t,o=zr(e);if(!o)return null;let r=o.uid;if(ke(r))return r;let n=o.claims;if(n&&ke(n.user_id))return n.user_id}catch{}return null},Wr=()=>{try{let t=["user","tokenUser","currentUser","ghl_user"];for(let e of t){let o=localStorage.getItem(e);if(o)try{let r=JSON.parse(o),n=r.id??r._id;if(ke(n))return n}catch{}}}catch{}return null},L=()=>Ur()||qr()||Vr()||Wr()||null;var ko="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",jr=`${ko}/active-follow-up-get`,Kr=`${ko}/active-follow-up-set`,le="ws-crm-active-follow-up-button",Dt={amber:{label:"Add to Active Follow Up",bg:s.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:s.amber,clickable:!0,dotColor:s.amber},blue:{label:"In Active Follow Up",bg:s.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:s.blue,clickable:!1,dotColor:s.blue},gray:{label:"Not in Follow Up",bg:s.slate,border:s.steel,color:s.coolGray,clickable:!1,dotColor:s.coolGray},loading:{label:"Active Follow Up\u2026",bg:s.slate,border:s.steel,color:s.ash,clickable:!1,dotColor:s.ash},error:{label:"Follow Up status unavailable",bg:s.slate,border:s.steel,color:s.ash,clickable:!1,dotColor:s.crimson}},Yr=()=>{let t=C();return!t||!t.locationId?null:{contactId:t.contactId,tenantId:t.locationId}},Xr=async t=>{let e=await fetch(jr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},Jr=async(t,e)=>{let o=await fetch(Kr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!o.ok)throw new Error(`Set webhook returned ${o.status} ${o.statusText}`);return await o.json()},de=(t,e)=>{let o=Dt[e];t.dataset.state=e,t.disabled=!o.clickable,t.style.cursor=o.clickable?"pointer":"default",t.style.background=o.bg,t.style.borderColor=o.border,t.style.color=o.color;let r=t.querySelector(".ws-afu-dot");r&&(r.style.background=o.dotColor);let n=t.querySelector(".ws-afu-label");n&&(n.textContent=o.label)},fo=t=>{let e=document.createElement("button");return e.type="button",e.id=le,e.dataset.contactId=t,e.style.cssText=`
  ...
  `,K=`
    ...
    padding-right: 28px;
  `,Ge=M=>['<option value="">(unset)</option>'].concat(k.map($=>`<option value="${X($)}"${$===M?" selected":""}>${H($)}</option>`)).join("");u.innerHTML=`
    <div style="
      font-family: ${v.mono};
      font-size: 10px;
      letter-spacing: 0.14em;
  ...
  `,g.appendChild(u),document.body.appendChild(g);let N=u.querySelector("#ws-pc-purchase-price"),se=u.querySelector("#ws-pc-emd"),te=u.querySelector("#ws-pc-cash-at-close"),ie=!1;te?.addEventListener("input",()=>{ie=!0});let _e=()=>{if(ie)return;let M=Number(N?.value||0),$=Number(se?.value||0);te&&(te.value=String(Math.max(0,M-$)))};N?.addEventListener("input",_e),se?.addEventListener("input",_e);let ze=()=>g.remove();g.addEventListener("click",M=>{M.target===g&&ze()}),u.querySelector("#ws-pc-cancel")?.addEventListener("click",ze);let z=u.querySelector("#ws-pc-submit"),R=u.querySelector("#ws-pc-error");z?.addEventListener("click",async()=>{if(z){R&&(R.textContent=""),z.disabled=!0,z.textContent="Opening\u2026";try{let M=(u.querySelector("#ws-pc-seller-name")?.value||"").trim(),$=Number(N?.value||0),ve=Number(se?.value||0),ye=Number(te?.value||0),ge=(u.querySelector("#ws-pc-days-to-close")?.value??"").trim(),fe=(u.querySelector("#ws-pc-inspection-days")?.value??"").trim(),we=(u.querySelector("#ws-pc-additional-terms")?.value??"").trim(),he=(u.querySelector("#ws-pc-current-occupancy")?.value??"").trim(),xe=(u.querySelector("#ws-pc-occupancy-at-close")?.value??"").trim(),P={};if(M!==i&&(P.sellerName=M),$!==l&&(P.purchasePrice=$),ve!==d&&(P.emd=ve),ye!==b&&(P.cashAtClose=ye),ge!==h&&(P.daysToClose=ge),fe!==m&&(P.inspectionDays=fe),we!==p&&(P.additionalTerms=we),he!==_&&(P.currentOccupancy=he),xe!==f&&(P.occupancyAtClose=xe),Object.keys(P).length>0){c.info("[prepare-contract] PA modal submit with overrides:",P),z.textContent="Saving\u2026";let Et=L(),Tt=await ut({tenantId:t,contactId:e,template:o,userId:Et,overrides:P});c.info("[prepare-contract] re-stage with overrides ok:",Tt),z.textContent="Opening\u2026"}else c.info("[prepare-contract] PA modal submit \u2014 no edits, skipping re-stage");let O=a?`${a} - Purchase Agreement`:void 0;await co(t,e,o,O)}catch(M){z.disabled=!1,z.textContent="Open Contract",R&&(R.textContent=M.message||"Failed to open contract."),c.warn("[prepare-contract] PA confirm submit failed:",M)}}})},ys=({tenantId:t,contactId:e,template:o,oppId:r,staged:n,userId:a,defaultClosingDate:i,defaultEMDDueDate:l})=>{document.getElementById(J)?.remove(),br();let d=n.debug||{},b=typeof d.propertyAddress=="string"?d.propertyAddress:"",h=typeof d.sellerDisplay=="string"?d.sellerDisplay:"",m=typeof d.assignmentPrice=="number"?d.assignmentPrice:0,p=typeof d.buyerEMD=="number"?d.buyerEMD:0,_=typeof d.assignmentFee=="number"?d.assignmentFee:0,f=typeof d.acqPurchasePrice=="number"?d.acqPurchasePrice:0,k=typeof d.closingOffice=="string"?d.closingOffice:"",g=typeof d.buyerCompanyName=="string"?d.buyerCompanyName:"",u=i,y=l,E="",T=document.createElement("div");T.id=J,T.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
