...
`;var ut=/^[A-Za-z0-9]{15,30}$/,G=o=>typeof o=="string"&&ut.test(o),yt=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},gt=()=>{let o=window.WS_CRM_USER_ID;return G(o)?o:null},ft=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(G(t))return t;return null},wt=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=yt(e);if(!t)return null;let r=t.uid;if(G(r))return r;let n=t.claims;if(n&&G(n.user_id))return n.user_id}catch{}return null},xt=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(G(n))return n}catch{}}}catch{}return null},F=()=>gt()||ft()||wt()||xt()||null;var Ce="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",kt=`${Ce}/active-follow-up-get`,Et=`${Ce}/active-follow-up-set`,H="ws-crm-active-follow-up-button",ue={amber:{label:"Add to Active Follow Up",bg:c.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:c.amber,clickable:!0,dotColor:c.amber},blue:{label:"In Active Follow Up",bg:c.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:c.blue,clickable:!1,dotColor:c.blue},gray:{label:"Not in Follow Up",bg:c.slate,border:c.steel,color:c.coolGray,clickable:!1,dotColor:c.coolGray},loading:{label:"Active Follow Up\u2026",bg:c.slate,border:c.steel,color:c.ash,clickable:!1,dotColor:c.ash},error:{label:"Follow Up status unavailable",bg:c.slate,border:c.steel,color:c.ash,clickable:!1,dotColor:c.crimson}},Tt=()=>{let o=f();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},St=async o=>{let e=await fetch(kt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},Ct=async(o,e)=>{let t=await fetch(Et,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},M=(o,e)=>{let t=ue[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},Ee=o=>{let e=document.createElement("button");return e.type="button",e.id=H,e.dataset.contactId=o,e.style.cssText=`
  ...
  `,t.forEach(l=>{let d=document.createElement("button");d.type="button",d.style.cssText=`
    ...
    `,d.innerHTML=`
      ...
      "></span>
      <span>${l.label}</span>
    `,d.addEventListener("mouseenter",()=>{d.style.background=c.slate}),d.addEventListener("mouseleave",()=>{d.style.background="transparent"}),d.addEventListener("click",async()=>{b.info(`Call Status set to "${l.label}" (${l.value}, ${l.color}) for contact ${o}`);let p=i.firstElementChild,m=i.children[1],h=i.children[2];p&&(p.style.background=_e[l.color]),m&&(m.textContent=l.label),s.style.display="none";let y=f()?.locationId,k=F();if(!y){b.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),m&&(m.textContent=`${l.label} (no tenant!)`),m&&(m.style.color=c.amber);return}if(e==="seller"&&l.color==="green"&&!k){b.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),m&&(m.textContent=`${l.label} (no user!)`),m&&(m.style.color=c.amber);return}let g=h?.textContent??"\u25BE",E=async _=>{h&&(h.textContent="\u2026"),i.style.opacity="0.75";try{await zt({contactId:o,locationId:y,status:l.label,userId:k,deadReason:_,webhookUrl:r,kind:e}),h&&(h.textContent="\u2713"),setTimeout(()=>{h&&(h.textContent=g),i.style.opacity="1"},900)}catch(w){b.warn("Call Status webhook failed:",w),h&&(h.textContent="\u2717"),m&&(m.style.color=c.amber),i.style.opacity="1",setTimeout(()=>{h&&(h.textContent=g),m&&(m.style.color=c.bone)},1800)}};if(e==="seller"&&l.value==="dead-deal"){let _=m?.textContent??"";Zt(w=>{E(w)},()=>{m&&(m.textContent=_)});return}if(e==="seller"&&l.value==="appointment-booked"){let _=m?.textContent??"";try{(await Wt({contactId:o,tenantId:y})).ok?await E():eo(()=>{E()},()=>{m&&(m.textContent=_)})}catch(w){b.warn("Appointment sanity check failed; firing anyway:",w),await E()}return}await E()}),s.appendChild(d)}),i.addEventListener("click",l=>{l.stopPropagation(),s.style.display=s.style.display==="none"?"block":"none"}),document.addEventListener("click",l=>{a.contains(l.target)||(s.style.display="none")}),a.appendChild(i),a.appendChild(s),a},Qt=o=>{document.getElementById(Ne)?.remove();let e=document.createElement("div");e.id=Ne,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `;let r=new Date,n=new Date(r.getTime()+5*24*60*60*1e3).toISOString().slice(0,10);t.innerHTML=`
    ...
    <div style="
      ...
      margin-bottom: 16px;
    ">
      <span style="color: ${c.ash}; font-family: ${u.mono}; font-size: 14px;">$</span>
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let t=oe();if(!t||!t.locationId||t.oppId!==o)return;let r=F(),n=e.querySelector(".ws-ni-label");e.disabled=!0,e.style.opacity="0.7",n&&(n.textContent="Closing\u2026");try{let a=await so({oppId:t.oppId,locationId:t.locationId,userId:r});if(a?.ok)n&&(n.textContent="Marked Not Interested"),e.style.background=c.crimsonGlow,e.style.color=c.crimson,b.info(`Not Interested set for opp ${o} \u2192 ${a.new_stage}`);else throw new Error("Webhook returned ok=false")}catch(a){b.error("Not Interested webhook failed",a),n&&(n.textContent="Try again"),e.disabled=!1,e.style.opacity="1",setTimeout(()=>{n&&n.textContent==="Try again"&&(n.textContent="Mark Not Interested")},3e3)}}),e},io=["[data-testid='OPPORTUNITY_HEADER']","[class*='opportunity-header']","[class*='opp-header']","header.flex.items-center"],lo=async()=>{for(let o of io)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},co=async()=>{let o=oe();if(!o||!o.locationId){document.getElementById(q)?.remove();return}let e=document.getElementById(q);if(e&&e.dataset.oppId===o.oppId)return;e&&e.remove();let t=await lo();if(!t){b.warn("Not Interested: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=oe();if(!r||r.oppId!==o.oppId)return;let n=We(o.oppId),a=document.createElement("span");a.id=`${q}-wrap`,a.style.cssText=`
    ...
    margin-left: 12px;
    vertical-align: middle;
  `,a.appendChild(n),t.appendChild(a),b.debug(`Mounted Not Interested button on opp ${o.oppId}`);let i=new MutationObserver(()=>{let s=document.getElementById(q);if(!(oe()?.oppId===o.oppId)){i.disconnect();return}if(!s){let d=We(o.oppId);a.appendChild(d),b.debug(`Re-mounted Not Interested button after wipe for opp ${o.oppId}`)}});i.observe(t,{childList:!0})},je=()=>{co()};var le="reos-theme-stylesheet",dt="reos-theme-fonts",bo=()=>{let o=document.getElementById(dt);o&&o.remove()},ho=`
/* ============================================
...
/* v0.11.9: Default icon \u2014 emerald (was ash). Tim's call after the
   ...
   v0.13.14: many of these SVGs use fill="currentColor" (not stroke),
   and the host button has color: rgb(237, 238, 240) !important set
   INLINE which prevents currentColor from inheriting our emerald
   through the cascade. Explicitly set BOTH fill and stroke on the
   path itself so we bypass currentColor entirely.
   v0.13.15: the buttons wrap their SVG in <i class="hr-icon-inner
   hr-icon">, which means the 18a-hr-icon-default rule (line ~4495)
   matches with specificity 0,1,1,2 (it uses the
   :not(#__reos_never_id) id-specificity bumper). Without the same
   bumper here, 18a wins and paints these icons fill:none + bone.
   Add the bumper to ALL selectors below so specificity ties (1
   id-equiv each) and source order (this block is below 18a) makes
   our rule win. */
...
/* Some paths use fill="none" deliberately (line-art icons rely
   on stroke only). Don't paint those \u2014 restore fill to none when
   the original markup says so. */
html body .sidebar-option-button:not(#__reos_never_id) svg path[fill="none"],
html body button[class*="sidebar-option-button"]:not(#__reos_never_id) svg path[fill="none"] {
  fill: none !important;
}
...
html body .sidebar-option-button:not(#__reos_never_id):hover svg path[fill="none"],
html body button[class*="sidebar-option-button"]:not(#__reos_never_id):hover svg path[fill="none"] {
  fill: none !important;
}
...
html body .sidebar-option-button:not(#__reos_never_id)[class*="text-blue-600"]:not([class*="hover:text-blue-600"]) svg path[fill="none"],
html body button[class*="sidebar-option-button"]:not(#__reos_never_id)[class*="text-blue-600"]:not([class*="hover:text-blue-600"]) svg path[fill="none"] {
  fill: none !important;
}
