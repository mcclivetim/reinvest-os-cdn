...
`,$=`
  ...
  flex-shrink: 0;
`;var Hn=/^[A-Za-z0-9]{15,30}$/,Le=t=>typeof t=="string"&&Hn.test(t),Pn=t=>{try{let e=t.split(".");if(e.length!==3)return null;let o=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=o+"=".repeat((4-o.length%4)%4);return JSON.parse(atob(r))}catch{return null}},On=()=>{let t=window.WS_CRM_USER_ID;return Le(t)?t:null},Nn=()=>{let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let o of e)if(Le(o))return o;return null},Bn=()=>{try{let t=localStorage.getItem("refreshedToken");if(!t)return null;let e=t.startsWith('"')?t.slice(1,-1):t,o=Pn(e);if(!o)return null;let r=o.uid;if(Le(r))return r;let n=o.claims;if(n&&Le(n.user_id))return n.user_id}catch{}return null},Rn=()=>{try{let t=["user","tokenUser","currentUser","ghl_user"];for(let e of t){let o=localStorage.getItem(e);if(o)try{let r=JSON.parse(o),n=r.id??r._id;if(Le(n))return n}catch{}}}catch{}return null},M=()=>On()||Nn()||Bn()||Rn()||null;var rr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",Gn=`${rr}/active-follow-up-get`,zn=`${rr}/active-follow-up-set`,pe="ws-crm-active-follow-up-button",Zt={amber:{label:"Add to Active Follow Up",bg:a.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:a.amber,clickable:!0,dotColor:a.amber},blue:{label:"In Active Follow Up",bg:a.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:a.blue,clickable:!1,dotColor:a.blue},gray:{label:"Not in Follow Up",bg:a.slate,border:a.steel,color:a.coolGray,clickable:!1,dotColor:a.coolGray},loading:{label:"Active Follow Up\u2026",bg:a.slate,border:a.steel,color:a.ash,clickable:!1,dotColor:a.ash},error:{label:"Follow Up status unavailable",bg:a.slate,border:a.steel,color:a.ash,clickable:!1,dotColor:a.crimson}},qn=()=>{let t=L();return!t||!t.locationId?null:{contactId:t.contactId,tenantId:t.locationId}},Un=async t=>{let e=await fetch(Gn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},Vn=async(t,e)=>{let o=await fetch(zn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!o.ok)throw new Error(`Set webhook returned ${o.status} ${o.statusText}`);return await o.json()},he=(t,e)=>{let o=Zt[e];t.dataset.state=e,t.disabled=!o.clickable,t.style.cursor=o.clickable?"pointer":"default",t.style.background=o.bg,t.style.borderColor=o.border,t.style.color=o.color;let r=t.querySelector(".ws-afu-dot");r&&(r.style.background=o.dotColor);let n=t.querySelector(".ws-afu-label");n&&(n.textContent=o.label)},er=t=>{let e=document.createElement("button");return e.type="button",e.id=pe,e.dataset.contactId=t,e.style.cssText=`
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 6px 12px;
  ...
  `,he(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=qn();if(!(!r||r.contactId!==t)){he(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await Vn(r,M()),i=n&&n.state&&n.state in Zt?n.state:"blue";he(e,i),b.info(`Active Follow Up set for ${t} \u2192 ${i}`)}catch(n){b.error("Active Follow Up Set webhook failed",n),he(e,"error"),setTimeout(()=>void it(e,r),4e3)}}}),e},it=async(t,e)=>{he(t,"loading");try{let o=await Un(e),r=o&&o.state&&o.state in Zt?o.state:"gray",n=document.getElementById(pe);if(!n||n.dataset.contactId!==e.contactId)return;he(n,r)}catch(o){b.warn("Active Follow Up Get webhook failed",o),he(t,"error")}},Wn=[".central-panel","[class*='central-panel']"],Jt="ws-crm-contact-actions-bar",jn=async()=>{for(let t of Wn)try{let e=await A(t,{timeoutMs:3e3});if(e)return e}catch{}try{return(await A("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},Kn=async(t,e)=>{let o=e.querySelector(`#${Jt}`);if(!o)try{o=await A(`#${Jt}`,{timeoutMs:3e3,root:e})}catch{o=null}if(o){let i=e.querySelector("#ws-crm-afu-wrap");return i&&i.remove(),o.appendChild(t),{container:o,placement:"actions-bar"}}let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
      ...
      flex-shrink: 0;
    `;let i=e.querySelector(`#${Jt}`);i&&i.parentElement===e?e.insertBefore(n,i.nextSibling):e.insertBefore(n,e.firstChild)}return n.appendChild(t),{container:n,placement:"fallback"}},Yn=()=>{let t=document.querySelector("[data-testid='CENTRALPANEL_NAME']");if(!t)return null;let e=t;for(let o=0;o<8&&!(!e||!e.parentElement);o++){let r=e.parentElement,n=(r.className||"").toString();if(n.includes("justify-between")&&n.includes("border-b")){let i=r.children[1];return i instanceof HTMLElement?i:r}e=r}return null},Xn=(t,e)=>{t.style.marginRight="4px",e.insertBefore(t,e.firstChild)},Jn=async t=>{if(t==="url"){let o=await jn();return o?{kind:"contact-detail",target:o}:null}let e=Yn();return e?{kind:"conversations",target:e}:null},tr=async(t,e)=>{if(e.kind==="contact-detail"){let{placement:o}=await Kn(t,e.target);return o}return Xn(t,e.target),"conversations-header"},Zn=async()=>{let t=L();if(!t||!t.locationId){document.getElementById(pe)?.remove(),document.getElementById("ws-crm-afu-wrap")?.remove();return}let e={contactId:t.contactId,tenantId:t.locationId},o=document.getElementById(pe);if(o&&o.dataset.contactId===e.contactId){it(o,e);return}o&&o.remove();let r=await Jn(t.source);if(!r){b.warn(`Active Follow Up: no mount target on ${t.source}-source page`);return}let n=L();if(!n||n.contactId!==e.contactId)return;let i=er(e.contactId),s=await tr(i,r);b.debug(`Active Follow Up mounted (${r.kind}/${s}) for ${e.contactId}`),or();let l=document.getElementById(pe)??i;it(l,e);let d=new MutationObserver(()=>{let c=document.getElementById(pe);if(!(L()?.contactId===e.contactId)){d.disconnect();return}if(!c){let m=er(e.contactId);tr(m,r).then(()=>{or(),it(m,e)}),b.debug(`Active Follow Up re-mounted after wipe for ${e.contactId}`)}});d.observe(r.target,{childList:!0})},$e=null,nr=()=>{let t=L()?.contactId??null;if($e&&$e.contactId===t)return;let e=Zn().finally(()=>{$e?.promise===e&&($e=null)});$e={contactId:t,promise:e}},or=()=>{let t=document.querySelectorAll(`#${pe}`);if(!(t.length<=1)){b.debug(`Active Follow Up: found ${t.length} duplicate buttons, keeping first, removing ${t.length-1}`);for(let e=1;e<t.length;e++)t[e].remove()}};var Qn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/type",Me=new Map,st=async(t,e)=>{if(!t)return null;if(Me.has(t))return Me.get(t)??null;if(!e)return b.warn(`Cannot fetch Contact Type for ${t} \u2014 tenant_id missing from URL context`),null;let o=new AbortController,r=setTimeout(()=>o.abort(),1500);try{let n=`${Qn}?contact_id=${encodeURIComponent(t)}&tenant_id=${encodeURIComponent(e)}`,i=await fetch(n,{method:"GET",headers:{Accept:"application/json"},signal:o.signal});if(clearTimeout(r),!i.ok)return b.warn(`Contact Type fetch returned ${i.status} for ${t}`),Me.set(t,null),null;let l=((await i.json()).contact_type||"").toLowerCase().trim()||null;return Me.set(t,l),b.debug(`Resolved Contact Type for ${t}: ${l??"(unset)"}`),l}catch(n){return clearTimeout(r),b.warn(`Contact Type fetch failed for ${t}:`,n),Me.set(t,null),null}};var lt=t=>t==="buyer"?"buyer":t==="seller"?"seller":t==="agent"||t==="closing_agent"||t==="closing_office"||t==="vendor"?"other":"seller";var sr="ws-buybox-nav-icon",to="ws-buybox-panel",ar="ws-buybox-modal",ea="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/buy-box/manage",oo="https://mcclivetim.github.io/reinvest-os-cdn/geo",dt=null,De=null,Fe=null,eo=async()=>{if(dt)return dt;try{let t=await fetch(`${oo}/states.json`);if(!t.ok)throw new Error(`HTTP ${t.status}`);return dt=await t.json(),dt}catch(t){return b.error("[buy-box] failed to fetch states.json",t),[]}},ta=async t=>{if(!De)try{let e=await fetch(`${oo}/counties.json`);if(!e.ok)throw new Error(`HTTP ${e.status}`);De=await e.json()}catch(e){return b.error("[buy-box] failed to fetch counties.json",e),[]}return De?.[t]??[]},oa=async t=>{if(!Fe)try{let e=await fetch(`${oo}/cities.json`);if(!e.ok)throw new Error(`HTTP ${e.status}`);Fe=await e.json()}catch(e){return b.error("[buy-box] failed to fetch cities.json",e),[]}return Fe?.[t]??[]},ro=async t=>await(await fetch(ea,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})).json(),ra=(t,e)=>ro({tenant_id:t,action:"list",buyer_contact_id:e}),na=(t,e,o,r)=>ro({tenant_id:t,action:"add",buyer_contact_id:e,geo_level:o,geo_value:r}),aa=(t,e)=>ro({tenant_id:t,action:"remove",entry_id:e}),F=t=>t.replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e]??e),ia=(t,e)=>{let o=e.find(r=>r.code===t);return o?`${o.name} (${t})`:t},sa='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" width="20" height="20"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" /></svg>',la='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.6" stroke="currentColor" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" d="M9 3h6m-9 4h12l-1 13a2 2 0 01-2 2H9a2 2 0 01-2-2L6 7zM4 7h16M10 11v6M14 11v6" /></svg>',da="nav.w-13, nav[class*='w-13']",ca=".flex-1.h-full.relative.overflow-hidden.rounded-lg",ba=async()=>{try{return await A(da,{timeoutMs:2500})}catch{return null}},pa=async()=>{try{return await A(ca,{timeoutMs:2500})}catch{return null}},ha=t=>t.querySelector("div[class*='bg-contacts-panel']")??null,ma=(t,e)=>{let o=document.createElement("div");return o.id=to,o.dataset.buyerContactId=e,o.style.cssText=`
  ...
  `,o.querySelector("#ws-buybox-add")?.addEventListener("click",()=>{ya(t,e,()=>no(t,e))}),o},no=async(t,e)=>{let o=document.querySelector("#ws-buybox-body");if(o)try{let[r,n]=await Promise.all([ra(t,e),eo()]);if(!r.ok){o.innerHTML=`<div style="color: ${a.crimson};">${F(r.error||"Failed to load buy-box entries")}</div>`;return}ua(o,r.grouped||{state:[],county:[],city:[],zip:[]},n,t,e)}catch(r){o.innerHTML=`<div style="color: ${a.crimson};">Network error: ${F(r.message)}</div>`}},ua=(t,e,o,r,n)=>{if(e.state.length+e.county.length+e.city.length+e.zip.length===0){t.innerHTML=`
    ...
    `;return}let l=[{key:"state",label:"States"},{key:"county",label:"Counties"},{key:"city",label:"Cities"},{key:"zip",label:"Zip Codes"}].filter(d=>e[d.key].length>0).map(d=>{let c=e[d.key].map(p=>{let m=d.key==="state"?ia(p.geo_value,o):p.geo_value;return`
      ...
      `}).join("");t.innerHTML=l,t.querySelectorAll(".ws-buybox-remove").forEach(d=>{d.addEventListener("mouseenter",()=>{d.style.color=a.crimson}),d.addEventListener("mouseleave",()=>{d.style.color=a.ash}),d.addEventListener("click",()=>{let c=d.dataset.entryId||"",p=d.dataset.entryLabel||"this area";confirm(`Remove "${p}" from this buyer's buy-box?`)&&(d.disabled=!0,aa(r,c).then(m=>{if(!m.ok){alert(`Failed to remove: ${m.error||"unknown error"}`),d.disabled=!1;return}no(r,n)}).catch(m=>{alert(`Network error: ${m.message}`),d.disabled=!1}))})})},ct=!1,lr=(t,e)=>{ct=e;let o=ha(t.panelArea);console.log(`[buy-box] setActive(${e}) \u2014 nativePanel=${!!o} panel=${!!t.panel} iconBtn=${!!t.iconBtn}`),e?(t.iconBtn.className=va,t.iconBtn.dataset.active="true",o&&(t.nativePanelOriginalDisplay=o.style.display||"",o.style.display="none"),t.panel.style.display="flex",no(t.tenantId,t.buyerContactId)):(t.iconBtn.className=dr,delete t.iconBtn.dataset.active,t.panel.style.display="none",o&&(o.style.display=t.nativePanelOriginalDisplay||""))},dr="relative w-8 h-8 rounded-lg flex items-center justify-center sidebar-option-button text-gray-900 hover:text-blue-600",va="relative w-8 h-8 rounded-lg flex items-center justify-center sidebar-option-button text-blue-600 bg-white",_a=t=>{let e=document.createElement("button");return e.id=sr,e.type="button",e.title="Buy-Box",e.setAttribute("aria-label","Buy-Box"),e.className=dr,e.innerHTML=`<span style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); display: block; line-height: 0;">${sa}</span>`,e.addEventListener("click",o=>{o.stopPropagation();let r=t();if(!r){console.warn("[buy-box] icon clicked but currentState is null");return}let n=!ct;console.log(`[buy-box] icon clicked \u2192 turningOn=${n}`),lr(r,n)}),e},ya=(t,e,o)=>{let r=document.getElementById(ar);r&&r.remove();let n=document.createElement("div");n.id=ar,n.style.cssText=`
  ...
  `,S=()=>{let g=i.querySelector("#ws-buybox-zip"),E=i.querySelector("#ws-buybox-err");g&&(g.addEventListener("input",()=>{g.value=g.value.replace(/\D/g,"").slice(0,5),E&&(E.textContent="")}),g.addEventListener("keydown",I=>{I.key==="Enter"&&(I.preventDefault(),C())}),i.querySelector("#ws-buybox-back")?.addEventListener("click",()=>{s.step=1,l()}),i.querySelector("#ws-buybox-cancel")?.addEventListener("click",()=>n.remove()),i.querySelector("#ws-buybox-save")?.addEventListener("click",()=>void C()),setTimeout(()=>g.focus(),0))},C=async()=>{let g=i.querySelector("#ws-buybox-zip"),E=i.querySelector("#ws-buybox-err"),I=(g?.value||"").trim();if(!/^\d{5}$/.test(I)){E&&(E.textContent="Zip must be exactly 5 digits.");return}await x(I)},x=async g=>{if(!s.granularity||!g)return;let E=i.querySelector("#ws-buybox-err"),I=i.querySelector("#ws-buybox-save");I&&(I.disabled=!0,I.textContent="Adding\u2026"),E&&(E.textContent="");try{let D=await na(t,e,s.granularity,g);if(!D.ok){E&&(E.textContent=D.error||"Failed to add entry."),I&&(I.disabled=!1,I.textContent="Add");return}o(),i.innerHTML=`
      ...
      `,setTimeout(()=>{s.step=1,s.granularity=null,s.stateCode=null,s.value=null,l()},900)}catch(D){E&&(E.textContent=`Network error: ${D.message}`),I&&(I.disabled=!1,I.textContent="Add")}};l()},Qt=!1,ge=null,He=null,ir=()=>{document.getElementById(sr)?.remove(),document.getElementById(to)?.remove(),He&&ge&&ge.nav.removeEventListener("click",He,!0),He=null,ge=null,ct=!1},cr=()=>{ga()},ga=async()=>{if(!Qt){Qt=!0;try{let t=L();if(!t){ir();return}let e=document.getElementById(to);if(e&&e.dataset.buyerContactId===t.contactId&&ge)return;ir();let o=null;try{o=await st(t.contactId,t.locationId)}catch(m){b.warn("[buy-box] Contact Type fetch failed",m);return}if(lt(o)!=="buyer")return;if(!t.locationId){b.warn("[buy-box] no locationId in context; can't mount widget");return}let[n,i]=await Promise.all([ba(),pa()]);if(!n||!i){b.warn("[buy-box] right-rail nav or panel area not found");return}let s=L();if(!s||s.contactId!==t.contactId)return;let l=ma(t.locationId,t.contactId),d=_a(()=>ge),c={buyerContactId:t.contactId,tenantId:t.locationId,nav:n,panelArea:i,iconBtn:d,panel:l,nativePanelOriginalDisplay:""};ge=c;let p=n.querySelector(".flex.flex-col.items-center.gap-2");p?p.appendChild(d):n.appendChild(d),i.appendChild(l),He=m=>{let h=m.target;if(!h||c.iconBtn.contains(h))return;h.closest("button.sidebar-option-button")&&ct&&lr(c,!1)},n.addEventListener("click",He,!0),b.info(`[buy-box] right-rail mount complete for buyer ${t.contactId}`)}finally{Qt=!1}}};var le="ws-crm-claim-release-pill",fa="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-get",wa="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-set",xa=async t=>{let e=await fetch(fa,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.locationId,contact_id:t.contactId,triggered_by_user_id:t.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Get returned ${e.status} ${e.statusText}`);return await e.json()},ka=async t=>{let e=await fetch(wa,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.locationId,contact_id:t.contactId,triggered_by_user_id:t.userId,action:t.action}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Set returned ${e.status} ${e.statusText}`);return await e.json()},hr=t=>{let e=Array.from(t.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},fe=(t,e)=>{let o=e==="claim"?"emerald":"amber",r=e==="claim"?a.emerald:a.amber;t.dataset.mode=e,t.style.cssText=Qo(o),t.title=e==="claim"?"Take ownership of this lead":"Return this lead to the pool",t.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `},Ea=({contactId:t,mode:e})=>{let o=document.createElement("button");return o.id=le,o.type="button",o.dataset.contactId=t,fe(o,e),o.addEventListener("mouseenter",()=>{o.style.transform="translateY(-1px)"}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)"}),o.addEventListener("click",async()=>{let r=o.dataset.mode??"claim",n=L();if(!n||!n.locationId||n.contactId!==t)return;let i=M();if(!i){b.warn("Claim/Release click: no user ID detected (login session not exposing it)");let l=o.querySelector(".ws-cr-label");l&&(l.textContent="Sign-in needed"),setTimeout(()=>fe(o,r),2500);return}b.info(`Claim/Release click \u2014 contact=${t} mode=${r} user=${i}`);let s=r==="claim"?"release":"claim";fe(o,s),o.disabled=!0,o.style.opacity="0.8";try{let l=await ka({contactId:t,locationId:n.locationId,userId:i,action:r}),d=l?.mode==="claim"||l?.mode==="release"?l.mode:s;fe(o,d),b.info(`Claim/Release done \u2192 new mode: ${d}`)}catch(l){b.error("Claim/Release Set webhook failed",l),fe(o,r);let d=o.querySelector(".ws-cr-label");if(d){let c=d.textContent??"";d.textContent="Try again",setTimeout(()=>{d.textContent==="Try again"&&(d.textContent=c)},2500)}}finally{o.disabled=!1,o.style.opacity="1"}}),o},br=async(t,e)=>{try{let o=await xa({contactId:e.contactId,locationId:e.locationId,userId:M()}),r=document.getElementById(le);if(!r||r.dataset.contactId!==e.contactId)return;let n=o?.mode==="release"?"release":"claim";fe(r,n)}catch(o){b.warn("Claim/Release Get failed; pill stays in default 'claim' mode",o)}},Ta=async(t,e=1e4)=>{let o=Date.now();for(;Date.now()-o<e;){let r=hr(t);if(r)return r;await new Promise(n=>setTimeout(n,150))}return null},pr=(t,e)=>{let o=Ea({contactId:e,mode:"claim"}),r=document.createElement("span");r.id=`${le}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(o),t.appendChild(r)},Sa=async()=>{let t=L();if(!t){document.getElementById(le)?.remove();return}let e=document.getElementById(le);if(!(e&&e.dataset.contactId===t.contactId)){e&&e.remove();try{let o=await A("#record-details-lhs",{timeoutMs:1e4}),r=await Ta(o),n=L();if(!n||n.contactId!==t.contactId)return;if(!r){b.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}pr(r,t.contactId),b.debug(`Mounted Claim/Release pill next to name for ${t.contactId}`);let i=document.getElementById(le);i&&t.locationId&&br(i,{contactId:t.contactId,locationId:t.locationId});let s=null;s=new MutationObserver(async()=>{if(!(L()?.contactId===t.contactId)){s?.disconnect();return}if(document.getElementById(le))return;let d=hr(o);if(d){pr(d,t.contactId);let c=document.getElementById(le);c&&t.locationId&&br(c,{contactId:t.contactId,locationId:t.locationId}),b.debug("Re-mounted Claim/Release pill after React wipe")}}),s.observe(o,{childList:!0,subtree:!0})}catch(o){b.warn("Could not mount Claim/Release pill:",o)}}},mr=()=>{Sa()};var fr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set",Ca="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/buyer-call-status/set";var Ia="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Aa=async({contactId:t,locationId:e,status:o,userId:r,deadReason:n,webhookUrl:i,kind:s})=>{let l=i||fr,d;if(s==="buyer"){let p=wr.find(h=>h.label===o),m=p?p.value:o.toLowerCase().replace(/\s+/g,"-");d={tenant_id:e,contact_id:t,new_status:m,triggered_by_user_id:r??""}}else{let p={call_status:o,triggered_by_user_id:r??""};n&&(p.dead_reason=n),d={contact_id:t,location:{id:e},customData:p}}let c=await fetch(l,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d),credentials:"omit"});if(!c.ok)throw new Error(`Webhook returned ${c.status} ${c.statusText}`)},La=async({contactId:t,tenantId:e})=>{let o=await fetch(Ia,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,contact_id:t}),credentials:"omit"});if(!o.ok)throw new Error(`Sanity check returned ${o.status} ${o.statusText}`);return await o.json()};var we="ws-crm-contact-actions-bar",$a="ws-crm-call-status-dropdown",ur="ws-crm-offer-made-modal",Ma=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],wr=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"vm-left",label:"VM Left",color:"purple"},{value:"qualified-buyer",label:"Qualified Buyer",color:"green"},{value:"upgrade-to-vip",label:"Upgrade to VIP",color:"green"},{value:"walkthrough-scheduled",label:"Walkthrough Scheduled",color:"green"},{value:"made-an-offer",label:"Made an Offer",color:"green"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"wrong-market",label:"Wrong Market",color:"orange"},{value:"no-longer-buying",label:"No Longer Buying",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"blacklisted",label:"Blacklisted",color:"red"}],Da=[".central-panel","[class*='central-panel']","[data-testid='central-panel']","[data-test='central-panel']",".contact-detail-page .central-panel",".contact-detail-page [class*='central']"],Fa=async()=>{for(let t of Da)try{let e=await A(t,{timeoutMs:2e3});if(e)return b.debug(`[actions-bar] mount target found via selector: ${t}`),e}catch{}try{let t=await A("#contact-conversation-panel",{timeoutMs:2e3});if(t.parentElement)return b.debug("[actions-bar] mount target found via #contact-conversation-panel.parentElement"),t.parentElement}catch{}for(let t of["[id*='conversation-panel']","[class*='conversation-panel']","[class*='contact-detail'] [class*='panel']","main [class*='central']","main [class*='panel']:not([class*='sidebar']):not([class*='nav'])"])try{let e=await A(t,{timeoutMs:1500});if(e)return b.debug(`[actions-bar] mount target via fallback: ${t}`),e.parentElement??e}catch{}return b.warn("[actions-bar] no mount target found via any selector \u2014 URL:",window.location.pathname),null},Ha=({contactId:t,kind:e})=>{let o=e==="buyer"?wr:Ma,r=e==="buyer"?Ca:fr,n=e==="buyer"?"Buyer Call Status":"Seller Call Status",i=document.createElement("div");i.id=$a,i.style.cssText="position: relative; flex-shrink: 0;";let s=document.createElement("button");s.type="button",s.style.cssText=$,s.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `,o.appendChild(r),document.body.appendChild(o);let n=()=>o.remove(),i=()=>{n(),e()};o.addEventListener("click",l=>{l.target===o&&i()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",i),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),t()});let s=l=>{l.key==="Escape"&&(document.removeEventListener("keydown",s),i())};document.addEventListener("keydown",s)},yr=(t,e)=>{let o=document.createElement("div");if(o.id=we,o.dataset.contactId=t,o.dataset.contactKind=e,o.style.cssText=`
    ...
    border-bottom: 1px solid ${a.steel};
    flex-shrink: 0;
  `,e==="seller"||e==="buyer"){let r=document.createElement("button");r.type="button",r.style.cssText=ae,r.textContent="Offer Made",r.addEventListener("click",()=>Na(t,e)),o.appendChild(r),o.appendChild(Ha({contactId:t,kind:e}))}return o},ao=!1,Ga=()=>{document.querySelectorAll(`[id="${we}"]`).forEach(t=>t.remove())},xr=async()=>{if(b.debug("[actions-bar:01] ensureMounted called; url:",window.location.pathname),ao){b.debug("[actions-bar:00-LOCKED] ensureMounted already in-flight \u2014 skip");return}ao=!0;try{await za()}finally{ao=!1}},za=async()=>{let t=L();if(!t){b.debug("[actions-bar:02-BAIL] no contact context (URL didn't match patterns AND no in-page contact link). URL:",window.location.pathname),Ga();return}b.debug(`[actions-bar:02] ctx resolved \u2014 contactId=${t.contactId} locationId=${t.locationId??"(null)"} source=${t.source}`);let e="seller";try{let c=await st(t.contactId,t.locationId),p=lt(c);p==="buyer"?e="buyer":p==="other"&&(e="other"),b.debug(`[actions-bar:03] kind resolved \u2014 kind=${e} (raw type=${c??"(null)"})`)}catch(c){b.warn("[actions-bar:03-FAIL] Contact Type resolution threw \u2014 defaulting to seller:",c)}let o=L();if(!o||o.contactId!==t.contactId){b.debug("[actions-bar:04-BAIL] context changed during Contact Type fetch \u2014 aborting mount");return}let r=Array.from(document.querySelectorAll(`[id="${we}"]`));if(r.length===1&&r[0].dataset.contactId===t.contactId&&r[0].dataset.contactKind===e){b.debug(`[actions-bar:05-SKIP] bar already mounted for contact=${t.contactId} kind=${e} \u2014 no-op`);return}r.length>0&&(b.debug(`[actions-bar:05] removing ${r.length} stale bar(s) before remount`),r.forEach(c=>c.remove()));let n=await Fa();if(!n){b.warn("[actions-bar:06-FAIL] findMountTarget returned null \u2014 no .central-panel or fallback selector matched within timeout. URL:",window.location.pathname);return}b.debug(`[actions-bar:06] mount target found \u2014 tagName=${n.tagName} class="${n.className}"`);let i=L();if(!i||i.contactId!==t.contactId){b.debug("[actions-bar:07-BAIL] context changed during findMountTarget wait \u2014 aborting mount");return}let s;try{s=yr(t.contactId,e)}catch(c){b.error("[actions-bar:08-FAIL] buildBar threw:",c);return}s.dataset.mountTarget=n.className||"central-panel";try{n.insertBefore(s,n.firstChild)}catch(c){b.error("[actions-bar:09-FAIL] insertBefore threw:",c);return}if(!document.getElementById(we)){b.warn("[actions-bar:10-DETACHED] bar inserted but not in document tree \u2014 target was likely re-rendered by Vue. Will retry next page-change.");return}b.debug(`[actions-bar:10-OK] bar mounted + verified in document for contact=${t.contactId} kind=${e}`);let d=null;d=new MutationObserver(()=>{if(!(L()?.contactId===t.contactId)){d?.disconnect();return}let p=document.querySelectorAll(`[id="${we}"]`);if(p.length>1){for(let m=1;m<p.length;m++)p[m].remove();b.debug(`[actions-bar:WATCHDOG] trimmed ${p.length-1} duplicate bar(s)`);return}if(p.length===0){let m=document.querySelector(".central-panel")||n,h=yr(t.contactId,e);h.dataset.mountTarget=s.dataset.mountTarget??"";try{m.insertBefore(h,m.firstChild),b.debug(`[actions-bar:WATCHDOG] re-mounted after wipe for ${t.contactId}`)}catch(v){b.warn("[actions-bar:WATCHDOG-FAIL] re-mount threw:",v)}}}),d.observe(n,{childList:!0})},gr=null,qa=()=>{gr===null&&(gr=window.setInterval(()=>{let t=L();t&&(document.getElementById(we)||(b.debug("[actions-bar:HEALER] no bar on contact page \u2014 re-triggering mount for",t.contactId),xr()))},3e3))},kr=()=>{qa(),xr()};var bt="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",Ua=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),Pe="data-ws-crm-favicon",io=t=>t.hasAttribute(Pe),Er=t=>{if(t.tagName!=="LINK")return!1;let e=(t.getAttribute("rel")??"").toLowerCase();return Ua.has(e)},pt=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{Er(r)&&!io(r)&&r.remove()});let e=document.head.querySelector(`link[${Pe}="1"]`);e?e.href!==bt&&(e.href=bt):(e=document.createElement("link"),e.setAttribute(Pe,"1"),e.rel="icon",e.type="image/png",e.href=bt,document.head.appendChild(e),b.debug("Favicon installed"));let o=document.head.querySelector(`link[${Pe}="apple"]`);o||(o=document.createElement("link"),o.setAttribute(Pe,"apple"),o.rel="apple-touch-icon",o.href=bt,document.head.appendChild(o))},Tr=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)pt();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),pt())},50)}new MutationObserver(r=>{let n=!1;for(let i of r)i.addedNodes.forEach(s=>{s instanceof HTMLLinkElement&&Er(s)&&!io(s)&&(n=!0)}),i.removedNodes.forEach(s=>{s instanceof HTMLLinkElement&&io(s)&&(n=!0)});n&&pt()}).observe(document.head,{childList:!0,subtree:!1});let e=0,o=window.setInterval(()=>{e+=1,pt(),e>=5&&window.clearInterval(o)},1e3)};var Va=["Acquisitions","Transaction Coordination","Disposition","Marketing Tracker"],Wa=/^(?:\d+\)\s*)?(Acquisitions|Transaction Coordination|Disposition|Marketing Tracker)\s*$/,ja=".hr-card.hr-modal.crm-opportunities-modal",P=()=>{let t=document.querySelector(ja);if(!t)return null;let e=t.querySelectorAll(".hr-base-selection-overlay__wrapper, .hr-base-selection");for(let o of e){let r=o.textContent?.trim()??"";if(!r)continue;let n=r.match(Wa);if(n){let i=n[1];if(Va.includes(i))return i}}return null},te=async(t=2500)=>{let e=Date.now();for(;Date.now()-e<t;){let o=P();if(o!==null)return o;await new Promise(r=>setTimeout(r,100))}return null};var Ka="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/offer-made/set",X="ws-crm-mt-offer-made-button",Sr="ws-crm-mt-offer-made-modal",Ya=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Xa=/\/v2\/location\/([A-Za-z0-9]+)/,ut=()=>{let t=window.location.pathname,e=t.match(Ya);if(!e)return null;let o=t.match(Xa);return{oppId:e[1],locationId:o?o[1]:null}},Ja=async t=>{let e=await fetch(Ka,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.locationId,mt_opp_id:t.oppId,kind:"buyer",amount:t.amount,triggered_by_user_id:t.userId}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},ht=()=>{document.getElementById(Sr)?.remove()},Za=t=>{ht();let e=document.createElement("div");e.id=Sr,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{let o=ut();!o||!o.locationId||o.oppId!==t||Za({oppId:o.oppId,locationId:o.locationId,onSuccess:()=>{let r=e.querySelector(".ws-mt-om-label");r&&(r.textContent="Offer Recorded"),e.style.background=a.emeraldGlow,e.style.color=a.emeraldBright,e.style.borderColor=a.emeraldBorder,setTimeout(()=>{r&&r.textContent==="Offer Recorded"&&(r.textContent="Mark Offer Made",e.style.cssText=$)},4e3)}})}),e},ei=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],ti=async()=>{for(let t of ei)try{let e=await A(t,{timeoutMs:2e3});if(e)return e}catch{}return null},mt=!1,bo=async()=>{if(!mt){mt=!0;try{await oi()}finally{mt=!1}}},oi=async()=>{let t=ut();if(!t||!t.locationId){document.getElementById(X)?.remove(),document.getElementById(`${X}-wrap`)?.remove(),Ne();return}let e=document.getElementById(X);if(e&&e.dataset.oppId===t.oppId){let l=P();l!==null&&l!=="Marketing Tracker"&&(e.remove(),document.getElementById(`${X}-wrap`)?.remove(),Ne());return}e&&(e.remove(),document.getElementById(`${X}-wrap`)?.remove());let o=await ti();if(!o){b.warn("MT Offer Made: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await te();if(r!=="Marketing Tracker"){b.debug(`[mt-offer] gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=ut();if(!n||n.oppId!==t.oppId)return;let i=Qa(t.oppId),s=document.createElement("span");s.id=`${X}-wrap`,s.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,o.classList.contains("description")&&(o.style.display="flex",o.style.alignItems="center",o.style.gap="16px",o.style.flexWrap="wrap"),s.appendChild(i),o.appendChild(s),b.debug(`Mounted MT Offer Made button on opp ${t.oppId}`),ri(t.oppId)},Oe=null,co=null,Ne=()=>{Oe?.disconnect(),Oe=null,co=null},ri=t=>{if(Oe&&co===t)return;Ne();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(co=t,Oe=new MutationObserver(()=>{let o=ut();if(!o||o.oppId!==t){Ne();return}let r=P();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(X)?.remove(),document.getElementById(`${X}-wrap`)?.remove(),Ne();return}document.getElementById(X)||bo()}),Oe.observe(e,{childList:!0,subtree:!0}))},so=null,lo,ni=()=>{so||(so=new MutationObserver(()=>{lo===void 0&&(lo=window.setTimeout(()=>{lo=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(X);t&&!e&&!mt&&bo()},300))}),so.observe(document.body,{childList:!0,subtree:!0}))},Cr=()=>{ni(),bo()};var ai="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/mt/not-interested",J="ws-crm-not-interested-button",ii=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,si=/\/v2\/location\/([A-Za-z0-9]+)/,_t=()=>{let t=window.location.pathname,e=t.match(ii);if(!e)return null;let o=t.match(si);return{oppId:e[1],locationId:o?o[1]:null}},li=async t=>{let e=await fetch(ai,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.locationId,mt_opp_id:t.oppId,triggered_by_user_id:t.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Not Interested webhook returned ${e.status} ${e.statusText}`);return await e.json()},di=t=>{let e=document.createElement("button");return e.id=J,e.type="button",e.dataset.oppId=t,e.style.cssText=$,e.title="Close this Marketing Tracker opp as Not Interested for THIS property only",e.innerHTML=`
    <span style="
      display: inline-block;
      width: 8px;
      height: 8px;
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let o=_t();if(!o||!o.locationId||o.oppId!==t)return;let r=M(),n=e.querySelector(".ws-ni-label");e.disabled=!0,e.style.opacity="0.7",n&&(n.textContent="Closing\u2026");try{let i=await li({oppId:o.oppId,locationId:o.locationId,userId:r});if(i?.ok)n&&(n.textContent="Marked Not Interested"),e.style.background=a.crimsonGlow,e.style.color=a.crimson,b.info(`Not Interested set for opp ${t} \u2192 ${i.new_stage}`);else throw new Error("Webhook returned ok=false")}catch(i){b.error("Not Interested webhook failed",i),n&&(n.textContent="Try again"),e.disabled=!1,e.style.opacity="1",setTimeout(()=>{n&&n.textContent==="Try again"&&(n.textContent="Mark Not Interested")},3e3)}}),e},ci=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],bi=async()=>{for(let t of ci)try{let e=await A(t,{timeoutMs:2e3});if(e)return e}catch{}return null},vt=!1,uo=async()=>{if(!vt){vt=!0;try{await pi()}finally{vt=!1}}},pi=async()=>{let t=_t();if(!t||!t.locationId){document.getElementById(J)?.remove(),document.getElementById(`${J}-wrap`)?.remove(),Re();return}let e=document.getElementById(J);if(e&&e.dataset.oppId===t.oppId){let l=P();l!==null&&l!=="Marketing Tracker"&&(e.remove(),document.getElementById(`${J}-wrap`)?.remove(),Re());return}e&&(e.remove(),document.getElementById(`${J}-wrap`)?.remove());let o=await bi();if(!o){b.warn("Not Interested: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await te();if(r!=="Marketing Tracker"){b.debug(`[not-interested] gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=_t();if(!n||n.oppId!==t.oppId)return;let i=di(t.oppId),s=document.createElement("span");s.id=`${J}-wrap`,s.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,o.classList.contains("description")&&(o.style.display="flex",o.style.alignItems="center",o.style.gap="16px",o.style.flexWrap="wrap"),s.appendChild(i),o.appendChild(s),b.debug(`Mounted Not Interested button on opp ${t.oppId}`),hi(t.oppId)},Be=null,mo=null,Re=()=>{Be?.disconnect(),Be=null,mo=null},hi=t=>{if(Be&&mo===t)return;Re();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(mo=t,Be=new MutationObserver(()=>{let o=_t();if(!o||o.oppId!==t){Re();return}let r=P();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(J)?.remove(),document.getElementById(`${J}-wrap`)?.remove(),Re();return}document.getElementById(J)||uo()}),Be.observe(e,{childList:!0,subtree:!0}))},po=null,ho,mi=()=>{po||(po=new MutationObserver(()=>{ho===void 0&&(ho=window.setTimeout(()=>{ho=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(J);t&&!e&&!vt&&uo()},300))}),po.observe(document.body,{childList:!0,subtree:!0}))},Ir=()=>{mi(),uo()};var ui="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/property/listing-price-changed",Z="ws-crm-reduce-price-button",Ar="ws-crm-reduce-price-modal",vi=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,_i=/\/v2\/location\/([A-Za-z0-9]+)/,ft=()=>{let t=window.location.pathname,e=t.match(vi);if(!e)return null;let o=t.match(_i);return{oppId:e[1],locationId:o?o[1]:null}},yi=async t=>{let e=await fetch(ui,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.locationId,dispo_opp_id:t.oppId,new_price:t.newPrice,triggered_by_user_id:t.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Reduce Price webhook returned ${e.status} ${e.statusText}`);let o=await e.text();try{let r=JSON.parse(o);if(r?.ok===!1&&r?.error)throw new Error(r.error);return r}catch(r){if(r instanceof Error&&/returned/.test(r.message))throw r;return{}}},yt=()=>{document.getElementById(Ar)?.remove()},gi=t=>{yt();let e=document.createElement("div");e.id=Ar,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,o.addEventListener("mouseenter",()=>{o.style.transform="translateY(-1px)"}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)"}),o.addEventListener("click",()=>{let r=ft();!r||!r.locationId||r.oppId!==t||gi({oppId:r.oppId,locationId:r.locationId,onSuccess:()=>{let n=o.querySelector(".ws-rp-label"),i=o.querySelector(":scope > span:first-child");n&&(n.textContent="Price Reduced \u2014 Buyers Alerted"),o.style.background=a.emeraldGlow,o.style.color=a.emeraldBright,o.style.borderColor=a.emeraldBorder,i&&(i.style.background=a.emerald,i.style.boxShadow=`0 0 6px ${a.emerald}`),setTimeout(()=>{n&&n.textContent==="Price Reduced \u2014 Buyers Alerted"&&(n.textContent="Reduce Price and Alert Buyers",o.style.cssText=$,i&&(i.style.background=a.amber,i.style.boxShadow=`0 0 6px ${a.amber}`))},4e3)}})}),o},wi=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],xi=async()=>{for(let t of wi)try{let e=await A(t,{timeoutMs:2e3});if(e)return e}catch{}return null},gt=!1,go=async()=>{if(!gt){gt=!0;try{await ki()}finally{gt=!1}}},ki=async()=>{let t=ft();if(!t||!t.locationId){document.getElementById(Z)?.remove(),document.getElementById(`${Z}-wrap`)?.remove(),ze();return}let e=document.getElementById(Z);if(e&&e.dataset.oppId===t.oppId){let l=P();l!==null&&l!=="Disposition"&&(e.remove(),document.getElementById(`${Z}-wrap`)?.remove(),ze());return}e&&(e.remove(),document.getElementById(`${Z}-wrap`)?.remove());let o=await xi();if(!o){b.warn("Reduce Price: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await te();if(r!=="Disposition"){b.debug(`[reduce-price] gated off \u2014 pipeline="${r}" (expected Disposition)`);return}let n=ft();if(!n||n.oppId!==t.oppId)return;let i=fi(t.oppId,t.locationId),s=document.createElement("span");s.id=`${Z}-wrap`,s.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,o.classList.contains("description")&&(o.style.display="flex",o.style.alignItems="center",o.style.gap="16px",o.style.flexWrap="wrap"),s.appendChild(i),o.appendChild(s),b.debug(`Mounted Reduce Price button on opp ${t.oppId}`),Ei(t.oppId)},Ge=null,yo=null,ze=()=>{Ge?.disconnect(),Ge=null,yo=null},Ei=t=>{if(Ge&&yo===t)return;ze();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(yo=t,Ge=new MutationObserver(()=>{let o=ft();if(!o||o.oppId!==t){ze();return}let r=P();if(r!==null&&r!=="Disposition"){document.getElementById(Z)?.remove(),document.getElementById(`${Z}-wrap`)?.remove(),ze();return}document.getElementById(Z)||go()}),Ge.observe(e,{childList:!0,subtree:!0}))},vo=null,_o,Ti=()=>{vo||(vo=new MutationObserver(()=>{_o===void 0&&(_o=window.setTimeout(()=>{_o=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(Z);t&&!e&&!gt&&go()},300))}),vo.observe(document.body,{childList:!0,subtree:!0}))},Lr=()=>{Ti(),go()};var Dr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/tc/send-to-title-co",de="ws-crm-send-to-closing-office-button",me="ws-crm-send-to-title-co-wrap",kt="ws-crm-stc-dropdown",Si=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Ci=/\/v2\/location\/([A-Za-z0-9]+)/,Et=()=>{let t=window.location.pathname,e=t.match(Si);if(!e)return null;let o=t.match(Ci);return{oppId:e[1],locationId:o?o[1]:null}},Ii=5*60*1e3,Fr=t=>`ws-crm-stc-cache:${t}`,Ai=t=>{try{let e=window.localStorage.getItem(Fr(t));if(!e)return null;let o=JSON.parse(e);return!o.ts||Date.now()-o.ts>Ii||!Array.isArray(o.checks)||o.checks.length===0?null:o}catch{return null}},Li=(t,e)=>{try{window.localStorage.setItem(Fr(t),JSON.stringify(e))}catch{}},$i=async(t,e)=>{try{let o=await fetch(Dr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:t,triggered_by_user_id:M()??null,dry_run:!0}),credentials:"omit"});return o.ok?await o.json():(b.warn(`[stc] dry_run returned HTTP ${o.status}`),null)}catch(o){return b.warn("[stc] dry_run fetch failed:",o),null}},Mi=async(t,e,o)=>{let r=await fetch(Dr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:t,triggered_by_user_id:M()??null,confirm_resend:o}),credentials:"omit"});if(!r.ok&&r.status!==200)throw new Error(`HTTP ${r.status}`);return await r.json()},Di=t=>{let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(!e)return!1;let o=e.querySelectorAll("a, button, li, [role='tab'], [role='menuitem'], .crm-opportunities-modal-content [class*='tab']");for(let n of o)if((n.textContent?.trim()??"")===t)return n.click(),!0;let r=e.querySelectorAll("*");for(let n of r){if(n.children.length>1)continue;if((n.textContent?.trim()??"")===t)return(n.closest("a, button, li, [role='tab']")||n.parentElement||n).click(),!0}return b.warn(`[stc] folder tab not found for "${t}"`),!1},$r=t=>{let e=document.getElementById(t);if(!e){b.warn(`[stc] field ${t} not found in DOM \u2014 can't scroll to it`);return}e.scrollIntoView({behavior:"smooth",block:"center"});let o=e.style.boxShadow,r=e.style.transition;e.style.transition="box-shadow 0.3s ease, background-color 0.3s ease",e.style.boxShadow=`0 0 0 3px ${a.emerald}, 0 0 18px ${a.emerald}`,window.setTimeout(()=>{e.style.boxShadow=o,window.setTimeout(()=>{e.style.transition=r},400)},2800)},Fi=(t,e)=>{if(e&&Di(e)){window.setTimeout(()=>$r(t),300);return}$r(t)},Q=()=>{document.getElementById(kt)?.remove()},wt=(t,e,o)=>{Q();let r=document.createElement("div");r.id=kt;let n=o.getBoundingClientRect();r.style.cssText=`
    position: fixed;
    top: ${n.bottom+6}px;
  ...
  `,oe(o,n);let i=new Set,s=async()=>{if(n.status==="sending")return;let w=await $i(t,e);if(!w){n={status:"error",message:"Check failed \u2014 click to retry"},oe(o,n);return}if(w.rate_limited){v=Date.now()+h,n={status:"error",checks:n.checks,message:"Rate-limited by GHL \u2014 will retry in 5m"},oe(o,n),b.warn(`[stc] dry_run rate-limited \u2014 cooling down until ${new Date(v).toLocaleTimeString()}`);return}if(!Array.isArray(w.checks)||w.checks.length===0){n={status:"error",message:w.error||"Check failed \u2014 click to retry"},oe(o,n),b.warn("[stc] dry_run returned no checks:",w);return}let _=w.pipeline_name;if(_&&_!=="Transaction Coordination"){b.debug(`[stc] dry_run reports pipeline="${_}" (not TC) \u2014 retracting button + caching`),Tt.add(t),document.getElementById(de)?.remove(),document.getElementById(me)?.remove(),Q(),Ue();return}let f=w.checks;if(n.checks&&n.checks.length===w.checks.length){let C=new Set;f=w.checks.map((x,g)=>n.checks[g]?.filled&&!x.filled?i.has(x.label)?x:(C.add(x.label),{...x,filled:!0}):x),i=C}else i=new Set;n={status:f.every(C=>C.filled)?"ready":"incomplete",checks:f,previouslySentAt:w.previously_sent_at??null},Li(t,{checks:f,previously_sent_at:w.previously_sent_at??null,pipeline_name:_,ts:Date.now()}),oe(o,n),document.getElementById(kt)&&wt(f,w.previously_sent_at,o)},l=async()=>{let w=!!n.previouslySentAt;n={status:"sending",checks:n.checks,previouslySentAt:n.previouslySentAt},oe(o,n),Q();try{let _=await Mi(t,e,w);if(_.needs_confirm){n={status:"ready",checks:_.checks,previouslySentAt:_.previously_sent_at},oe(o,n),_.checks&&wt(_.checks,_.previously_sent_at,o);return}if(_.ok){n={status:"sent",checks:_.checks,previouslySentAt:new Date().toISOString()},oe(o,n),window.setTimeout(()=>{k()},3500);return}n={status:"error",checks:_.checks,message:_.error||"Validation failed"},oe(o,n),_.checks&&wt(_.checks,_.previously_sent_at,o)}catch(_){b.error("[stc] send failed:",_),n={status:"error",message:"Network error \u2014 click to retry"},oe(o,n)}};o.addEventListener("click",()=>{switch(n.status){case"ready":l();break;case"incomplete":{document.getElementById(kt)?Q():n.checks&&wt(n.checks,n.previouslySentAt,o);break}case"error":case"sent":k();break}});let d,c=!1,p=Date.now(),m=1e4,h=5*60*1e3,v=0,k=async()=>{if(!c){if(Date.now()<v){b.debug(`[stc] dry_run suppressed \u2014 rate-limit cooldown until ${new Date(v).toLocaleTimeString()}`);return}c=!0,p=Date.now();try{await s()}finally{c=!1}}};k();let T=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(T){let w=new MutationObserver(()=>{if(d!==void 0||c)return;let f=Date.now()-p,S=Math.max(1500,m-f);d=window.setTimeout(()=>{d=void 0,n.status!=="sending"&&k()},S)});w.observe(T,{childList:!0,subtree:!0,characterData:!0}),o.dataset.observerActive="1";let _=window.setInterval(()=>{document.body.contains(o)||(w.disconnect(),d!==void 0&&window.clearTimeout(d),window.clearInterval(_))},5e3)}return o},Pi=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],Oi=async()=>{for(let t of Pi)try{let e=await A(t,{timeoutMs:1500});if(e)return b.debug(`[send-to-title-co] mount target: ${t}`),e}catch{}return b.warn(`[send-to-title-co] no mount target found. URL: ${window.location.pathname}`),null},Tt=new Set,Mr=null,Ni=t=>{t&&t!==Mr&&(Tt.clear(),Mr=t)},xt=!1,ko=async()=>{if(xt){b.debug("[stc] mount:00-LOCKED ensureMounted already in-flight \u2014 skip");return}xt=!0;try{await Bi()}catch(t){b.error("[stc] ensureMountedImpl threw:",t)}finally{xt=!1}},Bi=async()=>{b.debug("[stc] mount:01 ensureMountedImpl entered");let t=Et();if(!t||!t.locationId){b.debug(`[stc] mount:02-BAIL no ctx or no locationId. ctx=${JSON.stringify(t)}`),document.getElementById(de)?.remove(),document.getElementById(me)?.remove(),Q();return}if(b.debug(`[stc] mount:02 ctx ok \u2014 oppId=${t.oppId} locId=${t.locationId}`),Ni(t.oppId),Tt.has(t.oppId)){b.debug(`[stc] mount:02-BAIL opp ${t.oppId} previously confirmed non-TC this session`),document.getElementById(de)?.remove(),document.getElementById(me)?.remove(),Q();return}let e=document.getElementById(de);if(e&&e.dataset.oppId===t.oppId){b.debug("[stc] mount:03-SKIP existing button for same opp");let l=P();l!==null&&l!=="Transaction Coordination"&&(e.remove(),document.getElementById(me)?.remove(),Q(),Ue());return}e&&(b.debug("[stc] mount:03 removing stale existing button"),e.remove(),document.getElementById(me)?.remove(),Q()),b.debug("[stc] mount:04 calling findMountTarget");let o=await Oi();if(!o){b.debug("[stc] mount:04-BAIL findMountTarget returned null");return}b.debug(`[stc] mount:04 target found tag=${o.tagName} class="${o.className}"`);let r=P();if(r!==null&&r!=="Transaction Coordination"){b.debug(`[stc] mount:05-GATED DOM pipeline="${r}" \u2014 skipping mount`);return}let n=Et();if(!n||n.oppId!==t.oppId||!n.locationId){b.debug(`[stc] mount:06-BAIL ctx changed during awaits. was=${t.oppId} now=${n?.oppId}`);return}b.debug("[stc] mount:06 building button (pipeline gate will run via dry_run)");let i=Hi(t.oppId,n.locationId),s=document.createElement("span");s.id=me,s.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,o.classList.contains("description")&&(o.style.display="flex",o.style.alignItems="center",o.style.gap="16px",o.style.flexWrap="wrap"),s.appendChild(i),o.appendChild(s),b.debug(`[stc] mount:07-OK Mounted on opp ${t.oppId}, inDoc=${document.body.contains(i)}`),Ri(t.oppId)},qe=null,xo=null,Ue=()=>{qe?.disconnect(),qe=null,xo=null},Ri=t=>{if(qe&&xo===t)return;Ue();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(xo=t,qe=new MutationObserver(()=>{let o=Et();if(!o||o.oppId!==t){Ue();return}let r=P();if(r!==null&&r!=="Transaction Coordination"){document.getElementById(de)?.remove(),document.getElementById(me)?.remove(),Q(),Ue();return}document.getElementById(de)||ko()}),qe.observe(e,{childList:!0,subtree:!0}))},fo=null,wo,Gi=()=>{fo||(fo=new MutationObserver(()=>{wo===void 0&&(wo=window.setTimeout(()=>{wo=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(de);if(t&&!e&&!xt){let o=Et();if(o&&Tt.has(o.oppId))return;ko()}},300))}),fo.observe(document.body,{childList:!0,subtree:!0}))},Hr=()=>{Gi(),ko()};var zi="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/search",qi="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/opp/set-field",Ui="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/opp/get-field",Nr="ldtyB2UXKi9vOp8nLUH7",Vi="WVD2gb9swLsgcdBEtDVq",ee="ws-crm-closing-agent-picker-button",Br="ws-crm-closing-agent-picker-modal",Wi=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,ji=/\/v2\/location\/([A-Za-z0-9]+)/,je=()=>{let t=window.location.pathname,e=t.match(Wi);if(!e)return null;let o=t.match(ji);return{oppId:e[1],locationId:o?o[1]:null}},$o=async t=>{let e=await fetch(zi,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,query:t.query??"",contact_id:t.contactId??null,contact_type:t.contactType??null,limit:t.limit??20}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},Pr=async t=>{let e=await fetch(qi,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,opp_id:t.oppId,field_id:t.fieldId,value:t.value}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},Ki=async(t,e)=>{try{let o=await fetch(Ui,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,opp_id:t,field_id:Nr}),credentials:"omit"});if(!o.ok)return null;let r=await o.json(),n=typeof r.value=="string"&&r.value.trim()?r.value.trim():null;if(!n||!/^[A-Za-z0-9]{16,}$/.test(n))return null;let i=await $o({tenantId:e,contactId:n,limit:1});if(!i.ok||!i.results.length)return null;let s=i.results[0];return{id:s.id,name:s.name,companyName:s.companyName??""}}catch(o){return b.warn("[closing-agent] fetchCurrent failed:",o),null}},ue=null,Yi=60*60*1e3,Mo=t=>`ws-crm-to-cache:${t}`,Rr=t=>{try{let e=window.localStorage.getItem(Mo(t));if(!e)return null;let o=JSON.parse(e);return!o.ts||Date.now()-o.ts>Yi||typeof o.contactId!="string"||!o.contactId||typeof o.contactName!="string"||!o.contactName?null:o}catch{return null}},Gr=(t,e,o)=>{try{window.localStorage.setItem(Mo(t),JSON.stringify({contactId:e,contactName:o,ts:Date.now()}))}catch{}},Xi=t=>{try{window.localStorage.removeItem(Mo(t))}catch{}},Co="ws-crm-closing-agent-field-link",Ji=()=>{let t=document.querySelector(".crm-opportunities-modal");t&&(t.querySelectorAll(`.${Co}`).forEach(e=>e.remove()),t.querySelectorAll("input[data-ws-closing-agent-decorated]").forEach(e=>{e.style.removeProperty("color"),e.style.removeProperty("caret-color"),e.style.removeProperty("visibility"),delete e.dataset.wsClosingAgentDecorated}))},Eo=!1,Do=async t=>{if(Eo)return;let e=zr();if(!e)return;let o=(e.value||"").trim();if(!o){b.info("[closing-agent] tryDecorate: input has empty value");return}if(!/^[A-Za-z0-9]{16,}$/.test(o)){b.info(`[closing-agent] tryDecorate: value '${o}' doesn't match contact-ID pattern (expecting 16+ alphanumeric)`);return}if(e.dataset.wsClosingAgentDecorated!==o){Eo=!0;try{b.info(`[closing-agent] tryDecorate: looking up ${o}`);let r=await $o({tenantId:t,contactId:o,limit:1});if(!r.ok||!r.results.length){b.warn(`[closing-agent] tryDecorate: contact ${o} not found`);return}let n=r.results[0];b.info(`[closing-agent] tryDecorate: applying ${n.name} (${n.id})`),Io(n.id,n.name,t);let i=document.getElementById(ee);if(i){ce(i,{status:"linked",contactName:n.name});let s=je();s&&Gr(s.oppId,n.id,n.name)}}catch(r){b.warn("[closing-agent] tryDecorate lookup failed:",r)}finally{Eo=!1}}},Zi=/^TC\s*[-‐–—]\s*Closing\s+Agent$/i,zr=()=>{let e=document.querySelectorAll("*"),o=[],r=[],n=[];for(let d of e){let c=(d.textContent||"").trim();Zi.test(c)?o.push(d):c.length<80&&/Title\s+Officer/i.test(c)&&r.push(c);for(let p of["aria-label","placeholder","data-label","title","name","data-test"]){let m=d.getAttribute(p);m&&/Title\s+Officer/i.test(m)&&n.push({tag:d.tagName.toLowerCase(),attr:p,value:m})}}if(b.info(`[closing-agent] findInput: ${o.length} regex-matched, ${r.length} near-text matches, ${n.length} attribute matches`),r.length&&b.warn("[closing-agent] near-text samples:",[...new Set(r)].slice(0,8)),n.length&&b.warn("[closing-agent] attribute matches:",n.slice(0,8)),o.length===0)return null;o.sort((d,c)=>d.querySelectorAll("*").length-c.querySelectorAll("*").length);let s=o[0].parentElement,l=0;for(;s&&l<8;){let d=s.querySelector("input");if(d)return b.info(`[closing-agent] findInput: located input ${l} levels up from label`),d;s=s.parentElement,l+=1}return b.warn("[closing-agent] findInput: label found but no <input> within 8 ancestors"),null},Io=(t,e,o)=>{let r=zr();if(!r)return;let n=r.dataset.wsClosingAgentDecorated;if(n!==t){n&&r.parentElement?.querySelectorAll(`.${Co}`).forEach(s=>s.remove());{let i=r.parentElement;if(!i)return;r.dataset.wsClosingAgentDecorated=t,r.style.visibility="hidden",window.getComputedStyle(i).position==="static"&&(i.style.position="relative");let l=document.createElement("a");l.className=Co,l.href=`/v2/location/${o}/contacts/detail/${t}`,l.textContent=e,l.style.cssText=`
      position: absolute;
      inset: 0;
      display: flex;
      align-items: center;
      ...
      border-radius: ${y.sm};
      z-index: 2;
    `,l.addEventListener("mouseenter",()=>{l.style.textDecoration="underline"}),l.addEventListener("mouseleave",()=>{l.style.textDecoration="none"}),l.addEventListener("click",d=>{d.preventDefault(),d.stopPropagation(),window.location.href=l.href}),i.appendChild(l)}}},St=()=>{document.getElementById(Br)?.remove()},Qi=t=>{St();let e=document.createElement("div");e.id=Br,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `;let r={status:"loading"};ce(o,r);let n=Rr(t);return n&&(r={status:"linked",contactName:n.contactName},ce(o,r),ue={oppId:t,contactId:n.contactId,contactName:n.contactName,tenantId:e}),(async()=>{let s=await Ki(t,e);s?(r={status:"linked",contactName:s.name},ce(o,r),ue={oppId:t,contactId:s.id,contactName:s.name,tenantId:e},Io(s.id,s.name,e)):n||(r={status:"empty"},ce(o,r),ue?.oppId===t&&(ue=null),Ji(),Xi(t))})(),o.addEventListener("mouseenter",()=>{r.status!=="loading"&&r.status!=="saving"&&(o.style.transform="translateY(-1px)")}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)"}),o.addEventListener("click",()=>{r.status==="loading"||r.status==="saving"||Qi({oppId:t,locationId:e,onPick:async s=>{r={status:"saving"},ce(o,r);try{let l=[Pr({tenantId:e,oppId:t,fieldId:Nr,value:s.id})];s.companyName&&s.companyName.trim()&&l.push(Pr({tenantId:e,oppId:t,fieldId:Vi,value:s.companyName.trim()}));let c=(await Promise.all(l))[0];if(!c.ok)throw new Error(c.error??"Save failed");r={status:"linked",contactName:s.name},ce(o,r),ue={oppId:t,contactId:s.id,contactName:s.name,tenantId:e},Gr(t,s.id,s.name),Io(s.id,s.name,e),b.info(`Closing Agent linked: ${s.name} (${s.id})${s.companyName?` \u2014 Company: ${s.companyName}`:""}`)}catch(l){b.error("Closing Agent link failed",l),r={status:"empty"},ce(o,r)}}})}),o},ts=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading"],os=async()=>{for(let t of ts)try{let e=await A(t,{timeoutMs:2e3});if(e)return e}catch{}return null},Ve=!1,At=async()=>{if(!Ve){Ve=!0;try{await rs()}finally{Ve=!1}}},rs=async()=>{let t=je();if(!t||!t.locationId){document.getElementById(ee)?.remove(),document.getElementById(`${ee}-wrap`)?.remove(),It();return}let e=document.getElementById(ee);if(e&&e.dataset.oppId===t.oppId){let d=P();d!==null&&d!=="Transaction Coordination"&&(e.remove(),document.getElementById(`${ee}-wrap`)?.remove(),It());return}e&&(e.remove(),document.getElementById(`${ee}-wrap`)?.remove());let o=P();if(o!==null&&o!=="Transaction Coordination")return;let r=await os();if(!r||!(Rr(t.oppId)!==null||ue!==null&&ue.oppId===t.oppId)&&await te()!=="Transaction Coordination")return;let i=je();if(!i||i.oppId!==t.oppId||!i.locationId)return;let s=es(t.oppId,i.locationId),l=document.createElement("span");l.id=`${ee}-wrap`,l.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,r.classList.contains("description")&&(r.style.display="flex",r.style.alignItems="center",r.style.gap="16px",r.style.flexWrap="wrap"),l.appendChild(s),r.appendChild(l),b.debug(`Mounted Closing Agent picker on opp ${t.oppId}`),ns(t.oppId),Do(i.locationId)},We=null,Ao=null,Lo=null,It=()=>{We?.disconnect(),We=null,Ao=null,Lo=null},ns=t=>{let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(We&&Ao===t&&Lo===e||(It(),Ao=t,Lo=e,We=new MutationObserver(()=>{let o=je();if(!o||o.oppId!==t||!o.locationId){It();return}document.getElementById(ee)||At(),Do(o.locationId)}),We.observe(e,{childList:!0,subtree:!0})))},To=null,So,as=()=>{To||(To=new MutationObserver(()=>{So===void 0&&(So=window.setTimeout(()=>{So=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(ee);t&&!e&&!Ve&&At()},300))}),To.observe(document.body,{childList:!0,subtree:!0}))},Or,is=()=>{Or===void 0&&(Or=window.setInterval(()=>{let t=je();if(!t||!t.locationId||!!!document.querySelector(".crm-opportunities-modal"))return;!!!document.getElementById(ee)&&!Ve&&(b.info("[closing-agent] safety-net: button missing on opp page \u2014 remounting"),At()),Do(t.locationId)},2e3))},qr=()=>{b.info("[closing-agent] mountClosingAgentPickerButton invoked"),as(),is(),At()};var ss="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/team/manage",xe="ws-crm-manage-team-cta",Vr="ws-crm-manage-team-modal",ls=/\/v2\/location\/([A-Za-z0-9]+)\/settings\/staff\/team/,Wr={manager:"Manager",acq:"Acquisitions Rep",dispo:"Disposition Rep",tc:"Transaction Coordinator"},ds={manager:{bg:"rgba(75, 139, 245, 0.15)",fg:a.blue},acq:{bg:a.emeraldGlow,fg:a.emerald},dispo:{bg:"rgba(232, 163, 60, 0.15)",fg:a.amber},tc:{bg:"rgba(212, 63, 74, 0.15)",fg:a.crimson}},jr=()=>{let t=window.location.pathname.match(ls);return t?{tenantId:t[1]}:null},$t=async t=>{let e=await fetch(ss,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t),credentials:"omit"});if(!e.ok){let o="";try{o=await e.text()}catch{}throw new Error(`Team.Manage HTTP ${e.status}${o?` \u2014 ${o.slice(0,200)}`:""}`)}return await e.json()},Y=t=>String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),be=t=>Y(t),Fo="ws-crm-manage-team-toast",Oo=(t,e="info",o=4e3)=>{document.getElementById(Fo)?.remove();let r=e==="success"?a.emerald:e==="error"?a.crimson:a.ash,n=document.createElement("div");n.id=Fo,n.style.cssText=`
    position: fixed;
    bottom: 24px;
    right: 24px;
    z-index: 9999999;
  ...
  `,n.textContent=t,document.body.appendChild(n),o>0&&window.setTimeout(()=>{document.getElementById(Fo)===n&&n.remove()},o)},Ho=()=>{document.getElementById(Vr)?.remove()},cs=(t,e)=>{let o=ds[t]||{bg:a.slate,fg:a.ash};return`
    <span style="
      display: inline-block;
      background: ${o.bg};
      color: ${o.fg};
      ...
      margin-bottom: 4px;
    ">${Y(e)}</span>
  `},bs=(t,e)=>{let o=[t.first_name,t.last_name].filter(Boolean).join(" ")||"(no name)",r=t.roles.map(s=>cs(s.key,s.display)).join(""),n=e?"":"opacity: 0.45; pointer-events: none; cursor: not-allowed;",i=e?"":'title="Manager-only action"';return`
  ...
  `+t.map(r=>bs(r,e)).join(""),Mt=async t=>{Ho();let e=document.createElement("div");e.id=Vr,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999998;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(o),document.body.appendChild(e),o.querySelector("#ws-team-close")?.addEventListener("click",Ho);let r;try{r=await $t({tenant_id:t,action:"list"})}catch(d){let c=o.querySelector("#ws-team-content");c&&(c.innerHTML=`
    ...
    `:"",l=o.querySelector("#ws-team-content");if(l&&(l.innerHTML=s+ps(r.roster,i)),i)o.querySelector("#ws-team-add")?.addEventListener("click",()=>{hs(t)}),o.querySelectorAll(".ws-team-edit").forEach(d=>{d.addEventListener("click",c=>{let p=c.currentTarget;ms(t,{email:p.dataset.email||"",name:p.dataset.name||"",currentRoles:(p.dataset.currentRoles||"").split(",").filter(Boolean)})})}),o.querySelectorAll(".ws-team-remove").forEach(d=>{d.addEventListener("click",c=>{let p=c.currentTarget;us(t,{email:p.dataset.email||"",name:p.dataset.name||"",roleCount:parseInt(p.dataset.roleCount||"0",10)})})});else{let d=o.querySelector("#ws-team-actions");d&&(d.innerHTML=`
      ...
      `)}},hs=t=>{let e="ws-crm-manage-team-add-modal";document.getElementById(e)?.remove();let o=document.createElement("div");o.id=e,o.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `;let i=s=>{let l=e.currentRoles.includes(s)?"checked":"";return`
      <label style="display: flex; align-items: center; gap: 10px; padding: 10px 12px; background: ${a.slate}; border: 1px solid ${a.steel}; border-radius: ${y.sm}; cursor: pointer; margin-bottom: 6px;">
        ...
        <span style="font-size: 13px;">${Y(Wr[s])}</span>
      </label>
    `};n.innerHTML=`
  ...
  `,e.addEventListener("click",()=>{Mt(t)}),Lt(e),setTimeout(()=>Lt(e),200),setTimeout(()=>Lt(e),800),window.addEventListener("resize",()=>Lt(e)),e},ks=[".hr-card-header__main",".hr-page-header__main",".page-header h1",".page-header h2","h1.page-title","h2.page-title",".staff-page-wrapper",".settings-content"],Es=async()=>{for(let t of ks)try{let e=await A(t,{timeoutMs:1500});if(e)return e}catch{}return null},Po=!1,Ts=async()=>{if(!Po){Po=!0;try{let t=jr();if(!t){document.getElementById(xe)?.remove();return}let e=document.getElementById(xe);if(e){if(e.dataset.tenantId===t.tenantId)return;e.remove()}let o=await Es();if(!o){let i=document.createElement("div");i.id=`${xe}-floater`,i.style.cssText=`
        position: fixed;
        top: 88px;
        right: 32px;
        z-index: 99999;
      `;let s=Ur(t.tenantId);s.dataset.tenantId=t.tenantId,i.appendChild(s),document.body.appendChild(i),b.debug(`[manage-team] mounted (floater fallback) for tenant ${t.tenantId}`);return}let r=Ur(t.tenantId);r.dataset.tenantId=t.tenantId;let n=document.createElement("span");n.style.cssText="display: inline-flex; align-items: center; margin-left: 16px; vertical-align: middle;",n.appendChild(r),o.appendChild(n),b.debug(`[manage-team] mounted on My Staff page for tenant ${t.tenantId}`)}finally{Po=!1}}},Kr=()=>{if(!jr()){document.getElementById(xe)?.remove(),document.getElementById(`${xe}-floater`)?.remove();return}Ts()};var Ss="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contract/stage",Ro="ws-crm-prepare-contract-button",Yr="ws-crm-prepare-contract-menu",ne="ws-crm-prepare-contract-modal",Xr="ws-crm-modal-date-style",ie="https://services.leadconnectorhq.com",tn=()=>{if(document.getElementById(Xr))return;let t=document.createElement("style");t.id=Xr,t.textContent=`
  ...
  `,document.head.appendChild(t)},Go="__WS_CRM_CAPTURED_AUTH",Cs=50*60*1e3,on=()=>{try{let t=localStorage.getItem(Go);if(!t)return null;let e=JSON.parse(t);return e?.authorization?Date.now()-(e.capturedAt||0)>Cs?(localStorage.removeItem(Go),null):e:null}catch{return null}},Is=t=>{try{localStorage.setItem(Go,JSON.stringify(t))}catch{}},se=()=>{let t=on(),e=window;if(e.__WS_CRM_DIAG=e.__WS_CRM_DIAG||{},e.__WS_CRM_DIAG.authChecks=(e.__WS_CRM_DIAG.authChecks||0)+1,console.log(`[ws-crm-customizer] [auth-check #${e.__WS_CRM_DIAG.authChecks}]`,t?`present (captured ${Date.now()-t.capturedAt}ms ago, ${t.authorization.slice(0,20)}\u2026)`:"NULL \u2014 interceptor has not captured a token yet"),!t)throw new Error("GHL auth not yet captured. Try clicking around the page (refresh, open a contact) to trigger a GHL API call, then retry.");let o={Authorization:t.authorization,Version:"2021-07-28",Channel:"APP",Source:"WEB_USER"};return t.tokenId&&(o["token-id"]=t.tokenId),o},Jr=!1,As=()=>{if(Jr)return;Jr=!0;let t=window;t.__WS_CRM_DIAG=t.__WS_CRM_DIAG||{fetchCalls:0,xhrCalls:0,fetchAuthSeen:0,xhrAuthSeen:0,captured:0,lastSeenHeaders:null};let e=(l,d,c)=>{t.__WS_CRM_DIAG.lastSeenHeaders={url:l,headers:d,source:c};let p=d.authorization||d.Authorization;if(!p){t.__WS_CRM_DIAG.fetchCalls+t.__WS_CRM_DIAG.xhrCalls<=3&&console.log(`[ws-crm-customizer] [intercept:${c} no-auth]`,{url:l.split("?")[0],headerKeys:Object.keys(d)});return}c==="fetch"?t.__WS_CRM_DIAG.fetchAuthSeen++:t.__WS_CRM_DIAG.xhrAuthSeen++;let m=d["token-id"]||d["Token-Id"]||void 0,h=on()?.authorization;Is({authorization:p,tokenId:m,capturedAt:Date.now()}),h!==p&&(t.__WS_CRM_DIAG.captured++,console.log(`[ws-crm-customizer] [intercept:${c}] captured GHL auth from ${l.split("?")[0]} (${p.slice(0,24)}\u2026)`))},o=window.fetch;window.fetch=async function(...l){let d=typeof l[0]=="string"?l[0]:l[0]instanceof URL?l[0].toString():l[0].url,c=l[1]||{};if(d&&d.includes("services.leadconnectorhq.com")){t.__WS_CRM_DIAG.fetchCalls++;let p=c.headers;!p&&l[0]instanceof Request&&(p=l[0].headers);let m={};if(p instanceof Headers)p.forEach((h,v)=>m[v.toLowerCase()]=h);else if(Array.isArray(p))p.forEach(([h,v])=>m[h.toLowerCase()]=v);else if(p&&typeof p=="object")for(let h of Object.keys(p))m[h.toLowerCase()]=p[h];e(d,m,"fetch")}return o.apply(this,l)};let r=XMLHttpRequest.prototype,n=r.open,i=r.setRequestHeader,s=r.send;r.open=function(l,d,...c){return this.__wsCrmUrl=d,this.__wsCrmHeaders={},n.call(this,l,d,...c)},r.setRequestHeader=function(l,d){return this.__wsCrmHeaders&&typeof l=="string"&&(this.__wsCrmHeaders[l.toLowerCase()]=d),i.call(this,l,d)},r.send=function(l){let d=this.__wsCrmUrl,c=this.__wsCrmHeaders;return d&&d.includes("services.leadconnectorhq.com")&&c&&(t.__WS_CRM_DIAG.xhrCalls++,e(d,c,"xhr")),s.call(this,l)},b.debug("[prepare-contract] interceptors installed (fetch + XHR)")},rn=()=>{let t=L();return!t||!t.locationId?null:{contactId:t.contactId,tenantId:t.locationId}},Ls=t=>{let e=t.dataset;return e.tenantId&&e.contactId?{tenantId:e.tenantId,contactId:e.contactId,oppId:e.oppId||void 0}:e.tenantId&&e.oppId?{tenantId:e.tenantId,contactId:"",oppId:e.oppId}:rn()},$s=t=>{let e=(t||"").trim().toLowerCase();return/^\[?\s*aa\s*\]?\s*[-\s]/i.test(e)||/assignment.*(contract|agreement)/i.test(e)?"AA":/^\[?\s*pa\s*\]?\s*[-\s]/i.test(e)||/purchase.*(contract|agreement)/i.test(e)?"PA":"passthrough"},nn="__WS_CRM_TEMPLATES_CACHE_V1",Ms=24*60*60*1e3,Zr=t=>{try{let e=window.localStorage.getItem(nn);if(!e)return null;let o=JSON.parse(e);return o.tenantId!==t||Date.now()-o.cachedAt>Ms?null:o.templates}catch{return null}},Ds=(t,e)=>{try{let o={tenantId:t,templates:e,cachedAt:Date.now()};window.localStorage.setItem(nn,JSON.stringify(o))}catch{}},Fs=async t=>{let e=`${ie}/proposals/templates/bulk?type.in%5B%5D=proposal&type.in%5B%5D=estimate&locationId.eq=${encodeURIComponent(t)}&skip=0&limit=10`,o;try{o=await fetch(e,{method:"GET",headers:se()})}catch(s){let l=Zr(t);if(l)return b.info(`[prepare-contract] templates network failure, using cache (${l.length} items)`),l;throw s}if(!o.ok){let s=Zr(t);if(s)return b.info(`[prepare-contract] templates fetch ${o.status}, using cache (${s.length} items)`),s;let l="";try{l=await o.text()}catch{}let d=l?` \u2014 ${l.slice(0,200)}`:"";throw new Error(`Templates fetch returned ${o.status} ${o.statusText}${d}`)}let r=await o.json();console.log("[ws-crm-customizer] [templates response]",Array.isArray(r)?`array of ${r.length}`:`keys: ${Object.keys(r||{}).join(", ")}`,r);let n=[];Array.isArray(r)?n=r:Array.isArray(r?.templates)?n=r.templates:Array.isArray(r?.items)?n=r.items:Array.isArray(r?.data)?n=r.data:Array.isArray(r?.results)?n=r.results:Array.isArray(r?.docs)?n=r.docs:r?.data&&Array.isArray(r.data.templates)?n=r.data.templates:r?.data&&Array.isArray(r.data.items)&&(n=r.data.items);let i=n.map(s=>({id:(s._id??s.id??s.templateId)||"",name:s.name||s.title||"(unnamed template)",type:$s(s.name||s.title||"")})).filter(s=>!!s.id);return i.length>0&&Ds(t,i),i},Hs=async(t,e)=>{let o=await fetch(`${ie}/opportunities/pipelines?locationId=${encodeURIComponent(t)}`,{method:"GET",headers:se()});if(!o.ok)throw new Error(`Pipelines fetch ${o.status}`);let n=((await o.json()).pipelines||[]).find(l=>/acquisition/i.test(l.name));if(!n)throw new Error("No Acquisition pipeline found on tenant");let i=await fetch(`${ie}/opportunities/search?location_id=${encodeURIComponent(t)}&pipeline_id=${encodeURIComponent(n.id)}&contact_id=${encodeURIComponent(e)}&status=open&limit=20`,{method:"GET",headers:se()});if(!i.ok)throw new Error(`ACQ opp search ${i.status}`);return((await i.json()).opportunities||[]).map(l=>({id:l.id,name:l.name||"(unnamed opp)",propertyAddress:l.name||"",updatedAt:l.updatedAt||null})).sort((l,d)=>{let c=l.updatedAt?Date.parse(l.updatedAt):0;return(d.updatedAt?Date.parse(d.updatedAt):0)-c})},Ps=async(t,e)=>{let o=await fetch(`${ie}/opportunities/pipelines?locationId=${encodeURIComponent(t)}`,{method:"GET",headers:se()});if(!o.ok)throw new Error(`Pipelines fetch ${o.status}`);let n=((await o.json()).pipelines||[]).find(d=>/marketing\s*tracker/i.test(d.name));if(!n)throw new Error("No Marketing Tracker pipeline found on tenant");let i=await fetch(`${ie}/opportunities/search?location_id=${encodeURIComponent(t)}&pipeline_id=${encodeURIComponent(n.id)}&contact_id=${encodeURIComponent(e)}&status=open&limit=20`,{method:"GET",headers:se()});if(!i.ok)throw new Error(`MT opp search ${i.status}`);return((await i.json()).opportunities||[]).map(d=>({id:d.id,name:d.name||"(unnamed opp)",propertyAddress:d.name||"",updatedAt:d.updatedAt||null})).sort((d,c)=>{let p=d.updatedAt?Date.parse(d.updatedAt):0;return(c.updatedAt?Date.parse(c.updatedAt):0)-p})},Os=async t=>{let e=await fetch(Ss,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId,template_id:t.templateId,template_type:t.templateType,opp_id:t.oppId,closing_date:t.closingDate,buyer_earnest_due_date:t.buyerEarnestDueDate,lender_source:t.lenderSource,triggered_by_user_id:t.userId,overrides:t.overrides}),credentials:"omit"});if(!e.ok)throw new Error(`Contract.Stage HTTP ${e.status}`);return await e.json()},Ns=async(t,e,o)=>{let r=`${ie}/proposals/templates/to-proposal/${t}/${e}?locale=en-US&contactId=${encodeURIComponent(o)}`,n=await fetch(r,{method:"POST",headers:se()});if(!n.ok)throw new Error(`to-proposal HTTP ${n.status} ${n.statusText}`);let i=await n.json(),s=i._id||i.id||i.proposal?._id||i.proposal?.id;if(!s)throw new Error("to-proposal returned no proposal id");return s},Bs=async(t,e)=>{let o=await fetch(`${ie}/proposals/document/${e}?locationId=${encodeURIComponent(t)}`,{method:"GET",headers:se()});if(!o.ok){let i=await o.text().catch(()=>"");throw new Error(`GET proposal ${e} HTTP ${o.status} ${i.slice(0,200)}`)}let r=await o.json(),n=r?.document||r?.proposal||r;if(!n||typeof n!="object")throw new Error(`GET proposal returned unexpected shape: ${JSON.stringify(r).slice(0,200)}`);return n},Rs=async t=>{let e=await fetch(`${ie}/contacts/${t}`,{method:"GET",headers:se()});if(!e.ok){let n=await e.text().catch(()=>"");throw new Error(`GET contact ${t} HTTP ${e.status} ${n.slice(0,200)}`)}let o=await e.json(),r=o?.contact||o;return{firstName:r?.firstName||"",lastName:r?.lastName||"",email:r?.email||""}},Gs=async(t,e,o,r)=>{let n;try{n=await Bs(t,e)}catch(v){return{ok:!1,body:`fetchProposalDocument: ${v.message}`}}let i;try{i=await Rs(o)}catch(v){return{ok:!1,body:`fetchContactBasics: ${v.message}`}}let s=`${i.firstName} ${i.lastName}`.trim()||i.email||o,l={id:o,firstName:i.firstName,lastName:i.lastName,email:i.email,contactName:s,isPrimary:!0,hasCompleted:!1,imgUrl:"",entityName:"contacts",role:"signer"},p=[...(Array.isArray(n.recipients)?n.recipients:[]).filter(v=>v?.entityName!=="contacts"),l],m={name:r||n.name,locationId:n.locationId||t,pages:n.pages||[],variables:n.variables||[],fillableFields:n.fillableFields||[],timezone:n.timezone||{},recipients:p,grandTotal:n.grandTotal},h=await fetch(`${ie}/proposals/document/${e}`,{method:"PUT",headers:{...se(),"Content-Type":"application/json"},body:JSON.stringify(m)});if(!h.ok){let v=await h.text().catch(()=>"");return{ok:!1,status:h.status,body:v}}return{ok:!0}},zs=(t,e)=>{let o=`/v2/location/${t}/payments/proposals-estimates/edit/${e}`;window.location.href=o},Ft="__WS_CRM_PENDING_INSTRUCTIONS",qs=60*1e3,Us=[{matches:"2 signers",title:"Special Instructions for 2-Signer Agreement",bodyHtml:`
    ...
    `}],Vs=t=>{let e=(t||"").toLowerCase();for(let o of Us)if(e.includes(o.matches.toLowerCase()))return o;return null},Ws=(t,e,o)=>{try{let r={proposalId:t,title:e,bodyHtml:o,ts:Date.now()};localStorage.setItem(Ft,JSON.stringify(r))}catch{}},js=()=>{try{let t=localStorage.getItem(Ft);if(!t)return null;let e=JSON.parse(t);return!e?.proposalId||!e?.title||!e?.bodyHtml||Date.now()-(e.ts||0)>qs?(localStorage.removeItem(Ft),null):e}catch{return null}},Ks=()=>{try{localStorage.removeItem(Ft)}catch{}},Ys=(t,e)=>{document.getElementById(ne)?.remove();let o=document.createElement("div");o.id=ne,o.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.7);
  ...
  `,o.appendChild(r),document.body.appendChild(o),r.querySelector("#ws-pc-instructions-ack")?.addEventListener("click",()=>o.remove())},Xs=/\/v2\/location\/[^/]+\/payments\/proposals-estimates\/edit\/([a-zA-Z0-9]+)/,Js=()=>{let t=window.location.pathname.match(Xs);if(!t)return;let e=t[1],o=js();o&&o.proposalId===e&&(Ks(),window.setTimeout(()=>{Ys(o.title,o.bodyHtml)},600))},Qr="ws-crm-prepare-contract-toast",B=(t,e="info",o=4e3)=>{document.getElementById(Qr)?.remove();let r=document.createElement("div");r.id=Qr;let n=e==="error"?a.crimson:a.emerald;r.style.cssText=`
    position: fixed;
    bottom: 24px;
    right: 24px;
    z-index: 999999;
  ...
  `,r.textContent=t,document.body.appendChild(r),window.setTimeout(()=>r.remove(),o)},en=(t,e)=>new Promise(o=>{document.getElementById(ne)?.remove();let r=document.createElement("div");r.id=ne,r.style.cssText=`
      position: fixed;
      inset: 0;
      z-index: 999999;
      background: rgba(0, 0, 0, 0.6);
  ...
  `,I=`
    ...
    padding-right: 28px;
  `,D=N=>['<option value="">(unset)</option>'].concat(T.map(G=>`<option value="${re(G)}"${G===N?" selected":""}>${V(G)}</option>`)).join("");_.innerHTML=`
    <div style="
      font-family: ${u.mono};
      font-size: 10px;
      letter-spacing: 0.14em;
  ...
  `,w.appendChild(_),document.body.appendChild(w);let O=_.querySelector("#ws-pc-purchase-price"),j=_.querySelector("#ws-pc-emd"),U=_.querySelector("#ws-pc-cash-at-close"),H=!1;U?.addEventListener("input",()=>{H=!0});let ke=()=>{if(H)return;let N=Number(O?.value||0),G=Number(j?.value||0);U&&(U.value=String(Math.max(0,N-G)))};O?.addEventListener("input",ke),j?.addEventListener("input",ke);let et=()=>w.remove();w.addEventListener("click",N=>{N.target===w&&et()}),_.querySelector("#ws-pc-cancel")?.addEventListener("click",et);let K=_.querySelector("#ws-pc-submit"),W=_.querySelector("#ws-pc-error");K?.addEventListener("click",async()=>{if(K){W&&(W.textContent=""),K.disabled=!0,K.textContent="Opening\u2026";try{let N=(_.querySelector("#ws-pc-seller-name")?.value||"").trim(),G=Number(O?.value||0),Ee=Number(j?.value||0),Te=Number(U?.value||0),Se=(_.querySelector("#ws-pc-days-to-close")?.value??"").trim(),Ce=(_.querySelector("#ws-pc-inspection-days")?.value??"").trim(),Ie=(_.querySelector("#ws-pc-additional-terms")?.value??"").trim(),_e=(_.querySelector("#ws-pc-current-occupancy")?.value??"").trim(),Ae=(_.querySelector("#ws-pc-occupancy-at-close")?.value??"").trim(),z={};if(N!==s&&(z.sellerName=N),G!==l&&(z.purchasePrice=G),Ee!==d&&(z.emd=Ee),Te!==c&&(z.cashAtClose=Te),Se!==p&&(z.daysToClose=Se),Ce!==m&&(z.inspectionDays=Ce),Ie!==h&&(z.additionalTerms=Ie),_e!==v&&(z.currentOccupancy=_e),Ae!==k&&(z.occupancyAtClose=Ae),Object.keys(z).length>0){b.info("[prepare-contract] PA modal submit with overrides:",z),K.textContent="Saving\u2026";let Ut=M(),Vt=await Ht({tenantId:t,contactId:e,template:o,userId:Ut,overrides:z});b.info("[prepare-contract] re-stage with overrides ok:",Vt),K.textContent="Opening\u2026"}else b.info("[prepare-contract] PA modal submit \u2014 no edits, skipping re-stage");let q=i?`${i} - Purchase Agreement`:void 0;await Uo(t,e,o,q)}catch(N){K.disabled=!1,K.textContent="Open Contract",W&&(W.textContent=N.message||"Failed to open contract."),b.warn("[prepare-contract] PA confirm submit failed:",N)}}})},Qs=({tenantId:t,contactId:e,template:o,oppId:r,staged:n,userId:i,defaultClosingDate:s,defaultEMDDueDate:l})=>{document.getElementById(ne)?.remove(),tn();let d=n.debug||{},c=typeof d.propertyAddress=="string"?d.propertyAddress:"",p=typeof d.sellerDisplay=="string"?d.sellerDisplay:"",m=typeof d.assignmentPrice=="number"?d.assignmentPrice:0,h=typeof d.buyerEMD=="number"?d.buyerEMD:0,v=typeof d.assignmentFee=="number"?d.assignmentFee:0,k=typeof d.acqPurchasePrice=="number"?d.acqPurchasePrice:0,T=typeof d.closingOffice=="string"?d.closingOffice:"",w=typeof d.buyerCompanyName=="string"?d.buyerCompanyName:"",_=s,f=l,S="",C=document.createElement("div");C.id=ne,C.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,C.appendChild(x),document.body.appendChild(C);let U=x.querySelector("#ws-pc-assignment-price"),H=x.querySelector("#ws-pc-assignment-fee"),ke=!1;H?.addEventListener("input",()=>{ke=!0});let et=()=>{if(ke)return;let G=Number(U?.value||0);H&&(H.value=String(Math.max(0,G-k)))};U?.addEventListener("input",et);let K=()=>C.remove();C.addEventListener("click",G=>{G.target===C&&K()}),x.querySelector("#ws-pc-cancel")?.addEventListener("click",K);let W=x.querySelector("#ws-pc-submit"),N=x.querySelector("#ws-pc-error");W?.addEventListener("click",async()=>{if(!W)return;N&&(N.textContent="");let G=(x.querySelector("#ws-pc-seller-name")?.value||"").trim(),Ee=Number(U?.value||0),Te=Number(x.querySelector("#ws-pc-buyer-emd")?.value||0),Se=Number(H?.value||0),Ce=(x.querySelector("#ws-pc-closing-office")?.value||"").trim(),Ie=(x.querySelector("#ws-pc-buyer-llc")?.value||"").trim(),_e=(x.querySelector("#ws-pc-closing-date")?.value||"").trim(),Ae=(x.querySelector("#ws-pc-emd-due-date")?.value||"").trim(),z=(x.querySelector("#ws-pc-lender-source")?.value||"").trim();if(!_e){N&&(N.textContent="Closing Date is required.");return}W.disabled=!0,W.textContent="Opening\u2026";try{let q={};G!==p&&(q.sellerName=G),Ee!==m&&(q.assignmentPrice=Ee),Te!==h&&(q.buyerEMD=Te),Se!==v&&(q.assignmentFee=Se),Ce!==T&&(q.closingOffice=Ce),Ie!==w&&(q.buyerCompanyName=Ie);let Ut=_e!==_||Ae!==f||z!==S,Vt=Object.keys(q).length>0||Ut,Wt=n;Vt?(b.info("[prepare-contract] AA modal submit, re-staging with overrides:",q),W.textContent="Saving\u2026",Wt=await Ht({tenantId:t,contactId:e,template:o,oppId:r,closingDate:_e,buyerEarnestDueDate:Ae,lenderSource:z,userId:i,overrides:Object.keys(q).length>0?q:void 0}),b.info("[prepare-contract] AA re-stage ok:",Wt),W.textContent="Opening\u2026"):b.info("[prepare-contract] AA modal submit \u2014 no edits, skipping re-stage");let Ko=Wt?.debug?.propertyAddress||c,Ln=Ko?`${Ko} - Assignment Agreement`:void 0;await Uo(t,e,o,Ln),K()}catch(q){W.disabled=!1,W.textContent="Open Contract",N&&(N.textContent=q.message||"Failed to prepare contract."),b.warn("[prepare-contract] AA submit failed:",q)}})},Ht=async t=>{b.info(`[prepare-contract] staging \u2014 template "${t.template.name}" (${t.template.type})`);let e=await Os({tenantId:t.tenantId,contactId:t.contactId,templateId:t.template.id,templateType:t.template.type,oppId:t.oppId,closingDate:t.closingDate,buyerEarnestDueDate:t.buyerEarnestDueDate,lenderSource:t.lenderSource,userId:t.userId,overrides:t.overrides});if(b.info(`[prepare-contract] stage response: ok=${e.ok}`,e),!e.ok)throw new Error(e.error||"Contract.Stage returned not ok");return e},Uo=async(t,e,o,r)=>{b.info(`[prepare-contract] creating proposal from template ${o.id} for contact ${e}`);let n=await Ns(t,o.id,e);b.info(`[prepare-contract] proposal created: ${n}`),b.info(`[prepare-contract] attaching contact ${e} as primary signer${r?`, renaming to "${r}"`:""}`);let i=await Gs(t,n,e,r);i.ok?b.info(`[prepare-contract] contact attached to proposal ${n}`):(b.warn(`[prepare-contract] attach failed (status=${i.status??"n/a"}): ${i.body?.slice(0,300)??"no body"}`),B(`Proposal created but couldn't auto-attach contact (HTTP ${i.status??"?"}). You'll need to pick the recipient manually.`,"error",8e3),console.warn("[ws-crm-customizer] attach-recipient GHL response:",i.body));let s=Vs(o.name);s&&(b.info(`[prepare-contract] stashing post-redirect instructions for "${o.name}"`),Ws(n,s.title,s.bodyHtml)),zs(t,n)},el=async t=>{let e=Ls(t);if(!e){B("Could not resolve contact context.","error");return}document.getElementById(Yr)?.remove();let o=document.createElement("div");o.id=Yr,o.style.cssText=`
    position: absolute;
    top: calc(100% + 6px);
    right: 0;
    z-index: 999998;
  ...
  `,o.innerHTML=`
    ...
    ">Loading templates\u2026</div>`,t.appendChild(o);let r=i=>{!o.contains(i.target)&&i.target!==t&&(o.remove(),document.removeEventListener("click",r,!0))};window.setTimeout(()=>document.addEventListener("click",r,!0),0);let n;try{n=await Fs(e.tenantId)}catch(i){o.innerHTML=`
    ...
    `;let l=i.type==="PA"?a.emerald:i.type==="AA"?a.blue:a.coolGray;s.innerHTML=`
      ...
      </div>`,s.addEventListener("mouseenter",()=>{s.style.background=a.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async d=>{d.stopPropagation(),d.preventDefault(),b.info(`[prepare-contract] template picked: ${i.name} (type=${i.type})`),o.remove(),document.removeEventListener("click",r,!0);let c=M();try{if(i.type==="AA"){let p;if(e.oppId)p=e.oppId,b.info(`[prepare-contract] AA from MT opp page \u2014 using opp ${p} directly`);else{let _;try{_=await Ps(e.tenantId,e.contactId)}catch(f){B(`Failed to load buyer's MT opps: ${f.message}`,"error",6e3),b.warn("[prepare-contract] AA fetch opps failed:",f);return}if(_.length===0){B("No open MT deal found for this buyer. Create an MT opp first.","error",6e3);return}if(_.length===1)p=_[0].id,b.info(`[prepare-contract] auto-selected MT opp ${p} (only one)`);else{b.info(`[prepare-contract] showing MT opp picker (${_.length} opps)`);let f=await en(_,{perspective:"buyer",agreementLabel:"Assignment Agreement"});if(!f){b.info("[prepare-contract] MT opp picker cancelled");return}p=f,b.info(`[prepare-contract] picked MT opp ${p}`)}}let m=_=>{let f=_.getFullYear(),S=String(_.getMonth()+1).padStart(2,"0"),C=String(_.getDate()).padStart(2,"0");return`${f}-${S}-${C}`},h=_=>{let f=new Date;return f.setDate(f.getDate()+_),m(f)},v=h(45),k=h(7);B("Reading contract data\u2026","info",2500);let T;try{T=await Ht({tenantId:e.tenantId,contactId:e.contactId,template:i,oppId:p,closingDate:v,buyerEarnestDueDate:k,userId:c})}catch(_){B(`Failed to prepare contract: ${_.message}`,"error",6e3),b.warn("[prepare-contract] AA stage failed:",_);return}let w=e.contactId||T.contact_id||"";if(!w){B("Could not resolve buyer contact for this opp.","error",6e3);return}Qs({tenantId:e.tenantId,contactId:w,template:i,oppId:p,staged:T,userId:c,defaultClosingDate:v,defaultEMDDueDate:k})}else if(i.type==="PA"){if(!e.contactId){B("Purchase Agreements need to be started from the seller's contact page.","error",6e3);return}let p;try{p=await Hs(e.tenantId,e.contactId)}catch(v){B(`Failed to load ACQ opps: ${v.message}`,"error",6e3),b.warn("[prepare-contract] PA fetch opps failed:",v);return}if(p.length===0){B("No open ACQ deal found for this contact. Create an ACQ opp first.","error",6e3);return}let m;if(p.length===1)m=p[0].id,b.info(`[prepare-contract] auto-selected ACQ opp ${m} (only one)`);else{b.info(`[prepare-contract] showing ACQ opp picker (${p.length} opps)`);let v=await en(p,{perspective:"seller",agreementLabel:"Purchase Agreement"});if(!v){b.info("[prepare-contract] ACQ opp picker cancelled");return}m=v,b.info(`[prepare-contract] picked ACQ opp ${m}`)}B("Reading contract data\u2026","info",2500);let h;try{h=await Ht({tenantId:e.tenantId,contactId:e.contactId,template:i,oppId:m,userId:c})}catch(v){B(`Failed to prepare contract: ${v.message}`,"error",6e3),b.warn("[prepare-contract] PA stage failed:",v);return}Zs({tenantId:e.tenantId,contactId:e.contactId,template:i,staged:h})}else{if(!e.contactId){B("This template needs to be started from a contact page.","error",6e3);return}B(`Opening ${i.name}\u2026`,"info",2500);try{await Uo(e.tenantId,e.contactId,i)}catch(p){B(`Failed to open contract: ${p.message}`,"error",6e3),b.warn("[prepare-contract] passthrough failed:",p)}}}catch(p){b.warn("[prepare-contract] picker click handler failed:",p),B(`Unexpected error: ${p.message}`,"error",6e3)}}),o.appendChild(s)}},an=()=>{let t=document.createElement("button");return t.id=Ro,t.type="button",t.style.cssText=`${$} position: relative;`,t.innerHTML=`
    <span>Prepare Contract</span>
    <span style="font-size: 10px; color: ${a.ash};">\u25BE</span>
  `,t.addEventListener("mouseenter",()=>{t.style.borderColor=a.emerald}),t.addEventListener("mouseleave",()=>{t.style.borderColor=a.steel}),t.addEventListener("click",e=>{e.stopPropagation(),el(t)}),t},tl="ws-crm-contact-actions-bar",ol=async()=>{let t=rn();if(!t){document.getElementById(Ro)?.remove();return}let e=null;try{e=await A(`#${tl}`,{timeoutMs:3e3})}catch{b.debug("[prepare-contract] actions bar not found within timeout");return}if(!e)return;let o=document.getElementById(Ro);if(o){if(o.dataset.contactId===t.contactId)return;o.remove()}let r=an();r.dataset.contactId=t.contactId,r.dataset.tenantId=t.tenantId,e.appendChild(r),b.debug(`[prepare-contract] mounted for contact ${t.contactId}`)},ve="ws-crm-prepare-contract-button-opp",Ke=`${ve}-wrap`,rl=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,nl=/\/v2\/location\/([A-Za-z0-9]+)/,al=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],zo=()=>{let t=window.location.pathname,e=t.match(rl);if(!e)return null;let o=t.match(nl);return o?{oppId:e[1],tenantId:o[1]}:null},il=async()=>{for(let t of al)try{let e=await A(t,{timeoutMs:2e3});if(e)return e}catch{}return null},Dt=!1,Ye=null,qo=null,No=null,Bo,Xe=()=>{Ye?.disconnect(),Ye=null,qo=null},sl=t=>{if(Ye&&qo===t)return;Xe();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(qo=t,Ye=new MutationObserver(()=>{let o=zo();if(!o||o.oppId!==t){Xe();return}let r=P();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(ve)?.remove(),document.getElementById(Ke)?.remove(),Xe();return}document.getElementById(ve)||sn()}),Ye.observe(e,{childList:!0,subtree:!0}))},ll=()=>{No||(No=new MutationObserver(()=>{Bo===void 0&&(Bo=window.setTimeout(()=>{Bo=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(ve);t&&!e&&!Dt&&sn()},300))}),No.observe(document.body,{childList:!0,subtree:!0}))},sn=async()=>{if(!Dt){Dt=!0;try{let t=zo();if(!t){document.getElementById(ve)?.remove(),document.getElementById(Ke)?.remove(),Xe();return}let e=document.getElementById(ve);if(e&&e.dataset.oppId===t.oppId){let l=P();l!==null&&l!=="Marketing Tracker"&&(e.remove(),document.getElementById(Ke)?.remove(),Xe());return}e&&(e.remove(),document.getElementById(Ke)?.remove());let o=await il();if(!o){b.debug("[prepare-contract] opp mount target not found");return}let r=await te();if(r!=="Marketing Tracker"){b.debug(`[prepare-contract] opp mount gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=zo();if(!n||n.oppId!==t.oppId)return;let i=an();i.id=ve,i.dataset.oppId=t.oppId,i.dataset.tenantId=t.tenantId;let s=document.createElement("span");s.id=Ke,s.style.cssText=`
      display: inline-flex;
      align-items: center;
      flex-shrink: 0;
      vertical-align: middle;
      margin-left: 12px;
    `,o.classList.contains("description")&&(o.style.display="flex",o.style.alignItems="center",o.style.gap="16px",o.style.flexWrap="wrap"),s.appendChild(i),o.appendChild(s),b.debug(`[prepare-contract] mounted on MT opp ${t.oppId} (contactId deferred to click)`),sl(t.oppId)}finally{Dt=!1}}},ln=()=>{Js(),ol()};As();function V(t){return String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function re(t){return V(t)}var Gt="reos-theme-stylesheet",Sn="reos-theme-fonts",dl=()=>{let t=document.getElementById(Sn);t&&t.remove()},cl=`
/* ============================================
...
Click to dismiss for this session`,t.addEventListener("mouseenter",()=>{t.style.transform="scale(1.05)"}),t.addEventListener("mouseleave",()=>{t.style.transform="scale(1)"}),t.addEventListener("click",()=>{t.style.opacity="0",setTimeout(()=>t.remove(),200)}),document.body.appendChild(t)};console.log("[ws-crm-customizer] bundle-loaded");var R=(t,e)=>{try{b.info(`[main] calling ${t}`),e(),b.info(`[main] ${t} OK`)}catch(o){b.error(`[main:FAIL] ${t} threw:`,o)}},An=()=>{b.info(`Loaded v${nt} (built ${at})`),R("mountVersionBadge (initial)",qt),R("mountFavicon",Tr),R("mountThemeStylesheet",Cn),R("initPageRouter",Zo),b.info("[main] registering onPageChange subscriber"),Jo(t=>{b.info(`[main] onPageChange callback fired for page=${t}`),R("mountVersionBadge",qt),R("mountClaimReleaseToggle",mr),R("mountContactActionsBar",kr),R("mountActiveFollowUpButton",nr),R("mountMtOfferMadeButton",Cr),R("mountNotInterestedButton",Ir),R("mountReducePriceButton",Lr),R("mountSendToClosingOfficeButton",Hr),R("mountClosingAgentPickerButton",qr),R("mountPrepareContractButton",ln),R("mountManageTeam",Kr),R("mountBuyBoxWidget",cr)}),b.info("[main] onPageChange registered; main() complete")};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",An):An();})();
