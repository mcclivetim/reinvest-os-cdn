...
`;var I="ws-crm-claim-release-pill",tt=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},It=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?c.emerald:c.amber;return e.style.cssText=Z(r),e.innerHTML=`
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,Gt.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
...
/* 13. SCROLLBAR \u2014 match the theme.
   ...
   rgb(229, 231, 235) }) beat our `*` selector via specificity (0,0,1,1
   vs 0,0,1,0) and painted scrollbars Tailwind gray-200 inside that
   subtree. Tim noticed the Team Inbox scrollbar coming through as
   gray-200 while the Contact Details panel scrollbar correctly
   showed steel \u2014 that's the specificity war. !important wins it. */
