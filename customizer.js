...
`;var I="ws-crm-claim-release-pill",re=o=>{let e=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},De=({contactId:o,mode:e})=>{let t=document.createElement("button");t.id=I,t.type="button",t.dataset.contactId=o,t.dataset.mode=e;let r=e==="claim"?"emerald":"amber",a=e==="claim"?i.emerald:i.amber;return t.style.cssText=te(r),t.innerHTML=`
  ...
  `,$e.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
    ...
    `,s.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=i.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{b.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let l=t.firstElementChild,n=t.children[1],d=t.children[2];l&&(l.style.background=j[a.color]),n&&(n.textContent=a.label),r.style.display="none";let p=E()?.locationId,h=Be();if(!p){b.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=i.amber);return}if(a.color==="green"&&!h){b.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=i.amber);return}let g=d?.textContent??"\u25BE",x=async v=>{d&&(d.textContent="\u2026"),t.style.opacity="0.75";try{await Oe({contactId:o,locationId:p,status:a.label,userId:h,deadReason:v}),d&&(d.textContent="\u2713"),setTimeout(()=>{d&&(d.textContent=g),t.style.opacity="1"},900)}catch(m){b.warn("Call Status webhook failed:",m),d&&(d.textContent="\u2717"),n&&(n.style.color=i.amber),t.style.opacity="1",setTimeout(()=>{d&&(d.textContent=g),n&&(n.style.color=i.bone)},1800)}};if(a.value==="dead-deal"){let v=n?.textContent??"";qe(m=>{x(m)},()=>{n&&(n.textContent=v)});return}if(a.value==="appointment-booked"){let v=n?.textContent??"";try{(await Pe({contactId:o,tenantId:p})).ok?await x():je(()=>{x()},()=>{n&&(n.textContent=v)})}catch(m){b.warn("Appointment sanity check failed; firing anyway:",m),await x()}return}await x()}),r.appendChild(s)}),t.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{e.contains(a.target)||(r.style.display="none")}),e.appendChild(t),e.appendChild(r),e},We=o=>{document.getElementById(ne)?.remove();let e=document.createElement("div");e.id=ne,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
...
/* 18a-smartlist-tabs (v0.8.37). The Contacts smart-list page has
   a tabs bar (.d-flex.bar) with .d-flex.view.cursor-pointer
   children \u2014 "All", "Attack List", "Drip Replies", etc. plus
   "Add smart list". The active tab carries an extra .active
   class.

   Apply the canonical icon-rail pattern (matches top tabs,
   conv-header, conv-list tabs, contact-details segmented control):
   - Default: ash text, transparent bg.
   - Hover: emerald text, transparent bg.
   - Active: emerald text + emerald underline + transparent bg
     (no box). */
...
/* Hover: inactive tab \u2192 emerald text + emerald underline tease */
...
/* Active: emerald text + emerald underline, no bg. */
