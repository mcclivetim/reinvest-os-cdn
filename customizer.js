...
`,P=`
  ...
  flex-shrink: 0;
`;var nt=/^[A-Za-z0-9]{15,30}$/,R=o=>typeof o=="string"&&nt.test(o),at=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},st=()=>{let o=window.WS_CRM_USER_ID;return R(o)?o:null},it=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(R(t))return t;return null},lt=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=at(e);if(!t)return null;let r=t.uid;if(R(r))return r;let n=t.claims;if(n&&R(n.user_id))return n.user_id}catch{}return null},dt=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(R(n))return n}catch{}}}catch{}return null},F=()=>st()||it()||lt()||dt()||null;var we="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",ct=`${we}/active-follow-up-get`,bt=`${we}/active-follow-up-set`,O="ws-crm-active-follow-up-button",ce={amber:{label:"Add to Active Follow Up",bg:l.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:l.amber,clickable:!0,dotColor:l.amber},blue:{label:"In Active Follow Up",bg:l.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:l.blue,clickable:!1,dotColor:l.blue},gray:{label:"Not in Follow Up",bg:l.slate,border:l.steel,color:l.coolGray,clickable:!1,dotColor:l.coolGray},loading:{label:"Active Follow Up\u2026",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.ash},error:{label:"Follow Up status unavailable",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.crimson}},ht=()=>{let o=x();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},pt=async o=>{let e=await fetch(ct,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},mt=async(o,e)=>{let t=await fetch(bt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},D=(o,e)=>{let t=ce[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},ye=o=>{let e=document.createElement("button");return e.type="button",e.id=O,e.dataset.contactId=o,e.style.cssText=`
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 6px 12px;
  ...
  `,D(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=ht();if(!(!r||r.contactId!==o)){D(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await mt(r,F()),s=n&&n.state&&n.state in ce?n.state:"blue";D(e,s),h.info(`Active Follow Up set for ${o} \u2192 ${s}`)}catch(n){h.error("Active Follow Up Set webhook failed",n),D(e,"error"),setTimeout(()=>void Y(e,r),4e3)}}}),e},Y=async(o,e)=>{D(o,"loading");try{let t=await pt(e),r=t&&t.state&&t.state in ce?t.state:"gray",n=document.getElementById(O);if(!n||n.dataset.contactId!==e.contactId)return;D(n,r)}catch(t){h.warn("Active Follow Up Get webhook failed",t),D(o,"error")}},_t=[".central-panel","[class*='central-panel']"],ge="ws-crm-contact-actions-bar",vt=async()=>{for(let o of _t)try{let e=await S(o,{timeoutMs:3e3});if(e)return e}catch{}try{return(await S("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},ut=(o,e)=>{let t=e.querySelector(`#${ge}`);if(t)return t.appendChild(o),{container:t,placement:"actions-bar"};let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
      ...
      flex-shrink: 0;
    `;let s=e.querySelector(`#${ge}`);s&&s.parentElement===e?e.insertBefore(n,s.nextSibling):e.insertBefore(n,e.firstChild)}return n.appendChild(o),{container:n,placement:"fallback"}},yt=()=>{let o=document.querySelector("[data-testid='CENTRALPANEL_NAME']");if(!o)return null;let e=o;for(let t=0;t<8&&!(!e||!e.parentElement);t++){let r=e.parentElement,n=(r.className||"").toString();if(n.includes("justify-between")&&n.includes("border-b")){let s=r.children[1];return s instanceof HTMLElement?s:r}e=r}return null},gt=(o,e)=>{o.style.marginRight="4px",e.insertBefore(o,e.firstChild)},ft=async o=>{if(o==="url"){let t=await vt();return t?{kind:"contact-detail",target:t}:null}let e=yt();return e?{kind:"conversations",target:e}:null},fe=(o,e)=>e.kind==="contact-detail"?ut(o,e.target).placement:(gt(o,e.target),"conversations-header"),wt=async()=>{let o=x();if(!o||!o.locationId){document.getElementById(O)?.remove(),document.getElementById("ws-crm-afu-wrap")?.remove();return}let e={contactId:o.contactId,tenantId:o.locationId},t=document.getElementById(O);if(t&&t.dataset.contactId===e.contactId){Y(t,e);return}t&&t.remove();let r=await ft(o.source);if(!r){h.warn(`Active Follow Up: no mount target on ${o.source}-source page`);return}let n=x();if(!n||n.contactId!==e.contactId)return;let s=ye(e.contactId),i=fe(s,r);h.debug(`Active Follow Up mounted (${r.kind}/${i}) for ${e.contactId}`),Y(s,e);let a=new MutationObserver(()=>{let d=document.getElementById(O);if(!(x()?.contactId===e.contactId)){a.disconnect();return}if(!d){let p=ye(e.contactId);fe(p,r),Y(p,e),h.debug(`Active Follow Up re-mounted after wipe for ${e.contactId}`)}});a.observe(r.target,{childList:!0})},xe=()=>{wt()};var I="ws-crm-claim-release-pill",xt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-get",kt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-set",Et=async o=>{let e=await fetch(xt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,contact_id:o.contactId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Get returned ${e.status} ${e.statusText}`);return await e.json()},Tt=async o=>{let e=await fetch(kt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,contact_id:o.contactId,triggered_by_user_id:o.userId,action:o.action}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Set returned ${e.status} ${e.statusText}`);return await e.json()},Te=o=>{let e=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},H=(o,e)=>{let t=e==="claim"?"emerald":"amber",r=e==="claim"?l.emerald:l.amber;o.dataset.mode=e,o.style.cssText=K(t),o.title=e==="claim"?"Take ownership of this lead":"Return this lead to the pool",o.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `},St=({contactId:o,mode:e})=>{let t=document.createElement("button");return t.id=I,t.type="button",t.dataset.contactId=o,H(t,e),t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-1px)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",async()=>{let r=t.dataset.mode??"claim",n=x();if(!n||!n.locationId||n.contactId!==o)return;let s=F();if(!s){h.warn("Claim/Release click: no user ID detected (login session not exposing it)");let a=t.querySelector(".ws-cr-label");a&&(a.textContent="Sign-in needed"),setTimeout(()=>H(t,r),2500);return}h.info(`Claim/Release click \u2014 contact=${o} mode=${r} user=${s}`);let i=r==="claim"?"release":"claim";H(t,i),t.disabled=!0,t.style.opacity="0.8";try{let a=await Tt({contactId:o,locationId:n.locationId,userId:s,action:r}),d=a?.mode==="claim"||a?.mode==="release"?a.mode:i;H(t,d),h.info(`Claim/Release done \u2192 new mode: ${d}`)}catch(a){h.error("Claim/Release Set webhook failed",a),H(t,r);let d=t.querySelector(".ws-cr-label");if(d){let b=d.textContent??"";d.textContent="Try again",setTimeout(()=>{d.textContent==="Try again"&&(d.textContent=b)},2500)}}finally{t.disabled=!1,t.style.opacity="1"}}),t},ke=async(o,e)=>{try{let t=await Et({contactId:e.contactId,locationId:e.locationId,userId:F()}),r=document.getElementById(I);if(!r||r.dataset.contactId!==e.contactId)return;let n=t?.mode==="release"?"release":"claim";H(r,n)}catch(t){h.warn("Claim/Release Get failed; pill stays in default 'claim' mode",t)}},Ct=async(o,e=1e4)=>{let t=Date.now();for(;Date.now()-t<e;){let r=Te(o);if(r)return r;await new Promise(n=>setTimeout(n,150))}return null},Ee=(o,e)=>{let t=St({contactId:e,mode:"claim"}),r=document.createElement("span");r.id=`${I}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(t),o.appendChild(r)},At=async()=>{let o=x();if(!o){document.getElementById(I)?.remove();return}let e=document.getElementById(I);if(!(e&&e.dataset.contactId===o.contactId)){e&&e.remove();try{let t=await S("#record-details-lhs",{timeoutMs:1e4}),r=await Ct(t),n=x();if(!n||n.contactId!==o.contactId)return;if(!r){h.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}Ee(r,o.contactId),h.debug(`Mounted Claim/Release pill next to name for ${o.contactId}`);let s=document.getElementById(I);s&&o.locationId&&ke(s,{contactId:o.contactId,locationId:o.locationId});let i=null;i=new MutationObserver(async()=>{if(!(x()?.contactId===o.contactId)){i?.disconnect();return}if(document.getElementById(I))return;let d=Te(t);if(d){Ee(d,o.contactId);let b=document.getElementById(I);b&&o.locationId&&ke(b,{contactId:o.contactId,locationId:o.locationId}),h.debug("Re-mounted Claim/Release pill after React wipe")}}),i.observe(t,{childList:!0,subtree:!0})}catch(t){h.warn("Could not mount Claim/Release pill:",t)}}},Se=()=>{At()};var Ft="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set";var It="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Lt=async({contactId:o,locationId:e,status:t,userId:r,deadReason:n})=>{let s={call_status:t,triggered_by_user_id:r??""};n&&(s.dead_reason=n);let a=await fetch(Ft,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:o,location:{id:e},customData:s}),credentials:"omit"});if(!a.ok)throw new Error(`Webhook returned ${a.status} ${a.statusText}`)},Dt=async({contactId:o,tenantId:e})=>{let t=await fetch(It,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,contact_id:o}),credentials:"omit"});if(!t.ok)throw new Error(`Sanity check returned ${t.status} ${t.statusText}`);return await t.json()};var X="ws-crm-contact-actions-bar",Mt="ws-crm-call-status-dropdown",Ce="ws-crm-offer-made-modal",Ht=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],Nt=[".central-panel","[class*='central-panel']"],Pt=async()=>{for(let o of Nt)try{let e=await S(o,{timeoutMs:3e3});if(e)return e}catch{}try{return(await S("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},Rt=o=>{let e=document.createElement("div");e.id=Mt,e.style.cssText="position: relative; flex-shrink: 0;";let t=document.createElement("button");t.type="button",t.style.cssText=P,t.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,Ht.forEach(n=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
  ...
  `,t.appendChild(r),document.body.appendChild(t);let n=()=>t.remove(),s=()=>{n(),e()};t.addEventListener("click",a=>{a.target===t&&s()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",s),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),o()});let i=a=>{a.key==="Escape"&&(document.removeEventListener("keydown",i),s())};document.addEventListener("keydown",i)},Ie=o=>{let e=document.createElement("div");e.id=X,e.dataset.contactId=o,e.style.cssText=`
    ...
    border-bottom: 1px solid ${l.steel};
    flex-shrink: 0;
  `;let t=document.createElement("button");return t.type="button",t.style.cssText=N,t.textContent="Offer Made",t.addEventListener("click",()=>Ot(o)),e.appendChild(t),e.appendChild(Rt(o)),e},Vt=async()=>{let o=x();if(!o){document.getElementById(X)?.remove();return}let e=document.getElementById(X);if(e&&e.dataset.contactId===o.contactId)return;e&&e.remove();let t=await Pt();if(!t){h.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let r=x();if(!r||r.contactId!==o.contactId)return;let n=Ie(o.contactId);n.dataset.mountTarget=t.className||"central-panel",t.insertBefore(n,t.firstChild),h.debug(`Mounted contact actions bar for ${o.contactId}`);let s=null;s=new MutationObserver(()=>{let i=document.getElementById(X);if(!(x()?.contactId===o.contactId)){s?.disconnect();return}if(!i){let d=Ie(o.contactId);d.dataset.mountTarget=n.dataset.mountTarget??"",t.insertBefore(d,t.firstChild),h.debug(`Re-mounted actions bar after React wipe for ${o.contactId}`)}}),s.observe(t,{childList:!0})},Le=()=>{Vt()};var J="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",$t=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),B="data-ws-crm-favicon",be=o=>o.hasAttribute(B),De=o=>{if(o.tagName!=="LINK")return!1;let e=(o.getAttribute("rel")??"").toLowerCase();return $t.has(e)},Z=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{De(r)&&!be(r)&&r.remove()});let e=document.head.querySelector(`link[${B}="1"]`);e?e.href!==J&&(e.href=J):(e=document.createElement("link"),e.setAttribute(B,"1"),e.rel="icon",e.type="image/png",e.href=J,document.head.appendChild(e),h.debug("Favicon installed"));let t=document.head.querySelector(`link[${B}="apple"]`);t||(t=document.createElement("link"),t.setAttribute(B,"apple"),t.rel="apple-touch-icon",t.href=J,document.head.appendChild(t))},Me=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)Z();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),Z())},50)}new MutationObserver(r=>{let n=!1;for(let s of r)s.addedNodes.forEach(i=>{i instanceof HTMLLinkElement&&De(i)&&!be(i)&&(n=!0)}),s.removedNodes.forEach(i=>{i instanceof HTMLLinkElement&&be(i)&&(n=!0)});n&&Z()}).observe(document.head,{childList:!0,subtree:!1});let e=0,t=window.setInterval(()=>{e+=1,Z(),e>=5&&window.clearInterval(t)},1e3)};var Ut="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/mt/not-interested",G="ws-crm-not-interested-button",qt=/\/opportunities\/detail\/([A-Za-z0-9]+)/,zt=/\/v2\/location\/([A-Za-z0-9]+)/,Q=()=>{let o=window.location.pathname,e=o.match(qt);if(!e)return null;let t=o.match(zt);return{oppId:e[1],locationId:t?t[1]:null}},Wt=async o=>{let e=await fetch(Ut,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,mt_opp_id:o.oppId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Not Interested webhook returned ${e.status} ${e.statusText}`);return await e.json()},He=o=>{let e=document.createElement("button");return e.id=G,e.type="button",e.dataset.oppId=o,e.style.cssText=K("neutral"),e.style.fontFamily=v.sans,e.style.fontSize="12px",e.title="Close this Marketing Tracker opp as Not Interested for THIS property only",e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let t=Q();if(!t||!t.locationId||t.oppId!==o)return;let r=F(),n=e.querySelector(".ws-ni-label");e.disabled=!0,e.style.opacity="0.7",n&&(n.textContent="Closing\u2026");try{let s=await Wt({oppId:t.oppId,locationId:t.locationId,userId:r});if(s?.ok)n&&(n.textContent="Marked Not Interested"),e.style.background=l.crimsonGlow,e.style.color=l.crimson,h.info(`Not Interested set for opp ${o} \u2192 ${s.new_stage}`);else throw new Error("Webhook returned ok=false")}catch(s){h.error("Not Interested webhook failed",s),n&&(n.textContent="Try again"),e.disabled=!1,e.style.opacity="1",setTimeout(()=>{n&&n.textContent==="Try again"&&(n.textContent="Mark Not Interested")},3e3)}}),e},jt=["[data-testid='OPPORTUNITY_HEADER']","[class*='opportunity-header']","[class*='opp-header']","header.flex.items-center"],Kt=async()=>{for(let o of jt)try{let e=await S(o,{timeoutMs:2e3});if(e)return e}catch{}return null},Yt=async()=>{let o=Q();if(!o||!o.locationId){document.getElementById(G)?.remove();return}let e=document.getElementById(G);if(e&&e.dataset.oppId===o.oppId)return;e&&e.remove();let t=await Kt();if(!t){h.warn("Not Interested: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=Q();if(!r||r.oppId!==o.oppId)return;let n=He(o.oppId),s=document.createElement("span");s.id=`${G}-wrap`,s.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,s.appendChild(n),t.appendChild(s),h.debug(`Mounted Not Interested button on opp ${o.oppId}`);let i=new MutationObserver(()=>{let a=document.getElementById(G);if(!(Q()?.oppId===o.oppId)){i.disconnect();return}if(!a){let b=He(o.oppId);s.appendChild(b),h.debug(`Re-mounted Not Interested button after wipe for opp ${o.oppId}`)}});i.observe(t,{childList:!0})},Ne=()=>{Yt()};var ne="reos-theme-stylesheet",Ye="reos-theme-fonts",Xt=()=>{let o=document.getElementById(Ye);o&&o.remove()},Jt=`
/* ============================================
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var Qe=()=>{h.info(`Loaded v${W} (built ${j})`),Me(),Xe(),ue(),ve(o=>{h.debug("Page handler firing for:",o),Ze(),Se(),Le(),xe(),Ne()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Qe):Qe();})();
