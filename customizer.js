...
`;var I="ws-crm-claim-release-pill",rt=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Dt=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?i.emerald:i.amber;return e.style.cssText=et(r),e.innerHTML=`
  ...
  `,$t.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
    ...
    `,s.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=i.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{h.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let l=e.firstElementChild,n=e.children[1],c=e.children[2];l&&(l.style.background=j[a.color]),n&&(n.textContent=a.label),r.style.display="none";let b=_()?.locationId,p=Bt();if(!b){h.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=i.amber);return}if(a.color==="green"&&!p){h.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=i.amber);return}let g=c?.textContent??"\u25BE",x=async v=>{c&&(c.textContent="\u2026"),e.style.opacity="0.75";try{await Ot({contactId:o,locationId:b,status:a.label,userId:p,deadReason:v}),c&&(c.textContent="\u2713"),setTimeout(()=>{c&&(c.textContent=g),e.style.opacity="1"},900)}catch(m){h.warn("Call Status webhook failed:",m),c&&(c.textContent="\u2717"),n&&(n.style.color=i.amber),e.style.opacity="1",setTimeout(()=>{c&&(c.textContent=g),n&&(n.style.color=i.bone)},1800)}};if(a.value==="dead-deal"){let v=n?.textContent??"";qt(m=>{x(m)},()=>{n&&(n.textContent=v)});return}if(a.value==="appointment-booked"){let v=n?.textContent??"";try{(await Pt({contactId:o,tenantId:b})).ok?await x():jt(()=>{x()},()=>{n&&(n.textContent=v)})}catch(m){h.warn("Appointment sanity check failed; firing anyway:",m),await x()}return}await x()}),r.appendChild(s)}),e.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{t.contains(a.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},Wt=o=>{document.getElementById(nt)?.remove();let t=document.createElement("div");t.id=nt,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
