...
`,B=`
  ...
  flex-shrink: 0;
`;var St=/^[A-Za-z0-9]{15,30}$/,G=o=>typeof o=="string"&&St.test(o),Ct=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},At=()=>{let o=window.WS_CRM_USER_ID;return G(o)?o:null},It=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(G(t))return t;return null},Lt=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=Ct(e);if(!t)return null;let r=t.uid;if(G(r))return r;let n=t.claims;if(n&&G(n.user_id))return n.user_id}catch{}return null},Ft=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(G(n))return n}catch{}}}catch{}return null},A=()=>At()||It()||Lt()||Ft()||null;var Fe="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",Dt=`${Fe}/active-follow-up-get`,Mt=`${Fe}/active-follow-up-set`,H="ws-crm-active-follow-up-button",fe={amber:{label:"Add to Active Follow Up",bg:l.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:l.amber,clickable:!0,dotColor:l.amber},blue:{label:"In Active Follow Up",bg:l.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:l.blue,clickable:!1,dotColor:l.blue},gray:{label:"Not in Follow Up",bg:l.slate,border:l.steel,color:l.coolGray,clickable:!1,dotColor:l.coolGray},loading:{label:"Active Follow Up\u2026",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.ash},error:{label:"Follow Up status unavailable",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.crimson}},Ht=()=>{let o=f();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},Nt=async o=>{let e=await fetch(Dt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},Pt=async(o,e)=>{let t=await fetch(Mt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},N=(o,e)=>{let t=fe[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},Ae=o=>{let e=document.createElement("button");return e.type="button",e.id=H,e.dataset.contactId=o,e.style.cssText=`
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 6px 12px;
  ...
  `,N(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=Ht();if(!(!r||r.contactId!==o)){N(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await Pt(r,A()),a=n&&n.state&&n.state in fe?n.state:"blue";N(e,a),c.info(`Active Follow Up set for ${o} \u2192 ${a}`)}catch(n){c.error("Active Follow Up Set webhook failed",n),N(e,"error"),setTimeout(()=>void te(e,r),4e3)}}}),e},te=async(o,e)=>{N(o,"loading");try{let t=await Nt(e),r=t&&t.state&&t.state in fe?t.state:"gray",n=document.getElementById(H);if(!n||n.dataset.contactId!==e.contactId)return;N(n,r)}catch(t){c.warn("Active Follow Up Get webhook failed",t),N(o,"error")}},Rt=[".central-panel","[class*='central-panel']"],ge="ws-crm-contact-actions-bar",Ot=async()=>{for(let o of Rt)try{let e=await C(o,{timeoutMs:3e3});if(e)return e}catch{}try{return(await C("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},Bt=async(o,e)=>{let t=e.querySelector(`#${ge}`);if(!t)try{t=await C(`#${ge}`,{timeoutMs:3e3,root:e})}catch{t=null}if(t){let a=e.querySelector("#ws-crm-afu-wrap");return a&&a.remove(),t.appendChild(o),{container:t,placement:"actions-bar"}}let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
      ...
      flex-shrink: 0;
    `;let a=e.querySelector(`#${ge}`);a&&a.parentElement===e?e.insertBefore(n,a.nextSibling):e.insertBefore(n,e.firstChild)}return n.appendChild(o),{container:n,placement:"fallback"}},Gt=()=>{let o=document.querySelector("[data-testid='CENTRALPANEL_NAME']");if(!o)return null;let e=o;for(let t=0;t<8&&!(!e||!e.parentElement);t++){let r=e.parentElement,n=(r.className||"").toString();if(n.includes("justify-between")&&n.includes("border-b")){let a=r.children[1];return a instanceof HTMLElement?a:r}e=r}return null},$t=(o,e)=>{o.style.marginRight="4px",e.insertBefore(o,e.firstChild)},Vt=async o=>{if(o==="url"){let t=await Ot();return t?{kind:"contact-detail",target:t}:null}let e=Gt();return e?{kind:"conversations",target:e}:null},Ie=async(o,e)=>{if(e.kind==="contact-detail"){let{placement:t}=await Bt(o,e.target);return t}return $t(o,e.target),"conversations-header"},Ut=async()=>{let o=f();if(!o||!o.locationId){document.getElementById(H)?.remove(),document.getElementById("ws-crm-afu-wrap")?.remove();return}let e={contactId:o.contactId,tenantId:o.locationId},t=document.getElementById(H);if(t&&t.dataset.contactId===e.contactId){te(t,e);return}t&&t.remove();let r=await Vt(o.source);if(!r){c.warn(`Active Follow Up: no mount target on ${o.source}-source page`);return}let n=f();if(!n||n.contactId!==e.contactId)return;let a=Ae(e.contactId),i=await Ie(a,r);c.debug(`Active Follow Up mounted (${r.kind}/${i}) for ${e.contactId}`),Le();let s=document.getElementById(H)??a;te(s,e);let d=new MutationObserver(()=>{let b=document.getElementById(H);if(!(f()?.contactId===e.contactId)){d.disconnect();return}if(!b){let m=Ae(e.contactId);Ie(m,r).then(()=>{Le(),te(m,e)}),c.debug(`Active Follow Up re-mounted after wipe for ${e.contactId}`)}});d.observe(r.target,{childList:!0})},$=null,De=()=>{let o=f()?.contactId??null;if($&&$.contactId===o)return;let e=Ut().finally(()=>{$?.promise===e&&($=null)});$={contactId:o,promise:e}},Le=()=>{let o=document.querySelectorAll(`#${H}`);if(!(o.length<=1)){c.debug(`Active Follow Up: found ${o.length} duplicate buttons, keeping first, removing ${o.length-1}`);for(let e=1;e<o.length;e++)o[e].remove()}};var D="ws-crm-claim-release-pill",qt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-get",zt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-set",Wt=async o=>{let e=await fetch(qt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,contact_id:o.contactId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Get returned ${e.status} ${e.statusText}`);return await e.json()},jt=async o=>{let e=await fetch(zt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,contact_id:o.contactId,triggered_by_user_id:o.userId,action:o.action}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Set returned ${e.status} ${e.statusText}`);return await e.json()},Ne=o=>{let e=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},R=(o,e)=>{let t=e==="claim"?"emerald":"amber",r=e==="claim"?l.emerald:l.amber;o.dataset.mode=e,o.style.cssText=ee(t),o.title=e==="claim"?"Take ownership of this lead":"Return this lead to the pool",o.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `},Kt=({contactId:o,mode:e})=>{let t=document.createElement("button");return t.id=D,t.type="button",t.dataset.contactId=o,R(t,e),t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-1px)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",async()=>{let r=t.dataset.mode??"claim",n=f();if(!n||!n.locationId||n.contactId!==o)return;let a=A();if(!a){c.warn("Claim/Release click: no user ID detected (login session not exposing it)");let s=t.querySelector(".ws-cr-label");s&&(s.textContent="Sign-in needed"),setTimeout(()=>R(t,r),2500);return}c.info(`Claim/Release click \u2014 contact=${o} mode=${r} user=${a}`);let i=r==="claim"?"release":"claim";R(t,i),t.disabled=!0,t.style.opacity="0.8";try{let s=await jt({contactId:o,locationId:n.locationId,userId:a,action:r}),d=s?.mode==="claim"||s?.mode==="release"?s.mode:i;R(t,d),c.info(`Claim/Release done \u2192 new mode: ${d}`)}catch(s){c.error("Claim/Release Set webhook failed",s),R(t,r);let d=t.querySelector(".ws-cr-label");if(d){let b=d.textContent??"";d.textContent="Try again",setTimeout(()=>{d.textContent==="Try again"&&(d.textContent=b)},2500)}}finally{t.disabled=!1,t.style.opacity="1"}}),t},Me=async(o,e)=>{try{let t=await Wt({contactId:e.contactId,locationId:e.locationId,userId:A()}),r=document.getElementById(D);if(!r||r.dataset.contactId!==e.contactId)return;let n=t?.mode==="release"?"release":"claim";R(r,n)}catch(t){c.warn("Claim/Release Get failed; pill stays in default 'claim' mode",t)}},Yt=async(o,e=1e4)=>{let t=Date.now();for(;Date.now()-t<e;){let r=Ne(o);if(r)return r;await new Promise(n=>setTimeout(n,150))}return null},He=(o,e)=>{let t=Kt({contactId:e,mode:"claim"}),r=document.createElement("span");r.id=`${D}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(t),o.appendChild(r)},Xt=async()=>{let o=f();if(!o){document.getElementById(D)?.remove();return}let e=document.getElementById(D);if(!(e&&e.dataset.contactId===o.contactId)){e&&e.remove();try{let t=await C("#record-details-lhs",{timeoutMs:1e4}),r=await Yt(t),n=f();if(!n||n.contactId!==o.contactId)return;if(!r){c.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}He(r,o.contactId),c.debug(`Mounted Claim/Release pill next to name for ${o.contactId}`);let a=document.getElementById(D);a&&o.locationId&&Me(a,{contactId:o.contactId,locationId:o.locationId});let i=null;i=new MutationObserver(async()=>{if(!(f()?.contactId===o.contactId)){i?.disconnect();return}if(document.getElementById(D))return;let d=Ne(t);if(d){He(d,o.contactId);let b=document.getElementById(D);b&&o.locationId&&Me(b,{contactId:o.contactId,locationId:o.locationId}),c.debug("Re-mounted Claim/Release pill after React wipe")}}),i.observe(t,{childList:!0,subtree:!0})}catch(t){c.warn("Could not mount Claim/Release pill:",t)}}},Pe=()=>{Xt()};var Jt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/type",V=new Map,Re=async(o,e)=>{if(!o)return null;if(V.has(o))return V.get(o)??null;if(!e)return c.warn(`Cannot fetch Contact Type for ${o} \u2014 tenant_id missing from URL context`),null;let t=new AbortController,r=setTimeout(()=>t.abort(),1500);try{let n=`${Jt}?contact_id=${encodeURIComponent(o)}&tenant_id=${encodeURIComponent(e)}`,a=await fetch(n,{method:"GET",headers:{Accept:"application/json"},signal:t.signal});if(clearTimeout(r),!a.ok)return c.warn(`Contact Type fetch returned ${a.status} for ${o}`),V.set(o,null),null;let s=((await a.json()).contact_type||"").toLowerCase().trim()||null;return V.set(o,s),c.debug(`Resolved Contact Type for ${o}: ${s??"(unset)"}`),s}catch(n){return clearTimeout(r),c.warn(`Contact Type fetch failed for ${o}:`,n),V.set(o,null),null}};var Oe=o=>o==="buyer"?"buyer":"seller";var qe="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set",Qt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/buyer-call-status/set";var Zt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",eo=async({contactId:o,locationId:e,status:t,userId:r,deadReason:n,webhookUrl:a,kind:i})=>{let s=a||qe,d;if(i==="buyer"){let p=ze.find(h=>h.label===t),m=p?p.value:t.toLowerCase().replace(/\s+/g,"-");d={tenant_id:e,contact_id:o,new_status:m,triggered_by_user_id:r??""}}else{let p={call_status:t,triggered_by_user_id:r??""};n&&(p.dead_reason=n),d={contact_id:o,location:{id:e},customData:p}}let b=await fetch(s,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d),credentials:"omit"});if(!b.ok)throw new Error(`Webhook returned ${b.status} ${b.statusText}`)},to=async({contactId:o,tenantId:e})=>{let t=await fetch(Zt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,contact_id:o}),credentials:"omit"});if(!t.ok)throw new Error(`Sanity check returned ${t.status} ${t.statusText}`);return await t.json()};var O="ws-crm-contact-actions-bar",oo="ws-crm-call-status-dropdown",Be="ws-crm-offer-made-modal",ro=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],ze=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"vm-left",label:"VM Left",color:"purple"},{value:"qualified-buyer",label:"Qualified Buyer",color:"green"},{value:"upgrade-to-vip",label:"Upgrade to VIP",color:"green"},{value:"walkthrough-scheduled",label:"Walkthrough Scheduled",color:"green"},{value:"made-an-offer",label:"Made an Offer",color:"green"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"wrong-market",label:"Wrong Market",color:"orange"},{value:"no-longer-buying",label:"No Longer Buying",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"blacklisted",label:"Blacklisted",color:"red"}],no=[".central-panel","[class*='central-panel']","[data-testid='central-panel']","[data-test='central-panel']",".contact-detail-page .central-panel",".contact-detail-page [class*='central']"],ao=async()=>{for(let o of no)try{let e=await C(o,{timeoutMs:2e3});if(e)return c.info(`[actions-bar] mount target found via selector: ${o}`),e}catch{}try{let o=await C("#contact-conversation-panel",{timeoutMs:2e3});if(o.parentElement)return c.info("[actions-bar] mount target found via #contact-conversation-panel.parentElement"),o.parentElement}catch{}for(let o of["[id*='conversation-panel']","[class*='conversation-panel']","[class*='contact-detail'] [class*='panel']","main [class*='central']","main [class*='panel']:not([class*='sidebar']):not([class*='nav'])"])try{let e=await C(o,{timeoutMs:1500});if(e)return c.info(`[actions-bar] mount target via fallback: ${o}`),e.parentElement??e}catch{}return c.warn("[actions-bar] no mount target found via any selector \u2014 URL:",window.location.pathname),null},so=({contactId:o,kind:e})=>{let t=e==="buyer"?ze:ro,r=e==="buyer"?Qt:qe,n=e==="buyer"?"Buyer Call Status":"Seller Call Status",a=document.createElement("div");a.id=oo,a.style.cssText="position: relative; flex-shrink: 0;";let i=document.createElement("button");i.type="button",i.style.cssText=B,i.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `,t.forEach(d=>{let b=document.createElement("button");b.type="button",b.style.cssText=`
    ...
    `,b.innerHTML=`
      ...
      "></span>
      <span>${d.label}</span>
    `,b.addEventListener("mouseenter",()=>{b.style.background=l.slate}),b.addEventListener("mouseleave",()=>{b.style.background="transparent"}),b.addEventListener("click",async()=>{c.info(`Call Status set to "${d.label}" (${d.value}, ${d.color}) for contact ${o}`);let p=i.firstElementChild,m=i.children[1],h=i.children[2];p&&(p.style.background=ye[d.color]),m&&(m.textContent=d.label),s.style.display="none";let y=f()?.locationId,k=A();if(!y){c.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),m&&(m.textContent=`${d.label} (no tenant!)`),m&&(m.style.color=l.amber);return}if(e==="seller"&&d.color==="green"&&!k){c.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),m&&(m.textContent=`${d.label} (no user!)`),m&&(m.style.color=l.amber);return}let g=h?.textContent??"\u25BE",E=async _=>{h&&(h.textContent="\u2026"),i.style.opacity="0.75";try{await eo({contactId:o,locationId:y,status:d.label,userId:k,deadReason:_,webhookUrl:r,kind:e}),h&&(h.textContent="\u2713"),setTimeout(()=>{h&&(h.textContent=g),i.style.opacity="1"},900)}catch(w){c.warn("Call Status webhook failed:",w),h&&(h.textContent="\u2717"),m&&(m.style.color=l.amber),i.style.opacity="1",setTimeout(()=>{h&&(h.textContent=g),m&&(m.style.color=l.bone)},1800)}};if(e==="seller"&&d.value==="dead-deal"){let _=m?.textContent??"";lo(w=>{E(w)},()=>{m&&(m.textContent=_)});return}if(e==="seller"&&d.value==="appointment-booked"){let _=m?.textContent??"";try{(await to({contactId:o,tenantId:y})).ok?await E():co(()=>{E()},()=>{m&&(m.textContent=_)})}catch(w){c.warn("Appointment sanity check failed; firing anyway:",w),await E()}return}await E()}),s.appendChild(b)}),i.addEventListener("click",d=>{d.stopPropagation(),s.style.display=s.style.display==="none"?"block":"none"}),document.addEventListener("click",d=>{a.contains(d.target)||(s.style.display="none")}),a.appendChild(i),a.appendChild(s),a},io=o=>{document.getElementById(Be)?.remove();let e=document.createElement("div");e.id=Be,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,t.appendChild(r),document.body.appendChild(t);let n=()=>t.remove(),a=()=>{n(),e()};t.addEventListener("click",s=>{s.target===t&&a()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",a),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),o()});let i=s=>{s.key==="Escape"&&(document.removeEventListener("keydown",i),a())};document.addEventListener("keydown",i)},Ve=(o,e)=>{let t=document.createElement("div");t.id=O,t.dataset.contactId=o,t.dataset.contactKind=e,t.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    ...
    border-bottom: 1px solid ${l.steel};
    flex-shrink: 0;
  `;let r=document.createElement("button");return r.type="button",r.style.cssText=F,r.textContent="Offer Made",r.addEventListener("click",()=>io(o)),t.appendChild(r),t.appendChild(so({contactId:o,kind:e})),t},we=!1,bo=()=>{document.querySelectorAll(`[id="${O}"]`).forEach(o=>o.remove())},We=async()=>{if(c.info("[actions-bar:01] ensureMounted called; url:",window.location.pathname),we){c.info("[actions-bar:00-LOCKED] ensureMounted already in-flight \u2014 skip");return}we=!0;try{await ho()}finally{we=!1}},ho=async()=>{let o=f();if(!o){c.info("[actions-bar:02-BAIL] no contact context (URL didn't match patterns AND no in-page contact link). URL:",window.location.pathname),bo();return}c.info(`[actions-bar:02] ctx resolved \u2014 contactId=${o.contactId} locationId=${o.locationId??"(null)"} source=${o.source}`);let e="seller";try{let b=await Re(o.contactId,o.locationId);Oe(b)==="buyer"&&(e="buyer"),c.info(`[actions-bar:03] kind resolved \u2014 kind=${e} (raw type=${b??"(null)"})`)}catch(b){c.warn("[actions-bar:03-FAIL] Contact Type resolution threw \u2014 defaulting to seller:",b)}let t=f();if(!t||t.contactId!==o.contactId){c.info("[actions-bar:04-BAIL] context changed during Contact Type fetch \u2014 aborting mount");return}let r=Array.from(document.querySelectorAll(`[id="${O}"]`));if(r.length===1&&r[0].dataset.contactId===o.contactId&&r[0].dataset.contactKind===e){c.info(`[actions-bar:05-SKIP] bar already mounted for contact=${o.contactId} kind=${e} \u2014 no-op`);return}r.length>0&&(c.info(`[actions-bar:05] removing ${r.length} stale bar(s) before remount`),r.forEach(b=>b.remove()));let n=await ao();if(!n){c.warn("[actions-bar:06-FAIL] findMountTarget returned null \u2014 no .central-panel or fallback selector matched within timeout. URL:",window.location.pathname);return}c.info(`[actions-bar:06] mount target found \u2014 tagName=${n.tagName} class="${n.className}"`);let a=f();if(!a||a.contactId!==o.contactId){c.info("[actions-bar:07-BAIL] context changed during findMountTarget wait \u2014 aborting mount");return}let i;try{i=Ve(o.contactId,e)}catch(b){c.error("[actions-bar:08-FAIL] buildBar threw:",b);return}i.dataset.mountTarget=n.className||"central-panel";try{n.insertBefore(i,n.firstChild)}catch(b){c.error("[actions-bar:09-FAIL] insertBefore threw:",b);return}if(!document.getElementById(O)){c.warn("[actions-bar:10-DETACHED] bar inserted but not in document tree \u2014 target was likely re-rendered by Vue. Will retry next page-change.");return}c.info(`[actions-bar:10-OK] bar mounted + verified in document for contact=${o.contactId} kind=${e}`);let d=null;d=new MutationObserver(()=>{if(!(f()?.contactId===o.contactId)){d?.disconnect();return}let p=document.querySelectorAll(`[id="${O}"]`);if(p.length>1){for(let m=1;m<p.length;m++)p[m].remove();c.info(`[actions-bar:WATCHDOG] trimmed ${p.length-1} duplicate bar(s)`);return}if(p.length===0){let m=document.querySelector(".central-panel")||n,h=Ve(o.contactId,e);h.dataset.mountTarget=i.dataset.mountTarget??"";try{m.insertBefore(h,m.firstChild),c.info(`[actions-bar:WATCHDOG] re-mounted after wipe for ${o.contactId}`)}catch(u){c.warn("[actions-bar:WATCHDOG-FAIL] re-mount threw:",u)}}}),d.observe(n,{childList:!0})},Ue=null,po=()=>{Ue===null&&(Ue=window.setInterval(()=>{let o=f();o&&(document.getElementById(O)||(c.info("[actions-bar:HEALER] no bar on contact page \u2014 re-triggering mount for",o.contactId),We()))},3e3))},je=()=>{po(),We()};var oe="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",mo=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),U="data-ws-crm-favicon",xe=o=>o.hasAttribute(U),Ke=o=>{if(o.tagName!=="LINK")return!1;let e=(o.getAttribute("rel")??"").toLowerCase();return mo.has(e)},re=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{Ke(r)&&!xe(r)&&r.remove()});let e=document.head.querySelector(`link[${U}="1"]`);e?e.href!==oe&&(e.href=oe):(e=document.createElement("link"),e.setAttribute(U,"1"),e.rel="icon",e.type="image/png",e.href=oe,document.head.appendChild(e),c.debug("Favicon installed"));let t=document.head.querySelector(`link[${U}="apple"]`);t||(t=document.createElement("link"),t.setAttribute(U,"apple"),t.rel="apple-touch-icon",t.href=oe,document.head.appendChild(t))},Ye=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)re();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),re())},50)}new MutationObserver(r=>{let n=!1;for(let a of r)a.addedNodes.forEach(i=>{i instanceof HTMLLinkElement&&Ke(i)&&!xe(i)&&(n=!0)}),a.removedNodes.forEach(i=>{i instanceof HTMLLinkElement&&xe(i)&&(n=!0)});n&&re()}).observe(document.head,{childList:!0,subtree:!1});let e=0,t=window.setInterval(()=>{e+=1,re(),e>=5&&window.clearInterval(t)},1e3)};var _o="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/mt/not-interested",q="ws-crm-not-interested-button",vo=/\/opportunities\/detail\/([A-Za-z0-9]+)/,uo=/\/v2\/location\/([A-Za-z0-9]+)/,ne=()=>{let o=window.location.pathname,e=o.match(vo);if(!e)return null;let t=o.match(uo);return{oppId:e[1],locationId:t?t[1]:null}},yo=async o=>{let e=await fetch(_o,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,mt_opp_id:o.oppId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Not Interested webhook returned ${e.status} ${e.statusText}`);return await e.json()},Xe=o=>{let e=document.createElement("button");return e.id=q,e.type="button",e.dataset.oppId=o,e.style.cssText=ee("neutral"),e.style.fontFamily=v.sans,e.style.fontSize="12px",e.title="Close this Marketing Tracker opp as Not Interested for THIS property only",e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let t=ne();if(!t||!t.locationId||t.oppId!==o)return;let r=A(),n=e.querySelector(".ws-ni-label");e.disabled=!0,e.style.opacity="0.7",n&&(n.textContent="Closing\u2026");try{let a=await yo({oppId:t.oppId,locationId:t.locationId,userId:r});if(a?.ok)n&&(n.textContent="Marked Not Interested"),e.style.background=l.crimsonGlow,e.style.color=l.crimson,c.info(`Not Interested set for opp ${o} \u2192 ${a.new_stage}`);else throw new Error("Webhook returned ok=false")}catch(a){c.error("Not Interested webhook failed",a),n&&(n.textContent="Try again"),e.disabled=!1,e.style.opacity="1",setTimeout(()=>{n&&n.textContent==="Try again"&&(n.textContent="Mark Not Interested")},3e3)}}),e},go=["[data-testid='OPPORTUNITY_HEADER']","[class*='opportunity-header']","[class*='opp-header']","header.flex.items-center"],fo=async()=>{for(let o of go)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},wo=async()=>{let o=ne();if(!o||!o.locationId){document.getElementById(q)?.remove();return}let e=document.getElementById(q);if(e&&e.dataset.oppId===o.oppId)return;e&&e.remove();let t=await fo();if(!t){c.warn("Not Interested: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=ne();if(!r||r.oppId!==o.oppId)return;let n=Xe(o.oppId),a=document.createElement("span");a.id=`${q}-wrap`,a.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,a.appendChild(n),t.appendChild(a),c.debug(`Mounted Not Interested button on opp ${o.oppId}`);let i=new MutationObserver(()=>{let s=document.getElementById(q);if(!(ne()?.oppId===o.oppId)){i.disconnect();return}if(!s){let b=Xe(o.oppId);a.appendChild(b),c.debug(`Re-mounted Not Interested button after wipe for opp ${o.oppId}`)}});i.observe(t,{childList:!0})},Je=()=>{wo()};var xo="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/tc/send-to-title-co",z="ws-crm-send-to-title-co-button",Qe="ws-crm-send-to-title-co-modal",ko=/\/opportunities\/detail\/([A-Za-z0-9]+)/,To=/\/v2\/location\/([A-Za-z0-9]+)/,ae=()=>{let o=window.location.pathname,e=o.match(ko);if(!e)return null;let t=o.match(To);return{oppId:e[1],locationId:t?t[1]:null}},tt=async o=>{let e=await fetch(xo,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,tc_opp_id:o.oppId,triggered_by_user_id:o.userId??null,confirm_resend:o.confirmResend}),credentials:"omit"});if(!e.ok&&e.status!==200)throw new Error(`Send-to-Title-Co webhook returned ${e.status}`);return await e.json()},Ze=o=>{document.getElementById(Qe)?.remove();let e=document.createElement("div");e.id=Qe,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let t=ae();if(!t||!t.locationId||t.oppId!==o)return;let r=e.querySelector(".ws-stc-label");e.disabled=!0,e.style.opacity="0.7",r&&(r.textContent="Checking\u2026");try{let n=await tt({oppId:t.oppId,locationId:t.locationId,userId:A(),confirmResend:!1});if(e.disabled=!1,e.style.opacity="1",r&&(r.textContent="Send to Title Company"),n.needs_confirm){Ze({oppId:t.oppId,locationId:t.locationId,isResend:!0,previousSentAt:n.previously_sent_at??null,onConfirmed:()=>{r&&(r.textContent="Re-sent \u2713"),e.style.background=l.emerald,setTimeout(()=>{r&&(r.textContent="Send to Title Company"),e.style.background=""},4e3)}});return}if(n.ok){r&&(r.textContent="Sent \u2713"),e.style.background=l.emerald,setTimeout(()=>{r&&(r.textContent="Send to Title Company"),e.style.background=""},4e3);return}Ze({oppId:t.oppId,locationId:t.locationId,isResend:!1,onConfirmed:()=>{r&&(r.textContent="Sent \u2713"),e.style.background=l.emerald,setTimeout(()=>{r&&(r.textContent="Send to Title Company"),e.style.background=""},4e3)}})}catch(n){c.error("Send to Title Co preflight failed",n),e.disabled=!1,e.style.opacity="1",r&&(r.textContent="Try again"),setTimeout(()=>{r&&r.textContent==="Try again"&&(r.textContent="Send to Title Company")},3e3)}}),e},Eo=["[data-testid='OPPORTUNITY_HEADER']","[class*='opportunity-header']","[class*='opp-header']","header.flex.items-center"],So=async()=>{for(let o of Eo)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},Co=async()=>{let o=ae();if(!o||!o.locationId){document.getElementById(z)?.remove();return}let e=document.getElementById(z);if(e&&e.dataset.oppId===o.oppId)return;e&&e.remove();let t=await So();if(!t){c.warn("Send to Title Co: no opp-header mount target found within timeout");return}let r=ae();if(!r||r.oppId!==o.oppId)return;let n=et(o.oppId),a=document.createElement("span");a.id=`${z}-wrap`,a.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,a.appendChild(n),t.appendChild(a),c.debug(`Mounted Send to Title Co button on opp ${o.oppId}`);let i=new MutationObserver(()=>{let s=document.getElementById(z);if(!(ae()?.oppId===o.oppId)){i.disconnect();return}if(!s){let b=et(o.oppId);a.appendChild(b),c.debug(`Re-mounted Send to Title Co button after wipe for opp ${o.oppId}`)}});i.observe(t,{childList:!0})},ot=()=>{Co()};var be="reos-theme-stylesheet",yt="reos-theme-fonts",Ao=()=>{let o=document.getElementById(yt);o&&o.remove()},Io=`
/* ============================================
...
html body .crm-opportunities-modal:not(#__reos_never_id) .hr-upload {
  ...
  /* v0.13.34: dragger box transparent (was slate). Just the dashed border
     and the steel icon circle remain \u2014 no surround. */
  --n-dragger-color: transparent !important;
...
/* v0.13.33: bump icon size back to normal \u2014 v0.13.32 sized it 26px which
   was way too small relative to the dragger. 44px SVG inside an 88px
   circle is the standard upload-dragger proportion. */
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};console.log("[ws-crm-customizer] bundle-loaded");var I=(o,e)=>{try{c.info(`[main] calling ${o}`),e(),c.info(`[main] ${o} OK`)}catch(t){c.error(`[main:FAIL] ${o} threw:`,t)}},wt=()=>{c.info(`Loaded v${Q} (built ${Z})`),I("mountVersionBadge (initial)",pe),I("mountFavicon",Ye),I("mountThemeStylesheet",gt),I("initPageRouter",Ce),c.info("[main] registering onPageChange subscriber"),Se(o=>{c.info(`[main] onPageChange callback fired for page=${o}`),I("mountVersionBadge",pe),I("mountClaimReleaseToggle",Pe),I("mountContactActionsBar",je),I("mountActiveFollowUpButton",De),I("mountNotInterestedButton",Je),I("mountSendToTitleCoButton",ot)}),c.info("[main] onPageChange registered; main() complete")};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",wt):wt();})();
