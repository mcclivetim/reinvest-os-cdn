/* Wholesaling CRM Customizer v0.5.38 — built 2026-05-16T16:28:45.064Z */
"use strict";var WholesalingCRMCustomizer=(()=>{var T="[wholesaling-crm-customizer]",fe="__WS_CRM_DEBUG",ye=()=>!!window[fe],d={info(...o){console.log(T,...o)},warn(...o){console.warn(T,...o)},error(...o){console.error(T,...o)},debug(...o){ye()&&console.log(T,"[debug]",...o)}};var z=()=>{let o=window.location.pathname;return o.includes("/contacts/detail/")?"contact-detail":o.includes("/contacts/smart-list")||o.includes("/contacts/")?"contact-list":o.includes("/opportunities/detail/")?"opportunity-detail":o.includes("/opportunities/")?"opportunity-list":o.includes("/calendars/")?"calendar":o.includes("/dashboard")?"dashboard":o.includes("/conversations")?"conversations":o.includes("/settings")?"settings":"other"},I="",E="other",B=new Set,L=()=>{if(window.location.pathname!==I){I=window.location.pathname,E=z(),d.debug("Page changed:",E,I);for(let o of B)try{o(E)}catch(t){d.error("Subscriber threw:",t)}}},U=o=>(B.add(o),setTimeout(()=>o(E),0),()=>{B.delete(o)}),q=()=>{E=z(),I=window.location.pathname,d.debug("Router init, current page:",E);let o=history.pushState.bind(history),t=history.replaceState.bind(history);history.pushState=function(e,r,a){o(e,r,a),L()},history.replaceState=function(e,r,a){t(e,r,a),L()},window.addEventListener("popstate",L),setInterval(L,1e3)};var A="0.5.38",M="2026-05-16T16:28:45.065Z";var ve=/\/contacts\/detail\/([A-Za-z0-9]+)/,xe=/\/v2\/location\/([A-Za-z0-9]+)/,y=()=>{let o=window.location.pathname,t=o.match(ve);if(!t)return null;let e=o.match(xe);return{contactId:t[1],locationId:e?e[1]:null}};var S=(o,t={})=>{let{timeoutMs:e=8e3,pollMs:r=100,root:a=document}=t;return new Promise((n,l)=>{let s=a.querySelector(o);if(s){n(s);return}let i,p,g=()=>{u&&u.disconnect(),i!==void 0&&window.clearTimeout(i),p!==void 0&&window.clearInterval(p)},u=new MutationObserver(()=>{let m=a.querySelector(o);m&&(g(),n(m))});u.observe(document.body,{childList:!0,subtree:!0}),p=window.setInterval(()=>{let m=a.querySelector(o);m&&(g(),n(m))},r),i=window.setTimeout(()=>{g(),d.warn(`waitForElement timed out for selector: ${o}`),l(new Error(`Selector not found within ${e}ms: ${o}`))},e)})};var c={obsidian:"#0A0D12",graphite:"#12161D",slate:"#1A1F28",steel:"#252C36",bone:"#EDEEF0",ash:"#9098A3",coolGray:"#5A6470",emerald:"#0FB57E",emeraldBright:"#14C98B",blue:"#4B8BF5",amber:"#E8A33C",crimson:"#D43F4A",emeraldGlow:"rgba(15, 181, 126, 0.12)",emeraldBorder:"rgba(15, 181, 126, 0.3)",blueGlow:"rgba(75, 139, 245, 0.12)",amberGlow:"rgba(232, 163, 60, 0.12)",crimsonGlow:"rgba(212, 63, 74, 0.12)"},v={sm:"4px",md:"6px",lg:"10px",pill:"999px"},f={sans:"'Geist', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",mono:"'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace"},H={purple:"#8B5CF6",green:c.emerald,orange:c.amber,red:c.crimson},W=o=>{let t=o==="emerald"?c.emeraldGlow:o==="amber"?c.amberGlow:c.slate,e=o==="emerald"?c.emeraldBorder:o==="amber"?"rgba(232, 163, 60, 0.3)":c.steel,r=o==="emerald"?c.emerald:o==="amber"?c.amber:c.bone;return`
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: 4px 10px;
...
`,N=`
  ...
  flex-shrink: 0;
`;var _="ws-crm-claim-release-pill",j=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},we=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=_,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?c.emerald:c.amber;return e.style.cssText=W(r),e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,e.title=t==="claim"?"Take ownership of this lead":"Return this lead to the pool",e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{d.info(`Claim/Release pill clicked for contact ${o}: ${t}`);let n=e.lastElementChild,l=n?.textContent??"";n&&(n.textContent="Working\u2026"),e.style.opacity="0.7",e.disabled=!0,setTimeout(()=>{n&&(n.textContent=l),e.style.opacity="1",e.disabled=!1},700)}),e},ke=async(o,t=1e4)=>{let e=Date.now();for(;Date.now()-e<t;){let r=j(o);if(r)return r;await new Promise(a=>setTimeout(a,150))}return null},V=(o,t)=>{let e=we({contactId:t,mode:"claim"}),r=document.createElement("span");r.id=`${_}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(e),o.appendChild(r)},Ee=async()=>{let o=y();if(!o){document.getElementById(_)?.remove();return}let t=document.getElementById(_);if(!(t&&t.dataset.contactId===o.contactId)){t&&t.remove();try{let e=await S("#record-details-lhs",{timeoutMs:1e4}),r=await ke(e),a=y();if(!a||a.contactId!==o.contactId)return;if(!r){d.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}V(r,o.contactId),d.debug(`Mounted Claim/Release pill next to name for ${o.contactId}`);let n=null;n=new MutationObserver(async()=>{if(!(y()?.contactId===o.contactId)){n?.disconnect();return}if(document.getElementById(_))return;let s=j(e);s&&(V(s,o.contactId),d.debug("Re-mounted Claim/Release pill after React wipe"))}),n.observe(e,{childList:!0,subtree:!0})}catch(e){d.warn("Could not mount Claim/Release pill:",e)}}},Y=()=>{Ee()};var Se="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set",_e=()=>{let o=window.WS_CRM_USER_ID;if(o&&/^[A-Za-z0-9]{15,30}$/.test(o))return o;let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let r of e)if(typeof r=="string"&&/^[A-Za-z0-9]{15,30}$/.test(r))return r;try{let r=localStorage.getItem("user");if(r){let a=JSON.parse(r),n=a.id??a._id;if(n&&/^[A-Za-z0-9]{15,30}$/.test(n))return n}for(let a of["tokenUser","currentUser","ghl_user"]){let n=localStorage.getItem(a);if(n)try{let l=JSON.parse(n),s=l.id??l._id;if(s&&/^[A-Za-z0-9]{15,30}$/.test(s))return s}catch{}}}catch{}return null},Ce=async({contactId:o,locationId:t,status:e,userId:r})=>{let n=await fetch(Se,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:o,location:{id:t},customData:{call_status:e,triggered_by_user_id:r??""}}),credentials:"omit"});if(!n.ok)throw new Error(`Webhook returned ${n.status} ${n.statusText}`)},D="ws-crm-contact-actions-bar",Te="ws-crm-call-status-dropdown",Z="ws-crm-offer-made-modal",Le=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],Ie=[".central-panel","[class*='central-panel']"],Ae=async()=>{for(let o of Ie)try{let t=await S(o,{timeoutMs:3e3});if(t)return t}catch{}try{return(await S("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},Me=o=>{let t=document.createElement("div");t.id=Te,t.style.cssText="position: relative; flex-shrink: 0;";let e=document.createElement("button");e.type="button",e.style.cssText=N,e.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `,Le.forEach(a=>{let n=document.createElement("button");n.type="button",n.style.cssText=`
    ...
    `,n.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,n.addEventListener("mouseenter",()=>{n.style.background=c.slate}),n.addEventListener("mouseleave",()=>{n.style.background="transparent"}),n.addEventListener("click",async()=>{d.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let l=e.firstElementChild,s=e.children[1],i=e.children[2];l&&(l.style.background=H[a.color]),s&&(s.textContent=a.label),r.style.display="none";let g=y()?.locationId,u=_e();if(!g){d.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),s&&(s.textContent=`${a.label} (no tenant!)`),s&&(s.style.color=c.amber);return}if(a.color==="green"&&!u){d.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),s&&(s.textContent=`${a.label} (no user!)`),s&&(s.style.color=c.amber);return}let m=i?.textContent??"\u25BE";i&&(i.textContent="\u2026"),e.style.opacity="0.75";try{await Ce({contactId:o,locationId:g,status:a.label,userId:u}),i&&(i.textContent="\u2713"),setTimeout(()=>{i&&(i.textContent=m),e.style.opacity="1"},900)}catch(P){d.warn("Call Status webhook failed:",P),i&&(i.textContent="\u2717"),s&&(s.style.color=c.amber),e.style.opacity="1",setTimeout(()=>{i&&(i.textContent=m),s&&(s.style.color=c.bone)},1800)}}),r.appendChild(n)}),e.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{t.contains(a.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},De=o=>{document.getElementById(Z)?.remove();let t=document.createElement("div");t.id=Z,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var he=()=>{d.info(`Loaded v${A} (built ${M})`),pe(),q(),U(o=>{d.debug("Page handler firing for:",o),me(),Y(),K()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",he):he();})();
