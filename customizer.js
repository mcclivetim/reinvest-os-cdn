...
`;var I="ws-crm-claim-release-pill",re=o=>{let e=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},De=({contactId:o,mode:e})=>{let t=document.createElement("button");t.id=I,t.type="button",t.dataset.contactId=o,t.dataset.mode=e;let r=e==="claim"?"emerald":"amber",n=e==="claim"?l.emerald:l.amber;return t.style.cssText=te(r),t.innerHTML=`
  ...
  `,$e.forEach(n=>{let i=document.createElement("button");i.type="button",i.style.cssText=`
    ...
    `,i.innerHTML=`
      ...
      "></span>
      <span>${n.label}</span>
    `,i.addEventListener("mouseenter",()=>{i.style.background=l.slate}),i.addEventListener("mouseleave",()=>{i.style.background="transparent"}),i.addEventListener("click",async()=>{b.info(`Call Status set to "${n.label}" (${n.value}, ${n.color}) for contact ${o}`);let s=t.firstElementChild,a=t.children[1],d=t.children[2];s&&(s.style.background=j[n.color]),a&&(a.textContent=n.label),r.style.display="none";let p=E()?.locationId,m=Be();if(!p){b.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),a&&(a.textContent=`${n.label} (no tenant!)`),a&&(a.style.color=l.amber);return}if(n.color==="green"&&!m){b.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),a&&(a.textContent=`${n.label} (no user!)`),a&&(a.style.color=l.amber);return}let h=d?.textContent??"\u25BE",x=async g=>{d&&(d.textContent="\u2026"),t.style.opacity="0.75";try{await Oe({contactId:o,locationId:p,status:n.label,userId:m,deadReason:g}),d&&(d.textContent="\u2713"),setTimeout(()=>{d&&(d.textContent=h),t.style.opacity="1"},900)}catch(u){b.warn("Call Status webhook failed:",u),d&&(d.textContent="\u2717"),a&&(a.style.color=l.amber),t.style.opacity="1",setTimeout(()=>{d&&(d.textContent=h),a&&(a.style.color=l.bone)},1800)}};if(n.value==="dead-deal"){let g=a?.textContent??"";qe(u=>{x(u)},()=>{a&&(a.textContent=g)});return}if(n.value==="appointment-booked"){let g=a?.textContent??"";try{(await Pe({contactId:o,tenantId:p})).ok?await x():je(()=>{x()},()=>{a&&(a.textContent=g)})}catch(u){b.warn("Appointment sanity check failed; firing anyway:",u),await x()}return}await x()}),r.appendChild(i)}),t.addEventListener("click",n=>{n.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",n=>{e.contains(n.target)||(r.style.display="none")}),e.appendChild(t),e.appendChild(r),e},We=o=>{document.getElementById(ae)?.remove();let e=document.createElement("div");e.id=ae,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
