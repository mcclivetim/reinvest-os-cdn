...
`,L=`
  ...
  flex-shrink: 0;
`;var D="ws-crm-claim-release-pill",re=o=>{let e=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Ie=({contactId:o,mode:e})=>{let t=document.createElement("button");t.id=D,t.type="button",t.dataset.contactId=o,t.dataset.mode=e;let r=e==="claim"?"emerald":"amber",a=e==="claim"?i.emerald:i.amber;return t.style.cssText=te(r),t.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
...
/* 18a-native-checkbox (v0.8.51). Native <input type="checkbox">
   used across the app (Tabulator row select, etc.) was rendering
   with browser default white box. Tim wants:
   - Unchecked: slate bg + steel border (matches form-input chrome)
   - Checked: brand blue fill + bone checkmark
   - No hover state (would be noisy in long lists)

   Scope: native checkboxes only. Skip Naive UI / hr-checkbox
   wrappers which have their own styling via .n-checkbox / .hr-
   checkbox-* (covered elsewhere). */
