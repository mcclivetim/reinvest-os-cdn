...
`;var qt=/^[A-Za-z0-9]{15,30}$/,W=o=>typeof o=="string"&&qt.test(o),zt=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},Wt=()=>{let o=window.WS_CRM_USER_ID;return W(o)?o:null},jt=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(W(t))return t;return null},Kt=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=zt(e);if(!t)return null;let r=t.uid;if(W(r))return r;let n=t.claims;if(n&&W(n.user_id))return n.user_id}catch{}return null},Yt=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(W(n))return n}catch{}}}catch{}return null},A=()=>Wt()||jt()||Kt()||Yt()||null;var Je="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",Xt=`${Je}/active-follow-up-get`,Jt=`${Je}/active-follow-up-set`,O="ws-crm-active-follow-up-button",Fe={amber:{label:"Add to Active Follow Up",bg:l.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:l.amber,clickable:!0,dotColor:l.amber},blue:{label:"In Active Follow Up",bg:l.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:l.blue,clickable:!1,dotColor:l.blue},gray:{label:"Not in Follow Up",bg:l.slate,border:l.steel,color:l.coolGray,clickable:!1,dotColor:l.coolGray},loading:{label:"Active Follow Up\u2026",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.ash},error:{label:"Follow Up status unavailable",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.crimson}},Qt=()=>{let o=T();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},Zt=async o=>{let e=await fetch(Xt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},eo=async(o,e)=>{let t=await fetch(Jt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},B=(o,e)=>{let t=Fe[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},Ke=o=>{let e=document.createElement("button");return e.type="button",e.id=O,e.dataset.contactId=o,e.style.cssText=`
  ...
  `,t.forEach(d=>{let b=document.createElement("button");b.type="button",b.style.cssText=`
    ...
    `,b.innerHTML=`
      ...
      "></span>
      <span>${d.label}</span>
    `,b.addEventListener("mouseenter",()=>{b.style.background=l.slate}),b.addEventListener("mouseleave",()=>{b.style.background="transparent"}),b.addEventListener("click",async()=>{c.info(`Call Status set to "${d.label}" (${d.value}, ${d.color}) for contact ${o}`);let p=i.firstElementChild,m=i.children[1],h=i.children[2];p&&(p.style.background=Ie[d.color]),m&&(m.textContent=d.label),s.style.display="none";let y=T()?.locationId,g=A();if(!y){c.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),m&&(m.textContent=`${d.label} (no tenant!)`),m&&(m.style.color=l.amber);return}if(e==="seller"&&d.color==="green"&&!g){c.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),m&&(m.textContent=`${d.label} (no user!)`),m&&(m.style.color=l.amber);return}let v=h?.textContent??"\u25BE",f=async u=>{h&&(h.textContent="\u2026"),i.style.opacity="0.75";try{await go({contactId:o,locationId:y,status:d.label,userId:g,deadReason:u,webhookUrl:r,kind:e}),h&&(h.textContent="\u2713"),setTimeout(()=>{h&&(h.textContent=v),i.style.opacity="1"},900)}catch(x){c.warn("Call Status webhook failed:",x),h&&(h.textContent="\u2717"),m&&(m.style.color=l.amber),i.style.opacity="1",setTimeout(()=>{h&&(h.textContent=v),m&&(m.style.color=l.bone)},1800)}};if(e==="seller"&&d.value==="dead-deal"){let u=m?.textContent??"";Co(x=>{f(x)},()=>{m&&(m.textContent=u)});return}if(e==="seller"&&d.value==="appointment-booked"){let u=m?.textContent??"";try{(await fo({contactId:o,tenantId:y})).ok?await f():Ao(()=>{f()},()=>{m&&(m.textContent=u)})}catch(x){c.warn("Appointment sanity check failed; firing anyway:",x),await f()}return}await f()}),s.appendChild(b)}),i.addEventListener("click",d=>{d.stopPropagation(),s.style.display=s.style.display==="none"?"block":"none"}),document.addEventListener("click",d=>{a.contains(d.target)||(s.style.display="none")}),a.appendChild(i),a.appendChild(s),a},So=o=>{document.getElementById(at)?.remove();let e=document.createElement("div");e.id=at,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,r.appendChild(d);let b=["TC","ACQ","MT","DISPO","Property"],p={};o.forEach(_=>{p[_.group]||(p[_.group]=[]),p[_.group].push(_)});let m=[...b.filter(_=>p[_]),...Object.keys(p).filter(_=>!b.includes(_))];for(let _ of m){let y=p[_],g=document.createElement("div");g.style.cssText="margin-top: 12px;";let v=document.createElement("div");v.style.cssText=`
      font-family: ${w.mono};
      font-size: 9px;
      ...
      padding-bottom: 4px;
      border-bottom: 1px solid ${l.steel};
    `,v.textContent=_,g.appendChild(v),y.forEach(f=>{let u=document.createElement("div"),x=!f.filled&&f.navigable&&f.field_id;u.style.cssText=`
        display: flex;
        align-items: center;
        gap: 10px;
        padding: 6px 8px;
        ...
        ${x?"cursor: pointer;":""}
        transition: background 0.1s ease;
      `,x&&(u.addEventListener("mouseenter",()=>{u.style.background=l.slate}),u.addEventListener("mouseleave",()=>{u.style.background="transparent"}),u.addEventListener("click",I=>{I.stopPropagation(),f.field_id&&(Jo(f.field_id,f.folder),F())}));let S=f.filled?`<span style="color: ${l.emerald}; font-size: 14px; line-height: 1; flex-shrink: 0;">\u2713</span>`:`<span style="color: ${l.amber}; font-size: 14px; line-height: 1; flex-shrink: 0;">\u25CB</span>`,k=x?`<span style="margin-left: auto; color: ${l.ash}; font-size: 11px;">\u2192</span>`:"";u.innerHTML=`${S}<span style="flex: 1;">${f.label}</span>${k}`,g.appendChild(u)}),r.appendChild(g)}document.body.appendChild(r);let h=_=>{let y=_.target;!r.contains(y)&&!t.contains(y)&&(F(),document.removeEventListener("click",h))};window.setTimeout(()=>document.addEventListener("click",h),0)},M=(o,e)=>{let t=o.querySelector(".ws-stc-label"),r=o.querySelector(".ws-stc-dot"),n=o.querySelector(".ws-stc-chevron"),a,i,s=l.bone,d,b="Send to Title Company",p="",m="pointer",h="1";switch(e.status){case"checking":a="transparent",i=l.steel,s=l.ash,d=l.coolGray,b="Checking\u2026",m="wait",h="0.8";break;case"ready":a=l.emerald,i=l.emerald,s="#0a0e14",d="#0a0e14",b=e.previouslySentAt?"Re-send to Title Co":"Send to Title Company";break;case"incomplete":{let _=e.checks?.filter(g=>g.filled).length??0,y=e.checks?.length??0;a="transparent",i=l.amber,s=l.amber,d=l.amber,b=`${_} of ${y} fields ready`,p="\u25BE";break}case"sending":a=l.emerald,i=l.emerald,s="#0a0e14",d="#0a0e14",b="Sending\u2026",m="wait",h="0.8";break;case"sent":a=l.emerald,i=l.emerald,s="#0a0e14",d="#0a0e14",b="Sent \u2713";break;case"error":a="transparent",i=l.amber,s=l.amber,d=l.amber,b=e.message??"Try again";break}o.style.background=a,o.style.borderColor=i,o.style.color=s,o.style.cursor=m,o.style.opacity=h,r&&(r.style.background=d,r.style.boxShadow=e.status==="ready"||e.status==="sent"?`0 0 6px ${d}`:""),t&&(t.textContent=b),n&&(n.textContent=p)},Qo=(o,e)=>{let t=document.createElement("button");t.id=G,t.type="button",t.dataset.oppId=o;let r={status:"checking"};t.style.cssText=`
    ${P}
    background: transparent;
    border: 1px solid ${l.steel};
    color: ${l.ash};
  ...
  `,M(t,r);let n=new Set,a=async()=>{if(r.status==="sending")return;let m=await Ko(o,e);if(!m){r={status:"error",message:"Check failed \u2014 click to retry"},M(t,r);return}if(!Array.isArray(m.checks)||m.checks.length===0){r={status:"error",message:m.error||"Check failed \u2014 click to retry"},M(t,r),c.warn("[stc] dry_run returned no checks:",m);return}let h=m.pipeline_name;if(h&&h!=="Transaction Coordination"){c.info(`[stc] dry_run reports pipeline="${h}" (not TC) \u2014 retracting button`),document.getElementById(G)?.remove(),document.getElementById(q)?.remove(),F(),Z();return}let _=m.checks;if(r.checks&&r.checks.length===m.checks.length){let g=new Set;_=m.checks.map((v,f)=>r.checks[f]?.filled&&!v.filled?n.has(v.label)?v:(g.add(v.label),{...v,filled:!0}):v),n=g}else n=new Set;r={status:_.every(g=>g.filled)?"ready":"incomplete",checks:_,previouslySentAt:m.previously_sent_at??null},M(t,r),document.getElementById(ue)&&me(_,m.previously_sent_at,t)},i=async()=>{let m=!!r.previouslySentAt;r={status:"sending",checks:r.checks,previouslySentAt:r.previouslySentAt},M(t,r),F();try{let h=await Yo(o,e,m);if(h.needs_confirm){r={status:"ready",checks:h.checks,previouslySentAt:h.previously_sent_at},M(t,r),h.checks&&me(h.checks,h.previously_sent_at,t);return}if(h.ok){r={status:"sent",checks:h.checks,previouslySentAt:new Date().toISOString()},M(t,r),window.setTimeout(()=>{a()},3500);return}r={status:"error",checks:h.checks,message:h.error||"Validation failed"},M(t,r),h.checks&&me(h.checks,h.previously_sent_at,t)}catch(h){c.error("[stc] send failed:",h),r={status:"error",message:"Network error \u2014 click to retry"},M(t,r)}};t.addEventListener("click",()=>{switch(r.status){case"ready":i();break;case"incomplete":{document.getElementById(ue)?F():r.checks&&me(r.checks,r.previouslySentAt,t);break}case"error":case"sent":a();break}}),a();let s,d=Date.now(),b=1e4,p=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(p){let m=new MutationObserver(()=>{if(s!==void 0)return;let _=Date.now()-d,y=Math.max(1500,b-_);s=window.setTimeout(()=>{s=void 0,r.status!=="sending"&&(d=Date.now(),a())},y)});m.observe(p,{childList:!0,subtree:!0,characterData:!0}),t.dataset.observerActive="1";let h=window.setInterval(()=>{document.body.contains(t)||(m.disconnect(),s!==void 0&&window.clearTimeout(s),window.clearInterval(h))},5e3)}return t},Zo=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],er=async()=>{for(let o of Zo)try{let e=await C(o,{timeoutMs:1500});if(e)return c.info(`[send-to-title-co] mount target: ${o}`),e}catch{}return c.warn(`[send-to-title-co] no mount target found. URL: ${window.location.pathname}`),null},_e=!1,Ve=async()=>{if(_e){c.info("[stc] mount:00-LOCKED ensureMounted already in-flight \u2014 skip");return}_e=!0;try{await tr()}catch(o){c.error("[stc] ensureMountedImpl threw:",o)}finally{_e=!1}},tr=async()=>{c.info("[stc] mount:01 ensureMountedImpl entered");let o=Ge();if(!o||!o.locationId){c.info(`[stc] mount:02-BAIL no ctx or no locationId. ctx=${JSON.stringify(o)}`),document.getElementById(G)?.remove(),document.getElementById(q)?.remove(),F();return}c.info(`[stc] mount:02 ctx ok \u2014 oppId=${o.oppId} locId=${o.locationId}`);let e=document.getElementById(G);if(e&&e.dataset.oppId===o.oppId){c.info("[stc] mount:03-SKIP existing button for same opp");let i=R();i!==null&&i!=="Transaction Coordination"&&(e.remove(),document.getElementById(q)?.remove(),F(),Z());return}e&&(c.info("[stc] mount:03 removing stale existing button"),e.remove(),document.getElementById(q)?.remove(),F()),c.info("[stc] mount:04 calling findMountTarget");let t=await er();if(!t){c.info("[stc] mount:04-BAIL findMountTarget returned null");return}c.info(`[stc] mount:04 target found tag=${t.tagName} class="${t.className}"`);let r=Ge();if(!r||r.oppId!==o.oppId||!r.locationId){c.info(`[stc] mount:06-BAIL ctx changed during awaits. was=${o.oppId} now=${r?.oppId}`);return}c.info("[stc] mount:06 building button (pipeline gate will run via dry_run)");let n=Qo(o.oppId,r.locationId),a=document.createElement("span");a.id=q,a.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
