...
`;var x="ws-crm-claim-release-pill",N=e=>{let o=Array.from(e.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return o?o.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},ae=({contactId:e,mode:o})=>{let t=document.createElement("button");t.id=x,t.type="button",t.dataset.contactId=e,t.dataset.mode=o;let r=o==="claim"?"emerald":"amber",a=o==="claim"?s.emerald:s.amber;return t.style.cssText=O(r),t.innerHTML=`
  ...
  `,t.title=o==="claim"?"Take ownership of this lead":"Return this lead to the pool",t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-1px)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",()=>{i.info(`Claim/Release pill clicked for contact ${e}: ${o}`);let n=t.lastElementChild,d=n?.textContent??"";n&&(n.textContent="Working\u2026"),t.style.opacity="0.7",t.disabled=!0,setTimeout(()=>{n&&(n.textContent=d),t.style.opacity="1",t.disabled=!1},700)}),t},ne=async(e,o=1e4)=>{let t=Date.now();for(;Date.now()-t<o;){let r=N(e);if(r)return r;await new Promise(a=>setTimeout(a,150))}return null},H=(e,o)=>{let t=ae({contactId:o,mode:"claim"}),r=document.createElement("span");r.id=`${x}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(t),e.appendChild(r)},se=async()=>{let e=m();if(!e){document.getElementById(x)?.remove();return}let o=document.getElementById(x);if(!(o&&o.dataset.contactId===e.contactId)){o&&o.remove();try{let t=await v("#record-details-lhs",{timeoutMs:1e4}),r=await ne(t),a=m();if(!a||a.contactId!==e.contactId)return;if(!r){i.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}H(r,e.contactId),i.debug(`Mounted Claim/Release pill next to name for ${e.contactId}`);let n=null;n=new MutationObserver(async()=>{if(!(m()?.contactId===e.contactId)){n?.disconnect();return}if(document.getElementById(x))return;let l=N(t);l&&(H(l,e.contactId),i.debug("Re-mounted Claim/Release pill after React wipe"))}),n.observe(t,{childList:!0,subtree:!0})}catch(t){i.warn("Could not mount Claim/Release pill:",t)}}},P=()=>{se()};var ie="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set",le=()=>{let e=window.WS_CRM_USER_ID;if(e&&/^[A-Za-z0-9]{15,30}$/.test(e))return e;let o=window,t=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let r of t)if(typeof r=="string"&&/^[A-Za-z0-9]{15,30}$/.test(r))return r;try{let r=localStorage.getItem("user");if(r){let a=JSON.parse(r),n=a.id??a._id;if(n&&/^[A-Za-z0-9]{15,30}$/.test(n))return n}for(let a of["tokenUser","currentUser","ghl_user"]){let n=localStorage.getItem(a);if(n)try{let d=JSON.parse(n),l=d.id??d._id;if(l&&/^[A-Za-z0-9]{15,30}$/.test(l))return l}catch{}}}catch{}return null},ce=async({contactId:e,locationId:o,status:t,userId:r})=>{let n=await fetch(ie,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:e,location:{id:o},customData:{call_status:t,triggered_by_user_id:r??""}}),credentials:"omit"});if(!n.ok)throw new Error(`Webhook returned ${n.status} ${n.statusText}`)},T="ws-crm-contact-actions-bar",de="ws-crm-call-status-dropdown",G="ws-crm-offer-made-modal",pe=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],ue=[".central-panel","[class*='central-panel']"],me=async()=>{for(let e of ue)try{let o=await v(e,{timeoutMs:3e3});if(o)return o}catch{}try{return(await v("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},be=e=>{let o=document.createElement("div");o.id=de,o.style.cssText="position: relative; flex-shrink: 0;";let t=document.createElement("button");t.type="button",t.style.cssText=A,t.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,pe.forEach(a=>{let n=document.createElement("button");n.type="button",n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
    ...
    `,n.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,n.addEventListener("mouseenter",()=>{n.style.background=s.slate}),n.addEventListener("mouseleave",()=>{n.style.background="transparent"}),n.addEventListener("click",async()=>{i.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${e}`);let d=t.firstElementChild,l=t.children[1],c=t.children[2];d&&(d.style.background=D[a.color]),l&&(l.textContent=a.label),r.style.display="none";let g=m()?.locationId,f=le();if(!g){i.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),l&&(l.textContent=`${a.label} (no tenant!)`),l&&(l.style.color=s.amber);return}if(a.color==="green"&&!f){i.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),l&&(l.textContent=`${a.label} (no user!)`),l&&(l.style.color=s.amber);return}let h=c?.textContent??"\u25BE";c&&(c.textContent="\u2026"),t.style.opacity="0.75";try{await ce({contactId:e,locationId:g,status:a.label,userId:f}),c&&(c.textContent="\u2713"),setTimeout(()=>{c&&(c.textContent=h),t.style.opacity="1"},900)}catch(Q){i.warn("Call Status webhook failed:",Q),c&&(c.textContent="\u2717"),l&&(l.style.color=s.amber),t.style.opacity="1",setTimeout(()=>{c&&(c.textContent=h),l&&(l.style.color=s.bone)},1800)}}),r.appendChild(n)}),t.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{o.contains(a.target)||(r.style.display="none")}),o.appendChild(t),o.appendChild(r),o},he=e=>{document.getElementById(G)?.remove();let o=document.createElement("div");o.id=G,o.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,o.appendChild(t),document.body.appendChild(o);let n=()=>o.remove();o.addEventListener("click",d=>{d.target===o&&n()}),t.querySelector("#ws-offer-cancel")?.addEventListener("click",n),t.querySelector("#ws-offer-submit")?.addEventListener("click",()=>{let d=(t.querySelector("#ws-offer-amount")?.value??"").trim(),l=t.querySelector("#ws-offer-expires")?.value??"";if(!d||Number(d)<=0){i.warn("Offer Made submit blocked \u2014 invalid amount");return}i.info(`Offer Made recorded for contact ${e}: $${d}, expires ${l}`),n()}),setTimeout(()=>{t.querySelector("#ws-offer-amount")?.focus()},0)},z=e=>{let o=document.createElement("div");o.id=T,o.dataset.contactId=e,o.style.cssText=`
    ...
    flex-shrink: 0;
  `,o.appendChild(be(e));let t=document.createElement("button");return t.type="button",t.style.cssText=$,t.textContent="Offer Made",t.addEventListener("click",()=>he(e)),o.appendChild(t),o},ge=async()=>{let e=m();if(!e){document.getElementById(T)?.remove();return}let o=document.getElementById(T);if(o&&o.dataset.contactId===e.contactId)return;o&&o.remove();let t=await me();if(!t){i.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let r=m();if(!r||r.contactId!==e.contactId)return;let a=z(e.contactId);a.dataset.mountTarget=t.className||"central-panel",t.insertBefore(a,t.firstChild),i.debug(`Mounted contact actions bar for ${e.contactId}`);let n=null;n=new MutationObserver(()=>{let d=document.getElementById(T);if(!(m()?.contactId===e.contactId)){n?.disconnect();return}if(!d){let c=z(e.contactId);c.dataset.mountTarget=a.dataset.mountTarget??"",t.insertBefore(c,t.firstChild),i.debug(`Re-mounted actions bar after React wipe for ${e.contactId}`)}}),n.observe(t,{childList:!0})},U=()=>{ge()};var W="reos-theme-stylesheet",V="reos-theme-fonts",fe=()=>{if(document.getElementById(V))return;let e=document.createElement("link");e.rel="preconnect",e.href="https://fonts.googleapis.com",document.head.appendChild(e);let o=document.createElement("link");o.rel="preconnect",o.href="https://fonts.gstatic.com",o.crossOrigin="anonymous",document.head.appendChild(o);let t=document.createElement("link");t.id=V,t.rel="stylesheet",t.href="https://fonts.googleapis.com/css2?family=Geist:wght@400;500;600;700&family=Geist+Mono:wght@400;500;600&display=swap",document.head.appendChild(t)},ye=`
/* ============================================
