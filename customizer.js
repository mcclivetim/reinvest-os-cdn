...
`;var I="ws-crm-claim-release-pill",tt=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},It=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?c.emerald:c.amber;return e.style.cssText=Z(r),e.innerHTML=`
  ...
  `,Pt.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
    ...
    `,s.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=c.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{p.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let l=e.firstElementChild,n=e.children[1],i=e.children[2];l&&(l.style.background=W[a.color]),n&&(n.textContent=a.label),r.style.display="none";let h=_()?.locationId,b=Rt();if(!h){p.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=c.amber);return}if(a.color==="green"&&!b){p.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=c.amber);return}let g=i?.textContent??"\u25BE",x=async v=>{i&&(i.textContent="\u2026"),e.style.opacity="0.75";try{await Bt({contactId:o,locationId:h,status:a.label,userId:b,deadReason:v}),i&&(i.textContent="\u2713"),setTimeout(()=>{i&&(i.textContent=g),e.style.opacity="1"},900)}catch(m){p.warn("Call Status webhook failed:",m),i&&(i.textContent="\u2717"),n&&(n.style.color=c.amber),e.style.opacity="1",setTimeout(()=>{i&&(i.textContent=g),n&&(n.style.color=c.bone)},1800)}};if(a.value==="dead-deal"){let v=n?.textContent??"";qt(m=>{x(m)},()=>{n&&(n.textContent=v)});return}if(a.value==="appointment-booked"){let v=n?.textContent??"";try{(await Ot({contactId:o,tenantId:h})).ok?await x():Wt(()=>{x()},()=>{n&&(n.textContent=v)})}catch(m){p.warn("Appointment sanity check failed; firing anyway:",m),await x()}return}await x()}),r.appendChild(s)}),e.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{t.contains(a.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},zt=o=>{document.getElementById(ot)?.remove();let t=document.createElement("div");t.id=ot,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,e.appendChild(r),document.body.appendChild(e);let a=()=>e.remove(),s=()=>{a(),t()};e.addEventListener("click",n=>{n.target===e&&s()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",s),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{a(),o()});let l=n=>{n.key==="Escape"&&(document.removeEventListener("keydown",l),s())};document.addEventListener("keydown",l)},nt=o=>{let t=document.createElement("div");t.id=O,t.dataset.contactId=o,t.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
...
html body .custom-scrollbar[class*="custom-scrollbar"]::-webkit-scrollbar,
html body [class*="custom-scrollbar"][class~="custom-scrollbar"]::-webkit-scrollbar {
  width: 10px !important;
  height: 10px !important;
}
html body .custom-scrollbar[class*="custom-scrollbar"]::-webkit-scrollbar-track,
html body [class*="custom-scrollbar"][class~="custom-scrollbar"]::-webkit-scrollbar-track {
  background: var(--reos-obsidian) !important;
}
html body .custom-scrollbar[class*="custom-scrollbar"]::-webkit-scrollbar-thumb,
html body [class*="custom-scrollbar"][class~="custom-scrollbar"]::-webkit-scrollbar-thumb {
  background: var(--reos-steel) !important;
  border-radius: 4px !important;
}
html body .custom-scrollbar[class*="custom-scrollbar"]::-webkit-scrollbar-thumb:hover,
html body [class*="custom-scrollbar"][class~="custom-scrollbar"]::-webkit-scrollbar-thumb:hover {
  background: var(--reos-cool-gray) !important;
}
html body .custom-scrollbar[class*="custom-scrollbar"]::-webkit-scrollbar-corner,
html body [class*="custom-scrollbar"][class~="custom-scrollbar"]::-webkit-scrollbar-corner {
  background: var(--reos-obsidian) !important;
}
