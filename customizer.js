...
`;var Ft=/^[A-Za-z0-9]{15,30}$/,q=o=>typeof o=="string"&&Ft.test(o),Dt=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},Mt=()=>{let o=window.WS_CRM_USER_ID;return q(o)?o:null},Ht=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(q(t))return t;return null},Nt=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=Dt(e);if(!t)return null;let r=t.uid;if(q(r))return r;let n=t.claims;if(n&&q(n.user_id))return n.user_id}catch{}return null},Pt=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(q(n))return n}catch{}}}catch{}return null},A=()=>Mt()||Ht()||Nt()||Pt()||null;var Re="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",Ot=`${Re}/active-follow-up-get`,Rt=`${Re}/active-follow-up-set`,P="ws-crm-active-follow-up-button",Se={amber:{label:"Add to Active Follow Up",bg:c.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:c.amber,clickable:!0,dotColor:c.amber},blue:{label:"In Active Follow Up",bg:c.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:c.blue,clickable:!1,dotColor:c.blue},gray:{label:"Not in Follow Up",bg:c.slate,border:c.steel,color:c.coolGray,clickable:!1,dotColor:c.coolGray},loading:{label:"Active Follow Up\u2026",bg:c.slate,border:c.steel,color:c.ash,clickable:!1,dotColor:c.ash},error:{label:"Follow Up status unavailable",bg:c.slate,border:c.steel,color:c.ash,clickable:!1,dotColor:c.crimson}},Bt=()=>{let o=T();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},Gt=async o=>{let e=await fetch(Ot,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},$t=async(o,e)=>{let t=await fetch(Rt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},O=(o,e)=>{let t=Se[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},Ne=o=>{let e=document.createElement("button");return e.type="button",e.id=P,e.dataset.contactId=o,e.style.cssText=`
  ...
  `,t.forEach(l=>{let d=document.createElement("button");d.type="button",d.style.cssText=`
    ...
    `,d.innerHTML=`
      ...
      "></span>
      <span>${l.label}</span>
    `,d.addEventListener("mouseenter",()=>{d.style.background=c.slate}),d.addEventListener("mouseleave",()=>{d.style.background="transparent"}),d.addEventListener("click",async()=>{b.info(`Call Status set to "${l.label}" (${l.value}, ${l.color}) for contact ${o}`);let h=i.firstElementChild,m=i.children[1],p=i.children[2];h&&(h.style.background=Te[l.color]),m&&(m.textContent=l.label),s.style.display="none";let u=T()?.locationId,f=A();if(!u){b.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),m&&(m.textContent=`${l.label} (no tenant!)`),m&&(m.style.color=c.amber);return}if(e==="seller"&&l.color==="green"&&!f){b.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),m&&(m.textContent=`${l.label} (no user!)`),m&&(m.style.color=c.amber);return}let g=p?.textContent??"\u25BE",w=async v=>{p&&(p.textContent="\u2026"),i.style.opacity="0.75";try{await ao({contactId:o,locationId:u,status:l.label,userId:f,deadReason:v,webhookUrl:r,kind:e}),p&&(p.textContent="\u2713"),setTimeout(()=>{p&&(p.textContent=g),i.style.opacity="1"},900)}catch(x){b.warn("Call Status webhook failed:",x),p&&(p.textContent="\u2717"),m&&(m.style.color=c.amber),i.style.opacity="1",setTimeout(()=>{p&&(p.textContent=g),m&&(m.style.color=c.bone)},1800)}};if(e==="seller"&&l.value==="dead-deal"){let v=m?.textContent??"";mo(x=>{w(x)},()=>{m&&(m.textContent=v)});return}if(e==="seller"&&l.value==="appointment-booked"){let v=m?.textContent??"";try{(await so({contactId:o,tenantId:u})).ok?await w():_o(()=>{w()},()=>{m&&(m.textContent=v)})}catch(x){b.warn("Appointment sanity check failed; firing anyway:",x),await w()}return}await w()}),s.appendChild(d)}),i.addEventListener("click",l=>{l.stopPropagation(),s.style.display=s.style.display==="none"?"block":"none"}),document.addEventListener("click",l=>{a.contains(l.target)||(s.style.display="none")}),a.appendChild(i),a.appendChild(s),a},po=o=>{document.getElementById(We)?.remove();let e=document.createElement("div");e.id=We,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,r.appendChild(l);let d=["TC","ACQ","MT","DISPO","Property"],h={};o.forEach(_=>{h[_.group]||(h[_.group]=[]),h[_.group].push(_)});let m=[...d.filter(_=>h[_]),...Object.keys(h).filter(_=>!d.includes(_))];for(let _ of m){let u=h[_],f=document.createElement("div");f.style.cssText="margin-top: 12px;";let g=document.createElement("div");g.style.cssText=`
      font-family: ${y.mono};
      font-size: 9px;
      ...
      padding-bottom: 4px;
      border-bottom: 1px solid ${c.steel};
    `,g.textContent=_,f.appendChild(g),u.forEach(w=>{let v=document.createElement("div"),x=!w.filled&&w.navigable&&w.field_id;v.style.cssText=`
        display: flex;
        align-items: center;
        gap: 10px;
        padding: 6px 8px;
        ...
        ${x?"cursor: pointer;":""}
        transition: background 0.1s ease;
      `,x&&(v.addEventListener("mouseenter",()=>{v.style.background=c.slate}),v.addEventListener("mouseleave",()=>{v.style.background="transparent"}),v.addEventListener("click",L=>{L.stopPropagation(),w.field_id&&(Ho(w.field_id),F())}));let S=w.filled?`<span style="color: ${c.emerald}; font-size: 14px; line-height: 1; flex-shrink: 0;">\u2713</span>`:`<span style="color: ${c.amber}; font-size: 14px; line-height: 1; flex-shrink: 0;">\u25CB</span>`,k=x?`<span style="margin-left: auto; color: ${c.ash}; font-size: 11px;">\u2192</span>`:"";v.innerHTML=`${S}<span style="flex: 1;">${w.label}</span>${k}`,f.appendChild(v)}),r.appendChild(f)}document.body.appendChild(r);let p=_=>{let u=_.target;!r.contains(u)&&!t.contains(u)&&(F(),document.removeEventListener("click",p))};window.setTimeout(()=>document.addEventListener("click",p),0)},M=(o,e)=>{let t=o.querySelector(".ws-stc-label"),r=o.querySelector(".ws-stc-dot"),n=o.querySelector(".ws-stc-chevron"),a,i,s=c.bone,l,d="Send to Title Company",h="",m="pointer",p="1";switch(e.status){case"checking":a="transparent",i=c.steel,s=c.ash,l=c.coolGray,d="Checking\u2026",m="wait",p="0.8";break;case"ready":a=c.emerald,i=c.emerald,s="#0a0e14",l="#0a0e14",d=e.previouslySentAt?"Re-send to Title Co":"Send to Title Company";break;case"incomplete":{let _=e.checks?.filter(f=>f.filled).length??0,u=e.checks?.length??0;a="transparent",i=c.amber,s=c.amber,l=c.amber,d=`${_} of ${u} fields ready`,h="\u25BE";break}case"sending":a=c.emerald,i=c.emerald,s="#0a0e14",l="#0a0e14",d="Sending\u2026",m="wait",p="0.8";break;case"sent":a=c.emerald,i=c.emerald,s="#0a0e14",l="#0a0e14",d="Sent \u2713";break;case"error":a="transparent",i=c.amber,s=c.amber,l=c.amber,d=e.message??"Try again";break}o.style.background=a,o.style.borderColor=i,o.style.color=s,o.style.cursor=m,o.style.opacity=p,r&&(r.style.background=l,r.style.boxShadow=e.status==="ready"||e.status==="sent"?`0 0 6px ${l}`:""),t&&(t.textContent=d),n&&(n.textContent=h)},at=(o,e)=>{let t=document.createElement("button");t.id=Y,t.type="button",t.dataset.oppId=o;let r={status:"checking"};t.style.cssText=`
    ${N}
    background: transparent;
    border: 1px solid ${c.steel};
    color: ${c.ash};
  ...
  `,M(t,r);let n=async()=>{if(r.status==="sending")return;let l=await Do(o,e);if(!l){r={status:"error",message:"Check failed \u2014 click to retry"},M(t,r);return}r={status:l.ok?"ready":"incomplete",checks:l.checks,previouslySentAt:l.previously_sent_at??null},M(t,r),document.getElementById(be)&&l.checks&&ce(l.checks,l.previously_sent_at,t)},a=async()=>{let l=!!r.previouslySentAt;r={status:"sending",checks:r.checks,previouslySentAt:r.previouslySentAt},M(t,r),F();try{let d=await Mo(o,e,l);if(d.needs_confirm){r={status:"ready",checks:d.checks,previouslySentAt:d.previously_sent_at},M(t,r),d.checks&&ce(d.checks,d.previously_sent_at,t);return}if(d.ok){r={status:"sent",checks:d.checks,previouslySentAt:new Date().toISOString()},M(t,r),window.setTimeout(()=>{n()},3500);return}r={status:"error",checks:d.checks,message:d.error||"Validation failed"},M(t,r),d.checks&&ce(d.checks,d.previously_sent_at,t)}catch(d){b.error("[stc] send failed:",d),r={status:"error",message:"Network error \u2014 click to retry"},M(t,r)}};t.addEventListener("click",()=>{switch(r.status){case"ready":a();break;case"incomplete":{document.getElementById(be)?F():r.checks&&ce(r.checks,r.previouslySentAt,t);break}case"error":case"sent":n();break}}),n();let i,s=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(s){let l=new MutationObserver(()=>{i!==void 0&&window.clearTimeout(i),i=window.setTimeout(()=>{r.status!=="sending"&&n()},1500)});l.observe(s,{childList:!0,subtree:!0,characterData:!0}),t.dataset.observerActive="1";let d=window.setInterval(()=>{document.body.contains(t)||(l.disconnect(),i!==void 0&&window.clearTimeout(i),window.clearInterval(d))},5e3)}return t},No=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],Po=async()=>{for(let o of No)try{let e=await C(o,{timeoutMs:1500});if(e)return b.info(`[send-to-title-co] mount target: ${o}`),e}catch{}return b.warn(`[send-to-title-co] no mount target found. URL: ${window.location.pathname}`),null},Oo=async()=>{let o=Le();if(!o||!o.locationId){document.getElementById(Y)?.remove(),document.getElementById(K)?.remove(),F();return}let e=document.getElementById(Y);if(e&&e.dataset.oppId===o.oppId){let l=R();l!==null&&l!=="Transaction Coordination"&&(e.remove(),document.getElementById(K)?.remove(),F());return}e&&(e.remove(),document.getElementById(K)?.remove(),F());let t=await Po();if(!t)return;let r=await le();if(r!=="Transaction Coordination"){b.info(`[send-to-title-co] gated off \u2014 pipeline="${r}" (expected Transaction Coordination)`);return}let n=Le();if(!n||n.oppId!==o.oppId||!n.locationId)return;let a=at(o.oppId,n.locationId),i=document.createElement("span");i.id=K,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),i.appendChild(a),t.appendChild(i),b.debug(`Mounted Send to Title Co button on opp ${o.oppId}`);let s=new MutationObserver(()=>{if(!(Le()?.oppId===o.oppId)){s.disconnect();return}let d=R();if(d!==null&&d!=="Transaction Coordination"){document.getElementById(Y)?.remove(),document.getElementById(K)?.remove(),F(),s.disconnect();return}if(!document.getElementById(Y)){let h=at(o.oppId,o.locationId);i.appendChild(h)}});s.observe(t,{childList:!0})},it=()=>{Oo()};var ue="reos-theme-stylesheet",kt="reos-theme-fonts",Ro=()=>{let o=document.getElementById(kt);o&&o.remove()},Bo=`
/* ============================================
