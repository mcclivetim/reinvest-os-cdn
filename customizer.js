...
`,L=`
  ...
  flex-shrink: 0;
`;var I="ws-crm-claim-release-pill",re=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Ne=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",n=t==="claim"?l.emerald:l.amber;return e.style.cssText=te(r),e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,e.title=t==="claim"?"Take ownership of this lead":"Return this lead to the pool",e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{h.info(`Claim/Release pill clicked for contact ${o}: ${t}`);let s=e.lastElementChild,i=s?.textContent??"";s&&(s.textContent="Working\u2026"),e.style.opacity="0.7",e.disabled=!0,setTimeout(()=>{s&&(s.textContent=i),e.style.opacity="1",e.disabled=!1},700)}),e},He=async(o,t=1e4)=>{let e=Date.now();for(;Date.now()-e<t;){let r=re(o);if(r)return r;await new Promise(n=>setTimeout(n,150))}return null},oe=(o,t)=>{let e=Ne({contactId:t,mode:"claim"}),r=document.createElement("span");r.id=`${I}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(e),o.appendChild(r)},Me=async()=>{let o=E();if(!o){document.getElementById(I)?.remove();return}let t=document.getElementById(I);if(!(t&&t.dataset.contactId===o.contactId)){t&&t.remove();try{let e=await C("#record-details-lhs",{timeoutMs:1e4}),r=await He(e),n=E();if(!n||n.contactId!==o.contactId)return;if(!r){h.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}oe(r,o.contactId),h.debug(`Mounted Claim/Release pill next to name for ${o.contactId}`);let s=null;s=new MutationObserver(async()=>{if(!(E()?.contactId===o.contactId)){s?.disconnect();return}if(document.getElementById(I))return;let a=re(e);a&&(oe(a,o.contactId),h.debug("Re-mounted Claim/Release pill after React wipe"))}),s.observe(e,{childList:!0,subtree:!0})}catch(e){h.warn("Could not mount Claim/Release pill:",e)}}},ne=()=>{Me()};var Pe="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set";var Re="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Oe=()=>{let o=window.WS_CRM_USER_ID;if(o&&/^[A-Za-z0-9]{15,30}$/.test(o))return o;let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let r of e)if(typeof r=="string"&&/^[A-Za-z0-9]{15,30}$/.test(r))return r;try{let r=localStorage.getItem("user");if(r){let n=JSON.parse(r),s=n.id??n._id;if(s&&/^[A-Za-z0-9]{15,30}$/.test(s))return s}for(let n of["tokenUser","currentUser","ghl_user"]){let s=localStorage.getItem(n);if(s)try{let i=JSON.parse(s),a=i.id??i._id;if(a&&/^[A-Za-z0-9]{15,30}$/.test(a))return a}catch{}}}catch{}return null},Be=async({contactId:o,locationId:t,status:e,userId:r,deadReason:n})=>{let s={call_status:e,triggered_by_user_id:r??""};n&&(s.dead_reason=n);let a=await fetch(Pe,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:o,location:{id:t},customData:s}),credentials:"omit"});if(!a.ok)throw new Error(`Webhook returned ${a.status} ${a.statusText}`)},Ge=async({contactId:o,tenantId:t})=>{let e=await fetch(Re,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t,contact_id:o}),credentials:"omit"});if(!e.ok)throw new Error(`Sanity check returned ${e.status} ${e.statusText}`);return await e.json()};var O="ws-crm-contact-actions-bar",Ve="ws-crm-call-status-dropdown",ae="ws-crm-offer-made-modal",$e=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],Ue=[".central-panel","[class*='central-panel']"],ze=async()=>{for(let o of Ue)try{let t=await C(o,{timeoutMs:3e3});if(t)return t}catch{}try{return(await C("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},qe=o=>{let t=document.createElement("div");t.id=Ve,t.style.cssText="position: relative; flex-shrink: 0;";let e=document.createElement("button");e.type="button",e.style.cssText=L,e.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `,$e.forEach(n=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
    ...
    `,s.innerHTML=`
      <span style="
        ...
        height: 8px;
        border-radius: 50%;
        background: ${K[n.color]};
        flex-shrink: 0;
      "></span>
      <span>${n.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=l.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{h.info(`Call Status set to "${n.label}" (${n.value}, ${n.color}) for contact ${o}`);let i=e.firstElementChild,a=e.children[1],d=e.children[2];i&&(i.style.background=K[n.color]),a&&(a.textContent=n.label),r.style.display="none";let p=E()?.locationId,m=Oe();if(!p){h.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),a&&(a.textContent=`${n.label} (no tenant!)`),a&&(a.style.color=l.amber);return}if(n.color==="green"&&!m){h.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),a&&(a.textContent=`${n.label} (no user!)`),a&&(a.style.color=l.amber);return}let c=d?.textContent??"\u25BE",u=async v=>{d&&(d.textContent="\u2026"),e.style.opacity="0.75";try{await Be({contactId:o,locationId:p,status:n.label,userId:m,deadReason:v}),d&&(d.textContent="\u2713"),setTimeout(()=>{d&&(d.textContent=c),e.style.opacity="1"},900)}catch(_){h.warn("Call Status webhook failed:",_),d&&(d.textContent="\u2717"),a&&(a.style.color=l.amber),e.style.opacity="1",setTimeout(()=>{d&&(d.textContent=c),a&&(a.style.color=l.bone)},1800)}};if(n.value==="dead-deal"){let v=a?.textContent??"";je(_=>{u(_)},()=>{a&&(a.textContent=v)});return}if(n.value==="appointment-booked"){let v=a?.textContent??"";try{(await Ge({contactId:o,tenantId:p})).ok?await u():Ke(()=>{u()},()=>{a&&(a.textContent=v)})}catch(_){h.warn("Appointment sanity check failed; firing anyway:",_),await u()}return}await u()}),r.appendChild(s)}),e.addEventListener("click",n=>{n.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",n=>{t.contains(n.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},We=o=>{document.getElementById(ae)?.remove();let t=document.createElement("div");t.id=ae,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,e.appendChild(r),document.body.appendChild(e);let n=()=>e.remove(),s=()=>{n(),t()};e.addEventListener("click",a=>{a.target===e&&s()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",s),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),o()});let i=a=>{a.key==="Escape"&&(document.removeEventListener("keydown",i),s())};document.addEventListener("keydown",i)},le=o=>{let t=document.createElement("div");t.id=O,t.dataset.contactId=o,t.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    ...
    flex-shrink: 0;
  `,t.appendChild(qe(o));let e=document.createElement("button");return e.type="button",e.style.cssText=A,e.textContent="Offer Made",e.addEventListener("click",()=>We(o)),t.appendChild(e),t},Xe=async()=>{let o=E();if(!o){document.getElementById(O)?.remove();return}let t=document.getElementById(O);if(t&&t.dataset.contactId===o.contactId)return;t&&t.remove();let e=await ze();if(!e){h.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let r=E();if(!r||r.contactId!==o.contactId)return;let n=le(o.contactId);n.dataset.mountTarget=e.className||"central-panel",e.insertBefore(n,e.firstChild),h.debug(`Mounted contact actions bar for ${o.contactId}`);let s=null;s=new MutationObserver(()=>{let i=document.getElementById(O);if(!(E()?.contactId===o.contactId)){s?.disconnect();return}if(!i){let d=le(o.contactId);d.dataset.mountTarget=n.dataset.mountTarget??"",e.insertBefore(d,e.firstChild),h.debug(`Re-mounted actions bar after React wipe for ${o.contactId}`)}}),s.observe(e,{childList:!0})},de=()=>{Xe()};var B="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",Ye=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),D="data-ws-crm-favicon",X=o=>o.hasAttribute(D),ce=o=>{if(o.tagName!=="LINK")return!1;let t=(o.getAttribute("rel")??"").toLowerCase();return Ye.has(t)},G=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{ce(r)&&!X(r)&&r.remove()});let t=document.head.querySelector(`link[${D}="1"]`);t?t.href!==B&&(t.href=B):(t=document.createElement("link"),t.setAttribute(D,"1"),t.rel="icon",t.type="image/png",t.href=B,document.head.appendChild(t),h.debug("Favicon installed"));let e=document.head.querySelector(`link[${D}="apple"]`);e||(e=document.createElement("link"),e.setAttribute(D,"apple"),e.rel="apple-touch-icon",e.href=B,document.head.appendChild(e))},be=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)G();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),G())},50)}new MutationObserver(r=>{let n=!1;for(let s of r)s.addedNodes.forEach(i=>{i instanceof HTMLLinkElement&&ce(i)&&!X(i)&&(n=!0)}),s.removedNodes.forEach(i=>{i instanceof HTMLLinkElement&&X(i)&&(n=!0)});n&&G()}).observe(document.head,{childList:!0,subtree:!1});let t=0,e=window.setInterval(()=>{t+=1,G(),t>=5&&window.clearInterval(e)},1e3)};var q="reos-theme-stylesheet",Ee="reos-theme-fonts",Je=()=>{let o=document.getElementById(Ee);o&&o.remove()},Ze=`
/* ============================================
...
/* 18a-hr-date-panel (v0.9.82). Date-range picker popover (Invoices
   page Start Date / End Date) ships with --n-panel-color: #fff,
   gray-700 text, brand-blue active dates, light-gray dividers.
   Override every --n-* var to canon: graphite panel, steel border,
   bone text, ash weekdays/excluded, emerald active + emerald-glow
   range, slate hover. */
html body .hr-date-panel:not(#__reos_never_id),
html body [class*="hr-date-panel"]:not(#__reos_never_id) {
  --n-panel-color: var(--reos-graphite) !important;
  ...
  --n-calendar-title-color-hover: var(--reos-slate) !important;
  ...
  --n-item-color-disabled: transparent !important;
  --n-item-color-hover: var(--reos-slate) !important;
  --n-item-color-active: var(--reos-emerald) !important;
  --n-item-text-color-disabled: var(--reos-ash) !important;
...
/* Disabled (out-of-range) cells \u2014 ash readable, not invisible.
   v0.9.83: removed opacity 0.6 from v0.9.82 \u2014 combined with
   cool-gray it dropped contrast to nearly zero, hiding most of
   the calendar grid (May 21-31, Jun, etc.). */
...
/* Combined disabled+excluded (e.g. other-month + out-of-range)
   \u2014 even more muted: cool-gray full opacity. */
html body .hr-date-panel:not(#__reos_never_id) .hr-date-panel-date--excluded.hr-date-panel-date--disabled,
html body .hr-date-panel:not(#__reos_never_id) .hr-date-panel-date--excluded.hr-date-panel-date--disabled * {
  color: var(--reos-cool-gray) !important;
  -webkit-text-fill-color: var(--reos-cool-gray) !important;
}
/* Trigger overlay \u2014 explicit transparent at rest. Naive UI paints
   a ::before pseudo on this element with var(--n-item-color)
   defaulting to white; force the var to transparent + paint the
   pseudo transparent so the date number text behind shows. */
html body .hr-date-panel:not(#__reos_never_id) {
  --n-item-color: transparent !important;
}
...
/* Hover state on day cell trigger \u2014 slate via the pseudo and the
   element itself (cover both paint mechanisms Naive may use). */
html body .hr-date-panel:not(#__reos_never_id) .hr-date-panel-date:not(.hr-date-panel-date--disabled):hover .hr-date-panel-date__trigger,
html body .hr-date-panel:not(#__reos_never_id) .hr-date-panel-date:not(.hr-date-panel-date--disabled):hover .hr-date-panel-date__trigger::before {
  background-color: var(--reos-slate) !important;
  background: var(--reos-slate) !important;
}
/* Current / selected day \u2014 emerald bg + obsidian text. Cover
   both the trigger element and its ::before pseudo. */
html body .hr-date-panel:not(#__reos_never_id) .hr-date-panel-date--current .hr-date-panel-date__trigger,
html body .hr-date-panel:not(#__reos_never_id) .hr-date-panel-date--current .hr-date-panel-date__trigger::before,
html body .hr-date-panel:not(#__reos_never_id) .hr-date-panel-date--selected .hr-date-panel-date__trigger,
html body .hr-date-panel:not(#__reos_never_id) .hr-date-panel-date--selected .hr-date-panel-date__trigger::before {
  background-color: var(--reos-emerald) !important;
  background: var(--reos-emerald) !important;
  color: var(--reos-obsidian) !important;
}
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var Ce=()=>{h.info(`Loaded v${P} (built ${R})`),be(),Te(),ee(),Q(o=>{h.debug("Page handler firing for:",o),Fe(),ne(),de()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ce):Ce();})();
