...
`;var st=/^[A-Za-z0-9]{15,30}$/,R=o=>typeof o=="string"&&st.test(o),it=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},lt=()=>{let o=window.WS_CRM_USER_ID;return R(o)?o:null},dt=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(R(t))return t;return null},ct=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=it(e);if(!t)return null;let r=t.uid;if(R(r))return r;let n=t.claims;if(n&&R(n.user_id))return n.user_id}catch{}return null},bt=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(R(n))return n}catch{}}}catch{}return null},F=()=>lt()||dt()||ct()||bt()||null;var we="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",ht=`${we}/active-follow-up-get`,pt=`${we}/active-follow-up-set`,O="ws-crm-active-follow-up-button",ce={amber:{label:"Add to Active Follow Up",bg:l.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:l.amber,clickable:!0,dotColor:l.amber},blue:{label:"In Active Follow Up",bg:l.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:l.blue,clickable:!1,dotColor:l.blue},gray:{label:"Not in Follow Up",bg:l.slate,border:l.steel,color:l.coolGray,clickable:!1,dotColor:l.coolGray},loading:{label:"Active Follow Up\u2026",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.ash},error:{label:"Follow Up status unavailable",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.crimson}},mt=()=>{let o=x();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},_t=async o=>{let e=await fetch(ht,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},vt=async(o,e)=>{let t=await fetch(pt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},D=(o,e)=>{let t=ce[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},ye=o=>{let e=document.createElement("button");return e.type="button",e.id=O,e.dataset.contactId=o,e.style.cssText=`
  ...
  `,D(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=mt();if(!(!r||r.contactId!==o)){D(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await vt(r,F()),a=n&&n.state&&n.state in ce?n.state:"blue";D(e,a),h.info(`Active Follow Up set for ${o} \u2192 ${a}`)}catch(n){h.error("Active Follow Up Set webhook failed",n),D(e,"error"),setTimeout(()=>void Y(e,r),4e3)}}}),e},Y=async(o,e)=>{D(o,"loading");try{let t=await _t(e),r=t&&t.state&&t.state in ce?t.state:"gray",n=document.getElementById(O);if(!n||n.dataset.contactId!==e.contactId)return;D(n,r)}catch(t){h.warn("Active Follow Up Get webhook failed",t),D(o,"error")}},ut=[".central-panel","[class*='central-panel']"],ge="ws-crm-contact-actions-bar",yt=async()=>{for(let o of ut)try{let e=await S(o,{timeoutMs:3e3});if(e)return e}catch{}try{return(await S("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},gt=(o,e)=>{let t=e.querySelector(`#${ge}`);if(t)return t.appendChild(o),{container:t,placement:"actions-bar"};let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
  ...
  `,t.appendChild(r),document.body.appendChild(t);let n=()=>t.remove(),a=()=>{n(),e()};t.addEventListener("click",s=>{s.target===t&&a()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",a),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),o()});let i=s=>{s.key==="Escape"&&(document.removeEventListener("keydown",i),a())};document.addEventListener("keydown",i)},Ie=o=>{let e=document.createElement("div");e.id=X,e.dataset.contactId=o,e.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var tt=()=>{h.info(`Loaded v${W} (built ${j})`),He(),Ze(),ue(),ve(o=>{h.debug("Page handler firing for:",o),et(),Se(),Le(),xe(),Ne()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",tt):tt();})();
