...
`;var M="ws-crm-claim-release-pill",Q=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Ae=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=M,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?l.emerald:l.amber;return e.style.cssText=J(r),e.innerHTML=`
  ...
  `,e.title=t==="claim"?"Take ownership of this lead":"Return this lead to the pool",e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{p.info(`Claim/Release pill clicked for contact ${o}: ${t}`);let i=e.lastElementChild,s=i?.textContent??"";i&&(i.textContent="Working\u2026"),e.style.opacity="0.7",e.disabled=!0,setTimeout(()=>{i&&(i.textContent=s),e.style.opacity="1",e.disabled=!1},700)}),e},Me=async(o,t=1e4)=>{let e=Date.now();for(;Date.now()-e<t;){let r=Q(o);if(r)return r;await new Promise(a=>setTimeout(a,150))}return null},X=(o,t)=>{let e=Ae({contactId:t,mode:"claim"}),r=document.createElement("span");r.id=`${M}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(e),o.appendChild(r)},Fe=async()=>{let o=E();if(!o){document.getElementById(M)?.remove();return}let t=document.getElementById(M);if(!(t&&t.dataset.contactId===o.contactId)){t&&t.remove();try{let e=await L("#record-details-lhs",{timeoutMs:1e4}),r=await Me(e),a=E();if(!a||a.contactId!==o.contactId)return;if(!r){p.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}X(r,o.contactId),p.debug(`Mounted Claim/Release pill next to name for ${o.contactId}`);let i=null;i=new MutationObserver(async()=>{if(!(E()?.contactId===o.contactId)){i?.disconnect();return}if(document.getElementById(M))return;let n=Q(e);n&&(X(n,o.contactId),p.debug("Re-mounted Claim/Release pill after React wipe"))}),i.observe(e,{childList:!0,subtree:!0})}catch(e){p.warn("Could not mount Claim/Release pill:",e)}}},ee=()=>{Fe()};var De="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set";var $e="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Be=()=>{let o=window.WS_CRM_USER_ID;if(o&&/^[A-Za-z0-9]{15,30}$/.test(o))return o;let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let r of e)if(typeof r=="string"&&/^[A-Za-z0-9]{15,30}$/.test(r))return r;try{let r=localStorage.getItem("user");if(r){let a=JSON.parse(r),i=a.id??a._id;if(i&&/^[A-Za-z0-9]{15,30}$/.test(i))return i}for(let a of["tokenUser","currentUser","ghl_user"]){let i=localStorage.getItem(a);if(i)try{let s=JSON.parse(i),n=s.id??s._id;if(n&&/^[A-Za-z0-9]{15,30}$/.test(n))return n}catch{}}}catch{}return null},He=async({contactId:o,locationId:t,status:e,userId:r,deadReason:a})=>{let i={call_status:e,triggered_by_user_id:r??""};a&&(i.dead_reason=a);let n=await fetch(De,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:o,location:{id:t},customData:i}),credentials:"omit"});if(!n.ok)throw new Error(`Webhook returned ${n.status} ${n.statusText}`)},Re=async({contactId:o,tenantId:t})=>{let e=await fetch($e,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t,contact_id:o}),credentials:"omit"});if(!e.ok)throw new Error(`Sanity check returned ${e.status} ${e.statusText}`);return await e.json()};var N="ws-crm-contact-actions-bar",Ne="ws-crm-call-status-dropdown",te="ws-crm-offer-made-modal",Pe=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],Oe=[".central-panel","[class*='central-panel']"],Ge=async()=>{for(let o of Oe)try{let t=await L(o,{timeoutMs:3e3});if(t)return t}catch{}try{return(await L("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},ze=o=>{let t=document.createElement("div");t.id=Ne,t.style.cssText="position: relative; flex-shrink: 0;";let e=document.createElement("button");e.type="button",e.style.cssText=A,e.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,Pe.forEach(a=>{let i=document.createElement("button");i.type="button",i.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
    ...
    `,i.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,i.addEventListener("mouseenter",()=>{i.style.background=l.slate}),i.addEventListener("mouseleave",()=>{i.style.background="transparent"}),i.addEventListener("click",async()=>{p.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let s=e.firstElementChild,n=e.children[1],c=e.children[2];s&&(s.style.background=W[a.color]),n&&(n.textContent=a.label),r.style.display="none";let m=E()?.locationId,b=Be();if(!m){p.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=l.amber);return}if(a.color==="green"&&!b){p.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=l.amber);return}let v=c?.textContent??"\u25BE",w=async g=>{c&&(c.textContent="\u2026"),e.style.opacity="0.75";try{await He({contactId:o,locationId:m,status:a.label,userId:b,deadReason:g}),c&&(c.textContent="\u2713"),setTimeout(()=>{c&&(c.textContent=v),e.style.opacity="1"},900)}catch(h){p.warn("Call Status webhook failed:",h),c&&(c.textContent="\u2717"),n&&(n.style.color=l.amber),e.style.opacity="1",setTimeout(()=>{c&&(c.textContent=v),n&&(n.style.color=l.bone)},1800)}};if(a.value==="dead-deal"){let g=n?.textContent??"";qe(h=>{w(h)},()=>{n&&(n.textContent=g)});return}if(a.value==="appointment-booked"){let g=n?.textContent??"";try{(await Re({contactId:o,tenantId:m})).ok?await w():We(()=>{w()},()=>{n&&(n.textContent=g)})}catch(h){p.warn("Appointment sanity check failed; firing anyway:",h),await w()}return}await w()}),r.appendChild(i)}),e.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{t.contains(a.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},Ue=o=>{document.getElementById(te)?.remove();let t=document.createElement("div");t.id=te,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,t.appendChild(e),document.body.appendChild(t);let i=()=>t.remove();t.addEventListener("click",s=>{s.target===t&&i()}),e.querySelector("#ws-offer-cancel")?.addEventListener("click",i),e.querySelector("#ws-offer-submit")?.addEventListener("click",()=>{let s=(e.querySelector("#ws-offer-amount")?.value??"").trim(),n=e.querySelector("#ws-offer-expires")?.value??"";if(!s||Number(s)<=0){p.warn("Offer Made submit blocked \u2014 invalid amount");return}p.info(`Offer Made recorded for contact ${o}: $${s}, expires ${n}`),i()}),setTimeout(()=>{e.querySelector("#ws-offer-amount")?.focus()},0)},oe="ws-crm-dead-deal-modal",qe=(o,t)=>{document.getElementById(oe)?.remove();let e=document.createElement("div");e.id=oe,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(r),document.body.appendChild(e);let a=()=>e.remove(),i=()=>{a(),t()};e.addEventListener("click",n=>{n.target===e&&i()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",i),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{a(),o()});let s=n=>{n.key==="Escape"&&(document.removeEventListener("keydown",s),i())};document.addEventListener("keydown",s)},ae=o=>{let t=document.createElement("div");t.id=N,t.dataset.contactId=o,t.style.cssText=`
    ...
    flex-shrink: 0;
  `,t.appendChild(ze(o));let e=document.createElement("button");return e.type="button",e.style.cssText=I,e.textContent="Offer Made",e.addEventListener("click",()=>Ue(o)),t.appendChild(e),t},Ve=async()=>{let o=E();if(!o){document.getElementById(N)?.remove();return}let t=document.getElementById(N);if(t&&t.dataset.contactId===o.contactId)return;t&&t.remove();let e=await Ge();if(!e){p.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let r=E();if(!r||r.contactId!==o.contactId)return;let a=ae(o.contactId);a.dataset.mountTarget=e.className||"central-panel",e.insertBefore(a,e.firstChild),p.debug(`Mounted contact actions bar for ${o.contactId}`);let i=null;i=new MutationObserver(()=>{let s=document.getElementById(N);if(!(E()?.contactId===o.contactId)){i?.disconnect();return}if(!s){let c=ae(o.contactId);c.dataset.mountTarget=a.dataset.mountTarget??"",e.insertBefore(c,e.firstChild),p.debug(`Re-mounted actions bar after React wipe for ${o.contactId}`)}}),i.observe(e,{childList:!0})},ne=()=>{Ve()};var P="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",je=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),F="data-ws-crm-favicon",V=o=>o.hasAttribute(F),se=o=>{if(o.tagName!=="LINK")return!1;let t=(o.getAttribute("rel")??"").toLowerCase();return je.has(t)},O=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{se(r)&&!V(r)&&r.remove()});let t=document.head.querySelector(`link[${F}="1"]`);t?t.href!==P&&(t.href=P):(t=document.createElement("link"),t.setAttribute(F,"1"),t.rel="icon",t.type="image/png",t.href=P,document.head.appendChild(t),p.debug("Favicon installed"));let e=document.head.querySelector(`link[${F}="apple"]`);e||(e=document.createElement("link"),e.setAttribute(F,"apple"),e.rel="apple-touch-icon",e.href=P,document.head.appendChild(e))},ie=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)O();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),O())},50)}new MutationObserver(r=>{let a=!1;for(let i of r)i.addedNodes.forEach(s=>{s instanceof HTMLLinkElement&&se(s)&&!V(s)&&(a=!0)}),i.removedNodes.forEach(s=>{s instanceof HTMLLinkElement&&V(s)&&(a=!0)});a&&O()}).observe(document.head,{childList:!0,subtree:!1});let t=0,e=window.setInterval(()=>{t+=1,O(),t>=5&&window.clearInterval(e)},1e3)};var le="reos-theme-stylesheet",ce="reos-theme-fonts",Ke=()=>{if(document.getElementById(ce))return;let o=document.createElement("link");o.rel="preconnect",o.href="https://fonts.googleapis.com",document.head.appendChild(o);let t=document.createElement("link");t.rel="preconnect",t.href="https://fonts.gstatic.com",t.crossOrigin="anonymous",document.head.appendChild(t);let e=document.createElement("link");e.id=ce,e.rel="stylesheet",e.href="https://fonts.googleapis.com/css2?family=Geist:wght@400;500;600;700&family=Geist+Mono:wght@400;500;600&display=swap",document.head.appendChild(e)},Ye=`
/* ============================================
...
/* 11b. ACTIVE-ACCENT STANDARD \u2014 one canonical look for any "active"
   or "selected" UI state across the entire app. Tim flagged that
   we had three different treatments (sidebar nav, segmented tabs,
   pill toggles) all rendering inconsistently. This section
   consolidates: every recognized active variant gets emerald-glow
   background + emerald text + emerald icon + emerald border.

   Match the same look already used by:
     - Sidebar active (.sidebar-v2 a.active) \u2014 emerald + glow
     - Selected dropdown option (.n-base-select-option--selected) \u2014 emerald + glow
     - Active tab capsule (.hr-tabs-capsule) \u2014 glow + emerald border

   Selectors below are intentionally broad \u2014 class substring matches,
   ARIA states, common variant suffixes (--active, --selected, --on,
   .is-active, .is-selected, .selected). High specificity via html
   body + multiple class stacks where needed to beat GHL defaults. */

html body .is-active,
html body .is-selected,
html body [class*="--active"]:not(button):not(.n-base-select-option--active):not(.hr-tabs-capsule),
html body [class*="--selected"]:not(.n-base-select-option--selected),
html body [class*="-active"]:not(button):not(.n-base-select-option--active):not(.hr-tabs-capsule):not(.hr-tabs-tab--active),
html body [class*="-selected"]:not(.n-base-select-option--selected),
...
html body [aria-selected="true"]:not(.n-base-select-option--selected),
...
/* Force ICONS inside any active element to emerald too. SVG icons
   typically use stroke="currentColor" so the parent color cascades \u2014
   but some use explicit fill or have inline colors. Hit both. */
html body .is-active svg,
html body .is-selected svg,
html body [class*="--active"]:not(button):not(.hr-tabs-capsule) svg,
html body [class*="--selected"] svg,
html body [class*="-active"]:not(button):not(.hr-tabs-capsule) svg,
html body [class*="-selected"] svg,
...
html body .is-active [class*="icon"] svg,
html body [class*="--active"] [class*="icon"] svg,
html body [class*="-selected"] [class*="icon"] svg {
  color: var(--reos-emerald) !important;
  stroke: var(--reos-emerald) !important;
  fill: currentColor !important;
}
/* Path elements inside those SVGs */
html body .is-active svg path,
html body .is-selected svg path,
html body [class*="--active"]:not(button) svg path,
html body [class*="-active"]:not(button) svg path,
html body [class*="-selected"] svg path,
html body [aria-selected="true"] svg path,
html body [aria-current="page"] svg path,
html body [aria-current="true"] svg path {
  stroke: var(--reos-emerald) !important;
}

/* Descendants of active items: emerald text too (catches inner spans,
   <p>, etc.) */
html body .is-active *,
html body .is-selected *,
html body [class*="--active"]:not(button):not(.hr-tabs-capsule) *,
html body [class*="-active"]:not(button):not(.hr-tabs-capsule):not(.hr-tabs-tab--active) *,
html body [class*="-selected"] *,
html body [aria-current="page"] *,
html body [aria-current="true"] * {
  color: var(--reos-emerald) !important;
  -webkit-text-fill-color: var(--reos-emerald) !important;
...
`,Ze=[{selector:".hr-input__input-el, .hr-input__textarea-el, [class*='hr-input'] input, [class*='hr-input'] textarea",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".hr-collapse-item__header, .hr-collapse-item__header-main, .hr-collapse-item__title",styles:{color:"#EDEEF0","background-color":"#12161D"}},{selector:".hr-collapse-item__content, .hr-collapse-item__content-inner, .hr-collapse-item__content-wrap",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-tabs, .hr-tabs-nav, .hr-tabs-nav-scroll-content, .hr-tabs-content, .hr-tabs-pane, .hr-tabs-rail, .hr-tabs-tab-wrapper",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-tabs-capsule",styles:{"background-color":"rgba(15, 181, 126, 0.12)",border:"1px solid rgba(15, 181, 126, 0.3)"}},{selector:".hr-avatar, .hr-avatar__text, .hr-avatar__text p, .hr-avatar p, .hr-avatar span, .hr-avatar div, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']), [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']) span, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']) p, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']) div",styles:{color:"#0A0D12","-webkit-text-fill-color":"#0A0D12","font-weight":"600"}},{selector:".hr-button--tertiary",styles:{"background-color":"#1A1F28",color:"#0FB57E"}},{selector:".hr-text",styles:{color:"#EDEEF0"}},{selector:"[class*='field-value'], [class*='field-display'], [class*='empty-value'], [class*='field-empty'], [class*='value-display'], [class*='display-value']",styles:{color:"#9098A3","-webkit-text-fill-color":"#9098A3"}},{selector:".hr-select__value, .hr-select__display, .hr-select-trigger, .hr-select-trigger__value, .hr-base-select__value, .hr-base-select__display, [class*='select__value'], [class*='select__display'], [class*='select-trigger']",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".hr-select, .hr-select__input, .hr-select__inner, .hr-select-trigger, .hr-base-select, .hr-base-select__inner, .el-select, .el-select__wrapper, .el-input__wrapper, .el-input__inner, [role='combobox'], [aria-haspopup='listbox']",styles:{"background-color":"#1A1F28","border-color":"#252C36"}},{selector:"input[type='date'], input[type='datetime-local'], input[type='time'], input[type='month'], input[type='week'], .hr-date-picker, .hr-date-input, .hr-datepicker, .hr-time-picker, [class*='date-picker'], [class*='datepicker'], [class*='date-input'], [class*='time-picker'], [class*='time-input']",styles:{"background-color":"#1A1F28","border-color":"#252C36",color:"#EDEEF0"}},{selector:"[class*='F7F9FD'], [class*='F7F9FB'], [class*='F9FAFB'], [class*='eff4ff'], [class*='EFF4FF']",styles:{"background-color":"#12161D"}},{selector:"[style*='background-color: white'], [style*='background-color:white'], [style*='background-color: #fff'], [style*='background-color:#fff']",styles:{"background-color":"#12161D"}},{selector:".bg-gray-50, .bg-white, .bg-gray-100",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".n-base-select-menu, .n-popselect-menu, .n-popover",styles:{"background-color":"#12161D",color:"#EDEEF0","border-color":"#252C36"}}],G=()=>{for(let o of Ze){let t;try{t=document.querySelectorAll(o.selector)}catch{continue}t.forEach(e=>{for(let[r,a]of Object.entries(o.styles)){let i=e.style.getPropertyPriority(r),s=e.style.getPropertyValue(r);i==="important"&&s===a||e.style.setProperty(r,a,"important")}})}},j="#1A1F28",Je=26,Xe=31,Qe=40,de=new Set,pe=new Set,z=()=>{document.body.querySelectorAll("*").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']")||t.children.length>0||(t.textContent||"").trim()!=="--")return;let r=t.getBoundingClientRect();if(r.width===0||r.height===0||r.width>800)return;t.style.setProperty("display","none","important");let a=t.className?.toString()||"<no class>";!pe.has(a)&&a!=="<no class>"&&(pe.add(a),p.debug(`Hid empty "--" leaf: "${a}"`))})},me=new Set,et=()=>{document.body.querySelectorAll("*").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']"))return;let e=(t.className?.toString()||"").toLowerCase();if(e.includes("ai-button")||e.includes("ask-ai")||e.includes("bg-clip-text")||e.includes("text-transparent")||t.closest("[class*='ai-button'], [class*='ask-ai'], [class*='bg-clip-text'], [class*='text-transparent']"))return;let r=t.tagName.toLowerCase();if(r==="svg"||r==="path"||r==="circle"||r==="rect"||r==="polygon"||r==="polyline"||r==="line"||r==="g"||window.getComputedStyle(t).color!=="rgb(255, 255, 255)")return;t.style.setProperty("color","#EDEEF0","important"),t.style.setProperty("-webkit-text-fill-color","#EDEEF0","important");let s=t.className?.toString()||"<no class>";!me.has(s)&&s!=="<no class>"&&(me.add(s),p.debug(`Remapped pure-white text -> bone: "${s}"`))})},be=new Set,tt=()=>{document.querySelectorAll(".hl_header, header.hl_header, [class*='topbar'], [class*='top-bar'], header.app-header, aside.default-bg-color").forEach(t=>{t.querySelectorAll("button, a, [role='button'], [class*='rounded-full'], [class*='rounded-md'], [class*='rounded-xl'], [class*='pill'], span[class*='rounded'], div[class*='rounded']").forEach(r=>{if(r.closest("[id^='ws-crm-']"))return;let a=(r.className?.toString()||"").toLowerCase();if(a.includes("avatar"))return;let i=window.getComputedStyle(r),s=i.backgroundColor,n=i.backgroundImage||"",c=!1;if(n!=="none"&&n.includes("gradient"))c=!0;else if(s&&s!=="rgba(0, 0, 0, 0)"&&s!=="transparent"){let u=s.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(u){let y=parseInt(u[1],10),S=parseInt(u[2],10),C=parseInt(u[3],10);(y>=60||S>=60||C>=60)&&(y>230&&S>230&&C>230||(y>=80||S>=80||C>=80)&&(c=!0))}}if(!c)return;let m=(r.textContent||"").trim(),b=a,v=Array.from(r.querySelectorAll("*")).map(u=>(u.className?.toString()||"").toLowerCase()).join(" "),w=m.toLowerCase()==="ask ai"||b.includes("ai-button")||b.includes("ask-ai")||b.includes("askai"),g=b.includes("bg-clip-text")||b.includes("text-transparent")||v.includes("bg-clip-text")||v.includes("text-transparent"),h=Array.from(r.querySelectorAll("*")).some(u=>{let y=window.getComputedStyle(u),S=y.getPropertyValue("-webkit-background-clip")||"",C=y.getPropertyValue("background-clip")||"";return S.includes("text")||C.includes("text")});if(w||g||h){let u=["color","-webkit-text-fill-color","fill","stroke","background","background-color","background-image","background-clip","-webkit-background-clip","text-shadow"],y=S=>{u.forEach(C=>S.style.removeProperty(C))};y(r),r.querySelectorAll("*").forEach(y);return}let k="#12161D";r.style.setProperty("color",k,"important"),r.querySelectorAll("*").forEach(u=>{let y=u.tagName.toLowerCase();y==="svg"||y==="path"||y==="circle"||y==="rect"||y==="polygon"||y==="polyline"||y==="line"||y==="g"?(u.style.setProperty("fill",k,"important"),u.style.setProperty("stroke",k,"important"),u.style.setProperty("color",k,"important")):(u.style.setProperty("color",k,"important"),u.style.setProperty("-webkit-text-fill-color",k,"important"))});let _=r.className?.toString()||"<no class>";!be.has(_)&&_!=="<no class>"&&(be.add(_),p.debug(`Darkened SVG icons on colored button: "${_}"`))})})},ue=new Set,ot=()=>{document.querySelectorAll("[class*='avatar']").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;let e=t.getBoundingClientRect();if(e.width===0||e.height===0||e.width<20||e.width>64||e.height<20||e.height>64)return;let r=e.top+e.height/2;(t.parentElement||t).querySelectorAll("svg, img").forEach(n=>{let c=n;if(c.closest("[id^='ws-crm-']"))return;let d=n.getBoundingClientRect();if(d.width===0||d.width>28||d.height>28)return;let m=d.left<e.right&&d.right>e.left,b=d.top<e.bottom&&d.bottom>e.top;if(!(m&&b)||d.top+d.height/2<r)return;let w=d.width*d.height/(e.width*e.height);if(w>.55)return;let g=c,h=c.parentElement,k=0;for(;h&&h!==t&&k<3;){let u=h.getBoundingClientRect();if(u.width>0&&u.width<=30&&u.height<=30)g=h;else break;h=h.parentElement,k++}g.style.setProperty("transform","translateY(11px)","important");let _=(g.className?.toString()||"")+" "+g.tagName;ue.has(_)||(ue.add(_),p.debug(`Dropped activity badge 11px (${d.width.toFixed(0)}x${d.height.toFixed(0)} icon, ratio ${w.toFixed(2)}): "${_}"`))})})},he=new Set,rt=()=>{document.querySelectorAll(".n-dialog, [class*='n-dialog'], [class*='dialog'], [role='dialog']").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;t.querySelectorAll("div, span, i").forEach(r=>{if(r.closest("[id^='ws-crm-']"))return;let a=r.getBoundingClientRect();if(a.width===0||a.height===0||a.width>56||a.height>56||!r.querySelector("svg"))return;let s=window.getComputedStyle(r).backgroundColor.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!s)return;let n=parseInt(s[1],10),c=parseInt(s[2],10),d=parseInt(s[3],10);if(!(n>200&&c>200&&d>200))return;r.style.setProperty("background-color","transparent","important"),r.style.setProperty("background","transparent","important"),r.style.setProperty("box-shadow","none","important"),r.style.setProperty("border","none","important");let m=r.className?.toString()||"<no class>";!he.has(m)&&m!=="<no class>"&&(he.add(m),p.debug(`Stripped dialog icon circle (${a.width.toFixed(0)}x${a.height.toFixed(0)}, rgb(${n},${c},${d})): "${m}"`))})})},ge=new Set,at=()=>{document.querySelectorAll("[class*='ai-button'], [class*='ask-ai'], [class*='askai']").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;let e=t.parentElement,r=0;for(;e&&r<3&&!e.closest("[id^='ws-crm-']");){let a=(e.className?.toString()||"").toLowerCase();if(!(a.includes("ai-button")||a.includes("ask-ai")||a.includes("askai"))){let s=e.getBoundingClientRect();if(s.width>0&&s.height>0&&s.width<=400&&s.height<=80){e.style.setProperty("background-color","#12161D","important"),e.style.setProperty("background-image","none","important");let n=e.className?.toString()||"<no class>";!ge.has(n)&&n!=="<no class>"&&(ge.add(n),p.debug(`Painted Ask AI wrapper graphite: "${n}"`))}}e=e.parentElement,r++}})},ye=new Set,nt=()=>{let o=Array.from(document.querySelectorAll("#contact-conversation-panel, [class*='conversation-panel'], [class*='message-list'], [class*='activity-log'], .hl_header, header.hl_header, [class*='topbar'], [class*='top-bar'], header.app-header, .modal, .modal-content, [class*='modal'], [class*='dialog'], [role='dialog'], [class*='popover'], [class*='popup'], [class*='dropdown-menu'], [class*='select-menu'], [class*='popselect'], [class*='base-select'], .n-base-select-menu, .n-popselect, .n-popover")),t=document.querySelectorAll("div, section, aside"),e=[];t.forEach(a=>{if(a.id?.startsWith("ws-crm-")||a.closest("[id^='ws-crm-']"))return;let i=window.getComputedStyle(a);if(i.position!=="fixed"&&i.position!=="absolute")return;let s=parseInt(i.zIndex,10);if(!isFinite(s)||s<50)return;let n=a.getBoundingClientRect();n.width<240||n.height<120||n.width>1500||n.height>900||e.push(a)}),[...o,...e].forEach(a=>{a.querySelectorAll("*").forEach(s=>{if(s.id?.startsWith("ws-crm-")||s.closest("[id^='ws-crm-']"))return;let n=(s.className?.toString()||"").toLowerCase();if(n.includes("avatar")||n.includes("notification-dot")||n.includes("indicator")||n.includes("ai-button")||n.includes("ask-ai")||n.includes("bg-clip-text")||n.includes("text-transparent")||s.closest("[class*='ai-button'], [class*='ask-ai'], [class*='bg-clip-text'], [class*='text-transparent']"))return;let c=window.getComputedStyle(s).backgroundColor;if(!c||c==="rgba(0, 0, 0, 0)"||c==="transparent")return;let d=c.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!d)return;let m=parseInt(d[1],10),b=parseInt(d[2],10),v=parseInt(d[3],10);if(!(m>200&&b>200&&v>200))return;let g=s.getBoundingClientRect();if(g.width<24||g.height<16||g.width>1400)return;s.style.setProperty("background-color","#12161D","important"),s.style.setProperty("color","#EDEEF0","important");let h=s.className?.toString()||"<no class>";!ye.has(h)&&h!=="<no class>"&&(ye.add(h),p.debug(`Coerced light bg (${g.width.toFixed(0)}x${g.height.toFixed(0)}, rgb(${m},${b},${v})): "${h}"`))})})},fe=new Set,st=()=>{let o="input, select, textarea, [class*='hr-input'], [class*='hr-select'], [class*='hr-base-select'], [class*='hr-date-picker'], [class*='hr-datepicker']";document.body.querySelectorAll("*").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.children.length>0)return;let r=(e.textContent||"").trim();if(r.length===0||r.length>80||r==="--"||e.closest(o)||e.closest("[class*='avatar'], [class*='hr-avatar']")||e.closest("[class*='hr-collapse'], [class*='collapse-item'], [class*='collapse-header'], [class*='folder-header'], [class*='field-folder']"))return;let a=e,i=null;for(let m=0;m<4&&a;m++){let b=a.nextElementSibling;if(b&&(b.matches(o)||b.querySelector(o)!==null)){i=b;break}a=a.parentElement}if(!i)return;let s=e.getBoundingClientRect();if(s.width===0||s.height===0||s.width>400||s.height>60)return;e.style.setProperty("color","#9098A3","important"),e.style.setProperty("-webkit-text-fill-color","#9098A3","important");let n=i,c=n.getBoundingClientRect();c.width>0&&c.width<=800&&c.height>0&&c.height<=100&&(n.style.setProperty("background-color",j,"important"),n.style.setProperty("border-color","#252C36","important"),n.style.setProperty("border-style","solid","important"),n.style.setProperty("border-width","1px","important"),n.style.setProperty("border-radius","4px","important"));let d=e.className?.toString()||"<no class>";!fe.has(d)&&d!=="<no class>"&&(fe.add(d),p.debug(`Colored field label "${r.slice(0,30)}": "${d}"`))})},ve=new Set,it=()=>{document.querySelectorAll("[class*='avatar']").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;let e=(t.className?.toString()||"").toLowerCase();if(e.includes("dropdown")||e.includes("option-avatar"))return;let r=t.getBoundingClientRect();if(r.width===0||r.height===0||r.height>80||t.querySelector("svg, img")!==null)return;let i=(t.textContent||"").trim();if(i.length===0||i.length>4)return;let s=Math.min(r.width,r.height),n=Math.max(16,Math.min(24,Math.round(s*.6)));t.style.setProperty("font-size",`${n}px`,"important"),t.style.setProperty("line-height","1","important"),t.style.setProperty("display","flex","important"),t.style.setProperty("align-items","center","important"),t.style.setProperty("justify-content","center","important"),t.style.setProperty("text-align","center","important"),t.querySelectorAll("*").forEach(d=>{d.tagName==="svg"||d.tagName==="SVG"||d.tagName!=="IMG"&&(d.style.setProperty("font-size",`${n}px`,"important"),d.style.setProperty("line-height","1","important"),d.style.setProperty("text-align","center","important"))});let c=t.className?.toString()||"<no class>";!ve.has(c)&&c!=="<no class>"&&(ve.add(c),p.debug(`Resized avatar text to ${n}px (circle ${s.toFixed(0)}px): "${c}"`))})},xe=new Set,lt=()=>{document.querySelectorAll("input:not([type='checkbox']):not([type='radio']):not([type='button']):not([type='submit']):not([type='reset']):not([type='hidden']):not([type='file']), textarea, select").forEach(t=>{if(t.closest("[id^='ws-crm-']"))return;let e=t.getBoundingClientRect();if(e.width===0||e.height===0)return;t.style.setProperty("background-color",j,"important"),t.style.setProperty("border-color","#252C36","important"),t.style.setProperty("color","#EDEEF0","important"),t.style.setProperty("-webkit-text-fill-color","#EDEEF0","important");let r=t.className?.toString()||"<no class>";!xe.has(r)&&r!=="<no class>"&&(xe.add(r),p.debug(`Forced input slate bg: "${r}"`))})},U=()=>{document.body.querySelectorAll("*").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']")||t.closest("[class*='hr-collapse'], [class*='collapse-item'], [class*='collapse-header'], [class*='folder-header'], [class*='field-folder']"))return;let e=window.getComputedStyle(t).backgroundColor;if(!e||e==="rgba(0, 0, 0, 0)"||e==="transparent")return;let r=e.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!r)return;let a=parseInt(r[1],10),i=parseInt(r[2],10),s=parseInt(r[3],10);if(!(a<Je&&i<Xe&&s<Qe))return;let c=t.getBoundingClientRect();if(c.width===0||c.height===0||c.width>800||c.height>200)return;t.style.setProperty("background-color",j,"important");let d=t.className?.toString()||"<no class>";!de.has(d)&&d!=="<no class>"&&(de.add(d),p.debug(`Coerced dark bg to slate (${c.width.toFixed(0)}x${c.height.toFixed(0)}, rgb(${a},${i},${s})): "${d}"`))})},we=()=>{let o,t=()=>{o===void 0&&(o=window.setTimeout(()=>{o=void 0,G(),lt(),U(),z(),st(),nt(),at(),rt(),tt(),it(),ot(),et()},100))};G(),U(),z(),window.setTimeout(()=>{G(),U(),z()},500),window.setTimeout(()=>{G(),U(),z()},1500),new MutationObserver(()=>{t()}).observe(document.body,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["class","style"]})},ke=()=>{if(Ke(),!document.getElementById(le)){let o=document.createElement("style");o.id=le,o.textContent=Ye,document.head.appendChild(o),p.info("REInvest OS theme stylesheet mounted")}window.__reosForceInstalled||(window.__reosForceInstalled=!0,document.body?we():window.addEventListener("DOMContentLoaded",we))};var Ee="ws-crm-customizer-version-badge",_e=()=>{if(document.getElementById(Ee))return;let o=document.createElement("div");o.id=Ee,o.style.cssText=`
  ...
  `,o.textContent=`Customizer v${H}`,o.title=`Built ${R}
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var Se=()=>{p.info(`Loaded v${H} (built ${R})`),ie(),ke(),Z(),Y(o=>{p.debug("Page handler firing for:",o),_e(),ee(),ne()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Se):Se();})();
