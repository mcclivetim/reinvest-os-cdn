...
`;var Qr=/^[A-Za-z0-9]{15,30}$/,Ee=t=>typeof t=="string"&&Qr.test(t),Zr=t=>{try{let e=t.split(".");if(e.length!==3)return null;let o=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=o+"=".repeat((4-o.length%4)%4);return JSON.parse(atob(r))}catch{return null}},en=()=>{let t=window.WS_CRM_USER_ID;return Ee(t)?t:null},tn=()=>{let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let o of e)if(Ee(o))return o;return null},on=()=>{try{let t=localStorage.getItem("refreshedToken");if(!t)return null;let e=t.startsWith('"')?t.slice(1,-1):t,o=Zr(e);if(!o)return null;let r=o.uid;if(Ee(r))return r;let n=o.claims;if(n&&Ee(n.user_id))return n.user_id}catch{}return null},rn=()=>{try{let t=["user","tokenUser","currentUser","ghl_user"];for(let e of t){let o=localStorage.getItem(e);if(o)try{let r=JSON.parse(o),n=r.id??r._id;if(Ee(n))return n}catch{}}}catch{}return null},L=()=>en()||tn()||on()||rn()||null;var Fo="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",nn=`${Fo}/active-follow-up-get`,an=`${Fo}/active-follow-up-set`,le="ws-crm-active-follow-up-button",Nt={amber:{label:"Add to Active Follow Up",bg:s.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:s.amber,clickable:!0,dotColor:s.amber},blue:{label:"In Active Follow Up",bg:s.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:s.blue,clickable:!1,dotColor:s.blue},gray:{label:"Not in Follow Up",bg:s.slate,border:s.steel,color:s.coolGray,clickable:!1,dotColor:s.coolGray},loading:{label:"Active Follow Up\u2026",bg:s.slate,border:s.steel,color:s.ash,clickable:!1,dotColor:s.ash},error:{label:"Follow Up status unavailable",bg:s.slate,border:s.steel,color:s.ash,clickable:!1,dotColor:s.crimson}},sn=()=>{let t=C();return!t||!t.locationId?null:{contactId:t.contactId,tenantId:t.locationId}},ln=async t=>{let e=await fetch(nn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},dn=async(t,e)=>{let o=await fetch(an,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!o.ok)throw new Error(`Set webhook returned ${o.status} ${o.statusText}`);return await o.json()},de=(t,e)=>{let o=Nt[e];t.dataset.state=e,t.disabled=!o.clickable,t.style.cursor=o.clickable?"pointer":"default",t.style.background=o.bg,t.style.borderColor=o.border,t.style.color=o.color;let r=t.querySelector(".ws-afu-dot");r&&(r.style.background=o.dotColor);let n=t.querySelector(".ws-afu-label");n&&(n.textContent=o.label)},Lo=t=>{let e=document.createElement("button");return e.type="button",e.id=le,e.dataset.contactId=t,e.style.cssText=`
  ...
  `,r.textContent=t,document.body.appendChild(r),window.setTimeout(()=>r.remove(),o)},fr=(t,e)=>new Promise(o=>{document.getElementById(Q)?.remove();let r=document.createElement("div");r.id=Q,r.style.cssText=`
    ...
    `;let a=d=>{if(!d)return"";try{return new Date(d).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"2-digit"})}catch{return d}},i=t.map(d=>`
        <button type="button" class="ws-pc-acq-opp" data-opp-id="${J(d.id)}" style="
          ...
          font-family: ${y.sans};
        ">
          <div style="font-size: 14px; font-weight: 500; line-height: 1.3;">${H(d.propertyAddress||d.name)}</div>
          ${d.updatedAt?`<div style="font-size: 11px; color: ${s.ash}; margin-top: 4px;">Last updated ${H(a(d.updatedAt))}</div>`:""}
        </button>
      `).join("");n.innerHTML=`
      <div style="
        font-family: ${y.mono};
      ...
      <div style="
        ...
        font-weight: 500;
        margin-bottom: 4px;
      ">This ${H(e.perspective)} has ${t.length} open deals</div>
      <div style="
        font-size: 12px;
        color: ${s.ash};
        margin-bottom: 16px;
        line-height: 1.4;
      ">Which property is this contract for? The ${H(e.agreementLabel)} will use that opp's data.</div>
      <div>${i}</div>
  ...
  `,K=`
    ...
    padding-right: 28px;
  `,Ve=F=>['<option value="">(unset)</option>'].concat(k.map($=>`<option value="${J($)}"${$===F?" selected":""}>${H($)}</option>`)).join("");_.innerHTML=`
    <div style="
      font-family: ${y.mono};
      font-size: 10px;
      letter-spacing: 0.14em;
    ...
    <div style="margin-bottom: 12px;">
      <div style="${S}">
        ...
        <div style="${w}">${H(a||"(set on Property record)")}</div>
      </div>
      <div style="${S}">
      ...
      <div style="${S} align-items: flex-start;">
        ...
        <textarea id="ws-pc-additional-terms" rows="3" placeholder="Any extra clauses, contingencies, or notes for the contract" style="${E} resize: vertical; min-height: 60px; font-family: ${y.sans};">${H(p)}</textarea>
      </div>
    </div>
    <div style="
      font-size: 11px;
  ...
  `;w.innerHTML=`
    ...
    <div style="margin-bottom: 12px;">
      <div style="${N}">
        ...
        <div style="${Ve}">${H(b||"(set on Property record)")}</div>
      </div>
      <div style="${N}">
  ...
  `,T.appendChild(w),document.body.appendChild(T);let oe=w.querySelector("#ws-pc-assignment-price"),ie=w.querySelector("#ws-pc-assignment-fee"),ve=!1;ie?.addEventListener("input",()=>{ve=!0});let We=()=>{if(ve)return;let $=Number(oe?.value||0);ie&&(ie.value=String(Math.max(0,$-f)))};oe?.addEventListener("input",We);let U=()=>T.remove();T.addEventListener("click",$=>{$.target===T&&U()}),w.querySelector("#ws-pc-cancel")?.addEventListener("click",U);let R=w.querySelector("#ws-pc-submit"),F=w.querySelector("#ws-pc-error");R?.addEventListener("click",async()=>{if(!R)return;F&&(F.textContent="");let $=(w.querySelector("#ws-pc-seller-name")?.value||"").trim(),ye=Number(oe?.value||0),ge=Number(w.querySelector("#ws-pc-buyer-emd")?.value||0),fe=Number(ie?.value||0),we=(w.querySelector("#ws-pc-closing-office")?.value||"").trim(),xe=(w.querySelector("#ws-pc-buyer-llc")?.value||"").trim(),pe=(w.querySelector("#ws-pc-closing-date")?.value||"").trim(),ke=(w.querySelector("#ws-pc-emd-due-date")?.value||"").trim(),O=(w.querySelector("#ws-pc-lender-source")?.value||"").trim();if(!pe){F&&(F.textContent="Closing Date is required.");return}R.disabled=!0,R.textContent="Opening\u2026";try{let P={};$!==h&&(P.sellerName=$),ye!==m&&(P.assignmentPrice=ye),ge!==p&&(P.buyerEMD=ge),fe!==u&&(P.assignmentFee=fe),we!==k&&(P.closingOffice=we),xe!==v&&(P.buyerCompanyName=xe);let At=pe!==_||ke!==g||O!==E,Lt=Object.keys(P).length>0||At,Mt=n;Lt?(c.info("[prepare-contract] AA modal submit, re-staging with overrides:",P),R.textContent="Saving\u2026",Mt=await ft({tenantId:t,contactId:e,template:o,oppId:r,closingDate:pe,buyerEarnestDueDate:ke,lenderSource:O,userId:a,overrides:Object.keys(P).length>0?P:void 0}),c.info("[prepare-contract] AA re-stage ok:",Mt),R.textContent="Opening\u2026"):c.info("[prepare-contract] AA modal submit \u2014 no edits, skipping re-stage");let Eo=Mt?.debug?.propertyAddress||b,jr=Eo?`${Eo} - Assignment Agreement`:void 0;await go(t,e,o,jr),U()}catch(P){R.disabled=!1,R.textContent="Open Contract",F&&(F.textContent=P.message||"Failed to prepare contract."),c.warn("[prepare-contract] AA submit failed:",P)}})},ft=async t=>{c.info(`[prepare-contract] staging \u2014 template "${t.template.name}" (${t.template.type})`);let e=await ms({tenantId:t.tenantId,contactId:t.contactId,templateId:t.template.id,templateType:t.template.type,oppId:t.oppId,closingDate:t.closingDate,buyerEarnestDueDate:t.buyerEarnestDueDate,lenderSource:t.lenderSource,userId:t.userId,overrides:t.overrides});if(c.info(`[prepare-contract] stage response: ok=${e.ok}`,e),!e.ok)throw new Error(e.error||"Contract.Stage returned not ok");return e},go=async(t,e,o,r)=>{c.info(`[prepare-contract] creating proposal from template ${o.id} for contact ${e}`);let n=await us(t,o.id,e);c.info(`[prepare-contract] proposal created: ${n}`),c.info(`[prepare-contract] attaching contact ${e} as primary signer${r?`, renaming to "${r}"`:""}`);let a=await ys(t,n,e,r);a.ok?c.info(`[prepare-contract] contact attached to proposal ${n}`):(c.warn(`[prepare-contract] attach failed (status=${a.status??"n/a"}): ${a.body?.slice(0,300)??"no body"}`),G(`Proposal created but couldn't auto-attach contact (HTTP ${a.status??"?"}). You'll need to pick the recipient manually.`,"error",8e3),console.warn("[ws-crm-customizer] attach-recipient GHL response:",a.body));let i=xs(o.name);i&&(c.info(`[prepare-contract] stashing post-redirect instructions for "${o.name}"`),ks(n,i.title,i.bodyHtml)),gs(t,n)},Ms=async t=>{let e=await ds(t);if(!e){G("Could not resolve contact context.","error");return}document.getElementById(_r)?.remove();let o=document.createElement("div");o.id=_r,o.style.cssText=`
    position: absolute;
    top: calc(100% + 6px);
    right: 0;
    z-index: 999998;
  ...
  `,o.innerHTML=`
    ...
    ">Loading templates\u2026</div>`,t.appendChild(o);let r=a=>{!o.contains(a.target)&&a.target!==t&&(o.remove(),document.removeEventListener("click",r,!0))};window.setTimeout(()=>document.addEventListener("click",r,!0),0);let n;try{n=await bs(e.tenantId)}catch(a){o.innerHTML=`
      <div style="padding: 12px 14px; color: ${s.crimson}; font-size: 12px;">
        Failed to load templates: ${H(a.message)}
...
Click to dismiss for this session`,t.addEventListener("mouseenter",()=>{t.style.transform="scale(1.05)"}),t.addEventListener("mouseleave",()=>{t.style.transform="scale(1)"}),t.addEventListener("click",()=>{t.style.opacity="0",setTimeout(()=>t.remove(),200)}),document.body.appendChild(t)};console.log("[ws-crm-customizer] bundle-loaded");var B=(t,e)=>{try{c.info(`[main] calling ${t}`),e(),c.info(`[main] ${t} OK`)}catch(o){c.error(`[main:FAIL] ${t} threw:`,o)}},Wr=()=>{c.info(`Loaded v${Xe} (built ${Je})`),B("mountVersionBadge (initial)",It),B("mountFavicon",Jo),B("mountThemeStylesheet",qr),B("initPageRouter",Io),c.info("[main] registering onPageChange subscriber"),Co(t=>{c.info(`[main] onPageChange callback fired for page=${t}`),B("mountVersionBadge",It),B("mountClaimReleaseToggle",Ho),B("mountContactActionsBar",Yo),B("mountActiveFollowUpButton",$o),B("mountMtOfferMadeButton",Zo),B("mountNotInterestedButton",er),B("mountReducePriceButton",or),B("mountSendToClosingOfficeButton",ir),B("mountClosingAgentPickerButton",ur),B("mountPrepareContractButton",Tr)}),c.info("[main] onPageChange registered; main() complete")};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Wr):Wr();})();
