...
`;var I="ws-crm-claim-release-pill",ne=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},He=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",n=t==="claim"?d.emerald:d.amber;return e.style.cssText=oe(r),e.innerHTML=`
  ...
  `;let r=new Date,n=new Date(r.getTime()+5*24*60*60*1e3).toISOString().slice(0,10);e.innerHTML=`
    ...
    <div style="
      ...
      margin-bottom: 16px;
    ">
      <span style="color: ${d.ash}; font-family: ${f.mono}; font-size: 14px;">$</span>
...
/* Inner content surfaces \u2014 transparent so the wrapper graphite
   shows through (no double-card look). */
