...
`,L=`
  ...
  flex-shrink: 0;
`;var I="ws-crm-claim-release-pill",oe=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},De=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?l.emerald:l.amber;return e.style.cssText=ee(r),e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,e.title=t==="claim"?"Take ownership of this lead":"Return this lead to the pool",e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{h.info(`Claim/Release pill clicked for contact ${o}: ${t}`);let i=e.lastElementChild,s=i?.textContent??"";i&&(i.textContent="Working\u2026"),e.style.opacity="0.7",e.disabled=!0,setTimeout(()=>{i&&(i.textContent=s),e.style.opacity="1",e.disabled=!1},700)}),e},Ne=async(o,t=1e4)=>{let e=Date.now();for(;Date.now()-e<t;){let r=oe(o);if(r)return r;await new Promise(a=>setTimeout(a,150))}return null},te=(o,t)=>{let e=De({contactId:t,mode:"claim"}),r=document.createElement("span");r.id=`${I}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(e),o.appendChild(r)},He=async()=>{let o=E();if(!o){document.getElementById(I)?.remove();return}let t=document.getElementById(I);if(!(t&&t.dataset.contactId===o.contactId)){t&&t.remove();try{let e=await C("#record-details-lhs",{timeoutMs:1e4}),r=await Ne(e),a=E();if(!a||a.contactId!==o.contactId)return;if(!r){h.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}te(r,o.contactId),h.debug(`Mounted Claim/Release pill next to name for ${o.contactId}`);let i=null;i=new MutationObserver(async()=>{if(!(E()?.contactId===o.contactId)){i?.disconnect();return}if(document.getElementById(I))return;let n=oe(e);n&&(te(n,o.contactId),h.debug("Re-mounted Claim/Release pill after React wipe"))}),i.observe(e,{childList:!0,subtree:!0})}catch(e){h.warn("Could not mount Claim/Release pill:",e)}}},re=()=>{He()};var Me="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set";var Re="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Be=()=>{let o=window.WS_CRM_USER_ID;if(o&&/^[A-Za-z0-9]{15,30}$/.test(o))return o;let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let r of e)if(typeof r=="string"&&/^[A-Za-z0-9]{15,30}$/.test(r))return r;try{let r=localStorage.getItem("user");if(r){let a=JSON.parse(r),i=a.id??a._id;if(i&&/^[A-Za-z0-9]{15,30}$/.test(i))return i}for(let a of["tokenUser","currentUser","ghl_user"]){let i=localStorage.getItem(a);if(i)try{let s=JSON.parse(i),n=s.id??s._id;if(n&&/^[A-Za-z0-9]{15,30}$/.test(n))return n}catch{}}}catch{}return null},Oe=async({contactId:o,locationId:t,status:e,userId:r,deadReason:a})=>{let i={call_status:e,triggered_by_user_id:r??""};a&&(i.dead_reason=a);let n=await fetch(Me,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:o,location:{id:t},customData:i}),credentials:"omit"});if(!n.ok)throw new Error(`Webhook returned ${n.status} ${n.statusText}`)},Pe=async({contactId:o,tenantId:t})=>{let e=await fetch(Re,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t,contact_id:o}),credentials:"omit"});if(!e.ok)throw new Error(`Sanity check returned ${e.status} ${e.statusText}`);return await e.json()};var O="ws-crm-contact-actions-bar",Ge="ws-crm-call-status-dropdown",ae="ws-crm-offer-made-modal",Ve=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],$e=[".central-panel","[class*='central-panel']"],Ue=async()=>{for(let o of $e)try{let t=await C(o,{timeoutMs:3e3});if(t)return t}catch{}try{return(await C("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},qe=o=>{let t=document.createElement("div");t.id=Ge,t.style.cssText="position: relative; flex-shrink: 0;";let e=document.createElement("button");e.type="button",e.style.cssText=L,e.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,Ve.forEach(a=>{let i=document.createElement("button");i.type="button",i.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
    ...
    `,i.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,i.addEventListener("mouseenter",()=>{i.style.background=l.slate}),i.addEventListener("mouseleave",()=>{i.style.background="transparent"}),i.addEventListener("click",async()=>{h.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let s=e.firstElementChild,n=e.children[1],d=e.children[2];s&&(s.style.background=j[a.color]),n&&(n.textContent=a.label),r.style.display="none";let p=E()?.locationId,m=Be();if(!p){h.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=l.amber);return}if(a.color==="green"&&!m){h.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=l.amber);return}let c=d?.textContent??"\u25BE",v=async u=>{d&&(d.textContent="\u2026"),e.style.opacity="0.75";try{await Oe({contactId:o,locationId:p,status:a.label,userId:m,deadReason:u}),d&&(d.textContent="\u2713"),setTimeout(()=>{d&&(d.textContent=c),e.style.opacity="1"},900)}catch(y){h.warn("Call Status webhook failed:",y),d&&(d.textContent="\u2717"),n&&(n.style.color=l.amber),e.style.opacity="1",setTimeout(()=>{d&&(d.textContent=c),n&&(n.style.color=l.bone)},1800)}};if(a.value==="dead-deal"){let u=n?.textContent??"";We(y=>{v(y)},()=>{n&&(n.textContent=u)});return}if(a.value==="appointment-booked"){let u=n?.textContent??"";try{(await Pe({contactId:o,tenantId:p})).ok?await v():je(()=>{v()},()=>{n&&(n.textContent=u)})}catch(y){h.warn("Appointment sanity check failed; firing anyway:",y),await v()}return}await v()}),r.appendChild(i)}),e.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{t.contains(a.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},ze=o=>{document.getElementById(ae)?.remove();let t=document.createElement("div");t.id=ae,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,t.appendChild(e),document.body.appendChild(t);let i=()=>t.remove();t.addEventListener("click",s=>{s.target===t&&i()}),e.querySelector("#ws-offer-cancel")?.addEventListener("click",i),e.querySelector("#ws-offer-submit")?.addEventListener("click",()=>{let s=(e.querySelector("#ws-offer-amount")?.value??"").trim(),n=e.querySelector("#ws-offer-expires")?.value??"";if(!s||Number(s)<=0){h.warn("Offer Made submit blocked \u2014 invalid amount");return}h.info(`Offer Made recorded for contact ${o}: $${s}, expires ${n}`),i()}),setTimeout(()=>{e.querySelector("#ws-offer-amount")?.focus()},0)},ne="ws-crm-dead-deal-modal",We=(o,t)=>{document.getElementById(ne)?.remove();let e=document.createElement("div");e.id=ne,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(r),document.body.appendChild(e);let a=()=>e.remove(),i=()=>{a(),t()};e.addEventListener("click",n=>{n.target===e&&i()}),r.querySelector("#ws-dead-cancel")?.addEventListener("click",i);let s=()=>{let n=r.querySelector("#ws-dead-reason"),d=r.querySelector("#ws-dead-error"),b=(n?.value??"").trim();if(!b){d&&(d.textContent="Please enter a reason."),n?.focus();return}if(b.length<4){d&&(d.textContent="Please enter at least a few words."),n?.focus();return}a(),o(b)};r.querySelector("#ws-dead-submit")?.addEventListener("click",s),r.querySelector("#ws-dead-reason")?.addEventListener("keydown",n=>{n.key==="Enter"&&(n.metaKey||n.ctrlKey)&&(n.preventDefault(),s()),n.key==="Escape"&&i()}),setTimeout(()=>{r.querySelector("#ws-dead-reason")?.focus()},0)},se="ws-crm-appointment-sanity-modal",je=(o,t)=>{document.getElementById(se)?.remove();let e=document.createElement("div");e.id=se,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(r),document.body.appendChild(e);let a=()=>e.remove(),i=()=>{a(),t()};e.addEventListener("click",n=>{n.target===e&&i()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",i),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{a(),o()});let s=n=>{n.key==="Escape"&&(document.removeEventListener("keydown",s),i())};document.addEventListener("keydown",s)},ie=o=>{let t=document.createElement("div");t.id=O,t.dataset.contactId=o,t.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    ...
    flex-shrink: 0;
  `,t.appendChild(qe(o));let e=document.createElement("button");return e.type="button",e.style.cssText=A,e.textContent="Offer Made",e.addEventListener("click",()=>ze(o)),t.appendChild(e),t},Ke=async()=>{let o=E();if(!o){document.getElementById(O)?.remove();return}let t=document.getElementById(O);if(t&&t.dataset.contactId===o.contactId)return;t&&t.remove();let e=await Ue();if(!e){h.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let r=E();if(!r||r.contactId!==o.contactId)return;let a=ie(o.contactId);a.dataset.mountTarget=e.className||"central-panel",e.insertBefore(a,e.firstChild),h.debug(`Mounted contact actions bar for ${o.contactId}`);let i=null;i=new MutationObserver(()=>{let s=document.getElementById(O);if(!(E()?.contactId===o.contactId)){i?.disconnect();return}if(!s){let d=ie(o.contactId);d.dataset.mountTarget=a.dataset.mountTarget??"",e.insertBefore(d,e.firstChild),h.debug(`Re-mounted actions bar after React wipe for ${o.contactId}`)}}),i.observe(e,{childList:!0})},le=()=>{Ke()};var P="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",Ye=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),D="data-ws-crm-favicon",K=o=>o.hasAttribute(D),de=o=>{if(o.tagName!=="LINK")return!1;let t=(o.getAttribute("rel")??"").toLowerCase();return Ye.has(t)},G=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{de(r)&&!K(r)&&r.remove()});let t=document.head.querySelector(`link[${D}="1"]`);t?t.href!==P&&(t.href=P):(t=document.createElement("link"),t.setAttribute(D,"1"),t.rel="icon",t.type="image/png",t.href=P,document.head.appendChild(t),h.debug("Favicon installed"));let e=document.head.querySelector(`link[${D}="apple"]`);e||(e=document.createElement("link"),e.setAttribute(D,"apple"),e.rel="apple-touch-icon",e.href=P,document.head.appendChild(e))},ce=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)G();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),G())},50)}new MutationObserver(r=>{let a=!1;for(let i of r)i.addedNodes.forEach(s=>{s instanceof HTMLLinkElement&&de(s)&&!K(s)&&(a=!0)}),i.removedNodes.forEach(s=>{s instanceof HTMLLinkElement&&K(s)&&(a=!0)});a&&G()}).observe(document.head,{childList:!0,subtree:!1});let t=0,e=window.setInterval(()=>{t+=1,G(),t>=5&&window.clearInterval(e)},1e3)};var q="reos-theme-stylesheet",ke="reos-theme-fonts",Xe=()=>{let o=document.getElementById(ke);o&&o.remove()},Je=`
/* ============================================
...
/* Active state: only inner-emerald border on the dot wrapper,
   no outer halo. */
