...
/* 16e. INLINE LIGHT-NEUTRAL BORDERS \u2014 GHL panel layouts (Conversations
   page panels, etc.) ship with inline border-right / border-left /
   border-bottom: 1px solid rgb(234, 236, 240) (#EAECF0 neutral-200).
   On the dark theme that's a bright vertical/horizontal hairline
   between dark panels. Remap to steel so the divider matches the
   theme.

   Exact-value attribute match for the specific color value across
   all border shorthand directions. The fill / svg variants of this
   color are already handled in section 16a. */
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var Ct=()=>{p.info(`Loaded v${R} (built ${B})`),lt(),Et(),X(),J(o=>{p.debug("Page handler firing for:",o),St(),et(),st()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ct):Ct();})();
