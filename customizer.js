...
/* 16b. CHART TEXT \u2014 TEMPORARILY DISABLED in v0.7.51 as a diagnostic.
   Tim reports click-to-open-details on funnel rows is broken in
   REInvest OS (Customizer loaded) but works in GenieREI (no Customizer).
   v0.7.48 and v0.7.50 are functionally identical (revert only contains
   comments), so the click regression isn't from v0.7.49. Bisecting
   chart-related CSS by disabling this rule to test whether v0.7.36's
   chart-text fill rule is interfering with click handlers somehow.

   If click works at v0.7.51: this rule was the cause; rebuild with
     a narrower scope (maybe svg :not([onclick]) text, or per-stage
     opt-out).
   If click is still broken: bisect further (section 16a SVG fill remap,
     section 18d-related changes, etc.)
