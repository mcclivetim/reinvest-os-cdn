...
`,I=`
  ...
  flex-shrink: 0;
`;var D="ws-crm-claim-release-pill",ie=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Re=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=D,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",n=t==="claim"?c.emerald:c.amber;return e.style.cssText=ae(r),e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,We.forEach(n=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
    ...
    `,s.innerHTML=`
      <span style="
        ...
        height: 8px;
        border-radius: 50%;
        background: ${Z[n.color]};
        flex-shrink: 0;
      "></span>
      <span>${n.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=c.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{h.info(`Call Status set to "${n.label}" (${n.value}, ${n.color}) for contact ${o}`);let i=e.firstElementChild,a=e.children[1],l=e.children[2];i&&(i.style.background=Z[n.color]),a&&(a.textContent=n.label),r.style.display="none";let p=S()?.locationId,m=Ue();if(!p){h.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),a&&(a.textContent=`${n.label} (no tenant!)`),a&&(a.style.color=c.amber);return}if(n.color==="green"&&!m){h.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),a&&(a.textContent=`${n.label} (no user!)`),a&&(a.style.color=c.amber);return}let d=l?.textContent??"\u25BE",y=async u=>{l&&(l.textContent="\u2026"),e.style.opacity="0.75";try{await $e({contactId:o,locationId:p,status:n.label,userId:m,deadReason:u}),l&&(l.textContent="\u2713"),setTimeout(()=>{l&&(l.textContent=d),e.style.opacity="1"},900)}catch(_){h.warn("Call Status webhook failed:",_),l&&(l.textContent="\u2717"),a&&(a.style.color=c.amber),e.style.opacity="1",setTimeout(()=>{l&&(l.textContent=d),a&&(a.style.color=c.bone)},1800)}};if(n.value==="dead-deal"){let u=a?.textContent??"";Je(_=>{y(_)},()=>{a&&(a.textContent=u)});return}if(n.value==="appointment-booked"){let u=a?.textContent??"";try{(await qe({contactId:o,tenantId:p})).ok?await y():Ze(()=>{y()},()=>{a&&(a.textContent=u)})}catch(_){h.warn("Appointment sanity check failed; firing anyway:",_),await y()}return}await y()}),r.appendChild(s)}),e.addEventListener("click",n=>{n.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",n=>{t.contains(n.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},Xe=o=>{document.getElementById(de)?.remove();let t=document.createElement("div");t.id=de,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,e.appendChild(r),document.body.appendChild(e);let n=()=>e.remove(),s=()=>{n(),t()};e.addEventListener("click",a=>{a.target===e&&s()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",s),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),o()});let i=a=>{a.key==="Escape"&&(document.removeEventListener("keydown",i),s())};document.addEventListener("keydown",i)},he=o=>{let t=document.createElement("div");t.id=U,t.dataset.contactId=o,t.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
