...
`,L=`
  ...
  flex-shrink: 0;
`;var D="ws-crm-claim-release-pill",rt=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},It=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=D,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?l.emerald:l.amber;return e.style.cssText=et(r),e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,e.title=t==="claim"?"Take ownership of this lead":"Return this lead to the pool",e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{b.info(`Claim/Release pill clicked for contact ${o}: ${t}`);let i=e.lastElementChild,s=i?.textContent??"";i&&(i.textContent="Working\u2026"),e.style.opacity="0.7",e.disabled=!0,setTimeout(()=>{i&&(i.textContent=s),e.style.opacity="1",e.disabled=!1},700)}),e},Nt=async(o,t=1e4)=>{let e=Date.now();for(;Date.now()-e<t;){let r=rt(o);if(r)return r;await new Promise(a=>setTimeout(a,150))}return null},ot=(o,t)=>{let e=It({contactId:t,mode:"claim"}),r=document.createElement("span");r.id=`${D}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(e),o.appendChild(r)},Ht=async()=>{let o=E();if(!o){document.getElementById(D)?.remove();return}let t=document.getElementById(D);if(!(t&&t.dataset.contactId===o.contactId)){t&&t.remove();try{let e=await C("#record-details-lhs",{timeoutMs:1e4}),r=await Nt(e),a=E();if(!a||a.contactId!==o.contactId)return;if(!r){b.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}ot(r,o.contactId),b.debug(`Mounted Claim/Release pill next to name for ${o.contactId}`);let i=null;i=new MutationObserver(async()=>{if(!(E()?.contactId===o.contactId)){i?.disconnect();return}if(document.getElementById(D))return;let n=rt(e);n&&(ot(n,o.contactId),b.debug("Re-mounted Claim/Release pill after React wipe"))}),i.observe(e,{childList:!0,subtree:!0})}catch(e){b.warn("Could not mount Claim/Release pill:",e)}}},at=()=>{Ht()};var Mt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set";var Rt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Bt=()=>{let o=window.WS_CRM_USER_ID;if(o&&/^[A-Za-z0-9]{15,30}$/.test(o))return o;let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let r of e)if(typeof r=="string"&&/^[A-Za-z0-9]{15,30}$/.test(r))return r;try{let r=localStorage.getItem("user");if(r){let a=JSON.parse(r),i=a.id??a._id;if(i&&/^[A-Za-z0-9]{15,30}$/.test(i))return i}for(let a of["tokenUser","currentUser","ghl_user"]){let i=localStorage.getItem(a);if(i)try{let s=JSON.parse(i),n=s.id??s._id;if(n&&/^[A-Za-z0-9]{15,30}$/.test(n))return n}catch{}}}catch{}return null},Ot=async({contactId:o,locationId:t,status:e,userId:r,deadReason:a})=>{let i={call_status:e,triggered_by_user_id:r??""};a&&(i.dead_reason=a);let n=await fetch(Mt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:o,location:{id:t},customData:i}),credentials:"omit"});if(!n.ok)throw new Error(`Webhook returned ${n.status} ${n.statusText}`)},Pt=async({contactId:o,tenantId:t})=>{let e=await fetch(Rt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t,contact_id:o}),credentials:"omit"});if(!e.ok)throw new Error(`Sanity check returned ${e.status} ${e.statusText}`);return await e.json()};var O="ws-crm-contact-actions-bar",Gt="ws-crm-call-status-dropdown",nt="ws-crm-offer-made-modal",$t=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],Vt=[".central-panel","[class*='central-panel']"],Ut=async()=>{for(let o of Vt)try{let t=await C(o,{timeoutMs:3e3});if(t)return t}catch{}try{return(await C("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},zt=o=>{let t=document.createElement("div");t.id=Gt,t.style.cssText="position: relative; flex-shrink: 0;";let e=document.createElement("button");e.type="button",e.style.cssText=L,e.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,$t.forEach(a=>{let i=document.createElement("button");i.type="button",i.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var Ft=()=>{b.info(`Loaded v${R} (built ${B})`),bt(),Et(),tt(),Q(o=>{b.debug("Page handler firing for:",o),St(),at(),dt()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ft):Ft();})();
