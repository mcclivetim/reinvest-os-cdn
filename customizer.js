/* Wholesaling CRM Customizer v0.15.10 — built 2026-05-30T20:08:21.423Z */
"use strict";var WholesalingCRMCustomizer=(()=>{var je="[wholesaling-crm-customizer]",Kr="__WS_CRM_DEBUG",Yr=()=>!!window[Kr],c={info(...t){console.log(je,...t)},warn(...t){console.warn(je,...t)},error(...t){console.error(je,...t)},debug(...t){Yr()&&console.log(je,"[debug]",...t)}};var To=/(?:\/contacts\/detail\/|\/contacts\/|\/objects\/contact\/detail\/|\/objects\/contact\/)([A-Za-z0-9]{16,})(?:[/?#]|$)/,Dt=/\/v2\/location\/([A-Za-z0-9]+)/,Xr=t=>{let e=t.match(To);if(!e)return null;let o=t.match(Dt);return{contactId:e[1],locationId:o?o[1]:null,source:"url"}},Jr=()=>{let t=["#contact-conversation-panel a[href*='/contacts/detail/']",".central-panel a[href*='/contacts/detail/']"];for(let e of t){let o=document.querySelector(e);if(!o)continue;let r=o.getAttribute("href")||o.href||"",n=r.match(To);if(!n)continue;let a=r.match(Dt)||window.location.pathname.match(Dt);return{contactId:n[1],locationId:a?a[1]:null,source:"dom"}}return null},C=()=>{let t=Xr(window.location.pathname);return t||Jr()};var So=()=>{let t=window.location.pathname;return t.includes("/contacts/detail/")?"contact-detail":t.includes("/contacts/smart-list")||t.includes("/contacts/")?"contact-list":t.includes("/opportunities/detail/")?"opportunity-detail":t.includes("/opportunities/")?"opportunity-list":t.includes("/calendars/")?"calendar":t.includes("/dashboard")?"dashboard":t.includes("/conversations")?"conversations":t.includes("/settings")?"settings":"other"},Ye="",Ft=null,me="other",$t=new Set,Ke=()=>{let t=C()?.contactId??null;if(!(!(window.location.pathname!==Ye)&&!(t!==Ft))){Ye=window.location.pathname,Ft=t,me=So(),c.debug("Page/context changed:",me,Ye,"contact:",t);for(let r of $t)try{r(me)}catch(n){c.error("Subscriber threw:",n)}}},Co=t=>($t.add(t),setTimeout(()=>t(me),0),()=>{$t.delete(t)}),Io=()=>{me=So(),Ye=window.location.pathname,Ft=C()?.contactId??null,c.debug("Router init, current page:",me);let t=history.pushState.bind(history),e=history.replaceState.bind(history);history.pushState=function(o,r,n){t(o,r,n),Ke()},history.replaceState=function(o,r,n){e(o,r,n),Ke()},window.addEventListener("popstate",Ke),setInterval(Ke,1e3)};var Xe="0.15.10",Je="2026-05-30T20:08:21.424Z";var I=(t,e={})=>{let{timeoutMs:o=8e3,pollMs:r=100,root:n=document}=e;return new Promise((a,i)=>{let l=n.querySelector(t);if(l){a(l);return}let d,b,h=()=>{m&&m.disconnect(),d!==void 0&&window.clearTimeout(d),b!==void 0&&window.clearInterval(b)},m=new MutationObserver(()=>{let p=n.querySelector(t);p&&(h(),a(p))});m.observe(document.body,{childList:!0,subtree:!0}),b=window.setInterval(()=>{let p=n.querySelector(t);p&&(h(),a(p))},r),d=window.setTimeout(()=>{h(),c.warn(`waitForElement timed out for selector: ${t}`),i(new Error(`Selector not found within ${o}ms: ${t}`))},o)})};var s={obsidian:"#0A0D12",graphite:"#12161D",slate:"#1A1F28",steel:"#252C36",bone:"#EDEEF0",ash:"#9098A3",coolGray:"#5A6470",emerald:"#0FB57E",emeraldBright:"#14C98B",blue:"#4B8BF5",amber:"#E8A33C",crimson:"#D43F4A",emeraldGlow:"rgba(15, 181, 126, 0.12)",emeraldBorder:"rgba(15, 181, 126, 0.3)",blueGlow:"rgba(75, 139, 245, 0.12)",amberGlow:"rgba(232, 163, 60, 0.12)",crimsonGlow:"rgba(212, 63, 74, 0.12)"},x={sm:"4px",md:"6px",lg:"10px",pill:"999px"},y={sans:"'Geist', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",mono:"'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace"},Ot={purple:"#8B5CF6",green:s.emerald,orange:s.amber,red:s.crimson},Ao=t=>{let e=t==="emerald"?s.emeraldGlow:t==="amber"?s.amberGlow:s.slate,o=t==="emerald"?s.emeraldBorder:t==="amber"?"rgba(232, 163, 60, 0.3)":s.steel,r=t==="emerald"?s.emerald:t==="amber"?s.amber:s.bone;return`
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: 4px 10px;
...
`,D=`
  ...
  flex-shrink: 0;
`;var Qr=/^[A-Za-z0-9]{15,30}$/,Ee=t=>typeof t=="string"&&Qr.test(t),Zr=t=>{try{let e=t.split(".");if(e.length!==3)return null;let o=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=o+"=".repeat((4-o.length%4)%4);return JSON.parse(atob(r))}catch{return null}},en=()=>{let t=window.WS_CRM_USER_ID;return Ee(t)?t:null},tn=()=>{let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let o of e)if(Ee(o))return o;return null},on=()=>{try{let t=localStorage.getItem("refreshedToken");if(!t)return null;let e=t.startsWith('"')?t.slice(1,-1):t,o=Zr(e);if(!o)return null;let r=o.uid;if(Ee(r))return r;let n=o.claims;if(n&&Ee(n.user_id))return n.user_id}catch{}return null},rn=()=>{try{let t=["user","tokenUser","currentUser","ghl_user"];for(let e of t){let o=localStorage.getItem(e);if(o)try{let r=JSON.parse(o),n=r.id??r._id;if(Ee(n))return n}catch{}}}catch{}return null},L=()=>en()||tn()||on()||rn()||null;var Fo="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",nn=`${Fo}/active-follow-up-get`,an=`${Fo}/active-follow-up-set`,le="ws-crm-active-follow-up-button",Nt={amber:{label:"Add to Active Follow Up",bg:s.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:s.amber,clickable:!0,dotColor:s.amber},blue:{label:"In Active Follow Up",bg:s.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:s.blue,clickable:!1,dotColor:s.blue},gray:{label:"Not in Follow Up",bg:s.slate,border:s.steel,color:s.coolGray,clickable:!1,dotColor:s.coolGray},loading:{label:"Active Follow Up\u2026",bg:s.slate,border:s.steel,color:s.ash,clickable:!1,dotColor:s.ash},error:{label:"Follow Up status unavailable",bg:s.slate,border:s.steel,color:s.ash,clickable:!1,dotColor:s.crimson}},sn=()=>{let t=C();return!t||!t.locationId?null:{contactId:t.contactId,tenantId:t.locationId}},ln=async t=>{let e=await fetch(nn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},dn=async(t,e)=>{let o=await fetch(an,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!o.ok)throw new Error(`Set webhook returned ${o.status} ${o.statusText}`);return await o.json()},de=(t,e)=>{let o=Nt[e];t.dataset.state=e,t.disabled=!o.clickable,t.style.cursor=o.clickable?"pointer":"default",t.style.background=o.bg,t.style.borderColor=o.border,t.style.color=o.color;let r=t.querySelector(".ws-afu-dot");r&&(r.style.background=o.dotColor);let n=t.querySelector(".ws-afu-label");n&&(n.textContent=o.label)},Lo=t=>{let e=document.createElement("button");return e.type="button",e.id=le,e.dataset.contactId=t,e.style.cssText=`
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 6px 12px;
  ...
  `,de(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=sn();if(!(!r||r.contactId!==t)){de(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await dn(r,L()),a=n&&n.state&&n.state in Nt?n.state:"blue";de(e,a),c.info(`Active Follow Up set for ${t} \u2192 ${a}`)}catch(n){c.error("Active Follow Up Set webhook failed",n),de(e,"error"),setTimeout(()=>void Qe(e,r),4e3)}}}),e},Qe=async(t,e)=>{de(t,"loading");try{let o=await ln(e),r=o&&o.state&&o.state in Nt?o.state:"gray",n=document.getElementById(le);if(!n||n.dataset.contactId!==e.contactId)return;de(n,r)}catch(o){c.warn("Active Follow Up Get webhook failed",o),de(t,"error")}},cn=[".central-panel","[class*='central-panel']"],Pt="ws-crm-contact-actions-bar",bn=async()=>{for(let t of cn)try{let e=await I(t,{timeoutMs:3e3});if(e)return e}catch{}try{return(await I("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},hn=async(t,e)=>{let o=e.querySelector(`#${Pt}`);if(!o)try{o=await I(`#${Pt}`,{timeoutMs:3e3,root:e})}catch{o=null}if(o){let a=e.querySelector("#ws-crm-afu-wrap");return a&&a.remove(),o.appendChild(t),{container:o,placement:"actions-bar"}}let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
      ...
      flex-shrink: 0;
    `;let a=e.querySelector(`#${Pt}`);a&&a.parentElement===e?e.insertBefore(n,a.nextSibling):e.insertBefore(n,e.firstChild)}return n.appendChild(t),{container:n,placement:"fallback"}},pn=()=>{let t=document.querySelector("[data-testid='CENTRALPANEL_NAME']");if(!t)return null;let e=t;for(let o=0;o<8&&!(!e||!e.parentElement);o++){let r=e.parentElement,n=(r.className||"").toString();if(n.includes("justify-between")&&n.includes("border-b")){let a=r.children[1];return a instanceof HTMLElement?a:r}e=r}return null},mn=(t,e)=>{t.style.marginRight="4px",e.insertBefore(t,e.firstChild)},un=async t=>{if(t==="url"){let o=await bn();return o?{kind:"contact-detail",target:o}:null}let e=pn();return e?{kind:"conversations",target:e}:null},Mo=async(t,e)=>{if(e.kind==="contact-detail"){let{placement:o}=await hn(t,e.target);return o}return mn(t,e.target),"conversations-header"},_n=async()=>{let t=C();if(!t||!t.locationId){document.getElementById(le)?.remove(),document.getElementById("ws-crm-afu-wrap")?.remove();return}let e={contactId:t.contactId,tenantId:t.locationId},o=document.getElementById(le);if(o&&o.dataset.contactId===e.contactId){Qe(o,e);return}o&&o.remove();let r=await un(t.source);if(!r){c.warn(`Active Follow Up: no mount target on ${t.source}-source page`);return}let n=C();if(!n||n.contactId!==e.contactId)return;let a=Lo(e.contactId),i=await Mo(a,r);c.debug(`Active Follow Up mounted (${r.kind}/${i}) for ${e.contactId}`),Do();let l=document.getElementById(le)??a;Qe(l,e);let d=new MutationObserver(()=>{let b=document.getElementById(le);if(!(C()?.contactId===e.contactId)){d.disconnect();return}if(!b){let m=Lo(e.contactId);Mo(m,r).then(()=>{Do(),Qe(m,e)}),c.debug(`Active Follow Up re-mounted after wipe for ${e.contactId}`)}});d.observe(r.target,{childList:!0})},Te=null,$o=()=>{let t=C()?.contactId??null;if(Te&&Te.contactId===t)return;let e=_n().finally(()=>{Te?.promise===e&&(Te=null)});Te={contactId:t,promise:e}},Do=()=>{let t=document.querySelectorAll(`#${le}`);if(!(t.length<=1)){c.debug(`Active Follow Up: found ${t.length} duplicate buttons, keeping first, removing ${t.length-1}`);for(let e=1;e<t.length;e++)t[e].remove()}};var re="ws-crm-claim-release-pill",vn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-get",yn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-set",gn=async t=>{let e=await fetch(vn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.locationId,contact_id:t.contactId,triggered_by_user_id:t.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Get returned ${e.status} ${e.statusText}`);return await e.json()},fn=async t=>{let e=await fetch(yn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.locationId,contact_id:t.contactId,triggered_by_user_id:t.userId,action:t.action}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Set returned ${e.status} ${e.statusText}`);return await e.json()},No=t=>{let e=Array.from(t.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},ue=(t,e)=>{let o=e==="claim"?"emerald":"amber",r=e==="claim"?s.emerald:s.amber;t.dataset.mode=e,t.style.cssText=Ao(o),t.title=e==="claim"?"Take ownership of this lead":"Return this lead to the pool",t.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `},wn=({contactId:t,mode:e})=>{let o=document.createElement("button");return o.id=re,o.type="button",o.dataset.contactId=t,ue(o,e),o.addEventListener("mouseenter",()=>{o.style.transform="translateY(-1px)"}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)"}),o.addEventListener("click",async()=>{let r=o.dataset.mode??"claim",n=C();if(!n||!n.locationId||n.contactId!==t)return;let a=L();if(!a){c.warn("Claim/Release click: no user ID detected (login session not exposing it)");let l=o.querySelector(".ws-cr-label");l&&(l.textContent="Sign-in needed"),setTimeout(()=>ue(o,r),2500);return}c.info(`Claim/Release click \u2014 contact=${t} mode=${r} user=${a}`);let i=r==="claim"?"release":"claim";ue(o,i),o.disabled=!0,o.style.opacity="0.8";try{let l=await fn({contactId:t,locationId:n.locationId,userId:a,action:r}),d=l?.mode==="claim"||l?.mode==="release"?l.mode:i;ue(o,d),c.info(`Claim/Release done \u2192 new mode: ${d}`)}catch(l){c.error("Claim/Release Set webhook failed",l),ue(o,r);let d=o.querySelector(".ws-cr-label");if(d){let b=d.textContent??"";d.textContent="Try again",setTimeout(()=>{d.textContent==="Try again"&&(d.textContent=b)},2500)}}finally{o.disabled=!1,o.style.opacity="1"}}),o},Oo=async(t,e)=>{try{let o=await gn({contactId:e.contactId,locationId:e.locationId,userId:L()}),r=document.getElementById(re);if(!r||r.dataset.contactId!==e.contactId)return;let n=o?.mode==="release"?"release":"claim";ue(r,n)}catch(o){c.warn("Claim/Release Get failed; pill stays in default 'claim' mode",o)}},xn=async(t,e=1e4)=>{let o=Date.now();for(;Date.now()-o<e;){let r=No(t);if(r)return r;await new Promise(n=>setTimeout(n,150))}return null},Po=(t,e)=>{let o=wn({contactId:e,mode:"claim"}),r=document.createElement("span");r.id=`${re}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(o),t.appendChild(r)},kn=async()=>{let t=C();if(!t){document.getElementById(re)?.remove();return}let e=document.getElementById(re);if(!(e&&e.dataset.contactId===t.contactId)){e&&e.remove();try{let o=await I("#record-details-lhs",{timeoutMs:1e4}),r=await xn(o),n=C();if(!n||n.contactId!==t.contactId)return;if(!r){c.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}Po(r,t.contactId),c.debug(`Mounted Claim/Release pill next to name for ${t.contactId}`);let a=document.getElementById(re);a&&t.locationId&&Oo(a,{contactId:t.contactId,locationId:t.locationId});let i=null;i=new MutationObserver(async()=>{if(!(C()?.contactId===t.contactId)){i?.disconnect();return}if(document.getElementById(re))return;let d=No(o);if(d){Po(d,t.contactId);let b=document.getElementById(re);b&&t.locationId&&Oo(b,{contactId:t.contactId,locationId:t.locationId}),c.debug("Re-mounted Claim/Release pill after React wipe")}}),i.observe(o,{childList:!0,subtree:!0})}catch(o){c.warn("Could not mount Claim/Release pill:",o)}}},Ho=()=>{kn()};var En="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/type",Se=new Map,Bo=async(t,e)=>{if(!t)return null;if(Se.has(t))return Se.get(t)??null;if(!e)return c.warn(`Cannot fetch Contact Type for ${t} \u2014 tenant_id missing from URL context`),null;let o=new AbortController,r=setTimeout(()=>o.abort(),1500);try{let n=`${En}?contact_id=${encodeURIComponent(t)}&tenant_id=${encodeURIComponent(e)}`,a=await fetch(n,{method:"GET",headers:{Accept:"application/json"},signal:o.signal});if(clearTimeout(r),!a.ok)return c.warn(`Contact Type fetch returned ${a.status} for ${t}`),Se.set(t,null),null;let l=((await a.json()).contact_type||"").toLowerCase().trim()||null;return Se.set(t,l),c.debug(`Resolved Contact Type for ${t}: ${l??"(unset)"}`),l}catch(n){return clearTimeout(r),c.warn(`Contact Type fetch failed for ${t}:`,n),Se.set(t,null),null}};var Ro=t=>t==="buyer"?"buyer":t==="seller"?"seller":t==="agent"||t==="closing_agent"||t==="closing_office"||t==="vendor"?"other":"seller";var Wo="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set",Tn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/buyer-call-status/set";var Sn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Cn=async({contactId:t,locationId:e,status:o,userId:r,deadReason:n,webhookUrl:a,kind:i})=>{let l=a||Wo,d;if(i==="buyer"){let h=jo.find(p=>p.label===o),m=h?h.value:o.toLowerCase().replace(/\s+/g,"-");d={tenant_id:e,contact_id:t,new_status:m,triggered_by_user_id:r??""}}else{let h={call_status:o,triggered_by_user_id:r??""};n&&(h.dead_reason=n),d={contact_id:t,location:{id:e},customData:h}}let b=await fetch(l,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d),credentials:"omit"});if(!b.ok)throw new Error(`Webhook returned ${b.status} ${b.statusText}`)},In=async({contactId:t,tenantId:e})=>{let o=await fetch(Sn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,contact_id:t}),credentials:"omit"});if(!o.ok)throw new Error(`Sanity check returned ${o.status} ${o.statusText}`);return await o.json()};var _e="ws-crm-contact-actions-bar",An="ws-crm-call-status-dropdown",Go="ws-crm-offer-made-modal",Ln=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],jo=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"vm-left",label:"VM Left",color:"purple"},{value:"qualified-buyer",label:"Qualified Buyer",color:"green"},{value:"upgrade-to-vip",label:"Upgrade to VIP",color:"green"},{value:"walkthrough-scheduled",label:"Walkthrough Scheduled",color:"green"},{value:"made-an-offer",label:"Made an Offer",color:"green"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"wrong-market",label:"Wrong Market",color:"orange"},{value:"no-longer-buying",label:"No Longer Buying",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"blacklisted",label:"Blacklisted",color:"red"}],Mn=[".central-panel","[class*='central-panel']","[data-testid='central-panel']","[data-test='central-panel']",".contact-detail-page .central-panel",".contact-detail-page [class*='central']"],Dn=async()=>{for(let t of Mn)try{let e=await I(t,{timeoutMs:2e3});if(e)return c.debug(`[actions-bar] mount target found via selector: ${t}`),e}catch{}try{let t=await I("#contact-conversation-panel",{timeoutMs:2e3});if(t.parentElement)return c.debug("[actions-bar] mount target found via #contact-conversation-panel.parentElement"),t.parentElement}catch{}for(let t of["[id*='conversation-panel']","[class*='conversation-panel']","[class*='contact-detail'] [class*='panel']","main [class*='central']","main [class*='panel']:not([class*='sidebar']):not([class*='nav'])"])try{let e=await I(t,{timeoutMs:1500});if(e)return c.debug(`[actions-bar] mount target via fallback: ${t}`),e.parentElement??e}catch{}return c.warn("[actions-bar] no mount target found via any selector \u2014 URL:",window.location.pathname),null},Fn=({contactId:t,kind:e})=>{let o=e==="buyer"?jo:Ln,r=e==="buyer"?Tn:Wo,n=e==="buyer"?"Buyer Call Status":"Seller Call Status",a=document.createElement("div");a.id=An,a.style.cssText="position: relative; flex-shrink: 0;";let i=document.createElement("button");i.type="button",i.style.cssText=D,i.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `,o.forEach(d=>{let b=document.createElement("button");b.type="button",b.style.cssText=`
    ...
    `,b.innerHTML=`
      <span style="
        ...
        height: 8px;
        border-radius: 50%;
        background: ${Ot[d.color]};
        flex-shrink: 0;
      "></span>
      <span>${d.label}</span>
    `,b.addEventListener("mouseenter",()=>{b.style.background=s.slate}),b.addEventListener("mouseleave",()=>{b.style.background="transparent"}),b.addEventListener("click",async()=>{c.info(`Call Status set to "${d.label}" (${d.value}, ${d.color}) for contact ${t}`);let h=i.firstElementChild,m=i.children[1],p=i.children[2];h&&(h.style.background=Ot[d.color]),m&&(m.textContent=d.label),l.style.display="none";let f=C()?.locationId,k=L();if(!f){c.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),m&&(m.textContent=`${d.label} (no tenant!)`),m&&(m.style.color=s.amber);return}if(e==="seller"&&d.color==="green"&&!k){c.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),m&&(m.textContent=`${d.label} (no user!)`),m&&(m.style.color=s.amber);return}let v=p?.textContent??"\u25BE",_=async g=>{p&&(p.textContent="\u2026"),i.style.opacity="0.75";try{await Cn({contactId:t,locationId:f,status:d.label,userId:k,deadReason:g,webhookUrl:r,kind:e}),p&&(p.textContent="\u2713"),setTimeout(()=>{p&&(p.textContent=v),i.style.opacity="1"},900)}catch(E){c.warn("Call Status webhook failed:",E),p&&(p.textContent="\u2717"),m&&(m.style.color=s.amber),i.style.opacity="1",setTimeout(()=>{p&&(p.textContent=v),m&&(m.style.color=s.bone)},1800)}};if(e==="seller"&&d.value==="dead-deal"){let g=m?.textContent??"";Nn(E=>{_(E)},()=>{m&&(m.textContent=g)});return}if(e==="seller"&&d.value==="appointment-booked"){let g=m?.textContent??"";try{(await In({contactId:t,tenantId:f})).ok?await _():Hn(()=>{_()},()=>{m&&(m.textContent=g)})}catch(E){c.warn("Appointment sanity check failed; firing anyway:",E),await _()}return}await _()}),l.appendChild(b)}),i.addEventListener("click",d=>{d.stopPropagation(),l.style.display=l.style.display==="none"?"block":"none"}),document.addEventListener("click",d=>{a.contains(d.target)||(l.style.display="none")}),a.appendChild(i),a.appendChild(l),a},$n="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/offer-made/set",On=async t=>{let e=await fetch($n,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId,kind:t.kind,amount:t.amount,expires_at:t.expiresAt,triggered_by_user_id:t.userId}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},Pn=(t,e="seller")=>{document.getElementById(Go)?.remove();let o=e==="seller",r=o?"Record Your Offer":"Record Buyer's Offer",n=o?"Attributes the offer to you for KPI tracking. Updates the ACQ opportunity's offer fields.":"Logs the buyer's offer on their Marketing Tracker card. Advances them to 'Made an Offer'.",a=o?s.emerald:s.amber,i=document.createElement("div");i.id=Go,i.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `;let d=new Date,b=new Date(d.getTime()+5*24*60*60*1e3).toISOString().slice(0,10),h=o?`
    <label style="
      display: block;
      font-family: ${y.mono};
      font-size: 10px;
    ...
    <div style="
      ...
      margin-bottom: 16px;
    ">
      <span style="color: ${s.ash}; font-family: ${y.mono}; font-size: 14px;">$</span>
  ...
  `,i.appendChild(l),document.body.appendChild(i);let m=()=>i.remove();i.addEventListener("click",k=>{k.target===i&&m()});let p=l.querySelector("#ws-offer-error"),u=l.querySelector("#ws-offer-cancel"),f=l.querySelector("#ws-offer-submit");u?.addEventListener("click",m),f?.addEventListener("click",async()=>{let k=(l.querySelector("#ws-offer-amount")?.value??"").trim(),v=o?l.querySelector("#ws-offer-expires")?.value??"":"";if(!k||Number(k)<=0){p&&(p.textContent="Enter a positive offer amount."),c.warn("Offer Made submit blocked \u2014 invalid amount");return}let _=Number(k);if(_<100){p&&(p.textContent=`That looks low \u2014 did you mean $${(_*1e3).toLocaleString("en-US")}?`);return}let E=window.location.pathname.match(/\/v2\/location\/([A-Za-z0-9]+)/),T=E?E[1]:null;if(!T){p&&(p.textContent="Couldn't resolve tenant from URL. Refresh and try again.");return}p&&(p.textContent=""),f&&(f.disabled=!0,f.textContent="Recording\u2026",f.style.opacity="0.7"),u&&(u.disabled=!0);try{let w=await On({tenantId:T,contactId:t,kind:e,amount:_,expiresAt:v||null,userId:L()});if(!w.ok){p&&(p.textContent=w.error??"Something went wrong."),f&&(f.disabled=!1,f.textContent="Record Offer",f.style.opacity="1"),u&&(u.disabled=!1);return}c.info(`Offer Made (${e}) recorded for contact ${t}: $${_} on ${w.opp_pipeline} opp (stage: ${w.opp_stage_name??"?"})`),m()}catch(w){c.error("Offer Made webhook failed",w),p&&(p.textContent=w instanceof Error?w.message:"Network error \u2014 try again."),f&&(f.disabled=!1,f.textContent="Record Offer",f.style.opacity="1"),u&&(u.disabled=!1)}}),setTimeout(()=>{l.querySelector("#ws-offer-amount")?.focus()},0)},Uo="ws-crm-dead-deal-modal",Nn=(t,e)=>{document.getElementById(Uo)?.remove();let o=document.createElement("div");o.id=Uo,o.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,o.appendChild(r),document.body.appendChild(o);let n=()=>o.remove(),a=()=>{n(),e()};o.addEventListener("click",l=>{l.target===o&&a()}),r.querySelector("#ws-dead-cancel")?.addEventListener("click",a);let i=()=>{let l=r.querySelector("#ws-dead-reason"),d=r.querySelector("#ws-dead-error"),b=(l?.value??"").trim();if(!b){d&&(d.textContent="Please enter a reason."),l?.focus();return}if(b.length<4){d&&(d.textContent="Please enter at least a few words."),l?.focus();return}n(),t(b)};r.querySelector("#ws-dead-submit")?.addEventListener("click",i),r.querySelector("#ws-dead-reason")?.addEventListener("keydown",l=>{l.key==="Enter"&&(l.metaKey||l.ctrlKey)&&(l.preventDefault(),i()),l.key==="Escape"&&a()}),setTimeout(()=>{r.querySelector("#ws-dead-reason")?.focus()},0)},zo="ws-crm-appointment-sanity-modal",Hn=(t,e)=>{document.getElementById(zo)?.remove();let o=document.createElement("div");o.id=zo,o.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,o.appendChild(r),document.body.appendChild(o);let n=()=>o.remove(),a=()=>{n(),e()};o.addEventListener("click",l=>{l.target===o&&a()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",a),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),t()});let i=l=>{l.key==="Escape"&&(document.removeEventListener("keydown",i),a())};document.addEventListener("keydown",i)},qo=(t,e)=>{let o=document.createElement("div");if(o.id=_e,o.dataset.contactId=t,o.dataset.contactKind=e,o.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    ...
    border-bottom: 1px solid ${s.steel};
    flex-shrink: 0;
  `,e==="seller"||e==="buyer"){let r=document.createElement("button");r.type="button",r.style.cssText=te,r.textContent="Offer Made",r.addEventListener("click",()=>Pn(t,e)),o.appendChild(r),o.appendChild(Fn({contactId:t,kind:e}))}return o},Ht=!1,Bn=()=>{document.querySelectorAll(`[id="${_e}"]`).forEach(t=>t.remove())},Ko=async()=>{if(c.debug("[actions-bar:01] ensureMounted called; url:",window.location.pathname),Ht){c.debug("[actions-bar:00-LOCKED] ensureMounted already in-flight \u2014 skip");return}Ht=!0;try{await Rn()}finally{Ht=!1}},Rn=async()=>{let t=C();if(!t){c.debug("[actions-bar:02-BAIL] no contact context (URL didn't match patterns AND no in-page contact link). URL:",window.location.pathname),Bn();return}c.debug(`[actions-bar:02] ctx resolved \u2014 contactId=${t.contactId} locationId=${t.locationId??"(null)"} source=${t.source}`);let e="seller";try{let b=await Bo(t.contactId,t.locationId),h=Ro(b);h==="buyer"?e="buyer":h==="other"&&(e="other"),c.debug(`[actions-bar:03] kind resolved \u2014 kind=${e} (raw type=${b??"(null)"})`)}catch(b){c.warn("[actions-bar:03-FAIL] Contact Type resolution threw \u2014 defaulting to seller:",b)}let o=C();if(!o||o.contactId!==t.contactId){c.debug("[actions-bar:04-BAIL] context changed during Contact Type fetch \u2014 aborting mount");return}let r=Array.from(document.querySelectorAll(`[id="${_e}"]`));if(r.length===1&&r[0].dataset.contactId===t.contactId&&r[0].dataset.contactKind===e){c.debug(`[actions-bar:05-SKIP] bar already mounted for contact=${t.contactId} kind=${e} \u2014 no-op`);return}r.length>0&&(c.debug(`[actions-bar:05] removing ${r.length} stale bar(s) before remount`),r.forEach(b=>b.remove()));let n=await Dn();if(!n){c.warn("[actions-bar:06-FAIL] findMountTarget returned null \u2014 no .central-panel or fallback selector matched within timeout. URL:",window.location.pathname);return}c.debug(`[actions-bar:06] mount target found \u2014 tagName=${n.tagName} class="${n.className}"`);let a=C();if(!a||a.contactId!==t.contactId){c.debug("[actions-bar:07-BAIL] context changed during findMountTarget wait \u2014 aborting mount");return}let i;try{i=qo(t.contactId,e)}catch(b){c.error("[actions-bar:08-FAIL] buildBar threw:",b);return}i.dataset.mountTarget=n.className||"central-panel";try{n.insertBefore(i,n.firstChild)}catch(b){c.error("[actions-bar:09-FAIL] insertBefore threw:",b);return}if(!document.getElementById(_e)){c.warn("[actions-bar:10-DETACHED] bar inserted but not in document tree \u2014 target was likely re-rendered by Vue. Will retry next page-change.");return}c.debug(`[actions-bar:10-OK] bar mounted + verified in document for contact=${t.contactId} kind=${e}`);let d=null;d=new MutationObserver(()=>{if(!(C()?.contactId===t.contactId)){d?.disconnect();return}let h=document.querySelectorAll(`[id="${_e}"]`);if(h.length>1){for(let m=1;m<h.length;m++)h[m].remove();c.debug(`[actions-bar:WATCHDOG] trimmed ${h.length-1} duplicate bar(s)`);return}if(h.length===0){let m=document.querySelector(".central-panel")||n,p=qo(t.contactId,e);p.dataset.mountTarget=i.dataset.mountTarget??"";try{m.insertBefore(p,m.firstChild),c.debug(`[actions-bar:WATCHDOG] re-mounted after wipe for ${t.contactId}`)}catch(u){c.warn("[actions-bar:WATCHDOG-FAIL] re-mount threw:",u)}}}),d.observe(n,{childList:!0})},Vo=null,Gn=()=>{Vo===null&&(Vo=window.setInterval(()=>{let t=C();t&&(document.getElementById(_e)||(c.debug("[actions-bar:HEALER] no bar on contact page \u2014 re-triggering mount for",t.contactId),Ko()))},3e3))},Yo=()=>{Gn(),Ko()};var Ze="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",Un=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),Ce="data-ws-crm-favicon",Bt=t=>t.hasAttribute(Ce),Xo=t=>{if(t.tagName!=="LINK")return!1;let e=(t.getAttribute("rel")??"").toLowerCase();return Un.has(e)},et=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{Xo(r)&&!Bt(r)&&r.remove()});let e=document.head.querySelector(`link[${Ce}="1"]`);e?e.href!==Ze&&(e.href=Ze):(e=document.createElement("link"),e.setAttribute(Ce,"1"),e.rel="icon",e.type="image/png",e.href=Ze,document.head.appendChild(e),c.debug("Favicon installed"));let o=document.head.querySelector(`link[${Ce}="apple"]`);o||(o=document.createElement("link"),o.setAttribute(Ce,"apple"),o.rel="apple-touch-icon",o.href=Ze,document.head.appendChild(o))},Jo=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)et();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),et())},50)}new MutationObserver(r=>{let n=!1;for(let a of r)a.addedNodes.forEach(i=>{i instanceof HTMLLinkElement&&Xo(i)&&!Bt(i)&&(n=!0)}),a.removedNodes.forEach(i=>{i instanceof HTMLLinkElement&&Bt(i)&&(n=!0)});n&&et()}).observe(document.head,{childList:!0,subtree:!1});let e=0,o=window.setInterval(()=>{e+=1,et(),e>=5&&window.clearInterval(o)},1e3)};var zn=["Acquisitions","Transaction Coordination","Disposition","Marketing Tracker"],qn=/^(?:\d+\)\s*)?(Acquisitions|Transaction Coordination|Disposition|Marketing Tracker)\s*$/,Vn=".hr-card.hr-modal.crm-opportunities-modal",M=()=>{let t=document.querySelector(Vn);if(!t)return null;let e=t.querySelectorAll(".hr-base-selection-overlay__wrapper, .hr-base-selection");for(let o of e){let r=o.textContent?.trim()??"";if(!r)continue;let n=r.match(qn);if(n){let a=n[1];if(zn.includes(a))return a}}return null},Y=async(t=2500)=>{let e=Date.now();for(;Date.now()-e<t;){let o=M();if(o!==null)return o;await new Promise(r=>setTimeout(r,100))}return null};var Wn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/offer-made/set",z="ws-crm-mt-offer-made-button",Qo="ws-crm-mt-offer-made-modal",jn=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Kn=/\/v2\/location\/([A-Za-z0-9]+)/,rt=()=>{let t=window.location.pathname,e=t.match(jn);if(!e)return null;let o=t.match(Kn);return{oppId:e[1],locationId:o?o[1]:null}},Yn=async t=>{let e=await fetch(Wn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.locationId,mt_opp_id:t.oppId,kind:"buyer",amount:t.amount,triggered_by_user_id:t.userId}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},tt=()=>{document.getElementById(Qo)?.remove()},Xn=t=>{tt();let e=document.createElement("div");e.id=Qo,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(o),document.body.appendChild(e);let r=o.querySelector("#ws-mt-offer-amount"),n=o.querySelector("#ws-mt-offer-error"),a=o.querySelector("#ws-mt-offer-cancel"),i=o.querySelector("#ws-mt-offer-submit");setTimeout(()=>r?.focus(),50);let l=d=>{d.key==="Enter"?(d.preventDefault(),i?.click()):d.key==="Escape"&&(d.preventDefault(),a?.click())};o.addEventListener("keydown",l),e.addEventListener("click",d=>{d.target===e&&tt()}),a?.addEventListener("click",()=>tt()),i?.addEventListener("click",async()=>{if(!r||!n||!i)return;let d=r.value.trim();if(!d||Number(d)<=0){n.textContent="Enter a positive offer amount.",r.focus();return}let b=Number(d);if(b<100){n.textContent=`That looks low \u2014 did you mean $${(b*1e3).toLocaleString("en-US")}?`,r.focus();return}n.textContent="",i.disabled=!0,i.textContent="Recording\u2026",i.style.opacity="0.7",a&&(a.disabled=!0);try{let h=await Yn({oppId:t.oppId,locationId:t.locationId,amount:b,userId:L()});if(!h.ok){n.textContent=h.error??"Something went wrong.",i.disabled=!1,i.textContent="Record Offer",i.style.opacity="1",a&&(a.disabled=!1);return}tt(),t.onSuccess()}catch(h){c.error("MT Offer Made webhook failed",h),n.textContent=h instanceof Error?h.message:"Network error \u2014 try again.",i.disabled=!1,i.textContent="Record Offer",i.style.opacity="1",a&&(a.disabled=!1)}})},Jn=t=>{let e=document.createElement("button");return e.id=z,e.type="button",e.dataset.oppId=t,e.style.cssText=D,e.title="Record this buyer's offer on the property and advance to 'Made an Offer'",e.innerHTML=`
    <span style="
      display: inline-block;
      width: 8px;
      height: 8px;
      border-radius: 50%;
      background: ${s.emerald};
      box-shadow: 0 0 6px ${s.emerald};
      flex-shrink: 0;
    "></span>
    <span class="ws-mt-om-label">Mark Offer Made</span>
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{let o=rt();!o||!o.locationId||o.oppId!==t||Xn({oppId:o.oppId,locationId:o.locationId,onSuccess:()=>{let r=e.querySelector(".ws-mt-om-label");r&&(r.textContent="Offer Recorded"),e.style.background=s.emeraldGlow,e.style.color=s.emeraldBright,e.style.borderColor=s.emeraldBorder,setTimeout(()=>{r&&r.textContent==="Offer Recorded"&&(r.textContent="Mark Offer Made",e.style.cssText=D)},4e3)}})}),e},Qn=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],Zn=async()=>{for(let t of Qn)try{let e=await I(t,{timeoutMs:2e3});if(e)return e}catch{}return null},ot=!1,zt=async()=>{if(!ot){ot=!0;try{await ea()}finally{ot=!1}}},ea=async()=>{let t=rt();if(!t||!t.locationId){document.getElementById(z)?.remove(),document.getElementById(`${z}-wrap`)?.remove(),Ae();return}let e=document.getElementById(z);if(e&&e.dataset.oppId===t.oppId){let l=M();l!==null&&l!=="Marketing Tracker"&&(e.remove(),document.getElementById(`${z}-wrap`)?.remove(),Ae());return}e&&(e.remove(),document.getElementById(`${z}-wrap`)?.remove());let o=await Zn();if(!o){c.warn("MT Offer Made: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await Y();if(r!=="Marketing Tracker"){c.debug(`[mt-offer] gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=rt();if(!n||n.oppId!==t.oppId)return;let a=Jn(t.oppId),i=document.createElement("span");i.id=`${z}-wrap`,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,o.classList.contains("description")&&(o.style.display="flex",o.style.alignItems="center",o.style.gap="16px",o.style.flexWrap="wrap"),i.appendChild(a),o.appendChild(i),c.debug(`Mounted MT Offer Made button on opp ${t.oppId}`),ta(t.oppId)},Ie=null,Ut=null,Ae=()=>{Ie?.disconnect(),Ie=null,Ut=null},ta=t=>{if(Ie&&Ut===t)return;Ae();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(Ut=t,Ie=new MutationObserver(()=>{let o=rt();if(!o||o.oppId!==t){Ae();return}let r=M();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(z)?.remove(),document.getElementById(`${z}-wrap`)?.remove(),Ae();return}document.getElementById(z)||zt()}),Ie.observe(e,{childList:!0,subtree:!0}))},Rt=null,Gt,oa=()=>{Rt||(Rt=new MutationObserver(()=>{Gt===void 0&&(Gt=window.setTimeout(()=>{Gt=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(z);t&&!e&&!ot&&zt()},300))}),Rt.observe(document.body,{childList:!0,subtree:!0}))},Zo=()=>{oa(),zt()};var ra="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/mt/not-interested",q="ws-crm-not-interested-button",na=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,aa=/\/v2\/location\/([A-Za-z0-9]+)/,at=()=>{let t=window.location.pathname,e=t.match(na);if(!e)return null;let o=t.match(aa);return{oppId:e[1],locationId:o?o[1]:null}},sa=async t=>{let e=await fetch(ra,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.locationId,mt_opp_id:t.oppId,triggered_by_user_id:t.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Not Interested webhook returned ${e.status} ${e.statusText}`);return await e.json()},ia=t=>{let e=document.createElement("button");return e.id=q,e.type="button",e.dataset.oppId=t,e.style.cssText=D,e.title="Close this Marketing Tracker opp as Not Interested for THIS property only",e.innerHTML=`
    <span style="
      display: inline-block;
      width: 8px;
      height: 8px;
      ...
      box-shadow: 0 0 6px ${s.crimson};
      flex-shrink: 0;
    "></span>
    <span class="ws-ni-label">Mark Not Interested</span>
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let o=at();if(!o||!o.locationId||o.oppId!==t)return;let r=L(),n=e.querySelector(".ws-ni-label");e.disabled=!0,e.style.opacity="0.7",n&&(n.textContent="Closing\u2026");try{let a=await sa({oppId:o.oppId,locationId:o.locationId,userId:r});if(a?.ok)n&&(n.textContent="Marked Not Interested"),e.style.background=s.crimsonGlow,e.style.color=s.crimson,c.info(`Not Interested set for opp ${t} \u2192 ${a.new_stage}`);else throw new Error("Webhook returned ok=false")}catch(a){c.error("Not Interested webhook failed",a),n&&(n.textContent="Try again"),e.disabled=!1,e.style.opacity="1",setTimeout(()=>{n&&n.textContent==="Try again"&&(n.textContent="Mark Not Interested")},3e3)}}),e},la=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],da=async()=>{for(let t of la)try{let e=await I(t,{timeoutMs:2e3});if(e)return e}catch{}return null},nt=!1,jt=async()=>{if(!nt){nt=!0;try{await ca()}finally{nt=!1}}},ca=async()=>{let t=at();if(!t||!t.locationId){document.getElementById(q)?.remove(),document.getElementById(`${q}-wrap`)?.remove(),Me();return}let e=document.getElementById(q);if(e&&e.dataset.oppId===t.oppId){let l=M();l!==null&&l!=="Marketing Tracker"&&(e.remove(),document.getElementById(`${q}-wrap`)?.remove(),Me());return}e&&(e.remove(),document.getElementById(`${q}-wrap`)?.remove());let o=await da();if(!o){c.warn("Not Interested: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await Y();if(r!=="Marketing Tracker"){c.debug(`[not-interested] gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=at();if(!n||n.oppId!==t.oppId)return;let a=ia(t.oppId),i=document.createElement("span");i.id=`${q}-wrap`,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,o.classList.contains("description")&&(o.style.display="flex",o.style.alignItems="center",o.style.gap="16px",o.style.flexWrap="wrap"),i.appendChild(a),o.appendChild(i),c.debug(`Mounted Not Interested button on opp ${t.oppId}`),ba(t.oppId)},Le=null,Wt=null,Me=()=>{Le?.disconnect(),Le=null,Wt=null},ba=t=>{if(Le&&Wt===t)return;Me();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(Wt=t,Le=new MutationObserver(()=>{let o=at();if(!o||o.oppId!==t){Me();return}let r=M();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(q)?.remove(),document.getElementById(`${q}-wrap`)?.remove(),Me();return}document.getElementById(q)||jt()}),Le.observe(e,{childList:!0,subtree:!0}))},qt=null,Vt,ha=()=>{qt||(qt=new MutationObserver(()=>{Vt===void 0&&(Vt=window.setTimeout(()=>{Vt=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(q);t&&!e&&!nt&&jt()},300))}),qt.observe(document.body,{childList:!0,subtree:!0}))},er=()=>{ha(),jt()};var pa="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/property/listing-price-changed",V="ws-crm-reduce-price-button",tr="ws-crm-reduce-price-modal",ma=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,ua=/\/v2\/location\/([A-Za-z0-9]+)/,lt=()=>{let t=window.location.pathname,e=t.match(ma);if(!e)return null;let o=t.match(ua);return{oppId:e[1],locationId:o?o[1]:null}},_a=async t=>{let e=await fetch(pa,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.locationId,dispo_opp_id:t.oppId,new_price:t.newPrice,triggered_by_user_id:t.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Reduce Price webhook returned ${e.status} ${e.statusText}`);let o=await e.text();try{let r=JSON.parse(o);if(r?.ok===!1&&r?.error)throw new Error(r.error);return r}catch(r){if(r instanceof Error&&/returned/.test(r.message))throw r;return{}}},st=()=>{document.getElementById(tr)?.remove()},va=t=>{st();let e=document.createElement("div");e.id=tr,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(o),document.body.appendChild(e);let r=o.querySelector("#ws-reduce-price-input"),n=o.querySelector("#ws-reduce-error"),a=o.querySelector("#ws-reduce-cancel"),i=o.querySelector("#ws-reduce-submit");setTimeout(()=>r?.focus(),50),r?.addEventListener("input",()=>{let b=r.value.replace(/[^0-9.]/g,"").split(".");r.value=b[0]+(b.length>1?"."+b.slice(1).join(""):"")});let l=d=>{d.key==="Enter"?(d.preventDefault(),i?.click()):d.key==="Escape"&&(d.preventDefault(),a?.click())};o.addEventListener("keydown",l),e.addEventListener("click",d=>{d.target===e&&st()}),a?.addEventListener("click",()=>{st()}),i?.addEventListener("click",async()=>{if(!r||!n||!i)return;let d=r.value.trim();if(!d){n.textContent="Enter a new listing price.",r.focus();return}let b=parseFloat(d);if(!Number.isFinite(b)||b<=0){n.textContent="Price must be a positive number.",r.focus();return}if(b<1e3){n.textContent=`That looks low \u2014 did you mean $${Math.round(b*1e3).toLocaleString("en-US")}?`,r.focus();return}n.textContent="",i.disabled=!0,i.textContent="Reducing\u2026",i.style.opacity="0.7",a&&(a.disabled=!0);try{await _a({oppId:t.oppId,locationId:t.locationId,newPrice:b,userId:L()}),st(),t.onSuccess()}catch(h){c.error("Reduce Price webhook failed",h),n.textContent=h instanceof Error?h.message:"Something went wrong. Try again.",i.disabled=!1,i.textContent="Reduce Price",i.style.opacity="1",a&&(a.disabled=!1)}})},ya=(t,e)=>{let o=document.createElement("button");return o.id=V,o.type="button",o.dataset.oppId=t,o.style.cssText=D,o.title="Set a new listing price and notify active + previously-withdrew buyers",o.innerHTML=`
    <span style="
      display: inline-block;
      width: 8px;
      height: 8px;
      border-radius: 50%;
      background: ${s.amber};
      box-shadow: 0 0 6px ${s.amber};
      flex-shrink: 0;
    "></span>
    <span class="ws-rp-label">Reduce Price and Alert Buyers</span>
  `,o.addEventListener("mouseenter",()=>{o.style.transform="translateY(-1px)"}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)"}),o.addEventListener("click",()=>{let r=lt();!r||!r.locationId||r.oppId!==t||va({oppId:r.oppId,locationId:r.locationId,onSuccess:()=>{let n=o.querySelector(".ws-rp-label"),a=o.querySelector(":scope > span:first-child");n&&(n.textContent="Price Reduced \u2014 Buyers Alerted"),o.style.background=s.emeraldGlow,o.style.color=s.emeraldBright,o.style.borderColor=s.emeraldBorder,a&&(a.style.background=s.emerald,a.style.boxShadow=`0 0 6px ${s.emerald}`),setTimeout(()=>{n&&n.textContent==="Price Reduced \u2014 Buyers Alerted"&&(n.textContent="Reduce Price and Alert Buyers",o.style.cssText=D,a&&(a.style.background=s.amber,a.style.boxShadow=`0 0 6px ${s.amber}`))},4e3)}})}),o},ga=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],fa=async()=>{for(let t of ga)try{let e=await I(t,{timeoutMs:2e3});if(e)return e}catch{}return null},it=!1,Jt=async()=>{if(!it){it=!0;try{await wa()}finally{it=!1}}},wa=async()=>{let t=lt();if(!t||!t.locationId){document.getElementById(V)?.remove(),document.getElementById(`${V}-wrap`)?.remove(),Fe();return}let e=document.getElementById(V);if(e&&e.dataset.oppId===t.oppId){let l=M();l!==null&&l!=="Disposition"&&(e.remove(),document.getElementById(`${V}-wrap`)?.remove(),Fe());return}e&&(e.remove(),document.getElementById(`${V}-wrap`)?.remove());let o=await fa();if(!o){c.warn("Reduce Price: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await Y();if(r!=="Disposition"){c.debug(`[reduce-price] gated off \u2014 pipeline="${r}" (expected Disposition)`);return}let n=lt();if(!n||n.oppId!==t.oppId)return;let a=ya(t.oppId,t.locationId),i=document.createElement("span");i.id=`${V}-wrap`,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,o.classList.contains("description")&&(o.style.display="flex",o.style.alignItems="center",o.style.gap="16px",o.style.flexWrap="wrap"),i.appendChild(a),o.appendChild(i),c.debug(`Mounted Reduce Price button on opp ${t.oppId}`),xa(t.oppId)},De=null,Xt=null,Fe=()=>{De?.disconnect(),De=null,Xt=null},xa=t=>{if(De&&Xt===t)return;Fe();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(Xt=t,De=new MutationObserver(()=>{let o=lt();if(!o||o.oppId!==t){Fe();return}let r=M();if(r!==null&&r!=="Disposition"){document.getElementById(V)?.remove(),document.getElementById(`${V}-wrap`)?.remove(),Fe();return}document.getElementById(V)||Jt()}),De.observe(e,{childList:!0,subtree:!0}))},Kt=null,Yt,ka=()=>{Kt||(Kt=new MutationObserver(()=>{Yt===void 0&&(Yt=window.setTimeout(()=>{Yt=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(V);t&&!e&&!it&&Jt()},300))}),Kt.observe(document.body,{childList:!0,subtree:!0}))},or=()=>{ka(),Jt()};var ar="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/tc/send-to-title-co",ne="ws-crm-send-to-closing-office-button",ce="ws-crm-send-to-title-co-wrap",bt="ws-crm-stc-dropdown",Ea=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Ta=/\/v2\/location\/([A-Za-z0-9]+)/,ht=()=>{let t=window.location.pathname,e=t.match(Ea);if(!e)return null;let o=t.match(Ta);return{oppId:e[1],locationId:o?o[1]:null}},Sa=5*60*1e3,sr=t=>`ws-crm-stc-cache:${t}`,Ca=t=>{try{let e=window.localStorage.getItem(sr(t));if(!e)return null;let o=JSON.parse(e);return!o.ts||Date.now()-o.ts>Sa||!Array.isArray(o.checks)||o.checks.length===0?null:o}catch{return null}},Ia=(t,e)=>{try{window.localStorage.setItem(sr(t),JSON.stringify(e))}catch{}},Aa=async(t,e)=>{try{let o=await fetch(ar,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:t,triggered_by_user_id:L()??null,dry_run:!0}),credentials:"omit"});return o.ok?await o.json():(c.warn(`[stc] dry_run returned HTTP ${o.status}`),null)}catch(o){return c.warn("[stc] dry_run fetch failed:",o),null}},La=async(t,e,o)=>{let r=await fetch(ar,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:t,triggered_by_user_id:L()??null,confirm_resend:o}),credentials:"omit"});if(!r.ok&&r.status!==200)throw new Error(`HTTP ${r.status}`);return await r.json()},Ma=t=>{let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(!e)return!1;let o=e.querySelectorAll("a, button, li, [role='tab'], [role='menuitem'], .crm-opportunities-modal-content [class*='tab']");for(let n of o)if((n.textContent?.trim()??"")===t)return n.click(),!0;let r=e.querySelectorAll("*");for(let n of r){if(n.children.length>1)continue;if((n.textContent?.trim()??"")===t)return(n.closest("a, button, li, [role='tab']")||n.parentElement||n).click(),!0}return c.warn(`[stc] folder tab not found for "${t}"`),!1},rr=t=>{let e=document.getElementById(t);if(!e){c.warn(`[stc] field ${t} not found in DOM \u2014 can't scroll to it`);return}e.scrollIntoView({behavior:"smooth",block:"center"});let o=e.style.boxShadow,r=e.style.transition;e.style.transition="box-shadow 0.3s ease, background-color 0.3s ease",e.style.boxShadow=`0 0 0 3px ${s.emerald}, 0 0 18px ${s.emerald}`,window.setTimeout(()=>{e.style.boxShadow=o,window.setTimeout(()=>{e.style.transition=r},400)},2800)},Da=(t,e)=>{if(e&&Ma(e)){window.setTimeout(()=>rr(t),300);return}rr(t)},W=()=>{document.getElementById(bt)?.remove()},dt=(t,e,o)=>{W();let r=document.createElement("div");r.id=bt;let n=o.getBoundingClientRect();r.style.cssText=`
    position: fixed;
    top: ${n.bottom+6}px;
    ...
    padding: 14px 16px;
    font-family: ${y.sans};
  `;let a=t.length,i=t.filter(u=>u.filled).length,l=i===a,d=document.createElement("div");d.innerHTML=`
    <div style="
      font-family: ${y.mono};
      font-size: 10px;
      letter-spacing: 0.14em;
      text-transform: uppercase;
      color: ${l?s.emerald:s.amber};
      margin-bottom: 4px;
    ">${l?"Ready to send":"Required fields"}</div>
    <div style="font-size: 14px; font-weight: 500; color: ${s.bone}; margin-bottom: 6px;">${i} of ${a} ready</div>
    ${e?`<div style="
            font-size: 11px;
            color: ${s.amber};
            margin: 6px 0 4px;
  ...
  `,r.appendChild(d);let b=["TC","ACQ","MT","DISPO","Property"],h={};t.forEach(u=>{h[u.group]||(h[u.group]=[]),h[u.group].push(u)});let m=[...b.filter(u=>h[u]),...Object.keys(h).filter(u=>!b.includes(u))];for(let u of m){let f=h[u],k=document.createElement("div");k.style.cssText="margin-top: 12px;";let v=document.createElement("div");v.style.cssText=`
      font-family: ${y.mono};
      font-size: 9px;
      ...
      padding-bottom: 4px;
      border-bottom: 1px solid ${s.steel};
    `,v.textContent=u,k.appendChild(v),f.forEach(_=>{let g=document.createElement("div"),E=!_.filled&&_.navigable&&_.field_id;g.style.cssText=`
        display: flex;
        align-items: center;
        gap: 10px;
        padding: 6px 8px;
        border-radius: ${x.sm};
        font-size: 12px;
        color: ${_.filled?s.bone:s.ash};
        ${E?"cursor: pointer;":""}
        transition: background 0.1s ease;
      `,E&&(g.addEventListener("mouseenter",()=>{g.style.background=s.slate}),g.addEventListener("mouseleave",()=>{g.style.background="transparent"}),g.addEventListener("click",S=>{S.stopPropagation(),_.field_id&&(Da(_.field_id,_.folder),W())}));let T=_.filled?`<span style="color: ${s.emerald}; font-size: 14px; line-height: 1; flex-shrink: 0;">\u2713</span>`:`<span style="color: ${s.amber}; font-size: 14px; line-height: 1; flex-shrink: 0;">\u25CB</span>`,w=E?`<span style="margin-left: auto; color: ${s.ash}; font-size: 11px;">\u2192</span>`:"";g.innerHTML=`${T}<span style="flex: 1;">${_.label}</span>${w}`,k.appendChild(g)}),r.appendChild(k)}document.body.appendChild(r);let p=u=>{let f=u.target;!r.contains(f)&&!o.contains(f)&&(W(),document.removeEventListener("click",p))};window.setTimeout(()=>document.addEventListener("click",p),0)},X=(t,e)=>{let o=t.querySelector(".ws-stc-label"),r=t.querySelector(".ws-stc-dot"),n=t.querySelector(".ws-stc-chevron"),a,i,l=s.bone,d,b="Send to Closing Office",h="",m="pointer",p="1";switch(e.status){case"checking":a="transparent",i=s.steel,l=s.ash,d=s.coolGray,b="Checking\u2026",m="wait",p="0.8";break;case"ready":a=s.emerald,i=s.emerald,l="#0a0e14",d="#0a0e14",b=e.previouslySentAt?"Re-send to Closing Office":"Send to Closing Office";break;case"incomplete":{let u=e.checks?.filter(k=>k.filled).length??0,f=e.checks?.length??0;a="transparent",i=s.amber,l=s.amber,d=s.amber,b=`${u} of ${f} fields ready`,h="\u25BE";break}case"sending":a=s.emerald,i=s.emerald,l="#0a0e14",d="#0a0e14",b="Sending\u2026",m="wait",p="0.8";break;case"sent":a=s.emerald,i=s.emerald,l="#0a0e14",d="#0a0e14",b="Sent \u2713";break;case"error":a="transparent",i=s.amber,l=s.amber,d=s.amber,b=e.message??"Try again";break}t.style.background=a,t.style.borderColor=i,t.style.color=l,t.style.cursor=m,t.style.opacity=p,r&&(r.style.background=d,r.style.boxShadow=e.status==="ready"||e.status==="sent"?`0 0 6px ${d}`:""),o&&(o.textContent=b),n&&(n.textContent=h)},Fa=(t,e)=>{let o=document.createElement("button");o.id=ne,o.type="button",o.dataset.oppId=t;let r=Ca(t),n=r?{status:r.checks.every(v=>v.filled)?"ready":"incomplete",checks:r.checks,previouslySentAt:r.previously_sent_at}:{status:"checking"};o.style.cssText=`
    ${te}
    background: transparent;
    border: 1px solid ${s.steel};
    color: ${s.ash};
  ...
  `,X(o,n);let a=new Set,i=async()=>{if(n.status==="sending")return;let v=await Aa(t,e);if(!v){n={status:"error",message:"Check failed \u2014 click to retry"},X(o,n);return}if(v.rate_limited){u=Date.now()+p,n={status:"error",checks:n.checks,message:"Rate-limited by GHL \u2014 will retry in 5m"},X(o,n),c.warn(`[stc] dry_run rate-limited \u2014 cooling down until ${new Date(u).toLocaleTimeString()}`);return}if(!Array.isArray(v.checks)||v.checks.length===0){n={status:"error",message:v.error||"Check failed \u2014 click to retry"},X(o,n),c.warn("[stc] dry_run returned no checks:",v);return}let _=v.pipeline_name;if(_&&_!=="Transaction Coordination"){c.debug(`[stc] dry_run reports pipeline="${_}" (not TC) \u2014 retracting button + caching`),pt.add(t),document.getElementById(ne)?.remove(),document.getElementById(ce)?.remove(),W(),Oe();return}let g=v.checks;if(n.checks&&n.checks.length===v.checks.length){let T=new Set;g=v.checks.map((w,S)=>n.checks[S]?.filled&&!w.filled?a.has(w.label)?w:(T.add(w.label),{...w,filled:!0}):w),a=T}else a=new Set;n={status:g.every(T=>T.filled)?"ready":"incomplete",checks:g,previouslySentAt:v.previously_sent_at??null},Ia(t,{checks:g,previously_sent_at:v.previously_sent_at??null,pipeline_name:_,ts:Date.now()}),X(o,n),document.getElementById(bt)&&dt(g,v.previously_sent_at,o)},l=async()=>{let v=!!n.previouslySentAt;n={status:"sending",checks:n.checks,previouslySentAt:n.previouslySentAt},X(o,n),W();try{let _=await La(t,e,v);if(_.needs_confirm){n={status:"ready",checks:_.checks,previouslySentAt:_.previously_sent_at},X(o,n),_.checks&&dt(_.checks,_.previously_sent_at,o);return}if(_.ok){n={status:"sent",checks:_.checks,previouslySentAt:new Date().toISOString()},X(o,n),window.setTimeout(()=>{f()},3500);return}n={status:"error",checks:_.checks,message:_.error||"Validation failed"},X(o,n),_.checks&&dt(_.checks,_.previously_sent_at,o)}catch(_){c.error("[stc] send failed:",_),n={status:"error",message:"Network error \u2014 click to retry"},X(o,n)}};o.addEventListener("click",()=>{switch(n.status){case"ready":l();break;case"incomplete":{document.getElementById(bt)?W():n.checks&&dt(n.checks,n.previouslySentAt,o);break}case"error":case"sent":f();break}});let d,b=!1,h=Date.now(),m=1e4,p=5*60*1e3,u=0,f=async()=>{if(!b){if(Date.now()<u){c.debug(`[stc] dry_run suppressed \u2014 rate-limit cooldown until ${new Date(u).toLocaleTimeString()}`);return}b=!0,h=Date.now();try{await i()}finally{b=!1}}};f();let k=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(k){let v=new MutationObserver(()=>{if(d!==void 0||b)return;let g=Date.now()-h,E=Math.max(1500,m-g);d=window.setTimeout(()=>{d=void 0,n.status!=="sending"&&f()},E)});v.observe(k,{childList:!0,subtree:!0,characterData:!0}),o.dataset.observerActive="1";let _=window.setInterval(()=>{document.body.contains(o)||(v.disconnect(),d!==void 0&&window.clearTimeout(d),window.clearInterval(_))},5e3)}return o},$a=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],Oa=async()=>{for(let t of $a)try{let e=await I(t,{timeoutMs:1500});if(e)return c.debug(`[send-to-title-co] mount target: ${t}`),e}catch{}return c.warn(`[send-to-title-co] no mount target found. URL: ${window.location.pathname}`),null},pt=new Set,nr=null,Pa=t=>{t&&t!==nr&&(pt.clear(),nr=t)},ct=!1,to=async()=>{if(ct){c.debug("[stc] mount:00-LOCKED ensureMounted already in-flight \u2014 skip");return}ct=!0;try{await Na()}catch(t){c.error("[stc] ensureMountedImpl threw:",t)}finally{ct=!1}},Na=async()=>{c.debug("[stc] mount:01 ensureMountedImpl entered");let t=ht();if(!t||!t.locationId){c.debug(`[stc] mount:02-BAIL no ctx or no locationId. ctx=${JSON.stringify(t)}`),document.getElementById(ne)?.remove(),document.getElementById(ce)?.remove(),W();return}if(c.debug(`[stc] mount:02 ctx ok \u2014 oppId=${t.oppId} locId=${t.locationId}`),Pa(t.oppId),pt.has(t.oppId)){c.debug(`[stc] mount:02-BAIL opp ${t.oppId} previously confirmed non-TC this session`),document.getElementById(ne)?.remove(),document.getElementById(ce)?.remove(),W();return}let e=document.getElementById(ne);if(e&&e.dataset.oppId===t.oppId){c.debug("[stc] mount:03-SKIP existing button for same opp");let l=M();l!==null&&l!=="Transaction Coordination"&&(e.remove(),document.getElementById(ce)?.remove(),W(),Oe());return}e&&(c.debug("[stc] mount:03 removing stale existing button"),e.remove(),document.getElementById(ce)?.remove(),W()),c.debug("[stc] mount:04 calling findMountTarget");let o=await Oa();if(!o){c.debug("[stc] mount:04-BAIL findMountTarget returned null");return}c.debug(`[stc] mount:04 target found tag=${o.tagName} class="${o.className}"`);let r=M();if(r!==null&&r!=="Transaction Coordination"){c.debug(`[stc] mount:05-GATED DOM pipeline="${r}" \u2014 skipping mount`);return}let n=ht();if(!n||n.oppId!==t.oppId||!n.locationId){c.debug(`[stc] mount:06-BAIL ctx changed during awaits. was=${t.oppId} now=${n?.oppId}`);return}c.debug("[stc] mount:06 building button (pipeline gate will run via dry_run)");let a=Fa(t.oppId,n.locationId),i=document.createElement("span");i.id=ce,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,o.classList.contains("description")&&(o.style.display="flex",o.style.alignItems="center",o.style.gap="16px",o.style.flexWrap="wrap"),i.appendChild(a),o.appendChild(i),c.debug(`[stc] mount:07-OK Mounted on opp ${t.oppId}, inDoc=${document.body.contains(a)}`),Ha(t.oppId)},$e=null,eo=null,Oe=()=>{$e?.disconnect(),$e=null,eo=null},Ha=t=>{if($e&&eo===t)return;Oe();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(eo=t,$e=new MutationObserver(()=>{let o=ht();if(!o||o.oppId!==t){Oe();return}let r=M();if(r!==null&&r!=="Transaction Coordination"){document.getElementById(ne)?.remove(),document.getElementById(ce)?.remove(),W(),Oe();return}document.getElementById(ne)||to()}),$e.observe(e,{childList:!0,subtree:!0}))},Qt=null,Zt,Ba=()=>{Qt||(Qt=new MutationObserver(()=>{Zt===void 0&&(Zt=window.setTimeout(()=>{Zt=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(ne);if(t&&!e&&!ct){let o=ht();if(o&&pt.has(o.oppId))return;to()}},300))}),Qt.observe(document.body,{childList:!0,subtree:!0}))},ir=()=>{Ba(),to()};var Ra="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/search",Ga="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/opp/set-field",Ua="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/opp/get-field",cr="ldtyB2UXKi9vOp8nLUH7",za="WVD2gb9swLsgcdBEtDVq",j="ws-crm-closing-agent-picker-button",br="ws-crm-closing-agent-picker-modal",qa=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Va=/\/v2\/location\/([A-Za-z0-9]+)/,He=()=>{let t=window.location.pathname,e=t.match(qa);if(!e)return null;let o=t.match(Va);return{oppId:e[1],locationId:o?o[1]:null}},co=async t=>{let e=await fetch(Ra,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,query:t.query??"",contact_id:t.contactId??null,contact_type:t.contactType??null,limit:t.limit??20}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},lr=async t=>{let e=await fetch(Ga,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,opp_id:t.oppId,field_id:t.fieldId,value:t.value}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},Wa=async(t,e)=>{try{let o=await fetch(Ua,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,opp_id:t,field_id:cr}),credentials:"omit"});if(!o.ok)return null;let r=await o.json(),n=typeof r.value=="string"&&r.value.trim()?r.value.trim():null;if(!n||!/^[A-Za-z0-9]{16,}$/.test(n))return null;let a=await co({tenantId:e,contactId:n,limit:1});if(!a.ok||!a.results.length)return null;let i=a.results[0];return{id:i.id,name:i.name,companyName:i.companyName??""}}catch(o){return c.warn("[closing-agent] fetchCurrent failed:",o),null}},be=null,ja=60*60*1e3,bo=t=>`ws-crm-to-cache:${t}`,hr=t=>{try{let e=window.localStorage.getItem(bo(t));if(!e)return null;let o=JSON.parse(e);return!o.ts||Date.now()-o.ts>ja||typeof o.contactId!="string"||!o.contactId||typeof o.contactName!="string"||!o.contactName?null:o}catch{return null}},pr=(t,e,o)=>{try{window.localStorage.setItem(bo(t),JSON.stringify({contactId:e,contactName:o,ts:Date.now()}))}catch{}},Ka=t=>{try{window.localStorage.removeItem(bo(t))}catch{}},ao="ws-crm-closing-agent-field-link",Ya=()=>{let t=document.querySelector(".crm-opportunities-modal");t&&(t.querySelectorAll(`.${ao}`).forEach(e=>e.remove()),t.querySelectorAll("input[data-ws-closing-agent-decorated]").forEach(e=>{e.style.removeProperty("color"),e.style.removeProperty("caret-color"),e.style.removeProperty("visibility"),delete e.dataset.wsClosingAgentDecorated}))},oo=!1,ho=async t=>{if(oo)return;let e=mr();if(!e)return;let o=(e.value||"").trim();if(!o){c.info("[closing-agent] tryDecorate: input has empty value");return}if(!/^[A-Za-z0-9]{16,}$/.test(o)){c.info(`[closing-agent] tryDecorate: value '${o}' doesn't match contact-ID pattern (expecting 16+ alphanumeric)`);return}if(e.dataset.wsClosingAgentDecorated!==o){oo=!0;try{c.info(`[closing-agent] tryDecorate: looking up ${o}`);let r=await co({tenantId:t,contactId:o,limit:1});if(!r.ok||!r.results.length){c.warn(`[closing-agent] tryDecorate: contact ${o} not found`);return}let n=r.results[0];c.info(`[closing-agent] tryDecorate: applying ${n.name} (${n.id})`),so(n.id,n.name,t);let a=document.getElementById(j);if(a){ae(a,{status:"linked",contactName:n.name});let i=He();i&&pr(i.oppId,n.id,n.name)}}catch(r){c.warn("[closing-agent] tryDecorate lookup failed:",r)}finally{oo=!1}}},Xa=/^TC\s*[-‐–—]\s*Closing\s+Agent$/i,mr=()=>{let e=document.querySelectorAll("*"),o=[],r=[],n=[];for(let d of e){let b=(d.textContent||"").trim();Xa.test(b)?o.push(d):b.length<80&&/Title\s+Officer/i.test(b)&&r.push(b);for(let h of["aria-label","placeholder","data-label","title","name","data-test"]){let m=d.getAttribute(h);m&&/Title\s+Officer/i.test(m)&&n.push({tag:d.tagName.toLowerCase(),attr:h,value:m})}}if(c.info(`[closing-agent] findInput: ${o.length} regex-matched, ${r.length} near-text matches, ${n.length} attribute matches`),r.length&&c.warn("[closing-agent] near-text samples:",[...new Set(r)].slice(0,8)),n.length&&c.warn("[closing-agent] attribute matches:",n.slice(0,8)),o.length===0)return null;o.sort((d,b)=>d.querySelectorAll("*").length-b.querySelectorAll("*").length);let i=o[0].parentElement,l=0;for(;i&&l<8;){let d=i.querySelector("input");if(d)return c.info(`[closing-agent] findInput: located input ${l} levels up from label`),d;i=i.parentElement,l+=1}return c.warn("[closing-agent] findInput: label found but no <input> within 8 ancestors"),null},so=(t,e,o)=>{let r=mr();if(!r)return;let n=r.dataset.wsClosingAgentDecorated;if(n!==t){n&&r.parentElement?.querySelectorAll(`.${ao}`).forEach(i=>i.remove());{let a=r.parentElement;if(!a)return;r.dataset.wsClosingAgentDecorated=t,r.style.visibility="hidden",window.getComputedStyle(a).position==="static"&&(a.style.position="relative");let l=document.createElement("a");l.className=ao,l.href=`/v2/location/${o}/contacts/detail/${t}`,l.textContent=e,l.style.cssText=`
      position: absolute;
      inset: 0;
      display: flex;
      align-items: center;
      ...
      border-radius: ${x.sm};
      z-index: 2;
    `,l.addEventListener("mouseenter",()=>{l.style.textDecoration="underline"}),l.addEventListener("mouseleave",()=>{l.style.textDecoration="none"}),l.addEventListener("click",d=>{d.preventDefault(),d.stopPropagation(),window.location.href=l.href}),a.appendChild(l)}}},mt=()=>{document.getElementById(br)?.remove()},Ja=t=>{mt();let e=document.createElement("div");e.id=br,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,o.innerHTML=`
    ...
    <div id="ws-top-results"
      style="
        ...
        margin-bottom: 16px;
      ">
      <div style="padding: 16px; color: ${s.ash}; font-size: 13px; text-align: center;">
  ...
  `,e.appendChild(o),document.body.appendChild(e);let r=o.querySelector("#ws-top-search"),n=o.querySelector("#ws-top-results"),a=o.querySelector("#ws-top-cancel");setTimeout(()=>r?.focus(),50),e.addEventListener("click",b=>{b.target===e&&mt()}),a?.addEventListener("click",mt);let i,l="",d=async b=>{if(n&&b!==l){if(l=b,!b.trim()){n.innerHTML=`
        <div style="padding: 16px; color: ${s.ash}; font-size: 13px; text-align: center;">
          Start typing to search.
        </div>`;return}n.innerHTML=`
      <div style="padding: 16px; color: ${s.ash}; font-size: 13px; text-align: center;">
        Searching\u2026
      </div>`;try{let h=await co({tenantId:t.locationId,query:b,contactType:"closing_agent",limit:20});if(!h.ok||!Array.isArray(h.results)||h.results.length===0){n.innerHTML=`
          <div style="padding: 16px; color: ${s.ash}; font-size: 13px; text-align: center;">
            No results.
          ...
          transition: background 0.1s ease;
        `,p.innerHTML=`
          <div style="font-weight: 500;">${ut(m.name)}</div>
          <div style="color: ${s.ash}; font-size: 11px; margin-top: 2px;">
            ${ut(m.email||"(no email)")} \xB7 ${ut(m.phone||"no phone")}
            ${m.companyName?` \xB7 ${ut(m.companyName)}`:""}
          </div>
        `,p.addEventListener("mouseenter",()=>{p.style.background=s.slate}),p.addEventListener("mouseleave",()=>{p.style.background="transparent"}),p.addEventListener("click",()=>{mt(),t.onPick(m)}),n.appendChild(p)})}catch(h){c.error("Closing Agent picker search failed",h),n&&(n.innerHTML=`
          <div style="padding: 16px; color: ${s.crimson}; font-size: 13px; text-align: center;">
            Search failed. Try again.
          </div>`)}}};r?.addEventListener("input",()=>{let b=r.value;i!==void 0&&clearTimeout(i),i=window.setTimeout(()=>{d(b)},250)})},ut=t=>t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),ae=(t,e)=>{let o=t.querySelector(".ws-top-label"),r=t.querySelector(".ws-top-dot"),n="Link Closing Agent",a=s.amber,i=s.bone,l="transparent",d=s.steel,b="pointer";switch(e.status){case"loading":n="Loading Closing Agent\u2026",a=s.coolGray,i=s.ash,b="wait";break;case"empty":n="Link Closing Agent",a=s.amber;break;case"linked":n=e.contactName?`Closing Agent: ${e.contactName}`:"Closing Agent: \u2713",a=s.emerald,l=s.emeraldGlow,d=s.emeraldBorder,i=s.emeraldBright;break;case"saving":n="Linking\u2026",a=s.coolGray,i=s.ash,b="wait";break}o&&(o.textContent=n),r&&(r.style.background=a,r.style.boxShadow=`0 0 6px ${a}`),t.style.background=l,t.style.borderColor=d,t.style.color=i,t.style.cursor=b},Qa=(t,e)=>{let o=document.createElement("button");o.id=j,o.type="button",o.dataset.oppId=t,o.style.cssText=D,o.title="Pick the Closing Agent contact for this deal. Their email + phone will flow into the title-co handoff email.",o.innerHTML=`
  ...
  `;let r={status:"loading"};ae(o,r);let n=hr(t);return n&&(r={status:"linked",contactName:n.contactName},ae(o,r),be={oppId:t,contactId:n.contactId,contactName:n.contactName,tenantId:e}),(async()=>{let i=await Wa(t,e);i?(r={status:"linked",contactName:i.name},ae(o,r),be={oppId:t,contactId:i.id,contactName:i.name,tenantId:e},so(i.id,i.name,e)):n||(r={status:"empty"},ae(o,r),be?.oppId===t&&(be=null),Ya(),Ka(t))})(),o.addEventListener("mouseenter",()=>{r.status!=="loading"&&r.status!=="saving"&&(o.style.transform="translateY(-1px)")}),o.addEventListener("mouseleave",()=>{o.style.transform="translateY(0)"}),o.addEventListener("click",()=>{r.status==="loading"||r.status==="saving"||Ja({oppId:t,locationId:e,onPick:async i=>{r={status:"saving"},ae(o,r);try{let l=[lr({tenantId:e,oppId:t,fieldId:cr,value:i.id})];i.companyName&&i.companyName.trim()&&l.push(lr({tenantId:e,oppId:t,fieldId:za,value:i.companyName.trim()}));let b=(await Promise.all(l))[0];if(!b.ok)throw new Error(b.error??"Save failed");r={status:"linked",contactName:i.name},ae(o,r),be={oppId:t,contactId:i.id,contactName:i.name,tenantId:e},pr(t,i.id,i.name),so(i.id,i.name,e),c.info(`Closing Agent linked: ${i.name} (${i.id})${i.companyName?` \u2014 Company: ${i.companyName}`:""}`)}catch(l){c.error("Closing Agent link failed",l),r={status:"empty"},ae(o,r)}}})}),o},Za=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading"],es=async()=>{for(let t of Za)try{let e=await I(t,{timeoutMs:2e3});if(e)return e}catch{}return null},Pe=!1,vt=async()=>{if(!Pe){Pe=!0;try{await ts()}finally{Pe=!1}}},ts=async()=>{let t=He();if(!t||!t.locationId){document.getElementById(j)?.remove(),document.getElementById(`${j}-wrap`)?.remove(),_t();return}let e=document.getElementById(j);if(e&&e.dataset.oppId===t.oppId){let d=M();d!==null&&d!=="Transaction Coordination"&&(e.remove(),document.getElementById(`${j}-wrap`)?.remove(),_t());return}e&&(e.remove(),document.getElementById(`${j}-wrap`)?.remove());let o=M();if(o!==null&&o!=="Transaction Coordination")return;let r=await es();if(!r||!(hr(t.oppId)!==null||be!==null&&be.oppId===t.oppId)&&await Y()!=="Transaction Coordination")return;let a=He();if(!a||a.oppId!==t.oppId||!a.locationId)return;let i=Qa(t.oppId,a.locationId),l=document.createElement("span");l.id=`${j}-wrap`,l.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,r.classList.contains("description")&&(r.style.display="flex",r.style.alignItems="center",r.style.gap="16px",r.style.flexWrap="wrap"),l.appendChild(i),r.appendChild(l),c.debug(`Mounted Closing Agent picker on opp ${t.oppId}`),os(t.oppId),ho(a.locationId)},Ne=null,io=null,lo=null,_t=()=>{Ne?.disconnect(),Ne=null,io=null,lo=null},os=t=>{let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(Ne&&io===t&&lo===e||(_t(),io=t,lo=e,Ne=new MutationObserver(()=>{let o=He();if(!o||o.oppId!==t||!o.locationId){_t();return}document.getElementById(j)||vt(),ho(o.locationId)}),Ne.observe(e,{childList:!0,subtree:!0})))},ro=null,no,rs=()=>{ro||(ro=new MutationObserver(()=>{no===void 0&&(no=window.setTimeout(()=>{no=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(j);t&&!e&&!Pe&&vt()},300))}),ro.observe(document.body,{childList:!0,subtree:!0}))},dr,ns=()=>{dr===void 0&&(dr=window.setInterval(()=>{let t=He();if(!t||!t.locationId||!!!document.querySelector(".crm-opportunities-modal"))return;!!!document.getElementById(j)&&!Pe&&(c.info("[closing-agent] safety-net: button missing on opp page \u2014 remounting"),vt()),ho(t.locationId)},2e3))},ur=()=>{c.info("[closing-agent] mountClosingAgentPickerButton invoked"),rs(),ns(),vt()};var as="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contract/stage",uo="ws-crm-prepare-contract-button",_r="ws-crm-prepare-contract-menu",Q="ws-crm-prepare-contract-modal",vr="ws-crm-modal-date-style",Z="https://services.leadconnectorhq.com",wr=()=>{if(document.getElementById(vr))return;let t=document.createElement("style");t.id=vr,t.textContent=`
    #${Q} input[type="date"] { color-scheme: dark; }
  ...
  `,document.head.appendChild(t)},_o="__WS_CRM_CAPTURED_AUTH",ss=50*60*1e3,xr=()=>{try{let t=localStorage.getItem(_o);if(!t)return null;let e=JSON.parse(t);return e?.authorization?Date.now()-(e.capturedAt||0)>ss?(localStorage.removeItem(_o),null):e:null}catch{return null}},is=t=>{try{localStorage.setItem(_o,JSON.stringify(t))}catch{}},ee=()=>{let t=xr(),e=window;if(e.__WS_CRM_DIAG=e.__WS_CRM_DIAG||{},e.__WS_CRM_DIAG.authChecks=(e.__WS_CRM_DIAG.authChecks||0)+1,console.log(`[ws-crm-customizer] [auth-check #${e.__WS_CRM_DIAG.authChecks}]`,t?`present (captured ${Date.now()-t.capturedAt}ms ago, ${t.authorization.slice(0,20)}\u2026)`:"NULL \u2014 interceptor has not captured a token yet"),!t)throw new Error("GHL auth not yet captured. Try clicking around the page (refresh, open a contact) to trigger a GHL API call, then retry.");let o={Authorization:t.authorization,Version:"2021-07-28",Channel:"APP",Source:"WEB_USER"};return t.tokenId&&(o["token-id"]=t.tokenId),o},yr=!1,ls=()=>{if(yr)return;yr=!0;let t=window;t.__WS_CRM_DIAG=t.__WS_CRM_DIAG||{fetchCalls:0,xhrCalls:0,fetchAuthSeen:0,xhrAuthSeen:0,captured:0,lastSeenHeaders:null};let e=(l,d,b)=>{t.__WS_CRM_DIAG.lastSeenHeaders={url:l,headers:d,source:b};let h=d.authorization||d.Authorization;if(!h){t.__WS_CRM_DIAG.fetchCalls+t.__WS_CRM_DIAG.xhrCalls<=3&&console.log(`[ws-crm-customizer] [intercept:${b} no-auth]`,{url:l.split("?")[0],headerKeys:Object.keys(d)});return}b==="fetch"?t.__WS_CRM_DIAG.fetchAuthSeen++:t.__WS_CRM_DIAG.xhrAuthSeen++;let m=d["token-id"]||d["Token-Id"]||void 0,p=xr()?.authorization;is({authorization:h,tokenId:m,capturedAt:Date.now()}),p!==h&&(t.__WS_CRM_DIAG.captured++,console.log(`[ws-crm-customizer] [intercept:${b}] captured GHL auth from ${l.split("?")[0]} (${h.slice(0,24)}\u2026)`))},o=window.fetch;window.fetch=async function(...l){let d=typeof l[0]=="string"?l[0]:l[0]instanceof URL?l[0].toString():l[0].url,b=l[1]||{};if(d&&d.includes("services.leadconnectorhq.com")){t.__WS_CRM_DIAG.fetchCalls++;let h=b.headers;!h&&l[0]instanceof Request&&(h=l[0].headers);let m={};if(h instanceof Headers)h.forEach((p,u)=>m[u.toLowerCase()]=p);else if(Array.isArray(h))h.forEach(([p,u])=>m[p.toLowerCase()]=u);else if(h&&typeof h=="object")for(let p of Object.keys(h))m[p.toLowerCase()]=h[p];e(d,m,"fetch")}return o.apply(this,l)};let r=XMLHttpRequest.prototype,n=r.open,a=r.setRequestHeader,i=r.send;r.open=function(l,d,...b){return this.__wsCrmUrl=d,this.__wsCrmHeaders={},n.call(this,l,d,...b)},r.setRequestHeader=function(l,d){return this.__wsCrmHeaders&&typeof l=="string"&&(this.__wsCrmHeaders[l.toLowerCase()]=d),a.call(this,l,d)},r.send=function(l){let d=this.__wsCrmUrl,b=this.__wsCrmHeaders;return d&&d.includes("services.leadconnectorhq.com")&&b&&(t.__WS_CRM_DIAG.xhrCalls++,e(d,b,"xhr")),i.call(this,l)},c.debug("[prepare-contract] interceptors installed (fetch + XHR)")},kr=()=>{let t=C();return!t||!t.locationId?null:{contactId:t.contactId,tenantId:t.locationId}},ds=async t=>{let e=t.dataset;if(e.tenantId&&e.contactId)return{tenantId:e.tenantId,contactId:e.contactId,oppId:e.oppId||void 0};if(e.tenantId&&e.oppId){let o=await Rs(e.oppId,e.tenantId);return t.dataset.contactId=o,{tenantId:e.tenantId,contactId:o,oppId:e.oppId}}return kr()},cs=t=>{let e=(t||"").trim().toLowerCase();return/^\[?\s*aa\s*\]?\s*[-\s]/i.test(e)||/assignment.*(contract|agreement)/i.test(e)?"AA":/^\[?\s*pa\s*\]?\s*[-\s]/i.test(e)||/purchase.*(contract|agreement)/i.test(e)?"PA":"passthrough"},bs=async t=>{let e=`${Z}/proposals/templates/bulk?type.in%5B%5D=proposal&type.in%5B%5D=estimate&locationId.eq=${encodeURIComponent(t)}&skip=0&limit=10`,o=await fetch(e,{method:"GET",headers:ee()});if(!o.ok){let a="";try{a=await o.text()}catch{}let i=a?` \u2014 ${a.slice(0,200)}`:"";throw new Error(`Templates fetch returned ${o.status} ${o.statusText}${i}`)}let r=await o.json();console.log("[ws-crm-customizer] [templates response]",Array.isArray(r)?`array of ${r.length}`:`keys: ${Object.keys(r||{}).join(", ")}`,r);let n=[];return Array.isArray(r)?n=r:Array.isArray(r?.templates)?n=r.templates:Array.isArray(r?.items)?n=r.items:Array.isArray(r?.data)?n=r.data:Array.isArray(r?.results)?n=r.results:Array.isArray(r?.docs)?n=r.docs:r?.data&&Array.isArray(r.data.templates)?n=r.data.templates:r?.data&&Array.isArray(r.data.items)&&(n=r.data.items),n.map(a=>({id:(a._id??a.id??a.templateId)||"",name:a.name||a.title||"(unnamed template)",type:cs(a.name||a.title||"")})).filter(a=>!!a.id)},hs=async(t,e)=>{let o=await fetch(`${Z}/opportunities/pipelines?locationId=${encodeURIComponent(t)}`,{method:"GET",headers:ee()});if(!o.ok)throw new Error(`Pipelines fetch ${o.status}`);let n=((await o.json()).pipelines||[]).find(l=>/acquisition/i.test(l.name));if(!n)throw new Error("No Acquisition pipeline found on tenant");let a=await fetch(`${Z}/opportunities/search?location_id=${encodeURIComponent(t)}&pipeline_id=${encodeURIComponent(n.id)}&contact_id=${encodeURIComponent(e)}&status=open&limit=20`,{method:"GET",headers:ee()});if(!a.ok)throw new Error(`ACQ opp search ${a.status}`);return((await a.json()).opportunities||[]).map(l=>({id:l.id,name:l.name||"(unnamed opp)",propertyAddress:l.name||"",updatedAt:l.updatedAt||null})).sort((l,d)=>{let b=l.updatedAt?Date.parse(l.updatedAt):0;return(d.updatedAt?Date.parse(d.updatedAt):0)-b})},ps=async(t,e)=>{let o=await fetch(`${Z}/opportunities/pipelines?locationId=${encodeURIComponent(t)}`,{method:"GET",headers:ee()});if(!o.ok)throw new Error(`Pipelines fetch ${o.status}`);let n=((await o.json()).pipelines||[]).find(d=>/marketing\s*tracker/i.test(d.name));if(!n)throw new Error("No Marketing Tracker pipeline found on tenant");let a=await fetch(`${Z}/opportunities/search?location_id=${encodeURIComponent(t)}&pipeline_id=${encodeURIComponent(n.id)}&contact_id=${encodeURIComponent(e)}&status=open&limit=20`,{method:"GET",headers:ee()});if(!a.ok)throw new Error(`MT opp search ${a.status}`);return((await a.json()).opportunities||[]).map(d=>({id:d.id,name:d.name||"(unnamed opp)",propertyAddress:d.name||"",updatedAt:d.updatedAt||null})).sort((d,b)=>{let h=d.updatedAt?Date.parse(d.updatedAt):0;return(b.updatedAt?Date.parse(b.updatedAt):0)-h})},ms=async t=>{let e=await fetch(as,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId,template_id:t.templateId,template_type:t.templateType,opp_id:t.oppId,closing_date:t.closingDate,buyer_earnest_due_date:t.buyerEarnestDueDate,lender_source:t.lenderSource,triggered_by_user_id:t.userId,overrides:t.overrides}),credentials:"omit"});if(!e.ok)throw new Error(`Contract.Stage HTTP ${e.status}`);return await e.json()},us=async(t,e,o)=>{let r=`${Z}/proposals/templates/to-proposal/${t}/${e}?locale=en-US&contactId=${encodeURIComponent(o)}`,n=await fetch(r,{method:"POST",headers:ee()});if(!n.ok)throw new Error(`to-proposal HTTP ${n.status} ${n.statusText}`);let a=await n.json(),i=a._id||a.id||a.proposal?._id||a.proposal?.id;if(!i)throw new Error("to-proposal returned no proposal id");return i},_s=async(t,e)=>{let o=await fetch(`${Z}/proposals/document/${e}?locationId=${encodeURIComponent(t)}`,{method:"GET",headers:ee()});if(!o.ok){let a=await o.text().catch(()=>"");throw new Error(`GET proposal ${e} HTTP ${o.status} ${a.slice(0,200)}`)}let r=await o.json(),n=r?.document||r?.proposal||r;if(!n||typeof n!="object")throw new Error(`GET proposal returned unexpected shape: ${JSON.stringify(r).slice(0,200)}`);return n},vs=async t=>{let e=await fetch(`${Z}/contacts/${t}`,{method:"GET",headers:ee()});if(!e.ok){let n=await e.text().catch(()=>"");throw new Error(`GET contact ${t} HTTP ${e.status} ${n.slice(0,200)}`)}let o=await e.json(),r=o?.contact||o;return{firstName:r?.firstName||"",lastName:r?.lastName||"",email:r?.email||""}},ys=async(t,e,o,r)=>{let n;try{n=await _s(t,e)}catch(u){return{ok:!1,body:`fetchProposalDocument: ${u.message}`}}let a;try{a=await vs(o)}catch(u){return{ok:!1,body:`fetchContactBasics: ${u.message}`}}let i=`${a.firstName} ${a.lastName}`.trim()||a.email||o,l={id:o,firstName:a.firstName,lastName:a.lastName,email:a.email,contactName:i,isPrimary:!0,hasCompleted:!1,imgUrl:"",entityName:"contacts",role:"signer"},h=[...(Array.isArray(n.recipients)?n.recipients:[]).filter(u=>u?.entityName!=="contacts"),l],m={name:r||n.name,locationId:n.locationId||t,pages:n.pages||[],variables:n.variables||[],fillableFields:n.fillableFields||[],timezone:n.timezone||{},recipients:h,grandTotal:n.grandTotal},p=await fetch(`${Z}/proposals/document/${e}`,{method:"PUT",headers:{...ee(),"Content-Type":"application/json"},body:JSON.stringify(m)});if(!p.ok){let u=await p.text().catch(()=>"");return{ok:!1,status:p.status,body:u}}return{ok:!0}},gs=(t,e)=>{let o=`/v2/location/${t}/payments/proposals-estimates/edit/${e}`;window.location.href=o},gt="__WS_CRM_PENDING_INSTRUCTIONS",fs=60*1e3,ws=[{matches:"2 signers",title:"Special Instructions for 2-Signer Agreement",bodyHtml:`
    ...
    `}],xs=t=>{let e=(t||"").toLowerCase();for(let o of ws)if(e.includes(o.matches.toLowerCase()))return o;return null},ks=(t,e,o)=>{try{let r={proposalId:t,title:e,bodyHtml:o,ts:Date.now()};localStorage.setItem(gt,JSON.stringify(r))}catch{}},Es=()=>{try{let t=localStorage.getItem(gt);if(!t)return null;let e=JSON.parse(t);return!e?.proposalId||!e?.title||!e?.bodyHtml||Date.now()-(e.ts||0)>fs?(localStorage.removeItem(gt),null):e}catch{return null}},Ts=()=>{try{localStorage.removeItem(gt)}catch{}},Ss=(t,e)=>{document.getElementById(Q)?.remove();let o=document.createElement("div");o.id=Q,o.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `;let r=document.createElement("div");r.style.cssText=`
    ...
    overflow-y: auto;
    background: ${s.graphite};
    border: 1px solid ${s.emerald};
    border-left: 4px solid ${s.emerald};
    border-radius: ${x.lg};
    padding: 28px 32px;
    color: ${s.bone};
  ...
  `,o.appendChild(r),document.body.appendChild(o),r.querySelector("#ws-pc-instructions-ack")?.addEventListener("click",()=>o.remove())},Cs=/\/v2\/location\/[^/]+\/payments\/proposals-estimates\/edit\/([a-zA-Z0-9]+)/,Is=()=>{let t=window.location.pathname.match(Cs);if(!t)return;let e=t[1],o=Es();o&&o.proposalId===e&&(Ts(),window.setTimeout(()=>{Ss(o.title,o.bodyHtml)},600))},gr="ws-crm-prepare-contract-toast",H=(t,e="info",o=4e3)=>{document.getElementById(gr)?.remove();let r=document.createElement("div");r.id=gr;let n=e==="error"?s.crimson:s.emerald;r.style.cssText=`
    position: fixed;
    bottom: 24px;
    right: 24px;
    z-index: 999999;
  ...
  `,r.textContent=t,document.body.appendChild(r),window.setTimeout(()=>r.remove(),o)},fr=(t,e)=>new Promise(o=>{document.getElementById(Q)?.remove();let r=document.createElement("div");r.id=Q,r.style.cssText=`
      position: fixed;
      inset: 0;
      z-index: 999999;
      background: rgba(0, 0, 0, 0.6);
    ...
    `;let n=document.createElement("div");n.style.cssText=`
      ...
      color: ${s.bone};
      box-shadow: 0 12px 32px rgba(0, 0, 0, 0.5);
    `;let a=d=>{if(!d)return"";try{return new Date(d).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"2-digit"})}catch{return d}},i=t.map(d=>`
        <button type="button" class="ws-pc-acq-opp" data-opp-id="${J(d.id)}" style="
          display: block;
          width: 100%;
          text-align: left;
          background: ${s.slate};
        ...
        ">
          ...
          ${d.updatedAt?`<div style="font-size: 11px; color: ${s.ash}; margin-top: 4px;">Last updated ${B(a(d.updatedAt))}</div>`:""}
        </button>
      `).join("");n.innerHTML=`
      <div style="
        font-family: ${y.mono};
    ...
    `,r.appendChild(n),document.body.appendChild(r);let l=d=>{r.remove(),o(d)};r.addEventListener("click",d=>{d.target===r&&l(null)}),n.querySelector("#ws-pc-cancel")?.addEventListener("click",()=>l(null)),n.querySelectorAll(".ws-pc-acq-opp").forEach(d=>{d.addEventListener("mouseenter",()=>{d.style.borderColor=s.emerald,d.style.background=s.graphite}),d.addEventListener("mouseleave",()=>{d.style.borderColor=s.steel,d.style.background=s.slate}),d.addEventListener("click",b=>{b.stopPropagation(),b.preventDefault();let h=d.dataset.oppId||"";h&&l(h)})})}),As=({tenantId:t,contactId:e,template:o,staged:r})=>{document.getElementById(Q)?.remove(),wr();let n=r.debug||{},a=typeof n.propertyAddress=="string"?n.propertyAddress:"",i=typeof n.sellerName=="string"?n.sellerName:"",l=typeof n.purchasePrice=="number"?n.purchasePrice:0,d=typeof n.emd=="number"?n.emd:0,b=typeof n.cashAtClose=="number"?n.cashAtClose:l-d,h=n.daysToClose===void 0||n.daysToClose===null||n.daysToClose===""?"":String(n.daysToClose),m=n.inspectionDays===void 0||n.inspectionDays===null||n.inspectionDays===""?"":String(n.inspectionDays),p=typeof n.additionalTerms=="string"?n.additionalTerms:"",u=typeof n.currentOccupancy=="string"?n.currentOccupancy:"",f=typeof n.occupancyAtClose=="string"?n.occupancyAtClose:"",k=Array.isArray(n.occupancyOptions)&&n.occupancyOptions.length>0?n.occupancyOptions:["Vacant","Owner Occupied","Tenant Occupied","Squatter Occupied"],v=document.createElement("div");v.id=Q,v.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,K=`
    ...
    min-width: 0;
    appearance: none;
    background-image: linear-gradient(45deg, transparent 50%, ${s.ash} 50%), linear-gradient(135deg, ${s.ash} 50%, transparent 50%);
    background-position: calc(100% - 14px) 50%, calc(100% - 8px) 50%;
    background-size: 6px 6px;
    background-repeat: no-repeat;
    padding-right: 28px;
  `,Ve=F=>['<option value="">(unset)</option>'].concat(k.map($=>`<option value="${J($)}"${$===F?" selected":""}>${B($)}</option>`)).join("");_.innerHTML=`
    <div style="
      font-family: ${y.mono};
      font-size: 10px;
      letter-spacing: 0.14em;
    ...
    <div style="margin-bottom: 12px;">
      <div style="${S}">
        ...
        <div style="${w}">${B(a||"(set on Property record)")}</div>
      </div>
      <div style="${S}">
        <div style="${g}">Seller Name</div>
        <input id="ws-pc-seller-name" type="text" value="${J(i)}" placeholder="Seller name" style="${E}" />
      </div>
      <div style="${S}">
        <div style="${g}">Purchase Price</div>
        <div style="${A}">
          <span style="color: ${s.ash}; font-size: 13px;">$</span>
      ...
      <div style="${S}">
        ...
        <div style="${A}">
          <span style="color: ${s.ash}; font-size: 13px;">$</span>
          <input id="ws-pc-cash-at-close" type="number" min="0" step="0.01" value="${b}" style="${T}" />
        </div>
      </div>
      <div style="${S}">
        <div style="${g}">Days to Close</div>
        <input id="ws-pc-days-to-close" type="number" min="0" step="1" value="${J(h)}" placeholder="e.g. 30" style="${E}" />
      </div>
      <div style="${S}">
        <div style="${g}">Inspection Period (Days)</div>
        <input id="ws-pc-inspection-days" type="number" min="0" step="1" value="${J(m)}" placeholder="e.g. 10" style="${E}" />
      </div>
      <div style="${S}">
        <div style="${g}">Current Occupancy</div>
        <select id="ws-pc-current-occupancy" style="${K}">${Ve(u)}</select>
      </div>
      <div style="${S}">
        <div style="${g}">Occupancy at Close</div>
        <select id="ws-pc-occupancy-at-close" style="${K}">${Ve(f)}</select>
      </div>
      <div style="${S} align-items: flex-start;">
        <div style="${g}">Additional Terms</div>
        <textarea id="ws-pc-additional-terms" rows="3" placeholder="Any extra clauses, contingencies, or notes for the contract" style="${E} resize: vertical; min-height: 60px; font-family: ${y.sans};">${B(p)}</textarea>
      </div>
    </div>
    <div style="
      font-size: 11px;
  ...
  `,v.appendChild(_),document.body.appendChild(v);let N=_.querySelector("#ws-pc-purchase-price"),se=_.querySelector("#ws-pc-emd"),oe=_.querySelector("#ws-pc-cash-at-close"),ie=!1;oe?.addEventListener("input",()=>{ie=!0});let ve=()=>{if(ie)return;let F=Number(N?.value||0),$=Number(se?.value||0);oe&&(oe.value=String(Math.max(0,F-$)))};N?.addEventListener("input",ve),se?.addEventListener("input",ve);let We=()=>v.remove();v.addEventListener("click",F=>{F.target===v&&We()}),_.querySelector("#ws-pc-cancel")?.addEventListener("click",We);let U=_.querySelector("#ws-pc-submit"),G=_.querySelector("#ws-pc-error");U?.addEventListener("click",async()=>{if(U){G&&(G.textContent=""),U.disabled=!0,U.textContent="Opening\u2026";try{let F=(_.querySelector("#ws-pc-seller-name")?.value||"").trim(),$=Number(N?.value||0),ye=Number(se?.value||0),ge=Number(oe?.value||0),fe=(_.querySelector("#ws-pc-days-to-close")?.value??"").trim(),we=(_.querySelector("#ws-pc-inspection-days")?.value??"").trim(),xe=(_.querySelector("#ws-pc-additional-terms")?.value??"").trim(),pe=(_.querySelector("#ws-pc-current-occupancy")?.value??"").trim(),ke=(_.querySelector("#ws-pc-occupancy-at-close")?.value??"").trim(),O={};if(F!==i&&(O.sellerName=F),$!==l&&(O.purchasePrice=$),ye!==d&&(O.emd=ye),ge!==b&&(O.cashAtClose=ge),fe!==h&&(O.daysToClose=fe),we!==m&&(O.inspectionDays=we),xe!==p&&(O.additionalTerms=xe),pe!==u&&(O.currentOccupancy=pe),ke!==f&&(O.occupancyAtClose=ke),Object.keys(O).length>0){c.info("[prepare-contract] PA modal submit with overrides:",O),U.textContent="Saving\u2026";let At=L(),Lt=await ft({tenantId:t,contactId:e,template:o,userId:At,overrides:O});c.info("[prepare-contract] re-stage with overrides ok:",Lt),U.textContent="Opening\u2026"}else c.info("[prepare-contract] PA modal submit \u2014 no edits, skipping re-stage");let P=a?`${a} - Purchase Agreement`:void 0;await go(t,e,o,P)}catch(F){U.disabled=!1,U.textContent="Open Contract",G&&(G.textContent=F.message||"Failed to open contract."),c.warn("[prepare-contract] PA confirm submit failed:",F)}}})},Ls=({tenantId:t,contactId:e,template:o,oppId:r,staged:n,userId:a,defaultClosingDate:i,defaultEMDDueDate:l})=>{document.getElementById(Q)?.remove(),wr();let d=n.debug||{},b=typeof d.propertyAddress=="string"?d.propertyAddress:"",h=typeof d.sellerDisplay=="string"?d.sellerDisplay:"",m=typeof d.assignmentPrice=="number"?d.assignmentPrice:0,p=typeof d.buyerEMD=="number"?d.buyerEMD:0,u=typeof d.assignmentFee=="number"?d.assignmentFee:0,f=typeof d.acqPurchasePrice=="number"?d.acqPurchasePrice:0,k=typeof d.closingOffice=="string"?d.closingOffice:"",v=typeof d.buyerCompanyName=="string"?d.buyerCompanyName:"",_=i,g=l,E="",T=document.createElement("div");T.id=Q,T.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `;w.innerHTML=`
    ...
    <div style="margin-bottom: 12px;">
      <div style="${N}">
        ...
        <div style="${Ve}">${B(b||"(set on Property record)")}</div>
      </div>
      <div style="${N}">
        <div style="${S}">Seller Name</div>
        <input id="ws-pc-seller-name" type="text" value="${J(h)}" placeholder="Seller name" style="${A}" />
      </div>
      <div style="${N}">
        <div style="${S}">Buyer LLC <span style="color: ${s.coolGray}; text-transform: none; letter-spacing: 0; font-family: ${y.sans};">(assignee)</span></div>
        <input id="ws-pc-buyer-llc" type="text" value="${J(v)}" placeholder="Buyer's entity / LLC name" style="${A}" />
      </div>
      <div style="${N}">
        <div style="${S}">Assignment Price</div>
        <div style="${se}">
          <span style="color: ${s.ash}; font-size: 13px;">$</span>
          <input id="ws-pc-assignment-price" type="number" min="0" step="0.01" value="${m}" style="${K}" />
        </div>
      </div>
      <div style="${N}">
        ...
        <div style="${se}">
          <span style="color: ${s.ash}; font-size: 13px;">$</span>
          <input id="ws-pc-buyer-emd" type="number" min="0" step="0.01" value="${p}" style="${K}" />
        </div>
      </div>
      <div style="${N}">
        ...
        <div style="${se}">
          <span style="color: ${s.ash}; font-size: 13px;">$</span>
          <input id="ws-pc-assignment-fee" type="number" min="0" step="0.01" value="${u}" style="${K}" />
        </div>
      </div>
      <div style="${N}">
        <div style="${S}">Closing Office</div>
        <input id="ws-pc-closing-office" type="text" value="${J(k)}" placeholder="Title company" style="${A}" />
      </div>
      <div style="${N}">
        <div style="${S}">Closing Date</div>
        <input id="ws-pc-closing-date" type="date" value="${J(_)}" style="${A}" />
      </div>
      <div style="${N}">
        <div style="${S}">Buyer EMD Due Date</div>
        <input id="ws-pc-emd-due-date" type="date" value="${J(g)}" style="${A}" />
      </div>
      <div style="${N}">
        <div style="${S}">Lender Source <span style="color: ${s.coolGray}; text-transform: none; letter-spacing: 0; font-family: ${y.sans};">(optional)</span></div>
        <input id="ws-pc-lender-source" type="text" placeholder="e.g. ABC Hard Money LLC" style="${A}" />
      </div>
    </div>
    <div style="
      font-size: 11px;
  ...
  `,T.appendChild(w),document.body.appendChild(T);let oe=w.querySelector("#ws-pc-assignment-price"),ie=w.querySelector("#ws-pc-assignment-fee"),ve=!1;ie?.addEventListener("input",()=>{ve=!0});let We=()=>{if(ve)return;let $=Number(oe?.value||0);ie&&(ie.value=String(Math.max(0,$-f)))};oe?.addEventListener("input",We);let U=()=>T.remove();T.addEventListener("click",$=>{$.target===T&&U()}),w.querySelector("#ws-pc-cancel")?.addEventListener("click",U);let G=w.querySelector("#ws-pc-submit"),F=w.querySelector("#ws-pc-error");G?.addEventListener("click",async()=>{if(!G)return;F&&(F.textContent="");let $=(w.querySelector("#ws-pc-seller-name")?.value||"").trim(),ye=Number(oe?.value||0),ge=Number(w.querySelector("#ws-pc-buyer-emd")?.value||0),fe=Number(ie?.value||0),we=(w.querySelector("#ws-pc-closing-office")?.value||"").trim(),xe=(w.querySelector("#ws-pc-buyer-llc")?.value||"").trim(),pe=(w.querySelector("#ws-pc-closing-date")?.value||"").trim(),ke=(w.querySelector("#ws-pc-emd-due-date")?.value||"").trim(),O=(w.querySelector("#ws-pc-lender-source")?.value||"").trim();if(!pe){F&&(F.textContent="Closing Date is required.");return}G.disabled=!0,G.textContent="Opening\u2026";try{let P={};$!==h&&(P.sellerName=$),ye!==m&&(P.assignmentPrice=ye),ge!==p&&(P.buyerEMD=ge),fe!==u&&(P.assignmentFee=fe),we!==k&&(P.closingOffice=we),xe!==v&&(P.buyerCompanyName=xe);let At=pe!==_||ke!==g||O!==E,Lt=Object.keys(P).length>0||At,Mt=n;Lt?(c.info("[prepare-contract] AA modal submit, re-staging with overrides:",P),G.textContent="Saving\u2026",Mt=await ft({tenantId:t,contactId:e,template:o,oppId:r,closingDate:pe,buyerEarnestDueDate:ke,lenderSource:O,userId:a,overrides:Object.keys(P).length>0?P:void 0}),c.info("[prepare-contract] AA re-stage ok:",Mt),G.textContent="Opening\u2026"):c.info("[prepare-contract] AA modal submit \u2014 no edits, skipping re-stage");let Eo=Mt?.debug?.propertyAddress||b,jr=Eo?`${Eo} - Assignment Agreement`:void 0;await go(t,e,o,jr),U()}catch(P){G.disabled=!1,G.textContent="Open Contract",F&&(F.textContent=P.message||"Failed to prepare contract."),c.warn("[prepare-contract] AA submit failed:",P)}})},ft=async t=>{c.info(`[prepare-contract] staging \u2014 template "${t.template.name}" (${t.template.type})`);let e=await ms({tenantId:t.tenantId,contactId:t.contactId,templateId:t.template.id,templateType:t.template.type,oppId:t.oppId,closingDate:t.closingDate,buyerEarnestDueDate:t.buyerEarnestDueDate,lenderSource:t.lenderSource,userId:t.userId,overrides:t.overrides});if(c.info(`[prepare-contract] stage response: ok=${e.ok}`,e),!e.ok)throw new Error(e.error||"Contract.Stage returned not ok");return e},go=async(t,e,o,r)=>{c.info(`[prepare-contract] creating proposal from template ${o.id} for contact ${e}`);let n=await us(t,o.id,e);c.info(`[prepare-contract] proposal created: ${n}`),c.info(`[prepare-contract] attaching contact ${e} as primary signer${r?`, renaming to "${r}"`:""}`);let a=await ys(t,n,e,r);a.ok?c.info(`[prepare-contract] contact attached to proposal ${n}`):(c.warn(`[prepare-contract] attach failed (status=${a.status??"n/a"}): ${a.body?.slice(0,300)??"no body"}`),H(`Proposal created but couldn't auto-attach contact (HTTP ${a.status??"?"}). You'll need to pick the recipient manually.`,"error",8e3),console.warn("[ws-crm-customizer] attach-recipient GHL response:",a.body));let i=xs(o.name);i&&(c.info(`[prepare-contract] stashing post-redirect instructions for "${o.name}"`),ks(n,i.title,i.bodyHtml)),gs(t,n)},Ms=async t=>{let e;try{e=await ds(t)}catch(a){c.warn("[prepare-contract] context resolution failed:",a),H(`Could not resolve context: ${a.message}`,"error",8e3);return}if(!e){H("Could not resolve contact context.","error");return}document.getElementById(_r)?.remove();let o=document.createElement("div");o.id=_r,o.style.cssText=`
    position: absolute;
    top: calc(100% + 6px);
    right: 0;
    z-index: 999998;
  ...
  `,o.innerHTML=`
    ...
    ">Loading templates\u2026</div>`,t.appendChild(o);let r=a=>{!o.contains(a.target)&&a.target!==t&&(o.remove(),document.removeEventListener("click",r,!0))};window.setTimeout(()=>document.addEventListener("click",r,!0),0);let n;try{n=await bs(e.tenantId)}catch(a){o.innerHTML=`
      <div style="padding: 12px 14px; color: ${s.crimson}; font-size: 12px;">
        Failed to load templates: ${B(a.message)}
      </div>`;return}if(n.length===0){o.innerHTML=`
      <div style="padding: 12px 14px; color: ${s.ash}; font-size: 12px;">
        No proposal templates found on this tenant.
      </div>`;return}n.sort((a,i)=>{let l={PA:0,AA:1,passthrough:2};return l[a.type]!==l[i.type]?l[a.type]-l[i.type]:a.name.localeCompare(i.name)}),o.innerHTML="";for(let a of n){let i=document.createElement("button");i.type="button",i.style.cssText=`
      display: block;
      width: 100%;
      background: transparent;
      border: none;
      ...
      cursor: pointer;
      transition: background 0.1s ease;
    `;let l=a.type==="PA"?s.emerald:a.type==="AA"?s.blue:s.coolGray;i.innerHTML=`
      <div style="display: flex; align-items: baseline; gap: 8px;">
        <span style="
          ...
          flex-shrink: 0;
          min-width: 30px;
        ">${a.type==="passthrough"?"DOC":a.type}</span>
        <span style="flex: 1;">${B(a.name)}</span>
      </div>`,i.addEventListener("mouseenter",()=>{i.style.background=s.slate}),i.addEventListener("mouseleave",()=>{i.style.background="transparent"}),i.addEventListener("click",async d=>{d.stopPropagation(),d.preventDefault(),c.info(`[prepare-contract] template picked: ${a.name} (type=${a.type})`),o.remove(),document.removeEventListener("click",r,!0);let b=L();try{if(a.type==="AA"){let h;if(e.oppId)h=e.oppId,c.info(`[prepare-contract] AA from MT opp page \u2014 using opp ${h} directly`);else{let v;try{v=await ps(e.tenantId,e.contactId)}catch(_){H(`Failed to load buyer's MT opps: ${_.message}`,"error",6e3),c.warn("[prepare-contract] AA fetch opps failed:",_);return}if(v.length===0){H("No open MT deal found for this buyer. Create an MT opp first.","error",6e3);return}if(v.length===1)h=v[0].id,c.info(`[prepare-contract] auto-selected MT opp ${h} (only one)`);else{c.info(`[prepare-contract] showing MT opp picker (${v.length} opps)`);let _=await fr(v,{perspective:"buyer",agreementLabel:"Assignment Agreement"});if(!_){c.info("[prepare-contract] MT opp picker cancelled");return}h=_,c.info(`[prepare-contract] picked MT opp ${h}`)}}let m=v=>{let _=v.getFullYear(),g=String(v.getMonth()+1).padStart(2,"0"),E=String(v.getDate()).padStart(2,"0");return`${_}-${g}-${E}`},p=v=>{let _=new Date;return _.setDate(_.getDate()+v),m(_)},u=p(45),f=p(7);H("Reading contract data\u2026","info",2500);let k;try{k=await ft({tenantId:e.tenantId,contactId:e.contactId,template:a,oppId:h,closingDate:u,buyerEarnestDueDate:f,userId:b})}catch(v){H(`Failed to prepare contract: ${v.message}`,"error",6e3),c.warn("[prepare-contract] AA stage failed:",v);return}Ls({tenantId:e.tenantId,contactId:e.contactId,template:a,oppId:h,staged:k,userId:b,defaultClosingDate:u,defaultEMDDueDate:f})}else if(a.type==="PA"){let h;try{h=await hs(e.tenantId,e.contactId)}catch(u){H(`Failed to load ACQ opps: ${u.message}`,"error",6e3),c.warn("[prepare-contract] PA fetch opps failed:",u);return}if(h.length===0){H("No open ACQ deal found for this contact. Create an ACQ opp first.","error",6e3);return}let m;if(h.length===1)m=h[0].id,c.info(`[prepare-contract] auto-selected ACQ opp ${m} (only one)`);else{c.info(`[prepare-contract] showing ACQ opp picker (${h.length} opps)`);let u=await fr(h,{perspective:"seller",agreementLabel:"Purchase Agreement"});if(!u){c.info("[prepare-contract] ACQ opp picker cancelled");return}m=u,c.info(`[prepare-contract] picked ACQ opp ${m}`)}H("Reading contract data\u2026","info",2500);let p;try{p=await ft({tenantId:e.tenantId,contactId:e.contactId,template:a,oppId:m,userId:b})}catch(u){H(`Failed to prepare contract: ${u.message}`,"error",6e3),c.warn("[prepare-contract] PA stage failed:",u);return}As({tenantId:e.tenantId,contactId:e.contactId,template:a,staged:p})}else{H(`Opening ${a.name}\u2026`,"info",2500);try{await go(e.tenantId,e.contactId,a)}catch(h){H(`Failed to open contract: ${h.message}`,"error",6e3),c.warn("[prepare-contract] passthrough failed:",h)}}}catch(h){c.warn("[prepare-contract] picker click handler failed:",h),H(`Unexpected error: ${h.message}`,"error",6e3)}}),o.appendChild(i)}},Er=()=>{let t=document.createElement("button");return t.id=uo,t.type="button",t.style.cssText=`${D} position: relative;`,t.innerHTML=`
    <span>Prepare Contract</span>
    <span style="font-size: 10px; color: ${s.ash};">\u25BE</span>
  `,t.addEventListener("mouseenter",()=>{t.style.borderColor=s.emerald}),t.addEventListener("mouseleave",()=>{t.style.borderColor=s.steel}),t.addEventListener("click",e=>{e.stopPropagation(),Ms(t)}),t},Ds="ws-crm-contact-actions-bar",Fs=async()=>{let t=kr();if(!t){document.getElementById(uo)?.remove();return}let e=null;try{e=await I(`#${Ds}`,{timeoutMs:3e3})}catch{c.debug("[prepare-contract] actions bar not found within timeout");return}if(!e)return;let o=document.getElementById(uo);if(o){if(o.dataset.contactId===t.contactId)return;o.remove()}let r=Er();r.dataset.contactId=t.contactId,r.dataset.tenantId=t.tenantId,e.appendChild(r),c.debug(`[prepare-contract] mounted for contact ${t.contactId}`)},he="ws-crm-prepare-contract-button-opp",Be=`${he}-wrap`,$s=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Os=/\/v2\/location\/([A-Za-z0-9]+)/,Ps=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],vo=()=>{let t=window.location.pathname,e=t.match($s);if(!e)return null;let o=t.match(Os);return o?{oppId:e[1],tenantId:o[1]}:null},Ns=async()=>{for(let t of Ps)try{let e=await I(t,{timeoutMs:2e3});if(e)return e}catch{}return null},Hs=/\/contacts\/(?:detail\/|smart_list\/.*?\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Bs=()=>{let t=[".hr-card.hr-modal.crm-opportunities-modal",".crm-opportunities-modal"],e=null;for(let r of t)if(e=document.querySelector(r),e)break;if(!e)return null;let o=e.querySelectorAll("a[href]");for(let r of Array.from(o)){let a=(r.getAttribute("href")||"").match(Hs);if(a&&a[1])return a[1]}return null},Rs=async(t,e)=>{let o=Bs();if(o)return c.debug(`[prepare-contract] scraped contactId ${o} from opp modal`),o;c.debug("[prepare-contract] DOM scrape miss, falling back to API");let r=ee(),n=`${Z}/opportunities/search?location_id=${encodeURIComponent(e)}&id=${encodeURIComponent(t)}&limit=1`,a=await fetch(n,{method:"GET",headers:r});if(!a.ok){let b="";try{b=await a.text()}catch{}throw new Error(`Could not resolve buyer contact for opp ${t}: ${a.status} ${a.statusText}${b?` \u2014 ${b.slice(0,160)}`:""}`)}let i=await a.json(),l=i?.opportunities?.[0]||i?.[0]||{},d=l?.contactId||l?.contact_id||l?.contact?.id;if(!d)throw new Error("Could not find this opp's contact \u2014 re-open the opp and try again.");return d},yt=!1,Re=null,yo=null,po=null,mo,Ge=()=>{Re?.disconnect(),Re=null,yo=null},Gs=t=>{if(Re&&yo===t)return;Ge();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(yo=t,Re=new MutationObserver(()=>{let o=vo();if(!o||o.oppId!==t){Ge();return}let r=M();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(he)?.remove(),document.getElementById(Be)?.remove(),Ge();return}document.getElementById(he)||fo()}),Re.observe(e,{childList:!0,subtree:!0}))},Us=()=>{po||(po=new MutationObserver(()=>{mo===void 0&&(mo=window.setTimeout(()=>{mo=void 0;let t=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(he);t&&!e&&!yt&&fo()},300))}),po.observe(document.body,{childList:!0,subtree:!0}))},fo=async()=>{if(!yt){yt=!0;try{let t=vo();if(!t){document.getElementById(he)?.remove(),document.getElementById(Be)?.remove(),Ge();return}let e=document.getElementById(he);if(e&&e.dataset.oppId===t.oppId){let l=M();l!==null&&l!=="Marketing Tracker"&&(e.remove(),document.getElementById(Be)?.remove(),Ge());return}e&&(e.remove(),document.getElementById(Be)?.remove());let o=await Ns();if(!o){c.debug("[prepare-contract] opp mount target not found");return}let r=await Y();if(r!=="Marketing Tracker"){c.debug(`[prepare-contract] opp mount gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=vo();if(!n||n.oppId!==t.oppId)return;let a=Er();a.id=he,a.dataset.oppId=t.oppId,a.dataset.tenantId=t.tenantId;let i=document.createElement("span");i.id=Be,i.style.cssText=`
      display: inline-flex;
      align-items: center;
      flex-shrink: 0;
      vertical-align: middle;
      margin-left: 12px;
    `,o.classList.contains("description")&&(o.style.display="flex",o.style.alignItems="center",o.style.gap="16px",o.style.flexWrap="wrap"),i.appendChild(a),o.appendChild(i),c.debug(`[prepare-contract] mounted on MT opp ${t.oppId} (contactId deferred to click)`),Gs(t.oppId)}finally{yt=!1}}},Tr=()=>{Is(),Fs(),Us(),fo()};ls();function B(t){return String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function J(t){return B(t)}var St="reos-theme-stylesheet",zr="reos-theme-fonts",zs=()=>{let t=document.getElementById(zr);t&&t.remove()},qs=`
/* ============================================
...
`,Vs=[{selector:".hr-input__input-el, .hr-input__textarea-el, [class*='hr-input'] input, [class*='hr-input'] textarea",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".hr-data-table-wrapper, .hr-data-table-wrapper-responsive, .hr-data-table-wrapper-header, .hr-data-table-wrapper-footer, .hr-data-table, .hr-data-table__body, .hr-table-freezer-wrapper, [class*='hr-data-table-wrapper'], [class*='hr-table-freezer']",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".n-data-table-tr, tr.n-data-table-tr",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".back-to-sites, .n-tabs-nav, .n-tabs-nav-scroll-content, .n-tabs-nav-scroll-wrapper, .n-tabs-wrapper, .n-tabs-tab-wrapper, .n-tabs-tab-pad",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:"#setup-guide-button, .setup-guide-button-container",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:".default-avatar, .n-avatar.default-avatar",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:"#steps-sidebar, .steps-sidebar",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".search-box",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:".my-table.tabulator, .tabulator, .tabulator-tableholder, .tabulator-table, .tabulator-header, .tabulator-headers, .tabulator-footer, .tabulator-row, .table-container, .cardWrapper, .quick-filter-bar, #skeleton-loader, .skeleton-loader",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".tabulator-col, .tabulator-frozen, .tabulator-frozen-left, .tabulator-frozen-right",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:".mb-2\\.5.bg-white, div.bg-white, .bg-white.px-3, .mx-auto > .bg-white",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-data-table__header, .hr-data-table__cell-header, thead.hr-data-table__header",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:".hr-wrapper-container, .platform-ui__highrise, .full-screen.platform-ui__highrise, [class*='platform-ui__highrise'], .hl_topbar-tabs, .hl_wrapper",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-collapse-item__header, .hr-collapse-item__header-main, .hr-collapse-item__title",styles:{color:"#EDEEF0","background-color":"#12161D"}},{selector:".hr-collapse-item__content, .hr-collapse-item__content-inner, .hr-collapse-item__content-wrap",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-tabs, .hr-tabs-nav, .hr-tabs-nav-scroll-content, .hr-tabs-content, .hr-tabs-pane, .hr-tabs-rail, .hr-tabs-tab-wrapper",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-tabs-capsule",styles:{"background-color":"transparent",background:"transparent",border:"none"}},{selector:".hr-avatar:not(.default-avatar), .hr-avatar:not(.default-avatar) .hr-avatar__text, .hr-avatar:not(.default-avatar) .hr-avatar__text p, .hr-avatar:not(.default-avatar) p, .hr-avatar:not(.default-avatar) span, .hr-avatar:not(.default-avatar) div, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar), [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar) span, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar) p, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar) div",styles:{color:"#0A0D12","-webkit-text-fill-color":"#0A0D12","font-weight":"600"}},{selector:".default-avatar, .default-avatar *",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0",stroke:"#EDEEF0","font-weight":"600"}},{selector:".hr-button--tertiary",styles:{"background-color":"#1A1F28",color:"#0FB57E"}},{selector:".hr-text:not([role='button']):not(.cursor-pointer)",styles:{color:"#EDEEF0"}},{selector:"[class*='field-value'], [class*='field-display'], [class*='empty-value'], [class*='field-empty'], [class*='value-display'], [class*='display-value']",styles:{color:"#9098A3","-webkit-text-fill-color":"#9098A3"}},{selector:".hr-select__value, .hr-select__display, .hr-select-trigger, .hr-select-trigger__value, .hr-base-select__value, .hr-base-select__display, [class*='select__value'], [class*='select__display'], [class*='select-trigger']",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".hr-select, .hr-select__input, .hr-select__inner, .hr-select-trigger, .hr-base-select, .hr-base-select__inner, .el-select, .el-select__wrapper, .el-input__wrapper, .el-input__inner, [role='combobox'], [aria-haspopup='listbox']",styles:{"background-color":"#1A1F28","border-color":"#252C36"}},{selector:"input[type='date'], input[type='datetime-local'], input[type='time'], input[type='month'], input[type='week'], .hr-date-picker, .hr-date-input, .hr-datepicker, .hr-time-picker, [class*='date-picker'], [class*='datepicker'], [class*='date-input'], [class*='time-picker'], [class*='time-input']",styles:{"background-color":"#1A1F28","border-color":"#252C36",color:"#EDEEF0"}},{selector:"[class*='F7F9FD'], [class*='F7F9FB'], [class*='F9FAFB'], [class*='eff4ff'], [class*='EFF4FF'], [class*='ECEEF2'], [class*='eceef2']",styles:{"background-color":"#12161D"}},{selector:"[style*='background-color: white'], [style*='background-color:white'], [style*='background-color: #fff'], [style*='background-color:#fff'], [style*='background-color: #FFF'], [style*='background-color:#FFF'], [style*='background-color: #ffffff'], [style*='background-color:#ffffff'], [style*='background-color: #FFFFFF'], [style*='background-color:#FFFFFF'], [style*='background-color: rgb(255, 255, 255)'], [style*='background-color:rgb(255, 255, 255)'], [style*='background-color: rgb(255,255,255)'], [style*='background-color:rgb(255,255,255)']",styles:{"background-color":"#12161D"}},{selector:".chart-container.hl-card, [class*='chart-container'][class*='hl-card'], .dashboard-widget-type-custom .hl-card, .dashboard-widget-module-custom .hl-card",styles:{"background-color":"#12161D"}},{selector:".bg-gray-50, .bg-white, .bg-gray-100",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".n-base-select-menu, .n-popselect-menu, .n-popover, .n-dropdown, .n-dropdown-menu, .hr-base-select-menu, .hr-popselect-menu, .hr-popover, .hr-dropdown, .hr-dropdown-menu",styles:{"background-color":"#12161D",color:"#EDEEF0","border-color":"#252C36","--n-option-color":"transparent","--n-option-color-hover":"#1A1F28","--n-option-color-active":"rgba(15, 181, 126, 0.12)","--n-option-color-active-hover":"rgba(15, 181, 126, 0.12)","--n-option-text-color":"#EDEEF0","--n-option-text-color-hover":"#EDEEF0","--n-option-text-color-active":"#0FB57E","--n-option-text-color-disabled":"#9098A3","--n-color":"transparent","--n-color-hover":"#1A1F28","--n-color-active":"rgba(15, 181, 126, 0.12)","--n-text-color":"#EDEEF0","--n-text-color-hover":"#EDEEF0","--n-text-color-active":"#0FB57E","--n-text-color-disabled":"#9098A3"}},{selector:".n-dropdown-option--disabled, .n-dropdown-option--disabled .n-dropdown-option-body, .n-dropdown-option--disabled .n-dropdown-option-body__label, .n-dropdown-option--disabled .n-dropdown-option-body__prefix, .n-dropdown-option--disabled .n-dropdown-option-body__suffix, .n-dropdown-option[aria-disabled='true'], .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__label, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__prefix, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__suffix, .n-base-select-option--disabled, .n-base-select-option--disabled *, .n-base-select-option[aria-disabled='true'], .n-base-select-option[aria-disabled='true'] *, .hr-dropdown-option--disabled, .hr-dropdown-option--disabled *, .hr-dropdown-option[aria-disabled='true'], .hr-dropdown-option[aria-disabled='true'] *, .hr-base-select-option--disabled, .hr-base-select-option--disabled *, .hr-base-select-option[aria-disabled='true'], .hr-base-select-option[aria-disabled='true'] *",styles:{"background-color":"transparent",color:"#9098A3","-webkit-text-fill-color":"#9098A3"}},{selector:".hl-card-header [role='heading'], .hl-card-header p[role='heading'], [class*='hl-card-header'] [role='heading']",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".n-button:not(.n-button--primary-type):not(.n-button--error-type):not(.n-button--warning-type):not(.n-button--success-type):not(.n-button--info-type)",styles:{"background-color":"#1A1F28"}}],Ws=t=>{if(!t||t[0]!=="#")return t;let e=t.slice(1);if(e.length===3&&(e=e[0]+e[0]+e[1]+e[1]+e[2]+e[2]),e.length!==6&&e.length!==8)return t;let o=parseInt(e.slice(0,2),16),r=parseInt(e.slice(2,4),16),n=parseInt(e.slice(4,6),16);if(isNaN(o)||isNaN(r)||isNaN(n))return t;if(e.length===8){let a=parseInt(e.slice(6,8),16)/255;return`rgba(${o}, ${r}, ${n}, ${a})`}return`rgb(${o}, ${r}, ${n})`},js=new Set(["color","background-color","background","border-color","border-top-color","border-right-color","border-bottom-color","border-left-color","fill","stroke","-webkit-text-fill-color","caret-color","outline-color"]),Ks=["#inventory-table","#inventory","[id^='collections-']","[id^='products-']","[id^='reviews-']",".sites-container","[data-testid='wordpress-app-dashboard']","[data-testid$='-app-dashboard']","[data-testid$='-dashboard']:not([data-testid*='widget']):not([data-testid*='card'])","#wordpress-management-app","[id$='-management-app']"],wo=new WeakSet,Sr=()=>{try{if(!document.body)return;document.body.querySelectorAll("*").forEach(e=>{try{if(wo.has(e)&&e.style.getPropertyValue("background-color")==="rgb(18, 22, 29)"||e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.closest(".hl_header, header.hl_header, header.app-header"))return;let o=e.tagName;if(o==="INPUT"||o==="TEXTAREA"||o==="SELECT"||o==="SCRIPT"||o==="STYLE"||o==="SVG"||o==="PATH"||o==="IMG")return;let n=window.getComputedStyle(e).backgroundColor;if(!n||n==="rgba(0, 0, 0, 0)"||n==="transparent")return;let a=n.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?/);if(!a)return;let i=+a[1],l=+a[2],d=+a[3];if((a[4]!==void 0?+a[4]:1)<.5||i<230||l<230||d<230)return;if(e.style.getPropertyValue("background-color")==="rgb(18, 22, 29)"){wo.add(e);return}e.style.setProperty("background-color","rgb(18, 22, 29)","important"),e.style.setProperty("color","rgb(237, 238, 240)","important"),wo.add(e)}catch{}})}catch{}},Cr=new WeakSet,Ue=()=>{document.querySelectorAll(Ks.join(", ")).forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let o=e.querySelectorAll("*");[e,...Array.from(o)].forEach(r=>{if(Cr.has(r)&&r.style.getPropertyValue("background-color")==="rgb(18, 22, 29)"||r.id?.startsWith("ws-crm-")||r.tagName==="INPUT"||r.tagName==="TEXTAREA"||r.tagName==="SELECT"||r.tagName==="SCRIPT"||r.tagName==="STYLE"||r.tagName==="SVG"||r.tagName==="PATH")return;let a=window.getComputedStyle(r).backgroundColor;if(!a||a==="rgba(0, 0, 0, 0)"||a==="transparent")return;let i=a.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!i)return;let l=+i[1],d=+i[2],b=+i[3];l<230||d<230||b<230||(r.style.setProperty("background-color","#12161D","important"),r.style.setProperty("color","#EDEEF0","important"),Cr.add(r))})})},Ir=()=>{document.querySelectorAll(".hr-header-lite-left, .hr-header-lite-right, .hr-header-lite-content, .hr-header-lite-icon-item, [class*='hr-header-lite']:not([class*='container']):not([class*='wrapper']), .hr-breadcrumb, .hr-breadcrumb-item, .hr-breadcrumb-item__link, .hr-breadcrumb-item__separator").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let o=e.getAttribute("style")||"";(o.includes("background-color: rgb(26, 31, 40)")||o.includes("background-color: #1A1F28")||o.includes("background-color: #1a1f28"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("border-color"),e.style.removeProperty("border-style"),e.style.removeProperty("border-width"),e.style.removeProperty("border-radius"))})},wt=()=>{document.querySelectorAll(".hr-header-lite-container-v2, [id^='hr-header-lite-v-']").forEach(e=>{let o=e.parentElement,r=0;for(;o&&r<6&&o!==document.body;){if(r+=1,o.id?.startsWith("ws-crm-")||o.classList.contains("hl_header")||o.classList.contains("app-header")||o.tagName==="MAIN"||o.tagName==="BODY")return;let i=window.getComputedStyle(o).backgroundColor.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?/);if(i){let l=+i[1],d=+i[2],b=+i[3];(i[4]!==void 0?+i[4]:1)>=.5&&l>=180&&d>=180&&b>=180&&l+d+b>=600&&o.style.getPropertyValue("background-color")!=="rgb(18, 22, 29)"&&(o.style.setProperty("background-color","rgb(18, 22, 29)","important"),o.style.setProperty("color","rgb(237, 238, 240)","important"))}o=o.parentElement}})},Ar=()=>{let t="rgb(144, 152, 163)";document.querySelectorAll(".primary-property-name, .primary-property-name *, .folder-name, .folder-name *, .hr-form-item-label__asterisk, span.hr-form-item-label__asterisk").forEach(o=>{if(o.id?.startsWith("ws-crm-")||o.closest("[id^='ws-crm-']"))return;(o.getAttribute("style")||"").includes(t)&&(o.style.removeProperty("color"),o.style.removeProperty("-webkit-text-fill-color"))})},ze=()=>{for(let t of Vs){let e;try{e=document.querySelectorAll(t.selector)}catch{continue}e.forEach(o=>{for(let[r,n]of Object.entries(t.styles)){let a=o.style.getPropertyPriority(r),i=o.style.getPropertyValue(r),l=js.has(r)?Ws(n):n;a==="important"&&i===l||o.style.setProperty(r,n,"important")}})}},Ct="#1A1F28",Ys=26,Xs=31,Js=40,Lr=new Set,Mr=new Set,xt=()=>{document.body.querySelectorAll("*").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.children.length>0||(e.textContent||"").trim()!=="--")return;let r=e.getBoundingClientRect();if(r.width===0||r.height===0||r.width>800)return;e.style.setProperty("display","none","important");let n=e.className?.toString()||"<no class>";!Mr.has(n)&&n!=="<no class>"&&(Mr.add(n),c.debug(`Hid empty "--" leaf: "${n}"`))})},Qs=["M3 6","M5 6","M2 6","M6 6","M9 3h6","M16 6v","M19 6v","M5 6V","M16.5 4","48.816","2.991","14.74 9","9.26 9","2.244","M19 4h-","M19 7l-"],Zs=["trash","Trash","TRASH","delete","Delete","DELETE"],ei=()=>{document.querySelectorAll("button.hr-button, button.n-button, button.icon-only, .hr-button.icon-only, .n-button.icon-only").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.getAttribute("data-reos-trash")==="true")return;let o=e.querySelector("svg");if(!o)return;let r=!1,n=(o.getAttribute("class")||"")+" "+(o.getAttribute("data-icon")||"");if(Zs.some(a=>n.includes(a))&&(r=!0),r||o.querySelectorAll("path").forEach(i=>{if(r)return;let l=i.getAttribute("d")||"";Qs.some(d=>l.includes(d))&&(r=!0)}),!r){let a=(e.id||"").toLowerCase(),i=(e.getAttribute("aria-label")||"").toLowerCase(),l=(e.getAttribute("title")||"").toLowerCase();(a.includes("delete")||a.includes("remove")||a.includes("trash")||i.includes("delete")||i.includes("remove")||i.includes("trash")||l.includes("delete")||l.includes("remove")||l.includes("trash"))&&(r=!0)}r&&e.setAttribute("data-reos-trash","true")})},ti=new Set(["Delete Reminder","Delete","Remove","Trash","Edit","Copy","Duplicate","Add another reminder","Add","Close","Dismiss"]),oi=()=>{document.querySelectorAll(".hr-tooltip, .hr-popover-shared, [role='tooltip']").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let o=(e.textContent||"").trim();o.length===0||o.length>40||ti.has(o)&&e.style.display!=="none"&&(e.style.setProperty("display","none","important"),e.style.setProperty("visibility","hidden","important"),e.style.setProperty("opacity","0","important"),e.style.setProperty("pointer-events","none","important"))})},Dr=new Set,Fr=new WeakSet,ri=()=>{document.body.querySelectorAll("*").forEach(e=>{if(Fr.has(e)&&e.style.getPropertyValue("color")==="rgb(237, 238, 240)"||e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let o=(e.className?.toString()||"").toLowerCase();if(o.includes("ai-button")||o.includes("ask-ai")||o.includes("bg-clip-text")||o.includes("text-transparent")||e.closest("[class*='ai-button'], [class*='ask-ai'], [class*='bg-clip-text'], [class*='text-transparent']"))return;let r=e.tagName.toLowerCase();if(r==="svg"||r==="path"||r==="circle"||r==="rect"||r==="polygon"||r==="polyline"||r==="line"||r==="g")return;let n=!1;for(let m of Array.from(e.childNodes))if(m.nodeType===Node.TEXT_NODE&&(m.textContent||"").trim()){n=!0;break}if(!n)return;let a=Array.from(e.children);if(a.length>0&&a.every(m=>{let p=m.tagName.toLowerCase();return p==="svg"||p==="img"})&&!n)return;let d=window.getComputedStyle(e).color;if(!(d==="rgb(255, 255, 255)"||d==="rgba(255, 255, 255, 1)"||d==="#ffffff"||d==="white"))return;e.style.setProperty("color","#EDEEF0","important"),e.style.setProperty("-webkit-text-fill-color","#EDEEF0","important"),Fr.add(e);let h=e.className?.toString()||"<no class>";!Dr.has(h)&&h!=="<no class>"&&(Dr.add(h),c.debug(`Remapped pure-white text -> bone: "${h}"`))})},$r=new Set,ni=()=>{let t=document.querySelectorAll(".hl_header, header.hl_header, header.app-header, .hl_header--controls, .top-bar, .topbar"),e="rgb(18, 22, 29)";document.querySelectorAll(".hr-content-switcher svg, .hr-content-switcher svg *, .hr-content-switcher i, .hr-radio-button svg, .hr-radio-button svg *, .hr-radio-button i, [class*='hr-content-switcher'] svg, [class*='hr-content-switcher'] svg *, [class*='hr-content-switcher'] i").forEach(r=>{if(r.closest(".hl_header"))return;let n=r.getAttribute("style")||"";(n.includes(e)||n.includes("#12161D")||n.includes("#12161d"))&&(r.style.removeProperty("fill"),r.style.removeProperty("stroke"),r.style.removeProperty("color"))}),t.forEach(r=>{r.querySelectorAll("button, a, [role='button'], [class*='rounded-full'], [class*='rounded-md'], [class*='rounded-xl'], [class*='pill'], span[class*='rounded'], div[class*='rounded']").forEach(a=>{if(a.closest("[id^='ws-crm-']"))return;let i=(a.className?.toString()||"").toLowerCase();if(i.includes("avatar"))return;let l=window.getComputedStyle(a),d=l.backgroundColor,b=l.backgroundImage||"",h=!1;if(b!=="none"&&b.includes("gradient"))h=!0;else if(d&&d!=="rgba(0, 0, 0, 0)"&&d!=="transparent"){let T=d.match(/rgba?\(\d+,\s*\d+,\s*\d+,\s*([\d.]+)\)/);if(T&&parseFloat(T[1])<.6)return;let w=d.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(w){let S=parseInt(w[1],10),A=parseInt(w[2],10),K=parseInt(w[3],10);(S>=60||A>=60||K>=60)&&(S>230&&A>230&&K>230||(S>=80||A>=80||K>=80)&&(h=!0))}}if(!h)return;let p=(a.textContent||"").trim(),u=i,f=Array.from(a.querySelectorAll("*")).map(T=>(T.className?.toString()||"").toLowerCase()).join(" "),k=p.toLowerCase()==="ask ai"||u.includes("ai-button")||u.includes("ask-ai")||u.includes("askai"),v=u.includes("bg-clip-text")||u.includes("text-transparent")||f.includes("bg-clip-text")||f.includes("text-transparent"),_=Array.from(a.querySelectorAll("*")).some(T=>{let w=window.getComputedStyle(T),S=w.getPropertyValue("-webkit-background-clip")||"",A=w.getPropertyValue("background-clip")||"";return S.includes("text")||A.includes("text")});if(k||v||_){let T=["color","-webkit-text-fill-color","fill","stroke","background","background-color","background-image","background-clip","-webkit-background-clip","text-shadow"],w=S=>{T.forEach(A=>S.style.removeProperty(A))};w(a),a.querySelectorAll("*").forEach(w);return}let g="#12161D";a.style.setProperty("color",g,"important"),a.querySelectorAll("*").forEach(T=>{let w=T.tagName.toLowerCase();w==="svg"||w==="path"||w==="circle"||w==="rect"||w==="polygon"||w==="polyline"||w==="line"||w==="g"?(T.style.setProperty("fill",g,"important"),T.style.setProperty("stroke",g,"important"),T.style.setProperty("color",g,"important")):(T.style.setProperty("color",g,"important"),T.style.setProperty("-webkit-text-fill-color",g,"important"))});let E=a.className?.toString()||"<no class>";!$r.has(E)&&E!=="<no class>"&&($r.add(E),c.debug(`Darkened SVG icons on colored button: "${E}"`))})})},Or=new Set,ai=()=>{document.querySelectorAll("[class*='avatar']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;if((e.className?.toString()||"").toLowerCase().includes("default-avatar")){e.querySelectorAll(".n-avatar__text, [class*='avatar__text']").forEach(d=>{(d.getAttribute("style")||"").includes("translateY(11px)")&&d.style.removeProperty("transform")});return}let r=e.getBoundingClientRect();if(r.width===0||r.height===0||r.width<20||r.width>64||r.height<20||r.height>64)return;let n=r.top+r.height/2;(e.parentElement||e).querySelectorAll("svg, img").forEach(d=>{let b=d;if(b.closest("[id^='ws-crm-']"))return;let h=d.getBoundingClientRect();if(h.width===0||h.width>28||h.height>28)return;let m=h.left<r.right&&h.right>r.left,p=h.top<r.bottom&&h.bottom>r.top;if(!(m&&p)||h.top+h.height/2<n)return;let f=h.width*h.height/(r.width*r.height);if(f>.55)return;let k=b,v=b.parentElement,_=0;for(;v&&v!==e&&_<3;){let E=v.getBoundingClientRect();if(E.width>0&&E.width<=30&&E.height<=30)k=v;else break;v=v.parentElement,_++}k.style.setProperty("transform","translateY(11px)","important");let g=(k.className?.toString()||"")+" "+k.tagName;Or.has(g)||(Or.add(g),c.debug(`Dropped activity badge 11px (${h.width.toFixed(0)}x${h.height.toFixed(0)} icon, ratio ${f.toFixed(2)}): "${g}"`))})})},si="brightness(0) saturate(100%) invert(54%) sepia(78%) saturate(442%) hue-rotate(122deg) brightness(96%) contrast(89%)",ii=()=>{document.querySelectorAll("aside.default-bg-color a, [class*='sidebar'] a").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let o=(e.className?.toString()||"").toLowerCase();o.includes("active")||o.includes("router-link-active")?(e.style.setProperty("color","#0FB57E","important"),e.style.setProperty("-webkit-text-fill-color","#0FB57E","important"),e.style.setProperty("opacity","1","important"),e.querySelectorAll("span, p, div").forEach(n=>{n.style.setProperty("color","#0FB57E","important"),n.style.setProperty("-webkit-text-fill-color","#0FB57E","important")}),e.querySelectorAll("img").forEach(n=>{n.style.setProperty("filter",si,"important")}),e.querySelectorAll("svg").forEach(n=>{n.style.setProperty("color","#0FB57E","important"),n.style.setProperty("stroke","#0FB57E","important")})):((e.style.color==="rgb(18, 22, 29)"||e.style.color==="#12161D"||e.style.color==="#0FB57E"||e.style.color==="rgb(15, 181, 126)")&&(e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color")),e.querySelectorAll("span, p, div").forEach(n=>{let a=n.style.color;(a==="rgb(18, 22, 29)"||a==="#12161D"||a==="#0FB57E"||a==="rgb(15, 181, 126)")&&(n.style.removeProperty("color"),n.style.removeProperty("-webkit-text-fill-color"))}),e.querySelectorAll("img").forEach(n=>{n.style.filter&&n.style.filter.includes("hue-rotate(122")&&n.style.removeProperty("filter")}))})},Pr=new Set,li=()=>{document.querySelectorAll(".n-dialog, [class*='n-dialog'], [class*='dialog'], [role='dialog']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;e.querySelectorAll("div, span, i").forEach(r=>{if(r.closest("[id^='ws-crm-']"))return;let n=r.getBoundingClientRect();if(n.width===0||n.height===0||n.width>56||n.height>56||!r.querySelector("svg"))return;let i=window.getComputedStyle(r).backgroundColor.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!i)return;let l=parseInt(i[1],10),d=parseInt(i[2],10),b=parseInt(i[3],10);if(!(l>200&&d>200&&b>200))return;r.style.setProperty("background-color","transparent","important"),r.style.setProperty("background","transparent","important"),r.style.setProperty("box-shadow","none","important"),r.style.setProperty("border","none","important");let h=r.className?.toString()||"<no class>";!Pr.has(h)&&h!=="<no class>"&&(Pr.add(h),c.debug(`Stripped dialog icon circle (${n.width.toFixed(0)}x${n.height.toFixed(0)}, rgb(${l},${d},${b})): "${h}"`))})})},Nr=new Set,di=()=>{document.querySelectorAll("[class*='ai-button'], [class*='ask-ai'], [class*='askai']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let o=e.parentElement,r=0;for(;o&&r<3&&!o.closest("[id^='ws-crm-']");){let n=(o.className?.toString()||"").toLowerCase();if(!(n.includes("ai-button")||n.includes("ask-ai")||n.includes("askai"))){let i=o.getBoundingClientRect();if(i.width>0&&i.height>0&&i.width<=400&&i.height<=80){o.style.setProperty("background-color","#12161D","important"),o.style.setProperty("background-image","none","important");let l=o.className?.toString()||"<no class>";!Nr.has(l)&&l!=="<no class>"&&(Nr.add(l),c.debug(`Painted Ask AI wrapper graphite: "${l}"`))}}o=o.parentElement,r++}})},Hr=new Set,ci=()=>{let t=Array.from(document.querySelectorAll("#contact-conversation-panel, [class*='conversation-panel'], [class*='message-list'], [class*='activity-log'], .hl_header, header.hl_header, [class*='topbar'], [class*='top-bar'], header.app-header, .modal, .modal-content, [class*='modal'], [class*='dialog'], [role='dialog'], [class*='popover'], [class*='popup'], [class*='dropdown-menu'], [class*='select-menu'], [class*='popselect'], [class*='base-select'], .n-base-select-menu, .n-popselect, .n-popover")),e=document.querySelectorAll("div, section, aside"),o=[];e.forEach(n=>{if(n.id?.startsWith("ws-crm-")||n.closest("[id^='ws-crm-']"))return;let a=window.getComputedStyle(n);if(a.position!=="fixed"&&a.position!=="absolute")return;let i=parseInt(a.zIndex,10);if(!isFinite(i)||i<50)return;let l=n.getBoundingClientRect();l.width<240||l.height<120||l.width>1500||l.height>900||o.push(n)}),[...t,...o].forEach(n=>{n.querySelectorAll("*").forEach(i=>{if(i.id?.startsWith("ws-crm-")||i.closest("[id^='ws-crm-']"))return;let l=(i.className?.toString()||"").toLowerCase();if(l.includes("avatar")||l.includes("notification-dot")||l.includes("indicator"))return;if(l.includes("sidebar-option-button")||i.closest(".sidebar-option-button")||i.closest("nav[class*='w-13']")){let v=i.getAttribute("style")||"";(v.includes("background-color")||v.includes("color"))&&(i.style.removeProperty("background-color"),i.style.removeProperty("color"));return}if(l.includes("tabulator")||l.includes("tabulator-page")||i.closest("[class*='tabulator']")){(i.getAttribute("style")||"").includes("background-color")&&i.style.removeProperty("background-color");return}if(l.includes("hr-card")||i.closest(".hr-card")||i.id&&i.id.startsWith("opportunity-")){let v=i.getAttribute("style")||"";(v.includes("background-color")||v.includes("color"))&&(i.style.removeProperty("background-color"),i.style.removeProperty("color"));return}if(l.includes("ai-button")||l.includes("ask-ai")||l.includes("bg-clip-text")||l.includes("text-transparent")||i.closest("[class*='ai-button'], [class*='ask-ai'], [class*='bg-clip-text'], [class*='text-transparent']"))return;let d=window.getComputedStyle(i).backgroundColor;if(!d||d==="rgba(0, 0, 0, 0)"||d==="transparent")return;let b=d.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!b)return;let h=parseInt(b[1],10),m=parseInt(b[2],10),p=parseInt(b[3],10);if(!(h>200&&m>200&&p>200))return;let f=i.getBoundingClientRect();if(f.width<24||f.height<16||f.width>1400)return;i.style.setProperty("background-color","#12161D","important"),i.style.setProperty("color","#EDEEF0","important");let k=i.className?.toString()||"<no class>";!Hr.has(k)&&k!=="<no class>"&&(Hr.add(k),c.debug(`Coerced light bg (${f.width.toFixed(0)}x${f.height.toFixed(0)}, rgb(${h},${m},${p})): "${k}"`))})})},Br=new Set,bi=()=>{let t="input, select, textarea, [class*='hr-input'], [class*='hr-select'], [class*='hr-base-select'], [class*='hr-date-picker'], [class*='hr-datepicker']";document.body.querySelectorAll("*").forEach(o=>{if(o.id?.startsWith("ws-crm-")||o.closest("[id^='ws-crm-']")||o.children.length>0)return;let r=(o.textContent||"").trim();if(r.length===0||r.length>80||r==="--"||o.closest(t)||o.closest("[class*='avatar'], [class*='hr-avatar']")||o.closest("[class*='collapse-item__header'], [class*='hr-collapse-item__header'], [class*='collapse-header'], [class*='folder-header']")||o.closest("[class*='hr-tabs-tab'], [class*='tab-label'], [class*='tabs-nav'], [role='tab']"))return;if(o.matches("[role='heading']")||o.closest("[role='heading']")||o.matches("h1, h2, h3")||o.closest(".hl-card-header, [class*='hl-card-header'], [class*='card-header']")){o.matches("h1, h2, h3")&&(o.getAttribute("style")||"").includes("color")&&(o.style.removeProperty("color"),o.style.removeProperty("-webkit-text-fill-color"));return}if(o.closest(".hr-alert, .hr-alert-body, [class*='hr-alert']")){let g=o.getAttribute("style")||"";(g.includes("color")||g.includes("background"))&&(o.style.removeProperty("color"),o.style.removeProperty("-webkit-text-fill-color"),o.style.removeProperty("background-color"));return}if(o.closest("#SettingTexasSmsBlock")||o.matches("p.ui-text-sm-medium")&&o.parentElement?.classList?.contains("flex")&&o.parentElement?.classList?.contains("items-center")&&o.parentElement?.children?.length===1){(o.getAttribute("style")||"").includes("color")&&(o.style.removeProperty("color"),o.style.removeProperty("-webkit-text-fill-color"));return}if(o.closest(".hr-button, .n-button")||o.matches(".hr-button__content, .n-button__content")||o.closest(".hr-button__content, .n-button__content")){(o.getAttribute("style")||"").includes("color")&&(o.style.removeProperty("color"),o.style.removeProperty("-webkit-text-fill-color"));return}if(o.closest(".bg-primary-50, [class*='bg-primary-50']")){(o.getAttribute("style")||"").includes("color")&&(o.style.removeProperty("color"),o.style.removeProperty("-webkit-text-fill-color"));return}let n=o,a=null;for(let g=0;g<4&&n;g++){let E=n.nextElementSibling;if(E&&(E.matches(t)||E.querySelector(t)!==null)){a=E;break}n=n.parentElement}if(!a)return;let i=o.getBoundingClientRect();if(i.width===0||i.height===0||i.width>400||i.height>60)return;o.style.setProperty("color","#9098A3","important"),o.style.setProperty("-webkit-text-fill-color","#9098A3","important");let l=a,d=l.getBoundingClientRect(),b=l.className?.toString()||"",h=b.includes("schedule-ui-form-weekly-schedule-checkbox")||l.closest(".schedule-ui-form-weekly-schedule-checkbox")!==null,m=b.includes("hl-form-item")||b.includes("n-form-item--top-labelled")||l.closest(".hl-form-item")!==null,p=".hr-input, .hr-input-number, .hr-base-select, .hr-select, .hr-date-picker, .hr-datepicker",u=(b.includes("hr-form-item-blank")||b.includes("hr-form-item")||l.matches(p))&&(l.matches(p)||l.querySelector(p)!==null);u&&(l.getAttribute("style")||"").includes("background-color")&&(l.style.removeProperty("background-color"),l.style.removeProperty("border-color"),l.style.removeProperty("border-style"),l.style.removeProperty("border-width"),l.style.removeProperty("border-radius"));let f=b.includes("hr-radio")||l.closest(".hr-radio, [class*='hr-radio']")!==null;f&&(l.getAttribute("style")||"").includes("background-color")&&(l.style.removeProperty("background-color"),l.style.removeProperty("border-color"),l.style.removeProperty("border-style"),l.style.removeProperty("border-width"),l.style.removeProperty("border-radius"));let k=b.includes("hr-breadcrumb")||l.closest(".hr-breadcrumb, [class*='hr-breadcrumb']")!==null;k&&(l.getAttribute("style")||"").includes("background-color")&&(l.style.removeProperty("background-color"),l.style.removeProperty("border-color"),l.style.removeProperty("border-style"),l.style.removeProperty("border-width"),l.style.removeProperty("border-radius"));let v=b.includes("hr-header-lite")||l.closest("[class*='hr-header-lite']")!==null;v&&(l.getAttribute("style")||"").includes("background-color")&&(l.style.removeProperty("background-color"),l.style.removeProperty("border-color"),l.style.removeProperty("border-style"),l.style.removeProperty("border-width"),l.style.removeProperty("border-radius")),!h&&!m&&!u&&!f&&!k&&!v&&d.width>0&&d.width<=800&&d.height>0&&d.height<=100&&(l.style.setProperty("background-color",Ct,"important"),l.style.setProperty("border-color","#252C36","important"),l.style.setProperty("border-style","solid","important"),l.style.setProperty("border-width","1px","important"),l.style.setProperty("border-radius","4px","important"));let _=o.className?.toString()||"<no class>";!Br.has(_)&&_!=="<no class>"&&(Br.add(_),c.debug(`Colored field label "${r.slice(0,30)}": "${_}"`))})},Rr=new Set,hi=()=>{document.querySelectorAll("[class*='avatar']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let o=(e.className?.toString()||"").toLowerCase();if(o.includes("dropdown")||o.includes("option-avatar"))return;let r=e.getBoundingClientRect();if(r.width===0||r.height===0||r.height>80||e.querySelector("svg, img")!==null)return;let a=(e.textContent||"").trim();if(a.length===0||a.length>4)return;let i=Math.min(r.width,r.height),l=Math.max(16,Math.min(24,Math.round(i*.6)));e.style.setProperty("font-size",`${l}px`,"important"),e.style.setProperty("line-height","1","important"),e.style.setProperty("display","flex","important"),e.style.setProperty("align-items","center","important"),e.style.setProperty("justify-content","center","important"),e.style.setProperty("text-align","center","important"),e.querySelectorAll("*").forEach(b=>{b.tagName==="svg"||b.tagName==="SVG"||b.tagName!=="IMG"&&(b.style.setProperty("font-size",`${l}px`,"important"),b.style.setProperty("line-height","1","important"),b.style.setProperty("text-align","center","important"))});let d=e.className?.toString()||"<no class>";!Rr.has(d)&&d!=="<no class>"&&(Rr.add(d),c.debug(`Resized avatar text to ${l}px (circle ${i.toFixed(0)}px): "${d}"`))})},Gr=new Set,pi=new Set(["Enter Dashboard Name"]),mi=()=>{document.querySelectorAll("input:not([type='checkbox']):not([type='radio']):not([type='button']):not([type='submit']):not([type='reset']):not([type='hidden']):not([type='file']), textarea, select").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let o=e.getBoundingClientRect();if(o.width===0||o.height===0)return;let r=e.placeholder||"";if(pi.has(r)){(e.getAttribute("style")||"").includes("background-color")&&(e.style.removeProperty("background-color"),e.style.removeProperty("border-color"));return}e.style.setProperty("background-color",Ct,"important"),e.style.setProperty("border-color","#252C36","important"),e.style.setProperty("color","#EDEEF0","important"),e.style.setProperty("-webkit-text-fill-color","#EDEEF0","important");let n=e.closest(".hr-input-wrapper, .hr-input, .hr-input__input");n&&(n.style.setProperty("background-color",Ct,"important"),n.style.setProperty("border-color","#252C36","important"));let a=e.className?.toString()||"<no class>";!Gr.has(a)&&a!=="<no class>"&&(Gr.add(a),c.debug(`Forced input slate bg: "${a}"`))})},kt=()=>{document.body.querySelectorAll("*").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.closest("[class*='collapse-item__header'], [class*='hr-collapse-item__header'], [class*='collapse-header'], [class*='folder-header']")||e.id==="chat-filter"||e.id==="phone-calls"||e.id==="archive-conversation"||e.id==="star-toggle"||e.id==="read-toggle"||e.id==="delete-conversation")return;if(e.closest(".hr-tooltip, .hr-popover-shared, .hr-popover, [class*='hr-tooltip'], [class*='hr-popover']")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}if(e.closest(".hr-base-select-menu, .hr-select-menu, .hr-select__menu-container, .hr-dropdown-menu, [class*='hr-base-select-menu']")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}let o=e.className?.toString()||"";if(o.includes("bg-inherit")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(e.closest(".pipeline-ribbon")&&o.split(" ").includes("count")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(o.split(" ").includes("tabulator-page")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}if(["tabulator-cell","tabulator-row","tabulator-col","tabulator-header","tabulator-tableholder","tabulator-table"].some(p=>o.includes(p))){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(o.includes("schedule-ui-form-weekly-schedule-checkbox")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("border-color")||p.includes("border-style")||p.includes("border-width"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("border-color"),e.style.removeProperty("border-style"),e.style.removeProperty("border-width"));return}if(o.includes("n-button")&&o.includes("icon-only")&&!o.includes("quaternary")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(o.includes("hr-button--tertiary-type")||o.includes("n-button--tertiary-type")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color"),e.style.removeProperty("border-color"),e.style.removeProperty("border-style"),e.style.removeProperty("border-width"),e.style.removeProperty("border-radius"));return}if(o.includes("hr-data-table-td")||o.includes("hr-data-table-tr")||o.includes("hr-data-table__body-cell")||o.includes("hr-data-table__cell")||o.includes("hr-data-table__header-cell")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(o.includes("n-data-table-td--fixed-right")||o.includes("n-data-table-td--last-col")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(e.classList.contains("hr-tabs")||o.includes("hr-tabs-tab-wrapper")||o.includes("hr-tabs-nav")||o.includes("hr-tabs-nav-scroll-content")||o.includes("hr-tabs-content")||o.includes("hr-tabs-pane")||o.includes("hr-tabs-rail")||o.includes("hr-tabs-wrapper")||o.includes("hr-tabs-pane-wrapper")||o.includes("hr-tab-pane")){let p=e.getAttribute("style")||"";(p.includes("background-color: rgb(26, 31, 40)")||p.includes("background-color: #1A1F28")||p.includes("background-color: #1a1f28"))&&e.style.removeProperty("background-color");return}if(o.includes("hr-date-panel")||o.includes("n-date-panel")||e.closest(".hr-date-panel, .n-date-panel")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(o.includes("hr-tag")&&o.includes("ui-default")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if((o.includes("bg-purple-50")||o.includes("bg-purple-100"))&&(o.includes("rounded-lg")||o.includes("rounded-md")||o.includes("rounded-full"))){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(o.includes("n-alert")||e.closest(".n-alert")||o.includes("hl-warning")||o.includes("hl-error")||o.includes("hl-info")||o.includes("hl-success")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(o.includes("hr-card")||e.closest(".hr-card")||e.id&&e.id.startsWith("opportunity-")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(o.includes("bg-gray-50")||o.includes("bg-gray-100")||o.includes("bg-gray-200")||o.includes("bg-white")||o.includes("bg-blue-50")||o.includes("bg-blue-100")||o.includes("bg-sky-50")||o.includes("bg-indigo-50")||o.includes("bg-slate-50")||o.includes("bg-neutral-50")||o.includes("bg-zinc-50")||o.includes("bg-stone-50")||o.includes("F7F9FD")||o.includes("F9FAFB")||o.includes("ECEEF2")||o.includes("eceef2")||o.includes("EFF4FF")||o.includes("eff4ff")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(o.includes("hr-collapse-item")||o.includes("hr-collapse hr-accordion")||o.includes("hr-accordion"))return;if(o.includes("sidebar-option-button")||e.closest(".sidebar-option-button")||e.closest("nav[class*='w-13']")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}if(e.closest(".inbox-panel, .conversation-list-container, [class*='conversation-list-container']")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}let n=window.getComputedStyle(e).backgroundColor;if(!n||n==="rgba(0, 0, 0, 0)"||n==="transparent")return;let a=n.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!a)return;let i=parseInt(a[1],10),l=parseInt(a[2],10),d=parseInt(a[3],10);if(!(i<Ys&&l<Xs&&d<Js))return;let h=e.getBoundingClientRect();if(h.width===0||h.height===0||h.width>800||h.height>200)return;e.style.setProperty("background-color",Ct,"important");let m=e.className?.toString()||"<no class>";!Lr.has(m)&&m!=="<no class>"&&(Lr.add(m),c.debug(`Coerced dark bg to slate (${h.width.toFixed(0)}x${h.height.toFixed(0)}, rgb(${i},${l},${d})): "${m}"`))})},Et=()=>{let t="#4B8BF5",e=["#155EEF","#155eef","#004EEB","#004eeb","#175CD3","#175cd3"],o="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";document.querySelectorAll("img[src^='data:image/svg+xml;base64,']:not([data-reos-recolored-v3])").forEach(a=>{a.setAttribute("data-reos-recolored","1"),a.setAttribute("data-reos-recolored-v2","1"),a.setAttribute("data-reos-recolored-v3","1");let l=(a.getAttribute("src")||"").match(/^data:image\/svg\+xml;base64,(.+)$/);if(!l)return;let d;try{d=atob(l[1])}catch{return}let b=d,h=!1;if(e.forEach(u=>{b.includes(u)&&(b=b.split(u).join(t),h=!0)}),["#F8F9FC","#f8f9fc","#F9FAFB","#f9fafb","#F2F4F7","#f2f4f7"].forEach(u=>{b.includes(u)&&(b=b.split(u).join("transparent"),h=!0)}),a.closest(".toolbar_button, .editor-toolbar, [class*='toolbar']")!==null&&["#000000","#000","#040E13","#040e13","#111827","#111","#101828","#101"].forEach(f=>{b.includes(f)&&(b=b.split(f).join("#EDEEF0"),h=!0)}),!!h)try{let u=btoa(b);a.setAttribute("src",`data:image/svg+xml;base64,${u}`)}catch{}}),document.querySelectorAll(".hl-checkbox-group-item img:not([data-reos-masked]):not([data-reos-recolored]), .hl-checkbox-group-item-icon img:not([data-reos-masked]):not([data-reos-recolored]), .hl-checkbox-group-item-container img:not([data-reos-masked]):not([data-reos-recolored])").forEach(a=>{let i=a.getAttribute("src")||"";if(!i||i.startsWith("data:")||!/\.svg(?:[?#]|$)/i.test(i))return;a.setAttribute("data-reos-masked","1");let l=`url("${i}")`;a.style.setProperty("-webkit-mask-image",l,"important"),a.style.setProperty("-webkit-mask-size","contain","important"),a.style.setProperty("-webkit-mask-repeat","no-repeat","important"),a.style.setProperty("-webkit-mask-position","center","important"),a.style.setProperty("mask-image",l,"important"),a.style.setProperty("mask-size","contain","important"),a.style.setProperty("mask-repeat","no-repeat","important"),a.style.setProperty("mask-position","center","important"),a.style.setProperty("background-color","var(--reos-blue)","important"),a.setAttribute("src",o)})},Tt=()=>{document.querySelectorAll("[class*='n-date-panel']").forEach(e=>{[...e.classList].forEach(o=>{if(o.startsWith("n-date-panel")){let r="hr-date-panel"+o.slice(12);e.classList.contains(r)||e.classList.add(r)}})})},ui=()=>{document.querySelectorAll("button.hr-button.hr-button--3xs:not(.hr-button--primary-type), button.hr-button.hr-button--xs:not(.hr-button--primary-type), #manage-association-btn").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let o=e.getAttribute("style")||"";(o.includes("background-color")||o.includes("background:"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("background"))})},qe=()=>{let t=r=>{r.style.getPropertyValue("background-color")!=="rgb(26, 31, 40)"&&r.style.setProperty("background-color","rgb(26, 31, 40)","important")},e=document.querySelector(".crm-opportunities-modal");e&&e.querySelectorAll(".hr-base-selection").forEach(n=>{t(n),n.querySelectorAll("div, span").forEach(i=>{if(i.id?.startsWith("ws-crm-"))return;let l=window.getComputedStyle(i).backgroundColor;l==="rgba(0, 0, 0, 0)"||l==="transparent"||!l||t(i)})}),document.querySelectorAll(".hr-upload-dragger").forEach(r=>{t(r);let n=r.querySelector(".ui-upload-icon");n&&(n.style.setProperty("background-color","transparent","important"),n.style.setProperty("background","transparent","important"));let a="rgb(150, 162, 173)",i="rgb(0, 191, 124)",l=b=>{let h=r.querySelector(".ui-upload-icon svg");h&&(h.style.setProperty("color",b,"important"),h.style.setProperty("stroke",b,"important")),r.querySelectorAll(".ui-upload-icon svg path").forEach(m=>{m.style.setProperty("stroke",b,"important")})},d=!1;try{d=r.matches(":hover")}catch{}l(d?i:a),r.dataset.hoverBound!=="1"&&(r.dataset.hoverBound="1",r.addEventListener("mouseenter",()=>l(i)),r.addEventListener("mouseleave",()=>l(a)))})},xo=new WeakSet,_i=()=>{document.querySelectorAll('iframe[title="Email"]').forEach(e=>{if(!xo.has(e))try{let o=e.contentDocument||e.contentWindow?.document;if(!o||!o.head)return;if(o.querySelector("style[data-ws-crm-email-fix]")){xo.add(e);return}let r=o.createElement("style");r.setAttribute("data-ws-crm-email-fix","1"),r.textContent=`
        html, body {
          background-color: #FFFFFF !important;
          color: #101828 !important;
        }
        body { padding: 16px !important; }
      `,o.head.appendChild(r),e.style.background="#FFFFFF",e.style.borderRadius="6px",xo.add(e)}catch{}})},Ur=()=>{let t,o=()=>{t===void 0&&(t=window.setTimeout(()=>{t=void 0,Tt(),ze(),mi(),kt(),xt(),bi(),_i(),ci(),di(),li(),ii(),ni(),hi(),ai(),ri(),Et(),oi(),ei(),qe(),Ue(),Sr(),Ir(),Ar(),wt(),ui(),qe()},800))};Tt(),ze(),kt(),xt(),Et(),qe(),Ue(),Sr(),Ir(),Ar(),wt(),window.setTimeout(()=>{Tt(),ze(),kt(),xt(),Et(),qe(),Ue(),wt()},500),window.setTimeout(()=>{Tt(),ze(),kt(),xt(),Et(),qe(),Ue(),wt()},1500),typeof requestAnimationFrame=="function"&&requestAnimationFrame(()=>{Ue(),ze()}),new MutationObserver(()=>{o()}).observe(document.body,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["class","style"]})},ko,vi=()=>{let t=()=>{let o=document.getElementById(St);!o||!document.head||document.head.lastElementChild===o||document.head.appendChild(o)};new MutationObserver(o=>{let r=!1;for(let n of o){for(let a of Array.from(n.addedNodes)){if(!(a instanceof Element))continue;let i=a.tagName.toLowerCase();if(i==="style"||i==="link"){if(a.id===St||a.id===zr)continue;r=!0;break}}if(r)break}r&&ko===void 0&&(ko=window.setTimeout(()=>{ko=void 0,t()},150))}).observe(document.head,{childList:!0}),t()},qr=()=>{if(zs(),!document.getElementById(St)){let t=document.createElement("style");t.id=St,t.textContent=qs,document.head.appendChild(t),c.info("REInvest OS theme stylesheet mounted")}window.__reosForceInstalled||(window.__reosForceInstalled=!0,document.body?Ur():window.addEventListener("DOMContentLoaded",Ur)),window.__reosCascadeInstalled||(window.__reosCascadeInstalled=!0,vi())};var Vr="ws-crm-customizer-version-badge",It=()=>{if(!document.body){document.addEventListener("DOMContentLoaded",It,{once:!0});return}if(document.getElementById(Vr))return;let t=document.createElement("div");t.id=Vr,t.style.cssText=`
    position: fixed;
    bottom: 12px;
    right: 12px;
    z-index: 999999;
  ...
  `,t.textContent=`Customizer v${Xe}`,t.title=`Built ${Je}
Click to dismiss for this session`,t.addEventListener("mouseenter",()=>{t.style.transform="scale(1.05)"}),t.addEventListener("mouseleave",()=>{t.style.transform="scale(1)"}),t.addEventListener("click",()=>{t.style.opacity="0",setTimeout(()=>t.remove(),200)}),document.body.appendChild(t)};console.log("[ws-crm-customizer] bundle-loaded");var R=(t,e)=>{try{c.info(`[main] calling ${t}`),e(),c.info(`[main] ${t} OK`)}catch(o){c.error(`[main:FAIL] ${t} threw:`,o)}},Wr=()=>{c.info(`Loaded v${Xe} (built ${Je})`),R("mountVersionBadge (initial)",It),R("mountFavicon",Jo),R("mountThemeStylesheet",qr),R("initPageRouter",Io),c.info("[main] registering onPageChange subscriber"),Co(t=>{c.info(`[main] onPageChange callback fired for page=${t}`),R("mountVersionBadge",It),R("mountClaimReleaseToggle",Ho),R("mountContactActionsBar",Yo),R("mountActiveFollowUpButton",$o),R("mountMtOfferMadeButton",Zo),R("mountNotInterestedButton",er),R("mountReducePriceButton",or),R("mountSendToClosingOfficeButton",ir),R("mountClosingAgentPickerButton",ur),R("mountPrepareContractButton",Tr)}),c.info("[main] onPageChange registered; main() complete")};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Wr):Wr();})();
