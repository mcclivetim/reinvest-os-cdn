...
/* v0.13.99: field-description tooltips are out entirely. Tim's call \u2014
   they overlap fields when positioned by GHL's popover library and
   the information's not necessary for reps in the field. The (i)
   info-circle icon still renders (we don't touch it) but its popover
   never paints because we hide any .hr-tooltip whose content contains
   the field-description span pattern (.font-medium.text-gray-700).
   Other tooltips (Call Status hover, button labels, etc.) keep working
   because they don't match this signature. */
html body .hr-tooltip:has(.font-medium.text-gray-700),
html body .hr-popover-shared:has(.font-medium.text-gray-700),
html body [class*="hr-tooltip"]:has(.font-medium.text-gray-700),
html body [class*="hr-popover"]:has(.font-medium.text-gray-700) {
  display: none !important;
  visibility: hidden !important;
  opacity: 0 !important;
  pointer-events: none !important;
