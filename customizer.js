...
`,I=`
  ...
  flex-shrink: 0;
`;var Xo=/^[A-Za-z0-9]{15,30}$/,Z=o=>typeof o=="string"&&Xo.test(o),Jo=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},Zo=()=>{let o=window.WS_CRM_USER_ID;return Z(o)?o:null},Qo=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(Z(t))return t;return null},er=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=Jo(e);if(!t)return null;let r=t.uid;if(Z(r))return r;let n=t.claims;if(n&&Z(n.user_id))return n.user_id}catch{}return null},tr=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(Z(n))return n}catch{}}}catch{}return null},A=()=>Zo()||Qo()||er()||tr()||null;var Gt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",or=`${Gt}/active-follow-up-get`,rr=`${Gt}/active-follow-up-set`,z="ws-crm-active-follow-up-button",tt={amber:{label:"Add to Active Follow Up",bg:l.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:l.amber,clickable:!0,dotColor:l.amber},blue:{label:"In Active Follow Up",bg:l.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:l.blue,clickable:!1,dotColor:l.blue},gray:{label:"Not in Follow Up",bg:l.slate,border:l.steel,color:l.coolGray,clickable:!1,dotColor:l.coolGray},loading:{label:"Active Follow Up\u2026",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.ash},error:{label:"Follow Up status unavailable",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.crimson}},nr=()=>{let o=S();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},ar=async o=>{let e=await fetch(or,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},sr=async(o,e)=>{let t=await fetch(rr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},W=(o,e)=>{let t=tt[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},Bt=o=>{let e=document.createElement("button");return e.type="button",e.id=z,e.dataset.contactId=o,e.style.cssText=`
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 6px 12px;
  ...
  `,W(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=nr();if(!(!r||r.contactId!==o)){W(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await sr(r,A()),s=n&&n.state&&n.state in tt?n.state:"blue";W(e,s),b.info(`Active Follow Up set for ${o} \u2192 ${s}`)}catch(n){b.error("Active Follow Up Set webhook failed",n),W(e,"error"),setTimeout(()=>void we(e,r),4e3)}}}),e},we=async(o,e)=>{W(o,"loading");try{let t=await ar(e),r=t&&t.state&&t.state in tt?t.state:"gray",n=document.getElementById(z);if(!n||n.dataset.contactId!==e.contactId)return;W(n,r)}catch(t){b.warn("Active Follow Up Get webhook failed",t),W(o,"error")}},ir=[".central-panel","[class*='central-panel']"],et="ws-crm-contact-actions-bar",lr=async()=>{for(let o of ir)try{let e=await C(o,{timeoutMs:3e3});if(e)return e}catch{}try{return(await C("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},dr=async(o,e)=>{let t=e.querySelector(`#${et}`);if(!t)try{t=await C(`#${et}`,{timeoutMs:3e3,root:e})}catch{t=null}if(t){let s=e.querySelector("#ws-crm-afu-wrap");return s&&s.remove(),t.appendChild(o),{container:t,placement:"actions-bar"}}let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
      ...
      flex-shrink: 0;
    `;let s=e.querySelector(`#${et}`);s&&s.parentElement===e?e.insertBefore(n,s.nextSibling):e.insertBefore(n,e.firstChild)}return n.appendChild(o),{container:n,placement:"fallback"}},cr=()=>{let o=document.querySelector("[data-testid='CENTRALPANEL_NAME']");if(!o)return null;let e=o;for(let t=0;t<8&&!(!e||!e.parentElement);t++){let r=e.parentElement,n=(r.className||"").toString();if(n.includes("justify-between")&&n.includes("border-b")){let s=r.children[1];return s instanceof HTMLElement?s:r}e=r}return null},br=(o,e)=>{o.style.marginRight="4px",e.insertBefore(o,e.firstChild)},hr=async o=>{if(o==="url"){let t=await lr();return t?{kind:"contact-detail",target:t}:null}let e=cr();return e?{kind:"conversations",target:e}:null},Rt=async(o,e)=>{if(e.kind==="contact-detail"){let{placement:t}=await dr(o,e.target);return t}return br(o,e.target),"conversations-header"},pr=async()=>{let o=S();if(!o||!o.locationId){document.getElementById(z)?.remove(),document.getElementById("ws-crm-afu-wrap")?.remove();return}let e={contactId:o.contactId,tenantId:o.locationId},t=document.getElementById(z);if(t&&t.dataset.contactId===e.contactId){we(t,e);return}t&&t.remove();let r=await hr(o.source);if(!r){b.warn(`Active Follow Up: no mount target on ${o.source}-source page`);return}let n=S();if(!n||n.contactId!==e.contactId)return;let s=Bt(e.contactId),a=await Rt(s,r);b.debug(`Active Follow Up mounted (${r.kind}/${a}) for ${e.contactId}`),$t();let i=document.getElementById(z)??s;we(i,e);let d=new MutationObserver(()=>{let c=document.getElementById(z);if(!(S()?.contactId===e.contactId)){d.disconnect();return}if(!c){let m=Bt(e.contactId);Rt(m,r).then(()=>{$t(),we(m,e)}),b.debug(`Active Follow Up re-mounted after wipe for ${e.contactId}`)}});d.observe(r.target,{childList:!0})},Q=null,Ut=()=>{let o=S()?.contactId??null;if(Q&&Q.contactId===o)return;let e=pr().finally(()=>{Q?.promise===e&&(Q=null)});Q={contactId:o,promise:e}},$t=()=>{let o=document.querySelectorAll(`#${z}`);if(!(o.length<=1)){b.debug(`Active Follow Up: found ${o.length} duplicate buttons, keeping first, removing ${o.length-1}`);for(let e=1;e<o.length;e++)o[e].remove()}};var G="ws-crm-claim-release-pill",mr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-get",ur="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-set",_r=async o=>{let e=await fetch(mr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,contact_id:o.contactId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Get returned ${e.status} ${e.statusText}`);return await e.json()},vr=async o=>{let e=await fetch(ur,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,contact_id:o.contactId,triggered_by_user_id:o.userId,action:o.action}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Set returned ${e.status} ${e.statusText}`);return await e.json()},zt=o=>{let e=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},X=(o,e)=>{let t=e==="claim"?"emerald":"amber",r=e==="claim"?l.emerald:l.amber;o.dataset.mode=e,o.style.cssText=Pt(t),o.title=e==="claim"?"Take ownership of this lead":"Return this lead to the pool",o.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `},yr=({contactId:o,mode:e})=>{let t=document.createElement("button");return t.id=G,t.type="button",t.dataset.contactId=o,X(t,e),t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-1px)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",async()=>{let r=t.dataset.mode??"claim",n=S();if(!n||!n.locationId||n.contactId!==o)return;let s=A();if(!s){b.warn("Claim/Release click: no user ID detected (login session not exposing it)");let i=t.querySelector(".ws-cr-label");i&&(i.textContent="Sign-in needed"),setTimeout(()=>X(t,r),2500);return}b.info(`Claim/Release click \u2014 contact=${o} mode=${r} user=${s}`);let a=r==="claim"?"release":"claim";X(t,a),t.disabled=!0,t.style.opacity="0.8";try{let i=await vr({contactId:o,locationId:n.locationId,userId:s,action:r}),d=i?.mode==="claim"||i?.mode==="release"?i.mode:a;X(t,d),b.info(`Claim/Release done \u2192 new mode: ${d}`)}catch(i){b.error("Claim/Release Set webhook failed",i),X(t,r);let d=t.querySelector(".ws-cr-label");if(d){let c=d.textContent??"";d.textContent="Try again",setTimeout(()=>{d.textContent==="Try again"&&(d.textContent=c)},2500)}}finally{t.disabled=!1,t.style.opacity="1"}}),t},Vt=async(o,e)=>{try{let t=await _r({contactId:e.contactId,locationId:e.locationId,userId:A()}),r=document.getElementById(G);if(!r||r.dataset.contactId!==e.contactId)return;let n=t?.mode==="release"?"release":"claim";X(r,n)}catch(t){b.warn("Claim/Release Get failed; pill stays in default 'claim' mode",t)}},gr=async(o,e=1e4)=>{let t=Date.now();for(;Date.now()-t<e;){let r=zt(o);if(r)return r;await new Promise(n=>setTimeout(n,150))}return null},qt=(o,e)=>{let t=yr({contactId:e,mode:"claim"}),r=document.createElement("span");r.id=`${G}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(t),o.appendChild(r)},fr=async()=>{let o=S();if(!o){document.getElementById(G)?.remove();return}let e=document.getElementById(G);if(!(e&&e.dataset.contactId===o.contactId)){e&&e.remove();try{let t=await C("#record-details-lhs",{timeoutMs:1e4}),r=await gr(t),n=S();if(!n||n.contactId!==o.contactId)return;if(!r){b.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}qt(r,o.contactId),b.debug(`Mounted Claim/Release pill next to name for ${o.contactId}`);let s=document.getElementById(G);s&&o.locationId&&Vt(s,{contactId:o.contactId,locationId:o.locationId});let a=null;a=new MutationObserver(async()=>{if(!(S()?.contactId===o.contactId)){a?.disconnect();return}if(document.getElementById(G))return;let d=zt(t);if(d){qt(d,o.contactId);let c=document.getElementById(G);c&&o.locationId&&Vt(c,{contactId:o.contactId,locationId:o.locationId}),b.debug("Re-mounted Claim/Release pill after React wipe")}}),a.observe(t,{childList:!0,subtree:!0})}catch(t){b.warn("Could not mount Claim/Release pill:",t)}}},Wt=()=>{fr()};var wr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/type",ee=new Map,jt=async(o,e)=>{if(!o)return null;if(ee.has(o))return ee.get(o)??null;if(!e)return b.warn(`Cannot fetch Contact Type for ${o} \u2014 tenant_id missing from URL context`),null;let t=new AbortController,r=setTimeout(()=>t.abort(),1500);try{let n=`${wr}?contact_id=${encodeURIComponent(o)}&tenant_id=${encodeURIComponent(e)}`,s=await fetch(n,{method:"GET",headers:{Accept:"application/json"},signal:t.signal});if(clearTimeout(r),!s.ok)return b.warn(`Contact Type fetch returned ${s.status} for ${o}`),ee.set(o,null),null;let i=((await s.json()).contact_type||"").toLowerCase().trim()||null;return ee.set(o,i),b.debug(`Resolved Contact Type for ${o}: ${i??"(unset)"}`),i}catch(n){return clearTimeout(r),b.warn(`Contact Type fetch failed for ${o}:`,n),ee.set(o,null),null}};var Kt=o=>o==="buyer"?"buyer":o==="seller"?"seller":o==="agent"||o==="closing_agent"||o==="closing_office"||o==="vendor"?"other":"seller";var eo="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set",xr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/buyer-call-status/set";var kr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Er=async({contactId:o,locationId:e,status:t,userId:r,deadReason:n,webhookUrl:s,kind:a})=>{let i=s||eo,d;if(a==="buyer"){let h=to.find(p=>p.label===t),m=h?h.value:t.toLowerCase().replace(/\s+/g,"-");d={tenant_id:e,contact_id:o,new_status:m,triggered_by_user_id:r??""}}else{let h={call_status:t,triggered_by_user_id:r??""};n&&(h.dead_reason=n),d={contact_id:o,location:{id:e},customData:h}}let c=await fetch(i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d),credentials:"omit"});if(!c.ok)throw new Error(`Webhook returned ${c.status} ${c.statusText}`)},Tr=async({contactId:o,tenantId:e})=>{let t=await fetch(kr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,contact_id:o}),credentials:"omit"});if(!t.ok)throw new Error(`Sanity check returned ${t.status} ${t.statusText}`);return await t.json()};var J="ws-crm-contact-actions-bar",Sr="ws-crm-call-status-dropdown",Yt="ws-crm-offer-made-modal",Cr=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],to=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"vm-left",label:"VM Left",color:"purple"},{value:"qualified-buyer",label:"Qualified Buyer",color:"green"},{value:"upgrade-to-vip",label:"Upgrade to VIP",color:"green"},{value:"walkthrough-scheduled",label:"Walkthrough Scheduled",color:"green"},{value:"made-an-offer",label:"Made an Offer",color:"green"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"wrong-market",label:"Wrong Market",color:"orange"},{value:"no-longer-buying",label:"No Longer Buying",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"blacklisted",label:"Blacklisted",color:"red"}],Ir=[".central-panel","[class*='central-panel']","[data-testid='central-panel']","[data-test='central-panel']",".contact-detail-page .central-panel",".contact-detail-page [class*='central']"],Ar=async()=>{for(let o of Ir)try{let e=await C(o,{timeoutMs:2e3});if(e)return b.debug(`[actions-bar] mount target found via selector: ${o}`),e}catch{}try{let o=await C("#contact-conversation-panel",{timeoutMs:2e3});if(o.parentElement)return b.debug("[actions-bar] mount target found via #contact-conversation-panel.parentElement"),o.parentElement}catch{}for(let o of["[id*='conversation-panel']","[class*='conversation-panel']","[class*='contact-detail'] [class*='panel']","main [class*='central']","main [class*='panel']:not([class*='sidebar']):not([class*='nav'])"])try{let e=await C(o,{timeoutMs:1500});if(e)return b.debug(`[actions-bar] mount target via fallback: ${o}`),e.parentElement??e}catch{}return b.warn("[actions-bar] no mount target found via any selector \u2014 URL:",window.location.pathname),null},Lr=({contactId:o,kind:e})=>{let t=e==="buyer"?to:Cr,r=e==="buyer"?xr:eo,n=e==="buyer"?"Buyer Call Status":"Seller Call Status",s=document.createElement("div");s.id=Sr,s.style.cssText="position: relative; flex-shrink: 0;";let a=document.createElement("button");a.type="button",a.style.cssText=I,a.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `,t.forEach(d=>{let c=document.createElement("button");c.type="button",c.style.cssText=`
    ...
    `,c.innerHTML=`
      <span style="
        ...
        height: 8px;
        border-radius: 50%;
        background: ${Qe[d.color]};
        flex-shrink: 0;
      "></span>
      <span>${d.label}</span>
    `,c.addEventListener("mouseenter",()=>{c.style.background=l.slate}),c.addEventListener("mouseleave",()=>{c.style.background="transparent"}),c.addEventListener("click",async()=>{b.info(`Call Status set to "${d.label}" (${d.value}, ${d.color}) for contact ${o}`);let h=a.firstElementChild,m=a.children[1],p=a.children[2];h&&(h.style.background=Qe[d.color]),m&&(m.textContent=d.label),i.style.display="none";let g=S()?.locationId,x=A();if(!g){b.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),m&&(m.textContent=`${d.label} (no tenant!)`),m&&(m.style.color=l.amber);return}if(e==="seller"&&d.color==="green"&&!x){b.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),m&&(m.textContent=`${d.label} (no user!)`),m&&(m.style.color=l.amber);return}let v=p?.textContent??"\u25BE",_=async y=>{p&&(p.textContent="\u2026"),a.style.opacity="0.75";try{await Er({contactId:o,locationId:g,status:d.label,userId:x,deadReason:y,webhookUrl:r,kind:e}),p&&(p.textContent="\u2713"),setTimeout(()=>{p&&(p.textContent=v),a.style.opacity="1"},900)}catch(E){b.warn("Call Status webhook failed:",E),p&&(p.textContent="\u2717"),m&&(m.style.color=l.amber),a.style.opacity="1",setTimeout(()=>{p&&(p.textContent=v),m&&(m.style.color=l.bone)},1800)}};if(e==="seller"&&d.value==="dead-deal"){let y=m?.textContent??"";Or(E=>{_(E)},()=>{m&&(m.textContent=y)});return}if(e==="seller"&&d.value==="appointment-booked"){let y=m?.textContent??"";try{(await Tr({contactId:o,tenantId:g})).ok?await _():Nr(()=>{_()},()=>{m&&(m.textContent=y)})}catch(E){b.warn("Appointment sanity check failed; firing anyway:",E),await _()}return}await _()}),i.appendChild(c)}),a.addEventListener("click",d=>{d.stopPropagation(),i.style.display=i.style.display==="none"?"block":"none"}),document.addEventListener("click",d=>{s.contains(d.target)||(i.style.display="none")}),s.appendChild(a),s.appendChild(i),s},Fr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/offer-made/set",Mr=async o=>{let e=await fetch(Fr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,kind:o.kind,amount:o.amount,expires_at:o.expiresAt,triggered_by_user_id:o.userId}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},Dr=(o,e="seller")=>{document.getElementById(Yt)?.remove();let t=e==="seller",r=t?"Record Your Offer":"Record Buyer's Offer",n=t?"Attributes the offer to you for KPI tracking. Updates the ACQ opportunity's offer fields.":"Logs the buyer's offer on their Marketing Tracker card. Advances them to 'Made an Offer'.",s=t?l.emerald:l.amber,a=document.createElement("div");a.id=Yt,a.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,a.appendChild(i),document.body.appendChild(a);let m=()=>a.remove();a.addEventListener("click",x=>{x.target===a&&m()});let p=i.querySelector("#ws-offer-error"),u=i.querySelector("#ws-offer-cancel"),g=i.querySelector("#ws-offer-submit");u?.addEventListener("click",m),g?.addEventListener("click",async()=>{let x=(i.querySelector("#ws-offer-amount")?.value??"").trim(),v=t?i.querySelector("#ws-offer-expires")?.value??"":"";if(!x||Number(x)<=0){p&&(p.textContent="Enter a positive offer amount."),b.warn("Offer Made submit blocked \u2014 invalid amount");return}let _=Number(x);if(_<100){p&&(p.textContent=`That looks low \u2014 did you mean $${(_*1e3).toLocaleString("en-US")}?`);return}let E=window.location.pathname.match(/\/v2\/location\/([A-Za-z0-9]+)/),T=E?E[1]:null;if(!T){p&&(p.textContent="Couldn't resolve tenant from URL. Refresh and try again.");return}p&&(p.textContent=""),g&&(g.disabled=!0,g.textContent="Recording\u2026",g.style.opacity="0.7"),u&&(u.disabled=!0);try{let w=await Mr({tenantId:T,contactId:o,kind:e,amount:_,expiresAt:v||null,userId:A()});if(!w.ok){p&&(p.textContent=w.error??"Something went wrong."),g&&(g.disabled=!1,g.textContent="Record Offer",g.style.opacity="1"),u&&(u.disabled=!1);return}b.info(`Offer Made (${e}) recorded for contact ${o}: $${_} on ${w.opp_pipeline} opp (stage: ${w.opp_stage_name??"?"})`),m()}catch(w){b.error("Offer Made webhook failed",w),p&&(p.textContent=w instanceof Error?w.message:"Network error \u2014 try again."),g&&(g.disabled=!1,g.textContent="Record Offer",g.style.opacity="1"),u&&(u.disabled=!1)}}),setTimeout(()=>{i.querySelector("#ws-offer-amount")?.focus()},0)},Xt="ws-crm-dead-deal-modal",Or=(o,e)=>{document.getElementById(Xt)?.remove();let t=document.createElement("div");t.id=Xt,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,t.appendChild(r),document.body.appendChild(t);let n=()=>t.remove(),s=()=>{n(),e()};t.addEventListener("click",i=>{i.target===t&&s()}),r.querySelector("#ws-dead-cancel")?.addEventListener("click",s);let a=()=>{let i=r.querySelector("#ws-dead-reason"),d=r.querySelector("#ws-dead-error"),c=(i?.value??"").trim();if(!c){d&&(d.textContent="Please enter a reason."),i?.focus();return}if(c.length<4){d&&(d.textContent="Please enter at least a few words."),i?.focus();return}n(),o(c)};r.querySelector("#ws-dead-submit")?.addEventListener("click",a),r.querySelector("#ws-dead-reason")?.addEventListener("keydown",i=>{i.key==="Enter"&&(i.metaKey||i.ctrlKey)&&(i.preventDefault(),a()),i.key==="Escape"&&s()}),setTimeout(()=>{r.querySelector("#ws-dead-reason")?.focus()},0)},Jt="ws-crm-appointment-sanity-modal",Nr=(o,e)=>{document.getElementById(Jt)?.remove();let t=document.createElement("div");t.id=Jt,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,t.appendChild(r),document.body.appendChild(t);let n=()=>t.remove(),s=()=>{n(),e()};t.addEventListener("click",i=>{i.target===t&&s()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",s),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),o()});let a=i=>{i.key==="Escape"&&(document.removeEventListener("keydown",a),s())};document.addEventListener("keydown",a)},Zt=(o,e)=>{let t=document.createElement("div");if(t.id=J,t.dataset.contactId=o,t.dataset.contactKind=e,t.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    ...
    border-bottom: 1px solid ${l.steel};
    flex-shrink: 0;
  `,e==="seller"||e==="buyer"){let r=document.createElement("button");r.type="button",r.style.cssText=R,r.textContent="Offer Made",r.addEventListener("click",()=>Dr(o,e)),t.appendChild(r),t.appendChild(Lr({contactId:o,kind:e}))}return t},ot=!1,Hr=()=>{document.querySelectorAll(`[id="${J}"]`).forEach(o=>o.remove())},oo=async()=>{if(b.debug("[actions-bar:01] ensureMounted called; url:",window.location.pathname),ot){b.debug("[actions-bar:00-LOCKED] ensureMounted already in-flight \u2014 skip");return}ot=!0;try{await Pr()}finally{ot=!1}},Pr=async()=>{let o=S();if(!o){b.debug("[actions-bar:02-BAIL] no contact context (URL didn't match patterns AND no in-page contact link). URL:",window.location.pathname),Hr();return}b.debug(`[actions-bar:02] ctx resolved \u2014 contactId=${o.contactId} locationId=${o.locationId??"(null)"} source=${o.source}`);let e="seller";try{let c=await jt(o.contactId,o.locationId),h=Kt(c);h==="buyer"?e="buyer":h==="other"&&(e="other"),b.debug(`[actions-bar:03] kind resolved \u2014 kind=${e} (raw type=${c??"(null)"})`)}catch(c){b.warn("[actions-bar:03-FAIL] Contact Type resolution threw \u2014 defaulting to seller:",c)}let t=S();if(!t||t.contactId!==o.contactId){b.debug("[actions-bar:04-BAIL] context changed during Contact Type fetch \u2014 aborting mount");return}let r=Array.from(document.querySelectorAll(`[id="${J}"]`));if(r.length===1&&r[0].dataset.contactId===o.contactId&&r[0].dataset.contactKind===e){b.debug(`[actions-bar:05-SKIP] bar already mounted for contact=${o.contactId} kind=${e} \u2014 no-op`);return}r.length>0&&(b.debug(`[actions-bar:05] removing ${r.length} stale bar(s) before remount`),r.forEach(c=>c.remove()));let n=await Ar();if(!n){b.warn("[actions-bar:06-FAIL] findMountTarget returned null \u2014 no .central-panel or fallback selector matched within timeout. URL:",window.location.pathname);return}b.debug(`[actions-bar:06] mount target found \u2014 tagName=${n.tagName} class="${n.className}"`);let s=S();if(!s||s.contactId!==o.contactId){b.debug("[actions-bar:07-BAIL] context changed during findMountTarget wait \u2014 aborting mount");return}let a;try{a=Zt(o.contactId,e)}catch(c){b.error("[actions-bar:08-FAIL] buildBar threw:",c);return}a.dataset.mountTarget=n.className||"central-panel";try{n.insertBefore(a,n.firstChild)}catch(c){b.error("[actions-bar:09-FAIL] insertBefore threw:",c);return}if(!document.getElementById(J)){b.warn("[actions-bar:10-DETACHED] bar inserted but not in document tree \u2014 target was likely re-rendered by Vue. Will retry next page-change.");return}b.debug(`[actions-bar:10-OK] bar mounted + verified in document for contact=${o.contactId} kind=${e}`);let d=null;d=new MutationObserver(()=>{if(!(S()?.contactId===o.contactId)){d?.disconnect();return}let h=document.querySelectorAll(`[id="${J}"]`);if(h.length>1){for(let m=1;m<h.length;m++)h[m].remove();b.debug(`[actions-bar:WATCHDOG] trimmed ${h.length-1} duplicate bar(s)`);return}if(h.length===0){let m=document.querySelector(".central-panel")||n,p=Zt(o.contactId,e);p.dataset.mountTarget=a.dataset.mountTarget??"";try{m.insertBefore(p,m.firstChild),b.debug(`[actions-bar:WATCHDOG] re-mounted after wipe for ${o.contactId}`)}catch(u){b.warn("[actions-bar:WATCHDOG-FAIL] re-mount threw:",u)}}}),d.observe(n,{childList:!0})},Qt=null,Br=()=>{Qt===null&&(Qt=window.setInterval(()=>{let o=S();o&&(document.getElementById(J)||(b.debug("[actions-bar:HEALER] no bar on contact page \u2014 re-triggering mount for",o.contactId),oo()))},3e3))},ro=()=>{Br(),oo()};var xe="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",Rr=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),te="data-ws-crm-favicon",rt=o=>o.hasAttribute(te),no=o=>{if(o.tagName!=="LINK")return!1;let e=(o.getAttribute("rel")??"").toLowerCase();return Rr.has(e)},ke=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{no(r)&&!rt(r)&&r.remove()});let e=document.head.querySelector(`link[${te}="1"]`);e?e.href!==xe&&(e.href=xe):(e=document.createElement("link"),e.setAttribute(te,"1"),e.rel="icon",e.type="image/png",e.href=xe,document.head.appendChild(e),b.debug("Favicon installed"));let t=document.head.querySelector(`link[${te}="apple"]`);t||(t=document.createElement("link"),t.setAttribute(te,"apple"),t.rel="apple-touch-icon",t.href=xe,document.head.appendChild(t))},ao=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)ke();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),ke())},50)}new MutationObserver(r=>{let n=!1;for(let s of r)s.addedNodes.forEach(a=>{a instanceof HTMLLinkElement&&no(a)&&!rt(a)&&(n=!0)}),s.removedNodes.forEach(a=>{a instanceof HTMLLinkElement&&rt(a)&&(n=!0)});n&&ke()}).observe(document.head,{childList:!0,subtree:!1});let e=0,t=window.setInterval(()=>{e+=1,ke(),e>=5&&window.clearInterval(t)},1e3)};var $r=["Acquisitions","Transaction Coordination","Disposition","Marketing Tracker"],Gr=/^(?:\d+\)\s*)?(Acquisitions|Transaction Coordination|Disposition|Marketing Tracker)\s*$/,Ur=".hr-card.hr-modal.crm-opportunities-modal",L=()=>{let o=document.querySelector(Ur);if(!o)return null;let e=o.querySelectorAll(".hr-base-selection-overlay__wrapper, .hr-base-selection");for(let t of e){let r=t.textContent?.trim()??"";if(!r)continue;let n=r.match(Gr);if(n){let s=n[1];if($r.includes(s))return s}}return null},U=async(o=2500)=>{let e=Date.now();for(;Date.now()-e<o;){let t=L();if(t!==null)return t;await new Promise(r=>setTimeout(r,100))}return null};var Vr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/offer-made/set",D="ws-crm-mt-offer-made-button",so="ws-crm-mt-offer-made-modal",qr=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,zr=/\/v2\/location\/([A-Za-z0-9]+)/,Se=()=>{let o=window.location.pathname,e=o.match(qr);if(!e)return null;let t=o.match(zr);return{oppId:e[1],locationId:t?t[1]:null}},Wr=async o=>{let e=await fetch(Vr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,mt_opp_id:o.oppId,kind:"buyer",amount:o.amount,triggered_by_user_id:o.userId}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},Ee=()=>{document.getElementById(so)?.remove()},jr=o=>{Ee();let e=document.createElement("div");e.id=so,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(t),document.body.appendChild(e);let r=t.querySelector("#ws-mt-offer-amount"),n=t.querySelector("#ws-mt-offer-error"),s=t.querySelector("#ws-mt-offer-cancel"),a=t.querySelector("#ws-mt-offer-submit");setTimeout(()=>r?.focus(),50);let i=d=>{d.key==="Enter"?(d.preventDefault(),a?.click()):d.key==="Escape"&&(d.preventDefault(),s?.click())};t.addEventListener("keydown",i),e.addEventListener("click",d=>{d.target===e&&Ee()}),s?.addEventListener("click",()=>Ee()),a?.addEventListener("click",async()=>{if(!r||!n||!a)return;let d=r.value.trim();if(!d||Number(d)<=0){n.textContent="Enter a positive offer amount.",r.focus();return}let c=Number(d);if(c<100){n.textContent=`That looks low \u2014 did you mean $${(c*1e3).toLocaleString("en-US")}?`,r.focus();return}n.textContent="",a.disabled=!0,a.textContent="Recording\u2026",a.style.opacity="0.7",s&&(s.disabled=!0);try{let h=await Wr({oppId:o.oppId,locationId:o.locationId,amount:c,userId:A()});if(!h.ok){n.textContent=h.error??"Something went wrong.",a.disabled=!1,a.textContent="Record Offer",a.style.opacity="1",s&&(s.disabled=!1);return}Ee(),o.onSuccess()}catch(h){b.error("MT Offer Made webhook failed",h),n.textContent=h instanceof Error?h.message:"Network error \u2014 try again.",a.disabled=!1,a.textContent="Record Offer",a.style.opacity="1",s&&(s.disabled=!1)}})},Kr=o=>{let e=document.createElement("button");return e.id=D,e.type="button",e.dataset.oppId=o,e.style.cssText=I,e.title="Record this buyer's offer on the property and advance to 'Made an Offer'",e.innerHTML=`
    <span style="
      display: inline-block;
      width: 8px;
      height: 8px;
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{let t=Se();!t||!t.locationId||t.oppId!==o||jr({oppId:t.oppId,locationId:t.locationId,onSuccess:()=>{let r=e.querySelector(".ws-mt-om-label");r&&(r.textContent="Offer Recorded"),e.style.background=l.emeraldGlow,e.style.color=l.emeraldBright,e.style.borderColor=l.emeraldBorder,setTimeout(()=>{r&&r.textContent==="Offer Recorded"&&(r.textContent="Mark Offer Made",e.style.cssText=I)},4e3)}})}),e},Yr=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],Xr=async()=>{for(let o of Yr)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},Te=!1,it=async()=>{if(!Te){Te=!0;try{await Jr()}finally{Te=!1}}},Jr=async()=>{let o=Se();if(!o||!o.locationId){document.getElementById(D)?.remove(),document.getElementById(`${D}-wrap`)?.remove(),re();return}let e=document.getElementById(D);if(e&&e.dataset.oppId===o.oppId){let i=L();i!==null&&i!=="Marketing Tracker"&&(e.remove(),document.getElementById(`${D}-wrap`)?.remove(),re());return}e&&(e.remove(),document.getElementById(`${D}-wrap`)?.remove());let t=await Xr();if(!t){b.warn("MT Offer Made: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await U();if(r!=="Marketing Tracker"){b.debug(`[mt-offer] gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=Se();if(!n||n.oppId!==o.oppId)return;let s=Kr(o.oppId),a=document.createElement("span");a.id=`${D}-wrap`,a.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),a.appendChild(s),t.appendChild(a),b.debug(`Mounted MT Offer Made button on opp ${o.oppId}`),Zr(o.oppId)},oe=null,st=null,re=()=>{oe?.disconnect(),oe=null,st=null},Zr=o=>{if(oe&&st===o)return;re();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(st=o,oe=new MutationObserver(()=>{let t=Se();if(!t||t.oppId!==o){re();return}let r=L();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(D)?.remove(),document.getElementById(`${D}-wrap`)?.remove(),re();return}document.getElementById(D)||it()}),oe.observe(e,{childList:!0,subtree:!0}))},nt=null,at,Qr=()=>{nt||(nt=new MutationObserver(()=>{at===void 0&&(at=window.setTimeout(()=>{at=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(D);o&&!e&&!Te&&it()},300))}),nt.observe(document.body,{childList:!0,subtree:!0}))},io=()=>{Qr(),it()};var en="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/mt/not-interested",O="ws-crm-not-interested-button",tn=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,on=/\/v2\/location\/([A-Za-z0-9]+)/,Ie=()=>{let o=window.location.pathname,e=o.match(tn);if(!e)return null;let t=o.match(on);return{oppId:e[1],locationId:t?t[1]:null}},rn=async o=>{let e=await fetch(en,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,mt_opp_id:o.oppId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Not Interested webhook returned ${e.status} ${e.statusText}`);return await e.json()},nn=o=>{let e=document.createElement("button");return e.id=O,e.type="button",e.dataset.oppId=o,e.style.cssText=I,e.title="Close this Marketing Tracker opp as Not Interested for THIS property only",e.innerHTML=`
    <span style="
      display: inline-block;
      width: 8px;
      height: 8px;
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let t=Ie();if(!t||!t.locationId||t.oppId!==o)return;let r=A(),n=e.querySelector(".ws-ni-label");e.disabled=!0,e.style.opacity="0.7",n&&(n.textContent="Closing\u2026");try{let s=await rn({oppId:t.oppId,locationId:t.locationId,userId:r});if(s?.ok)n&&(n.textContent="Marked Not Interested"),e.style.background=l.crimsonGlow,e.style.color=l.crimson,b.info(`Not Interested set for opp ${o} \u2192 ${s.new_stage}`);else throw new Error("Webhook returned ok=false")}catch(s){b.error("Not Interested webhook failed",s),n&&(n.textContent="Try again"),e.disabled=!1,e.style.opacity="1",setTimeout(()=>{n&&n.textContent==="Try again"&&(n.textContent="Mark Not Interested")},3e3)}}),e},an=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],sn=async()=>{for(let o of an)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},Ce=!1,bt=async()=>{if(!Ce){Ce=!0;try{await ln()}finally{Ce=!1}}},ln=async()=>{let o=Ie();if(!o||!o.locationId){document.getElementById(O)?.remove(),document.getElementById(`${O}-wrap`)?.remove(),ae();return}let e=document.getElementById(O);if(e&&e.dataset.oppId===o.oppId){let i=L();i!==null&&i!=="Marketing Tracker"&&(e.remove(),document.getElementById(`${O}-wrap`)?.remove(),ae());return}e&&(e.remove(),document.getElementById(`${O}-wrap`)?.remove());let t=await sn();if(!t){b.warn("Not Interested: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await U();if(r!=="Marketing Tracker"){b.debug(`[not-interested] gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=Ie();if(!n||n.oppId!==o.oppId)return;let s=nn(o.oppId),a=document.createElement("span");a.id=`${O}-wrap`,a.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),a.appendChild(s),t.appendChild(a),b.debug(`Mounted Not Interested button on opp ${o.oppId}`),dn(o.oppId)},ne=null,ct=null,ae=()=>{ne?.disconnect(),ne=null,ct=null},dn=o=>{if(ne&&ct===o)return;ae();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(ct=o,ne=new MutationObserver(()=>{let t=Ie();if(!t||t.oppId!==o){ae();return}let r=L();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(O)?.remove(),document.getElementById(`${O}-wrap`)?.remove(),ae();return}document.getElementById(O)||bt()}),ne.observe(e,{childList:!0,subtree:!0}))},lt=null,dt,cn=()=>{lt||(lt=new MutationObserver(()=>{dt===void 0&&(dt=window.setTimeout(()=>{dt=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(O);o&&!e&&!Ce&&bt()},300))}),lt.observe(document.body,{childList:!0,subtree:!0}))},lo=()=>{cn(),bt()};var bn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/property/listing-price-changed",N="ws-crm-reduce-price-button",co="ws-crm-reduce-price-modal",hn=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,pn=/\/v2\/location\/([A-Za-z0-9]+)/,Fe=()=>{let o=window.location.pathname,e=o.match(hn);if(!e)return null;let t=o.match(pn);return{oppId:e[1],locationId:t?t[1]:null}},mn=async o=>{let e=await fetch(bn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,dispo_opp_id:o.oppId,new_price:o.newPrice,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Reduce Price webhook returned ${e.status} ${e.statusText}`);let t=await e.text();try{let r=JSON.parse(t);if(r?.ok===!1&&r?.error)throw new Error(r.error);return r}catch(r){if(r instanceof Error&&/returned/.test(r.message))throw r;return{}}},Ae=()=>{document.getElementById(co)?.remove()},un=o=>{Ae();let e=document.createElement("div");e.id=co,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(t),document.body.appendChild(e);let r=t.querySelector("#ws-reduce-price-input"),n=t.querySelector("#ws-reduce-error"),s=t.querySelector("#ws-reduce-cancel"),a=t.querySelector("#ws-reduce-submit");setTimeout(()=>r?.focus(),50),r?.addEventListener("input",()=>{let c=r.value.replace(/[^0-9.]/g,"").split(".");r.value=c[0]+(c.length>1?"."+c.slice(1).join(""):"")});let i=d=>{d.key==="Enter"?(d.preventDefault(),a?.click()):d.key==="Escape"&&(d.preventDefault(),s?.click())};t.addEventListener("keydown",i),e.addEventListener("click",d=>{d.target===e&&Ae()}),s?.addEventListener("click",()=>{Ae()}),a?.addEventListener("click",async()=>{if(!r||!n||!a)return;let d=r.value.trim();if(!d){n.textContent="Enter a new listing price.",r.focus();return}let c=parseFloat(d);if(!Number.isFinite(c)||c<=0){n.textContent="Price must be a positive number.",r.focus();return}if(c<1e3){n.textContent=`That looks low \u2014 did you mean $${Math.round(c*1e3).toLocaleString("en-US")}?`,r.focus();return}n.textContent="",a.disabled=!0,a.textContent="Reducing\u2026",a.style.opacity="0.7",s&&(s.disabled=!0);try{await mn({oppId:o.oppId,locationId:o.locationId,newPrice:c,userId:A()}),Ae(),o.onSuccess()}catch(h){b.error("Reduce Price webhook failed",h),n.textContent=h instanceof Error?h.message:"Something went wrong. Try again.",a.disabled=!1,a.textContent="Reduce Price",a.style.opacity="1",s&&(s.disabled=!1)}})},_n=(o,e)=>{let t=document.createElement("button");return t.id=N,t.type="button",t.dataset.oppId=o,t.style.cssText=I,t.title="Set a new listing price and notify active + previously-withdrew buyers",t.innerHTML=`
    <span style="
      display: inline-block;
      width: 8px;
      height: 8px;
      border-radius: 50%;
      background: ${l.amber};
      box-shadow: 0 0 6px ${l.amber};
      flex-shrink: 0;
    "></span>
    <span class="ws-rp-label">Reduce Price and Alert Buyers</span>
  `,t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-1px)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",()=>{let r=Fe();!r||!r.locationId||r.oppId!==o||un({oppId:r.oppId,locationId:r.locationId,onSuccess:()=>{let n=t.querySelector(".ws-rp-label"),s=t.querySelector(":scope > span:first-child");n&&(n.textContent="Price Reduced \u2014 Buyers Alerted"),t.style.background=l.emeraldGlow,t.style.color=l.emeraldBright,t.style.borderColor=l.emeraldBorder,s&&(s.style.background=l.emerald,s.style.boxShadow=`0 0 6px ${l.emerald}`),setTimeout(()=>{n&&n.textContent==="Price Reduced \u2014 Buyers Alerted"&&(n.textContent="Reduce Price and Alert Buyers",t.style.cssText=I,s&&(s.style.background=l.amber,s.style.boxShadow=`0 0 6px ${l.amber}`))},4e3)}})}),t},vn=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],yn=async()=>{for(let o of vn)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},Le=!1,ut=async()=>{if(!Le){Le=!0;try{await gn()}finally{Le=!1}}},gn=async()=>{let o=Fe();if(!o||!o.locationId){document.getElementById(N)?.remove(),document.getElementById(`${N}-wrap`)?.remove(),ie();return}let e=document.getElementById(N);if(e&&e.dataset.oppId===o.oppId){let i=L();i!==null&&i!=="Disposition"&&(e.remove(),document.getElementById(`${N}-wrap`)?.remove(),ie());return}e&&(e.remove(),document.getElementById(`${N}-wrap`)?.remove());let t=await yn();if(!t){b.warn("Reduce Price: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await U();if(r!=="Disposition"){b.debug(`[reduce-price] gated off \u2014 pipeline="${r}" (expected Disposition)`);return}let n=Fe();if(!n||n.oppId!==o.oppId)return;let s=_n(o.oppId,o.locationId),a=document.createElement("span");a.id=`${N}-wrap`,a.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),a.appendChild(s),t.appendChild(a),b.debug(`Mounted Reduce Price button on opp ${o.oppId}`),fn(o.oppId)},se=null,mt=null,ie=()=>{se?.disconnect(),se=null,mt=null},fn=o=>{if(se&&mt===o)return;ie();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(mt=o,se=new MutationObserver(()=>{let t=Fe();if(!t||t.oppId!==o){ie();return}let r=L();if(r!==null&&r!=="Disposition"){document.getElementById(N)?.remove(),document.getElementById(`${N}-wrap`)?.remove(),ie();return}document.getElementById(N)||ut()}),se.observe(e,{childList:!0,subtree:!0}))},ht=null,pt,wn=()=>{ht||(ht=new MutationObserver(()=>{pt===void 0&&(pt=window.setTimeout(()=>{pt=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(N);o&&!e&&!Le&&ut()},300))}),ht.observe(document.body,{childList:!0,subtree:!0}))},bo=()=>{wn(),ut()};var mo="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/tc/send-to-title-co",V="ws-crm-send-to-closing-office-button",j="ws-crm-send-to-title-co-wrap",Oe="ws-crm-stc-dropdown",xn=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,kn=/\/v2\/location\/([A-Za-z0-9]+)/,Ne=()=>{let o=window.location.pathname,e=o.match(xn);if(!e)return null;let t=o.match(kn);return{oppId:e[1],locationId:t?t[1]:null}},En=5*60*1e3,uo=o=>`ws-crm-stc-cache:${o}`,Tn=o=>{try{let e=window.localStorage.getItem(uo(o));if(!e)return null;let t=JSON.parse(e);return!t.ts||Date.now()-t.ts>En||!Array.isArray(t.checks)||t.checks.length===0?null:t}catch{return null}},Sn=(o,e)=>{try{window.localStorage.setItem(uo(o),JSON.stringify(e))}catch{}},Cn=async(o,e)=>{try{let t=await fetch(mo,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:o,triggered_by_user_id:A()??null,dry_run:!0}),credentials:"omit"});return t.ok?await t.json():(b.warn(`[stc] dry_run returned HTTP ${t.status}`),null)}catch(t){return b.warn("[stc] dry_run fetch failed:",t),null}},In=async(o,e,t)=>{let r=await fetch(mo,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:o,triggered_by_user_id:A()??null,confirm_resend:t}),credentials:"omit"});if(!r.ok&&r.status!==200)throw new Error(`HTTP ${r.status}`);return await r.json()},An=o=>{let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(!e)return!1;let t=e.querySelectorAll("a, button, li, [role='tab'], [role='menuitem'], .crm-opportunities-modal-content [class*='tab']");for(let n of t)if((n.textContent?.trim()??"")===o)return n.click(),!0;let r=e.querySelectorAll("*");for(let n of r){if(n.children.length>1)continue;if((n.textContent?.trim()??"")===o)return(n.closest("a, button, li, [role='tab']")||n.parentElement||n).click(),!0}return b.warn(`[stc] folder tab not found for "${o}"`),!1},ho=o=>{let e=document.getElementById(o);if(!e){b.warn(`[stc] field ${o} not found in DOM \u2014 can't scroll to it`);return}e.scrollIntoView({behavior:"smooth",block:"center"});let t=e.style.boxShadow,r=e.style.transition;e.style.transition="box-shadow 0.3s ease, background-color 0.3s ease",e.style.boxShadow=`0 0 0 3px ${l.emerald}, 0 0 18px ${l.emerald}`,window.setTimeout(()=>{e.style.boxShadow=t,window.setTimeout(()=>{e.style.transition=r},400)},2800)},Ln=(o,e)=>{if(e&&An(e)){window.setTimeout(()=>ho(o),300);return}ho(o)},H=()=>{document.getElementById(Oe)?.remove()},Me=(o,e,t)=>{H();let r=document.createElement("div");r.id=Oe;let n=t.getBoundingClientRect();r.style.cssText=`
    position: fixed;
    top: ${n.bottom+6}px;
    ...
    padding: 14px 16px;
    font-family: ${f.sans};
  `;let s=o.length,a=o.filter(u=>u.filled).length,i=a===s,d=document.createElement("div");d.innerHTML=`
    <div style="
      font-family: ${f.mono};
      font-size: 10px;
      letter-spacing: 0.14em;
      text-transform: uppercase;
      color: ${i?l.emerald:l.amber};
      margin-bottom: 4px;
    ">${i?"Ready to send":"Required fields"}</div>
    <div style="font-size: 14px; font-weight: 500; color: ${l.bone}; margin-bottom: 6px;">${a} of ${s} ready</div>
    ${e?`<div style="
            font-size: 11px;
            color: ${l.amber};
            margin: 6px 0 4px;
            ...
            background: ${l.slate};
            border-radius: ${k.sm};
            border-left: 2px solid ${l.amber};
          ">Previously sent: ${new Date(e).toLocaleString()}.
          Clicking Send again will re-send.</div>`:""}
  `,r.appendChild(d);let c=["TC","ACQ","MT","DISPO","Property"],h={};o.forEach(u=>{h[u.group]||(h[u.group]=[]),h[u.group].push(u)});let m=[...c.filter(u=>h[u]),...Object.keys(h).filter(u=>!c.includes(u))];for(let u of m){let g=h[u],x=document.createElement("div");x.style.cssText="margin-top: 12px;";let v=document.createElement("div");v.style.cssText=`
      font-family: ${f.mono};
      font-size: 9px;
      ...
      padding-bottom: 4px;
      border-bottom: 1px solid ${l.steel};
    `,v.textContent=u,x.appendChild(v),g.forEach(_=>{let y=document.createElement("div"),E=!_.filled&&_.navigable&&_.field_id;y.style.cssText=`
        display: flex;
        align-items: center;
        gap: 10px;
        padding: 6px 8px;
        border-radius: ${k.sm};
        font-size: 12px;
        color: ${_.filled?l.bone:l.ash};
        ${E?"cursor: pointer;":""}
        transition: background 0.1s ease;
      `,E&&(y.addEventListener("mouseenter",()=>{y.style.background=l.slate}),y.addEventListener("mouseleave",()=>{y.style.background="transparent"}),y.addEventListener("click",F=>{F.stopPropagation(),_.field_id&&(Ln(_.field_id,_.folder),H())}));let T=_.filled?`<span style="color: ${l.emerald}; font-size: 14px; line-height: 1; flex-shrink: 0;">\u2713</span>`:`<span style="color: ${l.amber}; font-size: 14px; line-height: 1; flex-shrink: 0;">\u25CB</span>`,w=E?`<span style="margin-left: auto; color: ${l.ash}; font-size: 11px;">\u2192</span>`:"";y.innerHTML=`${T}<span style="flex: 1;">${_.label}</span>${w}`,x.appendChild(y)}),r.appendChild(x)}document.body.appendChild(r);let p=u=>{let g=u.target;!r.contains(g)&&!t.contains(g)&&(H(),document.removeEventListener("click",p))};window.setTimeout(()=>document.addEventListener("click",p),0)},B=(o,e)=>{let t=o.querySelector(".ws-stc-label"),r=o.querySelector(".ws-stc-dot"),n=o.querySelector(".ws-stc-chevron"),s,a,i=l.bone,d,c="Send to Closing Office",h="",m="pointer",p="1";switch(e.status){case"checking":s="transparent",a=l.steel,i=l.ash,d=l.coolGray,c="Checking\u2026",m="wait",p="0.8";break;case"ready":s=l.emerald,a=l.emerald,i="#0a0e14",d="#0a0e14",c=e.previouslySentAt?"Re-send to Closing Office":"Send to Closing Office";break;case"incomplete":{let u=e.checks?.filter(x=>x.filled).length??0,g=e.checks?.length??0;s="transparent",a=l.amber,i=l.amber,d=l.amber,c=`${u} of ${g} fields ready`,h="\u25BE";break}case"sending":s=l.emerald,a=l.emerald,i="#0a0e14",d="#0a0e14",c="Sending\u2026",m="wait",p="0.8";break;case"sent":s=l.emerald,a=l.emerald,i="#0a0e14",d="#0a0e14",c="Sent \u2713";break;case"error":s="transparent",a=l.amber,i=l.amber,d=l.amber,c=e.message??"Try again";break}o.style.background=s,o.style.borderColor=a,o.style.color=i,o.style.cursor=m,o.style.opacity=p,r&&(r.style.background=d,r.style.boxShadow=e.status==="ready"||e.status==="sent"?`0 0 6px ${d}`:""),t&&(t.textContent=c),n&&(n.textContent=h)},Fn=(o,e)=>{let t=document.createElement("button");t.id=V,t.type="button",t.dataset.oppId=o;let r=Tn(o),n=r?{status:r.checks.every(v=>v.filled)?"ready":"incomplete",checks:r.checks,previouslySentAt:r.previously_sent_at}:{status:"checking"};t.style.cssText=`
    ${R}
    background: transparent;
    border: 1px solid ${l.steel};
    color: ${l.ash};
  ...
  `,B(t,n);let s=new Set,a=async()=>{if(n.status==="sending")return;let v=await Cn(o,e);if(!v){n={status:"error",message:"Check failed \u2014 click to retry"},B(t,n);return}if(v.rate_limited){u=Date.now()+p,n={status:"error",checks:n.checks,message:"Rate-limited by GHL \u2014 will retry in 5m"},B(t,n),b.warn(`[stc] dry_run rate-limited \u2014 cooling down until ${new Date(u).toLocaleTimeString()}`);return}if(!Array.isArray(v.checks)||v.checks.length===0){n={status:"error",message:v.error||"Check failed \u2014 click to retry"},B(t,n),b.warn("[stc] dry_run returned no checks:",v);return}let _=v.pipeline_name;if(_&&_!=="Transaction Coordination"){b.debug(`[stc] dry_run reports pipeline="${_}" (not TC) \u2014 retracting button + caching`),He.add(o),document.getElementById(V)?.remove(),document.getElementById(j)?.remove(),H(),de();return}let y=v.checks;if(n.checks&&n.checks.length===v.checks.length){let T=new Set;y=v.checks.map((w,F)=>n.checks[F]?.filled&&!w.filled?s.has(w.label)?w:(T.add(w.label),{...w,filled:!0}):w),s=T}else s=new Set;n={status:y.every(T=>T.filled)?"ready":"incomplete",checks:y,previouslySentAt:v.previously_sent_at??null},Sn(o,{checks:y,previously_sent_at:v.previously_sent_at??null,pipeline_name:_,ts:Date.now()}),B(t,n),document.getElementById(Oe)&&Me(y,v.previously_sent_at,t)},i=async()=>{let v=!!n.previouslySentAt;n={status:"sending",checks:n.checks,previouslySentAt:n.previouslySentAt},B(t,n),H();try{let _=await In(o,e,v);if(_.needs_confirm){n={status:"ready",checks:_.checks,previouslySentAt:_.previously_sent_at},B(t,n),_.checks&&Me(_.checks,_.previously_sent_at,t);return}if(_.ok){n={status:"sent",checks:_.checks,previouslySentAt:new Date().toISOString()},B(t,n),window.setTimeout(()=>{g()},3500);return}n={status:"error",checks:_.checks,message:_.error||"Validation failed"},B(t,n),_.checks&&Me(_.checks,_.previously_sent_at,t)}catch(_){b.error("[stc] send failed:",_),n={status:"error",message:"Network error \u2014 click to retry"},B(t,n)}};t.addEventListener("click",()=>{switch(n.status){case"ready":i();break;case"incomplete":{document.getElementById(Oe)?H():n.checks&&Me(n.checks,n.previouslySentAt,t);break}case"error":case"sent":g();break}});let d,c=!1,h=Date.now(),m=1e4,p=5*60*1e3,u=0,g=async()=>{if(!c){if(Date.now()<u){b.debug(`[stc] dry_run suppressed \u2014 rate-limit cooldown until ${new Date(u).toLocaleTimeString()}`);return}c=!0,h=Date.now();try{await a()}finally{c=!1}}};g();let x=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(x){let v=new MutationObserver(()=>{if(d!==void 0||c)return;let y=Date.now()-h,E=Math.max(1500,m-y);d=window.setTimeout(()=>{d=void 0,n.status!=="sending"&&g()},E)});v.observe(x,{childList:!0,subtree:!0,characterData:!0}),t.dataset.observerActive="1";let _=window.setInterval(()=>{document.body.contains(t)||(v.disconnect(),d!==void 0&&window.clearTimeout(d),window.clearInterval(_))},5e3)}return t},Mn=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],Dn=async()=>{for(let o of Mn)try{let e=await C(o,{timeoutMs:1500});if(e)return b.debug(`[send-to-title-co] mount target: ${o}`),e}catch{}return b.warn(`[send-to-title-co] no mount target found. URL: ${window.location.pathname}`),null},He=new Set,po=null,On=o=>{o&&o!==po&&(He.clear(),po=o)},De=!1,gt=async()=>{if(De){b.debug("[stc] mount:00-LOCKED ensureMounted already in-flight \u2014 skip");return}De=!0;try{await Nn()}catch(o){b.error("[stc] ensureMountedImpl threw:",o)}finally{De=!1}},Nn=async()=>{b.debug("[stc] mount:01 ensureMountedImpl entered");let o=Ne();if(!o||!o.locationId){b.debug(`[stc] mount:02-BAIL no ctx or no locationId. ctx=${JSON.stringify(o)}`),document.getElementById(V)?.remove(),document.getElementById(j)?.remove(),H();return}if(b.debug(`[stc] mount:02 ctx ok \u2014 oppId=${o.oppId} locId=${o.locationId}`),On(o.oppId),He.has(o.oppId)){b.debug(`[stc] mount:02-BAIL opp ${o.oppId} previously confirmed non-TC this session`),document.getElementById(V)?.remove(),document.getElementById(j)?.remove(),H();return}let e=document.getElementById(V);if(e&&e.dataset.oppId===o.oppId){b.debug("[stc] mount:03-SKIP existing button for same opp");let i=L();i!==null&&i!=="Transaction Coordination"&&(e.remove(),document.getElementById(j)?.remove(),H(),de());return}e&&(b.debug("[stc] mount:03 removing stale existing button"),e.remove(),document.getElementById(j)?.remove(),H()),b.debug("[stc] mount:04 calling findMountTarget");let t=await Dn();if(!t){b.debug("[stc] mount:04-BAIL findMountTarget returned null");return}b.debug(`[stc] mount:04 target found tag=${t.tagName} class="${t.className}"`);let r=L();if(r!==null&&r!=="Transaction Coordination"){b.debug(`[stc] mount:05-GATED DOM pipeline="${r}" \u2014 skipping mount`);return}let n=Ne();if(!n||n.oppId!==o.oppId||!n.locationId){b.debug(`[stc] mount:06-BAIL ctx changed during awaits. was=${o.oppId} now=${n?.oppId}`);return}b.debug("[stc] mount:06 building button (pipeline gate will run via dry_run)");let s=Fn(o.oppId,n.locationId),a=document.createElement("span");a.id=j,a.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),a.appendChild(s),t.appendChild(a),b.debug(`[stc] mount:07-OK Mounted on opp ${o.oppId}, inDoc=${document.body.contains(s)}`),Hn(o.oppId)},le=null,yt=null,de=()=>{le?.disconnect(),le=null,yt=null},Hn=o=>{if(le&&yt===o)return;de();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(yt=o,le=new MutationObserver(()=>{let t=Ne();if(!t||t.oppId!==o){de();return}let r=L();if(r!==null&&r!=="Transaction Coordination"){document.getElementById(V)?.remove(),document.getElementById(j)?.remove(),H(),de();return}document.getElementById(V)||gt()}),le.observe(e,{childList:!0,subtree:!0}))},_t=null,vt,Pn=()=>{_t||(_t=new MutationObserver(()=>{vt===void 0&&(vt=window.setTimeout(()=>{vt=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(V);if(o&&!e&&!De){let t=Ne();if(t&&He.has(t.oppId))return;gt()}},300))}),_t.observe(document.body,{childList:!0,subtree:!0}))},_o=()=>{Pn(),gt()};var Bn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/search",Rn="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/opp/set-field",$n="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/opp/get-field",go="ldtyB2UXKi9vOp8nLUH7",Gn="WVD2gb9swLsgcdBEtDVq",P="ws-crm-closing-agent-picker-button",fo="ws-crm-closing-agent-picker-modal",Un=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Vn=/\/v2\/location\/([A-Za-z0-9]+)/,he=()=>{let o=window.location.pathname,e=o.match(Un);if(!e)return null;let t=o.match(Vn);return{oppId:e[1],locationId:t?t[1]:null}},Ct=async o=>{let e=await fetch(Bn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,query:o.query??"",contact_id:o.contactId??null,contact_type:o.contactType??null,limit:o.limit??20}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},vo=async o=>{let e=await fetch(Rn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,opp_id:o.oppId,field_id:o.fieldId,value:o.value}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},qn=async(o,e)=>{try{let t=await fetch($n,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,opp_id:o,field_id:go}),credentials:"omit"});if(!t.ok)return null;let r=await t.json(),n=typeof r.value=="string"&&r.value.trim()?r.value.trim():null;if(!n||!/^[A-Za-z0-9]{16,}$/.test(n))return null;let s=await Ct({tenantId:e,contactId:n,limit:1});if(!s.ok||!s.results.length)return null;let a=s.results[0];return{id:a.id,name:a.name,companyName:a.companyName??""}}catch(t){return b.warn("[closing-agent] fetchCurrent failed:",t),null}},K=null,zn=60*60*1e3,It=o=>`ws-crm-to-cache:${o}`,wo=o=>{try{let e=window.localStorage.getItem(It(o));if(!e)return null;let t=JSON.parse(e);return!t.ts||Date.now()-t.ts>zn||typeof t.contactId!="string"||!t.contactId||typeof t.contactName!="string"||!t.contactName?null:t}catch{return null}},xo=(o,e,t)=>{try{window.localStorage.setItem(It(o),JSON.stringify({contactId:e,contactName:t,ts:Date.now()}))}catch{}},Wn=o=>{try{window.localStorage.removeItem(It(o))}catch{}},kt="ws-crm-closing-agent-field-link",jn=()=>{let o=document.querySelector(".crm-opportunities-modal");o&&(o.querySelectorAll(`.${kt}`).forEach(e=>e.remove()),o.querySelectorAll("input[data-ws-closing-agent-decorated]").forEach(e=>{e.style.removeProperty("color"),e.style.removeProperty("caret-color"),e.style.removeProperty("visibility"),delete e.dataset.wsClosingAgentDecorated}))},ft=!1,At=async o=>{if(ft)return;let e=ko();if(!e)return;let t=(e.value||"").trim();if(!t){b.info("[closing-agent] tryDecorate: input has empty value");return}if(!/^[A-Za-z0-9]{16,}$/.test(t)){b.info(`[closing-agent] tryDecorate: value '${t}' doesn't match contact-ID pattern (expecting 16+ alphanumeric)`);return}if(e.dataset.wsClosingAgentDecorated!==t){ft=!0;try{b.info(`[closing-agent] tryDecorate: looking up ${t}`);let r=await Ct({tenantId:o,contactId:t,limit:1});if(!r.ok||!r.results.length){b.warn(`[closing-agent] tryDecorate: contact ${t} not found`);return}let n=r.results[0];b.info(`[closing-agent] tryDecorate: applying ${n.name} (${n.id})`),Et(n.id,n.name,o);let s=document.getElementById(P);if(s){q(s,{status:"linked",contactName:n.name});let a=he();a&&xo(a.oppId,n.id,n.name)}}catch(r){b.warn("[closing-agent] tryDecorate lookup failed:",r)}finally{ft=!1}}},Kn=/^TC\s*[-‐–—]\s*Closing\s+Agent$/i,ko=()=>{let e=document.querySelectorAll("*"),t=[],r=[],n=[];for(let d of e){let c=(d.textContent||"").trim();Kn.test(c)?t.push(d):c.length<80&&/Title\s+Officer/i.test(c)&&r.push(c);for(let h of["aria-label","placeholder","data-label","title","name","data-test"]){let m=d.getAttribute(h);m&&/Title\s+Officer/i.test(m)&&n.push({tag:d.tagName.toLowerCase(),attr:h,value:m})}}if(b.info(`[closing-agent] findInput: ${t.length} regex-matched, ${r.length} near-text matches, ${n.length} attribute matches`),r.length&&b.warn("[closing-agent] near-text samples:",[...new Set(r)].slice(0,8)),n.length&&b.warn("[closing-agent] attribute matches:",n.slice(0,8)),t.length===0)return null;t.sort((d,c)=>d.querySelectorAll("*").length-c.querySelectorAll("*").length);let a=t[0].parentElement,i=0;for(;a&&i<8;){let d=a.querySelector("input");if(d)return b.info(`[closing-agent] findInput: located input ${i} levels up from label`),d;a=a.parentElement,i+=1}return b.warn("[closing-agent] findInput: label found but no <input> within 8 ancestors"),null},Et=(o,e,t)=>{let r=ko();if(!r)return;let n=r.dataset.wsClosingAgentDecorated;if(n!==o){n&&r.parentElement?.querySelectorAll(`.${kt}`).forEach(a=>a.remove());{let s=r.parentElement;if(!s)return;r.dataset.wsClosingAgentDecorated=o,r.style.visibility="hidden",window.getComputedStyle(s).position==="static"&&(s.style.position="relative");let i=document.createElement("a");i.className=kt,i.href=`/v2/location/${t}/contacts/detail/${o}`,i.textContent=e,i.style.cssText=`
      position: absolute;
      inset: 0;
      display: flex;
      align-items: center;
      ...
      border-radius: ${k.sm};
      z-index: 2;
    `,i.addEventListener("mouseenter",()=>{i.style.textDecoration="underline"}),i.addEventListener("mouseleave",()=>{i.style.textDecoration="none"}),i.addEventListener("click",d=>{d.preventDefault(),d.stopPropagation(),window.location.href=i.href}),s.appendChild(i)}}},Pe=()=>{document.getElementById(fo)?.remove()},Yn=o=>{Pe();let e=document.createElement("div");e.id=fo,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,t.innerHTML=`
    ...
    <div id="ws-top-results"
      style="
        ...
        border: 1px solid ${l.steel};
        border-radius: ${k.sm};
        background: ${l.obsidian};
        margin-bottom: 16px;
      ">
      <div style="padding: 16px; color: ${l.ash}; font-size: 13px; text-align: center;">
  ...
  `,e.appendChild(t),document.body.appendChild(e);let r=t.querySelector("#ws-top-search"),n=t.querySelector("#ws-top-results"),s=t.querySelector("#ws-top-cancel");setTimeout(()=>r?.focus(),50),e.addEventListener("click",c=>{c.target===e&&Pe()}),s?.addEventListener("click",Pe);let a,i="",d=async c=>{if(n&&c!==i){if(i=c,!c.trim()){n.innerHTML=`
        <div style="padding: 16px; color: ${l.ash}; font-size: 13px; text-align: center;">
          Start typing to search.
        </div>`;return}n.innerHTML=`
      <div style="padding: 16px; color: ${l.ash}; font-size: 13px; text-align: center;">
        Searching\u2026
      </div>`;try{let h=await Ct({tenantId:o.locationId,query:c,contactType:"closing_agent",limit:20});if(!h.ok||!Array.isArray(h.results)||h.results.length===0){n.innerHTML=`
          <div style="padding: 16px; color: ${l.ash}; font-size: 13px; text-align: center;">
            No results.
          ...
          transition: background 0.1s ease;
        `,p.innerHTML=`
          <div style="font-weight: 500;">${Be(m.name)}</div>
          <div style="color: ${l.ash}; font-size: 11px; margin-top: 2px;">
            ${Be(m.email||"(no email)")} \xB7 ${Be(m.phone||"no phone")}
            ${m.companyName?` \xB7 ${Be(m.companyName)}`:""}
          </div>
        `,p.addEventListener("mouseenter",()=>{p.style.background=l.slate}),p.addEventListener("mouseleave",()=>{p.style.background="transparent"}),p.addEventListener("click",()=>{Pe(),o.onPick(m)}),n.appendChild(p)})}catch(h){b.error("Closing Agent picker search failed",h),n&&(n.innerHTML=`
          <div style="padding: 16px; color: ${l.crimson}; font-size: 13px; text-align: center;">
            Search failed. Try again.
          </div>`)}}};r?.addEventListener("input",()=>{let c=r.value;a!==void 0&&clearTimeout(a),a=window.setTimeout(()=>{d(c)},250)})},Be=o=>o.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"),q=(o,e)=>{let t=o.querySelector(".ws-top-label"),r=o.querySelector(".ws-top-dot"),n="Link Closing Agent",s=l.amber,a=l.bone,i="transparent",d=l.steel,c="pointer";switch(e.status){case"loading":n="Loading Closing Agent\u2026",s=l.coolGray,a=l.ash,c="wait";break;case"empty":n="Link Closing Agent",s=l.amber;break;case"linked":n=e.contactName?`Closing Agent: ${e.contactName}`:"Closing Agent: \u2713",s=l.emerald,i=l.emeraldGlow,d=l.emeraldBorder,a=l.emeraldBright;break;case"saving":n="Linking\u2026",s=l.coolGray,a=l.ash,c="wait";break}t&&(t.textContent=n),r&&(r.style.background=s,r.style.boxShadow=`0 0 6px ${s}`),o.style.background=i,o.style.borderColor=d,o.style.color=a,o.style.cursor=c},Xn=(o,e)=>{let t=document.createElement("button");t.id=P,t.type="button",t.dataset.oppId=o,t.style.cssText=I,t.title="Pick the Closing Agent contact for this deal. Their email + phone will flow into the title-co handoff email.",t.innerHTML=`
  ...
  `;let r={status:"loading"};q(t,r);let n=wo(o);return n&&(r={status:"linked",contactName:n.contactName},q(t,r),K={oppId:o,contactId:n.contactId,contactName:n.contactName,tenantId:e}),(async()=>{let a=await qn(o,e);a?(r={status:"linked",contactName:a.name},q(t,r),K={oppId:o,contactId:a.id,contactName:a.name,tenantId:e},Et(a.id,a.name,e)):n||(r={status:"empty"},q(t,r),K?.oppId===o&&(K=null),jn(),Wn(o))})(),t.addEventListener("mouseenter",()=>{r.status!=="loading"&&r.status!=="saving"&&(t.style.transform="translateY(-1px)")}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",()=>{r.status==="loading"||r.status==="saving"||Yn({oppId:o,locationId:e,onPick:async a=>{r={status:"saving"},q(t,r);try{let i=[vo({tenantId:e,oppId:o,fieldId:go,value:a.id})];a.companyName&&a.companyName.trim()&&i.push(vo({tenantId:e,oppId:o,fieldId:Gn,value:a.companyName.trim()}));let c=(await Promise.all(i))[0];if(!c.ok)throw new Error(c.error??"Save failed");r={status:"linked",contactName:a.name},q(t,r),K={oppId:o,contactId:a.id,contactName:a.name,tenantId:e},xo(o,a.id,a.name),Et(a.id,a.name,e),b.info(`Closing Agent linked: ${a.name} (${a.id})${a.companyName?` \u2014 Company: ${a.companyName}`:""}`)}catch(i){b.error("Closing Agent link failed",i),r={status:"empty"},q(t,r)}}})}),t},Jn=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading"],Zn=async()=>{for(let o of Jn)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},ce=!1,$e=async()=>{if(!ce){ce=!0;try{await Qn()}finally{ce=!1}}},Qn=async()=>{let o=he();if(!o||!o.locationId){document.getElementById(P)?.remove(),document.getElementById(`${P}-wrap`)?.remove(),Re();return}let e=document.getElementById(P);if(e&&e.dataset.oppId===o.oppId){let d=L();d!==null&&d!=="Transaction Coordination"&&(e.remove(),document.getElementById(`${P}-wrap`)?.remove(),Re());return}e&&(e.remove(),document.getElementById(`${P}-wrap`)?.remove());let t=L();if(t!==null&&t!=="Transaction Coordination")return;let r=await Zn();if(!r||!(wo(o.oppId)!==null||K!==null&&K.oppId===o.oppId)&&await U()!=="Transaction Coordination")return;let s=he();if(!s||s.oppId!==o.oppId||!s.locationId)return;let a=Xn(o.oppId,s.locationId),i=document.createElement("span");i.id=`${P}-wrap`,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,r.classList.contains("description")&&(r.style.display="flex",r.style.alignItems="center",r.style.gap="16px",r.style.flexWrap="wrap"),i.appendChild(a),r.appendChild(i),b.debug(`Mounted Closing Agent picker on opp ${o.oppId}`),ea(o.oppId),At(s.locationId)},be=null,Tt=null,St=null,Re=()=>{be?.disconnect(),be=null,Tt=null,St=null},ea=o=>{let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(be&&Tt===o&&St===e||(Re(),Tt=o,St=e,be=new MutationObserver(()=>{let t=he();if(!t||t.oppId!==o||!t.locationId){Re();return}document.getElementById(P)||$e(),At(t.locationId)}),be.observe(e,{childList:!0,subtree:!0})))},wt=null,xt,ta=()=>{wt||(wt=new MutationObserver(()=>{xt===void 0&&(xt=window.setTimeout(()=>{xt=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(P);o&&!e&&!ce&&$e()},300))}),wt.observe(document.body,{childList:!0,subtree:!0}))},yo,oa=()=>{yo===void 0&&(yo=window.setInterval(()=>{let o=he();if(!o||!o.locationId||!!!document.querySelector(".crm-opportunities-modal"))return;!!!document.getElementById(P)&&!ce&&(b.info("[closing-agent] safety-net: button missing on opp page \u2014 remounting"),$e()),At(o.locationId)},2e3))},Eo=()=>{b.info("[closing-agent] mountClosingAgentPickerButton invoked"),ta(),oa(),$e()};var We="reos-theme-stylesheet",Uo="reos-theme-fonts",ra=()=>{let o=document.getElementById(Uo);o&&o.remove()},na=`
/* ============================================
...
/* v0.13.97: field-description tooltips (the small (i) icon next to a
   custom field that explains what the field stores) render their text
   via spans with the Tailwind .text-gray-700 utility. Our global remap
   at section 6 paints .text-gray-700 -> bone, which made tooltip help-
   text shout in bright bone over whatever field happened to sit under
   the popover. Tim asked for ash inside tooltips so help-text reads as
   secondary metadata, not primary content. */
html body .hr-tooltip .text-gray-700,
html body .hr-tooltip .text-gray-800,
html body .hr-tooltip .text-gray-900,
html body .hr-tooltip span,
html body .hr-tooltip p,
html body .hr-popover-shared .text-gray-700,
html body .hr-popover-shared .text-gray-800,
html body .hr-popover-shared .text-gray-900,
html body .hr-popover-shared span,
html body .hr-popover-shared p,
html body [class*="hr-tooltip"] .text-gray-700,
html body [class*="hr-popover"] .text-gray-700 {
  color: var(--reos-ash) !important;
  -webkit-text-fill-color: var(--reos-ash) !important;
}
...
`,aa=[{selector:".hr-input__input-el, .hr-input__textarea-el, [class*='hr-input'] input, [class*='hr-input'] textarea",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".hr-data-table-wrapper, .hr-data-table-wrapper-responsive, .hr-data-table-wrapper-header, .hr-data-table-wrapper-footer, .hr-data-table, .hr-data-table__body, .hr-table-freezer-wrapper, [class*='hr-data-table-wrapper'], [class*='hr-table-freezer']",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".n-data-table-tr, tr.n-data-table-tr",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".back-to-sites, .n-tabs-nav, .n-tabs-nav-scroll-content, .n-tabs-nav-scroll-wrapper, .n-tabs-wrapper, .n-tabs-tab-wrapper, .n-tabs-tab-pad",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:"#setup-guide-button, .setup-guide-button-container",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:".default-avatar, .n-avatar.default-avatar",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:"#steps-sidebar, .steps-sidebar",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".search-box",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:".my-table.tabulator, .tabulator, .tabulator-tableholder, .tabulator-table, .tabulator-header, .tabulator-headers, .tabulator-footer, .tabulator-row, .table-container, .cardWrapper, .quick-filter-bar, #skeleton-loader, .skeleton-loader",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".tabulator-col, .tabulator-frozen, .tabulator-frozen-left, .tabulator-frozen-right",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:".mb-2\\.5.bg-white, div.bg-white, .bg-white.px-3, .mx-auto > .bg-white",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-data-table__header, .hr-data-table__cell-header, thead.hr-data-table__header",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:".hr-wrapper-container, .platform-ui__highrise, .full-screen.platform-ui__highrise, [class*='platform-ui__highrise'], .hl_topbar-tabs, .hl_wrapper",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-collapse-item__header, .hr-collapse-item__header-main, .hr-collapse-item__title",styles:{color:"#EDEEF0","background-color":"#12161D"}},{selector:".hr-collapse-item__content, .hr-collapse-item__content-inner, .hr-collapse-item__content-wrap",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-tabs, .hr-tabs-nav, .hr-tabs-nav-scroll-content, .hr-tabs-content, .hr-tabs-pane, .hr-tabs-rail, .hr-tabs-tab-wrapper",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-tabs-capsule",styles:{"background-color":"transparent",background:"transparent",border:"none"}},{selector:".hr-avatar:not(.default-avatar), .hr-avatar:not(.default-avatar) .hr-avatar__text, .hr-avatar:not(.default-avatar) .hr-avatar__text p, .hr-avatar:not(.default-avatar) p, .hr-avatar:not(.default-avatar) span, .hr-avatar:not(.default-avatar) div, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar), [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar) span, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar) p, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar) div",styles:{color:"#0A0D12","-webkit-text-fill-color":"#0A0D12","font-weight":"600"}},{selector:".default-avatar, .default-avatar *",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0",stroke:"#EDEEF0","font-weight":"600"}},{selector:".hr-button--tertiary",styles:{"background-color":"#1A1F28",color:"#0FB57E"}},{selector:".hr-text:not([role='button']):not(.cursor-pointer)",styles:{color:"#EDEEF0"}},{selector:"[class*='field-value'], [class*='field-display'], [class*='empty-value'], [class*='field-empty'], [class*='value-display'], [class*='display-value']",styles:{color:"#9098A3","-webkit-text-fill-color":"#9098A3"}},{selector:".hr-select__value, .hr-select__display, .hr-select-trigger, .hr-select-trigger__value, .hr-base-select__value, .hr-base-select__display, [class*='select__value'], [class*='select__display'], [class*='select-trigger']",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".hr-select, .hr-select__input, .hr-select__inner, .hr-select-trigger, .hr-base-select, .hr-base-select__inner, .el-select, .el-select__wrapper, .el-input__wrapper, .el-input__inner, [role='combobox'], [aria-haspopup='listbox']",styles:{"background-color":"#1A1F28","border-color":"#252C36"}},{selector:"input[type='date'], input[type='datetime-local'], input[type='time'], input[type='month'], input[type='week'], .hr-date-picker, .hr-date-input, .hr-datepicker, .hr-time-picker, [class*='date-picker'], [class*='datepicker'], [class*='date-input'], [class*='time-picker'], [class*='time-input']",styles:{"background-color":"#1A1F28","border-color":"#252C36",color:"#EDEEF0"}},{selector:"[class*='F7F9FD'], [class*='F7F9FB'], [class*='F9FAFB'], [class*='eff4ff'], [class*='EFF4FF'], [class*='ECEEF2'], [class*='eceef2']",styles:{"background-color":"#12161D"}},{selector:"[style*='background-color: white'], [style*='background-color:white'], [style*='background-color: #fff'], [style*='background-color:#fff'], [style*='background-color: #FFF'], [style*='background-color:#FFF'], [style*='background-color: #ffffff'], [style*='background-color:#ffffff'], [style*='background-color: #FFFFFF'], [style*='background-color:#FFFFFF'], [style*='background-color: rgb(255, 255, 255)'], [style*='background-color:rgb(255, 255, 255)'], [style*='background-color: rgb(255,255,255)'], [style*='background-color:rgb(255,255,255)']",styles:{"background-color":"#12161D"}},{selector:".chart-container.hl-card, [class*='chart-container'][class*='hl-card'], .dashboard-widget-type-custom .hl-card, .dashboard-widget-module-custom .hl-card",styles:{"background-color":"#12161D"}},{selector:".bg-gray-50, .bg-white, .bg-gray-100",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".n-base-select-menu, .n-popselect-menu, .n-popover, .n-dropdown, .n-dropdown-menu, .hr-base-select-menu, .hr-popselect-menu, .hr-popover, .hr-dropdown, .hr-dropdown-menu",styles:{"background-color":"#12161D",color:"#EDEEF0","border-color":"#252C36","--n-option-color":"transparent","--n-option-color-hover":"#1A1F28","--n-option-color-active":"rgba(15, 181, 126, 0.12)","--n-option-color-active-hover":"rgba(15, 181, 126, 0.12)","--n-option-text-color":"#EDEEF0","--n-option-text-color-hover":"#EDEEF0","--n-option-text-color-active":"#0FB57E","--n-option-text-color-disabled":"#9098A3","--n-color":"transparent","--n-color-hover":"#1A1F28","--n-color-active":"rgba(15, 181, 126, 0.12)","--n-text-color":"#EDEEF0","--n-text-color-hover":"#EDEEF0","--n-text-color-active":"#0FB57E","--n-text-color-disabled":"#9098A3"}},{selector:".n-dropdown-option--disabled, .n-dropdown-option--disabled .n-dropdown-option-body, .n-dropdown-option--disabled .n-dropdown-option-body__label, .n-dropdown-option--disabled .n-dropdown-option-body__prefix, .n-dropdown-option--disabled .n-dropdown-option-body__suffix, .n-dropdown-option[aria-disabled='true'], .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__label, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__prefix, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__suffix, .n-base-select-option--disabled, .n-base-select-option--disabled *, .n-base-select-option[aria-disabled='true'], .n-base-select-option[aria-disabled='true'] *, .hr-dropdown-option--disabled, .hr-dropdown-option--disabled *, .hr-dropdown-option[aria-disabled='true'], .hr-dropdown-option[aria-disabled='true'] *, .hr-base-select-option--disabled, .hr-base-select-option--disabled *, .hr-base-select-option[aria-disabled='true'], .hr-base-select-option[aria-disabled='true'] *",styles:{"background-color":"transparent",color:"#9098A3","-webkit-text-fill-color":"#9098A3"}},{selector:".hl-card-header [role='heading'], .hl-card-header p[role='heading'], [class*='hl-card-header'] [role='heading']",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".n-button:not(.n-button--primary-type):not(.n-button--error-type):not(.n-button--warning-type):not(.n-button--success-type):not(.n-button--info-type)",styles:{"background-color":"#1A1F28"}}],sa=o=>{if(!o||o[0]!=="#")return o;let e=o.slice(1);if(e.length===3&&(e=e[0]+e[0]+e[1]+e[1]+e[2]+e[2]),e.length!==6&&e.length!==8)return o;let t=parseInt(e.slice(0,2),16),r=parseInt(e.slice(2,4),16),n=parseInt(e.slice(4,6),16);if(isNaN(t)||isNaN(r)||isNaN(n))return o;if(e.length===8){let s=parseInt(e.slice(6,8),16)/255;return`rgba(${t}, ${r}, ${n}, ${s})`}return`rgb(${t}, ${r}, ${n})`},ia=new Set(["color","background-color","background","border-color","border-top-color","border-right-color","border-bottom-color","border-left-color","fill","stroke","-webkit-text-fill-color","caret-color","outline-color"]),la=["#inventory-table","#inventory","[id^='collections-']","[id^='products-']","[id^='reviews-']",".sites-container","[data-testid='wordpress-app-dashboard']","[data-testid$='-app-dashboard']","[data-testid$='-dashboard']:not([data-testid*='widget']):not([data-testid*='card'])","#wordpress-management-app","[id$='-management-app']"],Lt=new WeakSet,To=()=>{try{if(!document.body)return;document.body.querySelectorAll("*").forEach(e=>{try{if(Lt.has(e)&&e.style.getPropertyValue("background-color")==="rgb(18, 22, 29)"||e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.closest(".hl_header, header.hl_header, header.app-header"))return;let t=e.tagName;if(t==="INPUT"||t==="TEXTAREA"||t==="SELECT"||t==="SCRIPT"||t==="STYLE"||t==="SVG"||t==="PATH"||t==="IMG")return;let n=window.getComputedStyle(e).backgroundColor;if(!n||n==="rgba(0, 0, 0, 0)"||n==="transparent")return;let s=n.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?/);if(!s)return;let a=+s[1],i=+s[2],d=+s[3];if((s[4]!==void 0?+s[4]:1)<.5||a<230||i<230||d<230)return;if(e.style.getPropertyValue("background-color")==="rgb(18, 22, 29)"){Lt.add(e);return}e.style.setProperty("background-color","rgb(18, 22, 29)","important"),e.style.setProperty("color","rgb(237, 238, 240)","important"),Lt.add(e)}catch{}})}catch{}},So=new WeakSet,pe=()=>{document.querySelectorAll(la.join(", ")).forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let t=e.querySelectorAll("*");[e,...Array.from(t)].forEach(r=>{if(So.has(r)&&r.style.getPropertyValue("background-color")==="rgb(18, 22, 29)"||r.id?.startsWith("ws-crm-")||r.tagName==="INPUT"||r.tagName==="TEXTAREA"||r.tagName==="SELECT"||r.tagName==="SCRIPT"||r.tagName==="STYLE"||r.tagName==="SVG"||r.tagName==="PATH")return;let s=window.getComputedStyle(r).backgroundColor;if(!s||s==="rgba(0, 0, 0, 0)"||s==="transparent")return;let a=s.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!a)return;let i=+a[1],d=+a[2],c=+a[3];i<230||d<230||c<230||(r.style.setProperty("background-color","#12161D","important"),r.style.setProperty("color","#EDEEF0","important"),So.add(r))})})},Co=()=>{document.querySelectorAll(".hr-header-lite-left, .hr-header-lite-right, .hr-header-lite-content, .hr-header-lite-icon-item, [class*='hr-header-lite']:not([class*='container']):not([class*='wrapper']), .hr-breadcrumb, .hr-breadcrumb-item, .hr-breadcrumb-item__link, .hr-breadcrumb-item__separator").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let t=e.getAttribute("style")||"";(t.includes("background-color: rgb(26, 31, 40)")||t.includes("background-color: #1A1F28")||t.includes("background-color: #1a1f28"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("border-color"),e.style.removeProperty("border-style"),e.style.removeProperty("border-width"),e.style.removeProperty("border-radius"))})},Ge=()=>{document.querySelectorAll(".hr-header-lite-container-v2, [id^='hr-header-lite-v-']").forEach(e=>{let t=e.parentElement,r=0;for(;t&&r<6&&t!==document.body;){if(r+=1,t.id?.startsWith("ws-crm-")||t.classList.contains("hl_header")||t.classList.contains("app-header")||t.tagName==="MAIN"||t.tagName==="BODY")return;let a=window.getComputedStyle(t).backgroundColor.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?/);if(a){let i=+a[1],d=+a[2],c=+a[3];(a[4]!==void 0?+a[4]:1)>=.5&&i>=180&&d>=180&&c>=180&&i+d+c>=600&&t.style.getPropertyValue("background-color")!=="rgb(18, 22, 29)"&&(t.style.setProperty("background-color","rgb(18, 22, 29)","important"),t.style.setProperty("color","rgb(237, 238, 240)","important"))}t=t.parentElement}})},Io=()=>{let o="rgb(144, 152, 163)";document.querySelectorAll(".primary-property-name, .primary-property-name *, .folder-name, .folder-name *, .hr-form-item-label__asterisk, span.hr-form-item-label__asterisk").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']"))return;(t.getAttribute("style")||"").includes(o)&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"))})},me=()=>{for(let o of aa){let e;try{e=document.querySelectorAll(o.selector)}catch{continue}e.forEach(t=>{for(let[r,n]of Object.entries(o.styles)){let s=t.style.getPropertyPriority(r),a=t.style.getPropertyValue(r),i=ia.has(r)?sa(n):n;s==="important"&&a===i||t.style.setProperty(r,n,"important")}})}},je="#1A1F28",da=26,ca=31,ba=40,Ao=new Set,Lo=new Set,Ue=()=>{document.body.querySelectorAll("*").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.children.length>0||(e.textContent||"").trim()!=="--")return;let r=e.getBoundingClientRect();if(r.width===0||r.height===0||r.width>800)return;e.style.setProperty("display","none","important");let n=e.className?.toString()||"<no class>";!Lo.has(n)&&n!=="<no class>"&&(Lo.add(n),b.debug(`Hid empty "--" leaf: "${n}"`))})},ha=["M3 6","M5 6","M2 6","M6 6","M9 3h6","M16 6v","M19 6v","M5 6V","M16.5 4","48.816","2.991","14.74 9","9.26 9","2.244","M19 4h-","M19 7l-"],pa=["trash","Trash","TRASH","delete","Delete","DELETE"],ma=()=>{document.querySelectorAll("button.hr-button, button.n-button, button.icon-only, .hr-button.icon-only, .n-button.icon-only").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.getAttribute("data-reos-trash")==="true")return;let t=e.querySelector("svg");if(!t)return;let r=!1,n=(t.getAttribute("class")||"")+" "+(t.getAttribute("data-icon")||"");if(pa.some(s=>n.includes(s))&&(r=!0),r||t.querySelectorAll("path").forEach(a=>{if(r)return;let i=a.getAttribute("d")||"";ha.some(d=>i.includes(d))&&(r=!0)}),!r){let s=(e.id||"").toLowerCase(),a=(e.getAttribute("aria-label")||"").toLowerCase(),i=(e.getAttribute("title")||"").toLowerCase();(s.includes("delete")||s.includes("remove")||s.includes("trash")||a.includes("delete")||a.includes("remove")||a.includes("trash")||i.includes("delete")||i.includes("remove")||i.includes("trash"))&&(r=!0)}r&&e.setAttribute("data-reos-trash","true")})},ua=new Set(["Delete Reminder","Delete","Remove","Trash","Edit","Copy","Duplicate","Add another reminder","Add","Close","Dismiss"]),_a=()=>{document.querySelectorAll(".hr-tooltip, .hr-popover-shared, [role='tooltip']").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let t=(e.textContent||"").trim();t.length===0||t.length>40||ua.has(t)&&e.style.display!=="none"&&(e.style.setProperty("display","none","important"),e.style.setProperty("visibility","hidden","important"),e.style.setProperty("opacity","0","important"),e.style.setProperty("pointer-events","none","important"))})},Fo=new Set,Mo=new WeakSet,va=()=>{document.body.querySelectorAll("*").forEach(e=>{if(Mo.has(e)&&e.style.getPropertyValue("color")==="rgb(237, 238, 240)"||e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let t=(e.className?.toString()||"").toLowerCase();if(t.includes("ai-button")||t.includes("ask-ai")||t.includes("bg-clip-text")||t.includes("text-transparent")||e.closest("[class*='ai-button'], [class*='ask-ai'], [class*='bg-clip-text'], [class*='text-transparent']"))return;let r=e.tagName.toLowerCase();if(r==="svg"||r==="path"||r==="circle"||r==="rect"||r==="polygon"||r==="polyline"||r==="line"||r==="g")return;let n=!1;for(let m of Array.from(e.childNodes))if(m.nodeType===Node.TEXT_NODE&&(m.textContent||"").trim()){n=!0;break}if(!n)return;let s=Array.from(e.children);if(s.length>0&&s.every(m=>{let p=m.tagName.toLowerCase();return p==="svg"||p==="img"})&&!n)return;let d=window.getComputedStyle(e).color;if(!(d==="rgb(255, 255, 255)"||d==="rgba(255, 255, 255, 1)"||d==="#ffffff"||d==="white"))return;e.style.setProperty("color","#EDEEF0","important"),e.style.setProperty("-webkit-text-fill-color","#EDEEF0","important"),Mo.add(e);let h=e.className?.toString()||"<no class>";!Fo.has(h)&&h!=="<no class>"&&(Fo.add(h),b.debug(`Remapped pure-white text -> bone: "${h}"`))})},Do=new Set,ya=()=>{let o=document.querySelectorAll(".hl_header, header.hl_header, header.app-header, .hl_header--controls, .top-bar, .topbar"),e="rgb(18, 22, 29)";document.querySelectorAll(".hr-content-switcher svg, .hr-content-switcher svg *, .hr-content-switcher i, .hr-radio-button svg, .hr-radio-button svg *, .hr-radio-button i, [class*='hr-content-switcher'] svg, [class*='hr-content-switcher'] svg *, [class*='hr-content-switcher'] i").forEach(r=>{if(r.closest(".hl_header"))return;let n=r.getAttribute("style")||"";(n.includes(e)||n.includes("#12161D")||n.includes("#12161d"))&&(r.style.removeProperty("fill"),r.style.removeProperty("stroke"),r.style.removeProperty("color"))}),o.forEach(r=>{r.querySelectorAll("button, a, [role='button'], [class*='rounded-full'], [class*='rounded-md'], [class*='rounded-xl'], [class*='pill'], span[class*='rounded'], div[class*='rounded']").forEach(s=>{if(s.closest("[id^='ws-crm-']"))return;let a=(s.className?.toString()||"").toLowerCase();if(a.includes("avatar"))return;let i=window.getComputedStyle(s),d=i.backgroundColor,c=i.backgroundImage||"",h=!1;if(c!=="none"&&c.includes("gradient"))h=!0;else if(d&&d!=="rgba(0, 0, 0, 0)"&&d!=="transparent"){let T=d.match(/rgba?\(\d+,\s*\d+,\s*\d+,\s*([\d.]+)\)/);if(T&&parseFloat(T[1])<.6)return;let w=d.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(w){let F=parseInt(w[1],10),$=parseInt(w[2],10),Ye=parseInt(w[3],10);(F>=60||$>=60||Ye>=60)&&(F>230&&$>230&&Ye>230||(F>=80||$>=80||Ye>=80)&&(h=!0))}}if(!h)return;let p=(s.textContent||"").trim(),u=a,g=Array.from(s.querySelectorAll("*")).map(T=>(T.className?.toString()||"").toLowerCase()).join(" "),x=p.toLowerCase()==="ask ai"||u.includes("ai-button")||u.includes("ask-ai")||u.includes("askai"),v=u.includes("bg-clip-text")||u.includes("text-transparent")||g.includes("bg-clip-text")||g.includes("text-transparent"),_=Array.from(s.querySelectorAll("*")).some(T=>{let w=window.getComputedStyle(T),F=w.getPropertyValue("-webkit-background-clip")||"",$=w.getPropertyValue("background-clip")||"";return F.includes("text")||$.includes("text")});if(x||v||_){let T=["color","-webkit-text-fill-color","fill","stroke","background","background-color","background-image","background-clip","-webkit-background-clip","text-shadow"],w=F=>{T.forEach($=>F.style.removeProperty($))};w(s),s.querySelectorAll("*").forEach(w);return}let y="#12161D";s.style.setProperty("color",y,"important"),s.querySelectorAll("*").forEach(T=>{let w=T.tagName.toLowerCase();w==="svg"||w==="path"||w==="circle"||w==="rect"||w==="polygon"||w==="polyline"||w==="line"||w==="g"?(T.style.setProperty("fill",y,"important"),T.style.setProperty("stroke",y,"important"),T.style.setProperty("color",y,"important")):(T.style.setProperty("color",y,"important"),T.style.setProperty("-webkit-text-fill-color",y,"important"))});let E=s.className?.toString()||"<no class>";!Do.has(E)&&E!=="<no class>"&&(Do.add(E),b.debug(`Darkened SVG icons on colored button: "${E}"`))})})},Oo=new Set,ga=()=>{document.querySelectorAll("[class*='avatar']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;if((e.className?.toString()||"").toLowerCase().includes("default-avatar")){e.querySelectorAll(".n-avatar__text, [class*='avatar__text']").forEach(d=>{(d.getAttribute("style")||"").includes("translateY(11px)")&&d.style.removeProperty("transform")});return}let r=e.getBoundingClientRect();if(r.width===0||r.height===0||r.width<20||r.width>64||r.height<20||r.height>64)return;let n=r.top+r.height/2;(e.parentElement||e).querySelectorAll("svg, img").forEach(d=>{let c=d;if(c.closest("[id^='ws-crm-']"))return;let h=d.getBoundingClientRect();if(h.width===0||h.width>28||h.height>28)return;let m=h.left<r.right&&h.right>r.left,p=h.top<r.bottom&&h.bottom>r.top;if(!(m&&p)||h.top+h.height/2<n)return;let g=h.width*h.height/(r.width*r.height);if(g>.55)return;let x=c,v=c.parentElement,_=0;for(;v&&v!==e&&_<3;){let E=v.getBoundingClientRect();if(E.width>0&&E.width<=30&&E.height<=30)x=v;else break;v=v.parentElement,_++}x.style.setProperty("transform","translateY(11px)","important");let y=(x.className?.toString()||"")+" "+x.tagName;Oo.has(y)||(Oo.add(y),b.debug(`Dropped activity badge 11px (${h.width.toFixed(0)}x${h.height.toFixed(0)} icon, ratio ${g.toFixed(2)}): "${y}"`))})})},fa="brightness(0) saturate(100%) invert(54%) sepia(78%) saturate(442%) hue-rotate(122deg) brightness(96%) contrast(89%)",wa=()=>{document.querySelectorAll("aside.default-bg-color a, [class*='sidebar'] a").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let t=(e.className?.toString()||"").toLowerCase();t.includes("active")||t.includes("router-link-active")?(e.style.setProperty("color","#0FB57E","important"),e.style.setProperty("-webkit-text-fill-color","#0FB57E","important"),e.style.setProperty("opacity","1","important"),e.querySelectorAll("span, p, div").forEach(n=>{n.style.setProperty("color","#0FB57E","important"),n.style.setProperty("-webkit-text-fill-color","#0FB57E","important")}),e.querySelectorAll("img").forEach(n=>{n.style.setProperty("filter",fa,"important")}),e.querySelectorAll("svg").forEach(n=>{n.style.setProperty("color","#0FB57E","important"),n.style.setProperty("stroke","#0FB57E","important")})):((e.style.color==="rgb(18, 22, 29)"||e.style.color==="#12161D"||e.style.color==="#0FB57E"||e.style.color==="rgb(15, 181, 126)")&&(e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color")),e.querySelectorAll("span, p, div").forEach(n=>{let s=n.style.color;(s==="rgb(18, 22, 29)"||s==="#12161D"||s==="#0FB57E"||s==="rgb(15, 181, 126)")&&(n.style.removeProperty("color"),n.style.removeProperty("-webkit-text-fill-color"))}),e.querySelectorAll("img").forEach(n=>{n.style.filter&&n.style.filter.includes("hue-rotate(122")&&n.style.removeProperty("filter")}))})},No=new Set,xa=()=>{document.querySelectorAll(".n-dialog, [class*='n-dialog'], [class*='dialog'], [role='dialog']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;e.querySelectorAll("div, span, i").forEach(r=>{if(r.closest("[id^='ws-crm-']"))return;let n=r.getBoundingClientRect();if(n.width===0||n.height===0||n.width>56||n.height>56||!r.querySelector("svg"))return;let a=window.getComputedStyle(r).backgroundColor.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!a)return;let i=parseInt(a[1],10),d=parseInt(a[2],10),c=parseInt(a[3],10);if(!(i>200&&d>200&&c>200))return;r.style.setProperty("background-color","transparent","important"),r.style.setProperty("background","transparent","important"),r.style.setProperty("box-shadow","none","important"),r.style.setProperty("border","none","important");let h=r.className?.toString()||"<no class>";!No.has(h)&&h!=="<no class>"&&(No.add(h),b.debug(`Stripped dialog icon circle (${n.width.toFixed(0)}x${n.height.toFixed(0)}, rgb(${i},${d},${c})): "${h}"`))})})},Ho=new Set,ka=()=>{document.querySelectorAll("[class*='ai-button'], [class*='ask-ai'], [class*='askai']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let t=e.parentElement,r=0;for(;t&&r<3&&!t.closest("[id^='ws-crm-']");){let n=(t.className?.toString()||"").toLowerCase();if(!(n.includes("ai-button")||n.includes("ask-ai")||n.includes("askai"))){let a=t.getBoundingClientRect();if(a.width>0&&a.height>0&&a.width<=400&&a.height<=80){t.style.setProperty("background-color","#12161D","important"),t.style.setProperty("background-image","none","important");let i=t.className?.toString()||"<no class>";!Ho.has(i)&&i!=="<no class>"&&(Ho.add(i),b.debug(`Painted Ask AI wrapper graphite: "${i}"`))}}t=t.parentElement,r++}})},Po=new Set,Ea=()=>{let o=Array.from(document.querySelectorAll("#contact-conversation-panel, [class*='conversation-panel'], [class*='message-list'], [class*='activity-log'], .hl_header, header.hl_header, [class*='topbar'], [class*='top-bar'], header.app-header, .modal, .modal-content, [class*='modal'], [class*='dialog'], [role='dialog'], [class*='popover'], [class*='popup'], [class*='dropdown-menu'], [class*='select-menu'], [class*='popselect'], [class*='base-select'], .n-base-select-menu, .n-popselect, .n-popover")),e=document.querySelectorAll("div, section, aside"),t=[];e.forEach(n=>{if(n.id?.startsWith("ws-crm-")||n.closest("[id^='ws-crm-']"))return;let s=window.getComputedStyle(n);if(s.position!=="fixed"&&s.position!=="absolute")return;let a=parseInt(s.zIndex,10);if(!isFinite(a)||a<50)return;let i=n.getBoundingClientRect();i.width<240||i.height<120||i.width>1500||i.height>900||t.push(n)}),[...o,...t].forEach(n=>{n.querySelectorAll("*").forEach(a=>{if(a.id?.startsWith("ws-crm-")||a.closest("[id^='ws-crm-']"))return;let i=(a.className?.toString()||"").toLowerCase();if(i.includes("avatar")||i.includes("notification-dot")||i.includes("indicator"))return;if(i.includes("sidebar-option-button")||a.closest(".sidebar-option-button")||a.closest("nav[class*='w-13']")){let v=a.getAttribute("style")||"";(v.includes("background-color")||v.includes("color"))&&(a.style.removeProperty("background-color"),a.style.removeProperty("color"));return}if(i.includes("tabulator")||i.includes("tabulator-page")||a.closest("[class*='tabulator']")){(a.getAttribute("style")||"").includes("background-color")&&a.style.removeProperty("background-color");return}if(i.includes("hr-card")||a.closest(".hr-card")||a.id&&a.id.startsWith("opportunity-")){let v=a.getAttribute("style")||"";(v.includes("background-color")||v.includes("color"))&&(a.style.removeProperty("background-color"),a.style.removeProperty("color"));return}if(i.includes("ai-button")||i.includes("ask-ai")||i.includes("bg-clip-text")||i.includes("text-transparent")||a.closest("[class*='ai-button'], [class*='ask-ai'], [class*='bg-clip-text'], [class*='text-transparent']"))return;let d=window.getComputedStyle(a).backgroundColor;if(!d||d==="rgba(0, 0, 0, 0)"||d==="transparent")return;let c=d.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!c)return;let h=parseInt(c[1],10),m=parseInt(c[2],10),p=parseInt(c[3],10);if(!(h>200&&m>200&&p>200))return;let g=a.getBoundingClientRect();if(g.width<24||g.height<16||g.width>1400)return;a.style.setProperty("background-color","#12161D","important"),a.style.setProperty("color","#EDEEF0","important");let x=a.className?.toString()||"<no class>";!Po.has(x)&&x!=="<no class>"&&(Po.add(x),b.debug(`Coerced light bg (${g.width.toFixed(0)}x${g.height.toFixed(0)}, rgb(${h},${m},${p})): "${x}"`))})})},Bo=new Set,Ta=()=>{let o="input, select, textarea, [class*='hr-input'], [class*='hr-select'], [class*='hr-base-select'], [class*='hr-date-picker'], [class*='hr-datepicker']";document.body.querySelectorAll("*").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']")||t.children.length>0)return;let r=(t.textContent||"").trim();if(r.length===0||r.length>80||r==="--"||t.closest(o)||t.closest("[class*='avatar'], [class*='hr-avatar']")||t.closest("[class*='collapse-item__header'], [class*='hr-collapse-item__header'], [class*='collapse-header'], [class*='folder-header']")||t.closest("[class*='hr-tabs-tab'], [class*='tab-label'], [class*='tabs-nav'], [role='tab']"))return;if(t.matches("[role='heading']")||t.closest("[role='heading']")||t.matches("h1, h2, h3")||t.closest(".hl-card-header, [class*='hl-card-header'], [class*='card-header']")){t.matches("h1, h2, h3")&&(t.getAttribute("style")||"").includes("color")&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"));return}if(t.closest(".hr-alert, .hr-alert-body, [class*='hr-alert']")){let y=t.getAttribute("style")||"";(y.includes("color")||y.includes("background"))&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"),t.style.removeProperty("background-color"));return}if(t.closest("#SettingTexasSmsBlock")||t.matches("p.ui-text-sm-medium")&&t.parentElement?.classList?.contains("flex")&&t.parentElement?.classList?.contains("items-center")&&t.parentElement?.children?.length===1){(t.getAttribute("style")||"").includes("color")&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"));return}if(t.closest(".hr-button, .n-button")||t.matches(".hr-button__content, .n-button__content")||t.closest(".hr-button__content, .n-button__content")){(t.getAttribute("style")||"").includes("color")&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"));return}if(t.closest(".bg-primary-50, [class*='bg-primary-50']")){(t.getAttribute("style")||"").includes("color")&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"));return}let n=t,s=null;for(let y=0;y<4&&n;y++){let E=n.nextElementSibling;if(E&&(E.matches(o)||E.querySelector(o)!==null)){s=E;break}n=n.parentElement}if(!s)return;let a=t.getBoundingClientRect();if(a.width===0||a.height===0||a.width>400||a.height>60)return;t.style.setProperty("color","#9098A3","important"),t.style.setProperty("-webkit-text-fill-color","#9098A3","important");let i=s,d=i.getBoundingClientRect(),c=i.className?.toString()||"",h=c.includes("schedule-ui-form-weekly-schedule-checkbox")||i.closest(".schedule-ui-form-weekly-schedule-checkbox")!==null,m=c.includes("hl-form-item")||c.includes("n-form-item--top-labelled")||i.closest(".hl-form-item")!==null,p=".hr-input, .hr-input-number, .hr-base-select, .hr-select, .hr-date-picker, .hr-datepicker",u=(c.includes("hr-form-item-blank")||c.includes("hr-form-item")||i.matches(p))&&(i.matches(p)||i.querySelector(p)!==null);u&&(i.getAttribute("style")||"").includes("background-color")&&(i.style.removeProperty("background-color"),i.style.removeProperty("border-color"),i.style.removeProperty("border-style"),i.style.removeProperty("border-width"),i.style.removeProperty("border-radius"));let g=c.includes("hr-radio")||i.closest(".hr-radio, [class*='hr-radio']")!==null;g&&(i.getAttribute("style")||"").includes("background-color")&&(i.style.removeProperty("background-color"),i.style.removeProperty("border-color"),i.style.removeProperty("border-style"),i.style.removeProperty("border-width"),i.style.removeProperty("border-radius"));let x=c.includes("hr-breadcrumb")||i.closest(".hr-breadcrumb, [class*='hr-breadcrumb']")!==null;x&&(i.getAttribute("style")||"").includes("background-color")&&(i.style.removeProperty("background-color"),i.style.removeProperty("border-color"),i.style.removeProperty("border-style"),i.style.removeProperty("border-width"),i.style.removeProperty("border-radius"));let v=c.includes("hr-header-lite")||i.closest("[class*='hr-header-lite']")!==null;v&&(i.getAttribute("style")||"").includes("background-color")&&(i.style.removeProperty("background-color"),i.style.removeProperty("border-color"),i.style.removeProperty("border-style"),i.style.removeProperty("border-width"),i.style.removeProperty("border-radius")),!h&&!m&&!u&&!g&&!x&&!v&&d.width>0&&d.width<=800&&d.height>0&&d.height<=100&&(i.style.setProperty("background-color",je,"important"),i.style.setProperty("border-color","#252C36","important"),i.style.setProperty("border-style","solid","important"),i.style.setProperty("border-width","1px","important"),i.style.setProperty("border-radius","4px","important"));let _=t.className?.toString()||"<no class>";!Bo.has(_)&&_!=="<no class>"&&(Bo.add(_),b.debug(`Colored field label "${r.slice(0,30)}": "${_}"`))})},Ro=new Set,Sa=()=>{document.querySelectorAll("[class*='avatar']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let t=(e.className?.toString()||"").toLowerCase();if(t.includes("dropdown")||t.includes("option-avatar"))return;let r=e.getBoundingClientRect();if(r.width===0||r.height===0||r.height>80||e.querySelector("svg, img")!==null)return;let s=(e.textContent||"").trim();if(s.length===0||s.length>4)return;let a=Math.min(r.width,r.height),i=Math.max(16,Math.min(24,Math.round(a*.6)));e.style.setProperty("font-size",`${i}px`,"important"),e.style.setProperty("line-height","1","important"),e.style.setProperty("display","flex","important"),e.style.setProperty("align-items","center","important"),e.style.setProperty("justify-content","center","important"),e.style.setProperty("text-align","center","important"),e.querySelectorAll("*").forEach(c=>{c.tagName==="svg"||c.tagName==="SVG"||c.tagName!=="IMG"&&(c.style.setProperty("font-size",`${i}px`,"important"),c.style.setProperty("line-height","1","important"),c.style.setProperty("text-align","center","important"))});let d=e.className?.toString()||"<no class>";!Ro.has(d)&&d!=="<no class>"&&(Ro.add(d),b.debug(`Resized avatar text to ${i}px (circle ${a.toFixed(0)}px): "${d}"`))})},$o=new Set,Ca=new Set(["Enter Dashboard Name"]),Ia=()=>{document.querySelectorAll("input:not([type='checkbox']):not([type='radio']):not([type='button']):not([type='submit']):not([type='reset']):not([type='hidden']):not([type='file']), textarea, select").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let t=e.getBoundingClientRect();if(t.width===0||t.height===0)return;let r=e.placeholder||"";if(Ca.has(r)){(e.getAttribute("style")||"").includes("background-color")&&(e.style.removeProperty("background-color"),e.style.removeProperty("border-color"));return}e.style.setProperty("background-color",je,"important"),e.style.setProperty("border-color","#252C36","important"),e.style.setProperty("color","#EDEEF0","important"),e.style.setProperty("-webkit-text-fill-color","#EDEEF0","important");let n=e.closest(".hr-input-wrapper, .hr-input, .hr-input__input");n&&(n.style.setProperty("background-color",je,"important"),n.style.setProperty("border-color","#252C36","important"));let s=e.className?.toString()||"<no class>";!$o.has(s)&&s!=="<no class>"&&($o.add(s),b.debug(`Forced input slate bg: "${s}"`))})},Ve=()=>{document.body.querySelectorAll("*").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.closest("[class*='collapse-item__header'], [class*='hr-collapse-item__header'], [class*='collapse-header'], [class*='folder-header']")||e.id==="chat-filter"||e.id==="phone-calls"||e.id==="archive-conversation"||e.id==="star-toggle"||e.id==="read-toggle"||e.id==="delete-conversation")return;if(e.closest(".hr-tooltip, .hr-popover-shared, .hr-popover, [class*='hr-tooltip'], [class*='hr-popover']")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}if(e.closest(".hr-base-select-menu, .hr-select-menu, .hr-select__menu-container, .hr-dropdown-menu, [class*='hr-base-select-menu']")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}let t=e.className?.toString()||"";if(t.includes("bg-inherit")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(e.closest(".pipeline-ribbon")&&t.split(" ").includes("count")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.split(" ").includes("tabulator-page")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}if(["tabulator-cell","tabulator-row","tabulator-col","tabulator-header","tabulator-tableholder","tabulator-table"].some(p=>t.includes(p))){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("schedule-ui-form-weekly-schedule-checkbox")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("border-color")||p.includes("border-style")||p.includes("border-width"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("border-color"),e.style.removeProperty("border-style"),e.style.removeProperty("border-width"));return}if(t.includes("n-button")&&t.includes("icon-only")&&!t.includes("quaternary")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("hr-button--tertiary-type")||t.includes("n-button--tertiary-type")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color"),e.style.removeProperty("border-color"),e.style.removeProperty("border-style"),e.style.removeProperty("border-width"),e.style.removeProperty("border-radius"));return}if(t.includes("hr-data-table-td")||t.includes("hr-data-table-tr")||t.includes("hr-data-table__body-cell")||t.includes("hr-data-table__cell")||t.includes("hr-data-table__header-cell")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("n-data-table-td--fixed-right")||t.includes("n-data-table-td--last-col")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(e.classList.contains("hr-tabs")||t.includes("hr-tabs-tab-wrapper")||t.includes("hr-tabs-nav")||t.includes("hr-tabs-nav-scroll-content")||t.includes("hr-tabs-content")||t.includes("hr-tabs-pane")||t.includes("hr-tabs-rail")||t.includes("hr-tabs-wrapper")||t.includes("hr-tabs-pane-wrapper")||t.includes("hr-tab-pane")){let p=e.getAttribute("style")||"";(p.includes("background-color: rgb(26, 31, 40)")||p.includes("background-color: #1A1F28")||p.includes("background-color: #1a1f28"))&&e.style.removeProperty("background-color");return}if(t.includes("hr-date-panel")||t.includes("n-date-panel")||e.closest(".hr-date-panel, .n-date-panel")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("hr-tag")&&t.includes("ui-default")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if((t.includes("bg-purple-50")||t.includes("bg-purple-100"))&&(t.includes("rounded-lg")||t.includes("rounded-md")||t.includes("rounded-full"))){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("n-alert")||e.closest(".n-alert")||t.includes("hl-warning")||t.includes("hl-error")||t.includes("hl-info")||t.includes("hl-success")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("hr-card")||e.closest(".hr-card")||e.id&&e.id.startsWith("opportunity-")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("bg-gray-50")||t.includes("bg-gray-100")||t.includes("bg-gray-200")||t.includes("bg-white")||t.includes("bg-blue-50")||t.includes("bg-blue-100")||t.includes("bg-sky-50")||t.includes("bg-indigo-50")||t.includes("bg-slate-50")||t.includes("bg-neutral-50")||t.includes("bg-zinc-50")||t.includes("bg-stone-50")||t.includes("F7F9FD")||t.includes("F9FAFB")||t.includes("ECEEF2")||t.includes("eceef2")||t.includes("EFF4FF")||t.includes("eff4ff")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("hr-collapse-item")||t.includes("hr-collapse hr-accordion")||t.includes("hr-accordion"))return;if(t.includes("sidebar-option-button")||e.closest(".sidebar-option-button")||e.closest("nav[class*='w-13']")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}if(e.closest(".inbox-panel, .conversation-list-container, [class*='conversation-list-container']")){let p=e.getAttribute("style")||"";(p.includes("background-color")||p.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}let n=window.getComputedStyle(e).backgroundColor;if(!n||n==="rgba(0, 0, 0, 0)"||n==="transparent")return;let s=n.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!s)return;let a=parseInt(s[1],10),i=parseInt(s[2],10),d=parseInt(s[3],10);if(!(a<da&&i<ca&&d<ba))return;let h=e.getBoundingClientRect();if(h.width===0||h.height===0||h.width>800||h.height>200)return;e.style.setProperty("background-color",je,"important");let m=e.className?.toString()||"<no class>";!Ao.has(m)&&m!=="<no class>"&&(Ao.add(m),b.debug(`Coerced dark bg to slate (${h.width.toFixed(0)}x${h.height.toFixed(0)}, rgb(${a},${i},${d})): "${m}"`))})},qe=()=>{let o="#4B8BF5",e=["#155EEF","#155eef","#004EEB","#004eeb","#175CD3","#175cd3"],t="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";document.querySelectorAll("img[src^='data:image/svg+xml;base64,']:not([data-reos-recolored-v3])").forEach(s=>{s.setAttribute("data-reos-recolored","1"),s.setAttribute("data-reos-recolored-v2","1"),s.setAttribute("data-reos-recolored-v3","1");let i=(s.getAttribute("src")||"").match(/^data:image\/svg\+xml;base64,(.+)$/);if(!i)return;let d;try{d=atob(i[1])}catch{return}let c=d,h=!1;if(e.forEach(u=>{c.includes(u)&&(c=c.split(u).join(o),h=!0)}),["#F8F9FC","#f8f9fc","#F9FAFB","#f9fafb","#F2F4F7","#f2f4f7"].forEach(u=>{c.includes(u)&&(c=c.split(u).join("transparent"),h=!0)}),s.closest(".toolbar_button, .editor-toolbar, [class*='toolbar']")!==null&&["#000000","#000","#040E13","#040e13","#111827","#111","#101828","#101"].forEach(g=>{c.includes(g)&&(c=c.split(g).join("#EDEEF0"),h=!0)}),!!h)try{let u=btoa(c);s.setAttribute("src",`data:image/svg+xml;base64,${u}`)}catch{}}),document.querySelectorAll(".hl-checkbox-group-item img:not([data-reos-masked]):not([data-reos-recolored]), .hl-checkbox-group-item-icon img:not([data-reos-masked]):not([data-reos-recolored]), .hl-checkbox-group-item-container img:not([data-reos-masked]):not([data-reos-recolored])").forEach(s=>{let a=s.getAttribute("src")||"";if(!a||a.startsWith("data:")||!/\.svg(?:[?#]|$)/i.test(a))return;s.setAttribute("data-reos-masked","1");let i=`url("${a}")`;s.style.setProperty("-webkit-mask-image",i,"important"),s.style.setProperty("-webkit-mask-size","contain","important"),s.style.setProperty("-webkit-mask-repeat","no-repeat","important"),s.style.setProperty("-webkit-mask-position","center","important"),s.style.setProperty("mask-image",i,"important"),s.style.setProperty("mask-size","contain","important"),s.style.setProperty("mask-repeat","no-repeat","important"),s.style.setProperty("mask-position","center","important"),s.style.setProperty("background-color","var(--reos-blue)","important"),s.setAttribute("src",t)})},ze=()=>{document.querySelectorAll("[class*='n-date-panel']").forEach(e=>{[...e.classList].forEach(t=>{if(t.startsWith("n-date-panel")){let r="hr-date-panel"+t.slice(12);e.classList.contains(r)||e.classList.add(r)}})})},Aa=()=>{document.querySelectorAll("button.hr-button.hr-button--3xs:not(.hr-button--primary-type), button.hr-button.hr-button--xs:not(.hr-button--primary-type), #manage-association-btn").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let t=e.getAttribute("style")||"";(t.includes("background-color")||t.includes("background:"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("background"))})},ue=()=>{let o=r=>{r.style.getPropertyValue("background-color")!=="rgb(26, 31, 40)"&&r.style.setProperty("background-color","rgb(26, 31, 40)","important")},e=document.querySelector(".crm-opportunities-modal");e&&e.querySelectorAll(".hr-base-selection").forEach(n=>{o(n),n.querySelectorAll("div, span").forEach(a=>{if(a.id?.startsWith("ws-crm-"))return;let i=window.getComputedStyle(a).backgroundColor;i==="rgba(0, 0, 0, 0)"||i==="transparent"||!i||o(a)})}),document.querySelectorAll(".hr-upload-dragger").forEach(r=>{o(r);let n=r.querySelector(".ui-upload-icon");n&&(n.style.setProperty("background-color","transparent","important"),n.style.setProperty("background","transparent","important"));let s="rgb(150, 162, 173)",a="rgb(0, 191, 124)",i=c=>{let h=r.querySelector(".ui-upload-icon svg");h&&(h.style.setProperty("color",c,"important"),h.style.setProperty("stroke",c,"important")),r.querySelectorAll(".ui-upload-icon svg path").forEach(m=>{m.style.setProperty("stroke",c,"important")})},d=!1;try{d=r.matches(":hover")}catch{}i(d?a:s),r.dataset.hoverBound!=="1"&&(r.dataset.hoverBound="1",r.addEventListener("mouseenter",()=>i(a)),r.addEventListener("mouseleave",()=>i(s)))})},Ft=new WeakSet,La=()=>{document.querySelectorAll('iframe[title="Email"]').forEach(e=>{if(!Ft.has(e))try{let t=e.contentDocument||e.contentWindow?.document;if(!t||!t.head)return;if(t.querySelector("style[data-ws-crm-email-fix]")){Ft.add(e);return}let r=t.createElement("style");r.setAttribute("data-ws-crm-email-fix","1"),r.textContent=`
        html, body {
          background-color: #FFFFFF !important;
          color: #101828 !important;
        }
        body { padding: 16px !important; }
      `,t.head.appendChild(r),e.style.background="#FFFFFF",e.style.borderRadius="6px",Ft.add(e)}catch{}})},Go=()=>{let o,t=()=>{o===void 0&&(o=window.setTimeout(()=>{o=void 0,ze(),me(),Ia(),Ve(),Ue(),Ta(),La(),Ea(),ka(),xa(),wa(),ya(),Sa(),ga(),va(),qe(),_a(),ma(),ue(),pe(),To(),Co(),Io(),Ge(),Aa(),ue()},800))};ze(),me(),Ve(),Ue(),qe(),ue(),pe(),To(),Co(),Io(),Ge(),window.setTimeout(()=>{ze(),me(),Ve(),Ue(),qe(),ue(),pe(),Ge()},500),window.setTimeout(()=>{ze(),me(),Ve(),Ue(),qe(),ue(),pe(),Ge()},1500),typeof requestAnimationFrame=="function"&&requestAnimationFrame(()=>{pe(),me()}),new MutationObserver(()=>{t()}).observe(document.body,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["class","style"]})},Mt,Fa=()=>{let o=()=>{let t=document.getElementById(We);!t||!document.head||document.head.lastElementChild===t||document.head.appendChild(t)};new MutationObserver(t=>{let r=!1;for(let n of t){for(let s of Array.from(n.addedNodes)){if(!(s instanceof Element))continue;let a=s.tagName.toLowerCase();if(a==="style"||a==="link"){if(s.id===We||s.id===Uo)continue;r=!0;break}}if(r)break}r&&Mt===void 0&&(Mt=window.setTimeout(()=>{Mt=void 0,o()},150))}).observe(document.head,{childList:!0}),o()},Vo=()=>{if(ra(),!document.getElementById(We)){let o=document.createElement("style");o.id=We,o.textContent=na,document.head.appendChild(o),b.info("REInvest OS theme stylesheet mounted")}window.__reosForceInstalled||(window.__reosForceInstalled=!0,document.body?Go():window.addEventListener("DOMContentLoaded",Go)),window.__reosCascadeInstalled||(window.__reosCascadeInstalled=!0,Fa())};var qo="ws-crm-customizer-version-badge",Ke=()=>{if(!document.body){document.addEventListener("DOMContentLoaded",Ke,{once:!0});return}if(document.getElementById(qo))return;let o=document.createElement("div");o.id=qo,o.style.cssText=`
    position: fixed;
    bottom: 12px;
    right: 12px;
    z-index: 999999;
  ...
  `,o.textContent=`Customizer v${ge}`,o.title=`Built ${fe}
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};console.log("[ws-crm-customizer] bundle-loaded");var M=(o,e)=>{try{b.info(`[main] calling ${o}`),e(),b.info(`[main] ${o} OK`)}catch(t){b.error(`[main:FAIL] ${o} threw:`,t)}},zo=()=>{b.info(`Loaded v${ge} (built ${fe})`),M("mountVersionBadge (initial)",Ke),M("mountFavicon",ao),M("mountThemeStylesheet",Vo),M("initPageRouter",Ht),b.info("[main] registering onPageChange subscriber"),Nt(o=>{b.info(`[main] onPageChange callback fired for page=${o}`),M("mountVersionBadge",Ke),M("mountClaimReleaseToggle",Wt),M("mountContactActionsBar",ro),M("mountActiveFollowUpButton",Ut),M("mountMtOfferMadeButton",io),M("mountNotInterestedButton",lo),M("mountReducePriceButton",bo),M("mountSendToClosingOfficeButton",_o),M("mountClosingAgentPickerButton",Eo)}),b.info("[main] onPageChange registered; main() complete")};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",zo):zo();})();
