...
`,I=`
  ...
  flex-shrink: 0;
`;var F="ws-crm-claim-release-pill",tt=o=>{let e=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Ft=({contactId:o,mode:e})=>{let t=document.createElement("button");t.id=F,t.type="button",t.dataset.contactId=o,t.dataset.mode=e;let r=e==="claim"?"emerald":"amber",a=e==="claim"?i.emerald:i.amber;return t.style.cssText=X(r),t.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,Ot.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
    ...
    `,s.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=i.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{p.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let l=t.firstElementChild,n=t.children[1],c=t.children[2];l&&(l.style.background=V[a.color]),n&&(n.textContent=a.label),r.style.display="none";let m=_()?.locationId,b=Ht();if(!m){p.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=i.amber);return}if(a.color==="green"&&!b){p.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=i.amber);return}let v=c?.textContent??"\u25BE",x=async y=>{c&&(c.textContent="\u2026"),t.style.opacity="0.75";try{await Nt({contactId:o,locationId:m,status:a.label,userId:b,deadReason:y}),c&&(c.textContent="\u2713"),setTimeout(()=>{c&&(c.textContent=v),t.style.opacity="1"},900)}catch(h){p.warn("Call Status webhook failed:",h),c&&(c.textContent="\u2717"),n&&(n.style.color=i.amber),t.style.opacity="1",setTimeout(()=>{c&&(c.textContent=v),n&&(n.style.color=i.bone)},1800)}};if(a.value==="dead-deal"){let y=n?.textContent??"";Wt(h=>{x(h)},()=>{n&&(n.textContent=y)});return}if(a.value==="appointment-booked"){let y=n?.textContent??"";try{(await Pt({contactId:o,tenantId:m})).ok?await x():Vt(()=>{x()},()=>{n&&(n.textContent=y)})}catch(h){p.warn("Appointment sanity check failed; firing anyway:",h),await x()}return}await x()}),r.appendChild(s)}),t.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{e.contains(a.target)||(r.style.display="none")}),e.appendChild(t),e.appendChild(r),e},qt=o=>{document.getElementById(ot)?.remove();let e=document.createElement("div");e.id=ot,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var Ct=()=>{p.info(`Loaded v${H} (built ${N})`),it(),Et(),J(),Z(o=>{p.debug("Page handler firing for:",o),St(),et(),st()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ct):Ct();})();
