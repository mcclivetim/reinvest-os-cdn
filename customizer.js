...
`;var D="ws-crm-claim-release-pill",rt=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},It=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=D,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?i.emerald:i.amber;return e.style.cssText=et(r),e.innerHTML=`
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,$t.forEach(a=>{let l=document.createElement("button");l.type="button",l.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
  ...
  `,t.appendChild(e),document.body.appendChild(t);let l=()=>t.remove();t.addEventListener("click",s=>{s.target===t&&l()}),e.querySelector("#ws-offer-cancel")?.addEventListener("click",l),e.querySelector("#ws-offer-submit")?.addEventListener("click",()=>{let s=(e.querySelector("#ws-offer-amount")?.value??"").trim(),n=e.querySelector("#ws-offer-expires")?.value??"";if(!s||Number(s)<=0){b.warn("Offer Made submit blocked \u2014 invalid amount");return}b.info(`Offer Made recorded for contact ${o}: $${s}, expires ${n}`),l()}),setTimeout(()=>{e.querySelector("#ws-offer-amount")?.focus()},0)},st="ws-crm-dead-deal-modal",qt=(o,t)=>{document.getElementById(st)?.remove();let e=document.createElement("div");e.id=st,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(r),document.body.appendChild(e);let a=()=>e.remove(),l=()=>{a(),t()};e.addEventListener("click",n=>{n.target===e&&l()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",l),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{a(),o()});let s=n=>{n.key==="Escape"&&(document.removeEventListener("keydown",s),l())};document.addEventListener("keydown",s)},it=o=>{let t=document.createElement("div");t.id=O,t.dataset.contactId=o,t.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
...
/* Kill any non-SVG child painting bg blue/white inside the button. */
html body .contact-options-button > *:not(svg):not(path),
html body span.contact-options-button > *:not(svg):not(path) {
  background-color: transparent !important;
  background: transparent !important;
  box-shadow: none !important;
}
