/* Wholesaling CRM Customizer v0.5.8 — built 2026-05-16T03:45:01.208Z */
"use strict";var WholesalingCRMCustomizer=(()=>{var w="[wholesaling-crm-customizer]",ee="__WS_CRM_DEBUG",te=()=>!!window[ee],i={info(...e){console.log(w,...e)},warn(...e){console.warn(w,...e)},error(...e){console.error(w,...e)},debug(...e){te()&&console.log(w,"[debug]",...e)}};var F=()=>{let e=window.location.pathname;return e.includes("/contacts/detail/")?"contact-detail":e.includes("/contacts/smart-list")||e.includes("/contacts/")?"contact-list":e.includes("/opportunities/detail/")?"opportunity-detail":e.includes("/opportunities/")?"opportunity-list":e.includes("/calendars/")?"calendar":e.includes("/dashboard")?"dashboard":e.includes("/conversations")?"conversations":e.includes("/settings")?"settings":"other"},E="",y="other",M=new Set,k=()=>{if(window.location.pathname!==E){E=window.location.pathname,y=F(),i.debug("Page changed:",y,E);for(let e of M)try{e(y)}catch(o){i.error("Subscriber threw:",o)}}},R=e=>(M.add(e),setTimeout(()=>e(y),0),()=>{M.delete(e)}),B=()=>{y=F(),E=window.location.pathname,i.debug("Router init, current page:",y);let e=history.pushState.bind(history),o=history.replaceState.bind(history);history.pushState=function(t,a,r){e(t,a,r),k()},history.replaceState=function(t,a,r){o(t,a,r),k()},window.addEventListener("popstate",k),setInterval(k,1e3)};var _="0.5.8",C="2026-05-16T03:45:01.209Z";var oe=/\/contacts\/detail\/([A-Za-z0-9]+)/,re=/\/v2\/location\/([A-Za-z0-9]+)/,m=()=>{let e=window.location.pathname,o=e.match(oe);if(!o)return null;let t=e.match(re);return{contactId:o[1],locationId:t?t[1]:null}};var v=(e,o={})=>{let{timeoutMs:t=8e3,pollMs:a=100,root:r=document}=o;return new Promise((n,d)=>{let l=r.querySelector(e);if(l){n(l);return}let c,u,g=()=>{f&&f.disconnect(),c!==void 0&&window.clearTimeout(c),u!==void 0&&window.clearInterval(u)},f=new MutationObserver(()=>{let h=r.querySelector(e);h&&(g(),n(h))});f.observe(document.body,{childList:!0,subtree:!0}),u=window.setInterval(()=>{let h=r.querySelector(e);h&&(g(),n(h))},a),c=window.setTimeout(()=>{g(),i.warn(`waitForElement timed out for selector: ${e}`),d(new Error(`Selector not found within ${t}ms: ${e}`))},t)})};var s={obsidian:"#0A0D12",graphite:"#12161D",slate:"#1A1F28",steel:"#252C36",bone:"#EDEEF0",ash:"#9098A3",coolGray:"#5A6470",emerald:"#0FB57E",emeraldBright:"#14C98B",blue:"#4B8BF5",amber:"#E8A33C",crimson:"#D43F4A",emeraldGlow:"rgba(15, 181, 126, 0.12)",emeraldBorder:"rgba(15, 181, 126, 0.3)",blueGlow:"rgba(75, 139, 245, 0.12)",amberGlow:"rgba(232, 163, 60, 0.12)",crimsonGlow:"rgba(212, 63, 74, 0.12)"},b={sm:"4px",md:"6px",lg:"10px",pill:"999px"},p={sans:"'Geist', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",mono:"'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace"},D={purple:"#8B5CF6",green:s.emerald,orange:s.amber,red:s.crimson},O=e=>{let o=e==="emerald"?s.emeraldGlow:e==="amber"?s.amberGlow:s.slate,t=e==="emerald"?s.emeraldBorder:e==="amber"?"rgba(232, 163, 60, 0.3)":s.steel,a=e==="emerald"?s.emerald:e==="amber"?s.amber:s.bone;return`
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: 4px 10px;
...
`,$=`
  ...
  flex-shrink: 0;
`;var x="ws-crm-claim-release-pill",P=e=>{let o=Array.from(e.children).find(a=>{if(!(a instanceof HTMLElement))return!1;let r=a.className?.toString()??"";return r.includes("flex")&&r.includes("flex-col")});return o?o.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},ae=({contactId:e,mode:o})=>{let t=document.createElement("button");t.id=x,t.type="button",t.dataset.contactId=e,t.dataset.mode=o;let a=o==="claim"?"emerald":"amber",r=o==="claim"?s.emerald:s.amber;return t.style.cssText=O(a),t.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,t.title=o==="claim"?"Take ownership of this lead":"Return this lead to the pool",t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-1px)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",()=>{i.info(`Claim/Release pill clicked for contact ${e}: ${o}`);let n=t.lastElementChild,d=n?.textContent??"";n&&(n.textContent="Working\u2026"),t.style.opacity="0.7",t.disabled=!0,setTimeout(()=>{n&&(n.textContent=d),t.style.opacity="1",t.disabled=!1},700)}),t},ne=async(e,o=1e4)=>{let t=Date.now();for(;Date.now()-t<o;){let a=P(e);if(a)return a;await new Promise(r=>setTimeout(r,150))}return null},H=(e,o)=>{let t=ae({contactId:o,mode:"claim"}),a=document.createElement("span");a.id=`${x}-wrap`,a.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,a.appendChild(t),e.appendChild(a)},se=async()=>{let e=m();if(!e){document.getElementById(x)?.remove();return}let o=document.getElementById(x);if(!(o&&o.dataset.contactId===e.contactId)){o&&o.remove();try{let t=await v("#record-details-lhs",{timeoutMs:1e4}),a=await ne(t),r=m();if(!r||r.contactId!==e.contactId)return;if(!a){i.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}H(a,e.contactId),i.debug(`Mounted Claim/Release pill next to name for ${e.contactId}`);let n=null;n=new MutationObserver(async()=>{if(!(m()?.contactId===e.contactId)){n?.disconnect();return}if(document.getElementById(x))return;let l=P(t);l&&(H(l,e.contactId),i.debug("Re-mounted Claim/Release pill after React wipe"))}),n.observe(t,{childList:!0,subtree:!0})}catch(t){i.warn("Could not mount Claim/Release pill:",t)}}},N=()=>{se()};var ie="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set",le=()=>{let e=window.WS_CRM_USER_ID;if(e&&/^[A-Za-z0-9]{15,30}$/.test(e))return e;let o=window,t=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let a of t)if(typeof a=="string"&&/^[A-Za-z0-9]{15,30}$/.test(a))return a;try{let a=localStorage.getItem("user");if(a){let r=JSON.parse(a),n=r.id??r._id;if(n&&/^[A-Za-z0-9]{15,30}$/.test(n))return n}for(let r of["tokenUser","currentUser","ghl_user"]){let n=localStorage.getItem(r);if(n)try{let d=JSON.parse(n),l=d.id??d._id;if(l&&/^[A-Za-z0-9]{15,30}$/.test(l))return l}catch{}}}catch{}return null},ce=async({contactId:e,locationId:o,status:t,userId:a})=>{let n=await fetch(ie,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:e,location:{id:o},customData:{call_status:t,triggered_by_user_id:a??""}}),credentials:"omit"});if(!n.ok)throw new Error(`Webhook returned ${n.status} ${n.statusText}`)},T="ws-crm-contact-actions-bar",de="ws-crm-call-status-dropdown",G="ws-crm-offer-made-modal",pe=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],ue=[".central-panel","[class*='central-panel']"],me=async()=>{for(let e of ue)try{let o=await v(e,{timeoutMs:3e3});if(o)return o}catch{}try{return(await v("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},be=e=>{let o=document.createElement("div");o.id=de,o.style.cssText="position: relative; flex-shrink: 0;";let t=document.createElement("button");t.type="button",t.style.cssText=$,t.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `;let a=document.createElement("div");return a.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,pe.forEach(r=>{let n=document.createElement("button");n.type="button",n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
    ...
    `,n.innerHTML=`
      ...
      "></span>
      <span>${r.label}</span>
    `,n.addEventListener("mouseenter",()=>{n.style.background=s.slate}),n.addEventListener("mouseleave",()=>{n.style.background="transparent"}),n.addEventListener("click",async()=>{i.info(`Call Status set to "${r.label}" (${r.value}, ${r.color}) for contact ${e}`);let d=t.firstElementChild,l=t.children[1],c=t.children[2];d&&(d.style.background=D[r.color]),l&&(l.textContent=r.label),a.style.display="none";let g=m()?.locationId,f=le();if(!g){i.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),l&&(l.textContent=`${r.label} (no tenant!)`),l&&(l.style.color=s.amber);return}if(r.color==="green"&&!f){i.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),l&&(l.textContent=`${r.label} (no user!)`),l&&(l.style.color=s.amber);return}let h=c?.textContent??"\u25BE";c&&(c.textContent="\u2026"),t.style.opacity="0.75";try{await ce({contactId:e,locationId:g,status:r.label,userId:f}),c&&(c.textContent="\u2713"),setTimeout(()=>{c&&(c.textContent=h),t.style.opacity="1"},900)}catch(Q){i.warn("Call Status webhook failed:",Q),c&&(c.textContent="\u2717"),l&&(l.style.color=s.amber),t.style.opacity="1",setTimeout(()=>{c&&(c.textContent=h),l&&(l.style.color=s.bone)},1800)}}),a.appendChild(n)}),t.addEventListener("click",r=>{r.stopPropagation(),a.style.display=a.style.display==="none"?"block":"none"}),document.addEventListener("click",r=>{o.contains(r.target)||(a.style.display="none")}),o.appendChild(t),o.appendChild(a),o},he=e=>{document.getElementById(G)?.remove();let o=document.createElement("div");o.id=G,o.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,o.appendChild(t),document.body.appendChild(o);let n=()=>o.remove();o.addEventListener("click",d=>{d.target===o&&n()}),t.querySelector("#ws-offer-cancel")?.addEventListener("click",n),t.querySelector("#ws-offer-submit")?.addEventListener("click",()=>{let d=(t.querySelector("#ws-offer-amount")?.value??"").trim(),l=t.querySelector("#ws-offer-expires")?.value??"";if(!d||Number(d)<=0){i.warn("Offer Made submit blocked \u2014 invalid amount");return}i.info(`Offer Made recorded for contact ${e}: $${d}, expires ${l}`),n()}),setTimeout(()=>{t.querySelector("#ws-offer-amount")?.focus()},0)},z=e=>{let o=document.createElement("div");o.id=T,o.dataset.contactId=e,o.style.cssText=`
    ...
    flex-shrink: 0;
  `,o.appendChild(be(e));let t=document.createElement("button");return t.type="button",t.style.cssText=A,t.textContent="Offer Made",t.addEventListener("click",()=>he(e)),o.appendChild(t),o},ge=async()=>{let e=m();if(!e){document.getElementById(T)?.remove();return}let o=document.getElementById(T);if(o&&o.dataset.contactId===e.contactId)return;o&&o.remove();let t=await me();if(!t){i.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let a=m();if(!a||a.contactId!==e.contactId)return;let r=z(e.contactId);r.dataset.mountTarget=t.className||"central-panel",t.insertBefore(r,t.firstChild),i.debug(`Mounted contact actions bar for ${e.contactId}`);let n=null;n=new MutationObserver(()=>{let d=document.getElementById(T);if(!(m()?.contactId===e.contactId)){n?.disconnect();return}if(!d){let c=z(e.contactId);c.dataset.mountTarget=r.dataset.mountTarget??"",t.insertBefore(c,t.firstChild),i.debug(`Re-mounted actions bar after React wipe for ${e.contactId}`)}}),n.observe(t,{childList:!0})},U=()=>{ge()};var W="reos-theme-stylesheet",V="reos-theme-fonts",fe=()=>{if(document.getElementById(V))return;let e=document.createElement("link");e.rel="preconnect",e.href="https://fonts.googleapis.com",document.head.appendChild(e);let o=document.createElement("link");o.rel="preconnect",o.href="https://fonts.gstatic.com",o.crossOrigin="anonymous",document.head.appendChild(o);let t=document.createElement("link");t.id=V,t.rel="stylesheet",t.href="https://fonts.googleapis.com/css2?family=Geist:wght@400;500;600;700&family=Geist+Mono:wght@400;500;600&display=swap",document.head.appendChild(t)},ye=`
/* ============================================
...
Click to dismiss for this session`,e.addEventListener("mouseenter",()=>{e.style.transform="scale(1.05)"}),e.addEventListener("mouseleave",()=>{e.style.transform="scale(1)"}),e.addEventListener("click",()=>{e.style.opacity="0",setTimeout(()=>e.remove(),200)}),document.body.appendChild(e)};var X=()=>{i.info(`Loaded v${_} (built ${C})`),Z(),B(),R(e=>{i.debug("Page handler firing for:",e),J(),N(),U()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",X):X();})();
