...
`;var Tr=/^[A-Za-z0-9]{15,30}$/,ne=t=>typeof t=="string"&&Tr.test(t),Sr=t=>{try{let e=t.split(".");if(e.length!==3)return null;let o=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=o+"=".repeat((4-o.length%4)%4);return JSON.parse(atob(r))}catch{return null}},Cr=()=>{let t=window.WS_CRM_USER_ID;return ne(t)?t:null},Ir=()=>{let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let o of e)if(ne(o))return o;return null},Ar=()=>{try{let t=localStorage.getItem("refreshedToken");if(!t)return null;let e=t.startsWith('"')?t.slice(1,-1):t,o=Sr(e);if(!o)return null;let r=o.uid;if(ne(r))return r;let n=o.claims;if(n&&ne(n.user_id))return n.user_id}catch{}return null},Lr=()=>{try{let t=["user","tokenUser","currentUser","ghl_user"];for(let e of t){let o=localStorage.getItem(e);if(o)try{let r=JSON.parse(o),n=r.id??r._id;if(ne(n))return n}catch{}}}catch{}return null},I=()=>Cr()||Ir()||Ar()||Lr()||null;var lo="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",Mr=`${lo}/active-follow-up-get`,Dr=`${lo}/active-follow-up-set`,Y="ws-crm-active-follow-up-button",ht={amber:{label:"Add to Active Follow Up",bg:s.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:s.amber,clickable:!0,dotColor:s.amber},blue:{label:"In Active Follow Up",bg:s.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:s.blue,clickable:!1,dotColor:s.blue},gray:{label:"Not in Follow Up",bg:s.slate,border:s.steel,color:s.coolGray,clickable:!1,dotColor:s.coolGray},loading:{label:"Active Follow Up\u2026",bg:s.slate,border:s.steel,color:s.ash,clickable:!1,dotColor:s.ash},error:{label:"Follow Up status unavailable",bg:s.slate,border:s.steel,color:s.ash,clickable:!1,dotColor:s.crimson}},Fr=()=>{let t=S();return!t||!t.locationId?null:{contactId:t.contactId,tenantId:t.locationId}},Hr=async t=>{let e=await fetch(Mr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},Pr=async(t,e)=>{let o=await fetch(Dr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t.tenantId,contact_id:t.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!o.ok)throw new Error(`Set webhook returned ${o.status} ${o.statusText}`);return await o.json()},X=(t,e)=>{let o=ht[e];t.dataset.state=e,t.disabled=!o.clickable,t.style.cursor=o.clickable?"pointer":"default",t.style.background=o.bg,t.style.borderColor=o.border,t.style.color=o.color;let r=t.querySelector(".ws-afu-dot");r&&(r.style.background=o.dotColor);let n=t.querySelector(".ws-afu-label");n&&(n.textContent=o.label)},ao=t=>{let e=document.createElement("button");return e.type="button",e.id=Y,e.dataset.contactId=t,e.style.cssText=`
  ...
  `,X(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=Fr();if(!(!r||r.contactId!==t)){X(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await Pr(r,I()),a=n&&n.state&&n.state in ht?n.state:"blue";X(e,a),b.info(`Active Follow Up set for ${t} \u2192 ${a}`)}catch(n){b.error("Active Follow Up Set webhook failed",n),X(e,"error"),setTimeout(()=>void De(e,r),4e3)}}}),e},De=async(t,e)=>{X(t,"loading");try{let o=await Hr(e),r=o&&o.state&&o.state in ht?o.state:"gray",n=document.getElementById(Y);if(!n||n.dataset.contactId!==e.contactId)return;X(n,r)}catch(o){b.warn("Active Follow Up Get webhook failed",o),X(t,"error")}},Or=[".central-panel","[class*='central-panel']"],bt="ws-crm-contact-actions-bar",$r=async()=>{for(let t of Or)try{let e=await C(t,{timeoutMs:3e3});if(e)return e}catch{}try{return(await C("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},Nr=async(t,e)=>{let o=e.querySelector(`#${bt}`);if(!o)try{o=await C(`#${bt}`,{timeoutMs:3e3,root:e})}catch{o=null}if(o){let a=e.querySelector("#ws-crm-afu-wrap");return a&&a.remove(),o.appendChild(t),{container:o,placement:"actions-bar"}}let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
  ...
  `,o.innerHTML=`
    ...
    `;let l=a.type==="PA"?s.emerald:a.type==="AA"?s.blue:s.coolGray;i.innerHTML=`
      ...
      </div>`,i.addEventListener("mouseenter",()=>{i.style.background=s.slate}),i.addEventListener("mouseleave",()=>{i.style.background="transparent"}),i.addEventListener("click",async d=>{d.stopPropagation(),d.preventDefault(),b.info(`[prepare-contract] template picked: ${a.name} (type=${a.type})`),o.remove(),document.removeEventListener("click",r,!0);let c=I();try{if(a.type==="AA"){let h;try{h=await Na(e.tenantId,e.contactId)}catch(m){Q(`Failed to load buyer's MT opps: ${m.message}`,"error");return}qa({tenantId:e.tenantId,contactId:e.contactId,template:a,mtOpps:h,userId:c})}else if(a.type==="PA"){Q("Reading contract data\u2026","info",2500);let h;try{h=await Ut({tenantId:e.tenantId,contactId:e.contactId,template:a,userId:c})}catch(m){Q(`Failed to prepare contract: ${m.message}`,"error",6e3),b.warn("[prepare-contract] PA stage failed:",m);return}Ua({tenantId:e.tenantId,contactId:e.contactId,template:a,staged:h})}else{Q(`Opening ${a.name}\u2026`,"info",2500);try{await qt(e.tenantId,e.contactId,a)}catch(h){Q(`Failed to open contract: ${h.message}`,"error",6e3),b.warn("[prepare-contract] passthrough failed:",h)}}}catch(h){b.warn("[prepare-contract] picker click handler failed:",h),Q(`Unexpected error: ${h.message}`,"error",6e3)}}),o.appendChild(i)}},Wa=()=>{let t=document.createElement("button");return t.id=Gt,t.type="button",t.style.cssText=`
    position: relative;
    background: ${s.slate};
    border: 1px solid ${s.steel};
    border-radius: ${x.sm};
