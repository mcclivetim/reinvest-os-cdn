...
`;var Ut=/^[A-Za-z0-9]{15,30}$/,z=o=>typeof o=="string"&&Ut.test(o),qt=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},zt=()=>{let o=window.WS_CRM_USER_ID;return z(o)?o:null},Wt=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(z(t))return t;return null},jt=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=qt(e);if(!t)return null;let r=t.uid;if(z(r))return r;let n=t.claims;if(n&&z(n.user_id))return n.user_id}catch{}return null},Kt=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(z(n))return n}catch{}}}catch{}return null},A=()=>zt()||Wt()||jt()||Kt()||null;var Qe="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",Yt=`${Qe}/active-follow-up-get`,Xt=`${Qe}/active-follow-up-set`,O="ws-crm-active-follow-up-button",De={amber:{label:"Add to Active Follow Up",bg:d.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:d.amber,clickable:!0,dotColor:d.amber},blue:{label:"In Active Follow Up",bg:d.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:d.blue,clickable:!1,dotColor:d.blue},gray:{label:"Not in Follow Up",bg:d.slate,border:d.steel,color:d.coolGray,clickable:!1,dotColor:d.coolGray},loading:{label:"Active Follow Up\u2026",bg:d.slate,border:d.steel,color:d.ash,clickable:!1,dotColor:d.ash},error:{label:"Follow Up status unavailable",bg:d.slate,border:d.steel,color:d.ash,clickable:!1,dotColor:d.crimson}},Jt=()=>{let o=T();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},Qt=async o=>{let e=await fetch(Yt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},Zt=async(o,e)=>{let t=await fetch(Xt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},B=(o,e)=>{let t=De[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},Ye=o=>{let e=document.createElement("button");return e.type="button",e.id=O,e.dataset.contactId=o,e.style.cssText=`
  ...
  `,B(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=Jt();if(!(!r||r.contactId!==o)){B(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await Zt(r,A()),a=n&&n.state&&n.state in De?n.state:"blue";B(e,a),b.info(`Active Follow Up set for ${o} \u2192 ${a}`)}catch(n){b.error("Active Follow Up Set webhook failed",n),B(e,"error"),setTimeout(()=>void le(e,r),4e3)}}}),e},le=async(o,e)=>{B(o,"loading");try{let t=await Qt(e),r=t&&t.state&&t.state in De?t.state:"gray",n=document.getElementById(O);if(!n||n.dataset.contactId!==e.contactId)return;B(n,r)}catch(t){b.warn("Active Follow Up Get webhook failed",t),B(o,"error")}},eo=[".central-panel","[class*='central-panel']"],Fe="ws-crm-contact-actions-bar",to=async()=>{for(let o of eo)try{let e=await C(o,{timeoutMs:3e3});if(e)return e}catch{}try{return(await C("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},oo=async(o,e)=>{let t=e.querySelector(`#${Fe}`);if(!t)try{t=await C(`#${Fe}`,{timeoutMs:3e3,root:e})}catch{t=null}if(t){let a=e.querySelector("#ws-crm-afu-wrap");return a&&a.remove(),t.appendChild(o),{container:t,placement:"actions-bar"}}let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
  ...
  `,t.forEach(l=>{let c=document.createElement("button");c.type="button",c.style.cssText=`
    ...
    `,c.innerHTML=`
      ...
      "></span>
      <span>${l.label}</span>
    `,c.addEventListener("mouseenter",()=>{c.style.background=d.slate}),c.addEventListener("mouseleave",()=>{c.style.background="transparent"}),c.addEventListener("click",async()=>{b.info(`Call Status set to "${l.label}" (${l.value}, ${l.color}) for contact ${o}`);let p=i.firstElementChild,m=i.children[1],h=i.children[2];p&&(p.style.background=Le[l.color]),m&&(m.textContent=l.label),s.style.display="none";let v=T()?.locationId,f=A();if(!v){b.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),m&&(m.textContent=`${l.label} (no tenant!)`),m&&(m.style.color=d.amber);return}if(e==="seller"&&l.color==="green"&&!f){b.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),m&&(m.textContent=`${l.label} (no user!)`),m&&(m.style.color=d.amber);return}let g=h?.textContent??"\u25BE",w=async u=>{h&&(h.textContent="\u2026"),i.style.opacity="0.75";try{await yo({contactId:o,locationId:v,status:l.label,userId:f,deadReason:u,webhookUrl:r,kind:e}),h&&(h.textContent="\u2713"),setTimeout(()=>{h&&(h.textContent=g),i.style.opacity="1"},900)}catch(x){b.warn("Call Status webhook failed:",x),h&&(h.textContent="\u2717"),m&&(m.style.color=d.amber),i.style.opacity="1",setTimeout(()=>{h&&(h.textContent=g),m&&(m.style.color=d.bone)},1800)}};if(e==="seller"&&l.value==="dead-deal"){let u=m?.textContent??"";So(x=>{w(x)},()=>{m&&(m.textContent=u)});return}if(e==="seller"&&l.value==="appointment-booked"){let u=m?.textContent??"";try{(await go({contactId:o,tenantId:v})).ok?await w():Co(()=>{w()},()=>{m&&(m.textContent=u)})}catch(x){b.warn("Appointment sanity check failed; firing anyway:",x),await w()}return}await w()}),s.appendChild(c)}),i.addEventListener("click",l=>{l.stopPropagation(),s.style.display=s.style.display==="none"?"block":"none"}),document.addEventListener("click",l=>{a.contains(l.target)||(s.style.display="none")}),a.appendChild(i),a.appendChild(s),a},Eo=o=>{document.getElementById(st)?.remove();let e=document.createElement("div");e.id=st,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let t=pe();if(!t||!t.locationId||t.oppId!==o)return;let r=A(),n=e.querySelector(".ws-ni-label");e.disabled=!0,e.style.opacity="0.7",n&&(n.textContent="Closing\u2026");try{let a=await Bo({oppId:t.oppId,locationId:t.locationId,userId:r});if(a?.ok)n&&(n.textContent="Marked Not Interested"),e.style.background=d.crimsonGlow,e.style.color=d.crimson,b.info(`Not Interested set for opp ${o} \u2192 ${a.new_stage}`);else throw new Error("Webhook returned ok=false")}catch(a){b.error("Not Interested webhook failed",a),n&&(n.textContent="Try again"),e.disabled=!1,e.style.opacity="1",setTimeout(()=>{n&&n.textContent==="Try again"&&(n.textContent="Mark Not Interested")},3e3)}}),e},Go=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],$o=async()=>{for(let o of Go)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},he=!1,Be=async()=>{if(!he){he=!0;try{await Vo()}finally{he=!1}}},Vo=async()=>{let o=pe();if(!o||!o.locationId){document.getElementById(L)?.remove(),document.getElementById(`${L}-wrap`)?.remove(),X();return}let e=document.getElementById(L);if(e&&e.dataset.oppId===o.oppId){let s=R();s!==null&&s!=="Marketing Tracker"&&(e.remove(),document.getElementById(`${L}-wrap`)?.remove(),X());return}e&&(e.remove(),document.getElementById(`${L}-wrap`)?.remove());let t=await $o();if(!t){b.warn("Not Interested: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await be();if(r!=="Marketing Tracker"){b.info(`[not-interested] gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=pe();if(!n||n.oppId!==o.oppId)return;let a=Ro(o.oppId),i=document.createElement("span");i.id=`${L}-wrap`,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),i.appendChild(a),t.appendChild(i),b.debug(`Mounted Not Interested button on opp ${o.oppId}`),Uo(o.oppId)},Y=null,Oe=null,X=()=>{Y?.disconnect(),Y=null,Oe=null},Uo=o=>{if(Y&&Oe===o)return;X();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(Oe=o,Y=new MutationObserver(()=>{let t=pe();if(!t||t.oppId!==o){X();return}let r=R();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(L)?.remove(),document.getElementById(`${L}-wrap`)?.remove(),X();return}document.getElementById(L)||Be()}),Y.observe(e,{childList:!0,subtree:!0}))},Ne=null,Pe,qo=()=>{Ne||(Ne=new MutationObserver(()=>{Pe===void 0&&(Pe=window.setTimeout(()=>{Pe=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(L);o&&!e&&!he&&Be()},300))}),Ne.observe(document.body,{childList:!0,subtree:!0}))},vt=()=>{qo(),Be()};var yt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/tc/send-to-title-co",U="ws-crm-send-to-title-co-button",J="ws-crm-send-to-title-co-wrap",ve="ws-crm-stc-dropdown",zo=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,Wo=/\/v2\/location\/([A-Za-z0-9]+)/,$e=()=>{let o=window.location.pathname,e=o.match(zo);if(!e)return null;let t=o.match(Wo);return{oppId:e[1],locationId:t?t[1]:null}},jo=async(o,e)=>{try{let t=await fetch(yt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:o,triggered_by_user_id:A()??null,dry_run:!0}),credentials:"omit"});return t.ok?await t.json():(b.warn(`[stc] dry_run returned HTTP ${t.status}`),null)}catch(t){return b.warn("[stc] dry_run fetch failed:",t),null}},Ko=async(o,e,t)=>{let r=await fetch(yt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:o,triggered_by_user_id:A()??null,confirm_resend:t}),credentials:"omit"});if(!r.ok&&r.status!==200)throw new Error(`HTTP ${r.status}`);return await r.json()},Yo=o=>{let e=document.getElementById(o);if(!e){b.warn(`[stc] field ${o} not found in DOM \u2014 can't scroll to it`);return}e.scrollIntoView({behavior:"smooth",block:"center"});let t=e.style.boxShadow,r=e.style.transition;e.style.transition="box-shadow 0.3s ease",e.style.boxShadow=`0 0 0 3px ${d.emerald}, 0 0 12px ${d.emerald}`,window.setTimeout(()=>{e.style.boxShadow=t,window.setTimeout(()=>{e.style.transition=r},400)},2400)},M=()=>{document.getElementById(ve)?.remove()},me=(o,e,t)=>{M();let r=document.createElement("div");r.id=ve;let n=t.getBoundingClientRect();r.style.cssText=`
    position: fixed;
    top: ${n.bottom+6}px;
  ...
  `,r.appendChild(l);let c=["TC","ACQ","MT","DISPO","Property"],p={};o.forEach(_=>{p[_.group]||(p[_.group]=[]),p[_.group].push(_)});let m=[...c.filter(_=>p[_]),...Object.keys(p).filter(_=>!c.includes(_))];for(let _ of m){let v=p[_],f=document.createElement("div");f.style.cssText="margin-top: 12px;";let g=document.createElement("div");g.style.cssText=`
      font-family: ${y.mono};
      font-size: 9px;
    ...
    `,g.textContent=_,f.appendChild(g),v.forEach(w=>{let u=document.createElement("div"),x=!w.filled&&w.navigable&&w.field_id;u.style.cssText=`
        ...
        border-radius: ${E.sm};
        font-size: 12px;
        color: ${w.filled?d.bone:d.ash};
        ${x?"cursor: pointer;":""}
        transition: background 0.1s ease;
      `,x&&(u.addEventListener("mouseenter",()=>{u.style.background=d.slate}),u.addEventListener("mouseleave",()=>{u.style.background="transparent"}),u.addEventListener("click",I=>{I.stopPropagation(),w.field_id&&(Yo(w.field_id),M())}));let S=w.filled?`<span style="color: ${d.emerald}; font-size: 14px; line-height: 1; flex-shrink: 0;">\u2713</span>`:`<span style="color: ${d.amber}; font-size: 14px; line-height: 1; flex-shrink: 0;">\u25CB</span>`,k=x?`<span style="margin-left: auto; color: ${d.ash}; font-size: 11px;">\u2192</span>`:"";u.innerHTML=`${S}<span style="flex: 1;">${w.label}</span>${k}`,f.appendChild(u)}),r.appendChild(f)}document.body.appendChild(r);let h=_=>{let v=_.target;!r.contains(v)&&!t.contains(v)&&(M(),document.removeEventListener("click",h))};window.setTimeout(()=>document.addEventListener("click",h),0)},D=(o,e)=>{let t=o.querySelector(".ws-stc-label"),r=o.querySelector(".ws-stc-dot"),n=o.querySelector(".ws-stc-chevron"),a,i,s=d.bone,l,c="Send to Title Company",p="",m="pointer",h="1";switch(e.status){case"checking":a="transparent",i=d.steel,s=d.ash,l=d.coolGray,c="Checking\u2026",m="wait",h="0.8";break;case"ready":a=d.emerald,i=d.emerald,s="#0a0e14",l="#0a0e14",c=e.previouslySentAt?"Re-send to Title Co":"Send to Title Company";break;case"incomplete":{let _=e.checks?.filter(f=>f.filled).length??0,v=e.checks?.length??0;a="transparent",i=d.amber,s=d.amber,l=d.amber,c=`${_} of ${v} fields ready`,p="\u25BE";break}case"sending":a=d.emerald,i=d.emerald,s="#0a0e14",l="#0a0e14",c="Sending\u2026",m="wait",h="0.8";break;case"sent":a=d.emerald,i=d.emerald,s="#0a0e14",l="#0a0e14",c="Sent \u2713";break;case"error":a="transparent",i=d.amber,s=d.amber,l=d.amber,c=e.message??"Try again";break}o.style.background=a,o.style.borderColor=i,o.style.color=s,o.style.cursor=m,o.style.opacity=h,r&&(r.style.background=l,r.style.boxShadow=e.status==="ready"||e.status==="sent"?`0 0 6px ${l}`:""),t&&(t.textContent=c),n&&(n.textContent=p)},Xo=(o,e)=>{let t=document.createElement("button");t.id=U,t.type="button",t.dataset.oppId=o;let r={status:"checking"};t.style.cssText=`
    ${P}
    background: transparent;
    border: 1px solid ${d.steel};
    color: ${d.ash};
  ...
  `,D(t,r);let n=async()=>{if(r.status==="sending")return;let l=await jo(o,e);if(!l){r={status:"error",message:"Check failed \u2014 click to retry"},D(t,r);return}if(!Array.isArray(l.checks)||l.checks.length===0){r={status:"error",message:l.error||"Check failed \u2014 click to retry"},D(t,r),b.warn("[stc] dry_run returned no checks:",l);return}r={status:l.ok?"ready":"incomplete",checks:l.checks,previouslySentAt:l.previously_sent_at??null},D(t,r),document.getElementById(ve)&&me(l.checks,l.previously_sent_at,t)},a=async()=>{let l=!!r.previouslySentAt;r={status:"sending",checks:r.checks,previouslySentAt:r.previouslySentAt},D(t,r),M();try{let c=await Ko(o,e,l);if(c.needs_confirm){r={status:"ready",checks:c.checks,previouslySentAt:c.previously_sent_at},D(t,r),c.checks&&me(c.checks,c.previously_sent_at,t);return}if(c.ok){r={status:"sent",checks:c.checks,previouslySentAt:new Date().toISOString()},D(t,r),window.setTimeout(()=>{n()},3500);return}r={status:"error",checks:c.checks,message:c.error||"Validation failed"},D(t,r),c.checks&&me(c.checks,c.previously_sent_at,t)}catch(c){b.error("[stc] send failed:",c),r={status:"error",message:"Network error \u2014 click to retry"},D(t,r)}};t.addEventListener("click",()=>{switch(r.status){case"ready":a();break;case"incomplete":{document.getElementById(ve)?M():r.checks&&me(r.checks,r.previouslySentAt,t);break}case"error":case"sent":n();break}}),n();let i,s=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(s){let l=new MutationObserver(()=>{i!==void 0&&window.clearTimeout(i),i=window.setTimeout(()=>{r.status!=="sending"&&n()},1500)});l.observe(s,{childList:!0,subtree:!0,characterData:!0}),t.dataset.observerActive="1";let c=window.setInterval(()=>{document.body.contains(t)||(l.disconnect(),i!==void 0&&window.clearTimeout(i),window.clearInterval(c))},5e3)}return t},Jo=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],Qo=async()=>{for(let o of Jo)try{let e=await C(o,{timeoutMs:1500});if(e)return b.info(`[send-to-title-co] mount target: ${o}`),e}catch{}return b.warn(`[send-to-title-co] no mount target found. URL: ${window.location.pathname}`),null},_e=!1,Ue=async()=>{if(_e){b.info("[stc] mount:00-LOCKED ensureMounted already in-flight \u2014 skip");return}_e=!0;try{await Zo()}catch(o){b.error("[stc] ensureMountedImpl threw:",o)}finally{_e=!1}},Zo=async()=>{b.info("[stc] mount:01 ensureMountedImpl entered");let o=$e();if(!o||!o.locationId){b.info(`[stc] mount:02-BAIL no ctx or no locationId. ctx=${JSON.stringify(o)}`),document.getElementById(U)?.remove(),document.getElementById(J)?.remove(),M();return}b.info(`[stc] mount:02 ctx ok \u2014 oppId=${o.oppId} locId=${o.locationId}`);let e=document.getElementById(U);if(e&&e.dataset.oppId===o.oppId){b.info("[stc] mount:03-SKIP existing button for same opp");let s=R();s!==null&&s!=="Transaction Coordination"&&(e.remove(),document.getElementById(J)?.remove(),M(),ue());return}e&&(b.info("[stc] mount:03 removing stale existing button"),e.remove(),document.getElementById(J)?.remove(),M()),b.info("[stc] mount:04 calling findMountTarget");let t=await Qo();if(!t){b.info("[stc] mount:04-BAIL findMountTarget returned null");return}b.info(`[stc] mount:04 target found tag=${t.tagName} class="${t.className}"`),b.info("[stc] mount:05 calling waitForOppPipelineName");let r=await be();if(b.info(`[stc] mount:05 pipeline detected = "${r}"`),r!=="Transaction Coordination"){b.info(`[stc] mount:05-BAIL gated off \u2014 pipeline="${r}" (expected Transaction Coordination)`);return}let n=$e();if(!n||n.oppId!==o.oppId||!n.locationId){b.info(`[stc] mount:06-BAIL ctx changed during awaits. was=${o.oppId} now=${n?.oppId}`);return}b.info("[stc] mount:06 building button");let a=Xo(o.oppId,n.locationId),i=document.createElement("span");i.id=J,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),i.appendChild(a),t.appendChild(i),b.info(`[stc] mount:07-OK Mounted on opp ${o.oppId}, inDoc=${document.body.contains(a)}`),er(o.oppId)},Q=null,Ve=null,ue=()=>{Q?.disconnect(),Q=null,Ve=null},er=o=>{if(Q&&Ve===o)return;ue();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(Ve=o,Q=new MutationObserver(()=>{let t=$e();if(!t||t.oppId!==o){ue();return}let r=R();if(r!==null&&r!=="Transaction Coordination"){document.getElementById(U)?.remove(),document.getElementById(J)?.remove(),M(),ue();return}document.getElementById(U)||Ue()}),Q.observe(e,{childList:!0,subtree:!0}))},Re=null,Ge,tr=()=>{Re||(Re=new MutationObserver(()=>{Ge===void 0&&(Ge=window.setTimeout(()=>{Ge=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(U);o&&!e&&!_e&&Ue()},300))}),Re.observe(document.body,{childList:!0,subtree:!0}))},gt=()=>{tr(),Ue()};var ke="reos-theme-stylesheet",Nt="reos-theme-fonts",or=()=>{let o=document.getElementById(Nt);o&&o.remove()},rr=`
/* ============================================
