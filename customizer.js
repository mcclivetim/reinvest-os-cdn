...
`,I=`
  ...
  flex-shrink: 0;
`;var F="ws-crm-claim-release-pill",Q=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Ie=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=F,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",n=t==="claim"?l.emerald:l.amber;return e.style.cssText=J(r),e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,Pe.forEach(n=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
    ...
    `,s.innerHTML=`
      ...
      "></span>
      <span>${n.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=l.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{p.info(`Call Status set to "${n.label}" (${n.value}, ${n.color}) for contact ${o}`);let i=e.firstElementChild,a=e.children[1],c=e.children[2];i&&(i.style.background=W[n.color]),a&&(a.textContent=n.label),r.style.display="none";let m=E()?.locationId,u=Be();if(!m){p.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),a&&(a.textContent=`${n.label} (no tenant!)`),a&&(a.style.color=l.amber);return}if(n.color==="green"&&!u){p.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),a&&(a.textContent=`${n.label} (no user!)`),a&&(a.style.color=l.amber);return}let v=c?.textContent??"\u25BE",w=async g=>{c&&(c.textContent="\u2026"),e.style.opacity="0.75";try{await He({contactId:o,locationId:m,status:n.label,userId:u,deadReason:g}),c&&(c.textContent="\u2713"),setTimeout(()=>{c&&(c.textContent=v),e.style.opacity="1"},900)}catch(h){p.warn("Call Status webhook failed:",h),c&&(c.textContent="\u2717"),a&&(a.style.color=l.amber),e.style.opacity="1",setTimeout(()=>{c&&(c.textContent=v),a&&(a.style.color=l.bone)},1800)}};if(n.value==="dead-deal"){let g=a?.textContent??"";qe(h=>{w(h)},()=>{a&&(a.textContent=g)});return}if(n.value==="appointment-booked"){let g=a?.textContent??"";try{(await Re({contactId:o,tenantId:m})).ok?await w():We(()=>{w()},()=>{a&&(a.textContent=g)})}catch(h){p.warn("Appointment sanity check failed; firing anyway:",h),await w()}return}await w()}),r.appendChild(s)}),e.addEventListener("click",n=>{n.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",n=>{t.contains(n.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},Ue=o=>{document.getElementById(te)?.remove();let t=document.createElement("div");t.id=te,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var Se=()=>{p.info(`Loaded v${H} (built ${R})`),ie(),ke(),Z(),Y(o=>{p.debug("Page handler firing for:",o),_e(),ee(),ae()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Se):Se();})();
