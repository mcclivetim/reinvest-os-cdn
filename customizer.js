/* Wholesaling CRM Customizer v0.7.86 — built 2026-05-19T13:26:52.827Z */
"use strict";var WholesalingCRMCustomizer=(()=>{var N="[wholesaling-crm-customizer]",Ft="__WS_CRM_DEBUG",At=()=>!!window[Ft],h={info(...o){console.log(N,...o)},warn(...o){console.warn(N,...o)},error(...o){console.error(N,...o)},debug(...o){At()&&console.log(N,"[debug]",...o)}};var Z=()=>{let o=window.location.pathname;return o.includes("/contacts/detail/")?"contact-detail":o.includes("/contacts/smart-list")||o.includes("/contacts/")?"contact-list":o.includes("/opportunities/detail/")?"opportunity-detail":o.includes("/opportunities/")?"opportunity-list":o.includes("/calendars/")?"calendar":o.includes("/dashboard")?"dashboard":o.includes("/conversations")?"conversations":o.includes("/settings")?"settings":"other"},R="",C="other",q=new Set,H=()=>{if(window.location.pathname!==R){R=window.location.pathname,C=Z(),h.debug("Page changed:",C,R);for(let o of q)try{o(C)}catch(t){h.error("Subscriber threw:",t)}}},Q=o=>(q.add(o),setTimeout(()=>o(C),0),()=>{q.delete(o)}),tt=()=>{C=Z(),R=window.location.pathname,h.debug("Router init, current page:",C);let o=history.pushState.bind(history),t=history.replaceState.bind(history);history.pushState=function(e,r,a){o(e,r,a),H()},history.replaceState=function(e,r,a){t(e,r,a),H()},window.addEventListener("popstate",H),setInterval(H,1e3)};var M="0.7.86",B="2026-05-19T13:26:52.828Z";var Lt=/\/contacts\/detail\/([A-Za-z0-9]+)/,It=/\/v2\/location\/([A-Za-z0-9]+)/,_=()=>{let o=window.location.pathname,t=o.match(Lt);if(!t)return null;let e=o.match(It);return{contactId:t[1],locationId:e?e[1]:null}};var F=(o,t={})=>{let{timeoutMs:e=8e3,pollMs:r=100,root:a=document}=t;return new Promise((s,l)=>{let n=a.querySelector(o);if(n){s(n);return}let i,c,p=()=>{b&&b.disconnect(),i!==void 0&&window.clearTimeout(i),c!==void 0&&window.clearInterval(c)},b=new MutationObserver(()=>{let g=a.querySelector(o);g&&(p(),s(g))});b.observe(document.body,{childList:!0,subtree:!0}),c=window.setInterval(()=>{let g=a.querySelector(o);g&&(p(),s(g))},r),i=window.setTimeout(()=>{p(),h.warn(`waitForElement timed out for selector: ${o}`),l(new Error(`Selector not found within ${e}ms: ${o}`))},e)})};var d={obsidian:"#0A0D12",graphite:"#12161D",slate:"#1A1F28",steel:"#252C36",bone:"#EDEEF0",ash:"#9098A3",coolGray:"#5A6470",emerald:"#0FB57E",emeraldBright:"#14C98B",blue:"#4B8BF5",amber:"#E8A33C",crimson:"#D43F4A",emeraldGlow:"rgba(15, 181, 126, 0.12)",emeraldBorder:"rgba(15, 181, 126, 0.3)",blueGlow:"rgba(75, 139, 245, 0.12)",amberGlow:"rgba(232, 163, 60, 0.12)",crimsonGlow:"rgba(212, 63, 74, 0.12)"},w={sm:"4px",md:"6px",lg:"10px",pill:"999px"},f={sans:"'Geist', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",mono:"'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace"},j={purple:"#8B5CF6",green:d.emerald,orange:d.amber,red:d.crimson},et=o=>{let t=o==="emerald"?d.emeraldGlow:o==="amber"?d.amberGlow:d.slate,e=o==="emerald"?d.emeraldBorder:o==="amber"?"rgba(232, 163, 60, 0.3)":d.steel,r=o==="emerald"?d.emerald:o==="amber"?d.amber:d.bone;return`
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: 4px 10px;
...
`;var I="ws-crm-claim-release-pill",rt=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Dt=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?d.emerald:d.amber;return e.style.cssText=et(r),e.innerHTML=`
  ...
  `,$t.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
    ...
    `,s.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=d.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{h.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let l=e.firstElementChild,n=e.children[1],i=e.children[2];l&&(l.style.background=j[a.color]),n&&(n.textContent=a.label),r.style.display="none";let p=_()?.locationId,b=Bt();if(!p){h.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=d.amber);return}if(a.color==="green"&&!b){h.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=d.amber);return}let g=i?.textContent??"\u25BE",x=async v=>{i&&(i.textContent="\u2026"),e.style.opacity="0.75";try{await Ot({contactId:o,locationId:p,status:a.label,userId:b,deadReason:v}),i&&(i.textContent="\u2713"),setTimeout(()=>{i&&(i.textContent=g),e.style.opacity="1"},900)}catch(m){h.warn("Call Status webhook failed:",m),i&&(i.textContent="\u2717"),n&&(n.style.color=d.amber),e.style.opacity="1",setTimeout(()=>{i&&(i.textContent=g),n&&(n.style.color=d.bone)},1800)}};if(a.value==="dead-deal"){let v=n?.textContent??"";qt(m=>{x(m)},()=>{n&&(n.textContent=v)});return}if(a.value==="appointment-booked"){let v=n?.textContent??"";try{(await Gt({contactId:o,tenantId:p})).ok?await x():jt(()=>{x()},()=>{n&&(n.textContent=v)})}catch(m){h.warn("Appointment sanity check failed; firing anyway:",m),await x()}return}await x()}),r.appendChild(s)}),e.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{t.contains(a.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},Wt=o=>{document.getElementById(nt)?.remove();let t=document.createElement("div");t.id=nt,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
...
/* 15i. CONVERSATION-HEADER ICON BUTTONS \u2014 the row of icon-only action
   ...
   Without per-icon rules, Tim's feedback was:
     1. Slate hover bg looked like an off-color square (slate against
        graphite is visible \u2014 too heavy a hover).
     2. The phone icon's hover bg read as a "horrible blue box" (slate
        looks blue-tinted in this size against graphite).
     3. The trash icon didn't turn crimson on hover even though the
        SVG has 'group-hover:text-red-600' \u2014 because Tailwind paints
        red-600 (Tailwind crimson) and our generic .text-red-600 rule
        does not match 'group-hover:text-red-600' (different class
        name).

   Per the ACTIVE-ACCENT STANDARD with the destructive variant:
     - Non-destructive hover: emerald-glow bg + emerald icon stroke.
     - Destructive (delete) hover: crimson-glow bg + crimson icon. */
