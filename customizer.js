...
`;var I="ws-crm-claim-release-pill",rt=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Dt=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?c.emerald:c.amber;return e.style.cssText=et(r),e.innerHTML=`
  ...
  `,$t.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
    ...
    `,s.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=c.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{h.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let l=e.firstElementChild,n=e.children[1],i=e.children[2];l&&(l.style.background=j[a.color]),n&&(n.textContent=a.label),r.style.display="none";let b=_()?.locationId,p=Bt();if(!b){h.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=c.amber);return}if(a.color==="green"&&!p){h.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=c.amber);return}let g=i?.textContent??"\u25BE",x=async v=>{i&&(i.textContent="\u2026"),e.style.opacity="0.75";try{await Ot({contactId:o,locationId:b,status:a.label,userId:p,deadReason:v}),i&&(i.textContent="\u2713"),setTimeout(()=>{i&&(i.textContent=g),e.style.opacity="1"},900)}catch(m){h.warn("Call Status webhook failed:",m),i&&(i.textContent="\u2717"),n&&(n.style.color=c.amber),e.style.opacity="1",setTimeout(()=>{i&&(i.textContent=g),n&&(n.style.color=c.bone)},1800)}};if(a.value==="dead-deal"){let v=n?.textContent??"";qt(m=>{x(m)},()=>{n&&(n.textContent=v)});return}if(a.value==="appointment-booked"){let v=n?.textContent??"";try{(await Gt({contactId:o,tenantId:b})).ok?await x():jt(()=>{x()},()=>{n&&(n.textContent=v)})}catch(m){h.warn("Appointment sanity check failed; firing anyway:",m),await x()}return}await x()}),r.appendChild(s)}),e.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{t.contains(a.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},Wt=o=>{document.getElementById(nt)?.remove();let t=document.createElement("div");t.id=nt,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,e.appendChild(r),document.body.appendChild(e);let a=()=>e.remove(),s=()=>{a(),t()};e.addEventListener("click",n=>{n.target===e&&s()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",s),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{a(),o()});let l=n=>{n.key==="Escape"&&(document.removeEventListener("keydown",l),s())};document.addEventListener("keydown",l)},it=o=>{let t=document.createElement("div");t.id=O,t.dataset.contactId=o,t.style.cssText=`
    ...
    flex-shrink: 0;
  `,t.appendChild(zt(o));let e=document.createElement("button");return e.type="button",e.style.cssText=A,e.textContent="Offer Made",e.addEventListener("click",()=>Wt(o)),t.appendChild(e),t},Kt=async()=>{let o=_();if(!o){document.getElementById(O)?.remove();return}let t=document.getElementById(O);if(t&&t.dataset.contactId===o.contactId)return;t&&t.remove();let e=await Vt();if(!e){h.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let r=_();if(!r||r.contactId!==o.contactId)return;let a=it(o.contactId);a.dataset.mountTarget=e.className||"central-panel",e.insertBefore(a,e.firstChild),h.debug(`Mounted contact actions bar for ${o.contactId}`);let s=null;s=new MutationObserver(()=>{let l=document.getElementById(O);if(!(_()?.contactId===o.contactId)){s?.disconnect();return}if(!l){let i=it(o.contactId);i.dataset.mountTarget=a.dataset.mountTarget??"",e.insertBefore(i,e.firstChild),h.debug(`Re-mounted actions bar after React wipe for ${o.contactId}`)}}),s.observe(e,{childList:!0})},ct=()=>{Kt()};var G="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",Yt=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),D="data-ws-crm-favicon",K=o=>o.hasAttribute(D),dt=o=>{if(o.tagName!=="LINK")return!1;let t=(o.getAttribute("rel")??"").toLowerCase();return Yt.has(t)},P=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{dt(r)&&!K(r)&&r.remove()});let t=document.head.querySelector(`link[${D}="1"]`);t?t.href!==G&&(t.href=G):(t=document.createElement("link"),t.setAttribute(D,"1"),t.rel="icon",t.type="image/png",t.href=G,document.head.appendChild(t),h.debug("Favicon installed"));let e=document.head.querySelector(`link[${D}="apple"]`);e||(e=document.createElement("link"),e.setAttribute(D,"apple"),e.rel="apple-touch-icon",e.href=G,document.head.appendChild(e))},ht=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)P();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),P())},50)}new MutationObserver(r=>{let a=!1;for(let s of r)s.addedNodes.forEach(l=>{l instanceof HTMLLinkElement&&dt(l)&&!K(l)&&(a=!0)}),s.removedNodes.forEach(l=>{l instanceof HTMLLinkElement&&K(l)&&(a=!0)});a&&P()}).observe(document.head,{childList:!0,subtree:!1});let t=0,e=window.setInterval(()=>{t+=1,P(),t>=5&&window.clearInterval(e)},1e3)};var z="reos-theme-stylesheet",X="reos-theme-fonts",Xt=()=>{if(document.getElementById(X))return;let o=document.createElement("link");o.rel="preconnect",o.href="https://fonts.googleapis.com",document.head.appendChild(o);let t=document.createElement("link");t.rel="preconnect",t.href="https://fonts.gstatic.com",t.crossOrigin="anonymous",document.head.appendChild(t);let e=document.createElement("link");e.id=X,e.rel="stylesheet",e.href="https://fonts.googleapis.com/css2?family=Geist:wght@400;500;600;700&family=Geist+Mono:wght@400;500;600&display=swap",document.head.appendChild(e)},Jt=`
/* ============================================
...
/* Active row container \u2014 explicit emerald-glow per canonical
   ACTIVE state (section 11b). GHL ships this as slate; replace
   with the standard 10% emerald-glow + emerald border. The
   :not(#__reos_never_id) trick bumps specificity to beat the
   Vue scoped-CSS rule (data-v-19922083 attribute selector). */
html body .inbox-panel .menu-item-container.active:not(#__reos_never_id) {
...
/* HOVER: subtle slate row highlight, bone text + emerald icon.
   No transition so we don't get the post-hover ghost flash. */
html body .inbox-panel .menu-item:not(.active):hover,
html body .inbox-panel .menu-item-container:not(.active):hover,
html body .inbox-panel .hr-collapse-item__header:not(.hr-collapse-item__header--active):hover {
  background-color: var(--reos-slate) !important;
  transition: none !important;
}
html body .inbox-panel .menu-item:not(.active):hover .menu-text,
html body .inbox-panel .menu-item:not(.active):hover .menu-item-content {
  color: var(--reos-bone) !important;
}
html body .inbox-panel .menu-item:not(.active):hover svg,
html body .inbox-panel .menu-item:not(.active):hover svg path {
  color: var(--reos-emerald) !important;
  stroke: var(--reos-emerald) !important;
}
...
/* Panel + layout container :hover override \u2014 section 18a-1 paints
   ANY [class*="ECEEF2"]:hover slate, which catches the inbox-panel
   (bg-[#ECEEF2]) and #conversations-layout (also bg-[#ECEEF2]).
   When the cursor lands on the panel surface it turns lighter
   slate while the FORCE_RULE-painted headers stay graphite \u2014 the
   headers then read as dark rectangles "in" the panel (Tim's
   "black in the box" complaint).

   Pin both to graphite on hover so they stay the same shade as
   the headers and the surface reads as one continuous panel.
   Use .inbox-panel.inbox-panel (class chain) for specificity
   bump (0,3,2) over the (0,2,2) of [class*="ECEEF2"]:hover. */
