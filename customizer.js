/* Wholesaling CRM Customizer v0.10.34 — built 2026-05-21T21:14:39.966Z */
"use strict";var WholesalingCRMCustomizer=(()=>{var R="[wholesaling-crm-customizer]",Ne="__WS_CRM_DEBUG",Me=()=>!!window[Ne],p={info(...r){console.log(R,...r)},warn(...r){console.warn(R,...r)},error(...r){console.error(R,...r)},debug(...r){Me()&&console.log(R,"[debug]",...r)}};var oe=()=>{let r=window.location.pathname;return r.includes("/contacts/detail/")?"contact-detail":r.includes("/contacts/smart-list")||r.includes("/contacts/")?"contact-list":r.includes("/opportunities/detail/")?"opportunity-detail":r.includes("/opportunities/")?"opportunity-list":r.includes("/calendars/")?"calendar":r.includes("/dashboard")?"dashboard":r.includes("/conversations")?"conversations":r.includes("/settings")?"settings":"other"},B="",C="other",J=new Set,O=()=>{if(window.location.pathname!==B){B=window.location.pathname,C=oe(),p.debug("Page changed:",C,B);for(let r of J)try{r(C)}catch(e){p.error("Subscriber threw:",e)}}},re=r=>(J.add(r),setTimeout(()=>r(C),0),()=>{J.delete(r)}),ne=()=>{C=oe(),B=window.location.pathname,p.debug("Router init, current page:",C);let r=history.pushState.bind(history),e=history.replaceState.bind(history);history.pushState=function(t,o,a){r(t,o,a),O()},history.replaceState=function(t,o,a){e(t,o,a),O()},window.addEventListener("popstate",O),setInterval(O,1e3)};var G="0.10.34",V="2026-05-21T21:14:39.967Z";var Pe=/\/contacts\/detail\/([A-Za-z0-9]+)/,Re=/\/v2\/location\/([A-Za-z0-9]+)/,F=()=>{let r=window.location.pathname,e=r.match(Pe);if(!e)return null;let t=r.match(Re);return{contactId:e[1],locationId:t?t[1]:null}};var L=(r,e={})=>{let{timeoutMs:t=8e3,pollMs:o=100,root:a=document}=e;return new Promise((s,i)=>{let n=a.querySelector(r);if(n){s(n);return}let l,b,h=()=>{m&&m.disconnect(),l!==void 0&&window.clearTimeout(l),b!==void 0&&window.clearInterval(b)},m=new MutationObserver(()=>{let d=a.querySelector(r);d&&(h(),s(d))});m.observe(document.body,{childList:!0,subtree:!0}),b=window.setInterval(()=>{let d=a.querySelector(r);d&&(h(),s(d))},o),l=window.setTimeout(()=>{h(),p.warn(`waitForElement timed out for selector: ${r}`),i(new Error(`Selector not found within ${t}ms: ${r}`))},t)})};var c={obsidian:"#0A0D12",graphite:"#12161D",slate:"#1A1F28",steel:"#252C36",bone:"#EDEEF0",ash:"#9098A3",coolGray:"#5A6470",emerald:"#0FB57E",emeraldBright:"#14C98B",blue:"#4B8BF5",amber:"#E8A33C",crimson:"#D43F4A",emeraldGlow:"rgba(15, 181, 126, 0.12)",emeraldBorder:"rgba(15, 181, 126, 0.3)",blueGlow:"rgba(75, 139, 245, 0.12)",amberGlow:"rgba(232, 163, 60, 0.12)",crimsonGlow:"rgba(212, 63, 74, 0.12)"},E={sm:"4px",md:"6px",lg:"10px",pill:"999px"},w={sans:"'Geist', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",mono:"'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace"},Z={purple:"#8B5CF6",green:c.emerald,orange:c.amber,red:c.crimson},ae=r=>{let e=r==="emerald"?c.emeraldGlow:r==="amber"?c.amberGlow:c.slate,t=r==="emerald"?c.emeraldBorder:r==="amber"?"rgba(232, 163, 60, 0.3)":c.steel,o=r==="emerald"?c.emerald:r==="amber"?c.amber:c.bone;return`
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: 4px 10px;
...
`,I=`
  ...
  flex-shrink: 0;
`;var H="ws-crm-claim-release-pill",ie=r=>{let e=Array.from(r.children).find(o=>{if(!(o instanceof HTMLElement))return!1;let a=o.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Oe=({contactId:r,mode:e})=>{let t=document.createElement("button");t.id=H,t.type="button",t.dataset.contactId=r,t.dataset.mode=e;let o=e==="claim"?"emerald":"amber",a=e==="claim"?c.emerald:c.amber;return t.style.cssText=ae(o),t.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,t.title=e==="claim"?"Take ownership of this lead":"Return this lead to the pool",t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-1px)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",()=>{p.info(`Claim/Release pill clicked for contact ${r}: ${e}`);let s=t.lastElementChild,i=s?.textContent??"";s&&(s.textContent="Working\u2026"),t.style.opacity="0.7",t.disabled=!0,setTimeout(()=>{s&&(s.textContent=i),t.style.opacity="1",t.disabled=!1},700)}),t},Be=async(r,e=1e4)=>{let t=Date.now();for(;Date.now()-t<e;){let o=ie(r);if(o)return o;await new Promise(a=>setTimeout(a,150))}return null},se=(r,e)=>{let t=Oe({contactId:e,mode:"claim"}),o=document.createElement("span");o.id=`${H}-wrap`,o.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,o.appendChild(t),r.appendChild(o)},Ge=async()=>{let r=F();if(!r){document.getElementById(H)?.remove();return}let e=document.getElementById(H);if(!(e&&e.dataset.contactId===r.contactId)){e&&e.remove();try{let t=await L("#record-details-lhs",{timeoutMs:1e4}),o=await Be(t),a=F();if(!a||a.contactId!==r.contactId)return;if(!o){p.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}se(o,r.contactId),p.debug(`Mounted Claim/Release pill next to name for ${r.contactId}`);let s=null;s=new MutationObserver(async()=>{if(!(F()?.contactId===r.contactId)){s?.disconnect();return}if(document.getElementById(H))return;let n=ie(t);n&&(se(n,r.contactId),p.debug("Re-mounted Claim/Release pill after React wipe"))}),s.observe(t,{childList:!0,subtree:!0})}catch(t){p.warn("Could not mount Claim/Release pill:",t)}}},le=()=>{Ge()};var Ve="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set";var Ue="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",$e=()=>{let r=window.WS_CRM_USER_ID;if(r&&/^[A-Za-z0-9]{15,30}$/.test(r))return r;let e=window,t=[e.appState?.user?.id,e.user?.id,e.currentUser?.id,e.LCUser?.id,e.HL?.user?.id];for(let o of t)if(typeof o=="string"&&/^[A-Za-z0-9]{15,30}$/.test(o))return o;try{let o=localStorage.getItem("user");if(o){let a=JSON.parse(o),s=a.id??a._id;if(s&&/^[A-Za-z0-9]{15,30}$/.test(s))return s}for(let a of["tokenUser","currentUser","ghl_user"]){let s=localStorage.getItem(a);if(s)try{let i=JSON.parse(s),n=i.id??i._id;if(n&&/^[A-Za-z0-9]{15,30}$/.test(n))return n}catch{}}}catch{}return null},qe=async({contactId:r,locationId:e,status:t,userId:o,deadReason:a})=>{let s={call_status:t,triggered_by_user_id:o??""};a&&(s.dead_reason=a);let n=await fetch(Ve,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:r,location:{id:e},customData:s}),credentials:"omit"});if(!n.ok)throw new Error(`Webhook returned ${n.status} ${n.statusText}`)},ze=async({contactId:r,tenantId:e})=>{let t=await fetch(Ue,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,contact_id:r}),credentials:"omit"});if(!t.ok)throw new Error(`Sanity check returned ${t.status} ${t.statusText}`);return await t.json()};var U="ws-crm-contact-actions-bar",We="ws-crm-call-status-dropdown",de="ws-crm-offer-made-modal",je=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],Ke=[".central-panel","[class*='central-panel']"],Ye=async()=>{for(let r of Ke)try{let e=await L(r,{timeoutMs:3e3});if(e)return e}catch{}try{return(await L("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},Xe=r=>{let e=document.createElement("div");e.id=We,e.style.cssText="position: relative; flex-shrink: 0;";let t=document.createElement("button");t.type="button",t.style.cssText=I,t.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `;let o=document.createElement("div");return o.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,je.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
    ...
    `,s.innerHTML=`
      <span style="
        ...
        height: 8px;
        border-radius: 50%;
        background: ${Z[a.color]};
        flex-shrink: 0;
      "></span>
      <span>${a.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=c.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{p.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${r}`);let i=t.firstElementChild,n=t.children[1],l=t.children[2];i&&(i.style.background=Z[a.color]),n&&(n.textContent=a.label),o.style.display="none";let h=F()?.locationId,m=$e();if(!h){p.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=c.amber);return}if(a.color==="green"&&!m){p.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=c.amber);return}let d=l?.textContent??"\u25BE",y=async u=>{l&&(l.textContent="\u2026"),t.style.opacity="0.75";try{await qe({contactId:r,locationId:h,status:a.label,userId:m,deadReason:u}),l&&(l.textContent="\u2713"),setTimeout(()=>{l&&(l.textContent=d),t.style.opacity="1"},900)}catch(v){p.warn("Call Status webhook failed:",v),l&&(l.textContent="\u2717"),n&&(n.style.color=c.amber),t.style.opacity="1",setTimeout(()=>{l&&(l.textContent=d),n&&(n.style.color=c.bone)},1800)}};if(a.value==="dead-deal"){let u=n?.textContent??"";Ze(v=>{y(v)},()=>{n&&(n.textContent=u)});return}if(a.value==="appointment-booked"){let u=n?.textContent??"";try{(await ze({contactId:r,tenantId:h})).ok?await y():Qe(()=>{y()},()=>{n&&(n.textContent=u)})}catch(v){p.warn("Appointment sanity check failed; firing anyway:",v),await y()}return}await y()}),o.appendChild(s)}),t.addEventListener("click",a=>{a.stopPropagation(),o.style.display=o.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{e.contains(a.target)||(o.style.display="none")}),e.appendChild(t),e.appendChild(o),e},Je=r=>{document.getElementById(de)?.remove();let e=document.createElement("div");e.id=de,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `;let o=new Date,a=new Date(o.getTime()+5*24*60*60*1e3).toISOString().slice(0,10);t.innerHTML=`
    ...
    <div style="
      ...
      margin-bottom: 16px;
    ">
      <span style="color: ${c.ash}; font-family: ${w.mono}; font-size: 14px;">$</span>
  ...
  `,t.appendChild(o),document.body.appendChild(t);let a=()=>t.remove(),s=()=>{a(),e()};t.addEventListener("click",n=>{n.target===t&&s()}),o.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",s),o.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{a(),r()});let i=n=>{n.key==="Escape"&&(document.removeEventListener("keydown",i),s())};document.addEventListener("keydown",i)},he=r=>{let e=document.createElement("div");e.id=U,e.dataset.contactId=r,e.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    ...
    flex-shrink: 0;
  `,e.appendChild(Xe(r));let t=document.createElement("button");return t.type="button",t.style.cssText=D,t.textContent="Offer Made",t.addEventListener("click",()=>Je(r)),e.appendChild(t),e},et=async()=>{let r=F();if(!r){document.getElementById(U)?.remove();return}let e=document.getElementById(U);if(e&&e.dataset.contactId===r.contactId)return;e&&e.remove();let t=await Ye();if(!t){p.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let o=F();if(!o||o.contactId!==r.contactId)return;let a=he(r.contactId);a.dataset.mountTarget=t.className||"central-panel",t.insertBefore(a,t.firstChild),p.debug(`Mounted contact actions bar for ${r.contactId}`);let s=null;s=new MutationObserver(()=>{let i=document.getElementById(U);if(!(F()?.contactId===r.contactId)){s?.disconnect();return}if(!i){let l=he(r.contactId);l.dataset.mountTarget=a.dataset.mountTarget??"",t.insertBefore(l,t.firstChild),p.debug(`Re-mounted actions bar after React wipe for ${r.contactId}`)}}),s.observe(t,{childList:!0})},pe=()=>{et()};var $="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",tt=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),N="data-ws-crm-favicon",Q=r=>r.hasAttribute(N),me=r=>{if(r.tagName!=="LINK")return!1;let e=(r.getAttribute("rel")??"").toLowerCase();return tt.has(e)},q=()=>{document.head.querySelectorAll("link[rel]").forEach(o=>{me(o)&&!Q(o)&&o.remove()});let e=document.head.querySelector(`link[${N}="1"]`);e?e.href!==$&&(e.href=$):(e=document.createElement("link"),e.setAttribute(N,"1"),e.rel="icon",e.type="image/png",e.href=$,document.head.appendChild(e),p.debug("Favicon installed"));let t=document.head.querySelector(`link[${N}="apple"]`);t||(t=document.createElement("link"),t.setAttribute(N,"apple"),t.rel="apple-touch-icon",t.href=$,document.head.appendChild(t))},_e=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)q();else{let o=window.setInterval(()=>{document.head&&(window.clearInterval(o),q())},50)}new MutationObserver(o=>{let a=!1;for(let s of o)s.addedNodes.forEach(i=>{i instanceof HTMLLinkElement&&me(i)&&!Q(i)&&(a=!0)}),s.removedNodes.forEach(i=>{i instanceof HTMLLinkElement&&Q(i)&&(a=!0)});a&&q()}).observe(document.head,{childList:!0,subtree:!1});let e=0,t=window.setInterval(()=>{e+=1,q(),e>=5&&window.clearInterval(t)},1e3)};var Y="reos-theme-stylesheet",Ce="reos-theme-fonts",ot=()=>{let r=document.getElementById(Ce);r&&r.remove()},rt=`
/* ============================================
...
/* v0.10.25: .hr-button--primary (without -type) \u2014 the Funnels page
   "New funnel" button uses .hr-button--default-type combined with
   .hr-button--primary as an emphasis modifier (NOT .hr-button--primary-type).
   Same canon: emerald bg + obsidian text + emerald border. */
...
/* v0.10.32: .default-avatar is GHL's "no image yet" placeholder
   ...
   circle, and leave the parent bg alone (the mint emerald-glow
   canon stays). */
...
`,nt=[{selector:".hr-input__input-el, .hr-input__textarea-el, [class*='hr-input'] input, [class*='hr-input'] textarea",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".hr-data-table-wrapper, .hr-data-table-wrapper-responsive, .hr-data-table-wrapper-header, .hr-data-table-wrapper-footer, .hr-data-table, .hr-data-table__body, .hr-table-freezer-wrapper, [class*='hr-data-table-wrapper'], [class*='hr-table-freezer']",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".mb-2\\.5.bg-white, div.bg-white, .bg-white.px-3, .mx-auto > .bg-white",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-data-table__header, .hr-data-table__cell-header, thead.hr-data-table__header",styles:{"background-color":"#1A1F28",color:"#EDEEF0"}},{selector:".hr-wrapper-container, .platform-ui__highrise, .full-screen.platform-ui__highrise, [class*='platform-ui__highrise'], .hl_topbar-tabs, .hl_wrapper",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-collapse-item__header, .hr-collapse-item__header-main, .hr-collapse-item__title",styles:{color:"#EDEEF0","background-color":"#12161D"}},{selector:".hr-collapse-item__content, .hr-collapse-item__content-inner, .hr-collapse-item__content-wrap",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-tabs, .hr-tabs-nav, .hr-tabs-nav-scroll-content, .hr-tabs-content, .hr-tabs-pane, .hr-tabs-rail, .hr-tabs-tab-wrapper",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".hr-tabs-capsule",styles:{"background-color":"transparent",background:"transparent",border:"none"}},{selector:".hr-avatar:not(.default-avatar), .hr-avatar:not(.default-avatar) .hr-avatar__text, .hr-avatar:not(.default-avatar) .hr-avatar__text p, .hr-avatar:not(.default-avatar) p, .hr-avatar:not(.default-avatar) span, .hr-avatar:not(.default-avatar) div, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar), [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar) span, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar) p, [class*='avatar']:not([class*='dropdown']):not([class*='option-avatar']):not([class*='option']):not(.default-avatar) div",styles:{color:"#0A0D12","-webkit-text-fill-color":"#0A0D12","font-weight":"600"}},{selector:".default-avatar, .default-avatar *",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0",stroke:"#EDEEF0","font-weight":"600"}},{selector:".hr-button--tertiary",styles:{"background-color":"#1A1F28",color:"#0FB57E"}},{selector:".hr-text:not([role='button']):not(.cursor-pointer)",styles:{color:"#EDEEF0"}},{selector:"[class*='field-value'], [class*='field-display'], [class*='empty-value'], [class*='field-empty'], [class*='value-display'], [class*='display-value']",styles:{color:"#9098A3","-webkit-text-fill-color":"#9098A3"}},{selector:".hr-select__value, .hr-select__display, .hr-select-trigger, .hr-select-trigger__value, .hr-base-select__value, .hr-base-select__display, [class*='select__value'], [class*='select__display'], [class*='select-trigger']",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".hr-select, .hr-select__input, .hr-select__inner, .hr-select-trigger, .hr-base-select, .hr-base-select__inner, .el-select, .el-select__wrapper, .el-input__wrapper, .el-input__inner, [role='combobox'], [aria-haspopup='listbox']",styles:{"background-color":"#1A1F28","border-color":"#252C36"}},{selector:"input[type='date'], input[type='datetime-local'], input[type='time'], input[type='month'], input[type='week'], .hr-date-picker, .hr-date-input, .hr-datepicker, .hr-time-picker, [class*='date-picker'], [class*='datepicker'], [class*='date-input'], [class*='time-picker'], [class*='time-input']",styles:{"background-color":"#1A1F28","border-color":"#252C36",color:"#EDEEF0"}},{selector:"[class*='F7F9FD'], [class*='F7F9FB'], [class*='F9FAFB'], [class*='eff4ff'], [class*='EFF4FF'], [class*='ECEEF2'], [class*='eceef2']",styles:{"background-color":"#12161D"}},{selector:"[style*='background-color: white'], [style*='background-color:white'], [style*='background-color: #fff'], [style*='background-color:#fff'], [style*='background-color: #FFF'], [style*='background-color:#FFF'], [style*='background-color: #ffffff'], [style*='background-color:#ffffff'], [style*='background-color: #FFFFFF'], [style*='background-color:#FFFFFF'], [style*='background-color: rgb(255, 255, 255)'], [style*='background-color:rgb(255, 255, 255)'], [style*='background-color: rgb(255,255,255)'], [style*='background-color:rgb(255,255,255)']",styles:{"background-color":"#12161D"}},{selector:".chart-container.hl-card, [class*='chart-container'][class*='hl-card'], .dashboard-widget-type-custom .hl-card, .dashboard-widget-module-custom .hl-card",styles:{"background-color":"#12161D"}},{selector:".bg-gray-50, .bg-white, .bg-gray-100",styles:{"background-color":"#12161D",color:"#EDEEF0"}},{selector:".n-base-select-menu, .n-popselect-menu, .n-popover, .n-dropdown, .n-dropdown-menu, .hr-base-select-menu, .hr-popselect-menu, .hr-popover, .hr-dropdown, .hr-dropdown-menu",styles:{"background-color":"#12161D",color:"#EDEEF0","border-color":"#252C36","--n-option-color":"transparent","--n-option-color-hover":"#1A1F28","--n-option-color-active":"rgba(15, 181, 126, 0.12)","--n-option-color-active-hover":"rgba(15, 181, 126, 0.12)","--n-option-text-color":"#EDEEF0","--n-option-text-color-hover":"#EDEEF0","--n-option-text-color-active":"#0FB57E","--n-option-text-color-disabled":"#9098A3","--n-color":"transparent","--n-color-hover":"#1A1F28","--n-color-active":"rgba(15, 181, 126, 0.12)","--n-text-color":"#EDEEF0","--n-text-color-hover":"#EDEEF0","--n-text-color-active":"#0FB57E","--n-text-color-disabled":"#9098A3"}},{selector:".n-dropdown-option--disabled, .n-dropdown-option--disabled .n-dropdown-option-body, .n-dropdown-option--disabled .n-dropdown-option-body__label, .n-dropdown-option--disabled .n-dropdown-option-body__prefix, .n-dropdown-option--disabled .n-dropdown-option-body__suffix, .n-dropdown-option[aria-disabled='true'], .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__label, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__prefix, .n-dropdown-option[aria-disabled='true'] .n-dropdown-option-body__suffix, .n-base-select-option--disabled, .n-base-select-option--disabled *, .n-base-select-option[aria-disabled='true'], .n-base-select-option[aria-disabled='true'] *, .hr-dropdown-option--disabled, .hr-dropdown-option--disabled *, .hr-dropdown-option[aria-disabled='true'], .hr-dropdown-option[aria-disabled='true'] *, .hr-base-select-option--disabled, .hr-base-select-option--disabled *, .hr-base-select-option[aria-disabled='true'], .hr-base-select-option[aria-disabled='true'] *",styles:{"background-color":"transparent",color:"#9098A3","-webkit-text-fill-color":"#9098A3"}},{selector:".hl-card-header [role='heading'], .hl-card-header p[role='heading'], [class*='hl-card-header'] [role='heading']",styles:{color:"#EDEEF0","-webkit-text-fill-color":"#EDEEF0"}},{selector:".n-button:not(.n-button--primary-type):not(.n-button--error-type):not(.n-button--warning-type):not(.n-button--success-type):not(.n-button--info-type)",styles:{"background-color":"#1A1F28"}}],at=r=>{if(!r||r[0]!=="#")return r;let e=r.slice(1);if(e.length===3&&(e=e[0]+e[0]+e[1]+e[1]+e[2]+e[2]),e.length!==6&&e.length!==8)return r;let t=parseInt(e.slice(0,2),16),o=parseInt(e.slice(2,4),16),a=parseInt(e.slice(4,6),16);if(isNaN(t)||isNaN(o)||isNaN(a))return r;if(e.length===8){let s=parseInt(e.slice(6,8),16)/255;return`rgba(${t}, ${o}, ${a}, ${s})`}return`rgb(${t}, ${o}, ${a})`},st=new Set(["color","background-color","background","border-color","border-top-color","border-right-color","border-bottom-color","border-left-color","fill","stroke","-webkit-text-fill-color","caret-color","outline-color"]),it=["#inventory-table","#inventory","[id^='collections-']","[id^='products-']","[id^='reviews-']"],M=()=>{document.querySelectorAll(it.join(", ")).forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let t=e.querySelectorAll("*");[e,...Array.from(t)].forEach(o=>{if(o.id?.startsWith("ws-crm-")||o.tagName==="INPUT"||o.tagName==="TEXTAREA"||o.tagName==="SELECT"||o.tagName==="SCRIPT"||o.tagName==="STYLE"||o.tagName==="SVG"||o.tagName==="PATH")return;let s=window.getComputedStyle(o).backgroundColor;if(!s||s==="rgba(0, 0, 0, 0)"||s==="transparent")return;let i=s.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!i)return;let n=+i[1],l=+i[2],b=+i[3];n<230||l<230||b<230||(o.style.setProperty("background-color","#12161D","important"),o.style.setProperty("color","#EDEEF0","important"))})})},ve=()=>{document.querySelectorAll(".hr-header-lite-left, .hr-header-lite-right, .hr-header-lite-content, .hr-header-lite-icon-item, [class*='hr-header-lite']:not([class*='container']):not([class*='wrapper']), .hr-breadcrumb, .hr-breadcrumb-item, .hr-breadcrumb-item__link, .hr-breadcrumb-item__separator").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let t=e.getAttribute("style")||"";(t.includes("background-color: rgb(26, 31, 40)")||t.includes("background-color: #1A1F28")||t.includes("background-color: #1a1f28"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("border-color"),e.style.removeProperty("border-style"),e.style.removeProperty("border-width"),e.style.removeProperty("border-radius"))})},P=()=>{for(let r of nt){let e;try{e=document.querySelectorAll(r.selector)}catch{continue}e.forEach(t=>{for(let[o,a]of Object.entries(r.styles)){let s=t.style.getPropertyPriority(o),i=t.style.getPropertyValue(o),n=st.has(o)?at(a):a;s==="important"&&i===n||t.style.setProperty(o,a,"important")}})}},te="#1A1F28",lt=26,dt=31,ct=40,ue=new Set,ye=new Set,z=()=>{document.body.querySelectorAll("*").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.children.length>0||(e.textContent||"").trim()!=="--")return;let o=e.getBoundingClientRect();if(o.width===0||o.height===0||o.width>800)return;e.style.setProperty("display","none","important");let a=e.className?.toString()||"<no class>";!ye.has(a)&&a!=="<no class>"&&(ye.add(a),p.debug(`Hid empty "--" leaf: "${a}"`))})},bt=["M3 6","M5 6","M2 6","M6 6","M9 3h6","M16 6v","M19 6v","M5 6V","M16.5 4","48.816","2.991","14.74 9","9.26 9","2.244","M19 4h-","M19 7l-"],ht=["trash","Trash","TRASH","delete","Delete","DELETE"],pt=()=>{document.querySelectorAll("button.hr-button, button.n-button, button.icon-only, .hr-button.icon-only, .n-button.icon-only").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.getAttribute("data-reos-trash")==="true")return;let t=e.querySelector("svg");if(!t)return;let o=!1,a=(t.getAttribute("class")||"")+" "+(t.getAttribute("data-icon")||"");if(ht.some(s=>a.includes(s))&&(o=!0),o||t.querySelectorAll("path").forEach(i=>{if(o)return;let n=i.getAttribute("d")||"";bt.some(l=>n.includes(l))&&(o=!0)}),!o){let s=(e.id||"").toLowerCase(),i=(e.getAttribute("aria-label")||"").toLowerCase(),n=(e.getAttribute("title")||"").toLowerCase();(s.includes("delete")||s.includes("remove")||s.includes("trash")||i.includes("delete")||i.includes("remove")||i.includes("trash")||n.includes("delete")||n.includes("remove")||n.includes("trash"))&&(o=!0)}o&&e.setAttribute("data-reos-trash","true")})},mt=new Set(["Delete Reminder","Delete","Remove","Trash","Edit","Copy","Duplicate","Add another reminder","Add","Close","Dismiss"]),_t=()=>{document.querySelectorAll(".hr-tooltip, .hr-popover-shared, [role='tooltip']").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let t=(e.textContent||"").trim();t.length===0||t.length>40||mt.has(t)&&e.style.display!=="none"&&(e.style.setProperty("display","none","important"),e.style.setProperty("visibility","hidden","important"),e.style.setProperty("opacity","0","important"),e.style.setProperty("pointer-events","none","important"))})},ge=new Set,vt=()=>{document.body.querySelectorAll("*").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']"))return;let t=(e.className?.toString()||"").toLowerCase();if(t.includes("ai-button")||t.includes("ask-ai")||t.includes("bg-clip-text")||t.includes("text-transparent")||e.closest("[class*='ai-button'], [class*='ask-ai'], [class*='bg-clip-text'], [class*='text-transparent']"))return;let o=e.tagName.toLowerCase();if(o==="svg"||o==="path"||o==="circle"||o==="rect"||o==="polygon"||o==="polyline"||o==="line"||o==="g")return;let a=!1;for(let m of Array.from(e.childNodes))if(m.nodeType===Node.TEXT_NODE&&(m.textContent||"").trim()){a=!0;break}if(!a)return;let s=Array.from(e.children);if(s.length>0&&s.every(m=>{let d=m.tagName.toLowerCase();return d==="svg"||d==="img"})&&!a)return;let l=window.getComputedStyle(e).color;if(!(l==="rgb(255, 255, 255)"||l==="rgba(255, 255, 255, 1)"||l==="#ffffff"||l==="white"))return;e.style.setProperty("color","#EDEEF0","important"),e.style.setProperty("-webkit-text-fill-color","#EDEEF0","important");let h=e.className?.toString()||"<no class>";!ge.has(h)&&h!=="<no class>"&&(ge.add(h),p.debug(`Remapped pure-white text -> bone: "${h}"`))})},fe=new Set,ut=()=>{let r=document.querySelectorAll(".hl_header, header.hl_header, header.app-header, .hl_header--controls, .top-bar, .topbar"),e="rgb(18, 22, 29)";document.querySelectorAll(".hr-content-switcher svg, .hr-content-switcher svg *, .hr-content-switcher i, .hr-radio-button svg, .hr-radio-button svg *, .hr-radio-button i, [class*='hr-content-switcher'] svg, [class*='hr-content-switcher'] svg *, [class*='hr-content-switcher'] i").forEach(o=>{if(o.closest(".hl_header"))return;let a=o.getAttribute("style")||"";(a.includes(e)||a.includes("#12161D")||a.includes("#12161d"))&&(o.style.removeProperty("fill"),o.style.removeProperty("stroke"),o.style.removeProperty("color"))}),r.forEach(o=>{o.querySelectorAll("button, a, [role='button'], [class*='rounded-full'], [class*='rounded-md'], [class*='rounded-xl'], [class*='pill'], span[class*='rounded'], div[class*='rounded']").forEach(s=>{if(s.closest("[id^='ws-crm-']"))return;let i=(s.className?.toString()||"").toLowerCase();if(i.includes("avatar"))return;let n=window.getComputedStyle(s),l=n.backgroundColor,b=n.backgroundImage||"",h=!1;if(b!=="none"&&b.includes("gradient"))h=!0;else if(l&&l!=="rgba(0, 0, 0, 0)"&&l!=="transparent"){let x=l.match(/rgba?\(\d+,\s*\d+,\s*\d+,\s*([\d.]+)\)/);if(x&&parseFloat(x[1])<.6)return;let g=l.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(g){let S=parseInt(g[1],10),A=parseInt(g[2],10),X=parseInt(g[3],10);(S>=60||A>=60||X>=60)&&(S>230&&A>230&&X>230||(S>=80||A>=80||X>=80)&&(h=!0))}}if(!h)return;let d=(s.textContent||"").trim(),y=i,u=Array.from(s.querySelectorAll("*")).map(x=>(x.className?.toString()||"").toLowerCase()).join(" "),v=d.toLowerCase()==="ask ai"||y.includes("ai-button")||y.includes("ask-ai")||y.includes("askai"),f=y.includes("bg-clip-text")||y.includes("text-transparent")||u.includes("bg-clip-text")||u.includes("text-transparent"),T=Array.from(s.querySelectorAll("*")).some(x=>{let g=window.getComputedStyle(x),S=g.getPropertyValue("-webkit-background-clip")||"",A=g.getPropertyValue("background-clip")||"";return S.includes("text")||A.includes("text")});if(v||f||T){let x=["color","-webkit-text-fill-color","fill","stroke","background","background-color","background-image","background-clip","-webkit-background-clip","text-shadow"],g=S=>{x.forEach(A=>S.style.removeProperty(A))};g(s),s.querySelectorAll("*").forEach(g);return}let _="#12161D";s.style.setProperty("color",_,"important"),s.querySelectorAll("*").forEach(x=>{let g=x.tagName.toLowerCase();g==="svg"||g==="path"||g==="circle"||g==="rect"||g==="polygon"||g==="polyline"||g==="line"||g==="g"?(x.style.setProperty("fill",_,"important"),x.style.setProperty("stroke",_,"important"),x.style.setProperty("color",_,"important")):(x.style.setProperty("color",_,"important"),x.style.setProperty("-webkit-text-fill-color",_,"important"))});let k=s.className?.toString()||"<no class>";!fe.has(k)&&k!=="<no class>"&&(fe.add(k),p.debug(`Darkened SVG icons on colored button: "${k}"`))})})},we=new Set,yt=()=>{document.querySelectorAll("[class*='avatar']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;if((e.className?.toString()||"").toLowerCase().includes("default-avatar")){e.querySelectorAll(".n-avatar__text, [class*='avatar__text']").forEach(l=>{(l.getAttribute("style")||"").includes("translateY(11px)")&&l.style.removeProperty("transform")});return}let o=e.getBoundingClientRect();if(o.width===0||o.height===0||o.width<20||o.width>64||o.height<20||o.height>64)return;let a=o.top+o.height/2;(e.parentElement||e).querySelectorAll("svg, img").forEach(l=>{let b=l;if(b.closest("[id^='ws-crm-']"))return;let h=l.getBoundingClientRect();if(h.width===0||h.width>28||h.height>28)return;let m=h.left<o.right&&h.right>o.left,d=h.top<o.bottom&&h.bottom>o.top;if(!(m&&d)||h.top+h.height/2<a)return;let u=h.width*h.height/(o.width*o.height);if(u>.55)return;let v=b,f=b.parentElement,T=0;for(;f&&f!==e&&T<3;){let k=f.getBoundingClientRect();if(k.width>0&&k.width<=30&&k.height<=30)v=f;else break;f=f.parentElement,T++}v.style.setProperty("transform","translateY(11px)","important");let _=(v.className?.toString()||"")+" "+v.tagName;we.has(_)||(we.add(_),p.debug(`Dropped activity badge 11px (${h.width.toFixed(0)}x${h.height.toFixed(0)} icon, ratio ${u.toFixed(2)}): "${_}"`))})})},gt="brightness(0) saturate(100%) invert(54%) sepia(78%) saturate(442%) hue-rotate(122deg) brightness(96%) contrast(89%)",ft=()=>{document.querySelectorAll("aside.default-bg-color a, [class*='sidebar'] a").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let t=(e.className?.toString()||"").toLowerCase();t.includes("active")||t.includes("router-link-active")?(e.style.setProperty("color","#0FB57E","important"),e.style.setProperty("-webkit-text-fill-color","#0FB57E","important"),e.style.setProperty("opacity","1","important"),e.querySelectorAll("span, p, div").forEach(a=>{a.style.setProperty("color","#0FB57E","important"),a.style.setProperty("-webkit-text-fill-color","#0FB57E","important")}),e.querySelectorAll("img").forEach(a=>{a.style.setProperty("filter",gt,"important")}),e.querySelectorAll("svg").forEach(a=>{a.style.setProperty("color","#0FB57E","important"),a.style.setProperty("stroke","#0FB57E","important")})):((e.style.color==="rgb(18, 22, 29)"||e.style.color==="#12161D"||e.style.color==="#0FB57E"||e.style.color==="rgb(15, 181, 126)")&&(e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color")),e.querySelectorAll("span, p, div").forEach(a=>{let s=a.style.color;(s==="rgb(18, 22, 29)"||s==="#12161D"||s==="#0FB57E"||s==="rgb(15, 181, 126)")&&(a.style.removeProperty("color"),a.style.removeProperty("-webkit-text-fill-color"))}),e.querySelectorAll("img").forEach(a=>{a.style.filter&&a.style.filter.includes("hue-rotate(122")&&a.style.removeProperty("filter")}))})},xe=new Set,wt=()=>{document.querySelectorAll(".n-dialog, [class*='n-dialog'], [class*='dialog'], [role='dialog']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;e.querySelectorAll("div, span, i").forEach(o=>{if(o.closest("[id^='ws-crm-']"))return;let a=o.getBoundingClientRect();if(a.width===0||a.height===0||a.width>56||a.height>56||!o.querySelector("svg"))return;let i=window.getComputedStyle(o).backgroundColor.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!i)return;let n=parseInt(i[1],10),l=parseInt(i[2],10),b=parseInt(i[3],10);if(!(n>200&&l>200&&b>200))return;o.style.setProperty("background-color","transparent","important"),o.style.setProperty("background","transparent","important"),o.style.setProperty("box-shadow","none","important"),o.style.setProperty("border","none","important");let h=o.className?.toString()||"<no class>";!xe.has(h)&&h!=="<no class>"&&(xe.add(h),p.debug(`Stripped dialog icon circle (${a.width.toFixed(0)}x${a.height.toFixed(0)}, rgb(${n},${l},${b})): "${h}"`))})})},ke=new Set,xt=()=>{document.querySelectorAll("[class*='ai-button'], [class*='ask-ai'], [class*='askai']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let t=e.parentElement,o=0;for(;t&&o<3&&!t.closest("[id^='ws-crm-']");){let a=(t.className?.toString()||"").toLowerCase();if(!(a.includes("ai-button")||a.includes("ask-ai")||a.includes("askai"))){let i=t.getBoundingClientRect();if(i.width>0&&i.height>0&&i.width<=400&&i.height<=80){t.style.setProperty("background-color","#12161D","important"),t.style.setProperty("background-image","none","important");let n=t.className?.toString()||"<no class>";!ke.has(n)&&n!=="<no class>"&&(ke.add(n),p.debug(`Painted Ask AI wrapper graphite: "${n}"`))}}t=t.parentElement,o++}})},Ee=new Set,kt=()=>{let r=Array.from(document.querySelectorAll("#contact-conversation-panel, [class*='conversation-panel'], [class*='message-list'], [class*='activity-log'], .hl_header, header.hl_header, [class*='topbar'], [class*='top-bar'], header.app-header, .modal, .modal-content, [class*='modal'], [class*='dialog'], [role='dialog'], [class*='popover'], [class*='popup'], [class*='dropdown-menu'], [class*='select-menu'], [class*='popselect'], [class*='base-select'], .n-base-select-menu, .n-popselect, .n-popover")),e=document.querySelectorAll("div, section, aside"),t=[];e.forEach(a=>{if(a.id?.startsWith("ws-crm-")||a.closest("[id^='ws-crm-']"))return;let s=window.getComputedStyle(a);if(s.position!=="fixed"&&s.position!=="absolute")return;let i=parseInt(s.zIndex,10);if(!isFinite(i)||i<50)return;let n=a.getBoundingClientRect();n.width<240||n.height<120||n.width>1500||n.height>900||t.push(a)}),[...r,...t].forEach(a=>{a.querySelectorAll("*").forEach(i=>{if(i.id?.startsWith("ws-crm-")||i.closest("[id^='ws-crm-']"))return;let n=(i.className?.toString()||"").toLowerCase();if(n.includes("avatar")||n.includes("notification-dot")||n.includes("indicator"))return;if(n.includes("sidebar-option-button")||i.closest(".sidebar-option-button")||i.closest("nav[class*='w-13']")){let f=i.getAttribute("style")||"";(f.includes("background-color")||f.includes("color"))&&(i.style.removeProperty("background-color"),i.style.removeProperty("color"));return}if(n.includes("tabulator")||n.includes("tabulator-page")||i.closest("[class*='tabulator']")){(i.getAttribute("style")||"").includes("background-color")&&i.style.removeProperty("background-color");return}if(n.includes("hr-card")||i.closest(".hr-card")||i.id&&i.id.startsWith("opportunity-")){let f=i.getAttribute("style")||"";(f.includes("background-color")||f.includes("color"))&&(i.style.removeProperty("background-color"),i.style.removeProperty("color"));return}if(n.includes("ai-button")||n.includes("ask-ai")||n.includes("bg-clip-text")||n.includes("text-transparent")||i.closest("[class*='ai-button'], [class*='ask-ai'], [class*='bg-clip-text'], [class*='text-transparent']"))return;let l=window.getComputedStyle(i).backgroundColor;if(!l||l==="rgba(0, 0, 0, 0)"||l==="transparent")return;let b=l.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!b)return;let h=parseInt(b[1],10),m=parseInt(b[2],10),d=parseInt(b[3],10);if(!(h>200&&m>200&&d>200))return;let u=i.getBoundingClientRect();if(u.width<24||u.height<16||u.width>1400)return;i.style.setProperty("background-color","#12161D","important"),i.style.setProperty("color","#EDEEF0","important");let v=i.className?.toString()||"<no class>";!Ee.has(v)&&v!=="<no class>"&&(Ee.add(v),p.debug(`Coerced light bg (${u.width.toFixed(0)}x${u.height.toFixed(0)}, rgb(${h},${m},${d})): "${v}"`))})})},Te=new Set,Et=()=>{let r="input, select, textarea, [class*='hr-input'], [class*='hr-select'], [class*='hr-base-select'], [class*='hr-date-picker'], [class*='hr-datepicker']";document.body.querySelectorAll("*").forEach(t=>{if(t.id?.startsWith("ws-crm-")||t.closest("[id^='ws-crm-']")||t.children.length>0)return;let o=(t.textContent||"").trim();if(o.length===0||o.length>80||o==="--"||t.closest(r)||t.closest("[class*='avatar'], [class*='hr-avatar']")||t.closest("[class*='collapse-item__header'], [class*='hr-collapse-item__header'], [class*='collapse-header'], [class*='folder-header']")||t.closest("[class*='hr-tabs-tab'], [class*='tab-label'], [class*='tabs-nav'], [role='tab']"))return;if(t.matches("[role='heading']")||t.closest("[role='heading']")||t.matches("h1, h2, h3")||t.closest(".hl-card-header, [class*='hl-card-header'], [class*='card-header']")){t.matches("h1, h2, h3")&&(t.getAttribute("style")||"").includes("color")&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"));return}if(t.closest(".hr-alert, .hr-alert-body, [class*='hr-alert']")){let _=t.getAttribute("style")||"";(_.includes("color")||_.includes("background"))&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"),t.style.removeProperty("background-color"));return}if(t.closest("#SettingTexasSmsBlock")||t.matches("p.ui-text-sm-medium")&&t.parentElement?.classList?.contains("flex")&&t.parentElement?.classList?.contains("items-center")&&t.parentElement?.children?.length===1){(t.getAttribute("style")||"").includes("color")&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"));return}if(t.closest(".hr-button, .n-button")||t.matches(".hr-button__content, .n-button__content")||t.closest(".hr-button__content, .n-button__content")){(t.getAttribute("style")||"").includes("color")&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"));return}if(t.closest(".bg-primary-50, [class*='bg-primary-50']")){(t.getAttribute("style")||"").includes("color")&&(t.style.removeProperty("color"),t.style.removeProperty("-webkit-text-fill-color"));return}let a=t,s=null;for(let _=0;_<4&&a;_++){let k=a.nextElementSibling;if(k&&(k.matches(r)||k.querySelector(r)!==null)){s=k;break}a=a.parentElement}if(!s)return;let i=t.getBoundingClientRect();if(i.width===0||i.height===0||i.width>400||i.height>60)return;t.style.setProperty("color","#9098A3","important"),t.style.setProperty("-webkit-text-fill-color","#9098A3","important");let n=s,l=n.getBoundingClientRect(),b=n.className?.toString()||"",h=b.includes("schedule-ui-form-weekly-schedule-checkbox")||n.closest(".schedule-ui-form-weekly-schedule-checkbox")!==null,m=b.includes("hl-form-item")||b.includes("n-form-item--top-labelled")||n.closest(".hl-form-item")!==null,d=".hr-input, .hr-input-number, .hr-base-select, .hr-select, .hr-date-picker, .hr-datepicker",y=(b.includes("hr-form-item-blank")||b.includes("hr-form-item")||n.matches(d))&&(n.matches(d)||n.querySelector(d)!==null);y&&(n.getAttribute("style")||"").includes("background-color")&&(n.style.removeProperty("background-color"),n.style.removeProperty("border-color"),n.style.removeProperty("border-style"),n.style.removeProperty("border-width"),n.style.removeProperty("border-radius"));let u=b.includes("hr-radio")||n.closest(".hr-radio, [class*='hr-radio']")!==null;u&&(n.getAttribute("style")||"").includes("background-color")&&(n.style.removeProperty("background-color"),n.style.removeProperty("border-color"),n.style.removeProperty("border-style"),n.style.removeProperty("border-width"),n.style.removeProperty("border-radius"));let v=b.includes("hr-breadcrumb")||n.closest(".hr-breadcrumb, [class*='hr-breadcrumb']")!==null;v&&(n.getAttribute("style")||"").includes("background-color")&&(n.style.removeProperty("background-color"),n.style.removeProperty("border-color"),n.style.removeProperty("border-style"),n.style.removeProperty("border-width"),n.style.removeProperty("border-radius"));let f=b.includes("hr-header-lite")||n.closest("[class*='hr-header-lite']")!==null;f&&(n.getAttribute("style")||"").includes("background-color")&&(n.style.removeProperty("background-color"),n.style.removeProperty("border-color"),n.style.removeProperty("border-style"),n.style.removeProperty("border-width"),n.style.removeProperty("border-radius")),!h&&!m&&!y&&!u&&!v&&!f&&l.width>0&&l.width<=800&&l.height>0&&l.height<=100&&(n.style.setProperty("background-color",te,"important"),n.style.setProperty("border-color","#252C36","important"),n.style.setProperty("border-style","solid","important"),n.style.setProperty("border-width","1px","important"),n.style.setProperty("border-radius","4px","important"));let T=t.className?.toString()||"<no class>";!Te.has(T)&&T!=="<no class>"&&(Te.add(T),p.debug(`Colored field label "${o.slice(0,30)}": "${T}"`))})},Se=new Set,Tt=()=>{document.querySelectorAll("[class*='avatar']").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let t=(e.className?.toString()||"").toLowerCase();if(t.includes("dropdown")||t.includes("option-avatar"))return;let o=e.getBoundingClientRect();if(o.width===0||o.height===0||o.height>80||e.querySelector("svg, img")!==null)return;let s=(e.textContent||"").trim();if(s.length===0||s.length>4)return;let i=Math.min(o.width,o.height),n=Math.max(16,Math.min(24,Math.round(i*.6)));e.style.setProperty("font-size",`${n}px`,"important"),e.style.setProperty("line-height","1","important"),e.style.setProperty("display","flex","important"),e.style.setProperty("align-items","center","important"),e.style.setProperty("justify-content","center","important"),e.style.setProperty("text-align","center","important"),e.querySelectorAll("*").forEach(b=>{b.tagName==="svg"||b.tagName==="SVG"||b.tagName!=="IMG"&&(b.style.setProperty("font-size",`${n}px`,"important"),b.style.setProperty("line-height","1","important"),b.style.setProperty("text-align","center","important"))});let l=e.className?.toString()||"<no class>";!Se.has(l)&&l!=="<no class>"&&(Se.add(l),p.debug(`Resized avatar text to ${n}px (circle ${i.toFixed(0)}px): "${l}"`))})},Fe=new Set,St=()=>{document.querySelectorAll("input:not([type='checkbox']):not([type='radio']):not([type='button']):not([type='submit']):not([type='reset']):not([type='hidden']):not([type='file']), textarea, select").forEach(e=>{if(e.closest("[id^='ws-crm-']"))return;let t=e.getBoundingClientRect();if(t.width===0||t.height===0)return;e.style.setProperty("background-color",te,"important"),e.style.setProperty("border-color","#252C36","important"),e.style.setProperty("color","#EDEEF0","important"),e.style.setProperty("-webkit-text-fill-color","#EDEEF0","important");let o=e.className?.toString()||"<no class>";!Fe.has(o)&&o!=="<no class>"&&(Fe.add(o),p.debug(`Forced input slate bg: "${o}"`))})},W=()=>{document.body.querySelectorAll("*").forEach(e=>{if(e.id?.startsWith("ws-crm-")||e.closest("[id^='ws-crm-']")||e.closest("[class*='collapse-item__header'], [class*='hr-collapse-item__header'], [class*='collapse-header'], [class*='folder-header']")||e.id==="chat-filter"||e.id==="phone-calls"||e.id==="archive-conversation"||e.id==="star-toggle"||e.id==="read-toggle"||e.id==="delete-conversation")return;if(e.closest(".hr-tooltip, .hr-popover-shared, .hr-popover, [class*='hr-tooltip'], [class*='hr-popover']")){let d=e.getAttribute("style")||"";(d.includes("background-color")||d.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}let t=e.className?.toString()||"";if(t.includes("bg-inherit")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(e.closest(".pipeline-ribbon")&&t.split(" ").includes("count")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.split(" ").includes("tabulator-page")){let d=e.getAttribute("style")||"";(d.includes("background-color")||d.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}if(["tabulator-cell","tabulator-row","tabulator-col","tabulator-header","tabulator-tableholder","tabulator-table"].some(d=>t.includes(d))){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("schedule-ui-form-weekly-schedule-checkbox")){let d=e.getAttribute("style")||"";(d.includes("background-color")||d.includes("border-color")||d.includes("border-style")||d.includes("border-width"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("border-color"),e.style.removeProperty("border-style"),e.style.removeProperty("border-width"));return}if(t.includes("n-button")&&t.includes("icon-only")&&!t.includes("quaternary")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("hr-button--tertiary-type")||t.includes("n-button--tertiary-type")){let d=e.getAttribute("style")||"";(d.includes("background-color")||d.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"),e.style.removeProperty("-webkit-text-fill-color"),e.style.removeProperty("border-color"),e.style.removeProperty("border-style"),e.style.removeProperty("border-width"),e.style.removeProperty("border-radius"));return}if(t.includes("hr-data-table-td")||t.includes("hr-data-table-tr")||t.includes("hr-data-table__body-cell")||t.includes("hr-data-table__cell")||t.includes("hr-data-table__header-cell")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("n-data-table-td--fixed-right")||t.includes("n-data-table-td--last-col")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(e.classList.contains("hr-tabs")||t.includes("hr-tabs-tab-wrapper")||t.includes("hr-tabs-nav")||t.includes("hr-tabs-nav-scroll-content")||t.includes("hr-tabs-content")||t.includes("hr-tabs-pane")||t.includes("hr-tabs-rail")||t.includes("hr-tabs-wrapper")||t.includes("hr-tabs-pane-wrapper")||t.includes("hr-tab-pane")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("hr-date-panel")||t.includes("n-date-panel")||e.closest(".hr-date-panel, .n-date-panel")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("hr-tag")&&t.includes("ui-default")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if((t.includes("bg-purple-50")||t.includes("bg-purple-100"))&&(t.includes("rounded-lg")||t.includes("rounded-md")||t.includes("rounded-full"))){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("n-alert")||e.closest(".n-alert")||t.includes("hl-warning")||t.includes("hl-error")||t.includes("hl-info")||t.includes("hl-success")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("hr-card")||e.closest(".hr-card")||e.id&&e.id.startsWith("opportunity-")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("bg-gray-50")||t.includes("bg-gray-100")||t.includes("bg-gray-200")||t.includes("bg-white")||t.includes("bg-blue-50")||t.includes("bg-blue-100")||t.includes("bg-sky-50")||t.includes("bg-indigo-50")||t.includes("bg-slate-50")||t.includes("bg-neutral-50")||t.includes("bg-zinc-50")||t.includes("bg-stone-50")||t.includes("F7F9FD")||t.includes("F9FAFB")||t.includes("ECEEF2")||t.includes("eceef2")||t.includes("EFF4FF")||t.includes("eff4ff")){(e.getAttribute("style")||"").includes("background-color")&&e.style.removeProperty("background-color");return}if(t.includes("hr-collapse-item")||t.includes("hr-collapse hr-accordion")||t.includes("hr-accordion"))return;if(t.includes("sidebar-option-button")||e.closest(".sidebar-option-button")||e.closest("nav[class*='w-13']")){let d=e.getAttribute("style")||"";(d.includes("background-color")||d.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}if(e.closest(".inbox-panel, .conversation-list-container, [class*='conversation-list-container']")){let d=e.getAttribute("style")||"";(d.includes("background-color")||d.includes("color"))&&(e.style.removeProperty("background-color"),e.style.removeProperty("color"));return}let a=window.getComputedStyle(e).backgroundColor;if(!a||a==="rgba(0, 0, 0, 0)"||a==="transparent")return;let s=a.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);if(!s)return;let i=parseInt(s[1],10),n=parseInt(s[2],10),l=parseInt(s[3],10);if(!(i<lt&&n<dt&&l<ct))return;let h=e.getBoundingClientRect();if(h.width===0||h.height===0||h.width>800||h.height>200)return;e.style.setProperty("background-color",te,"important");let m=e.className?.toString()||"<no class>";!ue.has(m)&&m!=="<no class>"&&(ue.add(m),p.debug(`Coerced dark bg to slate (${h.width.toFixed(0)}x${h.height.toFixed(0)}, rgb(${i},${n},${l})): "${m}"`))})},j=()=>{let r="#4B8BF5",e=["#155EEF","#155eef","#004EEB","#004eeb","#175CD3","#175cd3"],t="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";document.querySelectorAll("img[src^='data:image/svg+xml;base64,']:not([data-reos-recolored]), .toolbar_button img[src^='data:image/svg+xml;base64,']:not([data-reos-recolored-v2]), #editor-toolbar img[src^='data:image/svg+xml;base64,']:not([data-reos-recolored-v2])").forEach(s=>{s.setAttribute("data-reos-recolored","1"),s.setAttribute("data-reos-recolored-v2","1");let n=(s.getAttribute("src")||"").match(/^data:image\/svg\+xml;base64,(.+)$/);if(!n)return;let l;try{l=atob(n[1])}catch{return}let b=l,h=!1;if(e.forEach(d=>{b.includes(d)&&(b=b.split(d).join(r),h=!0)}),s.closest(".toolbar_button, .editor-toolbar, [class*='toolbar']")!==null&&["#000000","#000","#040E13","#040e13","#111827","#111","#101828","#101"].forEach(y=>{b.includes(y)&&(b=b.split(y).join("#EDEEF0"),h=!0)}),!!h)try{let d=btoa(b);s.setAttribute("src",`data:image/svg+xml;base64,${d}`)}catch{}}),document.querySelectorAll(".hl-checkbox-group-item img:not([data-reos-masked]):not([data-reos-recolored]), .hl-checkbox-group-item-icon img:not([data-reos-masked]):not([data-reos-recolored]), .hl-checkbox-group-item-container img:not([data-reos-masked]):not([data-reos-recolored])").forEach(s=>{let i=s.getAttribute("src")||"";if(!i||i.startsWith("data:")||!/\.svg(?:[?#]|$)/i.test(i))return;s.setAttribute("data-reos-masked","1");let n=`url("${i}")`;s.style.setProperty("-webkit-mask-image",n,"important"),s.style.setProperty("-webkit-mask-size","contain","important"),s.style.setProperty("-webkit-mask-repeat","no-repeat","important"),s.style.setProperty("-webkit-mask-position","center","important"),s.style.setProperty("mask-image",n,"important"),s.style.setProperty("mask-size","contain","important"),s.style.setProperty("mask-repeat","no-repeat","important"),s.style.setProperty("mask-position","center","important"),s.style.setProperty("background-color","var(--reos-blue)","important"),s.setAttribute("src",t)})},K=()=>{document.querySelectorAll("[class*='n-date-panel']").forEach(e=>{[...e.classList].forEach(t=>{if(t.startsWith("n-date-panel")){let o="hr-date-panel"+t.slice(12);e.classList.contains(o)||e.classList.add(o)}})})},Ae=()=>{let r,e=()=>{r===void 0&&(r=window.setTimeout(()=>{r=void 0,K(),P(),St(),W(),z(),Et(),kt(),xt(),wt(),ft(),ut(),Tt(),yt(),vt(),j(),_t(),pt(),M(),ve()},100))};K(),P(),W(),z(),j(),M(),ve(),window.setTimeout(()=>{K(),P(),W(),z(),j(),M()},500),window.setTimeout(()=>{K(),P(),W(),z(),j(),M()},1500),typeof requestAnimationFrame=="function"&&requestAnimationFrame(()=>{M(),P()}),new MutationObserver(()=>{e()}).observe(document.body,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["class","style"]})},ee,Ft=()=>{let r=()=>{let t=document.getElementById(Y);!t||!document.head||document.head.lastElementChild===t||document.head.appendChild(t)};new MutationObserver(t=>{let o=!1;for(let a of t){for(let s of Array.from(a.addedNodes)){if(!(s instanceof Element))continue;let i=s.tagName.toLowerCase();if(i==="style"||i==="link"){if(s.id===Y||s.id===Ce)continue;o=!0;break}}if(o)break}o&&ee===void 0&&(ee=window.setTimeout(()=>{ee=void 0,r()},150))}).observe(document.head,{childList:!0}),r()},Le=()=>{if(ot(),!document.getElementById(Y)){let r=document.createElement("style");r.id=Y,r.textContent=rt,document.head.appendChild(r),p.info("REInvest OS theme stylesheet mounted")}window.__reosForceInstalled||(window.__reosForceInstalled=!0,document.body?Ae():window.addEventListener("DOMContentLoaded",Ae)),window.__reosCascadeInstalled||(window.__reosCascadeInstalled=!0,Ft())};var De="ws-crm-customizer-version-badge",Ie=()=>{if(document.getElementById(De))return;let r=document.createElement("div");r.id=De,r.style.cssText=`
    position: fixed;
    bottom: 12px;
    right: 12px;
    z-index: 999999;
  ...
  `,r.textContent=`Customizer v${G}`,r.title=`Built ${V}
Click to dismiss for this session`,r.addEventListener("mouseenter",()=>{r.style.transform="scale(1.05)"}),r.addEventListener("mouseleave",()=>{r.style.transform="scale(1)"}),r.addEventListener("click",()=>{r.style.opacity="0",setTimeout(()=>r.remove(),200)}),document.body.appendChild(r)};var He=()=>{p.info(`Loaded v${G} (built ${V})`),_e(),Le(),ne(),re(r=>{p.debug("Page handler firing for:",r),Ie(),le(),pe()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",He):He();})();
