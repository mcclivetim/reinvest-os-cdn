/* Wholesaling CRM Customizer v0.10.29 — built 2026-05-21T20:19:53.792Z */
"use strict";var WholesalingCRMCustomizer=(()=>{var R="[wholesaling-crm-customizer]",He="__WS_CRM_DEBUG",Ne=()=>!!window[He],h={info(...o){console.log(R,...o)},warn(...o){console.warn(R,...o)},error(...o){console.error(R,...o)},debug(...o){Ne()&&console.log(R,"[debug]",...o)}};var oe=()=>{let o=window.location.pathname;return o.includes("/contacts/detail/")?"contact-detail":o.includes("/contacts/smart-list")||o.includes("/contacts/")?"contact-list":o.includes("/opportunities/detail/")?"opportunity-detail":o.includes("/opportunities/")?"opportunity-list":o.includes("/calendars/")?"calendar":o.includes("/dashboard")?"dashboard":o.includes("/conversations")?"conversations":o.includes("/settings")?"settings":"other"},B="",C="other",J=new Set,O=()=>{if(window.location.pathname!==B){B=window.location.pathname,C=oe(),h.debug("Page changed:",C,B);for(let o of J)try{o(C)}catch(t){h.error("Subscriber threw:",t)}}},re=o=>(J.add(o),setTimeout(()=>o(C),0),()=>{J.delete(o)}),ne=()=>{C=oe(),B=window.location.pathname,h.debug("Router init, current page:",C);let o=history.pushState.bind(history),t=history.replaceState.bind(history);history.pushState=function(e,r,a){o(e,r,a),O()},history.replaceState=function(e,r,a){t(e,r,a),O()},window.addEventListener("popstate",O),setInterval(O,1e3)};var G="0.10.29",V="2026-05-21T20:19:53.792Z";var Me=/\/contacts\/detail\/([A-Za-z0-9]+)/,Pe=/\/v2\/location\/([A-Za-z0-9]+)/,F=()=>{let o=window.location.pathname,t=o.match(Me);if(!t)return null;let e=o.match(Pe);return{contactId:t[1],locationId:e?e[1]:null}};var L=(o,t={})=>{let{timeoutMs:e=8e3,pollMs:r=100,root:a=document}=t;return new Promise((s,i)=>{let n=a.querySelector(o);if(n){s(n);return}let l,c,p=()=>{m&&m.disconnect(),l!==void 0&&window.clearTimeout(l),c!==void 0&&window.clearInterval(c)},m=new MutationObserver(()=>{let d=a.querySelector(o);d&&(p(),s(d))});m.observe(document.body,{childList:!0,subtree:!0}),c=window.setInterval(()=>{let d=a.querySelector(o);d&&(p(),s(d))},r),l=window.setTimeout(()=>{p(),h.warn(`waitForElement timed out for selector: ${o}`),i(new Error(`Selector not found within ${e}ms: ${o}`))},e)})};var b={obsidian:"#0A0D12",graphite:"#12161D",slate:"#1A1F28",steel:"#252C36",bone:"#EDEEF0",ash:"#9098A3",coolGray:"#5A6470",emerald:"#0FB57E",emeraldBright:"#14C98B",blue:"#4B8BF5",amber:"#E8A33C",crimson:"#D43F4A",emeraldGlow:"rgba(15, 181, 126, 0.12)",emeraldBorder:"rgba(15, 181, 126, 0.3)",blueGlow:"rgba(75, 139, 245, 0.12)",amberGlow:"rgba(232, 163, 60, 0.12)",crimsonGlow:"rgba(212, 63, 74, 0.12)"},k={sm:"4px",md:"6px",lg:"10px",pill:"999px"},f={sans:"'Geist', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",mono:"'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace"},Z={purple:"#8B5CF6",green:b.emerald,orange:b.amber,red:b.crimson},ae=o=>{let t=o==="emerald"?b.emeraldGlow:o==="amber"?b.amberGlow:b.slate,e=o==="emerald"?b.emeraldBorder:o==="amber"?"rgba(232, 163, 60, 0.3)":b.steel,r=o==="emerald"?b.emerald:o==="amber"?b.amber:b.bone;return`
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: 4px 10px;
...
`,D=`
  ...
  flex-shrink: 0;
`;var H="ws-crm-claim-release-pill",ie=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let a=r.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Re=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=H,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",a=t==="claim"?b.emerald:b.amber;return e.style.cssText=ae(r),e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,e.title=t==="claim"?"Take ownership of this lead":"Return this lead to the pool",e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",()=>{h.info(`Claim/Release pill clicked for contact ${o}: ${t}`);let s=e.lastElementChild,i=s?.textContent??"";s&&(s.textContent="Working\u2026"),e.style.opacity="0.7",e.disabled=!0,setTimeout(()=>{s&&(s.textContent=i),e.style.opacity="1",e.disabled=!1},700)}),e},Oe=async(o,t=1e4)=>{let e=Date.now();for(;Date.now()-e<t;){let r=ie(o);if(r)return r;await new Promise(a=>setTimeout(a,150))}return null},se=(o,t)=>{let e=Re({contactId:t,mode:"claim"}),r=document.createElement("span");r.id=`${H}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(e),o.appendChild(r)},Be=async()=>{let o=F();if(!o){document.getElementById(H)?.remove();return}let t=document.getElementById(H);if(!(t&&t.dataset.contactId===o.contactId)){t&&t.remove();try{let e=await L("#record-details-lhs",{timeoutMs:1e4}),r=await Oe(e),a=F();if(!a||a.contactId!==o.contactId)return;if(!r){h.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}se(r,o.contactId),h.debug(`Mounted Claim/Release pill next to name for ${o.contactId}`);let s=null;s=new MutationObserver(async()=>{if(!(F()?.contactId===o.contactId)){s?.disconnect();return}if(document.getElementById(H))return;let n=ie(e);n&&(se(n,o.contactId),h.debug("Re-mounted Claim/Release pill after React wipe"))}),s.observe(e,{childList:!0,subtree:!0})}catch(e){h.warn("Could not mount Claim/Release pill:",e)}}},le=()=>{Be()};var Ge="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set";var Ve="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Ue=()=>{let o=window.WS_CRM_USER_ID;if(o&&/^[A-Za-z0-9]{15,30}$/.test(o))return o;let t=window,e=[t.appState?.user?.id,t.user?.id,t.currentUser?.id,t.LCUser?.id,t.HL?.user?.id];for(let r of e)if(typeof r=="string"&&/^[A-Za-z0-9]{15,30}$/.test(r))return r;try{let r=localStorage.getItem("user");if(r){let a=JSON.parse(r),s=a.id??a._id;if(s&&/^[A-Za-z0-9]{15,30}$/.test(s))return s}for(let a of["tokenUser","currentUser","ghl_user"]){let s=localStorage.getItem(a);if(s)try{let i=JSON.parse(s),n=i.id??i._id;if(n&&/^[A-Za-z0-9]{15,30}$/.test(n))return n}catch{}}}catch{}return null},$e=async({contactId:o,locationId:t,status:e,userId:r,deadReason:a})=>{let s={call_status:e,triggered_by_user_id:r??""};a&&(s.dead_reason=a);let n=await fetch(Ge,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:o,location:{id:t},customData:s}),credentials:"omit"});if(!n.ok)throw new Error(`Webhook returned ${n.status} ${n.statusText}`)},qe=async({contactId:o,tenantId:t})=>{let e=await fetch(Ve,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:t,contact_id:o}),credentials:"omit"});if(!e.ok)throw new Error(`Sanity check returned ${e.status} ${e.statusText}`);return await e.json()};var U="ws-crm-contact-actions-bar",ze="ws-crm-call-status-dropdown",de="ws-crm-offer-made-modal",We=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],je=[".central-panel","[class*='central-panel']"],Ke=async()=>{for(let o of je)try{let t=await L(o,{timeoutMs:3e3});if(t)return t}catch{}try{return(await L("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},Ye=o=>{let t=document.createElement("div");t.id=ze,t.style.cssText="position: relative; flex-shrink: 0;";let e=document.createElement("button");e.type="button",e.style.cssText=D,e.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,We.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
    ...
    `,s.innerHTML=`
      ...
      "></span>
      <span>${a.label}</span>
    `,s.addEventListener("mouseenter",()=>{s.style.background=b.slate}),s.addEventListener("mouseleave",()=>{s.style.background="transparent"}),s.addEventListener("click",async()=>{h.info(`Call Status set to "${a.label}" (${a.value}, ${a.color}) for contact ${o}`);let i=e.firstElementChild,n=e.children[1],l=e.children[2];i&&(i.style.background=Z[a.color]),n&&(n.textContent=a.label),r.style.display="none";let p=F()?.locationId,m=Ue();if(!p){h.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),n&&(n.textContent=`${a.label} (no tenant!)`),n&&(n.style.color=b.amber);return}if(a.color==="green"&&!m){h.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),n&&(n.textContent=`${a.label} (no user!)`),n&&(n.style.color=b.amber);return}let d=l?.textContent??"\u25BE",y=async u=>{l&&(l.textContent="\u2026"),e.style.opacity="0.75";try{await $e({contactId:o,locationId:p,status:a.label,userId:m,deadReason:u}),l&&(l.textContent="\u2713"),setTimeout(()=>{l&&(l.textContent=d),e.style.opacity="1"},900)}catch(v){h.warn("Call Status webhook failed:",v),l&&(l.textContent="\u2717"),n&&(n.style.color=b.amber),e.style.opacity="1",setTimeout(()=>{l&&(l.textContent=d),n&&(n.style.color=b.bone)},1800)}};if(a.value==="dead-deal"){let u=n?.textContent??"";Je(v=>{y(v)},()=>{n&&(n.textContent=u)});return}if(a.value==="appointment-booked"){let u=n?.textContent??"";try{(await qe({contactId:o,tenantId:p})).ok?await y():Ze(()=>{y()},()=>{n&&(n.textContent=u)})}catch(v){h.warn("Appointment sanity check failed; firing anyway:",v),await y()}return}await y()}),r.appendChild(s)}),e.addEventListener("click",a=>{a.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",a=>{t.contains(a.target)||(r.style.display="none")}),t.appendChild(e),t.appendChild(r),t},Xe=o=>{document.getElementById(de)?.remove();let t=document.createElement("div");t.id=de,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,t.appendChild(e),document.body.appendChild(t);let s=()=>t.remove();t.addEventListener("click",i=>{i.target===t&&s()}),e.querySelector("#ws-offer-cancel")?.addEventListener("click",s),e.querySelector("#ws-offer-submit")?.addEventListener("click",()=>{let i=(e.querySelector("#ws-offer-amount")?.value??"").trim(),n=e.querySelector("#ws-offer-expires")?.value??"";if(!i||Number(i)<=0){h.warn("Offer Made submit blocked \u2014 invalid amount");return}h.info(`Offer Made recorded for contact ${o}: $${i}, expires ${n}`),s()}),setTimeout(()=>{e.querySelector("#ws-offer-amount")?.focus()},0)},ce="ws-crm-dead-deal-modal",Je=(o,t)=>{document.getElementById(ce)?.remove();let e=document.createElement("div");e.id=ce,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,e.appendChild(r),document.body.appendChild(e);let a=()=>e.remove(),s=()=>{a(),t()};e.addEventListener("click",n=>{n.target===e&&s()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",s),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{a(),o()});let i=n=>{n.key==="Escape"&&(document.removeEventListener("keydown",i),s())};document.addEventListener("keydown",i)},he=o=>{let t=document.createElement("div");t.id=U,t.dataset.contactId=o,t.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    ...
    flex-shrink: 0;
  `,t.appendChild(Ye(o));let e=document.createElement("button");return e.type="button",e.style.cssText=I,e.textContent="Offer Made",e.addEventListener("click",()=>Xe(o)),t.appendChild(e),t},Qe=async()=>{let o=F();if(!o){document.getElementById(U)?.remove();return}let t=document.getElementById(U);if(t&&t.dataset.contactId===o.contactId)return;t&&t.remove();let e=await Ke();if(!e){h.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let r=F();if(!r||r.contactId!==o.contactId)return;let a=he(o.contactId);a.dataset.mountTarget=e.className||"central-panel",e.insertBefore(a,e.firstChild),h.debug(`Mounted contact actions bar for ${o.contactId}`);let s=null;s=new MutationObserver(()=>{let i=document.getElementById(U);if(!(F()?.contactId===o.contactId)){s?.disconnect();return}if(!i){let l=he(o.contactId);l.dataset.mountTarget=a.dataset.mountTarget??"",e.insertBefore(l,e.firstChild),h.debug(`Re-mounted actions bar after React wipe for ${o.contactId}`)}}),s.observe(e,{childList:!0})},pe=()=>{Qe()};var $="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",et=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),N="data-ws-crm-favicon",Q=o=>o.hasAttribute(N),me=o=>{if(o.tagName!=="LINK")return!1;let t=(o.getAttribute("rel")??"").toLowerCase();return et.has(t)},q=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{me(r)&&!Q(r)&&r.remove()});let t=document.head.querySelector(`link[${N}="1"]`);t?t.href!==$&&(t.href=$):(t=document.createElement("link"),t.setAttribute(N,"1"),t.rel="icon",t.type="image/png",t.href=$,document.head.appendChild(t),h.debug("Favicon installed"));let e=document.head.querySelector(`link[${N}="apple"]`);e||(e=document.createElement("link"),e.setAttribute(N,"apple"),e.rel="apple-touch-icon",e.href=$,document.head.appendChild(e))},_e=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)q();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),q())},50)}new MutationObserver(r=>{let a=!1;for(let s of r)s.addedNodes.forEach(i=>{i instanceof HTMLLinkElement&&me(i)&&!Q(i)&&(a=!0)}),s.removedNodes.forEach(i=>{i instanceof HTMLLinkElement&&Q(i)&&(a=!0)});a&&q()}).observe(document.head,{childList:!0,subtree:!1});let t=0,e=window.setInterval(()=>{t+=1,q(),t>=5&&window.clearInterval(e)},1e3)};var Y="reos-theme-stylesheet",Ae="reos-theme-fonts",tt=()=>{let o=document.getElementById(Ae);o&&o.remove()},ot=`
/* ============================================
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};var De=()=>{h.info(`Loaded v${G} (built ${V})`),_e(),Ce(),ne(),re(o=>{h.debug("Page handler firing for:",o),Ie(),le(),pe()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",De):De();})();
