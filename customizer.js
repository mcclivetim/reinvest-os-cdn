/* Wholesaling CRM Customizer v0.10.25 — built 2026-05-21T19:51:11.926Z */
"use strict";var WholesalingCRMCustomizer=(()=>{var M="[wholesaling-crm-customizer]",Ie="__WS_CRM_DEBUG",De=()=>!!window[Ie],h={info(...r){console.log(M,...r)},warn(...r){console.warn(M,...r)},error(...r){console.error(M,...r)},debug(...r){De()&&console.log(M,"[debug]",...r)}};var ee=()=>{let r=window.location.pathname;return r.includes("/contacts/detail/")?"contact-detail":r.includes("/contacts/smart-list")||r.includes("/contacts/")?"contact-list":r.includes("/opportunities/detail/")?"opportunity-detail":r.includes("/opportunities/")?"opportunity-list":r.includes("/calendars/")?"calendar":r.includes("/dashboard")?"dashboard":r.includes("/conversations")?"conversations":r.includes("/settings")?"settings":"other"},R="",F="other",Y=new Set,P=()=>{if(window.location.pathname!==R){R=window.location.pathname,F=ee(),h.debug("Page changed:",F,R);for(let r of Y)try{r(F)}catch(t){h.error("Subscriber threw:",t)}}},te=r=>(Y.add(r),setTimeout(()=>r(F),0),()=>{Y.delete(r)}),oe=()=>{F=ee(),R=window.location.pathname,h.debug("Router init, current page:",F);let r=history.pushState.bind(history),t=history.replaceState.bind(history);history.pushState=function(e,o,a){r(e,o,a),P()},history.replaceState=function(e,o,a){t(e,o,a),P()},window.addEventListener("popstate",P),setInterval(P,1e3)};var O="0.10.25",B="2026-05-21T19:51:11.926Z";var Ne=/\/contacts\/detail\/([A-Za-z0-9]+)/,He=/\/v2\/location\/([A-Za-z0-9]+)/,T=()=>{let r=window.location.pathname,t=r.match(Ne);if(!t)return null;let e=r.match(He);return{contactId:t[1],locationId:e?e[1]:null}};var A=(r,t={})=>{let{timeoutMs:e=8e3,pollMs:o=100,root:a=document}=t;return new Promise((s,i)=>{let n=a.querySelector(r);if(n){s(n);return}let l,b,p=()=>{m&&m.disconnect(),l!==void 0&&window.clearTimeout(l),b!==void 0&&window.clearInterval(b)},m=new MutationObserver(()=>{let d=a.querySelector(r);d&&(p(),s(d))});m.observe(document.body,{childList:!0,subtree:!0}),b=window.setInterval(()=>{let d=a.querySelector(r);d&&(p(),s(d))},o),l=window.setTimeout(()=>{p(),h.warn(`waitForElement timed out for selector: ${r}`),i(new Error(`Selector not found within ${e}ms: ${r}`))},e)})};var c={obsidian:"#0A0D12",graphite:"#12161D",slate:"#1A1F28",steel:"#252C36",bone:"#EDEEF0",ash:"#9098A3",coolGray:"#5A6470",emerald:"#0FB57E",emeraldBright:"#14C98B",blue:"#4B8BF5",amber:"#E8A33C",crimson:"#D43F4A",emeraldGlow:"rgba(15, 181, 126, 0.12)",emeraldBorder:"rgba(15, 181, 126, 0.3)",blueGlow:"rgba(75, 139, 245, 0.12)",amberGlow:"rgba(232, 163, 60, 0.12)",crimsonGlow:"rgba(212, 63, 74, 0.12)"},k={sm:"4px",md:"6px",lg:"10px",pill:"999px"},f={sans:"'Geist', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif",mono:"'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace"},X={purple:"#8B5CF6",green:c.emerald,orange:c.amber,red:c.crimson},re=r=>{let t=r==="emerald"?c.emeraldGlow:r==="amber"?c.amberGlow:c.slate,e=r==="emerald"?c.emeraldBorder:r==="amber"?"rgba(232, 163, 60, 0.3)":c.steel,o=r==="emerald"?c.emerald:r==="amber"?c.amber:c.bone;return`
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: 4px 10px;
...
`;var I="ws-crm-claim-release-pill",ae=r=>{let t=Array.from(r.children).find(o=>{if(!(o instanceof HTMLElement))return!1;let a=o.className?.toString()??"";return a.includes("flex")&&a.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},Me=({contactId:r,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=r,e.dataset.mode=t;let o=t==="claim"?"emerald":"amber",a=t==="claim"?c.emerald:c.amber;return e.style.cssText=re(o),e.innerHTML=`
  ...
  `;let o=document.createElement("div");return o.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,qe.forEach(a=>{let s=document.createElement("button");s.type="button",s.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
...
Click to dismiss for this session`,r.addEventListener("mouseenter",()=>{r.style.transform="scale(1.05)"}),r.addEventListener("mouseleave",()=>{r.style.transform="scale(1)"}),r.addEventListener("click",()=>{r.style.opacity="0",setTimeout(()=>r.remove(),200)}),document.body.appendChild(r)};var Le=()=>{h.info(`Loaded v${O} (built ${B})`),pe(),Fe(),oe(),te(r=>{h.debug("Page handler firing for:",r),Ce(),se(),be()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Le):Le();})();
