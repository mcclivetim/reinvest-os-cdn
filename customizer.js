...
`;var Bt=/^[A-Za-z0-9]{15,30}$/,q=o=>typeof o=="string"&&Bt.test(o),Gt=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},$t=()=>{let o=window.WS_CRM_USER_ID;return q(o)?o:null},Vt=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(q(t))return t;return null},Ut=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=Gt(e);if(!t)return null;let r=t.uid;if(q(r))return r;let n=t.claims;if(n&&q(n.user_id))return n.user_id}catch{}return null},qt=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(q(n))return n}catch{}}}catch{}return null},A=()=>$t()||Vt()||Ut()||qt()||null;var We="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",zt=`${We}/active-follow-up-get`,Wt=`${We}/active-follow-up-set`,O="ws-crm-active-follow-up-button",Ie={amber:{label:"Add to Active Follow Up",bg:d.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:d.amber,clickable:!0,dotColor:d.amber},blue:{label:"In Active Follow Up",bg:d.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:d.blue,clickable:!1,dotColor:d.blue},gray:{label:"Not in Follow Up",bg:d.slate,border:d.steel,color:d.coolGray,clickable:!1,dotColor:d.coolGray},loading:{label:"Active Follow Up\u2026",bg:d.slate,border:d.steel,color:d.ash,clickable:!1,dotColor:d.ash},error:{label:"Follow Up status unavailable",bg:d.slate,border:d.steel,color:d.ash,clickable:!1,dotColor:d.crimson}},jt=()=>{let o=T();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},Kt=async o=>{let e=await fetch(zt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},Yt=async(o,e)=>{let t=await fetch(Wt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},R=(o,e)=>{let t=Ie[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},Ue=o=>{let e=document.createElement("button");return e.type="button",e.id=O,e.dataset.contactId=o,e.style.cssText=`
  ...
  `,R(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=jt();if(!(!r||r.contactId!==o)){R(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await Yt(r,A()),a=n&&n.state&&n.state in Ie?n.state:"blue";R(e,a),b.info(`Active Follow Up set for ${o} \u2192 ${a}`)}catch(n){b.error("Active Follow Up Set webhook failed",n),R(e,"error"),setTimeout(()=>void le(e,r),4e3)}}}),e},le=async(o,e)=>{R(o,"loading");try{let t=await Kt(e),r=t&&t.state&&t.state in Ie?t.state:"gray",n=document.getElementById(O);if(!n||n.dataset.contactId!==e.contactId)return;R(n,r)}catch(t){b.warn("Active Follow Up Get webhook failed",t),R(o,"error")}},Xt=[".central-panel","[class*='central-panel']"],Le="ws-crm-contact-actions-bar",Jt=async()=>{for(let o of Xt)try{let e=await C(o,{timeoutMs:3e3});if(e)return e}catch{}try{return(await C("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},Qt=async(o,e)=>{let t=e.querySelector(`#${Le}`);if(!t)try{t=await C(`#${Le}`,{timeoutMs:3e3,root:e})}catch{t=null}if(t){let a=e.querySelector("#ws-crm-afu-wrap");return a&&a.remove(),t.appendChild(o),{container:t,placement:"actions-bar"}}let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
