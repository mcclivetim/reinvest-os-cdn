...
`;var I="ws-crm-claim-release-pill",oe=o=>{let t=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return t?t.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},De=({contactId:o,mode:t})=>{let e=document.createElement("button");e.id=I,e.type="button",e.dataset.contactId=o,e.dataset.mode=t;let r=t==="claim"?"emerald":"amber",n=t==="claim"?l.emerald:l.amber;return e.style.cssText=ee(r),e.innerHTML=`
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,Ve.forEach(n=>{let i=document.createElement("button");i.type="button",i.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
...
/* Description text inside the card \u2014 currently inherits emerald
   from the .n-button:hover content rule (because parent is a
   button and we paint .n-button__content emerald on hover). Per
   canon, the description should stay ash even on hover; only
   the title should signal active/hover via color shift. */
html body .hl-checkbox-group-item:not(#__reos_never_id) p.hl-text-sm-regular,
html body .hl-checkbox-group-item:not(#__reos_never_id) .text-gray-500,
...
html body .hl-checkbox-group-item:not(#__reos_never_id):hover .text-gray-500,
html body .hl-checkbox-group-item:not(#__reos_never_id):hover [class*="hl-text-sm-regular"] {
  color: var(--reos-ash) !important;
  -webkit-text-fill-color: var(--reos-ash) !important;
}

/* 18a-n-steps (v0.9.52). Naive UI Steps component on the Import
   Data wizard ("Start / Upload / Map / Verify"). Active indicator
   ships brand-blue #155EEF; inactive indicators ship gray-200
   border + transparent fill. Canon:
...
/* Active step's splitter \u2014 emerald, signaling progression */
