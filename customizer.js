...
`;var so=/^[A-Za-z0-9]{15,30}$/,j=o=>typeof o=="string"&&so.test(o),io=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},lo=()=>{let o=window.WS_CRM_USER_ID;return j(o)?o:null},co=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(j(t))return t;return null},bo=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=io(e);if(!t)return null;let r=t.uid;if(j(r))return r;let n=t.claims;if(n&&j(n.user_id))return n.user_id}catch{}return null},ho=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(j(n))return n}catch{}}}catch{}return null},A=()=>lo()||co()||bo()||ho()||null;var dt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",po=`${dt}/active-follow-up-get`,mo=`${dt}/active-follow-up-set`,G="ws-crm-active-follow-up-button",Oe={amber:{label:"Add to Active Follow Up",bg:l.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:l.amber,clickable:!0,dotColor:l.amber},blue:{label:"In Active Follow Up",bg:l.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:l.blue,clickable:!1,dotColor:l.blue},gray:{label:"Not in Follow Up",bg:l.slate,border:l.steel,color:l.coolGray,clickable:!1,dotColor:l.coolGray},loading:{label:"Active Follow Up\u2026",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.ash},error:{label:"Follow Up status unavailable",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.crimson}},_o=()=>{let o=S();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},uo=async o=>{let e=await fetch(po,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},vo=async(o,e)=>{let t=await fetch(mo,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},$=(o,e)=>{let t=Oe[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},st=o=>{let e=document.createElement("button");return e.type="button",e.id=G,e.dataset.contactId=o,e.style.cssText=`
  ...
  `,t.forEach(d=>{let c=document.createElement("button");c.type="button",c.style.cssText=`
    ...
    `,c.innerHTML=`
      ...
      "></span>
      <span>${d.label}</span>
    `,c.addEventListener("mouseenter",()=>{c.style.background=l.slate}),c.addEventListener("mouseleave",()=>{c.style.background="transparent"}),c.addEventListener("click",async()=>{b.info(`Call Status set to "${d.label}" (${d.value}, ${d.color}) for contact ${o}`);let p=i.firstElementChild,m=i.children[1],h=i.children[2];p&&(p.style.background=Pe[d.color]),m&&(m.textContent=d.label),s.style.display="none";let g=S()?.locationId,w=A();if(!g){b.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),m&&(m.textContent=`${d.label} (no tenant!)`),m&&(m.style.color=l.amber);return}if(e==="seller"&&d.color==="green"&&!w){b.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),m&&(m.textContent=`${d.label} (no user!)`),m&&(m.style.color=l.amber);return}let v=h?.textContent??"\u25BE",u=async y=>{h&&(h.textContent="\u2026"),i.style.opacity="0.75";try{await Po({contactId:o,locationId:g,status:d.label,userId:w,deadReason:y,webhookUrl:r,kind:e}),h&&(h.textContent="\u2713"),setTimeout(()=>{h&&(h.textContent=v),i.style.opacity="1"},900)}catch(k){b.warn("Call Status webhook failed:",k),h&&(h.textContent="\u2717"),m&&(m.style.color=l.amber),i.style.opacity="1",setTimeout(()=>{h&&(h.textContent=v),m&&(m.style.color=l.bone)},1800)}};if(e==="seller"&&d.value==="dead-deal"){let y=m?.textContent??"";zo(k=>{u(k)},()=>{m&&(m.textContent=y)});return}if(e==="seller"&&d.value==="appointment-booked"){let y=m?.textContent??"";try{(await No({contactId:o,tenantId:g})).ok?await u():Wo(()=>{u()},()=>{m&&(m.textContent=y)})}catch(k){b.warn("Appointment sanity check failed; firing anyway:",k),await u()}return}await u()}),s.appendChild(c)}),i.addEventListener("click",d=>{d.stopPropagation(),s.style.display=s.style.display==="none"?"block":"none"}),document.addEventListener("click",d=>{a.contains(d.target)||(s.style.display="none")}),a.appendChild(i),a.appendChild(s),a},Uo="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/offer-made/set",Vo=async o=>{let e=await fetch(Uo,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,kind:o.kind,amount:o.amount,expires_at:o.expiresAt,triggered_by_user_id:o.userId}),credentials:"omit"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return e.json()},qo=(o,e="seller")=>{document.getElementById(vt)?.remove();let t=e==="seller",r=t?"Record Your Offer":"Record Buyer's Offer",n=t?"Attributes the offer to you for KPI tracking. Updates the ACQ opportunity's offer fields.":"Logs the buyer's offer on their Marketing Tracker card. Advances them to 'Made an Offer'.",a=t?l.emerald:l.amber,i=document.createElement("div");i.id=vt,i.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `;let d=new Date,c=new Date(d.getTime()+5*24*60*60*1e3).toISOString().slice(0,10),p=t?`
    ...
    <div style="
      ...
      margin-bottom: 16px;
    ">
      <span style="color: ${l.ash}; font-family: ${x.mono}; font-size: 14px;">$</span>
  ...
  `,i.appendChild(s),document.body.appendChild(i);let m=()=>i.remove();i.addEventListener("click",w=>{w.target===i&&m()});let h=s.querySelector("#ws-offer-error"),_=s.querySelector("#ws-offer-cancel"),g=s.querySelector("#ws-offer-submit");_?.addEventListener("click",m),g?.addEventListener("click",async()=>{let w=(s.querySelector("#ws-offer-amount")?.value??"").trim(),v=t?s.querySelector("#ws-offer-expires")?.value??"":"";if(!w||Number(w)<=0){h&&(h.textContent="Enter a positive offer amount."),b.warn("Offer Made submit blocked \u2014 invalid amount");return}let u=Number(w);if(u<100){h&&(h.textContent=`That looks low \u2014 did you mean $${(u*1e3).toLocaleString("en-US")}?`);return}let k=window.location.pathname.match(/\/v2\/location\/([A-Za-z0-9]+)/),E=k?k[1]:null;if(!E){h&&(h.textContent="Couldn't resolve tenant from URL. Refresh and try again.");return}h&&(h.textContent=""),g&&(g.disabled=!0,g.textContent="Recording\u2026",g.style.opacity="0.7"),_&&(_.disabled=!0);try{let f=await Vo({tenantId:E,contactId:o,kind:e,amount:u,expiresAt:v||null,userId:A()});if(!f.ok){h&&(h.textContent=f.error??"Something went wrong."),g&&(g.disabled=!1,g.textContent="Record Offer",g.style.opacity="1"),_&&(_.disabled=!1);return}b.info(`Offer Made (${e}) recorded for contact ${o}: $${u} on ${f.opp_pipeline} opp (stage: ${f.opp_stage_name??"?"})`),m()}catch(f){b.error("Offer Made webhook failed",f),h&&(h.textContent=f instanceof Error?f.message:"Network error \u2014 try again."),g&&(g.disabled=!1,g.textContent="Record Offer",g.style.opacity="1"),_&&(_.disabled=!1)}}),setTimeout(()=>{s.querySelector("#ws-offer-amount")?.focus()},0)},yt="ws-crm-dead-deal-modal",zo=(o,e)=>{document.getElementById(yt)?.remove();let t=document.createElement("div");t.id=yt,t.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,t.appendChild(r),document.body.appendChild(t);let n=()=>t.remove(),a=()=>{n(),e()};t.addEventListener("click",s=>{s.target===t&&a()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",a),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),o()});let i=s=>{s.key==="Escape"&&(document.removeEventListener("keydown",i),a())};document.addEventListener("keydown",i)},ft=(o,e)=>{let t=document.createElement("div");t.id=z,t.dataset.contactId=o,t.dataset.contactKind=e,t.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    ...
    border-bottom: 1px solid ${l.steel};
    flex-shrink: 0;
  `;let r=document.createElement("button");return r.type="button",r.style.cssText=B,r.textContent="Offer Made",r.addEventListener("click",()=>qo(o,e)),t.appendChild(r),t.appendChild($o({contactId:o,kind:e})),t},Re=!1,jo=()=>{document.querySelectorAll(`[id="${z}"]`).forEach(o=>o.remove())},Et=async()=>{if(b.debug("[actions-bar:01] ensureMounted called; url:",window.location.pathname),Re){b.debug("[actions-bar:00-LOCKED] ensureMounted already in-flight \u2014 skip");return}Re=!0;try{await Ko()}finally{Re=!1}},Ko=async()=>{let o=S();if(!o){b.debug("[actions-bar:02-BAIL] no contact context (URL didn't match patterns AND no in-page contact link). URL:",window.location.pathname),jo();return}b.debug(`[actions-bar:02] ctx resolved \u2014 contactId=${o.contactId} locationId=${o.locationId??"(null)"} source=${o.source}`);let e="seller";try{let c=await _t(o.contactId,o.locationId);ut(c)==="buyer"&&(e="buyer"),b.debug(`[actions-bar:03] kind resolved \u2014 kind=${e} (raw type=${c??"(null)"})`)}catch(c){b.warn("[actions-bar:03-FAIL] Contact Type resolution threw \u2014 defaulting to seller:",c)}let t=S();if(!t||t.contactId!==o.contactId){b.debug("[actions-bar:04-BAIL] context changed during Contact Type fetch \u2014 aborting mount");return}let r=Array.from(document.querySelectorAll(`[id="${z}"]`));if(r.length===1&&r[0].dataset.contactId===o.contactId&&r[0].dataset.contactKind===e){b.debug(`[actions-bar:05-SKIP] bar already mounted for contact=${o.contactId} kind=${e} \u2014 no-op`);return}r.length>0&&(b.debug(`[actions-bar:05] removing ${r.length} stale bar(s) before remount`),r.forEach(c=>c.remove()));let n=await Go();if(!n){b.warn("[actions-bar:06-FAIL] findMountTarget returned null \u2014 no .central-panel or fallback selector matched within timeout. URL:",window.location.pathname);return}b.debug(`[actions-bar:06] mount target found \u2014 tagName=${n.tagName} class="${n.className}"`);let a=S();if(!a||a.contactId!==o.contactId){b.debug("[actions-bar:07-BAIL] context changed during findMountTarget wait \u2014 aborting mount");return}let i;try{i=ft(o.contactId,e)}catch(c){b.error("[actions-bar:08-FAIL] buildBar threw:",c);return}i.dataset.mountTarget=n.className||"central-panel";try{n.insertBefore(i,n.firstChild)}catch(c){b.error("[actions-bar:09-FAIL] insertBefore threw:",c);return}if(!document.getElementById(z)){b.warn("[actions-bar:10-DETACHED] bar inserted but not in document tree \u2014 target was likely re-rendered by Vue. Will retry next page-change.");return}b.debug(`[actions-bar:10-OK] bar mounted + verified in document for contact=${o.contactId} kind=${e}`);let d=null;d=new MutationObserver(()=>{if(!(S()?.contactId===o.contactId)){d?.disconnect();return}let p=document.querySelectorAll(`[id="${z}"]`);if(p.length>1){for(let m=1;m<p.length;m++)p[m].remove();b.debug(`[actions-bar:WATCHDOG] trimmed ${p.length-1} duplicate bar(s)`);return}if(p.length===0){let m=document.querySelector(".central-panel")||n,h=ft(o.contactId,e);h.dataset.mountTarget=i.dataset.mountTarget??"";try{m.insertBefore(h,m.firstChild),b.debug(`[actions-bar:WATCHDOG] re-mounted after wipe for ${o.contactId}`)}catch(_){b.warn("[actions-bar:WATCHDOG-FAIL] re-mount threw:",_)}}}),d.observe(n,{childList:!0})},wt=null,Yo=()=>{wt===null&&(wt=window.setInterval(()=>{let o=S();o&&(document.getElementById(z)||(b.debug("[actions-bar:HEALER] no bar on contact page \u2014 re-triggering mount for",o.contactId),Et()))},3e3))},Tt=()=>{Yo(),Et()};var he="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",Xo=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),X="data-ws-crm-favicon",Be=o=>o.hasAttribute(X),St=o=>{if(o.tagName!=="LINK")return!1;let e=(o.getAttribute("rel")??"").toLowerCase();return Xo.has(e)},pe=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{St(r)&&!Be(r)&&r.remove()});let e=document.head.querySelector(`link[${X}="1"]`);e?e.href!==he&&(e.href=he):(e=document.createElement("link"),e.setAttribute(X,"1"),e.rel="icon",e.type="image/png",e.href=he,document.head.appendChild(e),b.debug("Favicon installed"));let t=document.head.querySelector(`link[${X}="apple"]`);t||(t=document.createElement("link"),t.setAttribute(X,"apple"),t.rel="apple-touch-icon",t.href=he,document.head.appendChild(t))},Ct=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)pe();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),pe())},50)}new MutationObserver(r=>{let n=!1;for(let a of r)a.addedNodes.forEach(i=>{i instanceof HTMLLinkElement&&St(i)&&!Be(i)&&(n=!0)}),a.removedNodes.forEach(i=>{i instanceof HTMLLinkElement&&Be(i)&&(n=!0)});n&&pe()}).observe(document.head,{childList:!0,subtree:!1});let e=0,t=window.setInterval(()=>{e+=1,pe(),e>=5&&window.clearInterval(t)},1e3)};var Jo=["Acquisitions","Transaction Coordination","Disposition","Marketing Tracker"],Qo=/^(?:\d+\)\s*)?(Acquisitions|Transaction Coordination|Disposition|Marketing Tracker)\s*$/,Zo=".hr-card.hr-modal.crm-opportunities-modal",L=()=>{let o=document.querySelector(Zo);if(!o)return null;let e=o.querySelectorAll(".hr-base-selection-overlay__wrapper, .hr-base-selection");for(let t of e){let r=t.textContent?.trim()??"";if(!r)continue;let n=r.match(Qo);if(n){let a=n[1];if(Jo.includes(a))return a}}return null},me=async(o=2500)=>{let e=Date.now();for(;Date.now()-e<o;){let t=L();if(t!==null)return t;await new Promise(r=>setTimeout(r,100))}return null};var er="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/mt/not-interested",F="ws-crm-not-interested-button",tr=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,or=/\/v2\/location\/([A-Za-z0-9]+)/,ue=()=>{let o=window.location.pathname,e=o.match(tr);if(!e)return null;let t=o.match(or);return{oppId:e[1],locationId:t?t[1]:null}},rr=async o=>{let e=await fetch(er,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,mt_opp_id:o.oppId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Not Interested webhook returned ${e.status} ${e.statusText}`);return await e.json()},nr=o=>{let e=document.createElement("button");return e.id=F,e.type="button",e.dataset.oppId=o,e.style.cssText=H,e.title="Close this Marketing Tracker opp as Not Interested for THIS property only",e.innerHTML=`
    <span style="
      display: inline-block;
      width: 8px;
      height: 8px;
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let t=ue();if(!t||!t.locationId||t.oppId!==o)return;let r=A(),n=e.querySelector(".ws-ni-label");e.disabled=!0,e.style.opacity="0.7",n&&(n.textContent="Closing\u2026");try{let a=await rr({oppId:t.oppId,locationId:t.locationId,userId:r});if(a?.ok)n&&(n.textContent="Marked Not Interested"),e.style.background=l.crimsonGlow,e.style.color=l.crimson,b.info(`Not Interested set for opp ${o} \u2192 ${a.new_stage}`);else throw new Error("Webhook returned ok=false")}catch(a){b.error("Not Interested webhook failed",a),n&&(n.textContent="Try again"),e.disabled=!1,e.style.opacity="1",setTimeout(()=>{n&&n.textContent==="Try again"&&(n.textContent="Mark Not Interested")},3e3)}}),e},ar=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],sr=async()=>{for(let o of ar)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},_e=!1,Ve=async()=>{if(!_e){_e=!0;try{await ir()}finally{_e=!1}}},ir=async()=>{let o=ue();if(!o||!o.locationId){document.getElementById(F)?.remove(),document.getElementById(`${F}-wrap`)?.remove(),Q();return}let e=document.getElementById(F);if(e&&e.dataset.oppId===o.oppId){let s=L();s!==null&&s!=="Marketing Tracker"&&(e.remove(),document.getElementById(`${F}-wrap`)?.remove(),Q());return}e&&(e.remove(),document.getElementById(`${F}-wrap`)?.remove());let t=await sr();if(!t){b.warn("Not Interested: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await me();if(r!=="Marketing Tracker"){b.debug(`[not-interested] gated off \u2014 pipeline="${r}" (expected Marketing Tracker)`);return}let n=ue();if(!n||n.oppId!==o.oppId)return;let a=nr(o.oppId),i=document.createElement("span");i.id=`${F}-wrap`,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),i.appendChild(a),t.appendChild(i),b.debug(`Mounted Not Interested button on opp ${o.oppId}`),lr(o.oppId)},J=null,Ue=null,Q=()=>{J?.disconnect(),J=null,Ue=null},lr=o=>{if(J&&Ue===o)return;Q();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(Ue=o,J=new MutationObserver(()=>{let t=ue();if(!t||t.oppId!==o){Q();return}let r=L();if(r!==null&&r!=="Marketing Tracker"){document.getElementById(F)?.remove(),document.getElementById(`${F}-wrap`)?.remove(),Q();return}document.getElementById(F)||Ve()}),J.observe(e,{childList:!0,subtree:!0}))},Ge=null,$e,dr=()=>{Ge||(Ge=new MutationObserver(()=>{$e===void 0&&($e=window.setTimeout(()=>{$e=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(F);o&&!e&&!_e&&Ve()},300))}),Ge.observe(document.body,{childList:!0,subtree:!0}))},At=()=>{dr(),Ve()};var cr="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/property/listing-price-changed",D="ws-crm-reduce-price-button",It="ws-crm-reduce-price-modal",br=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,hr=/\/v2\/location\/([A-Za-z0-9]+)/,ge=()=>{let o=window.location.pathname,e=o.match(br);if(!e)return null;let t=o.match(hr);return{oppId:e[1],locationId:t?t[1]:null}},pr=async o=>{let e=await fetch(cr,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,dispo_opp_id:o.oppId,new_price:o.newPrice,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Reduce Price webhook returned ${e.status} ${e.statusText}`);let t=await e.text();try{let r=JSON.parse(t);if(r?.ok===!1&&r?.error)throw new Error(r.error);return r}catch(r){if(r instanceof Error&&/returned/.test(r.message))throw r;return{}}},ve=()=>{document.getElementById(It)?.remove()},mr=o=>{ve();let e=document.createElement("div");e.id=It,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
    background: rgba(0, 0, 0, 0.6);
  ...
  `,t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-1px)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",()=>{let r=ge();!r||!r.locationId||r.oppId!==o||mr({oppId:r.oppId,locationId:r.locationId,onSuccess:()=>{let n=t.querySelector(".ws-rp-label"),a=t.querySelector(":scope > span:first-child");n&&(n.textContent="Price Reduced \u2014 Buyers Alerted"),t.style.background=l.emeraldGlow,t.style.color=l.emeraldBright,t.style.borderColor=l.emeraldBorder,a&&(a.style.background=l.emerald,a.style.boxShadow=`0 0 6px ${l.emerald}`),setTimeout(()=>{n&&n.textContent==="Price Reduced \u2014 Buyers Alerted"&&(n.textContent="Reduce Price and Alert Buyers",t.style.cssText=H,a&&(a.style.background=l.amber,a.style.boxShadow=`0 0 6px ${l.amber}`))},4e3)}})}),t},ur=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],vr=async()=>{for(let o of ur)try{let e=await C(o,{timeoutMs:2e3});if(e)return e}catch{}return null},ye=!1,je=async()=>{if(!ye){ye=!0;try{await yr()}finally{ye=!1}}},yr=async()=>{let o=ge();if(!o||!o.locationId){document.getElementById(D)?.remove(),document.getElementById(`${D}-wrap`)?.remove(),ee();return}let e=document.getElementById(D);if(e&&e.dataset.oppId===o.oppId){let s=L();s!==null&&s!=="Disposition"&&(e.remove(),document.getElementById(`${D}-wrap`)?.remove(),ee());return}e&&(e.remove(),document.getElementById(`${D}-wrap`)?.remove());let t=await vr();if(!t){b.warn("Reduce Price: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=await me();if(r!=="Disposition"){b.debug(`[reduce-price] gated off \u2014 pipeline="${r}" (expected Disposition)`);return}let n=ge();if(!n||n.oppId!==o.oppId)return;let a=_r(o.oppId,o.locationId),i=document.createElement("span");i.id=`${D}-wrap`,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),i.appendChild(a),t.appendChild(i),b.debug(`Mounted Reduce Price button on opp ${o.oppId}`),gr(o.oppId)},Z=null,We=null,ee=()=>{Z?.disconnect(),Z=null,We=null},gr=o=>{if(Z&&We===o)return;ee();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(We=o,Z=new MutationObserver(()=>{let t=ge();if(!t||t.oppId!==o){ee();return}let r=L();if(r!==null&&r!=="Disposition"){document.getElementById(D)?.remove(),document.getElementById(`${D}-wrap`)?.remove(),ee();return}document.getElementById(D)||je()}),Z.observe(e,{childList:!0,subtree:!0}))},qe=null,ze,fr=()=>{qe||(qe=new MutationObserver(()=>{ze===void 0&&(ze=window.setTimeout(()=>{ze=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(D);o&&!e&&!ye&&je()},300))}),qe.observe(document.body,{childList:!0,subtree:!0}))},Lt=()=>{fr(),je()};var Dt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/tc/send-to-title-co",U="ws-crm-send-to-title-co-button",W="ws-crm-send-to-title-co-wrap",xe="ws-crm-stc-dropdown",wr=/\/opportunities\/(?:detail\/)?([A-Za-z0-9]{16,})(?:[/?#]|$)/,xr=/\/v2\/location\/([A-Za-z0-9]+)/,Xe=()=>{let o=window.location.pathname,e=o.match(wr);if(!e)return null;let t=o.match(xr);return{oppId:e[1],locationId:t?t[1]:null}},kr=5*60*1e3,Mt=o=>`ws-crm-stc-cache:${o}`,Er=o=>{try{let e=window.localStorage.getItem(Mt(o));if(!e)return null;let t=JSON.parse(e);return!t.ts||Date.now()-t.ts>kr||!Array.isArray(t.checks)||t.checks.length===0?null:t}catch{return null}},Tr=(o,e)=>{try{window.localStorage.setItem(Mt(o),JSON.stringify(e))}catch{}},Sr=async(o,e)=>{try{let t=await fetch(Dt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:o,triggered_by_user_id:A()??null,dry_run:!0}),credentials:"omit"});return t.ok?await t.json():(b.warn(`[stc] dry_run returned HTTP ${t.status}`),null)}catch(t){return b.warn("[stc] dry_run fetch failed:",t),null}},Cr=async(o,e,t)=>{let r=await fetch(Dt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,tc_opp_id:o,triggered_by_user_id:A()??null,confirm_resend:t}),credentials:"omit"});if(!r.ok&&r.status!==200)throw new Error(`HTTP ${r.status}`);return await r.json()},Ar=o=>{let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(!e)return!1;let t=e.querySelectorAll("a, button, li, [role='tab'], [role='menuitem'], .crm-opportunities-modal-content [class*='tab']");for(let n of t)if((n.textContent?.trim()??"")===o)return n.click(),!0;let r=e.querySelectorAll("*");for(let n of r){if(n.children.length>1)continue;if((n.textContent?.trim()??"")===o)return(n.closest("a, button, li, [role='tab']")||n.parentElement||n).click(),!0}return b.warn(`[stc] folder tab not found for "${o}"`),!1},Ft=o=>{let e=document.getElementById(o);if(!e){b.warn(`[stc] field ${o} not found in DOM \u2014 can't scroll to it`);return}e.scrollIntoView({behavior:"smooth",block:"center"});let t=e.style.boxShadow,r=e.style.transition;e.style.transition="box-shadow 0.3s ease, background-color 0.3s ease",e.style.boxShadow=`0 0 0 3px ${l.emerald}, 0 0 18px ${l.emerald}`,window.setTimeout(()=>{e.style.boxShadow=t,window.setTimeout(()=>{e.style.transition=r},400)},2800)},Ir=(o,e)=>{if(e&&Ar(e)){window.setTimeout(()=>Ft(o),300);return}Ft(o)},N=()=>{document.getElementById(xe)?.remove()},fe=(o,e,t)=>{N();let r=document.createElement("div");r.id=xe;let n=t.getBoundingClientRect();r.style.cssText=`
    position: fixed;
    top: ${n.bottom+6}px;
    ...
    padding: 14px 16px;
    font-family: ${x.sans};
  `;let a=o.length,i=o.filter(_=>_.filled).length,s=i===a,d=document.createElement("div");d.innerHTML=`
    <div style="
      font-family: ${x.mono};
      font-size: 10px;
      letter-spacing: 0.14em;
  ...
  `,r.appendChild(d);let c=["TC","ACQ","MT","DISPO","Property"],p={};o.forEach(_=>{p[_.group]||(p[_.group]=[]),p[_.group].push(_)});let m=[...c.filter(_=>p[_]),...Object.keys(p).filter(_=>!c.includes(_))];for(let _ of m){let g=p[_],w=document.createElement("div");w.style.cssText="margin-top: 12px;";let v=document.createElement("div");v.style.cssText=`
      font-family: ${x.mono};
      font-size: 9px;
      ...
      padding-bottom: 4px;
      border-bottom: 1px solid ${l.steel};
    `,v.textContent=_,w.appendChild(v),g.forEach(u=>{let y=document.createElement("div"),k=!u.filled&&u.navigable&&u.field_id;y.style.cssText=`
        display: flex;
        align-items: center;
        gap: 10px;
        padding: 6px 8px;
  ...
  `,P(t,n);let a=new Set,i=async()=>{if(n.status==="sending")return;let v=await Sr(o,e);if(!v){n={status:"error",message:"Check failed \u2014 click to retry"},P(t,n);return}if(v.rate_limited){_=Date.now()+h,n={status:"error",checks:n.checks,message:"Rate-limited by GHL \u2014 will retry in 5m"},P(t,n),b.warn(`[stc] dry_run rate-limited \u2014 cooling down until ${new Date(_).toLocaleTimeString()}`);return}if(!Array.isArray(v.checks)||v.checks.length===0){n={status:"error",message:v.error||"Check failed \u2014 click to retry"},P(t,n),b.warn("[stc] dry_run returned no checks:",v);return}let u=v.pipeline_name;if(u&&u!=="Transaction Coordination"){b.debug(`[stc] dry_run reports pipeline="${u}" (not TC) \u2014 retracting button`),document.getElementById(U)?.remove(),document.getElementById(W)?.remove(),N(),oe();return}let y=v.checks;if(n.checks&&n.checks.length===v.checks.length){let E=new Set;y=v.checks.map((f,I)=>n.checks[I]?.filled&&!f.filled?a.has(f.label)?f:(E.add(f.label),{...f,filled:!0}):f),a=E}else a=new Set;n={status:y.every(E=>E.filled)?"ready":"incomplete",checks:y,previouslySentAt:v.previously_sent_at??null},Tr(o,{checks:y,previously_sent_at:v.previously_sent_at??null,pipeline_name:u,ts:Date.now()}),P(t,n),document.getElementById(xe)&&fe(y,v.previously_sent_at,t)},s=async()=>{let v=!!n.previouslySentAt;n={status:"sending",checks:n.checks,previouslySentAt:n.previouslySentAt},P(t,n),N();try{let u=await Cr(o,e,v);if(u.needs_confirm){n={status:"ready",checks:u.checks,previouslySentAt:u.previously_sent_at},P(t,n),u.checks&&fe(u.checks,u.previously_sent_at,t);return}if(u.ok){n={status:"sent",checks:u.checks,previouslySentAt:new Date().toISOString()},P(t,n),window.setTimeout(()=>{g()},3500);return}n={status:"error",checks:u.checks,message:u.error||"Validation failed"},P(t,n),u.checks&&fe(u.checks,u.previously_sent_at,t)}catch(u){b.error("[stc] send failed:",u),n={status:"error",message:"Network error \u2014 click to retry"},P(t,n)}};t.addEventListener("click",()=>{switch(n.status){case"ready":s();break;case"incomplete":{document.getElementById(xe)?N():n.checks&&fe(n.checks,n.previouslySentAt,t);break}case"error":case"sent":g();break}});let d,c=!1,p=Date.now(),m=1e4,h=5*60*1e3,_=0,g=async()=>{if(!c){if(Date.now()<_){b.debug(`[stc] dry_run suppressed \u2014 rate-limit cooldown until ${new Date(_).toLocaleTimeString()}`);return}c=!0,p=Date.now();try{await i()}finally{c=!1}}};g();let w=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");if(w){let v=new MutationObserver(()=>{if(d!==void 0||c)return;let y=Date.now()-p,k=Math.max(1500,m-y);d=window.setTimeout(()=>{d=void 0,n.status!=="sending"&&g()},k)});v.observe(w,{childList:!0,subtree:!0,characterData:!0}),t.dataset.observerActive="1";let u=window.setInterval(()=>{document.body.contains(t)||(v.disconnect(),d!==void 0&&window.clearTimeout(d),window.clearInterval(u))},5e3)}return t},Fr=[".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading .description",".crm-opportunities-modal .ui-modal-heading .description",".hr-card.hr-modal.crm-opportunities-modal .ui-modal-heading",".crm-opportunities-modal .ui-modal-heading",".hr-card.hr-modal.crm-opportunities-modal .hr-card-header__main",".crm-opportunities-modal .hr-card-header__main"],Dr=async()=>{for(let o of Fr)try{let e=await C(o,{timeoutMs:1500});if(e)return b.debug(`[send-to-title-co] mount target: ${o}`),e}catch{}return b.warn(`[send-to-title-co] no mount target found. URL: ${window.location.pathname}`),null},we=!1,Qe=async()=>{if(we){b.debug("[stc] mount:00-LOCKED ensureMounted already in-flight \u2014 skip");return}we=!0;try{await Mr()}catch(o){b.error("[stc] ensureMountedImpl threw:",o)}finally{we=!1}},Mr=async()=>{b.debug("[stc] mount:01 ensureMountedImpl entered");let o=Xe();if(!o||!o.locationId){b.debug(`[stc] mount:02-BAIL no ctx or no locationId. ctx=${JSON.stringify(o)}`),document.getElementById(U)?.remove(),document.getElementById(W)?.remove(),N();return}b.debug(`[stc] mount:02 ctx ok \u2014 oppId=${o.oppId} locId=${o.locationId}`);let e=document.getElementById(U);if(e&&e.dataset.oppId===o.oppId){b.debug("[stc] mount:03-SKIP existing button for same opp");let s=L();s!==null&&s!=="Transaction Coordination"&&(e.remove(),document.getElementById(W)?.remove(),N(),oe());return}e&&(b.debug("[stc] mount:03 removing stale existing button"),e.remove(),document.getElementById(W)?.remove(),N()),b.debug("[stc] mount:04 calling findMountTarget");let t=await Dr();if(!t){b.debug("[stc] mount:04-BAIL findMountTarget returned null");return}b.debug(`[stc] mount:04 target found tag=${t.tagName} class="${t.className}"`);let r=L();if(r!==null&&r!=="Transaction Coordination"){b.debug(`[stc] mount:05-GATED DOM pipeline="${r}" \u2014 skipping mount`);return}let n=Xe();if(!n||n.oppId!==o.oppId||!n.locationId){b.debug(`[stc] mount:06-BAIL ctx changed during awaits. was=${o.oppId} now=${n?.oppId}`);return}b.debug("[stc] mount:06 building button (pipeline gate will run via dry_run)");let a=Lr(o.oppId,n.locationId),i=document.createElement("span");i.id=W,i.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: auto;
    flex-shrink: 0;
    vertical-align: middle;
  `,t.classList.contains("description")&&(t.style.display="flex",t.style.alignItems="center",t.style.gap="16px",t.style.flexWrap="wrap"),i.appendChild(a),t.appendChild(i),b.debug(`[stc] mount:07-OK Mounted on opp ${o.oppId}, inDoc=${document.body.contains(a)}`),Hr(o.oppId)},te=null,Je=null,oe=()=>{te?.disconnect(),te=null,Je=null},Hr=o=>{if(te&&Je===o)return;oe();let e=document.querySelector(".hr-card.hr-modal.crm-opportunities-modal");e&&(Je=o,te=new MutationObserver(()=>{let t=Xe();if(!t||t.oppId!==o){oe();return}let r=L();if(r!==null&&r!=="Transaction Coordination"){document.getElementById(U)?.remove(),document.getElementById(W)?.remove(),N(),oe();return}document.getElementById(U)||Qe()}),te.observe(e,{childList:!0,subtree:!0}))},Ke=null,Ye,Pr=()=>{Ke||(Ke=new MutationObserver(()=>{Ye===void 0&&(Ye=window.setTimeout(()=>{Ye=void 0;let o=!!document.querySelector(".crm-opportunities-modal .ui-modal-heading .description"),e=!!document.getElementById(U);o&&!e&&!we&&Qe()},300))}),Ke.observe(document.body,{childList:!0,subtree:!0}))},Ht=()=>{Pr(),Qe()};var Ae="reos-theme-stylesheet",Qt="reos-theme-fonts",Nr=()=>{let o=document.getElementById(Qt);o&&o.remove()},Or=`
/* ============================================
