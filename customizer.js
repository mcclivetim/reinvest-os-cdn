...
`,P=`
  ...
  flex-shrink: 0;
`;var it=/^[A-Za-z0-9]{15,30}$/,R=o=>typeof o=="string"&&it.test(o),lt=o=>{try{let e=o.split(".");if(e.length!==3)return null;let t=e[1].replace(/-/g,"+").replace(/_/g,"/"),r=t+"=".repeat((4-t.length%4)%4);return JSON.parse(atob(r))}catch{return null}},dt=()=>{let o=window.WS_CRM_USER_ID;return R(o)?o:null},ct=()=>{let o=window,e=[o.appState?.user?.id,o.user?.id,o.currentUser?.id,o.LCUser?.id,o.HL?.user?.id];for(let t of e)if(R(t))return t;return null},bt=()=>{try{let o=localStorage.getItem("refreshedToken");if(!o)return null;let e=o.startsWith('"')?o.slice(1,-1):o,t=lt(e);if(!t)return null;let r=t.uid;if(R(r))return r;let n=t.claims;if(n&&R(n.user_id))return n.user_id}catch{}return null},ht=()=>{try{let o=["user","tokenUser","currentUser","ghl_user"];for(let e of o){let t=localStorage.getItem(e);if(t)try{let r=JSON.parse(t),n=r.id??r._id;if(R(n))return n}catch{}}}catch{}return null},F=()=>dt()||ct()||bt()||ht()||null;var ke="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact",pt=`${ke}/active-follow-up-get`,mt=`${ke}/active-follow-up-set`,O="ws-crm-active-follow-up-button",pe={amber:{label:"Add to Active Follow Up",bg:l.amberGlow,border:"rgba(232, 163, 60, 0.45)",color:l.amber,clickable:!0,dotColor:l.amber},blue:{label:"In Active Follow Up",bg:l.blueGlow,border:"rgba(75, 139, 245, 0.45)",color:l.blue,clickable:!1,dotColor:l.blue},gray:{label:"Not in Follow Up",bg:l.slate,border:l.steel,color:l.coolGray,clickable:!1,dotColor:l.coolGray},loading:{label:"Active Follow Up\u2026",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.ash},error:{label:"Follow Up status unavailable",bg:l.slate,border:l.steel,color:l.ash,clickable:!1,dotColor:l.crimson}},_t=()=>{let o=x();return!o||!o.locationId?null:{contactId:o.contactId,tenantId:o.locationId}},vt=async o=>{let e=await fetch(pt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId}),credentials:"omit"});if(!e.ok)throw new Error(`Get webhook returned ${e.status} ${e.statusText}`);return await e.json()},ut=async(o,e)=>{let t=await fetch(mt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.tenantId,contact_id:o.contactId,triggered_by_user_id:e??null}),credentials:"omit"});if(!t.ok)throw new Error(`Set webhook returned ${t.status} ${t.statusText}`);return await t.json()},D=(o,e)=>{let t=pe[e];o.dataset.state=e,o.disabled=!t.clickable,o.style.cursor=t.clickable?"pointer":"default",o.style.background=t.bg,o.style.borderColor=t.border,o.style.color=t.color;let r=o.querySelector(".ws-afu-dot");r&&(r.style.background=t.dotColor);let n=o.querySelector(".ws-afu-label");n&&(n.textContent=t.label)},fe=o=>{let e=document.createElement("button");return e.type="button",e.id=O,e.dataset.contactId=o,e.style.cssText=`
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 6px 12px;
  ...
  `,D(e,"loading"),e.addEventListener("click",async()=>{if(e.dataset.state!=="amber")return;let r=_t();if(!(!r||r.contactId!==o)){D(e,"blue"),e.style.transform="scale(0.97)",setTimeout(()=>e.style.transform="",120);try{let n=await ut(r,F()),a=n&&n.state&&n.state in pe?n.state:"blue";D(e,a),h.info(`Active Follow Up set for ${o} \u2192 ${a}`)}catch(n){h.error("Active Follow Up Set webhook failed",n),D(e,"error"),setTimeout(()=>void Y(e,r),4e3)}}}),e},Y=async(o,e)=>{D(o,"loading");try{let t=await vt(e),r=t&&t.state&&t.state in pe?t.state:"gray",n=document.getElementById(O);if(!n||n.dataset.contactId!==e.contactId)return;D(n,r)}catch(t){h.warn("Active Follow Up Get webhook failed",t),D(o,"error")}},yt=[".central-panel","[class*='central-panel']"],we="ws-crm-contact-actions-bar",gt=async()=>{for(let o of yt)try{let e=await S(o,{timeoutMs:3e3});if(e)return e}catch{}try{return(await S("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},ft=(o,e)=>{let t=e.querySelector(`#${we}`);if(t)return t.appendChild(o),{container:t,placement:"actions-bar"};let r="ws-crm-afu-wrap",n=e.querySelector(`#${r}`);if(!n){n=document.createElement("div"),n.id=r,n.style.cssText=`
      display: flex;
      align-items: center;
      gap: 8px;
      padding: 6px 12px;
      ...
      flex-shrink: 0;
    `;let a=e.querySelector(`#${we}`);a&&a.parentElement===e?e.insertBefore(n,a.nextSibling):e.insertBefore(n,e.firstChild)}return n.appendChild(o),{container:n,placement:"fallback"}},wt=()=>{let o=document.querySelector("[data-testid='CENTRALPANEL_NAME']");if(!o)return null;let e=o;for(let t=0;t<8&&!(!e||!e.parentElement);t++){let r=e.parentElement,n=(r.className||"").toString();if(n.includes("justify-between")&&n.includes("border-b")){let a=r.children[1];return a instanceof HTMLElement?a:r}e=r}return null},xt=(o,e)=>{o.style.marginRight="4px",e.insertBefore(o,e.firstChild)},kt=async o=>{if(o==="url"){let t=await gt();return t?{kind:"contact-detail",target:t}:null}let e=wt();return e?{kind:"conversations",target:e}:null},xe=(o,e)=>e.kind==="contact-detail"?ft(o,e.target).placement:(xt(o,e.target),"conversations-header"),Et=async()=>{let o=x();if(!o||!o.locationId){document.getElementById(O)?.remove(),document.getElementById("ws-crm-afu-wrap")?.remove();return}let e={contactId:o.contactId,tenantId:o.locationId},t=document.getElementById(O);if(t&&t.dataset.contactId===e.contactId){Y(t,e);return}t&&t.remove();let r=await kt(o.source);if(!r){h.warn(`Active Follow Up: no mount target on ${o.source}-source page`);return}let n=x();if(!n||n.contactId!==e.contactId)return;let a=fe(e.contactId),i=xe(a,r);h.debug(`Active Follow Up mounted (${r.kind}/${i}) for ${e.contactId}`),Y(a,e);let s=new MutationObserver(()=>{let d=document.getElementById(O);if(!(x()?.contactId===e.contactId)){s.disconnect();return}if(!d){let p=fe(e.contactId);xe(p,r),Y(p,e),h.debug(`Active Follow Up re-mounted after wipe for ${e.contactId}`)}});s.observe(r.target,{childList:!0})},Ee=()=>{Et()};var I="ws-crm-claim-release-pill",Tt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-get",St="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/claim-release-set",Ct=async o=>{let e=await fetch(Tt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,contact_id:o.contactId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Get returned ${e.status} ${e.statusText}`);return await e.json()},At=async o=>{let e=await fetch(St,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,contact_id:o.contactId,triggered_by_user_id:o.userId,action:o.action}),credentials:"omit"});if(!e.ok)throw new Error(`Claim/Release Set returned ${e.status} ${e.statusText}`);return await e.json()},Ce=o=>{let e=Array.from(o.children).find(r=>{if(!(r instanceof HTMLElement))return!1;let n=r.className?.toString()??"";return n.includes("flex")&&n.includes("flex-col")});return e?e.querySelector(".flex.items-center, [class~='flex'][class~='items-center']")??null:null},M=(o,e)=>{let t=e==="claim"?"emerald":"amber",r=e==="claim"?l.emerald:l.amber;o.dataset.mode=e,o.style.cssText=K(t),o.title=e==="claim"?"Take ownership of this lead":"Return this lead to the pool",o.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `},Ft=({contactId:o,mode:e})=>{let t=document.createElement("button");return t.id=I,t.type="button",t.dataset.contactId=o,M(t,e),t.addEventListener("mouseenter",()=>{t.style.transform="translateY(-1px)"}),t.addEventListener("mouseleave",()=>{t.style.transform="translateY(0)"}),t.addEventListener("click",async()=>{let r=t.dataset.mode??"claim",n=x();if(!n||!n.locationId||n.contactId!==o)return;let a=F();if(!a){h.warn("Claim/Release click: no user ID detected (login session not exposing it)");let s=t.querySelector(".ws-cr-label");s&&(s.textContent="Sign-in needed"),setTimeout(()=>M(t,r),2500);return}h.info(`Claim/Release click \u2014 contact=${o} mode=${r} user=${a}`);let i=r==="claim"?"release":"claim";M(t,i),t.disabled=!0,t.style.opacity="0.8";try{let s=await At({contactId:o,locationId:n.locationId,userId:a,action:r}),d=s?.mode==="claim"||s?.mode==="release"?s.mode:i;M(t,d),h.info(`Claim/Release done \u2192 new mode: ${d}`)}catch(s){h.error("Claim/Release Set webhook failed",s),M(t,r);let d=t.querySelector(".ws-cr-label");if(d){let b=d.textContent??"";d.textContent="Try again",setTimeout(()=>{d.textContent==="Try again"&&(d.textContent=b)},2500)}}finally{t.disabled=!1,t.style.opacity="1"}}),t},Te=async(o,e)=>{try{let t=await Ct({contactId:e.contactId,locationId:e.locationId,userId:F()}),r=document.getElementById(I);if(!r||r.dataset.contactId!==e.contactId)return;let n=t?.mode==="release"?"release":"claim";M(r,n)}catch(t){h.warn("Claim/Release Get failed; pill stays in default 'claim' mode",t)}},It=async(o,e=1e4)=>{let t=Date.now();for(;Date.now()-t<e;){let r=Ce(o);if(r)return r;await new Promise(n=>setTimeout(n,150))}return null},Se=(o,e)=>{let t=Ft({contactId:e,mode:"claim"}),r=document.createElement("span");r.id=`${I}-wrap`,r.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,r.appendChild(t),o.appendChild(r)},Lt=async()=>{let o=x();if(!o){document.getElementById(I)?.remove();return}let e=document.getElementById(I);if(!(e&&e.dataset.contactId===o.contactId)){e&&e.remove();try{let t=await S("#record-details-lhs",{timeoutMs:1e4}),r=await It(t),n=x();if(!n||n.contactId!==o.contactId)return;if(!r){h.warn("Claim/Release pill \u2014 name row not found within timeout; skipping mount");return}Se(r,o.contactId),h.debug(`Mounted Claim/Release pill next to name for ${o.contactId}`);let a=document.getElementById(I);a&&o.locationId&&Te(a,{contactId:o.contactId,locationId:o.locationId});let i=null;i=new MutationObserver(async()=>{if(!(x()?.contactId===o.contactId)){i?.disconnect();return}if(document.getElementById(I))return;let d=Ce(t);if(d){Se(d,o.contactId);let b=document.getElementById(I);b&&o.locationId&&Te(b,{contactId:o.contactId,locationId:o.locationId}),h.debug("Re-mounted Claim/Release pill after React wipe")}}),i.observe(t,{childList:!0,subtree:!0})}catch(t){h.warn("Could not mount Claim/Release pill:",t)}}},Ae=()=>{Lt()};var Dt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/call-status-set";var Ht="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/contact/appointment-status",Mt=async({contactId:o,locationId:e,status:t,userId:r,deadReason:n})=>{let a={call_status:t,triggered_by_user_id:r??""};n&&(a.dead_reason=n);let s=await fetch(Dt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contact_id:o,location:{id:e},customData:a}),credentials:"omit"});if(!s.ok)throw new Error(`Webhook returned ${s.status} ${s.statusText}`)},Nt=async({contactId:o,tenantId:e})=>{let t=await fetch(Ht,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:e,contact_id:o}),credentials:"omit"});if(!t.ok)throw new Error(`Sanity check returned ${t.status} ${t.statusText}`);return await t.json()};var X="ws-crm-contact-actions-bar",Pt="ws-crm-call-status-dropdown",Fe="ws-crm-offer-made-modal",Rt=[{value:"not-contacted",label:"Not Contacted",color:"purple"},{value:"needs-offer",label:"Needs Offer",color:"green"},{value:"appointment-booked",label:"Appointment Booked",color:"green"},{value:"negotiating",label:"Negotiating",color:"green"},{value:"under-contract",label:"Under Contract",color:"green"},{value:"offer-rejected",label:"Offer Rejected",color:"orange"},{value:"not-ready",label:"Not Ready",color:"orange"},{value:"wants-retail",label:"Wants Retail",color:"orange"},{value:"already-listed",label:"Already Listed",color:"orange"},{value:"working-with-a-buyer",label:"Working with a Buyer",color:"orange"},{value:"not-interested",label:"Not Interested",color:"orange"},{value:"sold-on-market",label:"Sold on Market",color:"red"},{value:"sold-off-market",label:"Sold off Market",color:"red"},{value:"wrong-number",label:"Wrong Number",color:"red"},{value:"dead-deal",label:"Dead Deal",color:"red"}],Ot=[".central-panel","[class*='central-panel']"],Bt=async()=>{for(let o of Ot)try{let e=await S(o,{timeoutMs:3e3});if(e)return e}catch{}try{return(await S("#contact-conversation-panel",{timeoutMs:3e3})).parentElement??null}catch{return null}},Gt=o=>{let e=document.createElement("div");e.id=Pt,e.style.cssText="position: relative; flex-shrink: 0;";let t=document.createElement("button");t.type="button",t.style.cssText=P,t.innerHTML=`
    <span style="
      display: inline-flex;
      width: 8px;
      height: 8px;
  ...
  `;let r=document.createElement("div");return r.style.cssText=`
    ...
    padding: 4px;
    display: none;
  `,Rt.forEach(n=>{let a=document.createElement("button");a.type="button",a.style.cssText=`
      display: flex;
      align-items: center;
      gap: 10px;
      width: 100%;
    ...
    `,a.innerHTML=`
      <span style="
        ...
        height: 8px;
        border-radius: 50%;
        background: ${he[n.color]};
        flex-shrink: 0;
      "></span>
      <span>${n.label}</span>
    `,a.addEventListener("mouseenter",()=>{a.style.background=l.slate}),a.addEventListener("mouseleave",()=>{a.style.background="transparent"}),a.addEventListener("click",async()=>{h.info(`Call Status set to "${n.label}" (${n.value}, ${n.color}) for contact ${o}`);let i=t.firstElementChild,s=t.children[1],d=t.children[2];i&&(i.style.background=he[n.color]),s&&(s.textContent=n.label),r.style.display="none";let p=x()?.locationId,m=F();if(!p){h.warn("Cannot fire Call Status webhook \u2014 locationId missing from URL context"),s&&(s.textContent=`${n.label} (no tenant!)`),s&&(s.style.color=l.amber);return}if(n.color==="green"&&!m){h.warn("Green Call Status clicked but no user ID found \u2014 n8n Green handler will throw. Set window.WS_CRM_USER_ID = '<your-ghl-user-id>' as a temporary workaround."),s&&(s.textContent=`${n.label} (no user!)`),s&&(s.style.color=l.amber);return}let c=d?.textContent??"\u25BE",g=async y=>{d&&(d.textContent="\u2026"),t.style.opacity="0.75";try{await Mt({contactId:o,locationId:p,status:n.label,userId:m,deadReason:y}),d&&(d.textContent="\u2713"),setTimeout(()=>{d&&(d.textContent=c),t.style.opacity="1"},900)}catch(u){h.warn("Call Status webhook failed:",u),d&&(d.textContent="\u2717"),s&&(s.style.color=l.amber),t.style.opacity="1",setTimeout(()=>{d&&(d.textContent=c),s&&(s.style.color=l.bone)},1800)}};if(n.value==="dead-deal"){let y=s?.textContent??"";Ut(u=>{g(u)},()=>{s&&(s.textContent=y)});return}if(n.value==="appointment-booked"){let y=s?.textContent??"";try{(await Nt({contactId:o,tenantId:p})).ok?await g():$t(()=>{g()},()=>{s&&(s.textContent=y)})}catch(u){h.warn("Appointment sanity check failed; firing anyway:",u),await g()}return}await g()}),r.appendChild(a)}),t.addEventListener("click",n=>{n.stopPropagation(),r.style.display=r.style.display==="none"?"block":"none"}),document.addEventListener("click",n=>{e.contains(n.target)||(r.style.display="none")}),e.appendChild(t),e.appendChild(r),e},Vt=o=>{document.getElementById(Fe)?.remove();let e=document.createElement("div");e.id=Fe,e.style.cssText=`
    position: fixed;
    inset: 0;
    z-index: 999999;
  ...
  `,t.appendChild(r),document.body.appendChild(t);let n=()=>t.remove(),a=()=>{n(),e()};t.addEventListener("click",s=>{s.target===t&&a()}),r.querySelector("#ws-appt-sanity-cancel")?.addEventListener("click",a),r.querySelector("#ws-appt-sanity-mark")?.addEventListener("click",()=>{n(),o()});let i=s=>{s.key==="Escape"&&(document.removeEventListener("keydown",i),a())};document.addEventListener("keydown",i)},De=o=>{let e=document.createElement("div");e.id=X,e.dataset.contactId=o,e.style.cssText=`
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 8px 12px;
    ...
    border-bottom: 1px solid ${l.steel};
    flex-shrink: 0;
  `;let t=document.createElement("button");return t.type="button",t.style.cssText=N,t.textContent="Offer Made",t.addEventListener("click",()=>Vt(o)),e.appendChild(t),e.appendChild(Gt(o)),e},qt=async()=>{let o=x();if(!o){document.getElementById(X)?.remove();return}let e=document.getElementById(X);if(e&&e.dataset.contactId===o.contactId)return;e&&e.remove();let t=await Bt();if(!t){h.warn("Could not mount actions bar \u2014 central panel not found within timeout");return}let r=x();if(!r||r.contactId!==o.contactId)return;let n=De(o.contactId);n.dataset.mountTarget=t.className||"central-panel",t.insertBefore(n,t.firstChild),h.debug(`Mounted contact actions bar for ${o.contactId}`);let a=null;a=new MutationObserver(()=>{let i=document.getElementById(X);if(!(x()?.contactId===o.contactId)){a?.disconnect();return}if(!i){let d=De(o.contactId);d.dataset.mountTarget=n.dataset.mountTarget??"",t.insertBefore(d,t.firstChild),h.debug(`Re-mounted actions bar after React wipe for ${o.contactId}`)}}),a.observe(t,{childList:!0})},He=()=>{qt()};var J="https://assets.cdn.filesafe.space/ZqGLfSMNKm26UQP7ENj1/media/6a08c6430a69f1e766a71d54.png",zt=new Set(["icon","shortcut icon","apple-touch-icon","apple-touch-icon-precomposed","mask-icon"]),B="data-ws-crm-favicon",me=o=>o.hasAttribute(B),Me=o=>{if(o.tagName!=="LINK")return!1;let e=(o.getAttribute("rel")??"").toLowerCase();return zt.has(e)},Q=()=>{document.head.querySelectorAll("link[rel]").forEach(r=>{Me(r)&&!me(r)&&r.remove()});let e=document.head.querySelector(`link[${B}="1"]`);e?e.href!==J&&(e.href=J):(e=document.createElement("link"),e.setAttribute(B,"1"),e.rel="icon",e.type="image/png",e.href=J,document.head.appendChild(e),h.debug("Favicon installed"));let t=document.head.querySelector(`link[${B}="apple"]`);t||(t=document.createElement("link"),t.setAttribute(B,"apple"),t.rel="apple-touch-icon",t.href=J,document.head.appendChild(t))},Ne=()=>{if(window.__reosFaviconInstalled)return;if(window.__reosFaviconInstalled=!0,document.head)Q();else{let r=window.setInterval(()=>{document.head&&(window.clearInterval(r),Q())},50)}new MutationObserver(r=>{let n=!1;for(let a of r)a.addedNodes.forEach(i=>{i instanceof HTMLLinkElement&&Me(i)&&!me(i)&&(n=!0)}),a.removedNodes.forEach(i=>{i instanceof HTMLLinkElement&&me(i)&&(n=!0)});n&&Q()}).observe(document.head,{childList:!0,subtree:!1});let e=0,t=window.setInterval(()=>{e+=1,Q(),e>=5&&window.clearInterval(t)},1e3)};var Wt="https://n8n.srv942822.hstgr.cloud/webhook/reinvest-os/mt/not-interested",G="ws-crm-not-interested-button",jt=/\/opportunities\/detail\/([A-Za-z0-9]+)/,Kt=/\/v2\/location\/([A-Za-z0-9]+)/,Z=()=>{let o=window.location.pathname,e=o.match(jt);if(!e)return null;let t=o.match(Kt);return{oppId:e[1],locationId:t?t[1]:null}},Yt=async o=>{let e=await fetch(Wt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tenant_id:o.locationId,mt_opp_id:o.oppId,triggered_by_user_id:o.userId??null}),credentials:"omit"});if(!e.ok)throw new Error(`Not Interested webhook returned ${e.status} ${e.statusText}`);return await e.json()},Pe=o=>{let e=document.createElement("button");return e.id=G,e.type="button",e.dataset.oppId=o,e.style.cssText=K("neutral"),e.style.fontFamily=v.sans,e.style.fontSize="12px",e.title="Close this Marketing Tracker opp as Not Interested for THIS property only",e.innerHTML=`
    <span style="
      width: 6px;
      height: 6px;
      border-radius: 50%;
  ...
  `,e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-1px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"}),e.addEventListener("click",async()=>{let t=Z();if(!t||!t.locationId||t.oppId!==o)return;let r=F(),n=e.querySelector(".ws-ni-label");e.disabled=!0,e.style.opacity="0.7",n&&(n.textContent="Closing\u2026");try{let a=await Yt({oppId:t.oppId,locationId:t.locationId,userId:r});if(a?.ok)n&&(n.textContent="Marked Not Interested"),e.style.background=l.crimsonGlow,e.style.color=l.crimson,h.info(`Not Interested set for opp ${o} \u2192 ${a.new_stage}`);else throw new Error("Webhook returned ok=false")}catch(a){h.error("Not Interested webhook failed",a),n&&(n.textContent="Try again"),e.disabled=!1,e.style.opacity="1",setTimeout(()=>{n&&n.textContent==="Try again"&&(n.textContent="Mark Not Interested")},3e3)}}),e},Xt=["[data-testid='OPPORTUNITY_HEADER']","[class*='opportunity-header']","[class*='opp-header']","header.flex.items-center"],Jt=async()=>{for(let o of Xt)try{let e=await S(o,{timeoutMs:2e3});if(e)return e}catch{}return null},Qt=async()=>{let o=Z();if(!o||!o.locationId){document.getElementById(G)?.remove();return}let e=document.getElementById(G);if(e&&e.dataset.oppId===o.oppId)return;e&&e.remove();let t=await Jt();if(!t){h.warn("Not Interested: no opp-header mount target found within timeout \u2014 skipping mount");return}let r=Z();if(!r||r.oppId!==o.oppId)return;let n=Pe(o.oppId),a=document.createElement("span");a.id=`${G}-wrap`,a.style.cssText=`
    display: inline-flex;
    align-items: center;
    margin-left: 12px;
    vertical-align: middle;
  `,a.appendChild(n),t.appendChild(a),h.debug(`Mounted Not Interested button on opp ${o.oppId}`);let i=new MutationObserver(()=>{let s=document.getElementById(G);if(!(Z()?.oppId===o.oppId)){i.disconnect();return}if(!s){let b=Pe(o.oppId);a.appendChild(b),h.debug(`Re-mounted Not Interested button after wipe for opp ${o.oppId}`)}});i.observe(t,{childList:!0})},Re=()=>{Qt()};var ae="reos-theme-stylesheet",Ze="reos-theme-fonts",Zt=()=>{let o=document.getElementById(Ze);o&&o.remove()},eo=`
/* ============================================
...
Click to dismiss for this session`,o.addEventListener("mouseenter",()=>{o.style.transform="scale(1.05)"}),o.addEventListener("mouseleave",()=>{o.style.transform="scale(1)"}),o.addEventListener("click",()=>{o.style.opacity="0",setTimeout(()=>o.remove(),200)}),document.body.appendChild(o)};console.log("[ws-crm-customizer] bundle-loaded");var ot=()=>{h.info(`Loaded v${W} (built ${j})`);try{ie()}catch(o){h.warn("mountVersionBadge failed:",o)}Ne(),et(),ge(),ye(o=>{h.debug("Page handler firing for:",o),ie(),Ae(),He(),Ee(),Re()})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ot):ot();})();
